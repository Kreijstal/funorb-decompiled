/*
 * Decompiled by CFR-JS 0.4.0.
 */
class id {
    private od field_h;
    static int field_m;
    private boolean field_n;
    static boolean field_o;
    private static int field_u;
    private int field_s;
    private static rp field_q;
    private long field_k;
    int[] field_d;
    private int field_j;
    private int field_b;
    private int field_i;
    private int field_f;
    private long field_t;
    private long field_g;
    private od[] field_l;
    private boolean field_e;
    private int field_r;
    private int field_p;
    private int field_c;
    private od[] field_a;

    int h() throws Exception {
        return this.field_f;
    }

    void f() {
    }

    final synchronized void c() {
        try {
            int decompiledRegionSelector0 = 0;
            Throwable decompiledCaughtException = null;
            long var1 = 0L;
            Exception var3 = null;
            int var3_int = 0;
            int var4 = 0;
            if (!this.field_n) {
              var1 = lo.a((byte) -46);
              try {
                L0: {
                  L1: {
                    if (var1 <= this.field_k + 6000L) {
                      break L1;
                    } else {
                      this.field_k = var1 - 6000L;
                      break L1;
                    }
                  }
                  L2: while (true) {
                    if (var1 <= this.field_k + 5000L) {
                      break L0;
                    } else {
                      this.a(256);
                      this.field_k = this.field_k + (long)(256000 / field_m);
                      var1 = lo.a((byte) 119);
                      continue L2;
                    }
                  }
                }
              } catch (java.lang.Exception decompiledCaughtParameter0) {
                decompiledCaughtException = decompiledCaughtParameter0;
                L3: {
                  var3 = (Exception) (Object) decompiledCaughtException;
                  this.field_k = var1;
                  break L3;
                }
              }
              if (this.field_d != null) {
                try {
                  L4: {
                    L5: {
                      if (this.field_g == 0L) {
                        break L5;
                      } else {
                        if (var1 >= this.field_g) {
                          this.b(this.field_f);
                          this.field_g = 0L;
                          this.field_e = true;
                          break L5;
                        } else {
                          decompiledRegionSelector0 = 0;
                          break L4;
                        }
                      }
                    }
                    L6: {
                      var3_int = this.h();
                      if (this.field_j - var3_int <= this.field_r) {
                        break L6;
                      } else {
                        this.field_r = this.field_j - var3_int;
                        break L6;
                      }
                    }
                    L7: {
                      var4 = this.field_i + this.field_b;
                      if (var4 + 256 <= 16384) {
                        break L7;
                      } else {
                        var4 = 16128;
                        break L7;
                      }
                    }
                    L8: {
                      if (var4 + 256 <= this.field_f) {
                        break L8;
                      } else {
                        L9: {
                          this.field_f = this.field_f + 1024;
                          if (this.field_f <= 16384) {
                            break L9;
                          } else {
                            this.field_f = 16384;
                            break L9;
                          }
                        }
                        this.f();
                        this.b(this.field_f);
                        var3_int = 0;
                        this.field_e = true;
                        if (var4 + 256 <= this.field_f) {
                          break L8;
                        } else {
                          var4 = this.field_f - 256;
                          this.field_b = var4 - this.field_i;
                          break L8;
                        }
                      }
                    }
                    L10: while (true) {
                      if (var3_int >= var4) {
                        L11: {
                          if (var1 <= this.field_t) {
                            break L11;
                          } else {
                            L12: {
                              if (this.field_e) {
                                this.field_e = false;
                                break L12;
                              } else {
                                L13: {
                                  if (this.field_r != 0) {
                                    break L13;
                                  } else {
                                    if (this.field_c != 0) {
                                      break L13;
                                    } else {
                                      this.f();
                                      this.field_g = var1 + 2000L;
                                      decompiledRegionSelector0 = 1;
                                      break L4;
                                    }
                                  }
                                }
                                this.field_b = Math.min(this.field_c, this.field_r);
                                this.field_c = this.field_r;
                                break L12;
                              }
                            }
                            this.field_r = 0;
                            this.field_t = var1 + 2000L;
                            break L11;
                          }
                        }
                        this.field_j = var3_int;
                        decompiledRegionSelector0 = 2;
                        break L4;
                      } else {
                        this.a(this.field_d, 256);
                        this.e();
                        var3_int += 256;
                        continue L10;
                      }
                    }
                  }
                } catch (java.lang.Exception decompiledCaughtParameter1) {
                  decompiledCaughtException = decompiledCaughtParameter1;
                  L14: {
                    var3 = (Exception) (Object) decompiledCaughtException;
                    this.f();
                    this.field_g = var1 + 2000L;
                    decompiledRegionSelector0 = 2;
                    break L14;
                  }
                }
                if (decompiledRegionSelector0 == 0) {
                  return;
                } else {
                  if (decompiledRegionSelector0 == 1) {
                    return;
                  } else {
                    return;
                  }
                }
              } else {
                return;
              }
            } else {
              return;
            }
        } catch (RuntimeException | Error decompiledUncheckedException) {
            throw decompiledUncheckedException;
        } catch (Throwable decompiledCheckedException) {
            throw new RuntimeException(decompiledCheckedException);
        }
    }

    final synchronized void d() {
        int var1;
        int var2;
        L0: {
          if (field_q == null) {
            break L0;
          } else {
            var1 = 1;
            var2 = 0;
            L1: while (true) {
              if (var2 >= 2) {
                if (var1 == 0) {
                  break L0;
                } else {
                  field_q.field_d = true;
                  L2: while (true) {
                    if (!field_q.field_b) {
                      field_q = null;
                      break L0;
                    } else {
                      fh.a(true, 50L);
                      continue L2;
                    }
                  }
                }
              } else {
                L3: {
                  if (field_q.field_c[var2] != this) {
                    break L3;
                  } else {
                    field_q.field_c[var2] = null;
                    break L3;
                  }
                }
                if (field_q.field_c[var2] != null) {
                  var1 = 0;
                  var2++;
                  continue L1;
                } else {
                  var2++;
                  continue L1;
                }
              }
            }
          }
        }
        this.f();
        this.field_d = null;
        this.field_n = true;
    }

    private final static void a(od param0) {
        param0.field_f = false;
        if (param0.field_h != null) {
            param0.field_h.field_f = 0;
        }
        od var1 = param0.d();
        while (var1 != null) {
            id.a(var1);
            var1 = param0.a();
        }
    }

    final synchronized void a() {
        try {
            this.field_e = true;
            try {
                this.g();
            } catch (Exception exception) {
                this.f();
                this.field_g = lo.a((byte) -29) + 2000L;
            }
        } catch (RuntimeException | Error decompiledUncheckedException) {
            throw decompiledUncheckedException;
        } catch (Throwable decompiledCheckedException) {
            throw new RuntimeException(decompiledCheckedException);
        }
    }

    void a(java.awt.Component param0) throws Exception {
    }

    void e() throws Exception {
    }

    public static void b() {
        field_q = null;
    }

    private final void a(od param0, int param1) {
        int var3 = param1 >> 5;
        od var4 = this.field_a[var3];
        if (var4 == null) {
            this.field_l[var3] = param0;
        } else {
            var4.field_i = param0;
        }
        this.field_a[var3] = param0;
        param0.field_g = param1;
    }

    final static void a(int param0, boolean param1, int param2) {
        if (param0 < 8000 || param0 > 48000) {
            throw new IllegalArgumentException();
        }
        field_m = param0;
        field_o = param1 ? true : false;
        field_u = param2;
    }

    void b(int param0) throws Exception {
    }

    final synchronized void b(od param0) {
        this.field_h = param0;
    }

    private final void a(int param0) {
        this.field_p = this.field_p - param0;
        if (this.field_p < 0) {
            this.field_p = 0;
        }
        if (this.field_h != null) {
            this.field_h.a(param0);
            return;
        }
    }

    void g() throws Exception {
    }

    private final void a(int[] param0, int param1) {
        int var3;
        int var4;
        int var5;
        int var6;
        int var7_int;
        Object var7;
        int var8_int;
        od[] var8;
        int var9;
        Object var10;
        od var11;
        cg var12;
        int var13;
        od var14;
        int var15_int;
        od var15;
        L0: {
          var3 = param1;
          if (!field_o) {
            break L0;
          } else {
            var3 = var3 << 1;
            break L0;
          }
        }
        L1: {
          eg.a(param0, 0, var3);
          this.field_p = this.field_p - param1;
          if (this.field_h == null) {
            break L1;
          } else {
            if (this.field_p > 0) {
              break L1;
            } else {
              this.field_p = this.field_p + (field_m >> 4);
              id.a(this.field_h);
              this.a(this.field_h, this.field_h.b());
              var4 = 0;
              var5 = 255;
              var6 = 7;
              L2: while (true) {
                L3: {
                  if (var5 == 0) {
                    break L3;
                  } else {
                    L4: {
                      if (var6 >= 0) {
                        var7_int = var6;
                        var8_int = 0;
                        break L4;
                      } else {
                        var7_int = var6 & 3;
                        var8_int = -(var6 >> 2);
                        break L4;
                      }
                    }
                    var9 = var5 >>> var7_int & 286331153;
                    L5: while (true) {
                      if (var9 != 0) {
                        L6: {
                          if ((var9 & 1) != 0) {
                            var5 = var5 & (1 << var7_int ^ -1);
                            var10 = null;
                            var11 = this.field_l[var7_int];
                            var14 = var11;
                            var14 = var11;
                            L7: while (true) {
                              if (var11 == null) {
                                break L6;
                              } else {
                                L8: {
                                  var12 = var11.field_h;
                                  if (var12 == null) {
                                    break L8;
                                  } else {
                                    if (var12.field_f <= var8_int) {
                                      break L8;
                                    } else {
                                      var5 = var5 | 1 << var7_int;
                                      var10 = var11;
                                      var11 = var11.field_i;
                                      continue L7;
                                    }
                                  }
                                }
                                L9: {
                                  var11.field_f = true;
                                  var13 = var11.c();
                                  var4 = var4 + var13;
                                  if (var12 == null) {
                                    break L9;
                                  } else {
                                    var12.field_f = var12.field_f + var13;
                                    break L9;
                                  }
                                }
                                if (var4 < this.field_s) {
                                  L10: {
                                    var14 = var11.d();
                                    if (var14 == null) {
                                      break L10;
                                    } else {
                                      var15_int = var11.field_g;
                                      L11: while (true) {
                                        if (var14 == null) {
                                          break L10;
                                        } else {
                                          this.a(var14, var15_int * var14.b() >> 8);
                                          var14 = var11.a();
                                          continue L11;
                                        }
                                      }
                                    }
                                  }
                                  L12: {
                                    var15 = var11.field_i;
                                    var11.field_i = null;
                                    if (var10 != null) {
                                      ((od) (var10)).field_i = var15;
                                      break L12;
                                    } else {
                                      this.field_l[var7_int] = var15;
                                      break L12;
                                    }
                                  }
                                  L13: {
                                    if (var15 != null) {
                                      break L13;
                                    } else {
                                      this.field_a[var7_int] = (od) (var10);
                                      break L13;
                                    }
                                  }
                                  var11 = var15;
                                  continue L7;
                                } else {
                                  break L3;
                                }
                              }
                            }
                          } else {
                            break L6;
                          }
                        }
                        var7_int += 4;
                        var8_int++;
                        var9 = var9 >>> 4;
                        continue L5;
                      } else {
                        var6--;
                        continue L2;
                      }
                    }
                  }
                }
                var6 = 0;
                L14: while (true) {
                  if (var6 >= 8) {
                    break L1;
                  } else {
                    var7 = this.field_l[var6];
                    var8 = this.field_l;
                    var9 = var6;
                    this.field_a[var6] = null;
                    var8[var9] = null;
                    L15: while (true) {
                      if (var7 == null) {
                        var6++;
                        continue L14;
                      } else {
                        var10 = ((od) (var7)).field_i;
                        ((od) (var7)).field_i = null;
                        var7 = var10;
                        continue L15;
                      }
                    }
                  }
                }
              }
            }
          }
        }
        L16: {
          if (this.field_p >= 0) {
            break L16;
          } else {
            this.field_p = 0;
            break L16;
          }
        }
        L17: {
          if (this.field_h == null) {
            break L17;
          } else {
            this.field_h.a(param0, 0, param1);
            break L17;
          }
        }
        this.field_k = lo.a((byte) -90);
    }

    final static id a(un param0, java.awt.Component param1, int param2, int param3) {
        try {
            eo var4 = null;
            Throwable var4_ref = null;
            eo var5 = null;
            eo stackIn_10_0 = null;
            int stackIn_10_1 = 0;
            eo stackIn_11_0 = null;
            int stackIn_11_1 = 0;
            int stackIn_11_2 = 0;
            eo stackIn_21_0 = null;
            Throwable decompiledCaughtException = null;
            if (field_m != 0) {
              L0: {
                if (param2 < 0) {
                  break L0;
                } else {
                  if (param2 < 2) {
                    L1: {
                      if (param3 >= 256) {
                        break L1;
                      } else {
                        param3 = 256;
                        break L1;
                      }
                    }
                    try {
                      L2: {
                        L3: {
                          var5 = new eo();
                          var4 = var5;
                          stackIn_10_0 = (eo) (var4);

                          stackIn_10_1 = 256;

                          if (!field_o) {
                            stackIn_11_0 = (eo) ((Object) stackIn_10_0);
                            stackIn_11_1 = stackIn_10_1;
                            stackIn_11_2 = 1;
                            break L3;
                          } else {
                            stackIn_11_0 = (eo) ((Object) stackIn_10_0);
                            stackIn_11_1 = stackIn_10_1;
                            stackIn_11_2 = 2;
                            break L3;
                          }
                        }
                        L4: {
                          ((id) ((Object) stackIn_11_0)).field_d = new int[stackIn_11_1 * stackIn_11_2];
                          ((id) ((Object) var4)).field_i = param3;
                          ((id) ((Object) var4)).a(param1);
                          ((id) ((Object) var4)).field_f = (param3 & -1024) + 1024;
                          if (((id) ((Object) var4)).field_f <= 16384) {
                            break L4;
                          } else {
                            ((id) ((Object) var4)).field_f = 16384;
                            break L4;
                          }
                        }
                        L5: {
                          ((id) ((Object) var4)).b(((id) ((Object) var4)).field_f);
                          if (field_u <= 0) {
                            break L5;
                          } else {
                            if (field_q != null) {
                              break L5;
                            } else {
                              field_q = new rp();
                              field_q.field_e = param0;
                              param0.a((Runnable) ((Object) field_q), 2, field_u);
                              break L5;
                            }
                          }
                        }
                        L6: {
                          if (field_q == null) {
                            break L6;
                          } else {
                            if (field_q.field_c[param2] == null) {
                              field_q.field_c[param2] = (id) ((Object) var5);
                              break L6;
                            } else {
                              throw new IllegalArgumentException();
                            }
                          }
                        }
                        stackIn_21_0 = (eo) (var4);
                        break L2;
                      }
                    } catch (java.lang.Throwable decompiledCaughtParameter0) {
                      decompiledCaughtException = decompiledCaughtParameter0;
                      var4_ref = decompiledCaughtException;
                      return new id();
                    }
                    return (id) ((Object) stackIn_21_0);
                  } else {
                    break L0;
                  }
                }
              }
              throw new IllegalArgumentException();
            } else {
              throw new IllegalStateException();
            }
        } catch (RuntimeException | Error decompiledUncheckedException) {
            throw decompiledUncheckedException;
        } catch (Throwable decompiledCheckedException) {
            throw new RuntimeException(decompiledCheckedException);
        }
    }

    id() {
        this.field_n = false;
        this.field_s = 32;
        this.field_k = lo.a((byte) 79);
        this.field_j = 0;
        this.field_a = new od[8];
        this.field_t = 0L;
        this.field_e = true;
        this.field_p = 0;
        this.field_c = 0;
        this.field_g = 0L;
        this.field_l = new od[8];
        this.field_r = 0;
    }
}
