/*
 * Decompiled by CFR-JS 0.4.0.
 */
final class tg {
    static int field_b;
    static char[] field_d;
    static ko field_c;
    static String field_a;
    static byte[][] field_e;

    final static void a(float param0, String param1, byte param2, boolean param3) {
        if (param2 != -41) {
            field_a = null;
        }
        if (!(td.field_b != null)) {
            td.field_b = new kl(qe.field_g, sd.field_T);
            qe.field_g.b(param2 ^ 3223, (qm) (Object) td.field_b);
        }
        td.field_b.a(8405024, param3, param0, param1);
        kh.a();
        d.a((byte) -75, true);
    }

    final static void a(int param0, int param1, int param2, int param3, int param4, int param5, jg param6, int param7) {
        int[] var8 = null;
        int var13 = 0;
        int var14 = 0;
        int var15 = 0;
        int var16 = 0;
        int var17 = 0;
        int var18 = 0;
        int var19 = 0;
        int var20 = 0;
        hm var21 = null;
        int var22 = 0;
        int var23 = 0;
        int var24 = 0;
        int var25 = 0;
        int var26 = 0;
        int var27 = 0;
        int var28 = 0;
        int var29 = 0;
        int var30 = 0;
        int var31 = 0;
        int var32 = 0;
        int var33 = 0;
        int var34 = 0;
        int var35 = 0;
        int var36 = 0;
        int var37 = 0;
        int var38 = 0;
        int var39 = 0;
        int var40 = 0;
        int[] var41 = null;
        int var44 = 0;
        jg var45 = null;
        int[] var50 = null;
        int[] var55 = null;
        int[] var60 = null;
        byte[] var62 = null;
        int[] var63 = null;
        int[] var64 = null;
        int[] var65 = null;
        int[] var66 = null;
        int[] var67 = null;
        int stackIn_14_0 = 0;
        int[] stackIn_15_0 = null;
        int[] stackIn_16_0 = null;
        int[] stackIn_17_0 = null;
        int stackIn_17_1 = 0;
        int stackIn_25_0 = 0;
        int stackIn_28_0 = 0;
        int stackIn_31_0 = 0;
        hm stackIn_39_0 = null;
        int stackIn_45_0 = 0;
        int stackIn_49_0 = 0;
        short stackOut_24_0 = 0;
        int stackOut_23_0 = 0;
        int stackOut_27_0 = 0;
        short stackOut_26_0 = 0;
        short stackOut_30_0 = 0;
        int stackOut_29_0 = 0;
        hm stackOut_38_0 = null;
        Object stackOut_37_0 = null;
        int stackOut_48_0 = 0;
        int stackOut_47_0 = 0;
        int stackOut_44_0 = 0;
        int stackOut_43_0 = 0;
        int stackOut_13_0 = 0;
        int stackOut_12_0 = 0;
        int stackOut_10_0 = 0;
        int[] stackOut_14_0 = null;
        int[] stackOut_16_0 = null;
        int stackOut_16_1 = 0;
        int[] stackOut_15_0 = null;
        int stackOut_15_1 = 0;
        L0: {
          L1: {
            var40 = CrazyCrystals.field_B;
            var45 = param6;
            if (var45.field_o == null) {
              break L1;
            } else {
              if (-2 > (var45.field_s ^ -1)) {
                var62 = var45.field_o;
                ln.a(-17512, 0, var62, to.field_b, 0);
                break L0;
              } else {
                break L1;
              }
            }
          }
          qf.h(-30693);
          break L0;
        }
        var66 = new int[param6.field_D];
        var60 = var66;
        var55 = var60;
        var50 = var55;
        var41 = var50;
        var8 = var41;
        var67 = new int[param6.field_D];
        var64 = cf.field_b;
        var63 = jp.field_a;
        var65 = vm.field_d;
        var13 = 0;
        L2: while (true) {
          if (var13 >= param6.field_D) {
            L3: {
              if (param3 == -9206) {
                break L3;
              } else {
                field_d = null;
                break L3;
              }
            }
            var44 = 0;
            var13 = var44;
            L4: while (true) {
              if (var44 >= pi.field_o) {
                return;
              } else {
                L5: {
                  var14 = gh.field_E[var44];
                  var15 = param6.field_K[var14];
                  var16 = param6.field_g[var14];
                  var17 = param6.field_c[var14];
                  if (param6.field_A[var14] < cf.field_b.length) {
                    stackOut_24_0 = param6.field_A[var14];
                    stackIn_25_0 = stackOut_24_0;
                    break L5;
                  } else {
                    stackOut_23_0 = -1;
                    stackIn_25_0 = stackOut_23_0;
                    break L5;
                  }
                }
                L6: {
                  var18 = stackIn_25_0;
                  if (cf.field_b.length <= param6.field_v[var14]) {
                    stackOut_27_0 = -1;
                    stackIn_28_0 = stackOut_27_0;
                    break L6;
                  } else {
                    stackOut_26_0 = param6.field_v[var14];
                    stackIn_28_0 = stackOut_26_0;
                    break L6;
                  }
                }
                L7: {
                  var19 = stackIn_28_0;
                  if (param6.field_R[var14] < cf.field_b.length) {
                    stackOut_30_0 = param6.field_R[var14];
                    stackIn_31_0 = stackOut_30_0;
                    break L7;
                  } else {
                    stackOut_29_0 = -1;
                    stackIn_31_0 = stackOut_29_0;
                    break L7;
                  }
                }
                L8: {
                  L9: {
                    var20 = stackIn_31_0;
                    if (ud.field_S == null) {
                      break L9;
                    } else {
                      if (param6.field_B == null) {
                        break L9;
                      } else {
                        if (var14 >= param6.field_B.length) {
                          break L9;
                        } else {
                          if (-1 == param6.field_B[var14]) {
                            break L9;
                          } else {
                            if (param6.field_B[var14] < ud.field_S.length) {
                              stackOut_38_0 = ud.field_S[param6.field_B[var14]];
                              stackIn_39_0 = stackOut_38_0;
                              break L8;
                            } else {
                              break L9;
                            }
                          }
                        }
                      }
                    }
                  }
                  stackOut_37_0 = null;
                  stackIn_39_0 = (hm) (Object) stackOut_37_0;
                  break L8;
                }
                L10: {
                  var21 = stackIn_39_0;
                  var22 = dn.field_a[var15];
                  var23 = qk.field_E[var15];
                  var24 = dn.field_a[var16];
                  var25 = qk.field_E[var16];
                  var26 = dn.field_a[var17];
                  var27 = qk.field_E[var17];
                  if (var18 != var19) {
                    break L10;
                  } else {
                    if (var20 == var19) {
                      L11: {
                        var28 = var66[var18];
                        var29 = var67[var18];
                        if (var21 != null) {
                          stackOut_48_0 = var21.field_b;
                          stackIn_49_0 = stackOut_48_0;
                          break L11;
                        } else {
                          stackOut_47_0 = 8355711;
                          stackIn_49_0 = stackOut_47_0;
                          break L11;
                        }
                      }
                      var30 = stackIn_49_0;
                      var31 = var30 & 16711935;
                      var32 = var30 & 65280;
                      var33 = (var28 * var31 & -16711763) >>> -564026232 | (var32 * var28 & 16711882) >>> 173404232;
                      var33 = var33 + 65793 * var29;
                      pp.a(var27, var25, var23, param3 + 9206, var26, var24, (var33 & 16711422) >> 1343130145, var22);
                      var44++;
                      continue L4;
                    } else {
                      break L10;
                    }
                  }
                }
                L12: {
                  var28 = var66[var18];
                  var29 = var66[var19];
                  var30 = var66[var20];
                  var31 = var67[var18];
                  var32 = var67[var19];
                  var33 = var67[var20];
                  if (var21 != null) {
                    stackOut_44_0 = var21.field_b;
                    stackIn_45_0 = stackOut_44_0;
                    break L12;
                  } else {
                    stackOut_43_0 = 8355711;
                    stackIn_45_0 = stackOut_43_0;
                    break L12;
                  }
                }
                var34 = stackIn_45_0;
                var35 = 16711935 & var34;
                var36 = var34 & 65280;
                var37 = var36 * var28 >>> 252257672 & 1325465344 | (var35 * var28 & -16711693) >>> -1720531160;
                var38 = (var29 * var36 & 16711862) >>> -730092408 | -1224802049 & var29 * var35 >>> 457450568;
                var38 = var38 + var32 * 65793;
                var37 = var37 + 65793 * var31;
                var39 = 1191117055 & var35 * var30 >>> -1385460600 | var30 * var36 >>> -623329944 & 1845559040;
                var39 = var39 + 65793 * var33;
                b.a((var38 & 65480) >> 2118020168, var22, var24, var27, var38 & 255, 255 & var39, var26, var39 >> 1677886448, var37 >> -1889118032, var38 >> 36391152, 44, var25, 255 & var37, var23, (65374 & var39) >> -838957432, (var37 & 65471) >> -1314119640);
                var44++;
                continue L4;
              }
            }
          } else {
            L13: {
              var14 = param5 * var63[var13] + (var64[var13] * param1 - -(var65[var13] * param0)) >> -2134970360;
              if (0 <= var14) {
                break L13;
              } else {
                var14 = -var14;
                break L13;
              }
            }
            L14: {
              if ((var14 ^ -1) <= -1) {
                if (var14 < 128) {
                  stackOut_13_0 = 128 - -var14;
                  stackIn_14_0 = stackOut_13_0;
                  break L14;
                } else {
                  stackOut_12_0 = 256;
                  stackIn_14_0 = stackOut_12_0;
                  break L14;
                }
              } else {
                stackOut_10_0 = 128;
                stackIn_14_0 = stackOut_10_0;
                break L14;
              }
            }
            L15: {
              var14 = stackIn_14_0;
              var15 = param4 * var65[var13] + (param7 * var63[var13] + param2 * var64[var13]) >> -1251210040;
              stackOut_14_0 = s.field_h;
              stackIn_16_0 = stackOut_14_0;
              stackIn_15_0 = stackOut_14_0;
              if ((var15 ^ -1) <= -1) {
                stackOut_16_0 = (int[]) (Object) stackIn_16_0;
                stackOut_16_1 = var15;
                stackIn_17_0 = stackOut_16_0;
                stackIn_17_1 = stackOut_16_1;
                break L15;
              } else {
                stackOut_15_0 = (int[]) (Object) stackIn_15_0;
                stackOut_15_1 = -var15;
                stackIn_17_0 = stackOut_15_0;
                stackIn_17_1 = stackOut_15_1;
                break L15;
              }
            }
            var15 = stackIn_17_0[stackIn_17_1];
            var14 = (-var15 + 256) * var14 >>> 281263816;
            var66[var13] = var14;
            var67[var13] = var15;
            var13++;
            continue L2;
          }
        }
    }

    public static void a(byte param0) {
        field_a = null;
        field_c = null;
        if (param0 != -100) {
            Object var2 = null;
            tg.a(-124, 27, 6, 79, -72, -110, (jg) null, 54);
        }
        field_e = null;
        field_d = null;
    }

    final static void a(ng param0, ng param1, java.math.BigInteger param2, int param3, java.math.BigInteger param4) {
        jl.a(0, param1, param0.field_h, param2, param0.field_f, param4, 4);
        if (param3 >= -102) {
            field_d = null;
        }
    }

    static {
        $cfr$clinit();
    }

    private static void $cfr$clinit() {
        field_b = 49;
        field_e = new byte[250][];
        field_d = new char[]{(char)8364, (char)0, (char)8218, (char)402, (char)8222, (char)8230, (char)8224, (char)8225, (char)710, (char)8240, (char)352, (char)8249, (char)338, (char)0, (char)381, (char)0, (char)0, (char)8216, (char)8217, (char)8220, (char)8221, (char)8226, (char)8211, (char)8212, (char)732, (char)8482, (char)353, (char)8250, (char)339, (char)0, (char)382, (char)376};
        field_a = "To Level";
    }
}
