/*
 * Decompiled by CFR-JS 0.4.0.
 */
final class a implements java.awt.event.MouseListener, java.awt.event.MouseMotionListener, java.awt.event.FocusListener {
    static int field_a;
    static ue field_b;
    static dl field_c;
    static am field_d;

    public final synchronized void mouseEntered(java.awt.event.MouseEvent param0) {
        try {
            if (null != tm.field_b) {
                aq.field_s = 0;
                fq.field_C = param0.getX();
                tn.field_s = param0.getY();
                wl.field_m = true;
            }
        } catch (RuntimeException runtimeException) {
            throw dn.a((Throwable) ((Object) runtimeException), "a.mouseEntered(" + (param0 != null ? "{...}" : "null") + ')');
        }
    }

    public final synchronized void mousePressed(java.awt.event.MouseEvent param0) {
        int var2_int = 0;
        RuntimeException stackIn_14_0 = null;
        StringBuilder stackIn_14_1 = null;
        RuntimeException stackIn_15_0 = null;
        StringBuilder stackIn_15_1 = null;
        String stackIn_15_2 = null;
        RuntimeException decompiledCaughtException = null;
        RuntimeException var2 = null;
        try {
          L0: {
            L1: {
              if (null != tm.field_b) {
                L2: {
                  aq.field_s = 0;
                  kk.field_g = param0.getX();
                  sb.field_I = param0.getY();
                  lo.a((byte) -57);
                  if (javax.swing.SwingUtilities.isRightMouseButton(param0)) {
                    th.field_y = 2;
                    uc.field_b = 2;
                    break L2;
                  } else {
                    th.field_y = 1;
                    uc.field_b = 1;
                    break L2;
                  }
                }
                L3: {
                  var2_int = param0.getModifiers();
                  wl.field_m = true;
                  if ((var2_int & 4) != 0) {
                    break L3;
                  } else {
                    break L3;
                  }
                }
                L4: {
                  if (0 == (16 & var2_int)) {
                    break L4;
                  } else {
                    break L4;
                  }
                }
                if ((8 & var2_int) != 0) {
                  break L1;
                } else {
                  break L1;
                }
              } else {
                break L1;
              }
            }
            L5: {
              if (param0.isPopupTrigger()) {
                param0.consume();
                break L5;
              } else {
                break L5;
              }
            }
            break L0;
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L6: {
            var2 = decompiledCaughtException;
            stackIn_14_0 = (RuntimeException) (var2);

            stackIn_14_1 = new StringBuilder().append("a.mousePressed(");

            if (param0 == null) {
              stackIn_15_0 = (RuntimeException) ((Object) stackIn_14_0);
              stackIn_15_1 = (StringBuilder) ((Object) stackIn_14_1);
              stackIn_15_2 = "null";
              break L6;
            } else {
              stackIn_15_0 = (RuntimeException) ((Object) stackIn_14_0);
              stackIn_15_1 = (StringBuilder) ((Object) stackIn_14_1);
              stackIn_15_2 = "{...}";
              break L6;
            }
          }
          throw dn.a((Throwable) ((Object) stackIn_15_0), stackIn_15_2 + ')');
        }
    }

    public final synchronized void mouseExited(java.awt.event.MouseEvent param0) {
        RuntimeException stackIn_6_0 = null;
        StringBuilder stackIn_6_1 = null;
        RuntimeException stackIn_7_0 = null;
        StringBuilder stackIn_7_1 = null;
        String stackIn_7_2 = null;
        RuntimeException decompiledCaughtException = null;
        RuntimeException var2 = null;
        try {
          L0: {
            L1: {
              if (tm.field_b != null) {
                aq.field_s = 0;
                fq.field_C = -1;
                tn.field_s = -1;
                wl.field_m = true;
                break L1;
              } else {
                break L1;
              }
            }
            break L0;
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L2: {
            var2 = decompiledCaughtException;
            stackIn_6_0 = (RuntimeException) (var2);

            stackIn_6_1 = new StringBuilder().append("a.mouseExited(");

            if (param0 == null) {
              stackIn_7_0 = (RuntimeException) ((Object) stackIn_6_0);
              stackIn_7_1 = (StringBuilder) ((Object) stackIn_6_1);
              stackIn_7_2 = "null";
              break L2;
            } else {
              stackIn_7_0 = (RuntimeException) ((Object) stackIn_6_0);
              stackIn_7_1 = (StringBuilder) ((Object) stackIn_6_1);
              stackIn_7_2 = "{...}";
              break L2;
            }
          }
          throw dn.a((Throwable) ((Object) stackIn_7_0), stackIn_7_2 + ')');
        }
    }

    public final synchronized void mouseMoved(java.awt.event.MouseEvent param0) {
        try {
            if (tm.field_b != null) {
                aq.field_s = 0;
                fq.field_C = param0.getX();
                tn.field_s = param0.getY();
                wl.field_m = true;
            }
        } catch (RuntimeException runtimeException) {
            throw dn.a((Throwable) ((Object) runtimeException), "a.mouseMoved(" + (param0 != null ? "{...}" : "null") + ')');
        }
    }

    public final synchronized void mouseDragged(java.awt.event.MouseEvent param0) {
        try {
            if (tm.field_b != null) {
                aq.field_s = 0;
                fq.field_C = param0.getX();
                tn.field_s = param0.getY();
                wl.field_m = true;
            }
        } catch (RuntimeException runtimeException) {
            throw dn.a((Throwable) ((Object) runtimeException), "a.mouseDragged(" + (param0 != null ? "{...}" : "null") + ')');
        }
    }

    public final synchronized void mouseReleased(java.awt.event.MouseEvent param0) {
        int var2_int = 0;
        RuntimeException stackIn_10_0 = null;
        StringBuilder stackIn_10_1 = null;
        RuntimeException stackIn_11_0 = null;
        StringBuilder stackIn_11_1 = null;
        String stackIn_11_2 = null;
        RuntimeException decompiledCaughtException = null;
        RuntimeException var2 = null;
        try {
          L0: {
            L1: {
              if (null == tm.field_b) {
                break L1;
              } else {
                L2: {
                  aq.field_s = 0;
                  uc.field_b = 0;
                  wl.field_m = true;
                  var2_int = param0.getModifiers();
                  if (-1 == (var2_int & 4 ^ -1)) {
                    break L2;
                  } else {
                    break L2;
                  }
                }
                L3: {
                  if (-1 != (var2_int & 8 ^ -1)) {
                    break L3;
                  } else {
                    break L3;
                  }
                }
                if ((var2_int & 16) != 0) {
                  break L1;
                } else {
                  break L1;
                }
              }
            }
            L4: {
              if (param0.isPopupTrigger()) {
                param0.consume();
                break L4;
              } else {
                break L4;
              }
            }
            break L0;
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L5: {
            var2 = decompiledCaughtException;
            stackIn_10_0 = (RuntimeException) (var2);

            stackIn_10_1 = new StringBuilder().append("a.mouseReleased(");

            if (param0 == null) {
              stackIn_11_0 = (RuntimeException) ((Object) stackIn_10_0);
              stackIn_11_1 = (StringBuilder) ((Object) stackIn_10_1);
              stackIn_11_2 = "null";
              break L5;
            } else {
              stackIn_11_0 = (RuntimeException) ((Object) stackIn_10_0);
              stackIn_11_1 = (StringBuilder) ((Object) stackIn_10_1);
              stackIn_11_2 = "{...}";
              break L5;
            }
          }
          throw dn.a((Throwable) ((Object) stackIn_11_0), stackIn_11_2 + ')');
        }
    }

    final static void a(int param0, int param1, int param2, int param3, int param4, vc param5, int param6, int param7, vc param8, int param9, int param10, int param11, hj param12, hj param13, int param14, hj param15, int param16, int param17, int param18, int param19, int param20) {
        RuntimeException stackIn_6_0 = null;
        StringBuilder stackIn_6_1 = null;
        RuntimeException stackIn_7_0 = null;
        StringBuilder stackIn_7_1 = null;
        String stackIn_7_2 = null;
        StringBuilder stackIn_9_1 = null;
        StringBuilder stackIn_10_1 = null;
        String stackIn_10_2 = null;
        StringBuilder stackIn_12_1 = null;
        StringBuilder stackIn_13_1 = null;
        String stackIn_13_2 = null;
        StringBuilder stackIn_15_1 = null;
        StringBuilder stackIn_16_1 = null;
        String stackIn_16_2 = null;
        StringBuilder stackIn_18_1 = null;
        StringBuilder stackIn_19_1 = null;
        String stackIn_19_2 = null;
        int decompiledRegionSelector0 = 0;
        RuntimeException decompiledCaughtException = null;
        RuntimeException var21 = null;
        try {
          L0: {
            if (param2 == 0) {
              sl.a(param5, param8, 107, param18);
              mo.a(param20, param3, param4, param0, (byte) -16);
              mh.a(param7, -20, param1);
              f.a(param16, param15, param17, param13, param14, (byte) 104);
              u.a(param6, param12, 250, param9);
              vf.a(param11, (byte) 118, param10, param19);
              decompiledRegionSelector0 = 1;
              break L0;
            } else {
              decompiledRegionSelector0 = 0;
              break L0;
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L1: {
            var21 = decompiledCaughtException;
            stackIn_6_0 = (RuntimeException) (var21);

            stackIn_6_1 = new StringBuilder().append("a.A(").append(param0).append(',').append(param1).append(',').append(param2).append(',').append(param3).append(',').append(param4).append(',');

            if (param5 == null) {
              stackIn_7_0 = (RuntimeException) ((Object) stackIn_6_0);
              stackIn_7_1 = (StringBuilder) ((Object) stackIn_6_1);
              stackIn_7_2 = "null";
              break L1;
            } else {
              stackIn_7_0 = (RuntimeException) ((Object) stackIn_6_0);
              stackIn_7_1 = (StringBuilder) ((Object) stackIn_6_1);
              stackIn_7_2 = "{...}";
              break L1;
            }
          }
          L2: {


            stackIn_9_1 = ((StringBuilder) (Object) stackIn_7_1).append(stackIn_7_2).append(',').append(param6).append(',').append(param7).append(',');

            if (param8 == null) {
              stackIn_7_0 = (RuntimeException) ((Object) stackIn_7_0);
              stackIn_10_1 = (StringBuilder) ((Object) stackIn_9_1);
              stackIn_10_2 = "null";
              break L2;
            } else {
              stackIn_7_0 = (RuntimeException) ((Object) stackIn_7_0);
              stackIn_10_1 = (StringBuilder) ((Object) stackIn_9_1);
              stackIn_10_2 = "{...}";
              break L2;
            }
          }
          L3: {


            stackIn_12_1 = ((StringBuilder) (Object) stackIn_10_1).append(stackIn_10_2).append(',').append(param9).append(',').append(param10).append(',').append(param11).append(',');

            if (param12 == null) {
              stackIn_7_0 = (RuntimeException) ((Object) stackIn_7_0);
              stackIn_13_1 = (StringBuilder) ((Object) stackIn_12_1);
              stackIn_13_2 = "null";
              break L3;
            } else {
              stackIn_7_0 = (RuntimeException) ((Object) stackIn_7_0);
              stackIn_13_1 = (StringBuilder) ((Object) stackIn_12_1);
              stackIn_13_2 = "{...}";
              break L3;
            }
          }
          L4: {


            stackIn_15_1 = ((StringBuilder) (Object) stackIn_13_1).append(stackIn_13_2).append(',');

            if (param13 == null) {
              stackIn_7_0 = (RuntimeException) ((Object) stackIn_7_0);
              stackIn_16_1 = (StringBuilder) ((Object) stackIn_15_1);
              stackIn_16_2 = "null";
              break L4;
            } else {
              stackIn_7_0 = (RuntimeException) ((Object) stackIn_7_0);
              stackIn_16_1 = (StringBuilder) ((Object) stackIn_15_1);
              stackIn_16_2 = "{...}";
              break L4;
            }
          }
          L5: {


            stackIn_18_1 = ((StringBuilder) (Object) stackIn_16_1).append(stackIn_16_2).append(',').append(param14).append(',');

            if (param15 == null) {
              stackIn_7_0 = (RuntimeException) ((Object) stackIn_7_0);
              stackIn_19_1 = (StringBuilder) ((Object) stackIn_18_1);
              stackIn_19_2 = "null";
              break L5;
            } else {
              stackIn_7_0 = (RuntimeException) ((Object) stackIn_7_0);
              stackIn_19_1 = (StringBuilder) ((Object) stackIn_18_1);
              stackIn_19_2 = "{...}";
              break L5;
            }
          }
          throw dn.a((Throwable) ((Object) stackIn_7_0), stackIn_19_2 + ',' + param16 + ',' + param17 + ',' + param18 + ',' + param19 + ',' + param20 + ')');
        }
        if (decompiledRegionSelector0 == 0) {
          return;
        } else {
          return;
        }
    }

    public final void focusGained(java.awt.event.FocusEvent param0) {
    }

    public static void a(boolean param0) {
        field_b = null;
        field_d = null;
        field_c = null;
        if (!param0) {
            hj var2 = (hj) null;
            a.a(74, 5, -18, 74, -59, (vc) null, -54, -86, (vc) null, -31, 85, -40, (hj) null, (hj) null, 11, (hj) null, -17, -43, 64, -124, 62);
        }
    }

    public final synchronized void focusLost(java.awt.event.FocusEvent param0) {
        RuntimeException stackIn_6_0 = null;
        StringBuilder stackIn_6_1 = null;
        RuntimeException stackIn_7_0 = null;
        StringBuilder stackIn_7_1 = null;
        String stackIn_7_2 = null;
        RuntimeException decompiledCaughtException = null;
        RuntimeException var2 = null;
        try {
          L0: {
            L1: {
              if (tm.field_b != null) {
                uc.field_b = 0;
                break L1;
              } else {
                break L1;
              }
            }
            break L0;
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L2: {
            var2 = decompiledCaughtException;
            stackIn_6_0 = (RuntimeException) (var2);

            stackIn_6_1 = new StringBuilder().append("a.focusLost(");

            if (param0 == null) {
              stackIn_7_0 = (RuntimeException) ((Object) stackIn_6_0);
              stackIn_7_1 = (StringBuilder) ((Object) stackIn_6_1);
              stackIn_7_2 = "null";
              break L2;
            } else {
              stackIn_7_0 = (RuntimeException) ((Object) stackIn_6_0);
              stackIn_7_1 = (StringBuilder) ((Object) stackIn_6_1);
              stackIn_7_2 = "{...}";
              break L2;
            }
          }
          throw dn.a((Throwable) ((Object) stackIn_7_0), stackIn_7_2 + ')');
        }
    }

    final static void a(byte param0) {
        int var1;
        int var2;
        CharSequence var3;
        L0: {
          var2 = CrazyCrystals.field_B;
          sd.field_T = null;
          dj.field_t = false;
          if (param0 == 63) {
            break L0;
          } else {
            var3 = (CharSequence) null;
            a.a(-70, (CharSequence) null);
            break L0;
          }
        }
        L1: {
          if (ne.field_d) {
            sp.field_a.l(126);
            break L1;
          } else {
            var1 = kl.field_V;
            if ((var1 ^ -1) >= -1) {
              sp.field_a.i((byte) -78);
              rf.a(param0 + -60);
              break L1;
            } else {
              if ((var1 ^ -1) != -2) {
                sd.field_T = ci.a(new String[]{Integer.toString(var1)}, gh.field_t, 3);
                sd.field_T = dk.a(new CharSequence[]{(CharSequence) ((Object) sd.field_T), (CharSequence) ((Object) "<br>"), (CharSequence) ((Object) qe.field_h)}, (byte) -55);
                sp.field_a.i((byte) -78);
                rf.a(param0 + -60);
                break L1;
              } else {
                sd.field_T = va.field_a;
                sd.field_T = dk.a(new CharSequence[]{(CharSequence) ((Object) sd.field_T), (CharSequence) ((Object) "<br>"), (CharSequence) ((Object) qe.field_h)}, (byte) -55);
                sp.field_a.i((byte) -78);
                rf.a(param0 + -60);
                break L1;
              }
            }
          }
        }
    }

    final static boolean a(int param0, CharSequence param1) {
        int var2_int = 0;
        RuntimeException var2 = null;
        boolean stackIn_1_0 = false;
        RuntimeException stackIn_4_0 = null;
        StringBuilder stackIn_4_1 = null;
        RuntimeException stackIn_5_0 = null;
        StringBuilder stackIn_5_1 = null;
        String stackIn_5_2 = null;
        RuntimeException decompiledCaughtException = null;
        try {
          L0: {
            var2_int = -38 / ((45 - param0) / 48);
            stackIn_1_0 = qn.a((byte) -123, false, param1);
            break L0;
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L1: {
            var2 = decompiledCaughtException;
            stackIn_4_0 = (RuntimeException) (var2);

            stackIn_4_1 = new StringBuilder().append("a.E(").append(param0).append(',');

            if (param1 == null) {
              stackIn_5_0 = (RuntimeException) ((Object) stackIn_4_0);
              stackIn_5_1 = (StringBuilder) ((Object) stackIn_4_1);
              stackIn_5_2 = "null";
              break L1;
            } else {
              stackIn_5_0 = (RuntimeException) ((Object) stackIn_4_0);
              stackIn_5_1 = (StringBuilder) ((Object) stackIn_4_1);
              stackIn_5_2 = "{...}";
              break L1;
            }
          }
          throw dn.a((Throwable) ((Object) stackIn_5_0), stackIn_5_2 + ')');
        }
        return stackIn_1_0;
    }

    final static void a(int param0, int param1, boolean param2) {
        int stackIn_59_0 = 0;
        int stackIn_62_0 = 0;
        String[] stackIn_69_0 = null;
        int stackIn_69_1 = 0;
        String[] stackIn_70_0 = null;
        int stackIn_70_1 = 0;
        String stackIn_70_2 = null;
        String[] stackIn_74_0 = null;
        int stackIn_74_1 = 0;
        int stackIn_74_2 = 0;
        int stackIn_74_3 = 0;
        String stackIn_77_2 = null;
        int stackIn_90_0 = 0;
        int stackIn_91_0 = 0;
        int stackIn_91_1 = 0;
        int stackIn_109_0 = 0;
        int stackIn_110_0 = 0;
        int stackIn_110_1 = 0;
        int decompiledRegionSelector0 = 0;
        Throwable caughtException = null;
        RuntimeException decompiledCaughtException = null;
        int var3_int = 0;
        RuntimeException var3 = null;
        int var4 = 0;
        String[] var4_ref_String__ = null;
        int var5 = 0;
        int var6 = 0;
        int var7 = 0;
        int var8 = 0;
        int var9 = 0;
        String[] var10 = null;
        String var11 = null;
        String var12 = null;
        var9 = CrazyCrystals.field_B;
        try {
          L0: {
            L1: {
              rd.field_h = true;
              am.field_g = param1;
              if (am.field_g == 0) {
                var3_int = v.a(ec.field_m, ug.field_a, -27815, vk.field_L, eb.field_y);
                var4 = 3 + var3_int;
                ub.field_b = new int[var4];
                je.field_i = new String[var4];
                var5 = 0;
                L2: while (true) {
                  if (var5 >= var4) {
                    sn.field_a = new int[2];
                    var5 = 0;
                    L3: while (true) {
                      if (var5 >= var3_int) {
                        je.field_i[var4 + -3] = "";
                        je.field_i[-2 + var4] = ig.field_m;
                        ub.field_b[-2 + var4] = 0;
                        sn.field_a[0] = 4;
                        je.field_i[var4 + -1] = qo.field_o;
                        ub.field_b[-1 + var4] = 1;
                        sn.field_a[1] = 5;
                        break L1;
                      } else {
                        je.field_i[var5] = eb.field_y[var5];
                        var5++;
                        continue L3;
                      }
                    }
                  } else {
                    ub.field_b[var5] = -1;
                    var5++;
                    continue L2;
                  }
                }
              } else {
                if (am.field_g != 1) {
                  if (am.field_g == 2) {
                    var3_int = v.a(ec.field_m, ci.a(new String[]{"<br><%0><br>"}, rg.field_b, 3), -27815, vk.field_L, eb.field_y);
                    var4 = -1;
                    var5 = 0;
                    L4: while (true) {
                      L5: {
                        if (var3_int <= var5) {
                          break L5;
                        } else {
                          if ("<%0>".equals(eb.field_y[var5])) {
                            var4 = var5;
                            break L5;
                          } else {
                            var5++;
                            continue L4;
                          }
                        }
                      }
                      if (var4 == -1) {
                        throw new IllegalStateException();
                      } else {
                        jl.field_a = new String[var4];
                        eg.a(eb.field_y, 0, jl.field_a, 0, var4);
                        lo.field_c = new String[-1 + var3_int - var4];
                        eg.a(eb.field_y, 1 + var4, lo.field_c, 0, -1 + (-var4 + var3_int));
                        var3_int = v.a(ec.field_m, ci.a(new String[]{"<br><%0><br>"}, jg.field_t, 3), -27815, vk.field_L, eb.field_y);
                        var4 = -1;
                        var5 = 0;
                        L6: while (true) {
                          L7: {
                            if (var3_int <= var5) {
                              break L7;
                            } else {
                              if (!"<%0>".equals(eb.field_y[var5])) {
                                var5++;
                                continue L6;
                              } else {
                                var4 = var5;
                                break L7;
                              }
                            }
                          }
                          if (-1 == var4) {
                            throw new IllegalStateException();
                          } else {
                            L8: {
                              sl.field_r = new String[var4];
                              eg.a(eb.field_y, 0, sl.field_r, 0, var4);
                              ne.field_a = new String[-var4 + (var3_int - 1)];
                              eg.a(eb.field_y, 1 + var4, ne.field_a, 0, -1 + (-var4 + var3_int));
                              if (jl.field_a.length < sl.field_r.length) {
                                stackIn_59_0 = sl.field_r.length;
                                break L8;
                              } else {
                                stackIn_59_0 = jl.field_a.length;
                                break L8;
                              }
                            }
                            L9: {
                              var5 = stackIn_59_0;
                              if (lo.field_c.length >= ne.field_a.length) {
                                stackIn_62_0 = lo.field_c.length;
                                break L9;
                              } else {
                                stackIn_62_0 = ne.field_a.length;
                                break L9;
                              }
                            }
                            var6 = stackIn_62_0;
                            var7 = var6 + (7 - -var5);
                            je.field_i = new String[var7];
                            ub.field_b = new int[var7];
                            var8 = 0;
                            L10: while (true) {
                              if (var8 >= var7) {
                                je.field_i[0] = sa.field_rb;
                                ub.field_b[1] = 0;
                                je.field_i[1] = qc.field_h;
                                sn.field_a = new int[2];
                                je.field_i[5] = "";
                                sn.field_a[1] = 2;
                                sn.field_a[0] = 5;
                                je.field_i[2] = jk.field_h;
                                je.field_i[3] = jb.field_a;
                                je.field_i[4] = ca.field_g;
                                ub.field_b[3] = 1;
                                var8 = 0;
                                L11: while (true) {
                                  if (var5 <= var8) {
                                    je.field_i[var5 + 6] = null;
                                    ub.field_b[6 - -var5] = -2;
                                    var8 = 0;
                                    L12: while (true) {
                                      if (var8 >= var6) {
                                        wm.field_d = lo.a((byte) -60);
                                        break L1;
                                      } else {
                                        stackIn_74_0 = je.field_i;
                                        stackIn_74_1 = var8 + var5 + 7;
                                        stackIn_74_2 = ne.field_a.length;
                                        stackIn_74_3 = var8;
                                        L13: {




                                          if (stackIn_74_2 > stackIn_74_3) {
                                            stackIn_74_0 = (String[]) ((Object) stackIn_74_0);

                                            stackIn_77_2 = ne.field_a[var8];
                                            break L13;
                                          } else {
                                            stackIn_74_0 = (String[]) ((Object) stackIn_74_0);

                                            stackIn_77_2 = "";
                                            break L13;
                                          }
                                        }
                                        stackIn_74_0[stackIn_74_1] = stackIn_77_2;
                                        var8++;
                                        continue L12;
                                      }
                                    }
                                  } else {
                                    L14: {
                                      stackIn_69_0 = je.field_i;

                                      stackIn_69_1 = 6 - -var8;

                                      if ((-var5 + (var8 + sl.field_r.length) ^ -1) <= -1) {
                                        stackIn_70_0 = (String[]) ((Object) stackIn_69_0);
                                        stackIn_70_1 = stackIn_69_1;
                                        stackIn_70_2 = sl.field_r[sl.field_r.length + (var8 - var5)];
                                        break L14;
                                      } else {
                                        stackIn_70_0 = (String[]) ((Object) stackIn_69_0);
                                        stackIn_70_1 = stackIn_69_1;
                                        stackIn_70_2 = "";
                                        break L14;
                                      }
                                    }
                                    stackIn_70_0[stackIn_70_1] = stackIn_70_2;
                                    var8++;
                                    continue L11;
                                  }
                                }
                              } else {
                                ub.field_b[var8] = -1;
                                var8++;
                                continue L10;
                              }
                            }
                          }
                        }
                      }
                    }
                  } else {
                    if (3 != am.field_g) {
                      if (-5 != (am.field_g ^ -1)) {
                        if (-6 != (am.field_g ^ -1)) {
                          throw new IllegalArgumentException();
                        } else {
                          var3_int = v.a(ec.field_m, fn.field_i, -27815, vk.field_L, eb.field_y);
                          var4 = var3_int + 3;
                          je.field_i = new String[var4];
                          ub.field_b = new int[var4];
                          var5 = 0;
                          L15: while (true) {
                            if (var4 <= var5) {
                              sn.field_a = new int[2];
                              var5 = 0;
                              L16: while (true) {
                                if (var3_int <= var5) {
                                  je.field_i[var4 - 3] = "";
                                  je.field_i[-2 + var4] = ia.field_h;
                                  ub.field_b[-2 + var4] = 0;
                                  sn.field_a[0] = 3;
                                  je.field_i[-1 + var4] = qo.field_o;
                                  ub.field_b[-1 + var4] = 1;
                                  sn.field_a[1] = 5;
                                  break L1;
                                } else {
                                  je.field_i[var5] = eb.field_y[var5];
                                  var5++;
                                  continue L16;
                                }
                              }
                            } else {
                              ub.field_b[var5] = -1;
                              var5++;
                              continue L15;
                            }
                          }
                        }
                      } else {
                        var3_int = v.a(ec.field_m, nh.field_C, -27815, vk.field_L, eb.field_y);
                        var4 = var3_int + 2;
                        ub.field_b = new int[var4];
                        je.field_i = new String[var4];
                        var5 = 0;
                        L17: while (true) {
                          if (var5 >= var4) {
                            sn.field_a = new int[1];
                            var5 = 0;
                            L18: while (true) {
                              if (var5 >= var3_int) {
                                je.field_i[var4 - 2] = "";
                                je.field_i[-1 + var4] = qo.field_o;
                                ub.field_b[var4 - 1] = 0;
                                sn.field_a[0] = 5;
                                break L1;
                              } else {
                                je.field_i[var5] = eb.field_y[var5];
                                var5++;
                                continue L18;
                              }
                            }
                          } else {
                            ub.field_b[var5] = -1;
                            var5++;
                            continue L17;
                          }
                        }
                      }
                    } else {
                      L19: {
                        if (te.field_a.field_h) {
                          var3_int = v.a(ec.field_m, ib.field_m, -27815, vk.field_L, eb.field_y);
                          break L19;
                        } else {
                          var3_int = v.a(ec.field_m, gn.field_U, -27815, vk.field_L, eb.field_y);
                          break L19;
                        }
                      }
                      var4 = var3_int + 2;
                      ub.field_b = new int[var4];
                      je.field_i = new String[var4];
                      var5 = 0;
                      L20: while (true) {
                        if (var4 <= var5) {
                          sn.field_a = new int[1];
                          var5 = 0;
                          L21: while (true) {
                            if (var5 >= var3_int) {
                              je.field_i[var4 - 2] = "";
                              je.field_i[-1 + var4] = qo.field_o;
                              ub.field_b[-1 + var4] = 0;
                              sn.field_a[0] = 5;
                              break L1;
                            } else {
                              je.field_i[var5] = eb.field_y[var5];
                              var5++;
                              continue L21;
                            }
                          }
                        } else {
                          ub.field_b[var5] = -1;
                          var5++;
                          continue L20;
                        }
                      }
                    }
                  }
                } else {
                  var3_int = v.a(ec.field_m, ug.field_a, -27815, vk.field_L, eb.field_y);
                  var4 = var3_int + 2;
                  ub.field_b = new int[var4];
                  je.field_i = new String[var4];
                  var5 = 0;
                  L22: while (true) {
                    if (var5 >= var4) {
                      sn.field_a = new int[1];
                      var5 = 0;
                      L23: while (true) {
                        if (var3_int <= var5) {
                          je.field_i[var4 + -2] = "";
                          je.field_i[var4 - 1] = qo.field_o;
                          ub.field_b[var4 - 1] = 0;
                          sn.field_a[0] = 5;
                          break L1;
                        } else {
                          je.field_i[var5] = eb.field_y[var5];
                          var5++;
                          continue L23;
                        }
                      }
                    } else {
                      ub.field_b[var5] = -1;
                      var5++;
                      continue L22;
                    }
                  }
                }
              }
            }
            lb.field_l.field_b = sn.field_a.length;
            var3_int = 0;
            var4 = 0;
            L24: while (true) {
              if (var4 >= je.field_i.length) {
                L25: {
                  if ((am.field_g ^ -1) != -3) {
                    break L25;
                  } else {
                    var10 = jl.field_a;
                    var4_ref_String__ = var10;
                    var5 = 0;
                    L26: while (true) {
                      if (var5 >= var10.length) {
                        var4_ref_String__ = lo.field_c;
                        var5 = 0;
                        L27: while (true) {
                          if (var5 >= var4_ref_String__.length) {
                            break L25;
                          } else {
                            var12 = var4_ref_String__[var5];
                            var7 = vm.a((byte) -113, false, var12);
                            if (var3_int < var7) {
                              var3_int = var7;
                              var5++;
                              continue L27;
                            } else {
                              var5++;
                              continue L27;
                            }
                          }
                        }
                      } else {
                        var11 = var10[var5];
                        var7 = vm.a((byte) 98, false, var11);
                        if ((var7 ^ -1) < (var3_int ^ -1)) {
                          var3_int = var7;
                          var5++;
                          continue L26;
                        } else {
                          var5++;
                          continue L26;
                        }
                      }
                    }
                  }
                }
                ri.field_a = oa.field_b + (var3_int + -(var3_int >> 1790602177));
                gp.field_a = (field_a + bn.field_c << 2031346433) * lb.field_l.field_b;
                bf.field_c = -(var3_int >> 262385153) + oa.field_b;
                var4 = 0;
                L28: while (true) {
                  if (var4 >= je.field_i.length) {
                    pf.field_n = -(gp.field_a >> 685711809) + rn.field_n;
                    cd.field_z = new int[je.field_i.length][];
                    if (param0 > 103) {
                      var4 = 0;
                      var5 = pf.field_n;
                      L29: while (true) {
                        if (je.field_i.length <= var4) {
                          L30: {
                            if (2 == am.field_g) {
                              lb.field_l.a((byte) 41, param2, -1, -1);
                              break L30;
                            } else {
                              lb.field_l.a((byte) 41, param2, be.a(-1013410975, qh.field_i, bm.field_h), 0);
                              break L30;
                            }
                          }
                          decompiledRegionSelector0 = 1;
                          break L0;
                        } else {
                          L31: {
                            var6 = ub.field_b[var4];
                            if ((var6 ^ -1) <= -1) {
                              var7 = vm.a((byte) -112, true, je.field_i[var4]);
                              var5 = var5 + field_a;
                              var8 = -(var7 >> 303659489) + oa.field_b;
                              cd.field_z[var4] = new int[4];
                              cd.field_z[var4][0] = var8 + -to.field_d;
                              cd.field_z[var4][1] = var5;
                              cd.field_z[var4][2] = var7 + (to.field_d << 1569554401);
                              var5 = var5 + (field_a + (bn.field_c << -819165439) + gh.field_w);
                              cd.field_z[var4][3] = gh.field_w + (bn.field_c << -508643039);
                              break L31;
                            } else {
                              var5 = var5 + io.field_O;
                              break L31;
                            }
                          }
                          var4++;
                          continue L29;
                        }
                      }
                    } else {
                      decompiledRegionSelector0 = 0;
                      break L0;
                    }
                  } else {
                    L32: {
                      stackIn_109_0 = gp.field_a;

                      if ((ub.field_b[var4] ^ -1) <= -1) {
                        stackIn_110_0 = stackIn_109_0;
                        stackIn_110_1 = gh.field_w;
                        break L32;
                      } else {
                        stackIn_110_0 = stackIn_109_0;
                        stackIn_110_1 = io.field_O;
                        break L32;
                      }
                    }
                    gp.field_a = stackIn_110_0 + stackIn_110_1;
                    var4++;
                    continue L28;
                  }
                }
              } else {
                L33: {
                  stackIn_90_0 = 90;

                  if (ub.field_b[var4] < 0) {
                    stackIn_91_0 = stackIn_90_0;
                    stackIn_91_1 = 0;
                    break L33;
                  } else {
                    stackIn_91_0 = stackIn_90_0;
                    stackIn_91_1 = 1;
                    break L33;
                  }
                }
                var5 = vm.a((byte) stackIn_91_0, stackIn_91_1 != 0, je.field_i[var4]);
                if (var3_int < var5) {
                  var3_int = var5;
                  var4++;
                  continue L24;
                } else {
                  var4++;
                  continue L24;
                }
              }
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          var3 = decompiledCaughtException;
          throw dn.a((Throwable) ((Object) var3), "a.C(" + param0 + ',' + param1 + ',' + param2 + ')');
        }
        if (decompiledRegionSelector0 == 0) {
          return;
        } else {
          return;
        }
    }

    public final void mouseClicked(java.awt.event.MouseEvent param0) {
        try {
            if (param0.isPopupTrigger()) {
                param0.consume();
            }
        } catch (RuntimeException runtimeException) {
            throw dn.a((Throwable) ((Object) runtimeException), "a.mouseClicked(" + (param0 != null ? "{...}" : "null") + ')');
        }
    }

    static {
    }
}
