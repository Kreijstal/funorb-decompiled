/*
 * Decompiled by CFR-JS 0.4.0.
 */
final class tc extends id {
    private db[] field_x;
    static int field_r;
    static int field_p;
    static int field_v;
    static int field_w;
    static java.applet.Applet field_u;
    private db field_t;
    static int[] field_s;
    static String field_q;

    final static String c(int param0) {
        if (!(dj.field_a != o.field_l)) {
            return lc.field_p;
        }
        if (!(dj.field_a != el.field_E)) {
            return gk.field_c;
        }
        if (param0 != -5) {
            field_p = -104;
        }
        if (!(al.field_f.a(true))) {
            return gk.field_c;
        }
        return va.field_a;
    }

    final db a(byte param0, int param1) {
        int var3 = -43 % ((param0 - 73) / 41);
        db dupTemp$0 = new db();
        this.field_x[param1] = dupTemp$0;
        return dupTemp$0;
    }

    final static void a(kl param0, int param1, boolean param2) {
        try {
            nl.field_e.b(-104, param0);
            if (param2) {
                field_u = (java.applet.Applet) null;
            }
            ca.a(param1, param0, (byte) 71);
        } catch (RuntimeException runtimeException) {
            throw ma.a((Throwable) ((Object) runtimeException), "tc.F(" + (param0 != null ? "{...}" : "null") + ',' + param1 + ',' + param2 + ')');
        }
    }

    tc(tc param0, boolean param1) {
        this();
        try {
            param0.a(param1, -59, (tc) (this));
        } catch (RuntimeException runtimeException) {
            throw ma.a((Throwable) ((Object) runtimeException), "tc.<init>(" + (param0 != null ? "{...}" : "null") + ',' + param1 + ')');
        }
    }

    public static void e(int param0) {
        if (param0 > -52) {
            return;
        }
        field_s = null;
        field_u = null;
        field_q = null;
    }

    final void a(int param0, pb[] param1) {
        db[] var3 = null;
        int var4 = 0;
        db var5 = null;
        int var6 = 0;
        db[] var7 = null;
        RuntimeException stackIn_12_0 = null;
        StringBuilder stackIn_12_1 = null;
        RuntimeException stackIn_13_0 = null;
        StringBuilder stackIn_13_1 = null;
        String stackIn_13_2 = null;
        RuntimeException decompiledCaughtException = null;
        RuntimeException var3_ref = null;
        var6 = stellarshard.field_B;
        try {
          L0: {
            L1: {
              var7 = this.field_x;
              var3 = var7;
              if (param0 < -64) {
                break L1;
              } else {
                this.field_t = (db) null;
                break L1;
              }
            }
            var4 = 0;
            L2: while (true) {
              if (var7.length <= var4) {
                break L0;
              } else {
                L3: {
                  var5 = var7[var4];
                  if (var5 != null) {
                    var5.field_d = param1;
                    break L3;
                  } else {
                    break L3;
                  }
                }
                var4++;
                continue L2;
              }
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L4: {
            var3_ref = decompiledCaughtException;
            stackIn_12_0 = (RuntimeException) (var3_ref);

            stackIn_12_1 = new StringBuilder().append("tc.I(").append(param0).append(',');

            if (param1 == null) {
              stackIn_13_0 = (RuntimeException) ((Object) stackIn_12_0);
              stackIn_13_1 = (StringBuilder) ((Object) stackIn_12_1);
              stackIn_13_2 = "null";
              break L4;
            } else {
              stackIn_13_0 = (RuntimeException) ((Object) stackIn_12_0);
              stackIn_13_1 = (StringBuilder) ((Object) stackIn_12_1);
              stackIn_13_2 = "{...}";
              break L4;
            }
          }
          throw ma.a((Throwable) ((Object) stackIn_13_0), stackIn_13_2 + ')');
        }
    }

    final static void d(int param0) {
        if (param0 != 1) {
            field_p = 121;
        }
    }

    final static int a(int param0, byte param1) {
        int var2;
        int var3;
        L0: {
          var3 = stellarshard.field_B;
          if (param1 >= 124) {
            break L0;
          } else {
            field_q = (String) null;
            break L0;
          }
        }
        L1: {
          kc.field_b = 0;
          k.field_b = null;
          dl.field_A = null;
          var2 = ll.field_b;
          ll.field_b = l.field_b;
          od.field_v.field_i = od.field_v.field_i + 1;
          if ((param0 ^ -1) == -52) {
            od.field_v.field_g = 2;
            break L1;
          } else {
            if (-51 != (param0 ^ -1)) {
              od.field_v.field_g = 1;
              break L1;
            } else {
              od.field_v.field_g = 5;
              break L1;
            }
          }
        }
        L2: {
          l.field_b = var2;
          if (-3 < (od.field_v.field_i ^ -1)) {
            break L2;
          } else {
            if (51 != param0) {
              break L2;
            } else {
              return 2;
            }
          }
        }
        L3: {
          if (2 > od.field_v.field_i) {
            break L3;
          } else {
            if (param0 == 50) {
              return 5;
            } else {
              break L3;
            }
          }
        }
        if ((od.field_v.field_i ^ -1) <= -5) {
          return 1;
        } else {
          return -1;
        }
    }

    public tc() {
        this.field_x = new db[6];
        this.field_t = new db();
        db dupTemp$0 = new db();
        this.field_x[0] = dupTemp$0;
        db var1 = dupTemp$0;
        var1.a((byte) 62);
    }

    final void a(int param0, int param1, pb[] param2) {
        int var4_int = 0;
        try {
            var4_int = param0;
            if (this.field_x[var4_int] == null) {
                this.field_x[var4_int] = new db();
            }
            this.field_x[param0].field_d = param2;
            var4_int = -105 % ((param1 - 56) / 32);
        } catch (RuntimeException runtimeException) {
            throw ma.a((Throwable) ((Object) runtimeException), "tc.G(" + param0 + ',' + param1 + ',' + (param2 != null ? "{...}" : "null") + ')');
        }
    }

    final void a(boolean param0, pb param1) {
        db[] var3 = null;
        int var4 = 0;
        db var5 = null;
        int var6 = 0;
        db[] var7 = null;
        RuntimeException stackIn_11_0 = null;
        StringBuilder stackIn_11_1 = null;
        RuntimeException stackIn_12_0 = null;
        StringBuilder stackIn_12_1 = null;
        String stackIn_12_2 = null;
        int decompiledRegionSelector0 = 0;
        RuntimeException decompiledCaughtException = null;
        RuntimeException var3_ref = null;
        var6 = stellarshard.field_B;
        try {
          L0: {
            var7 = this.field_x;
            var3 = var7;
            if (param0) {
              var4 = 0;
              L1: while (true) {
                if (var4 >= var7.length) {
                  decompiledRegionSelector0 = 1;
                  break L0;
                } else {
                  L2: {
                    var5 = var7[var4];
                    if (var5 == null) {
                      break L2;
                    } else {
                      var5.field_b = param1;
                      break L2;
                    }
                  }
                  var4++;
                  continue L1;
                }
              }
            } else {
              decompiledRegionSelector0 = 0;
              break L0;
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L3: {
            var3_ref = decompiledCaughtException;
            stackIn_11_0 = (RuntimeException) (var3_ref);

            stackIn_11_1 = new StringBuilder().append("tc.H(").append(param0).append(',');

            if (param1 == null) {
              stackIn_12_0 = (RuntimeException) ((Object) stackIn_11_0);
              stackIn_12_1 = (StringBuilder) ((Object) stackIn_11_1);
              stackIn_12_2 = "null";
              break L3;
            } else {
              stackIn_12_0 = (RuntimeException) ((Object) stackIn_11_0);
              stackIn_12_1 = (StringBuilder) ((Object) stackIn_11_1);
              stackIn_12_2 = "{...}";
              break L3;
            }
          }
          throw ma.a((Throwable) ((Object) stackIn_12_0), stackIn_12_2 + ')');
        }
        if (decompiledRegionSelector0 == 0) {
          return;
        } else {
          return;
        }
    }

    public final void a(boolean param0, int param1, int param2, rj param3, int param4) {
        rj stackIn_3_0 = null;
        RuntimeException stackIn_33_0 = null;
        StringBuilder stackIn_33_1 = null;
        RuntimeException stackIn_34_0 = null;
        StringBuilder stackIn_34_1 = null;
        String stackIn_34_2 = null;
        RuntimeException decompiledCaughtException = null;
        RuntimeException var6 = null;
        db var7 = null;
        db var9 = null;
        ig var10 = null;
        db var11 = null;
        db var12 = null;
        db var13 = null;
        db var14 = null;
        try {
          L0: {
            L1: {
              if (param3 instanceof ig) {
                stackIn_3_0 = (rj) (param3);
                break L1;
              } else {
                stackIn_3_0 = null;
                break L1;
              }
            }
            L2: {
              var10 = (ig) ((Object) stackIn_3_0);
              if (param4 == 4549) {
                break L2;
              } else {
                tc.c(22);
                break L2;
              }
            }
            L3: {
              gg.a(126, param3.field_p + (param1 + param3.field_w), param3.field_k + param2, param2 - -param3.field_k - -param3.field_t, param1 - -param3.field_w);
              if (var10 == null) {
                break L3;
              } else {
                param0 = param0 & var10.field_E;
                break L3;
              }
            }
            L4: {
              var7 = this.field_x[0];
              this.field_t.a((byte) 76);
              var7.a((tc) (this), this.field_t, param4 ^ 4584, param2, param3, param1);
              if (var10 != null) {
                L5: {
                  if (!var10.field_z) {
                    break L5;
                  } else {
                    var11 = this.field_x[1];
                    if (var11 != null) {
                      var11.a((tc) (this), this.field_t, 94, param2, param3, param1);
                      break L5;
                    } else {
                      break L5;
                    }
                  }
                }
                if (!var10.field_q) {
                  break L4;
                } else {
                  L6: {
                    var12 = this.field_x[3];
                    if (-1 == (var10.field_o ^ -1)) {
                      break L6;
                    } else {
                      if (var12 != null) {
                        var12.a((tc) (this), this.field_t, 126, param2, param3, param1);
                        break L4;
                      } else {
                        break L6;
                      }
                    }
                  }
                  var9 = this.field_x[2];
                  if (var9 == null) {
                    break L4;
                  } else {
                    var9.a((tc) (this), this.field_t, param4 ^ -4525, param2, param3, param1);
                    break L4;
                  }
                }
              } else {
                break L4;
              }
            }
            L7: {
              if (param3.e(0)) {
                var13 = this.field_x[5];
                if (var13 != null) {
                  var13.a((tc) (this), this.field_t, 99, param2, param3, param1);
                  break L7;
                } else {
                  break L7;
                }
              } else {
                break L7;
              }
            }
            L8: {
              if (param0) {
                break L8;
              } else {
                var14 = this.field_x[4];
                if (var14 != null) {
                  var14.a((tc) (this), this.field_t, -122, param2, param3, param1);
                  break L8;
                } else {
                  break L8;
                }
              }
            }
            this.field_t.a(param2, (tc) (this), param3, param4 + -4547, param1);
            id.b((byte) -58);
            break L0;
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L9: {
            var6 = decompiledCaughtException;
            stackIn_33_0 = (RuntimeException) (var6);

            stackIn_33_1 = new StringBuilder().append("tc.A(").append(param0).append(',').append(param1).append(',').append(param2).append(',');

            if (param3 == null) {
              stackIn_34_0 = (RuntimeException) ((Object) stackIn_33_0);
              stackIn_34_1 = (StringBuilder) ((Object) stackIn_33_1);
              stackIn_34_2 = "null";
              break L9;
            } else {
              stackIn_34_0 = (RuntimeException) ((Object) stackIn_33_0);
              stackIn_34_1 = (StringBuilder) ((Object) stackIn_33_1);
              stackIn_34_2 = "{...}";
              break L9;
            }
          }
          throw ma.a((Throwable) ((Object) stackIn_34_0), stackIn_34_2 + ',' + param4 + ')');
        }
    }

    private final void a(boolean param0, int param1, tc param2) {
        db dupTemp$2 = null;
        int var4_int = 0;
        int var5 = 0;
        db var6 = null;
        db var7 = null;
        int var8 = 0;
        db stackIn_8_0 = null;
        db stackIn_9_0 = null;
        db stackIn_9_1 = null;
        RuntimeException stackIn_15_0 = null;
        StringBuilder stackIn_15_1 = null;
        RuntimeException stackIn_16_0 = null;
        StringBuilder stackIn_16_1 = null;
        String stackIn_16_2 = null;
        RuntimeException decompiledCaughtException = null;
        RuntimeException var4 = null;
        var8 = stellarshard.field_B;
        try {
          L0: {
            L1: {
              var4_int = -94 % ((6 - param1) / 63);
              super.a(param2, (byte) -34);
              if (!param0) {
                ri.a(this.field_x, 0, param2.field_x, 0, 6);
                break L1;
              } else {
                var5 = 0;
                L2: while (true) {
                  if ((var5 ^ -1) <= -7) {
                    break L1;
                  } else {
                    L3: {
                      var6 = this.field_x[var5];
                      if (var6 != null) {
                        L4: {
                          var7 = param2.field_x[var5];
                          stackIn_8_0 = (db) (var6);

                          if (var7 == null) {
                            dupTemp$2 = new db();
                            param2.field_x[var5] = dupTemp$2;
                            stackIn_9_0 = (db) ((Object) stackIn_8_0);
                            stackIn_9_1 = (db) (dupTemp$2);
                            break L4;
                          } else {
                            stackIn_9_0 = (db) ((Object) stackIn_8_0);
                            stackIn_9_1 = (db) (var7);
                            break L4;
                          }
                        }
                        ((db) (Object) stackIn_9_0).a(stackIn_9_1, true);
                        break L3;
                      } else {
                        param2.field_x[var5] = null;
                        break L3;
                      }
                    }
                    var5++;
                    continue L2;
                  }
                }
              }
            }
            break L0;
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L5: {
            var4 = decompiledCaughtException;
            stackIn_15_0 = (RuntimeException) (var4);

            stackIn_15_1 = new StringBuilder().append("tc.K(").append(param0).append(',').append(param1).append(',');

            if (param2 == null) {
              stackIn_16_0 = (RuntimeException) ((Object) stackIn_15_0);
              stackIn_16_1 = (StringBuilder) ((Object) stackIn_15_1);
              stackIn_16_2 = "null";
              break L5;
            } else {
              stackIn_16_0 = (RuntimeException) ((Object) stackIn_15_0);
              stackIn_16_1 = (StringBuilder) ((Object) stackIn_15_1);
              stackIn_16_2 = "{...}";
              break L5;
            }
          }
          throw ma.a((Throwable) ((Object) stackIn_16_0), stackIn_16_2 + ')');
        }
    }

    static {
        field_v = 0;
        field_q = "Continue";
    }
}
