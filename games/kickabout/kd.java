/*
 * Decompiled by CFR-JS 0.4.0.
 */
import java.util.*;

final class kd extends be {
    static String field_w;
    private tm[] field_t;
    static ut[][] field_y;
    static ff field_r;
    static boolean field_s;
    static String field_x;
    static String field_u;
    static String field_v;
    static hd field_z;
    static String field_A;

    final boolean a(String param0, boolean param1) {
        try {
            lk var5 = null;
            tf var6 = null;
            q var7 = null;
            int stackIn_7_0 = 0;
            int stackIn_10_0 = 0;
            RuntimeException stackIn_14_0 = null;
            StringBuilder stackIn_14_1 = null;
            RuntimeException stackIn_15_0 = null;
            StringBuilder stackIn_15_1 = null;
            String stackIn_15_2 = null;
            Throwable decompiledCaughtException = null;
            fi var3 = null;
            dw var3_ref = null;
            RuntimeException var3_ref2 = null;
            hd var4 = null;
            try {
              L0: {
                try {
                  L1: {
                    L2: {
                      var6 = fj.a(false, param0);
                      if (param1) {
                        break L2;
                      } else {
                        this.b(5);
                        break L2;
                      }
                    }
                    var7 = new q(var6);
                    var5 = jh.a(var7, 1422);
                    if (!var7.a(6)) {
                      throw new fi(var7.a((byte) 114).field_f);
                    } else {
                      this.a(-98, var5.a(param1, this.field_t));
                      stackIn_7_0 = 1;
                      break L1;
                    }
                  }
                } catch (fi decompiledCaughtParameter0) {
                  decompiledCaughtException = decompiledCaughtParameter0;
                  var3 = (fi) (Object) decompiledCaughtException;
                  param0 = "<col=55ff55>" + param0.substring(0, var3.field_f) + "</col>" + param0.substring(var3.field_f);
                  var4 = (hd) ((Object) this.field_g.field_Ab.field_jb.d(-9033));
                  var4.field_E = this.field_a + param0;
                  return false;
                } catch (dw decompiledCaughtParameter1) {
                  decompiledCaughtException = decompiledCaughtParameter1;
                  var3_ref = (dw) (Object) decompiledCaughtException;
                  this.a(var3_ref.field_c, (byte) 94);
                  stackIn_10_0 = 0;
                  return stackIn_10_0 != 0;
                }
                break L0;
              }
            } catch (java.lang.RuntimeException decompiledCaughtParameter2) {
              decompiledCaughtException = decompiledCaughtParameter2;
              L3: {
                var3_ref2 = (RuntimeException) (Object) decompiledCaughtException;
                stackIn_14_0 = (RuntimeException) (var3_ref2);

                stackIn_14_1 = new StringBuilder().append("kd.E(");

                if (param0 == null) {
                  stackIn_15_0 = (RuntimeException) ((Object) stackIn_14_0);
                  stackIn_15_1 = (StringBuilder) ((Object) stackIn_14_1);
                  stackIn_15_2 = "null";
                  break L3;
                } else {
                  stackIn_15_0 = (RuntimeException) ((Object) stackIn_14_0);
                  stackIn_15_1 = (StringBuilder) ((Object) stackIn_14_1);
                  stackIn_15_2 = "{...}";
                  break L3;
                }
              }
              throw nb.a((Throwable) ((Object) stackIn_15_0), stackIn_15_2 + ',' + param1 + ')');
            }
            return stackIn_7_0 != 0;
        } catch (RuntimeException | Error decompiledUncheckedException) {
            throw decompiledUncheckedException;
        } catch (Throwable decompiledCheckedException) {
            throw new RuntimeException(decompiledCheckedException);
        }
    }

    final static boolean a(char param0, byte param1) {
        char[] var2 = null;
        int var3 = 0;
        int var4 = 0;
        int var5 = 0;
        char[] var6 = null;
        int stackIn_9_0 = 0;
        int stackIn_17_0 = 0;
        int stackIn_23_0 = 0;
        int decompiledRegionSelector0 = 0;
        RuntimeException decompiledCaughtException = null;
        RuntimeException var2_ref = null;
        var5 = Kickabout.field_G;
        try {
          L0: {
            L1: {
              L2: {
                if (param0 <= 0) {
                  break L2;
                } else {
                  if (128 > param0) {
                    break L1;
                  } else {
                    break L2;
                  }
                }
              }
              L3: {
                if (param0 < 160) {
                  break L3;
                } else {
                  if (255 >= param0) {
                    break L1;
                  } else {
                    break L3;
                  }
                }
              }
              L4: {
                if (param0 == 0) {
                  break L4;
                } else {
                  var6 = ts.field_c;
                  var2 = var6;
                  var3 = 0;
                  L5: while (true) {
                    if (var3 >= var6.length) {
                      break L4;
                    } else {
                      var4 = var6[var3];
                      if ((param0 ^ -1) == (var4 ^ -1)) {
                        stackIn_17_0 = 1;
                        decompiledRegionSelector0 = 1;
                        break L0;
                      } else {
                        var3++;
                        continue L5;
                      }
                    }
                  }
                }
              }
              if (param1 >= 63) {
                stackIn_23_0 = 0;
                decompiledRegionSelector0 = 2;
                break L0;
              } else {
                kd.a('{', (byte) -51);
                return false;
              }
            }
            stackIn_9_0 = 1;
            decompiledRegionSelector0 = 0;
            break L0;
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          var2_ref = decompiledCaughtException;
          throw nb.a((Throwable) ((Object) var2_ref), "kd.BA(" + param0 + ',' + param1 + ')');
        }
        if (decompiledRegionSelector0 == 0) {
          return stackIn_9_0 != 0;
        } else {
          if (decompiledRegionSelector0 == 1) {
            return stackIn_17_0 != 0;
          } else {
            return stackIn_23_0 != 0;
          }
        }
    }

    final String[] b(byte param0) {
        int incrementValue$0 = 0;
        String[] stackIn_23_0 = null;
        RuntimeException decompiledCaughtException = null;
        RuntimeException var2 = null;
        int var3 = 0;
        lk[] var4 = null;
        int var5 = 0;
        String var6 = null;
        int var8_int = 0;
        String[] var8 = null;
        int var9 = 0;
        int var10 = 0;
        tm var11 = null;
        int var12 = 0;
        String var13 = null;
        lk var14 = null;
        o var15 = null;
        var12 = Kickabout.field_G;
        try {
          L0: {
            L1: {
              var13 = this.field_k.j((byte) -16);
              var6 = var13;
              var6 = var13;
              var3 = this.field_k.j(-1);
              var4 = w.a(var13, (byte) -81);
              var5 = mi.a(var3, var4, 1);
              this.field_j.field_s = 0;
              var6 = "";
              if ((var5 ^ -1) > -1) {
                break L1;
              } else {
                L2: {
                  var14 = var4[var5];
                  var8_int = -var14.field_f + var3;
                  if (var14.field_k != vu.field_Ab) {
                    break L2;
                  } else {
                    if ((var8_int ^ -1) >= -1) {
                      break L2;
                    } else {
                      var6 = var14.field_h.substring(0, Math.min(var14.field_h.length(), var8_int));
                      this.field_j.field_s = this.field_d.a(var13.substring(0, var14.field_f));
                      break L1;
                    }
                  }
                }
                var9 = var5;
                var10 = 0;
                L3: while (true) {
                  L4: {
                    if ((var9 ^ -1) > -1) {
                      break L4;
                    } else {
                      L5: {
                        if ((var4[var9].field_k ^ -1) != (og.field_c ^ -1)) {
                          break L5;
                        } else {
                          var10++;
                          break L5;
                        }
                      }
                      L6: {
                        if (si.field_g != var4[var9].field_k) {
                          break L6;
                        } else {
                          var10--;
                          break L6;
                        }
                      }
                      if (-1 < (var10 ^ -1)) {
                        break L4;
                      } else {
                        var9--;
                        continue L3;
                      }
                    }
                  }
                  if ((var9 ^ -1) >= -1) {
                    break L1;
                  } else {
                    if (var4[var9 - 1].field_k == vu.field_Ab) {
                      var6 = var4[var9 + -1].field_h;
                      this.field_j.field_s = this.field_d.a(var13.substring(0, var4[var9 + -1].field_f));
                      break L1;
                    } else {
                      break L1;
                    }
                  }
                }
              }
            }
            var15 = this.b(var6, (byte) -91);
            var8 = new String[var15.c(-1976587888)];
            var9 = 0;
            var10 = 43 % ((-2 - param0) / 41);
            var11 = (tm) ((Object) var15.a(-124));
            L7: while (true) {
              if (var11 == null) {
                stackIn_23_0 = (String[]) (var8);
                break L0;
              } else {
                incrementValue$0 = var9;
                var9++;
                var8[incrementValue$0] = var11.field_s;
                var11 = (tm) ((Object) var15.b(0));
                continue L7;
              }
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          var2 = decompiledCaughtException;
          throw nb.a((Throwable) ((Object) var2), "kd.D(" + param0 + ')');
        }
        return stackIn_23_0;
    }

    kd(tf param0, hu param1, bu param2) {
        super(param1, param2);
        int incrementValue$3 = 0;
        int var4_int = 0;
        tm var5 = null;
        RuntimeException stackIn_10_0 = null;
        StringBuilder stackIn_10_1 = null;
        RuntimeException stackIn_11_0 = null;
        StringBuilder stackIn_11_1 = null;
        String stackIn_11_2 = null;
        RuntimeException stackIn_14_0 = null;
        StringBuilder stackIn_14_1 = null;
        RuntimeException stackIn_15_0 = null;
        StringBuilder stackIn_15_1 = null;
        String stackIn_15_2 = null;
        RuntimeException stackIn_18_0 = null;
        StringBuilder stackIn_18_1 = null;
        RuntimeException stackIn_19_0 = null;
        StringBuilder stackIn_19_1 = null;
        String stackIn_19_2 = null;
        RuntimeException decompiledCaughtException = null;
        RuntimeException var4 = null;
        try {
          L0: {
            this.field_t = new tm[param0.a(false)];
            var4_int = 0;
            var5 = (tm) ((Object) param0.g(24009));
            L1: while (true) {
              if (var5 == null) {
                this.a("# <col=ffaaff>FunOrbConsole (loaded " + this.field_t.length + " methods at " + new Date() + ")</col>", (byte) 94);
                break L0;
              } else {
                incrementValue$3 = var4_int;
                var4_int++;
                this.field_t[incrementValue$3] = var5;
                var5 = (tm) ((Object) param0.c(33));
                continue L1;
              }
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L2: {
            var4 = decompiledCaughtException;
            stackIn_10_0 = (RuntimeException) (var4);

            stackIn_10_1 = new StringBuilder().append("kd.<init>(");

            if (param0 == null) {
              stackIn_11_0 = (RuntimeException) ((Object) stackIn_10_0);
              stackIn_11_1 = (StringBuilder) ((Object) stackIn_10_1);
              stackIn_11_2 = "null";
              break L2;
            } else {
              stackIn_11_0 = (RuntimeException) ((Object) stackIn_10_0);
              stackIn_11_1 = (StringBuilder) ((Object) stackIn_10_1);
              stackIn_11_2 = "{...}";
              break L2;
            }
          }
          L3: {
            stackIn_14_0 = (RuntimeException) ((Object) stackIn_11_0);

            stackIn_14_1 = ((StringBuilder) (Object) stackIn_11_1).append(stackIn_11_2).append(',');

            if (param1 == null) {
              stackIn_15_0 = (RuntimeException) ((Object) stackIn_14_0);
              stackIn_15_1 = (StringBuilder) ((Object) stackIn_14_1);
              stackIn_15_2 = "null";
              break L3;
            } else {
              stackIn_15_0 = (RuntimeException) ((Object) stackIn_11_0);
              stackIn_15_1 = (StringBuilder) ((Object) stackIn_14_1);
              stackIn_15_2 = "{...}";
              break L3;
            }
          }
          L4: {
            stackIn_18_0 = (RuntimeException) ((Object) stackIn_15_0);

            stackIn_18_1 = ((StringBuilder) (Object) stackIn_15_1).append(stackIn_15_2).append(',');

            if (param2 == null) {
              stackIn_19_0 = (RuntimeException) ((Object) stackIn_18_0);
              stackIn_19_1 = (StringBuilder) ((Object) stackIn_18_1);
              stackIn_19_2 = "null";
              break L4;
            } else {
              stackIn_19_0 = (RuntimeException) ((Object) stackIn_15_0);
              stackIn_19_1 = (StringBuilder) ((Object) stackIn_18_1);
              stackIn_19_2 = "{...}";
              break L4;
            }
          }
          throw nb.a((Throwable) ((Object) stackIn_19_0), stackIn_19_2 + ')');
        }
    }

    public static void c(int param0) {
        try {
            field_z = null;
            field_w = null;
            field_A = null;
            field_u = null;
            if (param0 != -1) {
                kd.a('4', (byte) -128);
            }
            field_r = null;
            field_v = null;
            field_y = (ut[][]) null;
            field_x = null;
        } catch (RuntimeException runtimeException) {
            throw nb.a((Throwable) ((Object) runtimeException), "kd.V(" + param0 + ')');
        }
    }

    public final boolean a(int param0, int param1, char param2) {
        boolean discarded$0 = false;
        boolean stackIn_3_0 = false;
        int stackIn_19_0 = 0;
        int stackIn_23_0 = 0;
        int stackIn_24_0 = 0;
        int stackIn_24_1 = 0;
        int stackIn_28_0 = 0;
        int stackIn_29_0 = 0;
        int stackIn_29_1 = 0;
        int stackIn_35_0 = 0;
        int stackIn_39_0 = 0;
        int stackIn_82_0 = 0;
        int decompiledRegionSelector0 = 0;
        RuntimeException decompiledCaughtException = null;
        String var4 = null;
        RuntimeException var4_ref = null;
        int var5 = 0;
        int var6 = 0;
        int var7 = 0;
        int var8 = 0;
        int var9 = 0;
        int var10 = 0;
        int var11 = 0;
        String var12 = null;
        var11 = Kickabout.field_G;
        try {
          L0: {
            if (this.a((byte) 114)) {
              L1: {
                var12 = this.field_k.j((byte) -16);
                var5 = this.field_k.j(-1);
                var6 = 0;
                var7 = 0;
                var8 = 0;
                var9 = 0;
                if (param0 == 11516) {
                  break L1;
                } else {
                  this.b(102);
                  break L1;
                }
              }
              L2: while (true) {
                if (var9 >= var5) {
                  L3: {
                    if (var5 > 0) {
                      stackIn_35_0 = var12.charAt(var5 + -1);
                      break L3;
                    } else {
                      stackIn_35_0 = 65535;
                      break L3;
                    }
                  }
                  L4: {
                    var9 = stackIn_35_0;
                    if (var12.length() > var5) {
                      stackIn_39_0 = var12.charAt(var5);
                      break L4;
                    } else {
                      stackIn_39_0 = 65535;
                      break L4;
                    }
                  }
                  L5: {
                    L6: {
                      L7: {
                        var10 = stackIn_39_0;
                        if (var6 == 0) {
                          break L7;
                        } else {
                          if (var8 != 0) {
                            break L7;
                          } else {
                            if ((param2 ^ -1) != -35) {
                              break L7;
                            } else {
                              if ((var10 ^ -1) == -35) {
                                break L6;
                              } else {
                                break L7;
                              }
                            }
                          }
                        }
                      }
                      L8: {
                        if (var7 == 0) {
                          break L8;
                        } else {
                          if (var8 != 0) {
                            break L8;
                          } else {
                            if (param2 != 39) {
                              break L8;
                            } else {
                              if ((var10 ^ -1) == -40) {
                                break L6;
                              } else {
                                break L8;
                              }
                            }
                          }
                        }
                      }
                      L9: {
                        if (var6 != 0) {
                          break L9;
                        } else {
                          if (var7 != 0) {
                            break L9;
                          } else {
                            if (param2 != 41) {
                              break L9;
                            } else {
                              if (var10 != 41) {
                                break L9;
                              } else {
                                break L6;
                              }
                            }
                          }
                        }
                      }
                      L10: {
                        if ((param1 ^ -1) != -86) {
                          break L10;
                        } else {
                          L11: {
                            L12: {
                              if (var9 != 40) {
                                break L12;
                              } else {
                                if (41 == var10) {
                                  break L11;
                                } else {
                                  break L12;
                                }
                              }
                            }
                            L13: {
                              if (var6 == 0) {
                                break L13;
                              } else {
                                if (34 != var9) {
                                  break L13;
                                } else {
                                  if ((var10 ^ -1) == -35) {
                                    break L11;
                                  } else {
                                    break L13;
                                  }
                                }
                              }
                            }
                            if (var7 == 0) {
                              break L10;
                            } else {
                              if (var9 != 39) {
                                break L10;
                              } else {
                                if (var10 != 39) {
                                  break L10;
                                } else {
                                  break L11;
                                }
                              }
                            }
                          }
                          this.field_k.a(101, '￿', 875);
                          break L10;
                        }
                      }
                      L14: {
                        discarded$0 = super.a(11516, param1, param2);
                        var4 = this.field_k.j((byte) -16);
                        var5 = this.field_k.j(-1);
                        if (var6 != 0) {
                          break L14;
                        } else {
                          if (var7 != 0) {
                            break L14;
                          } else {
                            if (param2 != 40) {
                              break L14;
                            } else {
                              this.field_k.a(-1, ')', param0 ^ 12183);
                              this.field_k.a(var5, (byte) -96, var5);
                              break L14;
                            }
                          }
                        }
                      }
                      L15: {
                        if (var6 != 0) {
                          break L15;
                        } else {
                          if (var7 != 0) {
                            break L15;
                          } else {
                            if (param2 != 34) {
                              break L15;
                            } else {
                              this.field_k.a(-1, '"', 875);
                              this.field_k.a(var5, (byte) -97, var5);
                              break L15;
                            }
                          }
                        }
                      }
                      if (var6 != 0) {
                        break L5;
                      } else {
                        if (var7 != 0) {
                          break L5;
                        } else {
                          if ((param2 ^ -1) != -40) {
                            break L5;
                          } else {
                            this.field_k.a(-1, '\'', 875);
                            this.field_k.a(var5, (byte) -94, var5);
                            break L5;
                          }
                        }
                      }
                    }
                    this.field_k.f(-85, 1 + var5);
                    break L5;
                  }
                  cv.field_c = aq.a(this.field_k.j((byte) -16), 120);
                  stackIn_82_0 = 1;
                  decompiledRegionSelector0 = 1;
                  break L0;
                } else {
                  L16: {
                    if (var8 != 0) {
                      var8 = 0;
                      break L16;
                    } else {
                      L17: {
                        L18: {
                          var10 = var12.charAt(var9);
                          if (var6 != 0) {
                            break L18;
                          } else {
                            if (var7 != 0) {
                              break L18;
                            } else {
                              stackIn_19_0 = 0;
                              break L17;
                            }
                          }
                        }
                        if (var10 == 92) {
                          stackIn_19_0 = 1;
                          break L17;
                        } else {
                          stackIn_19_0 = 0;
                          break L17;
                        }
                      }
                      L19: {
                        L20: {
                          var8 = stackIn_19_0;
                          stackIn_23_0 = var6;

                          if (var7 != 0) {
                            break L20;
                          } else {

                            if ((var10 ^ -1) != -35) {
                              break L20;
                            } else {
                              stackIn_24_0 = stackIn_23_0;
                              stackIn_24_1 = 1;
                              break L19;
                            }
                          }
                        }
                        stackIn_24_0 = stackIn_23_0;
                        stackIn_24_1 = 0;
                        break L19;
                      }
                      L21: {
                        L22: {
                          var6 = stackIn_24_0 ^ stackIn_24_1;
                          stackIn_28_0 = var7;

                          if (var6 != 0) {
                            break L22;
                          } else {

                            if (var10 != 39) {
                              break L22;
                            } else {
                              stackIn_29_0 = stackIn_28_0;
                              stackIn_29_1 = 1;
                              break L21;
                            }
                          }
                        }
                        stackIn_29_0 = stackIn_28_0;
                        stackIn_29_1 = 0;
                        break L21;
                      }
                      var7 = stackIn_29_0 ^ stackIn_29_1;
                      break L16;
                    }
                  }
                  var9++;
                  continue L2;
                }
              }
            } else {
              stackIn_3_0 = super.a(11516, param1, param2);
              decompiledRegionSelector0 = 0;
              break L0;
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          var4_ref = decompiledCaughtException;
          throw nb.a((Throwable) ((Object) var4_ref), "kd.T(" + param0 + ',' + param1 + ',' + param2 + ')');
        }
        if (decompiledRegionSelector0 == 0) {
          return stackIn_3_0;
        } else {
          return stackIn_82_0 != 0;
        }
    }

    private final o b(String param0, byte param1) {
        o var3 = null;
        int var4 = 0;
        int var5 = 0;
        int var6 = 0;
        o var7 = null;
        o stackIn_9_0 = null;
        RuntimeException stackIn_13_0 = null;
        StringBuilder stackIn_13_1 = null;
        RuntimeException stackIn_14_0 = null;
        StringBuilder stackIn_14_1 = null;
        String stackIn_14_2 = null;
        RuntimeException decompiledCaughtException = null;
        RuntimeException var3_ref = null;
        var6 = Kickabout.field_G;
        try {
          L0: {
            var4 = 117 / ((param1 - 27) / 48);
            var7 = new o();
            var3 = var7;
            var5 = 0;
            L1: while (true) {
              if (this.field_t.length <= var5) {
                stackIn_9_0 = (o) (var3);
                break L0;
              } else {
                L2: {
                  if (this.field_t[var5].field_o.startsWith(param0)) {
                    this.field_t[var5].f(47);
                    var7.a(127, this.field_t[var5]);
                    break L2;
                  } else {
                    break L2;
                  }
                }
                var5++;
                continue L1;
              }
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L3: {
            var3_ref = decompiledCaughtException;
            stackIn_13_0 = (RuntimeException) (var3_ref);

            stackIn_13_1 = new StringBuilder().append("kd.U(");

            if (param0 == null) {
              stackIn_14_0 = (RuntimeException) ((Object) stackIn_13_0);
              stackIn_14_1 = (StringBuilder) ((Object) stackIn_13_1);
              stackIn_14_2 = "null";
              break L3;
            } else {
              stackIn_14_0 = (RuntimeException) ((Object) stackIn_13_0);
              stackIn_14_1 = (StringBuilder) ((Object) stackIn_13_1);
              stackIn_14_2 = "{...}";
              break L3;
            }
          }
          throw nb.a((Throwable) ((Object) stackIn_14_0), stackIn_14_2 + ',' + param1 + ')');
        }
        return stackIn_9_0;
    }

    final void b(int param0) {
        StringBuilder discarded$0 = null;
        StringBuilder discarded$1 = null;
        int stackIn_26_0 = 0;
        int stackIn_27_0 = 0;
        int stackIn_27_1 = 0;
        int decompiledRegionSelector0 = 0;
        RuntimeException decompiledCaughtException = null;
        RuntimeException var2 = null;
        int var3 = 0;
        lk[] var4 = null;
        int var5 = 0;
        String var6 = null;
        int var8_int = 0;
        StringBuilder var8 = null;
        int var9 = 0;
        int var10 = 0;
        int var11 = 0;
        int var12 = 0;
        tm var13_ref_tm = null;
        int var13 = 0;
        tm var14 = null;
        int var15 = 0;
        String var16 = null;
        lk var17 = null;
        o var18 = null;
        var15 = Kickabout.field_G;
        try {
          L0: {
            L1: {
              var16 = this.field_k.j((byte) -16);
              var6 = var16;
              var6 = var16;
              var3 = this.field_k.j(-1);
              var4 = w.a(var16, (byte) -33);
              var5 = mi.a(var3, var4, 1);
              var6 = "";
              if (var5 >= 0) {
                var17 = var4[var5];
                var8_int = var3 + -var17.field_f;
                if (var17.field_k != vu.field_Ab) {
                  break L1;
                } else {
                  if (0 >= var8_int) {
                    break L1;
                  } else {
                    if (var8_int <= var17.field_h.length()) {
                      var6 = var17.field_h.substring(0, var8_int);
                      break L1;
                    } else {
                      break L1;
                    }
                  }
                }
              } else {
                break L1;
              }
            }
            var18 = this.b(var6, (byte) -124);
            if (0 == var18.c(-1976587888)) {
              decompiledRegionSelector0 = 0;
              break L0;
            } else {
              var8 = new StringBuilder();
              var11 = -93 / ((-44 - param0) / 36);
              var9 = var6.length();
              var10 = 1;
              var12 = 2147483647;
              var13_ref_tm = (tm) ((Object) var18.a(-120));
              L2: while (true) {
                if (var13_ref_tm == null) {
                  L3: while (true) {
                    L4: {
                      if (var10 == 0) {
                        break L4;
                      } else {
                        if ((var9 ^ -1) <= (var12 ^ -1)) {
                          break L4;
                        } else {
                          var13 = ((tm) ((Object) var18.a(-119))).field_o.charAt(var9);
                          var14 = (tm) ((Object) var18.b(0));
                          L5: while (true) {
                            if (var14 == null) {
                              var9++;
                              if (var10 != 0) {
                                discarded$0 = var8.append((char) var13);
                                continue L3;
                              } else {
                                continue L3;
                              }
                            } else {
                              L6: {
                                stackIn_26_0 = var10;

                                if (var13 != var14.field_o.charAt(var9)) {
                                  stackIn_27_0 = stackIn_26_0;
                                  stackIn_27_1 = 0;
                                  break L6;
                                } else {
                                  stackIn_27_0 = stackIn_26_0;
                                  stackIn_27_1 = 1;
                                  break L6;
                                }
                              }
                              var10 = stackIn_27_0 & stackIn_27_1;
                              var14 = (tm) ((Object) var18.b(0));
                              continue L5;
                            }
                          }
                        }
                      }
                    }
                    L7: {
                      var13 = var8.length() + var3;
                      if (var12 != var6.length() + var8.length()) {
                        break L7;
                      } else {
                        discarded$1 = var8.append("()");
                        var13++;
                        if (1 != var18.c(-1976587888)) {
                          break L7;
                        } else {
                          if (-1 != (((tm) ((Object) var18.a(-121))).field_p.length ^ -1)) {
                            break L7;
                          } else {
                            var13++;
                            break L7;
                          }
                        }
                      }
                    }
                    this.field_k.b(var16.substring(0, var3) + var8.toString() + var16.substring(var3), -23174);
                    this.field_k.f(-56, var13);
                    decompiledRegionSelector0 = 1;
                    break L0;
                  }
                } else {
                  L8: {
                    if (var12 <= var13_ref_tm.field_o.length()) {
                      break L8;
                    } else {
                      var12 = var13_ref_tm.field_o.length();
                      break L8;
                    }
                  }
                  var13_ref_tm = (tm) ((Object) var18.b(0));
                  continue L2;
                }
              }
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          var2 = decompiledCaughtException;
          throw nb.a((Throwable) ((Object) var2), "kd.R(" + param0 + ')');
        }
        if (decompiledRegionSelector0 == 0) {
          return;
        } else {
          return;
        }
    }

    final static void a(int param0, qb param1, boolean param2, String param3) {
        RuntimeException stackIn_17_0 = null;
        StringBuilder stackIn_17_1 = null;
        RuntimeException stackIn_18_0 = null;
        StringBuilder stackIn_18_1 = null;
        String stackIn_18_2 = null;
        RuntimeException stackIn_21_0 = null;
        StringBuilder stackIn_21_1 = null;
        RuntimeException stackIn_22_0 = null;
        StringBuilder stackIn_22_1 = null;
        String stackIn_22_2 = null;
        RuntimeException decompiledCaughtException = null;
        RuntimeException var4 = null;
        int var5 = 0;
        int var6 = 0;
        uv var7 = null;
        String var8 = null;
        uv var9 = null;
        String var10 = null;
        uv var11 = null;
        String var12 = null;
        uv var13 = null;
        try {
          L0: {
            L1: {
              el.a(param1, false, sp.field_Jb, -1, (String) null, (int[]) null, 0L, (String) null, param1.h((byte) 77));
              if (!param1.field_Cb) {
                break L1;
              } else {
                L2: {
                  if (param1.field_Mb == 2) {
                    break L2;
                  } else {
                    if (2 <= tk.field_Ib) {
                      break L2;
                    } else {
                      break L1;
                    }
                  }
                }
                var7 = wi.field_v;
                var8 = vo.a((byte) -18, ej.field_Ab, new String[]{param3});
                var7.field_j.a(0, 10, var8);
                break L1;
              }
            }
            L3: {
              wi.field_v.d(param0 + -14);
              if (!param2) {
                break L3;
              } else {
                if (wi.field_v.field_c != jm.field_h) {
                  var11 = wi.field_v;
                  var12 = vo.a((byte) -18, br.field_a, new String[]{param3});
                  var11.field_j.a(param0 + -15, 15, var12);
                  break L3;
                } else {
                  var9 = wi.field_v;
                  var10 = vo.a((byte) -18, cl.field_h, new String[]{param3});
                  var9.field_j.a(0, 16, var10);
                  break L3;
                }
              }
            }
            L4: {
              var13 = wi.field_v;
              var5 = sm.field_q;
              if (param0 == 15) {
                break L4;
              } else {
                field_x = (String) null;
                break L4;
              }
            }
            var6 = vc.field_y;
            var13.field_j.a(0, 0, var5, var6, 0);
            break L0;
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L5: {
            var4 = decompiledCaughtException;
            stackIn_17_0 = (RuntimeException) (var4);

            stackIn_17_1 = new StringBuilder().append("kd.AA(").append(param0).append(',');

            if (param1 == null) {
              stackIn_18_0 = (RuntimeException) ((Object) stackIn_17_0);
              stackIn_18_1 = (StringBuilder) ((Object) stackIn_17_1);
              stackIn_18_2 = "null";
              break L5;
            } else {
              stackIn_18_0 = (RuntimeException) ((Object) stackIn_17_0);
              stackIn_18_1 = (StringBuilder) ((Object) stackIn_17_1);
              stackIn_18_2 = "{...}";
              break L5;
            }
          }
          L6: {
            stackIn_21_0 = (RuntimeException) ((Object) stackIn_18_0);

            stackIn_21_1 = ((StringBuilder) (Object) stackIn_18_1).append(stackIn_18_2).append(',').append(param2).append(',');

            if (param3 == null) {
              stackIn_22_0 = (RuntimeException) ((Object) stackIn_21_0);
              stackIn_22_1 = (StringBuilder) ((Object) stackIn_21_1);
              stackIn_22_2 = "null";
              break L6;
            } else {
              stackIn_22_0 = (RuntimeException) ((Object) stackIn_18_0);
              stackIn_22_1 = (StringBuilder) ((Object) stackIn_21_1);
              stackIn_22_2 = "{...}";
              break L6;
            }
          }
          throw nb.a((Throwable) ((Object) stackIn_22_0), stackIn_22_2 + ')');
        }
    }

    public final void a(int param0) {
        String var4 = null;
        try {
            var4 = this.field_k.field_Lb.field_E;
            String var2 = var4;
            this.field_k.field_Lb.field_E = cv.field_c;
            int var3 = 114 % ((param0 - 50) / 45);
            super.a(-76);
            this.field_k.field_Lb.field_E = var4;
        } catch (RuntimeException runtimeException) {
            throw nb.a((Throwable) ((Object) runtimeException), "kd.L(" + param0 + ')');
        }
    }

    final static kg[] a(int param0, int param1, boolean param2, boolean param3, int param4, int param5) {
        kg[] stackIn_9_0 = null;
        int stackIn_16_0 = 0;
        kg[] stackIn_31_0 = null;
        int decompiledRegionSelector0 = 0;
        RuntimeException decompiledCaughtException = null;
        kg[] var6 = null;
        RuntimeException var6_ref = null;
        kg var7_ref_kg = null;
        int var7 = 0;
        int var8 = 0;
        tf var9 = null;
        int var10 = 0;
        int var11 = 0;
        int var12 = 0;
        int var13 = 0;
        int var14 = 0;
        int var15 = 0;
        kg[] var16 = null;
        var15 = Kickabout.field_G;
        try {
          L0: {
            var16 = new kg[6];
            var6 = var16;
            if (!vi.field_n) {
              var7_ref_kg = pj.a(param2, -87, new tf(), 0);
              var8 = 0;
              L1: while (true) {
                if (var16.length <= var8) {
                  stackIn_9_0 = (kg[]) (var6);
                  decompiledRegionSelector0 = 0;
                  break L0;
                } else {
                  var16[var8] = var7_ref_kg;
                  var8++;
                  continue L1;
                }
              }
            } else {
              L2: {
                var7 = oe.field_p[0].field_o;
                if (!param3) {
                  break L2;
                } else {
                  kd.a('^', (byte) 107);
                  break L2;
                }
              }
              L3: {
                if (param2) {
                  stackIn_16_0 = 6;
                  break L3;
                } else {
                  stackIn_16_0 = 2;
                  break L3;
                }
              }
              var8 = stackIn_16_0;
              var9 = new tf();
              var10 = 0;
              L4: while (true) {
                if ((var10 ^ -1) <= -4) {
                  var10 = 0;
                  L5: while (true) {
                    if ((var10 ^ -1) <= -7) {
                      stackIn_31_0 = (kg[]) (var6);
                      decompiledRegionSelector0 = 1;
                      break L0;
                    } else {
                      jm.a(100, 9863, var10 * (-param1 + param0) / 6 + param1);
                      var16[var10] = pj.a(param2, -76, var9, var10);
                      var10++;
                      continue L5;
                    }
                  }
                } else {
                  var11 = 0;
                  L6: while (true) {
                    if ((var11 ^ -1) <= -10) {
                      var10++;
                      continue L4;
                    } else {
                      if ((param5 ^ -1) <= (p.a((byte) -24, 256) ^ -1)) {
                        L7: {
                          var12 = p.a((byte) -24, 5);
                          var13 = p.a((byte) -24, 5);
                          var14 = 50 * var10 + 32 - -var13;
                          if (param2) {
                            break L7;
                          } else {
                            var14 = var7 - (var14 + 12);
                            break L7;
                          }
                        }
                        var9.a(new nr(var14, 63 + (var11 * 26 + 14 * var10), var8, var12), 3);
                        var11++;
                        continue L6;
                      } else {
                        var11++;
                        continue L6;
                      }
                    }
                  }
                }
              }
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          var6_ref = decompiledCaughtException;
          throw nb.a((Throwable) ((Object) var6_ref), "kd.W(" + param0 + ',' + param1 + ',' + param2 + ',' + param3 + ',' + param4 + ',' + param5 + ')');
        }
        if (decompiledRegionSelector0 == 0) {
          return stackIn_9_0;
        } else {
          return stackIn_31_0;
        }
    }

    static {
        field_r = new ff(10, 2, 2, 0);
        field_u = "<%0> slot";
        field_s = false;
        field_v = "Please enter your date of birth to enable chat:";
        field_x = "Warning: if you quit, you will lose any game you are in the middle of!";
        field_A = "Music";
    }
}
