/*
 * Decompiled by CFR-JS 0.4.0.
 */
final class bv extends jd {
    private String field_k;
    static String field_p;
    static String field_m;
    static rk[] field_l;
    static String field_o;
    private boolean field_j;
    static String field_n;

    final rv a(String param0, int param1) {
        wn var3 = null;
        RuntimeException var3_ref = null;
        CharSequence var4 = null;
        rv stackIn_2_0 = null;
        rv stackIn_5_0 = null;
        rv stackIn_12_0 = null;
        rv stackIn_17_0 = null;
        RuntimeException stackIn_20_0 = null;
        StringBuilder stackIn_20_1 = null;
        RuntimeException stackIn_21_0 = null;
        StringBuilder stackIn_21_1 = null;
        String stackIn_21_2 = null;
        int decompiledRegionSelector0 = 0;
        RuntimeException decompiledCaughtException = null;
        try {
          L0: {
            var4 = (CharSequence) ((Object) param0);
            if (ih.a(var4, true)) {
              if (param1 == -5) {
                L1: {
                  if (!param0.equals(this.field_k)) {
                    L2: {
                      var3 = cp.a(param1 + -86, param0);
                      if (var3 == null) {
                        break L2;
                      } else {
                        if (null == var3.field_d) {
                          this.field_k = param0;
                          this.field_j = var3.field_g;
                          break L1;
                        } else {
                          break L2;
                        }
                      }
                    }
                    stackIn_12_0 = en.field_p;
                    decompiledRegionSelector0 = 2;
                    break L0;
                  } else {
                    break L1;
                  }
                }
                L3: {
                  if (this.field_j) {
                    stackIn_17_0 = he.field_yb;
                    break L3;
                  } else {
                    stackIn_17_0 = jt.field_Bb;
                    break L3;
                  }
                }
                decompiledRegionSelector0 = 3;
                break L0;
              } else {
                stackIn_5_0 = (rv) null;
                decompiledRegionSelector0 = 1;
                break L0;
              }
            } else {
              stackIn_2_0 = jt.field_Bb;
              decompiledRegionSelector0 = 0;
              break L0;
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L4: {
            var3_ref = decompiledCaughtException;
            stackIn_20_0 = (RuntimeException) (var3_ref);

            stackIn_20_1 = new StringBuilder().append("bv.E(");

            if (param0 == null) {
              stackIn_21_0 = (RuntimeException) ((Object) stackIn_20_0);
              stackIn_21_1 = (StringBuilder) ((Object) stackIn_20_1);
              stackIn_21_2 = "null";
              break L4;
            } else {
              stackIn_21_0 = (RuntimeException) ((Object) stackIn_20_0);
              stackIn_21_1 = (StringBuilder) ((Object) stackIn_20_1);
              stackIn_21_2 = "{...}";
              break L4;
            }
          }
          throw nb.a((Throwable) ((Object) stackIn_21_0), stackIn_21_2 + ',' + param1 + ')');
        }
        if (decompiledRegionSelector0 == 0) {
          return stackIn_2_0;
        } else {
          if (decompiledRegionSelector0 == 1) {
            return stackIn_5_0;
          } else {
            if (decompiledRegionSelector0 == 2) {
              return stackIn_12_0;
            } else {
              return stackIn_17_0;
            }
          }
        }
    }

    final void c(byte param0) {
        if (param0 <= 70) {
            return;
        }
        this.field_k = null;
    }

    final String a(byte param0, String param1) {
        String var3 = null;
        RuntimeException var3_ref = null;
        wn var4 = null;
        CharSequence var5 = null;
        String stackIn_5_0 = null;
        Object stackIn_12_0 = null;
        String stackIn_16_0 = null;
        String stackIn_18_0 = null;
        RuntimeException stackIn_21_0 = null;
        StringBuilder stackIn_21_1 = null;
        RuntimeException stackIn_22_0 = null;
        StringBuilder stackIn_22_1 = null;
        String stackIn_22_2 = null;
        int decompiledRegionSelector0 = 0;
        RuntimeException decompiledCaughtException = null;
        try {
          L0: {
            L1: {
              if (param0 == 25) {
                break L1;
              } else {
                field_o = (String) null;
                break L1;
              }
            }
            var5 = (CharSequence) ((Object) param1);
            var3 = hk.a(var5, false);
            if (var3 != null) {
              stackIn_5_0 = (String) (var3);
              decompiledRegionSelector0 = 0;
              break L0;
            } else {
              L2: {
                if (!param1.equals(this.field_k)) {
                  var4 = cp.a(-37, param1);
                  if (var4 != null) {
                    if (var4.field_d == null) {
                      this.field_k = param1;
                      this.field_j = var4.field_g;
                      break L2;
                    } else {
                      stackIn_12_0 = null;
                      decompiledRegionSelector0 = 1;
                      break L0;
                    }
                  } else {
                    return null;
                  }
                } else {
                  break L2;
                }
              }
              if (this.field_j) {
                stackIn_18_0 = Kickabout.field_I;
                decompiledRegionSelector0 = 3;
                break L0;
              } else {
                stackIn_16_0 = se.field_B;
                decompiledRegionSelector0 = 2;
                break L0;
              }
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L3: {
            var3_ref = decompiledCaughtException;
            stackIn_21_0 = (RuntimeException) (var3_ref);

            stackIn_21_1 = new StringBuilder().append("bv.A(").append(param0).append(',');

            if (param1 == null) {
              stackIn_22_0 = (RuntimeException) ((Object) stackIn_21_0);
              stackIn_22_1 = (StringBuilder) ((Object) stackIn_21_1);
              stackIn_22_2 = "null";
              break L3;
            } else {
              stackIn_22_0 = (RuntimeException) ((Object) stackIn_21_0);
              stackIn_22_1 = (StringBuilder) ((Object) stackIn_21_1);
              stackIn_22_2 = "{...}";
              break L3;
            }
          }
          throw nb.a((Throwable) ((Object) stackIn_22_0), stackIn_22_2 + ')');
        }
        if (decompiledRegionSelector0 == 0) {
          return stackIn_5_0;
        } else {
          if (decompiledRegionSelector0 == 1) {
            return (String) ((Object) stackIn_12_0);
          } else {
            if (decompiledRegionSelector0 == 2) {
              return stackIn_16_0;
            } else {
              return stackIn_18_0;
            }
          }
        }
    }

    final static void a(java.applet.Applet param0, String param1, byte param2) {
        try {
            String var7 = null;
            String var5 = null;
            String var4 = null;
            String var8 = null;
            try {
                lu.field_t = param1;
                try {
                    if (param2 != 15) {
                        String var6 = (String) null;
                        bv.a((java.applet.Applet) null, (String) null, (byte) 80);
                    }
                    var7 = param0.getParameter("cookieprefix");
                    var5 = var7;
                    var5 = var7;
                    var4 = param0.getParameter("cookiehost");
                    var5 = var4;
                    var5 = var4;
                    var8 = var7 + "session=" + param1 + "; version=1; path=/; domain=" + var4;
                    var5 = var8;
                    var5 = var8;
                    var5 = var8;
                    if (0 == param1.length()) {
                        var5 = var8 + "; Expires=Thu, 01-Jan-1970 00:00:00 GMT; Max-Age=0";
                    }
                    mm.a("document.cookie=\"" + var5 + "\"", param0, (byte) -19);
                } catch (Throwable throwable) {
                }
                nt.a(param0, (byte) 99);
            } catch (RuntimeException runtimeException) {
                throw nb.a((Throwable) ((Object) runtimeException), "bv.B(" + (param0 != null ? "{...}" : "null") + ',' + (param1 != null ? "{...}" : "null") + ',' + param2 + ')');
            }
        } catch (RuntimeException | Error decompiledUncheckedException) {
            throw decompiledUncheckedException;
        } catch (Throwable decompiledCheckedException) {
            throw new RuntimeException(decompiledCheckedException);
        }
    }

    final static void a(int param0, int param1, int param2, int param3, int param4, he param5, np param6) {
        uv var12 = null;
        int var8 = 0;
        int var9 = 0;
        int var10 = 0;
        int var11 = 0;
        if (param3 < 90) {
            return;
        }
        try {
            el.a(param5, false, param6, -1, param5.field_Rb, (int[]) null, param5.field_Ab, param5.field_Nb, -1);
            wi.field_v.a(true, 0);
            wi.field_v.a(0, -116, (int[]) null);
            wi.field_v.b(-80);
            var12 = wi.field_v;
            var8 = param1;
            var9 = param0;
            var10 = param4;
            var11 = param2;
            var12.field_j.a(0, var10, var8, var9, var11);
        } catch (RuntimeException runtimeException) {
            throw nb.a((Throwable) ((Object) runtimeException), "bv.C(" + param0 + ',' + param1 + ',' + param2 + ',' + param3 + ',' + param4 + ',' + (param5 != null ? "{...}" : "null") + ',' + (param6 != null ? "{...}" : "null") + ')');
        }
    }

    bv(ga param0) {
        super(param0);
        this.field_j = false;
    }

    final static void a(int param0, String param1, boolean param2, java.applet.Applet param3) {
        try {
            RuntimeException stackIn_13_0 = null;
            StringBuilder stackIn_13_1 = null;
            RuntimeException stackIn_14_0 = null;
            StringBuilder stackIn_14_1 = null;
            String stackIn_14_2 = null;
            StringBuilder stackIn_16_1 = null;
            StringBuilder stackIn_17_1 = null;
            String stackIn_17_2 = null;
            int decompiledRegionSelector0 = 0;
            Throwable decompiledCaughtException = null;
            java.net.MalformedURLException var4 = null;
            RuntimeException var4_ref = null;
            try {
              L0: {
                if (param0 == 0) {
                  L1: {
                    if (bu.field_h.startsWith("win")) {
                      if (fv.a((byte) -92, param1)) {
                        decompiledRegionSelector0 = 1;
                        break L0;
                      } else {
                        break L1;
                      }
                    } else {
                      break L1;
                    }
                  }
                  try {
                    L2: {
                      param3.getAppletContext().showDocument(new java.net.URL(param1), "_blank");
                      break L2;
                    }
                  } catch (java.net.MalformedURLException decompiledCaughtParameter0) {
                    decompiledCaughtException = decompiledCaughtParameter0;
                    L3: {
                      var4 = (java.net.MalformedURLException) (Object) decompiledCaughtException;
                      bd.a("MGR1: " + param1, (Throwable) null, 1);
                      break L3;
                    }
                  }
                  decompiledRegionSelector0 = 2;
                  break L0;
                } else {
                  decompiledRegionSelector0 = 0;
                  break L0;
                }
              }
            } catch (java.lang.RuntimeException decompiledCaughtParameter1) {
              decompiledCaughtException = decompiledCaughtParameter1;
              L4: {
                var4_ref = (RuntimeException) (Object) decompiledCaughtException;
                stackIn_13_0 = (RuntimeException) (var4_ref);

                stackIn_13_1 = new StringBuilder().append("bv.G(").append(param0).append(',');

                if (param1 == null) {
                  stackIn_14_0 = (RuntimeException) ((Object) stackIn_13_0);
                  stackIn_14_1 = (StringBuilder) ((Object) stackIn_13_1);
                  stackIn_14_2 = "null";
                  break L4;
                } else {
                  stackIn_14_0 = (RuntimeException) ((Object) stackIn_13_0);
                  stackIn_14_1 = (StringBuilder) ((Object) stackIn_13_1);
                  stackIn_14_2 = "{...}";
                  break L4;
                }
              }
              L5: {


                stackIn_16_1 = ((StringBuilder) (Object) stackIn_14_1).append(stackIn_14_2).append(',').append(param2).append(',');

                if (param3 == null) {
                  stackIn_14_0 = (RuntimeException) ((Object) stackIn_14_0);
                  stackIn_17_1 = (StringBuilder) ((Object) stackIn_16_1);
                  stackIn_17_2 = "null";
                  break L5;
                } else {
                  stackIn_14_0 = (RuntimeException) ((Object) stackIn_14_0);
                  stackIn_17_1 = (StringBuilder) ((Object) stackIn_16_1);
                  stackIn_17_2 = "{...}";
                  break L5;
                }
              }
              throw nb.a((Throwable) ((Object) stackIn_14_0), stackIn_17_2 + ')');
            }
            if (decompiledRegionSelector0 == 0) {
              return;
            } else {
              if (decompiledRegionSelector0 == 1) {
                return;
              } else {
                return;
              }
            }
        } catch (RuntimeException | Error decompiledUncheckedException) {
            throw decompiledUncheckedException;
        } catch (Throwable decompiledCheckedException) {
            throw new RuntimeException(decompiledCheckedException);
        }
    }

    final static void a(ob param0, int param1, int param2) {
        try {
            if (param1 != 256) {
                String var4 = (String) null;
                bv.a((java.applet.Applet) null, (String) null, (byte) 46);
            }
            ui.g(0).a(param2, (byte) 15, param0);
        } catch (RuntimeException runtimeException) {
            throw nb.a((Throwable) ((Object) runtimeException), "bv.J(" + (param0 != null ? "{...}" : "null") + ',' + param1 + ',' + param2 + ')');
        }
    }

    public static void g(int param0) {
        field_m = null;
        field_o = null;
        field_p = null;
        if (param0 > -29) {
            field_l = (rk[]) null;
        }
        field_n = null;
        field_l = null;
    }

    final static void a(boolean param0, int[] param1, int param2) {
        double[] stackIn_20_0 = null;
        int stackIn_20_1 = 0;
        double[] stackIn_21_0 = null;
        int stackIn_21_1 = 0;
        double stackIn_21_2 = 0.0;
        double[] stackIn_23_0 = null;
        int stackIn_23_1 = 0;
        double[] stackIn_24_0 = null;
        int stackIn_24_1 = 0;
        double stackIn_24_2 = 0.0;
        RuntimeException stackIn_48_0 = null;
        StringBuilder stackIn_48_1 = null;
        RuntimeException stackIn_49_0 = null;
        StringBuilder stackIn_49_1 = null;
        String stackIn_49_2 = null;
        RuntimeException decompiledCaughtException = null;
        RuntimeException var3 = null;
        int var4_int = 0;
        Object var4 = null;
        double[] var5 = null;
        double var6 = 0.0;
        int var8 = 0;
        int var9 = 0;
        int var10 = 0;
        int var11 = 0;
        int var12 = 0;
        int var13 = 0;
        ea var14 = null;
        ea var15 = null;
        ea var16 = null;
        ea var17 = null;
        int[] var18 = null;
        int var19 = 0;
        int var20 = 0;
        int var21 = 0;
        int var22 = 0;
        ea[] var23 = null;
        int[] var24 = null;
        int var25 = 0;
        ea[] var26 = null;
        double[] var27 = null;
        int[] var28 = null;
        double[] var29 = null;
        int[] var30 = null;
        var22 = Kickabout.field_G;
        try {
          L0: {
            mh.field_j = param1;
            sc.field_D = param1.length;
            hh.field_d = param2;
            var23 = new ea[256];
            var26 = var23;
            var4_int = 0;
            L1: while (true) {
              if (var4_int >= 256) {
                L2: {
                  var4 = null;
                  var29 = new double[256];
                  var27 = var29;
                  var5 = var27;
                  var6 = 0.0;
                  var8 = 0;
                  var9 = 0;
                  if (param0) {
                    break L2;
                  } else {
                    field_n = (String) null;
                    break L2;
                  }
                }
                var10 = 0;
                var11 = 0;
                var12 = 0;
                bb.field_Mb = new int[sc.field_D];
                var13 = 0;
                var11 = 0;
                L3: while (true) {
                  if (-34 >= (var11 ^ -1)) {
                    wp.a(uc.field_C, ao.field_i, ra.field_F, 121, gm.field_fb, tf.field_g);
                    ld.a(ao.field_i, gm.field_fb, uc.field_C, ra.field_F, tf.field_g, false);
                    var14 = var26[0];
                    var26[0].field_k = 0;
                    var15 = var26[0];
                    var15.field_i = 0;
                    var14.field_c = 0;
                    var16 = var26[0];
                    var26[0].field_e = 32;
                    var17 = var26[0];
                    var10 = 0;
                    var17.field_d = 32;
                    var16.field_l = 32;
                    var8 = 1;
                    L4: while (true) {
                      L5: {
                        if (var8 >= hh.field_d) {
                          break L5;
                        } else {
                          L6: {
                            if (-1 == (tr.a(var26[var8], false, var26[var10]) ^ -1)) {
                              var8--;
                              var5[var10] = 0.0;
                              break L6;
                            } else {
                              L7: {
                                stackIn_20_0 = (double[]) (var5);

                                stackIn_20_1 = var10;

                                if (1 >= var26[var10].field_h) {
                                  stackIn_21_0 = (double[]) ((Object) stackIn_20_0);
                                  stackIn_21_1 = stackIn_20_1;
                                  stackIn_21_2 = 0.0;
                                  break L7;
                                } else {
                                  stackIn_21_0 = (double[]) ((Object) stackIn_20_0);
                                  stackIn_21_1 = stackIn_20_1;
                                  stackIn_21_2 = er.a((byte) 108, var26[var10]);
                                  break L7;
                                }
                              }
                              L8: {
                                stackIn_21_0[stackIn_21_1] = stackIn_21_2;
                                stackIn_23_0 = (double[]) (var5);

                                stackIn_23_1 = var8;

                                if (var26[var8].field_h > 1) {
                                  stackIn_24_0 = (double[]) ((Object) stackIn_23_0);
                                  stackIn_24_1 = stackIn_23_1;
                                  stackIn_24_2 = er.a((byte) 108, var26[var8]);
                                  break L8;
                                } else {
                                  stackIn_24_0 = (double[]) ((Object) stackIn_23_0);
                                  stackIn_24_1 = stackIn_23_1;
                                  stackIn_24_2 = 0.0;
                                  break L8;
                                }
                              }
                              stackIn_24_0[stackIn_24_1] = stackIn_24_2;
                              break L6;
                            }
                          }
                          var10 = 0;
                          var6 = var29[0];
                          var12 = 1;
                          L9: while (true) {
                            if (var8 < var12) {
                              if (var6 <= 0.0) {
                                hh.field_d = var8 + 1;
                                break L5;
                              } else {
                                var8++;
                                continue L4;
                              }
                            } else {
                              L10: {
                                if (var6 < var29[var12]) {
                                  var6 = var29[var12];
                                  var10 = var12;
                                  break L10;
                                } else {
                                  break L10;
                                }
                              }
                              var12++;
                              continue L9;
                            }
                          }
                        }
                      }
                      var30 = new int[256];
                      var28 = var30;
                      var18 = var28;
                      var24 = new int[35937];
                      var12 = 0;
                      L11: while (true) {
                        if (var12 >= hh.field_d) {
                          var8 = 0;
                          L12: while (true) {
                            if (sc.field_D <= var8) {
                              break L0;
                            } else {
                              param1[var8] = var18[qj.b(var24[bb.field_Mb[var8]], 255)];
                              var8++;
                              continue L12;
                            }
                          }
                        } else {
                          L13: {
                            av.a(var12, 1, var24, var26[var12]);
                            var9 = fr.a(var26[var12], ao.field_i, true);
                            if (-1 == (var9 ^ -1)) {
                              var30[var12] = 0;
                              break L13;
                            } else {
                              var19 = fr.a(var26[var12], ra.field_F, true) / var9 & 255;
                              var20 = fr.a(var26[var12], tf.field_g, true) / var9 & 255;
                              var21 = fr.a(var26[var12], gm.field_fb, param0) / var9 & 255;
                              var18[var12] = var21 + (var19 << 39986832) + (var20 << 329803688);
                              break L13;
                            }
                          }
                          var12++;
                          continue L11;
                        }
                      }
                    }
                  } else {
                    var12 = 0;
                    L14: while (true) {
                      if (var12 >= 33) {
                        var11++;
                        continue L3;
                      } else {
                        var25 = 0;
                        var13 = var25;
                        L15: while (true) {
                          if (var25 >= 33) {
                            var12++;
                            continue L14;
                          } else {
                            ao.field_i[var11][var12][var25] = 0;
                            ra.field_F[var11][var12][var25] = 0;
                            tf.field_g[var11][var12][var25] = 0;
                            gm.field_fb[var11][var12][var25] = 0;
                            uc.field_C[var11][var12][var25] = 0.0;
                            var25++;
                            continue L15;
                          }
                        }
                      }
                    }
                  }
                }
              } else {
                var23[var4_int] = new ea();
                var4_int++;
                continue L1;
              }
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L16: {
            var3 = decompiledCaughtException;
            stackIn_48_0 = (RuntimeException) (var3);

            stackIn_48_1 = new StringBuilder().append("bv.K(").append(param0).append(',');

            if (param1 == null) {
              stackIn_49_0 = (RuntimeException) ((Object) stackIn_48_0);
              stackIn_49_1 = (StringBuilder) ((Object) stackIn_48_1);
              stackIn_49_2 = "null";
              break L16;
            } else {
              stackIn_49_0 = (RuntimeException) ((Object) stackIn_48_0);
              stackIn_49_1 = (StringBuilder) ((Object) stackIn_48_1);
              stackIn_49_2 = "{...}";
              break L16;
            }
          }
          throw nb.a((Throwable) ((Object) stackIn_49_0), stackIn_49_2 + ',' + param2 + ')');
        }
    }

    static {
        field_p = "To Customer Support";
        field_o = "Quit to website";
        field_n = "to the Qualifying Round";
        field_m = "<%0>'s game";
    }
}
