/*
 * Decompiled by CFR-JS 0.4.0.
 */
class du {
    private static pp field_i;
    int[] field_p;
    static boolean field_o;
    private int field_g;
    private static int field_m;
    private boolean field_f;
    static int field_k;
    private hj field_l;
    private long field_u;
    private hj[] field_t;
    private int field_r;
    private boolean field_e;
    private int field_h;
    private hj[] field_b;
    private int field_n;
    private int field_c;
    private int field_q;
    private int field_a;
    private long field_d;
    private long field_j;
    private int field_s;

    final synchronized void d() {
        try {
            this.field_e = true;
            try {
                this.f();
            } catch (Exception exception) {
                this.g();
                this.field_j = wq.a(-20) + 2000L;
            }
        } catch (RuntimeException | Error decompiledUncheckedException) {
            throw decompiledUncheckedException;
        } catch (Throwable decompiledCheckedException) {
            throw new RuntimeException(decompiledCheckedException);
        }
    }

    final static du a(wi param0, java.awt.Component param1, int param2, int param3) {
        try {
            et var4 = null;
            Throwable var4_ref = null;
            et var5 = null;
            et stackIn_10_0 = null;
            int stackIn_10_1 = 0;
            et stackIn_11_0 = null;
            int stackIn_11_1 = 0;
            int stackIn_11_2 = 0;
            et stackIn_21_0 = null;
            Throwable decompiledCaughtException = null;
            if (field_k != 0) {
              L0: {
                if (param2 < 0) {
                  break L0;
                } else {
                  if (param2 < 2) {
                    L1: {
                      if (param3 >= 256) {
                        break L1;
                      } else {
                        param3 = 256;
                        break L1;
                      }
                    }
                    try {
                      L2: {
                        L3: {
                          var5 = new et();
                          var4 = var5;
                          stackIn_10_0 = (et) (var4);

                          stackIn_10_1 = 256;

                          if (!field_o) {
                            stackIn_11_0 = (et) ((Object) stackIn_10_0);
                            stackIn_11_1 = stackIn_10_1;
                            stackIn_11_2 = 1;
                            break L3;
                          } else {
                            stackIn_11_0 = (et) ((Object) stackIn_10_0);
                            stackIn_11_1 = stackIn_10_1;
                            stackIn_11_2 = 2;
                            break L3;
                          }
                        }
                        L4: {
                          ((du) ((Object) stackIn_11_0)).field_p = new int[stackIn_11_1 * stackIn_11_2];
                          ((du) ((Object) var4)).field_a = param3;
                          ((du) ((Object) var4)).a(param1);
                          ((du) ((Object) var4)).field_s = (param3 & -1024) + 1024;
                          if (((du) ((Object) var4)).field_s <= 16384) {
                            break L4;
                          } else {
                            ((du) ((Object) var4)).field_s = 16384;
                            break L4;
                          }
                        }
                        L5: {
                          ((du) ((Object) var4)).a(((du) ((Object) var4)).field_s);
                          if (field_m <= 0) {
                            break L5;
                          } else {
                            if (field_i != null) {
                              break L5;
                            } else {
                              field_i = new pp();
                              field_i.field_g = param0;
                              param0.a(true, (Runnable) ((Object) field_i), field_m);
                              break L5;
                            }
                          }
                        }
                        L6: {
                          if (field_i == null) {
                            break L6;
                          } else {
                            if (field_i.field_c[param2] == null) {
                              field_i.field_c[param2] = (du) ((Object) var5);
                              break L6;
                            } else {
                              throw new IllegalArgumentException();
                            }
                          }
                        }
                        stackIn_21_0 = (et) (var4);
                        break L2;
                      }
                    } catch (java.lang.Throwable decompiledCaughtParameter0) {
                      decompiledCaughtException = decompiledCaughtParameter0;
                      var4_ref = decompiledCaughtException;
                      return new du();
                    }
                    return (du) ((Object) stackIn_21_0);
                  } else {
                    break L0;
                  }
                }
              }
              throw new IllegalArgumentException();
            } else {
              throw new IllegalStateException();
            }
        } catch (RuntimeException | Error decompiledUncheckedException) {
            throw decompiledUncheckedException;
        } catch (Throwable decompiledCheckedException) {
            throw new RuntimeException(decompiledCheckedException);
        }
    }

    private final static void b(hj param0) {
        param0.field_n = false;
        if (param0.field_l != null) {
            param0.field_l.field_k = 0;
        }
        hj var1 = param0.a();
        while (var1 != null) {
            du.b(var1);
            var1 = param0.c();
        }
    }

    void c() throws Exception {
    }

    int a() throws Exception {
        return this.field_s;
    }

    final synchronized void a(hj param0) {
        this.field_l = param0;
    }

    final synchronized void e() {
        int var1;
        int var2;
        L0: {
          if (field_i == null) {
            break L0;
          } else {
            var1 = 1;
            var2 = 0;
            L1: while (true) {
              if (var2 >= 2) {
                if (var1 == 0) {
                  break L0;
                } else {
                  field_i.field_a = true;
                  L2: while (true) {
                    if (!field_i.field_b) {
                      field_i = null;
                      break L0;
                    } else {
                      tg.a(50L, (byte) 112);
                      continue L2;
                    }
                  }
                }
              } else {
                L3: {
                  if (field_i.field_c[var2] != this) {
                    break L3;
                  } else {
                    field_i.field_c[var2] = null;
                    break L3;
                  }
                }
                if (field_i.field_c[var2] != null) {
                  var1 = 0;
                  var2++;
                  continue L1;
                } else {
                  var2++;
                  continue L1;
                }
              }
            }
          }
        }
        this.g();
        this.field_p = null;
        this.field_f = true;
    }

    private final void a(int[] param0, int param1) {
        int var3;
        int var4;
        int var5;
        int var6;
        int var7_int;
        Object var7;
        int var8_int;
        hj[] var8;
        int var9;
        Object var10;
        hj var11;
        rr var12;
        int var13;
        hj var14;
        int var15_int;
        hj var15;
        L0: {
          var3 = param1;
          if (!field_o) {
            break L0;
          } else {
            var3 = var3 << 1;
            break L0;
          }
        }
        L1: {
          la.a(param0, 0, var3);
          this.field_c = this.field_c - param1;
          if (this.field_l == null) {
            break L1;
          } else {
            if (this.field_c > 0) {
              break L1;
            } else {
              this.field_c = this.field_c + (field_k >> 4);
              du.b(this.field_l);
              this.a(this.field_l, this.field_l.b());
              var4 = 0;
              var5 = 255;
              var6 = 7;
              L2: while (true) {
                L3: {
                  if (var5 == 0) {
                    break L3;
                  } else {
                    L4: {
                      if (var6 >= 0) {
                        var7_int = var6;
                        var8_int = 0;
                        break L4;
                      } else {
                        var7_int = var6 & 3;
                        var8_int = -(var6 >> 2);
                        break L4;
                      }
                    }
                    var9 = var5 >>> var7_int & 286331153;
                    L5: while (true) {
                      if (var9 != 0) {
                        L6: {
                          if ((var9 & 1) != 0) {
                            var5 = var5 & (1 << var7_int ^ -1);
                            var10 = null;
                            var11 = this.field_t[var7_int];
                            var14 = var11;
                            var14 = var11;
                            L7: while (true) {
                              if (var11 == null) {
                                break L6;
                              } else {
                                L8: {
                                  var12 = var11.field_l;
                                  if (var12 == null) {
                                    break L8;
                                  } else {
                                    if (var12.field_k <= var8_int) {
                                      break L8;
                                    } else {
                                      var5 = var5 | 1 << var7_int;
                                      var10 = var11;
                                      var11 = var11.field_m;
                                      continue L7;
                                    }
                                  }
                                }
                                L9: {
                                  var11.field_n = true;
                                  var13 = var11.d();
                                  var4 = var4 + var13;
                                  if (var12 == null) {
                                    break L9;
                                  } else {
                                    var12.field_k = var12.field_k + var13;
                                    break L9;
                                  }
                                }
                                if (var4 < this.field_g) {
                                  L10: {
                                    var14 = var11.a();
                                    if (var14 == null) {
                                      break L10;
                                    } else {
                                      var15_int = var11.field_k;
                                      L11: while (true) {
                                        if (var14 == null) {
                                          break L10;
                                        } else {
                                          this.a(var14, var15_int * var14.b() >> 8);
                                          var14 = var11.c();
                                          continue L11;
                                        }
                                      }
                                    }
                                  }
                                  L12: {
                                    var15 = var11.field_m;
                                    var11.field_m = null;
                                    if (var10 != null) {
                                      ((hj) (var10)).field_m = var15;
                                      break L12;
                                    } else {
                                      this.field_t[var7_int] = var15;
                                      break L12;
                                    }
                                  }
                                  L13: {
                                    if (var15 != null) {
                                      break L13;
                                    } else {
                                      this.field_b[var7_int] = (hj) (var10);
                                      break L13;
                                    }
                                  }
                                  var11 = var15;
                                  continue L7;
                                } else {
                                  break L3;
                                }
                              }
                            }
                          } else {
                            break L6;
                          }
                        }
                        var7_int += 4;
                        var8_int++;
                        var9 = var9 >>> 4;
                        continue L5;
                      } else {
                        var6--;
                        continue L2;
                      }
                    }
                  }
                }
                var6 = 0;
                L14: while (true) {
                  if (var6 >= 8) {
                    break L1;
                  } else {
                    var7 = this.field_t[var6];
                    var8 = this.field_t;
                    var9 = var6;
                    this.field_b[var6] = null;
                    var8[var9] = null;
                    L15: while (true) {
                      if (var7 == null) {
                        var6++;
                        continue L14;
                      } else {
                        var10 = ((hj) (var7)).field_m;
                        ((hj) (var7)).field_m = null;
                        var7 = var10;
                        continue L15;
                      }
                    }
                  }
                }
              }
            }
          }
        }
        L16: {
          if (this.field_c >= 0) {
            break L16;
          } else {
            this.field_c = 0;
            break L16;
          }
        }
        L17: {
          if (this.field_l == null) {
            break L17;
          } else {
            this.field_l.b(param0, 0, param1);
            break L17;
          }
        }
        this.field_u = wq.a(-54);
    }

    private final void b(int param0) {
        this.field_c = this.field_c - param0;
        if (this.field_c < 0) {
            this.field_c = 0;
        }
        if (this.field_l != null) {
            this.field_l.a(param0);
            return;
        }
    }

    final static void a(int param0, boolean param1, int param2) {
        if (param0 < 8000 || param0 > 48000) {
            throw new IllegalArgumentException();
        }
        field_k = param0;
        field_o = param1 ? true : false;
        field_m = param2;
    }

    void f() throws Exception {
    }

    final synchronized void b() {
        try {
            int decompiledRegionSelector0 = 0;
            Throwable decompiledCaughtException = null;
            long var1 = 0L;
            Exception var3 = null;
            int var3_int = 0;
            int var4 = 0;
            if (!this.field_f) {
              var1 = wq.a(-124);
              try {
                L0: {
                  L1: {
                    if (var1 <= this.field_u + 6000L) {
                      break L1;
                    } else {
                      this.field_u = var1 - 6000L;
                      break L1;
                    }
                  }
                  L2: while (true) {
                    if (var1 <= this.field_u + 5000L) {
                      break L0;
                    } else {
                      this.b(256);
                      this.field_u = this.field_u + (long)(256000 / field_k);
                      var1 = wq.a(-119);
                      continue L2;
                    }
                  }
                }
              } catch (java.lang.Exception decompiledCaughtParameter0) {
                decompiledCaughtException = decompiledCaughtParameter0;
                L3: {
                  var3 = (Exception) (Object) decompiledCaughtException;
                  this.field_u = var1;
                  break L3;
                }
              }
              if (this.field_p != null) {
                try {
                  L4: {
                    L5: {
                      if (this.field_j == 0L) {
                        break L5;
                      } else {
                        if (var1 >= this.field_j) {
                          this.a(this.field_s);
                          this.field_j = 0L;
                          this.field_e = true;
                          break L5;
                        } else {
                          decompiledRegionSelector0 = 0;
                          break L4;
                        }
                      }
                    }
                    L6: {
                      var3_int = this.a();
                      if (this.field_q - var3_int <= this.field_n) {
                        break L6;
                      } else {
                        this.field_n = this.field_q - var3_int;
                        break L6;
                      }
                    }
                    L7: {
                      var4 = this.field_a + this.field_r;
                      if (var4 + 256 <= 16384) {
                        break L7;
                      } else {
                        var4 = 16128;
                        break L7;
                      }
                    }
                    L8: {
                      if (var4 + 256 <= this.field_s) {
                        break L8;
                      } else {
                        L9: {
                          this.field_s = this.field_s + 1024;
                          if (this.field_s <= 16384) {
                            break L9;
                          } else {
                            this.field_s = 16384;
                            break L9;
                          }
                        }
                        this.g();
                        this.a(this.field_s);
                        var3_int = 0;
                        this.field_e = true;
                        if (var4 + 256 <= this.field_s) {
                          break L8;
                        } else {
                          var4 = this.field_s - 256;
                          this.field_r = var4 - this.field_a;
                          break L8;
                        }
                      }
                    }
                    L10: while (true) {
                      if (var3_int >= var4) {
                        L11: {
                          if (var1 <= this.field_d) {
                            break L11;
                          } else {
                            L12: {
                              if (this.field_e) {
                                this.field_e = false;
                                break L12;
                              } else {
                                L13: {
                                  if (this.field_n != 0) {
                                    break L13;
                                  } else {
                                    if (this.field_h != 0) {
                                      break L13;
                                    } else {
                                      this.g();
                                      this.field_j = var1 + 2000L;
                                      decompiledRegionSelector0 = 1;
                                      break L4;
                                    }
                                  }
                                }
                                this.field_r = Math.min(this.field_h, this.field_n);
                                this.field_h = this.field_n;
                                break L12;
                              }
                            }
                            this.field_n = 0;
                            this.field_d = var1 + 2000L;
                            break L11;
                          }
                        }
                        this.field_q = var3_int;
                        decompiledRegionSelector0 = 2;
                        break L4;
                      } else {
                        this.a(this.field_p, 256);
                        this.c();
                        var3_int += 256;
                        continue L10;
                      }
                    }
                  }
                } catch (java.lang.Exception decompiledCaughtParameter1) {
                  decompiledCaughtException = decompiledCaughtParameter1;
                  L14: {
                    var3 = (Exception) (Object) decompiledCaughtException;
                    this.g();
                    this.field_j = var1 + 2000L;
                    decompiledRegionSelector0 = 2;
                    break L14;
                  }
                }
                if (decompiledRegionSelector0 == 0) {
                  return;
                } else {
                  if (decompiledRegionSelector0 == 1) {
                    return;
                  } else {
                    return;
                  }
                }
              } else {
                return;
              }
            } else {
              return;
            }
        } catch (RuntimeException | Error decompiledUncheckedException) {
            throw decompiledUncheckedException;
        } catch (Throwable decompiledCheckedException) {
            throw new RuntimeException(decompiledCheckedException);
        }
    }

    private final void a(hj param0, int param1) {
        int var3 = param1 >> 5;
        hj var4 = this.field_b[var3];
        if (var4 == null) {
            this.field_t[var3] = param0;
        } else {
            var4.field_m = param0;
        }
        this.field_b[var3] = param0;
        param0.field_k = param1;
    }

    void a(java.awt.Component param0) throws Exception {
    }

    void a(int param0) throws Exception {
    }

    public static void h() {
        field_i = null;
    }

    void g() {
    }

    du() {
        this.field_f = false;
        this.field_g = 32;
        this.field_u = wq.a(-12);
        this.field_e = true;
        this.field_t = new hj[8];
        this.field_h = 0;
        this.field_b = new hj[8];
        this.field_c = 0;
        this.field_n = 0;
        this.field_d = 0L;
        this.field_q = 0;
        this.field_j = 0L;
    }
}
