/*
 * Decompiled by CFR-JS 0.4.0.
 */
final class ef {
    private int field_d;
    private int field_n;
    private int field_f;
    private boolean field_b;
    static gu field_k;
    private int field_v;
    private dj field_l;
    private int field_a;
    private int field_c;
    private int field_p;
    private int field_j;
    static String field_i;
    private int field_t;
    private int field_h;
    private int field_s;
    static String field_g;
    private int field_o;
    private float field_q;
    private int field_u;
    private int field_r;
    private int field_e;
    static int field_m;

    public static void a(int param0) {
        float discarded$2 = 0.0f;
        if (param0 != 0) {
          discarded$2 = ef.a(-0.783437192440033f, -0.508948802947998f, 119, 0.7036879658699036f, 56);
          field_k = null;
          field_g = null;
          field_i = null;
          return;
        } else {
          field_k = null;
          field_g = null;
          field_i = null;
          return;
        }
    }

    final static void a(byte param0, rk param1, gk param2) {
        RuntimeException var3 = null;
        int var4 = 0;
        RuntimeException stackIn_25_0 = null;
        StringBuilder stackIn_25_1 = null;
        RuntimeException stackIn_26_0 = null;
        StringBuilder stackIn_26_1 = null;
        RuntimeException stackIn_27_0 = null;
        StringBuilder stackIn_27_1 = null;
        String stackIn_27_2 = null;
        RuntimeException stackIn_28_0 = null;
        StringBuilder stackIn_28_1 = null;
        RuntimeException stackIn_29_0 = null;
        StringBuilder stackIn_29_1 = null;
        RuntimeException stackIn_30_0 = null;
        StringBuilder stackIn_30_1 = null;
        String stackIn_30_2 = null;
        RuntimeException decompiledCaughtException = null;
        RuntimeException stackOut_24_0 = null;
        StringBuilder stackOut_24_1 = null;
        RuntimeException stackOut_26_0 = null;
        StringBuilder stackOut_26_1 = null;
        String stackOut_26_2 = null;
        RuntimeException stackOut_25_0 = null;
        StringBuilder stackOut_25_1 = null;
        String stackOut_25_2 = null;
        RuntimeException stackOut_27_0 = null;
        StringBuilder stackOut_27_1 = null;
        RuntimeException stackOut_29_0 = null;
        StringBuilder stackOut_29_1 = null;
        String stackOut_29_2 = null;
        RuntimeException stackOut_28_0 = null;
        StringBuilder stackOut_28_1 = null;
        String stackOut_28_2 = null;
        var4 = AceOfSkies.field_G ? 1 : 0;
        try {
          L0: {
            L1: {
              rj.field_b = param2;
              bl.field_g = "";
              if (param0 > 93) {
                break L1;
              } else {
                field_k = (gu) null;
                break L1;
              }
            }
            L2: {
              bm.field_I = param1;
              if (!rr.field_b.startsWith("win")) {
                if (rr.field_b.startsWith("linux")) {
                  bl.field_g = bl.field_g + "linux/";
                  break L2;
                } else {
                  if (!rr.field_b.startsWith("mac")) {
                    break L2;
                  } else {
                    bl.field_g = bl.field_g + "macos/";
                    break L2;
                  }
                }
              } else {
                bl.field_g = bl.field_g + "windows/";
                break L2;
              }
            }
            L3: {
              if (bm.field_I.field_p) {
                bl.field_g = bl.field_g + "msjava/";
                break L3;
              } else {
                L4: {
                  if (rr.field_a.startsWith("amd64")) {
                    break L4;
                  } else {
                    if (!rr.field_a.startsWith("x86_64")) {
                      L5: {
                        if (rr.field_a.startsWith("i386")) {
                          break L5;
                        } else {
                          if (rr.field_a.startsWith("i486")) {
                            break L5;
                          } else {
                            if (rr.field_a.startsWith("i586")) {
                              break L5;
                            } else {
                              if (rr.field_a.startsWith("x86")) {
                                break L5;
                              } else {
                                if (!rr.field_a.startsWith("ppc")) {
                                  bl.field_g = bl.field_g + "universal/";
                                  break L3;
                                } else {
                                  bl.field_g = bl.field_g + "ppc/";
                                  break L3;
                                }
                              }
                            }
                          }
                        }
                      }
                      bl.field_g = bl.field_g + "x86/";
                      break L3;
                    } else {
                      break L4;
                    }
                  }
                }
                bl.field_g = bl.field_g + "x86_64/";
                break L3;
              }
            }
            break L0;
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L6: {
            var3 = decompiledCaughtException;
            stackOut_24_0 = (RuntimeException) (var3);
            stackOut_24_1 = new StringBuilder().append("ef.D(").append(param0).append(',');
            stackIn_26_0 = stackOut_24_0;
            stackIn_26_1 = stackOut_24_1;
            stackIn_25_0 = stackOut_24_0;
            stackIn_25_1 = stackOut_24_1;
            if (param1 == null) {
              stackOut_26_0 = (RuntimeException) ((Object) stackIn_26_0);
              stackOut_26_1 = (StringBuilder) ((Object) stackIn_26_1);
              stackOut_26_2 = "null";
              stackIn_27_0 = stackOut_26_0;
              stackIn_27_1 = stackOut_26_1;
              stackIn_27_2 = stackOut_26_2;
              break L6;
            } else {
              stackOut_25_0 = (RuntimeException) ((Object) stackIn_25_0);
              stackOut_25_1 = (StringBuilder) ((Object) stackIn_25_1);
              stackOut_25_2 = "{...}";
              stackIn_27_0 = stackOut_25_0;
              stackIn_27_1 = stackOut_25_1;
              stackIn_27_2 = stackOut_25_2;
              break L6;
            }
          }
          L7: {
            stackOut_27_0 = (RuntimeException) ((Object) stackIn_27_0);
            stackOut_27_1 = ((StringBuilder) (Object) stackIn_27_1).append(stackIn_27_2).append(',');
            stackIn_29_0 = stackOut_27_0;
            stackIn_29_1 = stackOut_27_1;
            stackIn_28_0 = stackOut_27_0;
            stackIn_28_1 = stackOut_27_1;
            if (param2 == null) {
              stackOut_29_0 = (RuntimeException) ((Object) stackIn_29_0);
              stackOut_29_1 = (StringBuilder) ((Object) stackIn_29_1);
              stackOut_29_2 = "null";
              stackIn_30_0 = stackOut_29_0;
              stackIn_30_1 = stackOut_29_1;
              stackIn_30_2 = stackOut_29_2;
              break L7;
            } else {
              stackOut_28_0 = (RuntimeException) ((Object) stackIn_28_0);
              stackOut_28_1 = (StringBuilder) ((Object) stackIn_28_1);
              stackOut_28_2 = "{...}";
              stackIn_30_0 = stackOut_28_0;
              stackIn_30_1 = stackOut_28_1;
              stackIn_30_2 = stackOut_28_2;
              break L7;
            }
          }
          throw pn.a((Throwable) ((Object) stackIn_30_0), stackIn_30_2 + ')');
        }
    }

    private final void a(byte param0) {
        int var2 = 0;
        int var3 = 0;
        int var4 = 0;
        int var5 = 0;
        var5 = AceOfSkies.field_G ? 1 : 0;
        if (param0 <= -115) {
          var2 = this.field_e;
          var3 = this.field_p;
          if (this.a(false)) {
            L0: {
              if (this.field_a >= var3) {
                if (this.field_o <= var3) {
                  break L0;
                } else {
                  var3 = this.field_o;
                  break L0;
                }
              } else {
                var3 = this.field_a;
                break L0;
              }
            }
            if (this.field_d < var2) {
              L1: {
                var2 = this.field_d;
                if (0.0f >= this.field_q) {
                  break L1;
                } else {
                  var4 = (int)(this.field_q * (float)var3 + 0.5f);
                  if (var2 < var4) {
                    var3 = (int)((float)var2 / this.field_q);
                    break L1;
                  } else {
                    if (var4 >= var2) {
                      break L1;
                    } else {
                      var2 = var4;
                      if (td.field_b != var2) {
                        L2: {
                          this.field_l.a(var3, false, var2);
                          if (-1 <= (this.field_e ^ -1)) {
                            break L2;
                          } else {
                            ci.field_g = (-td.field_b + this.field_e) / 2;
                            break L2;
                          }
                        }
                        return;
                      } else {
                        if (var3 == ps.field_h) {
                          if (-1 > (this.field_e ^ -1)) {
                            ci.field_g = (-td.field_b + this.field_e) / 2;
                            return;
                          } else {
                            return;
                          }
                        } else {
                          this.field_l.a(var3, false, var2);
                          if (-1 > (this.field_e ^ -1)) {
                            ci.field_g = (-td.field_b + this.field_e) / 2;
                            return;
                          } else {
                            return;
                          }
                        }
                      }
                    }
                  }
                }
              }
              if (td.field_b != var2) {
                this.field_l.a(var3, false, var2);
                if (-1 > (this.field_e ^ -1)) {
                  ci.field_g = (-td.field_b + this.field_e) / 2;
                  return;
                } else {
                  return;
                }
              } else {
                if (var3 == ps.field_h) {
                  if (-1 > (this.field_e ^ -1)) {
                    ci.field_g = (-td.field_b + this.field_e) / 2;
                    return;
                  } else {
                    return;
                  }
                } else {
                  this.field_l.a(var3, false, var2);
                  if (-1 > (this.field_e ^ -1)) {
                    ci.field_g = (-td.field_b + this.field_e) / 2;
                    return;
                  } else {
                    return;
                  }
                }
              }
            } else {
              if (var2 < this.field_v) {
                L3: {
                  var2 = this.field_v;
                  if (0.0f >= this.field_q) {
                    break L3;
                  } else {
                    var4 = (int)(this.field_q * (float)var3 + 0.5f);
                    if (var2 < var4) {
                      var3 = (int)((float)var2 / this.field_q);
                      break L3;
                    } else {
                      if (var4 >= var2) {
                        break L3;
                      } else {
                        var2 = var4;
                        break L3;
                      }
                    }
                  }
                }
                L4: {
                  if (td.field_b != var2) {
                    this.field_l.a(var3, false, var2);
                    break L4;
                  } else {
                    if (var3 == ps.field_h) {
                      break L4;
                    } else {
                      this.field_l.a(var3, false, var2);
                      break L4;
                    }
                  }
                }
                if (-1 > (this.field_e ^ -1)) {
                  ci.field_g = (-td.field_b + this.field_e) / 2;
                  return;
                } else {
                  return;
                }
              } else {
                if (0.0f < this.field_q) {
                  var4 = (int)(this.field_q * (float)var3 + 0.5f);
                  if (var2 >= var4) {
                    if (var4 >= var2) {
                      L5: {
                        if (td.field_b != var2) {
                          this.field_l.a(var3, false, var2);
                          break L5;
                        } else {
                          if (var3 == ps.field_h) {
                            break L5;
                          } else {
                            this.field_l.a(var3, false, var2);
                            break L5;
                          }
                        }
                      }
                      L6: {
                        if (-1 <= (this.field_e ^ -1)) {
                          break L6;
                        } else {
                          ci.field_g = (-td.field_b + this.field_e) / 2;
                          break L6;
                        }
                      }
                      return;
                    } else {
                      var2 = var4;
                      L7: {
                        if (td.field_b != var2) {
                          this.field_l.a(var3, false, var2);
                          break L7;
                        } else {
                          if (var3 == ps.field_h) {
                            break L7;
                          } else {
                            this.field_l.a(var3, false, var2);
                            break L7;
                          }
                        }
                      }
                      L8: {
                        if (-1 <= (this.field_e ^ -1)) {
                          break L8;
                        } else {
                          ci.field_g = (-td.field_b + this.field_e) / 2;
                          break L8;
                        }
                      }
                      return;
                    }
                  } else {
                    L9: {
                      var3 = (int)((float)var2 / this.field_q);
                      if (td.field_b != var2) {
                        this.field_l.a(var3, false, var2);
                        break L9;
                      } else {
                        if (var3 == ps.field_h) {
                          break L9;
                        } else {
                          this.field_l.a(var3, false, var2);
                          break L9;
                        }
                      }
                    }
                    L10: {
                      if (-1 <= (this.field_e ^ -1)) {
                        break L10;
                      } else {
                        ci.field_g = (-td.field_b + this.field_e) / 2;
                        break L10;
                      }
                    }
                    return;
                  }
                } else {
                  L11: {
                    if (td.field_b != var2) {
                      this.field_l.a(var3, false, var2);
                      break L11;
                    } else {
                      if (var3 == ps.field_h) {
                        break L11;
                      } else {
                        this.field_l.a(var3, false, var2);
                        break L11;
                      }
                    }
                  }
                  L12: {
                    if (-1 <= (this.field_e ^ -1)) {
                      break L12;
                    } else {
                      ci.field_g = (-td.field_b + this.field_e) / 2;
                      break L12;
                    }
                  }
                  return;
                }
              }
            }
          } else {
            this.field_b = false;
            return;
          }
        } else {
          return;
        }
    }

    final hj b(byte param0) {
        hj var2 = null;
        this.field_n = ps.field_h;
        this.field_r = td.field_b;
        this.field_l.a(this.field_u, false, this.field_c);
        if (param0 != -111) {
          L0: {
            this.field_p = -7;
            or.field_c = false;
            var2 = uj.a(pt.field_b, 0, this.field_c, 0, 0, this.field_u);
            if (var2 == null) {
              this.c(1);
              break L0;
            } else {
              break L0;
            }
          }
          return var2;
        } else {
          L1: {
            or.field_c = false;
            var2 = uj.a(pt.field_b, 0, this.field_c, 0, 0, this.field_u);
            if (var2 == null) {
              this.c(1);
              break L1;
            } else {
              break L1;
            }
          }
          return var2;
        }
    }

    final static float a(float param0, float param1, int param2, float param3, int param4) {
        float[] var5 = null;
        float[] var6 = null;
        if (param2 >= -30) {
          field_k = (gu) null;
          var6 = uj.field_a[param4];
          var5 = var6;
          return var6[1] * param3 + var6[0] * param0 + var6[2] * param1;
        } else {
          var6 = uj.field_a[param4];
          var5 = var6;
          return var6[1] * param3 + var6[0] * param0 + var6[2] * param1;
        }
    }

    final void c(int param0) {
        if (param0 != 1) {
          this.a((byte) -112);
          this.field_l.a(this.field_n, false, this.field_r);
          return;
        } else {
          this.field_l.a(this.field_n, false, this.field_r);
          return;
        }
    }

    final void a(int param0, int param1, int param2) {
        this.field_p = param2;
        this.field_e = param1;
        if (param0 != 0) {
            this.a(-48, -58, -98);
        }
    }

    final static void a(int param0, mg param1, mg param2, int param3, int param4, int param5) {
        try {
            if (param0 < 15) {
                field_g = (String) null;
            }
            as.field_c = param3;
            ej.field_a = param2;
            pp.field_a = param5;
            u.field_g = param1;
            ud.field_g = param4;
        } catch (RuntimeException runtimeException) {
            throw pn.a((Throwable) ((Object) runtimeException), "ef.E(" + param0 + ',' + (param1 != null ? "{...}" : "null") + ',' + (param2 != null ? "{...}" : "null") + ',' + param3 + ',' + param4 + ',' + param5 + ')');
        }
    }

    final boolean a(boolean param0) {
        if (!param0) {
          if (this.field_s <= ki.field_g) {
            if (qr.field_a <= 0) {
              return false;
            } else {
              return true;
            }
          } else {
            return false;
          }
        } else {
          this.field_u = -43;
          if (this.field_s <= ki.field_g) {
            if (qr.field_a <= 0) {
              return false;
            } else {
              return true;
            }
          } else {
            return false;
          }
        }
    }

    final void b(int param0) {
        float discarded$5 = 0.0f;
        int fieldTemp$6 = 0;
        int fieldTemp$7 = 0;
        int fieldTemp$8 = 0;
        int fieldTemp$9 = 0;
        if (jf.field_a == null) {
          if (param0 < 38) {
            discarded$5 = ef.a(0.5743587017059326f, 1.6069247722625732f, 74, -0.5514667630195618f, -105);
            if (qr.field_a > 0) {
              if (this.field_b) {
                fieldTemp$6 = this.field_j - 1;
                this.field_j = this.field_j - 1;
                if (0 >= fieldTemp$6) {
                  this.field_j = this.field_f;
                  if (ki.field_g < this.field_s) {
                    this.field_b = false;
                    return;
                  } else {
                    this.a((byte) -122);
                    return;
                  }
                } else {
                  return;
                }
              } else {
                if (this.field_e > td.field_b) {
                  ci.field_g = (this.field_e + -td.field_b) / 2;
                  if (this.field_t == td.field_b) {
                    if (this.field_h != ps.field_h) {
                      this.field_l.a(this.field_h, false, this.field_t);
                      return;
                    } else {
                      return;
                    }
                  } else {
                    this.field_l.a(this.field_h, false, this.field_t);
                    return;
                  }
                } else {
                  if (0 >= this.field_e) {
                    if (this.field_t == td.field_b) {
                      if (this.field_h != ps.field_h) {
                        this.field_l.a(this.field_h, false, this.field_t);
                        return;
                      } else {
                        return;
                      }
                    } else {
                      this.field_l.a(this.field_h, false, this.field_t);
                      return;
                    }
                  } else {
                    ci.field_g = 0;
                    if (this.field_t == td.field_b) {
                      if (this.field_h == ps.field_h) {
                        return;
                      } else {
                        this.field_l.a(this.field_h, false, this.field_t);
                        return;
                      }
                    } else {
                      this.field_l.a(this.field_h, false, this.field_t);
                      return;
                    }
                  }
                }
              }
            } else {
              this.field_b = false;
              if (this.field_b) {
                fieldTemp$7 = this.field_j - 1;
                this.field_j = this.field_j - 1;
                if (0 >= fieldTemp$7) {
                  this.field_j = this.field_f;
                  if (ki.field_g < this.field_s) {
                    this.field_b = false;
                    return;
                  } else {
                    this.a((byte) -122);
                    return;
                  }
                } else {
                  return;
                }
              } else {
                if (this.field_e <= td.field_b) {
                  if (0 >= this.field_e) {
                    L0: {
                      if (this.field_t != td.field_b) {
                        this.field_l.a(this.field_h, false, this.field_t);
                        break L0;
                      } else {
                        if (this.field_h != ps.field_h) {
                          this.field_l.a(this.field_h, false, this.field_t);
                          break L0;
                        } else {
                          break L0;
                        }
                      }
                    }
                    return;
                  } else {
                    ci.field_g = 0;
                    L1: {
                      if (this.field_t != td.field_b) {
                        this.field_l.a(this.field_h, false, this.field_t);
                        break L1;
                      } else {
                        if (this.field_h != ps.field_h) {
                          this.field_l.a(this.field_h, false, this.field_t);
                          break L1;
                        } else {
                          break L1;
                        }
                      }
                    }
                    return;
                  }
                } else {
                  ci.field_g = (this.field_e + -td.field_b) / 2;
                  L2: {
                    if (this.field_t != td.field_b) {
                      this.field_l.a(this.field_h, false, this.field_t);
                      break L2;
                    } else {
                      if (this.field_h != ps.field_h) {
                        this.field_l.a(this.field_h, false, this.field_t);
                        break L2;
                      } else {
                        break L2;
                      }
                    }
                  }
                  return;
                }
              }
            }
          } else {
            if (qr.field_a > 0) {
              if (this.field_b) {
                fieldTemp$8 = this.field_j - 1;
                this.field_j = this.field_j - 1;
                if (0 >= fieldTemp$8) {
                  this.field_j = this.field_f;
                  if (ki.field_g < this.field_s) {
                    this.field_b = false;
                    return;
                  } else {
                    this.a((byte) -122);
                    return;
                  }
                } else {
                  return;
                }
              } else {
                if (this.field_e <= td.field_b) {
                  if (0 >= this.field_e) {
                    if (this.field_t == td.field_b) {
                      if (this.field_h == ps.field_h) {
                        return;
                      } else {
                        this.field_l.a(this.field_h, false, this.field_t);
                        return;
                      }
                    } else {
                      this.field_l.a(this.field_h, false, this.field_t);
                      return;
                    }
                  } else {
                    L3: {
                      ci.field_g = 0;
                      if (this.field_t != td.field_b) {
                        this.field_l.a(this.field_h, false, this.field_t);
                        break L3;
                      } else {
                        if (this.field_h != ps.field_h) {
                          this.field_l.a(this.field_h, false, this.field_t);
                          break L3;
                        } else {
                          break L3;
                        }
                      }
                    }
                    return;
                  }
                } else {
                  L4: {
                    ci.field_g = (this.field_e + -td.field_b) / 2;
                    if (this.field_t != td.field_b) {
                      this.field_l.a(this.field_h, false, this.field_t);
                      break L4;
                    } else {
                      if (this.field_h != ps.field_h) {
                        this.field_l.a(this.field_h, false, this.field_t);
                        break L4;
                      } else {
                        break L4;
                      }
                    }
                  }
                  return;
                }
              }
            } else {
              this.field_b = false;
              if (this.field_b) {
                fieldTemp$9 = this.field_j - 1;
                this.field_j = this.field_j - 1;
                if (0 >= fieldTemp$9) {
                  this.field_j = this.field_f;
                  if (ki.field_g < this.field_s) {
                    this.field_b = false;
                    return;
                  } else {
                    this.a((byte) -122);
                    return;
                  }
                } else {
                  return;
                }
              } else {
                L5: {
                  if (this.field_e > td.field_b) {
                    ci.field_g = (this.field_e + -td.field_b) / 2;
                    break L5;
                  } else {
                    if (0 >= this.field_e) {
                      break L5;
                    } else {
                      ci.field_g = 0;
                      break L5;
                    }
                  }
                }
                L6: {
                  if (this.field_t != td.field_b) {
                    this.field_l.a(this.field_h, false, this.field_t);
                    break L6;
                  } else {
                    if (this.field_h != ps.field_h) {
                      this.field_l.a(this.field_h, false, this.field_t);
                      break L6;
                    } else {
                      break L6;
                    }
                  }
                }
                return;
              }
            }
          }
        } else {
          return;
        }
    }

    private ef() throws Throwable {
        throw new Error();
    }

    static {
        field_i = "Menu";
        field_g = "Achievements are awarded in classic mode only";
        field_k = new gu("email");
        field_m = 0;
    }
}
