/*
 * Decompiled by CFR-JS 0.4.0.
 */
final class kp {
    wf field_e;
    static kc field_a;
    private wf field_c;
    static kc field_d;
    static String field_b;
    static int[] field_f;

    final wf b(int param0) {
        wf var2;
        if (param0 == 4095) {
          var2 = this.field_e.field_c;
          if (this.field_e == var2) {
            return null;
          } else {
            var2.c(-124);
            return var2;
          }
        } else {
          this.field_c = (wf) null;
          var2 = this.field_e.field_c;
          if (this.field_e == var2) {
            return null;
          } else {
            var2.c(-124);
            return var2;
          }
        }
    }

    final boolean a(boolean param0) {
        int stackIn_4_0 = 0;
        int stackIn_8_0 = 0;
        if (!param0) {
          L0: {
            this.field_e = (wf) null;
            if (this.field_e.field_a != this.field_e) {
              stackIn_8_0 = 0;
              break L0;
            } else {
              stackIn_8_0 = 1;
              break L0;
            }
          }
          return stackIn_8_0 != 0;
        } else {
          L1: {
            if (this.field_e.field_a != this.field_e) {
              stackIn_4_0 = 0;
              break L1;
            } else {
              stackIn_4_0 = 1;
              break L1;
            }
          }
          return stackIn_4_0 != 0;
        }
    }

    final wf a(int param0) {
        wf var2;
        if (param0 > 50) {
          var2 = this.field_c;
          if (this.field_e == var2) {
            this.field_c = null;
            return null;
          } else {
            this.field_c = var2.field_c;
            return var2;
          }
        } else {
          return (wf) null;
        }
    }

    final static int a(int param0, as param1) {
        int var2_int = 0;
        RuntimeException var2 = null;
        int stackIn_4_0 = 0;
        int stackIn_7_0 = 0;
        int stackIn_11_0 = 0;
        int stackIn_13_0 = 0;
        int stackIn_15_0 = 0;
        RuntimeException stackIn_19_0 = null;
        StringBuilder stackIn_19_1 = null;
        RuntimeException stackIn_20_0 = null;
        StringBuilder stackIn_20_1 = null;
        String stackIn_20_2 = null;
        int decompiledRegionSelector0 = 0;
        RuntimeException decompiledCaughtException = null;
        try {
          L0: {
            var2_int = 80 / ((-18 - param0) / 45);
            if (param1 == bd.field_C) {
              stackIn_15_0 = 7681;
              decompiledRegionSelector0 = 4;
              break L0;
            } else {
              if (ru.field_a == param1) {
                stackIn_13_0 = 8448;
                decompiledRegionSelector0 = 3;
                break L0;
              } else {
                if (param1 != du.field_l) {
                  if (param1 != lo.field_j) {
                    if (fs.field_d == param1) {
                      stackIn_11_0 = 34023;
                      decompiledRegionSelector0 = 2;
                      break L0;
                    } else {
                      throw new IllegalArgumentException();
                    }
                  } else {
                    stackIn_7_0 = 260;
                    decompiledRegionSelector0 = 1;
                    break L0;
                  }
                } else {
                  stackIn_4_0 = 34165;
                  decompiledRegionSelector0 = 0;
                  break L0;
                }
              }
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L1: {
            var2 = decompiledCaughtException;
            stackIn_19_0 = (RuntimeException) (var2);

            stackIn_19_1 = new StringBuilder().append("kp.F(").append(param0).append(',');

            if (param1 == null) {
              stackIn_20_0 = (RuntimeException) ((Object) stackIn_19_0);
              stackIn_20_1 = (StringBuilder) ((Object) stackIn_19_1);
              stackIn_20_2 = "null";
              break L1;
            } else {
              stackIn_20_0 = (RuntimeException) ((Object) stackIn_19_0);
              stackIn_20_1 = (StringBuilder) ((Object) stackIn_19_1);
              stackIn_20_2 = "{...}";
              break L1;
            }
          }
          throw pn.a((Throwable) ((Object) stackIn_20_0), stackIn_20_2 + ')');
        }
        if (decompiledRegionSelector0 == 0) {
          return stackIn_4_0;
        } else {
          if (decompiledRegionSelector0 == 1) {
            return stackIn_7_0;
          } else {
            if (decompiledRegionSelector0 == 2) {
              return stackIn_11_0;
            } else {
              if (decompiledRegionSelector0 == 3) {
                return stackIn_13_0;
              } else {
                return stackIn_15_0;
              }
            }
          }
        }
    }

    final static int a(boolean param0, String param1, boolean param2) {
        RuntimeException var3 = null;
        int stackIn_2_0 = 0;
        int stackIn_5_0 = 0;
        int stackIn_7_0 = 0;
        RuntimeException stackIn_10_0 = null;
        StringBuilder stackIn_10_1 = null;
        RuntimeException stackIn_11_0 = null;
        StringBuilder stackIn_11_1 = null;
        String stackIn_11_2 = null;
        int decompiledRegionSelector0 = 0;
        RuntimeException decompiledCaughtException = null;
        try {
          L0: {
            if (param0) {
              if (param2) {
                stackIn_7_0 = pk.field_e.a(param1);
                decompiledRegionSelector0 = 2;
                break L0;
              } else {
                stackIn_5_0 = nh.field_c.a(param1);
                decompiledRegionSelector0 = 1;
                break L0;
              }
            } else {
              stackIn_2_0 = -40;
              decompiledRegionSelector0 = 0;
              break L0;
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L1: {
            var3 = decompiledCaughtException;
            stackIn_10_0 = (RuntimeException) (var3);

            stackIn_10_1 = new StringBuilder().append("kp.E(").append(param0).append(',');

            if (param1 == null) {
              stackIn_11_0 = (RuntimeException) ((Object) stackIn_10_0);
              stackIn_11_1 = (StringBuilder) ((Object) stackIn_10_1);
              stackIn_11_2 = "null";
              break L1;
            } else {
              stackIn_11_0 = (RuntimeException) ((Object) stackIn_10_0);
              stackIn_11_1 = (StringBuilder) ((Object) stackIn_10_1);
              stackIn_11_2 = "{...}";
              break L1;
            }
          }
          throw pn.a((Throwable) ((Object) stackIn_11_0), stackIn_11_2 + ',' + param2 + ')');
        }
        if (decompiledRegionSelector0 == 0) {
          return stackIn_2_0;
        } else {
          if (decompiledRegionSelector0 == 1) {
            return stackIn_5_0;
          } else {
            return stackIn_7_0;
          }
        }
    }

    final wf e(int param0) {
        wf var2;
        var2 = this.field_e.field_a;
        if (param0 > 119) {
          if (var2 == this.field_e) {
            return null;
          } else {
            var2.c(-124);
            return var2;
          }
        } else {
          return (wf) null;
        }
    }

    final wf a(byte param0) {
        wf var2;
        var2 = this.field_e.field_c;
        if (var2 == this.field_e) {
          this.field_c = null;
          return null;
        } else {
          this.field_c = var2.field_c;
          if (param0 > -26) {
            return (wf) null;
          } else {
            return var2;
          }
        }
    }

    public static void f(int param0) {
        as var2;
        field_b = null;
        field_f = null;
        if (param0 != -19854) {
          var2 = (as) null;
          kp.a(-73, (as) null);
          field_d = null;
          field_a = null;
          return;
        } else {
          field_d = null;
          field_a = null;
          return;
        }
    }

    final void a(int param0, wf param1) {
        try {
            if (!(null == param1.field_c)) {
                param1.c(-128);
            }
            param1.field_c = this.field_e.field_c;
            param1.field_a = this.field_e;
            if (param0 < 65) {
                String var4 = (String) null;
                kp.a(false, (String) null, true);
            }
            param1.field_c.field_a = param1;
            param1.field_a.field_c = param1;
        } catch (RuntimeException runtimeException) {
            throw pn.a((Throwable) ((Object) runtimeException), "kp.I(" + param0 + ',' + (param1 != null ? "{...}" : "null") + ')');
        }
    }

    final void a(wf param0, int param1) {
        RuntimeException var3 = null;
        RuntimeException stackIn_8_0 = null;
        StringBuilder stackIn_8_1 = null;
        RuntimeException stackIn_9_0 = null;
        StringBuilder stackIn_9_1 = null;
        String stackIn_9_2 = null;
        RuntimeException decompiledCaughtException = null;
        try {
          L0: {
            L1: {
              if (param0.field_c != null) {
                param0.c(param1 + -25736);
                break L1;
              } else {
                break L1;
              }
            }
            param0.field_a = this.field_e.field_a;
            param0.field_c = this.field_e;
            param0.field_c.field_a = param0;
            param0.field_a.field_c = param0;
            if (param1 == 25611) {
              break L0;
            } else {
              this.field_e = (wf) null;
              return;
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L2: {
            var3 = decompiledCaughtException;
            stackIn_8_0 = (RuntimeException) (var3);

            stackIn_8_1 = new StringBuilder().append("kp.G(");

            if (param0 == null) {
              stackIn_9_0 = (RuntimeException) ((Object) stackIn_8_0);
              stackIn_9_1 = (StringBuilder) ((Object) stackIn_8_1);
              stackIn_9_2 = "null";
              break L2;
            } else {
              stackIn_9_0 = (RuntimeException) ((Object) stackIn_8_0);
              stackIn_9_1 = (StringBuilder) ((Object) stackIn_8_1);
              stackIn_9_2 = "{...}";
              break L2;
            }
          }
          throw pn.a((Throwable) ((Object) stackIn_9_0), stackIn_9_2 + ',' + param1 + ')');
        }
    }

    final wf b(byte param0) {
        wf var2;
        String var3;
        if (param0 == 103) {
          var2 = this.field_c;
          if (var2 == this.field_e) {
            this.field_c = null;
            return null;
          } else {
            this.field_c = var2.field_a;
            return var2;
          }
        } else {
          var3 = (String) null;
          kp.a(false, (String) null, true);
          var2 = this.field_c;
          if (var2 == this.field_e) {
            this.field_c = null;
            return null;
          } else {
            this.field_c = var2.field_a;
            return var2;
          }
        }
    }

    final wf d(int param0) {
        wf var2;
        var2 = this.field_e.field_a;
        if (var2 == this.field_e) {
          this.field_c = null;
          return null;
        } else {
          if (param0 != 268435455) {
            kp.f(-106);
            this.field_c = var2.field_a;
            return var2;
          } else {
            this.field_c = var2.field_a;
            return var2;
          }
        }
    }

    final void c(int param0) {
        wf var2;
        int var3;
        wf var4;
        var3 = AceOfSkies.field_G ? 1 : 0;
        L0: while (true) {
          var2 = this.field_e.field_a;
          if (var2 == this.field_e) {
            if (param0 != 268435455) {
              var4 = (wf) null;
              this.a((wf) null, 37);
              this.field_c = null;
              return;
            } else {
              this.field_c = null;
              return;
            }
          } else {
            var2.c(param0 + -268435583);
            continue L0;
          }
        }
    }

    public kp() {
        this.field_e = new wf();
        this.field_e.field_c = this.field_e;
        this.field_e.field_a = this.field_e;
    }

    static {
        field_a = new kc();
        field_d = new kc();
        field_b = ",";
        field_f = new int[]{0, 1, 3, 7, 15, 31, 63, 127, 255, 511, 1023, 2047, 4095, 8191, 16383, 32767, 65535, 131071, 262143, 524287, 1048575, 2097151, 4194303, 8388607, 16777215, 33554431, 67108863, 134217727, 268435455, 536870911, 1073741823, 2147483647, -1};
    }
}
