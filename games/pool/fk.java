/*
 * Decompiled by CFR-JS 0.4.0.
 */
import java.util.*;

final class fk extends oo {
    private jg field_db;
    static int field_Z;
    private jg field_W;
    private byte field_S;
    private int[] field_L;
    private nb field_bb;
    private jg field_O;
    private jg field_hb;
    private jg field_fb;
    private jg field_J;
    static lq field_cb;
    private jg field_Y;
    static int field_ab;
    private ji[] field_K;
    private jg field_Q;
    static int field_V;
    private jg field_N;
    private int[] field_R;
    static String field_eb;
    private int[] field_X;
    private int[] field_M;
    static hr field_gb;
    private jg field_T;
    private jg field_U;
    private jg field_P;

    final void a(al param0, int param1) {
        int var3_int = 0;
        int var4_int = 0;
        bf var4 = null;
        int var5_int = 0;
        bf var5 = null;
        bf var6 = null;
        bf var7 = null;
        int var8 = 0;
        RuntimeException stackIn_19_0 = null;
        StringBuilder stackIn_19_1 = null;
        RuntimeException stackIn_20_0 = null;
        StringBuilder stackIn_20_1 = null;
        String stackIn_20_2 = null;
        RuntimeException decompiledCaughtException = null;
        RuntimeException var3 = null;
        var8 = Pool.field_O;
        try {
          L0: {
            super.a(param0, param1);
            t.field_a = new int[3][17][33];
            ro.field_v = new int[3][17][33];
            fm.field_K = new jd[16][32];
            cn.field_a = new jd[16][32];
            var3_int = 0;
            L1: while (true) {
              if (-17 > (var3_int ^ -1)) {
                var3_int = 0;
                L2: while (true) {
                  if (-17 >= (var3_int ^ -1)) {
                    v.field_i = ta.a(new int[]{0, 16744383, 256, 0}, param1 + -22022);
                    this.o(10);
                    this.field_K = new ji[12];
                    var3_int = 0;
                    L3: while (true) {
                      if (-7 >= (var3_int ^ -1)) {
                        break L0;
                      } else {
                        var4 = param0.field_c[(var3_int * 4 - -3) % param0.field_c.length];
                        var5 = param0.field_c[(var3_int * 4 - -4) % param0.field_c.length];
                        var6 = param0.field_c[(5 + 4 * var3_int) % param0.field_c.length];
                        var7 = param0.field_c[(6 + 4 * var3_int) % param0.field_c.length];
                        this.field_K[2 * var3_int] = new ji(new int[]{var5.field_a - -var5.field_f >> 777718030, var5.field_j - -var5.field_l >> -1920014322, var6.field_a >> 741297613, var6.field_j >> 167718317, var7.field_a >> 1253804301, var7.field_j >> 429742605});
                        this.field_K[1 + var3_int * 2] = new ji(new int[]{var5.field_a - -var5.field_f >> -1726287826, var5.field_j + var5.field_l >> 136050510, var5.field_a >> -1853557939, var5.field_j >> 2134857261, var4.field_a >> -1003109331, var4.field_j >> 376572653});
                        var3_int++;
                        continue L3;
                      }
                    }
                  } else {
                    var4_int = 0;
                    L4: while (true) {
                      if ((var4_int ^ -1) <= -33) {
                        var3_int++;
                        continue L2;
                      } else {
                        fm.field_K[var3_int][var4_int] = new jd();
                        cn.field_a[var3_int][var4_int] = new jd();
                        var4_int++;
                        continue L4;
                      }
                    }
                  }
                }
              } else {
                var4_int = var3_int * 2304 / 16;
                var5_int = 0;
                L5: while (true) {
                  if (-33 > (var5_int ^ -1)) {
                    var3_int++;
                    continue L1;
                  } else {
                    t.field_a[0][var3_int][var5_int] = 4608 * var5_int / 32;
                    t.field_a[1][var3_int][var5_int] = var4_int;
                    t.field_a[2][var3_int][var5_int] = 0;
                    var5_int++;
                    continue L5;
                  }
                }
              }
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L6: {
            var3 = decompiledCaughtException;
            stackIn_19_0 = (RuntimeException) (var3);

            stackIn_19_1 = new StringBuilder().append("fk.B(");

            if (param0 == null) {
              stackIn_20_0 = (RuntimeException) ((Object) stackIn_19_0);
              stackIn_20_1 = (StringBuilder) ((Object) stackIn_19_1);
              stackIn_20_2 = "null";
              break L6;
            } else {
              stackIn_20_0 = (RuntimeException) ((Object) stackIn_19_0);
              stackIn_20_1 = (StringBuilder) ((Object) stackIn_19_1);
              stackIn_20_2 = "{...}";
              break L6;
            }
          }
          throw wm.a((Throwable) ((Object) stackIn_20_0), stackIn_20_2 + ',' + param1 + ')');
        }
    }

    final void a(int param0, int param1, byte param2, int param3) {
        int var5;
        int var6;
        int var7;
        L0: {
          var7 = Pool.field_O;
          var5 = 119 / ((param2 - -44) / 56);
          super.a(param0, param1, (byte) -104, param3);
          var6 = param0;
          if ((var6 ^ -1) == -6) {
            var6 = ga.a(4, true, ea.field_r);
            if (0 == var6) {
              kp.a(100, this.field_hb, param3, param1);
              break L0;
            } else {
              if (var6 != 1) {
                if (-3 == (var6 ^ -1)) {
                  kp.a(100, this.field_db, param3, param1);
                  break L0;
                } else {
                  if (var6 == 3) {
                    kp.a(100, this.field_T, param3, param1);
                    break L0;
                  } else {
                    break L0;
                  }
                }
              } else {
                kp.a(100, this.field_N, param3, param1);
                break L0;
              }
            }
          } else {
            if (2 != var6) {
              if (3 != var6) {
                if ((var6 ^ -1) == -8) {
                  kp.a(100, this.field_W, param3, param1);
                  break L0;
                } else {
                  if ((var6 ^ -1) == -9) {
                    kp.a(100, this.field_Q, param3, param1);
                    break L0;
                  } else {
                    if (-7 != (var6 ^ -1)) {
                      if ((var6 ^ -1) == -5) {
                        kp.a(100, this.field_J, param3, param1);
                        break L0;
                      } else {
                        if ((var6 ^ -1) != -10) {
                          if (var6 == 10) {
                            kp.a(100, this.field_fb, param3, param1 * 64 >> -1998278968);
                            break L0;
                          } else {
                            break L0;
                          }
                        } else {
                          L1: {
                            if (null == this.field_bb) {
                              break L1;
                            } else {
                              this.a((byte) 73, true, param0);
                              break L1;
                            }
                          }
                          this.field_bb = nb.a(this.field_U, 256, 0);
                          this.field_bb.e(-1);
                          mn.a(-24296, this.field_bb);
                          this.field_bb.d(4096, 16 * param1 >> 1289461288);
                          break L0;
                        }
                      }
                    } else {
                      kp.a(100, this.field_O, param3, param1);
                      System.out.println("playing respot the white");
                      break L0;
                    }
                  }
                }
              } else {
                kp.a(100, this.field_Y, param3, param1);
                break L0;
              }
            } else {
              kp.a(100, this.field_P, param3, param1);
              break L0;
            }
          }
        }
    }

    private final static void a(int[] param0) {
        int var1;
        int var2;
        int var3;
        int var4;
        int var5;
        int var6;
        int var7;
        int var8;
        int var9;
        int var10;
        int var11;
        int var12;
        int var13;
        int[] var15;
        int[] var16;
        int[] stackIn_2_0 = null;
        int[] stackIn_3_0 = null;
        int stackIn_3_1 = 0;
        int var14;
        L0: {
          var16 = f.a((byte) 100, param0);
          var15 = var16;
          param0 = var15;
          param0[0] = param0[0] - 2304;
          param0[1] = param0[1] - 1152;
          param0[0] = param0[0] >> 8;
          param0[1] = param0[1] >> 8;
          param0[2] = param0[2] >> 8;
          var1 = 65536 / qh.field_l;
          mf.field_l = mf.field_l & 127;
          me.field_x = -64 + mf.field_l;
          gn.field_b = 0 + mf.field_l;
          stackIn_2_0 = v.field_i;

          if (mf.field_l >= 16) {
            stackIn_3_0 = (int[]) ((Object) stackIn_2_0);
            stackIn_3_1 = mf.field_l >> 1;
            break L0;
          } else {
            stackIn_3_0 = (int[]) ((Object) stackIn_2_0);
            stackIn_3_1 = (16 - mf.field_l) * (v.field_i.length - 1) >> 4;
            break L0;
          }
        }
        L1: {
          ba.field_tb = stackIn_3_0[stackIn_3_1];
          bm.field_h = 64 + mf.field_l;
          rb.field_g = 128 + mf.field_l;
          cl.field_g = v.field_i[64 + (mf.field_l >> 1)];
          eq.field_Y = 192 + mf.field_l;
          gn.field_d = 256 + mf.field_l;
          pn.field_s = v.field_i[128 + (mf.field_l >> 1)];
          mp.field_O = 320 + mf.field_l;
          ll.field_c = 384 + mf.field_l;
          nm.field_f = v.field_i[192 + (mf.field_l >> 1)];
          jp.field_V = ll.field_c * ll.field_c;
          var2 = qh.field_c;
          var3 = 256 - var16[2];
          var4 = var16[8] * var3;
          var5 = var16[11] * var3;
          var6 = ll.field_c - var16[0];
          var7 = ll.field_c - var16[1];
          var8 = var16[9] * var6 + var16[10] * var7 + var5 >> 16;
          if (var8 <= 0) {
            break L1;
          } else {
            var9 = qh.field_c + fb.field_d + (var16[6] * var6 + var16[7] * var7 + var4) >> 7 / var8;
            if (var9 <= 0) {
              break L1;
            } else {
              if (var9 >= var2) {
                break L1;
              } else {
                var2 = var9;
                break L1;
              }
            }
          }
        }
        L2: {
          var6 = -ll.field_c - var16[0];
          var7 = ll.field_c - var16[1];
          var8 = var16[9] * var6 + var16[10] * var7 + var5 >> 16;
          if (var8 <= 0) {
            break L2;
          } else {
            var9 = qh.field_c + fb.field_d + (var16[6] * var6 + var16[7] * var7 + var4) >> 7 / var8;
            if (var9 <= 0) {
              break L2;
            } else {
              if (var9 >= var2) {
                break L2;
              } else {
                var2 = var9;
                break L2;
              }
            }
          }
        }
        L3: {
          var6 = ll.field_c - var16[0];
          var7 = -ll.field_c - var16[1];
          var8 = var16[9] * var6 + var16[10] * var7 + var5 >> 16;
          if (var8 <= 0) {
            break L3;
          } else {
            var9 = qh.field_c + fb.field_d + (var16[6] * var6 + var16[7] * var7 + var4) >> 7 / var8;
            if (var9 <= 0) {
              break L3;
            } else {
              if (var9 >= var2) {
                break L3;
              } else {
                var2 = var9;
                break L3;
              }
            }
          }
        }
        L4: {
          var6 = -ll.field_c - var16[0];
          var7 = -ll.field_c - var16[1];
          var8 = var16[9] * var6 + var16[10] * var7 + var5 >> 16;
          if (var8 <= 0) {
            break L4;
          } else {
            var9 = qh.field_c + fb.field_d + (var16[6] * var6 + var16[7] * var7 + var4) >> 7 / var8;
            if (var9 <= 0) {
              break L4;
            } else {
              if (var9 >= var2) {
                break L4;
              } else {
                var2 = var9;
                break L4;
              }
            }
          }
        }
        var3 = var2 * qh.field_l;
        var4 = var2;
        L5: while (true) {
          if (var4 >= qh.field_g) {
            return;
          } else {
            var5 = var4 - 240;
            var10 = var16[9] + (var16[3] * (qh.field_b - 320) + var16[6] * var5 >> 9) >> 1;
            var11 = var16[10] + (var16[4] * (qh.field_b - 320) + var16[7] * var5 >> 9) >> 1;
            var12 = var16[11] + (var16[5] * (qh.field_b - 320) + var16[8] * var5 >> 9) >> 1;
            var13 = var10 * var10 + var11 * var11 + var12 * var12 >> 16;
            if (var13 > 0) {
              var13 = rf.a(var13, -126);
              var10 = lh.a(var13, (byte) -82, (long)var10);
              var11 = lh.a(var13, (byte) -125, (long)var11);
              var12 = lh.a(var13, (byte) -109, (long)var12);
              if (var12 != 0) {
                var14 = lh.a(var12, (byte) -62, (long)(256 - var16[2]));
                var6 = var16[0] + (var14 * var10 >> 16);
                var8 = var16[1] + (var14 * var11 >> 16);
                var10 = var16[9] + (var16[3] * (qh.field_j - 320) + var16[6] * var5 >> 9) >> 1;
                var11 = var16[10] + (var16[4] * (qh.field_j - 320) + var16[7] * var5 >> 9) >> 1;
                var12 = var16[11] + (var16[5] * (qh.field_j - 320) + var16[8] * var5 >> 9) >> 1;
                var13 = var10 * var10 + var11 * var11 + var12 * var12 >> 16;
                if (var13 > 0) {
                  var13 = rf.a(var13, -126);
                  var10 = lh.a(var13, (byte) -115, (long)var10);
                  var11 = lh.a(var13, (byte) -115, (long)var11);
                  var12 = lh.a(var13, (byte) -97, (long)var12);
                  if (var12 != 0) {
                    var14 = lh.a(var12, (byte) -100, (long)(256 - var16[2]));
                    var7 = var16[0] + (var14 * var10 >> 16);
                    var9 = var16[1] + (var14 * var11 >> 16);
                    var10 = (var9 - var8) * var1;
                    var11 = (var7 - var6) * var1;
                    var12 = (var8 << 16) + qh.field_b * var10;
                    var13 = (var6 << 16) + qh.field_b * var11;
                    fk.a(0, 0, 0, 0, var12, var13, var10, var11, var3);
                    var3 = var3 + qh.field_l;
                    var4++;
                    continue L5;
                  } else {
                    var4++;
                    continue L5;
                  }
                } else {
                  var4++;
                  continue L5;
                }
              } else {
                var4++;
                continue L5;
              }
            } else {
              var4++;
              continue L5;
            }
          }
        }
    }

    final void d(boolean param0) {
        super.d(param0);
        v.field_i = null;
        t.field_a = (int[][][]) null;
        fm.field_K = (jd[][]) null;
        ro.field_v = (int[][][]) null;
        ma.field_b = (bl[][][]) null;
        cn.field_a = (jd[][]) null;
        this.field_K = null;
    }

    private final static void a(int param0, int param1, int param2, int param3, int param4, int param5, int param6, int param7, int param8) {
        int var9;
        int var10;
        int var11;
        int var12;
        var9 = qh.field_b;
        L0: while (true) {
          if (var9 >= qh.field_j) {
            return;
          } else {
            L1: {
              var10 = param4 >> 16;
              var11 = param5 >> 16;
              if (var11 <= ll.field_c) {
                if (var10 <= ll.field_c) {
                  if (var11 >= -ll.field_c) {
                    if (var10 >= -ll.field_c) {
                      var12 = var11 * var11 + var10 * var10;
                      if (var12 < 0) {
                        break L1;
                      } else {
                        if (var12 <= jp.field_V) {
                          L2: {
                            param1 = ke.a(false, var12);
                            if (param1 <= me.field_x) {
                              break L2;
                            } else {
                              if (param1 >= gn.field_b) {
                                break L2;
                              } else {
                                L3: {
                                  param1 = (param1 - me.field_x - 32) * 255 >> 5;
                                  if (param1 >= 0) {
                                    break L3;
                                  } else {
                                    param1 = -param1;
                                    break L3;
                                  }
                                }
                                param0 = ba.field_tb;
                                param1 = 255 - param1 >> 1;
                                param2 = (param0 & 16711935) * param1;
                                param0 = (param2 & -16711936) + (param0 * param1 - param2 & 16711680) >>> 8;
                                param2 = qh.field_d[param8];
                                param3 = param0 + param2;
                                param0 = (param0 & 16711935) + (param2 & 16711935);
                                param2 = (param0 & 16777472) + (param3 - param0 & 65536);
                                qh.field_d[param8] = param3 - param2 | param2 - (param2 >>> 8);
                                break L1;
                              }
                            }
                          }
                          L4: {
                            if (param1 <= bm.field_h) {
                              break L4;
                            } else {
                              if (param1 >= rb.field_g) {
                                break L4;
                              } else {
                                L5: {
                                  param1 = (param1 - bm.field_h - 32) * 255 >> 5;
                                  if (param1 >= 0) {
                                    break L5;
                                  } else {
                                    param1 = -param1;
                                    break L5;
                                  }
                                }
                                param0 = cl.field_g;
                                param1 = 255 - param1 >> 1;
                                param2 = (param0 & 16711935) * param1;
                                param0 = (param2 & -16711936) + (param0 * param1 - param2 & 16711680) >>> 8;
                                param2 = qh.field_d[param8];
                                param3 = param0 + param2;
                                param0 = (param0 & 16711935) + (param2 & 16711935);
                                param2 = (param0 & 16777472) + (param3 - param0 & 65536);
                                qh.field_d[param8] = param3 - param2 | param2 - (param2 >>> 8);
                                break L1;
                              }
                            }
                          }
                          L6: {
                            if (param1 <= eq.field_Y) {
                              break L6;
                            } else {
                              if (param1 >= gn.field_d) {
                                break L6;
                              } else {
                                L7: {
                                  param1 = (param1 - eq.field_Y - 32) * 255 >> 5;
                                  if (param1 >= 0) {
                                    break L7;
                                  } else {
                                    param1 = -param1;
                                    break L7;
                                  }
                                }
                                param0 = pn.field_s;
                                param1 = 255 - param1 >> 1;
                                param2 = (param0 & 16711935) * param1;
                                param0 = (param2 & -16711936) + (param0 * param1 - param2 & 16711680) >>> 8;
                                param2 = qh.field_d[param8];
                                param3 = param0 + param2;
                                param0 = (param0 & 16711935) + (param2 & 16711935);
                                param2 = (param0 & 16777472) + (param3 - param0 & 65536);
                                qh.field_d[param8] = param3 - param2 | param2 - (param2 >>> 8);
                                break L1;
                              }
                            }
                          }
                          if (param1 <= mp.field_O) {
                            break L1;
                          } else {
                            if (param1 >= ll.field_c) {
                              break L1;
                            } else {
                              L8: {
                                param1 = (param1 - mp.field_O - 32) * 255 >> 5;
                                if (param1 >= 0) {
                                  break L8;
                                } else {
                                  param1 = -param1;
                                  break L8;
                                }
                              }
                              param0 = nm.field_f;
                              param1 = 255 - param1 >> 1;
                              param2 = (param0 & 16711935) * param1;
                              param0 = (param2 & -16711936) + (param0 * param1 - param2 & 16711680) >>> 8;
                              param2 = qh.field_d[param8];
                              param3 = param0 + param2;
                              param0 = (param0 & 16711935) + (param2 & 16711935);
                              param2 = (param0 & 16777472) + (param3 - param0 & 65536);
                              qh.field_d[param8] = param3 - param2 | param2 - (param2 >>> 8);
                              break L1;
                            }
                          }
                        } else {
                          break L1;
                        }
                      }
                    } else {
                      break L1;
                    }
                  } else {
                    break L1;
                  }
                } else {
                  break L1;
                }
              } else {
                break L1;
              }
            }
            param4 = param4 + param6;
            param5 = param5 + param7;
            param8++;
            var9++;
            continue L0;
          }
        }
    }

    final int h(int param0) {
        if (param0 != 684) {
            this.n(16);
        }
        return 0;
    }

    final void a(cf param0, int param1) {
        try {
            this.field_P = param0.a("", "plasma_ball_hard", (byte) 121);
            this.field_Y = param0.a("", "plasma_ball_soft", (byte) 126);
            this.field_W = param0.a("", "plasma_cue_hard", (byte) 126);
            int var3_int = -128 % ((39 - param1) / 38);
            this.field_Q = param0.a("", "plasma_cue_soft", (byte) 127);
            this.field_J = param0.a("", "plasma_pot", (byte) 123);
            this.field_O = param0.a("", "plasma_respot_the_white", (byte) 125);
            this.field_hb = param0.a("", "plasma_cushion1", (byte) 127);
            this.field_N = param0.a("", "plasma_cushion2", (byte) 124);
            this.field_db = param0.a("", "plasma_cushion3", (byte) 124);
            this.field_T = param0.a("", "plasma_cushion4", (byte) 122);
            this.field_fb = param0.a("", "plasma_loop", (byte) 125);
            this.field_U = param0.a("", "plasma_shot_mode_loop", (byte) 123);
        } catch (RuntimeException runtimeException) {
            throw wm.a((Throwable) ((Object) runtimeException), "fk.M(" + (param0 != null ? "{...}" : "null") + ',' + param1 + ')');
        }
    }

    final byte g(int param0) {
        int var2 = 83 % ((9 - param0) / 63);
        return (byte) -128;
    }

    private final void a(pq[] param0, byte param1) {
        jd dupTemp$0 = null;
        RuntimeException stackIn_37_0 = null;
        StringBuilder stackIn_37_1 = null;
        RuntimeException stackIn_38_0 = null;
        StringBuilder stackIn_38_1 = null;
        String stackIn_38_2 = null;
        int decompiledRegionSelector0 = 0;
        RuntimeException decompiledCaughtException = null;
        int var3_int = 0;
        jd[] var3 = null;
        RuntimeException var3_ref = null;
        jd[] var4 = null;
        int var4_int = 0;
        jd[] var5 = null;
        int var5_int = 0;
        int var6_int = 0;
        jd[][] var6 = null;
        jd var7 = null;
        jd var8 = null;
        jd var9 = null;
        jd var10 = null;
        jd var11 = null;
        jd var12 = null;
        jd var13 = null;
        jd var14 = null;
        jd var15 = null;
        int var16 = 0;
        int var17 = 0;
        int var18 = 0;
        var18 = Pool.field_O;
        try {
          L0: {
            var3_int = 0;
            L1: while (true) {
              if (param0.length <= var3_int) {
                var3 = fm.field_K[0];
                var4 = var3;
                var5 = var4;
                var6_int = 0;
                if (param1 == -122) {
                  L2: while (true) {
                    if (fm.field_K.length <= var6_int) {
                      var6 = fm.field_K;
                      fm.field_K = cn.field_a;
                      cn.field_a = var6;
                      decompiledRegionSelector0 = 1;
                      break L0;
                    } else {
                      L3: {
                        if (var6_int + 1 >= fm.field_K.length) {
                          break L3;
                        } else {
                          var5 = fm.field_K[1 + var6_int];
                          break L3;
                        }
                      }
                      var7 = var3[0];
                      var8 = var7;
                      var9 = var8;
                      var10 = var4[0];
                      var11 = var10;
                      var12 = var11;
                      var13 = var5[0];
                      var14 = var13;
                      var15 = var14;
                      var16 = 0;
                      L4: while (true) {
                        if (var16 >= var4.length) {
                          var3 = var4;
                          var4 = var5;
                          var6_int++;
                          continue L2;
                        } else {
                          L5: {
                            if ((1 + var16 ^ -1) > (var4.length ^ -1)) {
                              var12 = var4[1 + var16];
                              var15 = var5[1 + var16];
                              var9 = var3[var16 + 1];
                              break L5;
                            } else {
                              break L5;
                            }
                          }
                          L6: {
                            var17 = (10 * var14.field_g >> -1093222840) + ((var12.field_g * 10 >> 486663240) + (var10.field_g * 10 >> -1412194712) + ((5 * var9.field_g >> 1788707528) + ((5 * var7.field_g >> -865602264) - -(10 * var8.field_g >> 1730619720) - -(var11.field_g * 180 >> 1495002312)))) + ((5 * var13.field_g >> -1945874744) - -(5 * var15.field_g >> 35663080));
                            if (-4097 > (var17 ^ -1)) {
                              var17 = 4096;
                              break L6;
                            } else {
                              break L6;
                            }
                          }
                          L7: {
                            if ((var17 ^ -1) > -1) {
                              var17 = 0;
                              break L7;
                            } else {
                              break L7;
                            }
                          }
                          cn.field_a[var6_int][var16].field_g = var17;
                          var7 = var8;
                          var10 = var11;
                          var8 = var9;
                          var13 = var14;
                          var11 = var12;
                          var14 = var15;
                          var16++;
                          continue L4;
                        }
                      }
                    }
                  }
                } else {
                  decompiledRegionSelector0 = 0;
                  break L0;
                }
              } else {
                if (param0[var3_int].field_s) {
                  L8: {
                    var4_int = (param0[var3_int].field_g >> -1113032755) * 32 / 4608;
                    var5_int = 16 * (param0[var3_int].field_k >> -17402643) / 2304;
                    if ((var4_int ^ -1) <= -1) {
                      if (-33 < (var4_int ^ -1)) {
                        break L8;
                      } else {
                        var4_int = 31;
                        break L8;
                      }
                    } else {
                      var4_int = 0;
                      break L8;
                    }
                  }
                  L9: {
                    if ((var5_int ^ -1) <= -1) {
                      if (16 <= var5_int) {
                        var5_int = 15;
                        break L9;
                      } else {
                        break L9;
                      }
                    } else {
                      var5_int = 0;
                      break L9;
                    }
                  }
                  dupTemp$0 = fm.field_K[var5_int][var4_int];
                  dupTemp$0.field_g = dupTemp$0.field_g + (1024 - -ga.a(256, true, ea.field_r));
                  var3_int++;
                  continue L1;
                } else {
                  var3_int++;
                  continue L1;
                }
              }
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L10: {
            var3_ref = decompiledCaughtException;
            stackIn_37_0 = (RuntimeException) (var3_ref);

            stackIn_37_1 = new StringBuilder().append("fk.U(");

            if (param0 == null) {
              stackIn_38_0 = (RuntimeException) ((Object) stackIn_37_0);
              stackIn_38_1 = (StringBuilder) ((Object) stackIn_37_1);
              stackIn_38_2 = "null";
              break L10;
            } else {
              stackIn_38_0 = (RuntimeException) ((Object) stackIn_37_0);
              stackIn_38_1 = (StringBuilder) ((Object) stackIn_37_1);
              stackIn_38_2 = "{...}";
              break L10;
            }
          }
          throw wm.a((Throwable) ((Object) stackIn_38_0), stackIn_38_2 + ',' + param1 + ')');
        }
        if (decompiledRegionSelector0 == 0) {
          return;
        } else {
          return;
        }
    }

    final void a(byte param0, int[] param1) {
        RuntimeException stackIn_13_0 = null;
        StringBuilder stackIn_13_1 = null;
        RuntimeException stackIn_14_0 = null;
        StringBuilder stackIn_14_1 = null;
        String stackIn_14_2 = null;
        RuntimeException decompiledCaughtException = null;
        RuntimeException var3 = null;
        int var4 = 0;
        int var5 = 0;
        int var6 = 0;
        int var7 = 0;
        int var8 = 0;
        int[] var9 = null;
        int[] var13 = null;
        var8 = Pool.field_O;
        try {
          L0: {
            var9 = f.a((byte) 96, param1);
            var13 = var9;
            var9[0] = var9[0] - 2304;
            var9[1] = var9[1] - 1152;
            var9[2] = var9[2] >> 0;
            var9[1] = var9[1] >> 0;
            var9[0] = var9[0] >> 0;
            var9[1] = var9[1] >> 8;
            var9[0] = var9[0] >> 8;
            var4 = -81 / ((param0 - -64) / 52);
            var9[2] = var9[2] >> 8;
            var5 = 0;
            L1: while (true) {
              if (ma.field_b.length <= var5) {
                break L0;
              } else {
                var6 = 0;
                L2: while (true) {
                  if (var6 >= ma.field_b[var5].length) {
                    var5++;
                    continue L1;
                  } else {
                    var7 = 0;
                    L3: while (true) {
                      if (var7 >= ma.field_b[var5][var6].length) {
                        var6++;
                        continue L2;
                      } else {
                        ma.field_b[var5][var6][var7].a(-56, var13);
                        var7++;
                        continue L3;
                      }
                    }
                  }
                }
              }
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L4: {
            var3 = decompiledCaughtException;
            stackIn_13_0 = (RuntimeException) (var3);

            stackIn_13_1 = new StringBuilder().append("fk.N(").append(param0).append(',');

            if (param1 == null) {
              stackIn_14_0 = (RuntimeException) ((Object) stackIn_13_0);
              stackIn_14_1 = (StringBuilder) ((Object) stackIn_13_1);
              stackIn_14_2 = "null";
              break L4;
            } else {
              stackIn_14_0 = (RuntimeException) ((Object) stackIn_13_0);
              stackIn_14_1 = (StringBuilder) ((Object) stackIn_13_1);
              stackIn_14_2 = "{...}";
              break L4;
            }
          }
          throw wm.a((Throwable) ((Object) stackIn_14_0), stackIn_14_2 + ')');
        }
    }

    final void b(int param0, int param1) {
        if (!(this.field_bb == null)) {
            this.field_bb.f(64 * param1 / 4096 + 256);
        }
        int var3 = -113 % ((param0 - 11) / 52);
    }

    public static void m(int param0) {
        field_eb = null;
        field_cb = null;
        field_gb = null;
        int var1 = -122 / ((param0 - -65) / 38);
    }

    private final void a(boolean param0, int[] param1, uf param2) {
        int stackIn_26_0 = 0;
        int stackIn_29_1 = 0;
        RuntimeException stackIn_34_0 = null;
        StringBuilder stackIn_34_1 = null;
        RuntimeException stackIn_35_0 = null;
        StringBuilder stackIn_35_1 = null;
        String stackIn_35_2 = null;
        StringBuilder stackIn_37_1 = null;
        StringBuilder stackIn_38_1 = null;
        String stackIn_38_2 = null;
        RuntimeException decompiledCaughtException = null;
        int var4_int = 0;
        RuntimeException var4 = null;
        int var5_int = 0;
        int[] var5 = null;
        int var6 = 0;
        int var7 = 0;
        int var8 = 0;
        int var9 = 0;
        int var10 = 0;
        var9 = Pool.field_O;
        try {
          L0: {
            L1: {
              if (!param0) {
                break L1;
              } else {
                this.g(2);
                break L1;
              }
            }
            L2: {
              if (param2.k(-38)) {
                L3: {
                  if (!id.field_c.field_g) {
                    break L3;
                  } else {
                    param2.field_I.a(id.field_c.field_l, param2.field_G.field_k, (byte) 47, param2.field_G.field_h, param2.field_G.field_e, param2.field_G.field_d);
                    id.field_c.field_g = false;
                    break L3;
                  }
                }
                var4_int = param2.field_f;
                this.field_R[0] = (id.field_c.field_l[0] >> 855981741) + (var4_int * id.field_c.field_l[9] >> 1256540685);
                this.field_X[0] = (id.field_c.field_l[10] * var4_int >> -90699795) + (id.field_c.field_l[1] >> 558844909);
                this.field_L[0] = (id.field_c.field_l[11] * var4_int >> 85220429) + (id.field_c.field_l[2] >> 483161549);
                this.field_M[0] = 1;
                this.field_R[1] = ((var4_int + 456) * id.field_c.field_l[9] >> -92631571) + (id.field_c.field_l[0] >> 1542128109);
                this.field_X[1] = ((456 - -var4_int) * id.field_c.field_l[10] >> 779812205) + (id.field_c.field_l[1] >> -487783603);
                this.field_L[1] = (id.field_c.field_l[11] * (456 - -var4_int) >> 1562723661) + (id.field_c.field_l[2] >> 240674253);
                this.field_M[1] = 3;
                var5_int = 0;
                L4: while (true) {
                  if (this.field_R.length <= var5_int) {
                    L5: {
                      if (this.field_L[0] > 0) {
                        break L5;
                      } else {
                        if (this.field_L[1] > 0) {
                          break L5;
                        } else {
                          param2.D(-125);
                          break L2;
                        }
                      }
                    }
                    L6: {
                      if (!hk.a(113, this.field_R[1], this.field_R[0], this.field_X[1], this.field_L[1], this.field_L[0], this.field_X[0])) {
                        this.field_S = ic.a(this.field_X, 0, this.field_R, this.field_M, this.field_L, 4);
                        var10 = 0;
                        var5_int = var10;
                        L7: while (true) {
                          if (this.field_R.length <= var10) {
                            L8: {
                              var5 = bc.field_c;
                              if (param2.field_I.field_f instanceof af) {
                                var6 = ((af) ((Object) param2.field_I.field_f)).a(-1);
                                if (-1 == var6) {
                                  break L8;
                                } else {
                                  var5 = hf.a(var6, (byte) 73);
                                  break L8;
                                }
                              } else {
                                break L8;
                              }
                            }
                            L9: {
                              if (0 != (1 & this.field_S)) {
                                stackIn_26_0 = 2;
                                break L9;
                              } else {
                                stackIn_26_0 = 1;
                                break L9;
                              }
                            }
                            L10: {


                              if ((this.field_S & 2) != 0) {

                                stackIn_29_1 = 2;
                                break L10;
                              } else {

                                stackIn_29_1 = 1;
                                break L10;
                              }
                            }
                            wn.a((byte) stackIn_26_0, (byte) stackIn_29_1, var5, this.field_X, 127, 0, this.field_R, this.field_M);
                            break L6;
                          } else {
                            this.field_R[var10] = 1280 - -((this.field_R[var10] << 1192030443) / this.field_L[var10]);
                            this.field_X[var10] = 960 - -((this.field_X[var10] << -576204469) / this.field_L[var10]);
                            this.field_M[var10] = (this.field_M[var10] << 1349214896) / this.field_L[var10];
                            var10++;
                            continue L7;
                          }
                        }
                      } else {
                        break L6;
                      }
                    }
                    param2.D(-125);
                    break L2;
                  } else {
                    var6 = this.field_R[var5_int] + -param1[0];
                    var7 = -param1[1] + this.field_X[var5_int];
                    var8 = -param1[2] + this.field_L[var5_int];
                    this.field_R[var5_int] = param1[3] * var6 + var7 * param1[4] - -(param1[5] * var8) >> 283284880;
                    this.field_X[var5_int] = param1[8] * var8 + (var7 * param1[7] + param1[6] * var6) >> 362764336;
                    this.field_L[var5_int] = var8 * param1[11] + (param1[9] * var6 - -(var7 * param1[10])) >> -1329703024;
                    var5_int++;
                    continue L4;
                  }
                }
              } else {
                break L2;
              }
            }
            break L0;
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L11: {
            var4 = decompiledCaughtException;
            stackIn_34_0 = (RuntimeException) (var4);

            stackIn_34_1 = new StringBuilder().append("fk.K(").append(param0).append(',');

            if (param1 == null) {
              stackIn_35_0 = (RuntimeException) ((Object) stackIn_34_0);
              stackIn_35_1 = (StringBuilder) ((Object) stackIn_34_1);
              stackIn_35_2 = "null";
              break L11;
            } else {
              stackIn_35_0 = (RuntimeException) ((Object) stackIn_34_0);
              stackIn_35_1 = (StringBuilder) ((Object) stackIn_34_1);
              stackIn_35_2 = "{...}";
              break L11;
            }
          }
          L12: {


            stackIn_37_1 = ((StringBuilder) (Object) stackIn_35_1).append(stackIn_35_2).append(',');

            if (param2 == null) {
              stackIn_35_0 = (RuntimeException) ((Object) stackIn_35_0);
              stackIn_38_1 = (StringBuilder) ((Object) stackIn_37_1);
              stackIn_38_2 = "null";
              break L12;
            } else {
              stackIn_35_0 = (RuntimeException) ((Object) stackIn_35_0);
              stackIn_38_1 = (StringBuilder) ((Object) stackIn_37_1);
              stackIn_38_2 = "{...}";
              break L12;
            }
          }
          throw wm.a((Throwable) ((Object) stackIn_35_0), stackIn_38_2 + ')');
        }
    }

    final void a(int param0, uf param1) {
        int var3_int = 0;
        int var4 = Pool.field_O;
        try {
            super.a(param0, param1);
            for (var3_int = 0; var3_int < this.field_K.length; var3_int++) {
                this.field_K[var3_int].a((byte) -43);
            }
            this.n(2);
            this.a(param1.field_I.field_l, (byte) -122);
            mf.field_l = mf.field_l - 1;
        } catch (RuntimeException runtimeException) {
            throw wm.a((Throwable) ((Object) runtimeException), "fk.T(" + param0 + ',' + (param1 != null ? "{...}" : "null") + ')');
        }
    }

    final void a(int param0, di param1) {
        try {
            this.field_b = cg.a(param1, "", "pool_plasma");
            this.field_i = cg.a(param1, "", "pool_plasma_win_jingle");
            this.field_n = cg.a(param1, "", "pool_plasma_lose_jingle");
            if (param0 != 1) {
                field_gb = (hr) null;
            }
        } catch (RuntimeException runtimeException) {
            throw wm.a((Throwable) ((Object) runtimeException), "fk.Q(" + param0 + ',' + (param1 != null ? "{...}" : "null") + ')');
        }
    }

    private final void n(int param0) {
        int var2;
        bl[][] var3;
        int var4_int;
        bl[] var4;
        bl[] var5;
        bl[] var6;
        int var6_int;
        int var7;
        bl var7_ref_bl;
        bl var8;
        bl var9;
        bl var10;
        bl var11;
        bl var12;
        bl var13;
        bl var14;
        bl var15;
        bl var16;
        int var17;
        int var18;
        bl[] var19;
        var18 = Pool.field_O;
        if (param0 == 2) {
          var2 = 0;
          L0: while (true) {
            if (var2 >= ma.field_b.length) {
              return;
            } else {
              var3 = ma.field_b[var2];
              var4_int = 0;
              L1: while (true) {
                if (-17 >= (var4_int ^ -1)) {
                  var4 = var3[0];
                  var5 = var4;
                  var6 = var5;
                  var7 = 0;
                  L2: while (true) {
                    if (var7 >= 16) {
                      var2++;
                      continue L0;
                    } else {
                      L3: {
                        if (var7 >= 15) {
                          break L3;
                        } else {
                          var6 = var3[1 + var7];
                          break L3;
                        }
                      }
                      var8 = var4[0];
                      var9 = var8;
                      var10 = var9;
                      var11 = var5[0];
                      var12 = var11;
                      var13 = var12;
                      var14 = var6[0];
                      var15 = var14;
                      var16 = var15;
                      var17 = 0;
                      L4: while (true) {
                        if (var17 >= 16) {
                          var4 = var5;
                          var5 = var6;
                          var7++;
                          continue L2;
                        } else {
                          L5: {
                            if (15 > var17) {
                              var16 = var6[var17 - -1];
                              var13 = var5[var17 + 1];
                              break L5;
                            } else {
                              break L5;
                            }
                          }
                          L6: {
                            if (128 >= var12.field_E) {
                              break L6;
                            } else {
                              L7: {
                                L8: {
                                  if (-1 == (var11.field_E ^ -1)) {
                                    L9: {
                                      if (ga.a(250, true, ea.field_r) == 0) {
                                        var11.field_E = var11.field_E + 1;
                                        break L9;
                                      } else {
                                        break L9;
                                      }
                                    }
                                    if (var13.field_E == 0) {
                                      if (ga.a(250, true, ea.field_r) == 0) {
                                        var13.field_E = var13.field_E + 1;
                                        break L8;
                                      } else {
                                        if (var9.field_E != 0) {
                                          break L7;
                                        } else {
                                          if (ga.a(250, true, ea.field_r) != 0) {
                                            break L7;
                                          } else {
                                            var9.field_E = var9.field_E + 1;
                                            break L7;
                                          }
                                        }
                                      }
                                    } else {
                                      if (var9.field_E != 0) {
                                        break L7;
                                      } else {
                                        if (ga.a(250, true, ea.field_r) != 0) {
                                          break L7;
                                        } else {
                                          var9.field_E = var9.field_E + 1;
                                          break L7;
                                        }
                                      }
                                    }
                                  } else {
                                    if (var13.field_E == 0) {
                                      if (ga.a(250, true, ea.field_r) == 0) {
                                        var13.field_E = var13.field_E + 1;
                                        break L8;
                                      } else {
                                        if (var9.field_E != 0) {
                                          break L7;
                                        } else {
                                          if (ga.a(250, true, ea.field_r) != 0) {
                                            break L7;
                                          } else {
                                            var9.field_E = var9.field_E + 1;
                                            break L7;
                                          }
                                        }
                                      }
                                    } else {
                                      if (var9.field_E != 0) {
                                        break L7;
                                      } else {
                                        if (ga.a(250, true, ea.field_r) != 0) {
                                          break L7;
                                        } else {
                                          var9.field_E = var9.field_E + 1;
                                          break L7;
                                        }
                                      }
                                    }
                                  }
                                }
                                if (var9.field_E != 0) {
                                  break L7;
                                } else {
                                  if (ga.a(250, true, ea.field_r) != 0) {
                                    break L7;
                                  } else {
                                    var9.field_E = var9.field_E + 1;
                                    break L7;
                                  }
                                }
                              }
                              if (var15.field_E != 0) {
                                break L6;
                              } else {
                                if (0 != ga.a(250, true, ea.field_r)) {
                                  break L6;
                                } else {
                                  var15.field_E = var15.field_E + 1;
                                  break L6;
                                }
                              }
                            }
                          }
                          L10: {
                            if (-1 > (var12.field_y ^ -1)) {
                              L11: {
                                L12: {
                                  if (!var8.field_c) {
                                    if (var8.field_E == 0) {
                                      var8.field_c = true;
                                      if (var10.field_c) {
                                        break L12;
                                      } else {
                                        if (-1 != (var10.field_E ^ -1)) {
                                          break L12;
                                        } else {
                                          var10.field_c = true;
                                          break L12;
                                        }
                                      }
                                    } else {
                                      if (!var10.field_c) {
                                        if (-1 != (var10.field_E ^ -1)) {
                                          break L12;
                                        } else {
                                          var10.field_c = true;
                                          break L12;
                                        }
                                      } else {
                                        if (var14.field_c) {
                                          break L11;
                                        } else {
                                          if (0 != var14.field_E) {
                                            break L11;
                                          } else {
                                            var14.field_c = true;
                                            break L11;
                                          }
                                        }
                                      }
                                    }
                                  } else {
                                    if (var10.field_c) {
                                      break L12;
                                    } else {
                                      if (-1 != (var10.field_E ^ -1)) {
                                        break L12;
                                      } else {
                                        var10.field_c = true;
                                        break L12;
                                      }
                                    }
                                  }
                                }
                                if (var14.field_c) {
                                  break L11;
                                } else {
                                  if (0 != var14.field_E) {
                                    break L11;
                                  } else {
                                    var14.field_c = true;
                                    break L11;
                                  }
                                }
                              }
                              L13: {
                                if (var16.field_c) {
                                  break L13;
                                } else {
                                  if (-1 == (var16.field_E ^ -1)) {
                                    var16.field_c = true;
                                    break L13;
                                  } else {
                                    break L13;
                                  }
                                }
                              }
                              var8.field_E = var8.field_E + 1;
                              var14.field_E = var14.field_E + 1;
                              var10.field_E = var10.field_E + 1;
                              var16.field_E = var16.field_E + 1;
                              break L10;
                            } else {
                              break L10;
                            }
                          }
                          var8 = var9;
                          var9 = var10;
                          var11 = var12;
                          var12 = var13;
                          var14 = var15;
                          var15 = var16;
                          var17++;
                          continue L4;
                        }
                      }
                    }
                  }
                } else {
                  var19 = var3[var4_int];
                  var6_int = 0;
                  L14: while (true) {
                    if (-17 >= (var6_int ^ -1)) {
                      var4_int++;
                      continue L1;
                    } else {
                      L15: {
                        var7_ref_bl = var19[var6_int];
                        if ((var7_ref_bl.field_E ^ -1) < -1) {
                          if (!var7_ref_bl.field_c) {
                            var7_ref_bl.field_E = var7_ref_bl.field_E - 2;
                            if (var7_ref_bl.field_E > 0) {
                              break L15;
                            } else {
                              var7_ref_bl.field_E = 0;
                              break L15;
                            }
                          } else {
                            var7_ref_bl.field_E = var7_ref_bl.field_E + 2;
                            if (-257 >= (var7_ref_bl.field_E ^ -1)) {
                              L16: {
                                if (0 != ga.a(10, true, ea.field_r)) {
                                  break L16;
                                } else {
                                  var7_ref_bl.field_y = var7_ref_bl.field_y + 1;
                                  break L16;
                                }
                              }
                              var7_ref_bl.field_c = false;
                              break L15;
                            } else {
                              break L15;
                            }
                          }
                        } else {
                          if (ga.a(2500, true, ea.field_r) == 0) {
                            var7_ref_bl.field_E = 1;
                            var7_ref_bl.field_c = true;
                            break L15;
                          } else {
                            var7_ref_bl.field_E = 0;
                            break L15;
                          }
                        }
                      }
                      if (-1 > (var7_ref_bl.field_y ^ -1)) {
                        var7_ref_bl.field_y = var7_ref_bl.field_y + 1;
                        if (var7_ref_bl.field_y >= 256) {
                          var7_ref_bl.field_y = 0;
                          var6_int++;
                          continue L14;
                        } else {
                          var6_int++;
                          continue L14;
                        }
                      } else {
                        var6_int++;
                        continue L14;
                      }
                    }
                  }
                }
              }
            }
          }
        } else {
          return;
        }
    }

    final void a(byte param0, uf param1) {
        int stackIn_18_0 = 0;
        int stackIn_21_1 = 0;
        Object stackIn_28_0;
        int[] stackIn_28_1;
        int stackIn_28_2;
        int stackIn_28_3;
        pq[] stackIn_28_4;
        Object stackIn_29_0;
        int[] stackIn_29_1;
        int stackIn_29_2;
        int stackIn_29_3;
        pq[] stackIn_29_4;
        int stackIn_29_5;
        RuntimeException stackIn_38_0 = null;
        StringBuilder stackIn_38_1 = null;
        RuntimeException stackIn_39_0 = null;
        StringBuilder stackIn_39_1 = null;
        String stackIn_39_2 = null;
        RuntimeException decompiledCaughtException = null;
        pq[] var3 = null;
        RuntimeException var3_ref = null;
        int[] var4 = null;
        int[] var5_ref_int__ = null;
        int var5 = 0;
        int var6_int = 0;
        int[] var6 = null;
        int var7 = 0;
        int var8 = 0;
        int var9 = 0;
        int var10 = 0;
        int[] var11 = null;
        int var12 = 0;
        int[] var13 = null;
        pq[] var14 = null;
        int[] var15 = null;
        int[] var16 = null;
        int[] var17 = null;
        var10 = Pool.field_O;
        try {
          L0: {
            var14 = param1.field_I.field_l;
            var3 = var14;
            var16 = param1.field_ab;
            var13 = var16;
            var4 = var13;
            this.a(false, var16, var14);
            en.b(0);
            fk.a(var16);
            var17 = f.a((byte) 77, var16);
            var15 = var17;
            var5_ref_int__ = var15;
            var5_ref_int__[0] = var5_ref_int__[0] - 2304;
            var5_ref_int__[1] = var5_ref_int__[1] - 1152;
            var5_ref_int__[2] = var5_ref_int__[2] >> 2;
            var5_ref_int__[1] = var5_ref_int__[1] >> 2;
            var5_ref_int__[0] = var5_ref_int__[0] >> 2;
            this.field_R[0] = 0;
            this.field_X[0] = 0;
            this.field_L[0] = 16384;
            this.field_R[1] = 0;
            this.field_M[0] = 4;
            this.field_X[1] = 0;
            this.field_L[1] = 1;
            this.field_M[1] = 4;
            var6_int = 0;
            L1: while (true) {
              if (var6_int >= this.field_R.length) {
                L2: {
                  L3: {
                    if ((this.field_L[0] ^ -1) < -1) {
                      break L3;
                    } else {
                      if (-1 <= (this.field_L[1] ^ -1)) {
                        break L2;
                      } else {
                        break L3;
                      }
                    }
                  }
                  if (!hk.a(59, this.field_R[1], this.field_R[0], this.field_X[1], this.field_L[1], this.field_L[0], this.field_X[0])) {
                    this.field_S = ic.a(this.field_X, 0, this.field_R, this.field_M, this.field_L, 4);
                    var12 = 0;
                    var6_int = var12;
                    L4: while (true) {
                      if (this.field_R.length <= var12) {
                        L5: {
                          var6 = bc.field_c;
                          if (!(param1.field_I.field_f instanceof af)) {
                            break L5;
                          } else {
                            var7 = ((af) ((Object) param1.field_I.field_f)).a(-1);
                            if ((var7 ^ -1) == 0) {
                              break L5;
                            } else {
                              var6 = hf.a(var7, (byte) 96);
                              break L5;
                            }
                          }
                        }
                        L6: {
                          if ((1 & this.field_S) == 0) {
                            stackIn_18_0 = 1;
                            break L6;
                          } else {
                            stackIn_18_0 = 2;
                            break L6;
                          }
                        }
                        L7: {


                          if (-1 == (2 & this.field_S ^ -1)) {

                            stackIn_21_1 = 1;
                            break L7;
                          } else {

                            stackIn_21_1 = 2;
                            break L7;
                          }
                        }
                        wn.a((byte) stackIn_18_0, (byte) stackIn_21_1, var6, this.field_X, 127, 0, this.field_R, this.field_M);
                        break L2;
                      } else {
                        this.field_R[var12] = 1280 - -((this.field_R[var12] << 1521114923) / this.field_L[var12]);
                        this.field_X[var12] = (this.field_X[var12] << -19690805) / this.field_L[var12] + 960;
                        this.field_M[var12] = (this.field_M[var12] << 2053933520) / this.field_L[var12];
                        var12++;
                        continue L4;
                      }
                    }
                  } else {
                    break L2;
                  }
                }
                this.b((byte) 44, var16);
                this.a(var16, (byte) 79, false, var14, false);
                this.a(77, var16);
                this.c((byte) -25, var16);
                var5 = 0;
                L8: while (true) {
                  if (this.field_K.length <= var5) {
                    L9: {
                      if (0L != (pd.field_i & 1073741824L)) {
                        break L9;
                      } else {
                        L10: {
                          stackIn_28_0 = this;

                          stackIn_28_1 = (int[]) (var4);

                          stackIn_28_2 = 122;

                          stackIn_28_3 = 1;

                          stackIn_28_4 = (pq[]) (var3);

                          if (-1 != (param1.field_I.j(20).field_q ^ -1)) {
                            stackIn_29_0 = this;
                            stackIn_29_1 = (int[]) ((Object) stackIn_28_1);
                            stackIn_29_2 = stackIn_28_2;
                            stackIn_29_3 = stackIn_28_3;
                            stackIn_29_4 = (pq[]) ((Object) stackIn_28_4);
                            stackIn_29_5 = 0;
                            break L10;
                          } else {
                            stackIn_29_0 = this;
                            stackIn_29_1 = (int[]) ((Object) stackIn_28_1);
                            stackIn_29_2 = stackIn_28_2;
                            stackIn_29_3 = stackIn_28_3;
                            stackIn_29_4 = (pq[]) ((Object) stackIn_28_4);
                            stackIn_29_5 = 1;
                            break L10;
                          }
                        }
                        this.a(stackIn_29_1, (byte) stackIn_29_2, stackIn_29_3 != 0, stackIn_29_4, stackIn_29_5 != 0);
                        break L9;
                      }
                    }
                    L11: {
                      if (param0 == -55) {
                        break L11;
                      } else {
                        var11 = (int[]) null;
                        this.c((byte) 102, (int[]) null);
                        break L11;
                      }
                    }
                    L12: {
                      if (-1L == (1073741824L & pd.field_i ^ -1L)) {
                        this.a(false, var16, param1);
                        break L12;
                      } else {
                        break L12;
                      }
                    }
                    break L0;
                  } else {
                    this.field_K[var5].a(false, (byte) 98, var16);
                    var5++;
                    continue L8;
                  }
                }
              } else {
                var7 = this.field_R[var6_int] + -var17[0];
                var8 = this.field_X[var6_int] - var17[1];
                var9 = this.field_L[var6_int] + -var17[2];
                this.field_R[var6_int] = var9 * var17[5] + var17[3] * var7 + var17[4] * var8 >> 1826206160;
                this.field_X[var6_int] = var17[7] * var8 + (var17[6] * var7 + var9 * var17[8]) >> -1888273488;
                this.field_L[var6_int] = var17[11] * var9 + (var17[9] * var7 - -(var8 * var17[10])) >> -625361968;
                var6_int++;
                continue L1;
              }
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L13: {
            var3_ref = decompiledCaughtException;
            stackIn_38_0 = (RuntimeException) (var3_ref);

            stackIn_38_1 = new StringBuilder().append("fk.W(").append(param0).append(',');

            if (param1 == null) {
              stackIn_39_0 = (RuntimeException) ((Object) stackIn_38_0);
              stackIn_39_1 = (StringBuilder) ((Object) stackIn_38_1);
              stackIn_39_2 = "null";
              break L13;
            } else {
              stackIn_39_0 = (RuntimeException) ((Object) stackIn_38_0);
              stackIn_39_1 = (StringBuilder) ((Object) stackIn_38_1);
              stackIn_39_2 = "{...}";
              break L13;
            }
          }
          throw wm.a((Throwable) ((Object) stackIn_39_0), stackIn_39_2 + ')');
        }
    }

    private final void c(byte param0, int[] param1) {
        int var3_int = 0;
        int[] var4 = null;
        int[] var5 = null;
        int[] var6 = null;
        int[] var7 = null;
        int[] var8 = null;
        int[] var9 = null;
        int var10 = 0;
        int var11 = 0;
        int var12 = 0;
        int var13 = 0;
        int var14 = 0;
        int var15 = 0;
        jd[] var16 = null;
        int var17 = 0;
        int var18 = 0;
        int var19 = 0;
        int var20 = 0;
        int var21 = 0;
        int var22 = 0;
        int var23 = 0;
        jd var24 = null;
        int var25 = 0;
        int var26 = 0;
        int[] var27 = null;
        int[] var28 = null;
        int[] var29 = null;
        int[] var30 = null;
        int[] var31 = null;
        int[] var32 = null;
        int[] var33 = null;
        int[] var34 = null;
        int[] var35 = null;
        int[] var36 = null;
        int[] var37 = null;
        int[] var38 = null;
        RuntimeException stackIn_18_0 = null;
        StringBuilder stackIn_18_1 = null;
        RuntimeException stackIn_19_0 = null;
        StringBuilder stackIn_19_1 = null;
        String stackIn_19_2 = null;
        RuntimeException decompiledCaughtException = null;
        RuntimeException var3 = null;
        var26 = Pool.field_O;
        try {
          L0: {
            L1: {
              var3_int = 0;
              var34 = ro.field_v[0][var3_int];
              var29 = var34;
              var27 = var29;
              var4 = var27;
              var5 = ro.field_v[1][var3_int];
              if (param0 == -25) {
                break L1;
              } else {
                this.field_Y = (jg) null;
                break L1;
              }
            }
            var35 = ro.field_v[2][var3_int];
            var30 = var35;
            var28 = var30;
            var6 = var28;
            pl.a(var5, var34.length, 0, param1, 0, var27, t.field_a[1][var3_int], t.field_a[2][var3_int], t.field_a[0][var3_int].length, var35, -26182, t.field_a[0][var3_int]);
            var3_int = 1;
            L2: while (true) {
              if (16 < var3_int) {
                break L0;
              } else {
                var36 = ro.field_v[0][var3_int];
                var31 = var36;
                var7 = var31;
                var38 = ro.field_v[1][var3_int];
                var33 = var38;
                var8 = var33;
                var37 = ro.field_v[2][var3_int];
                var32 = var37;
                var9 = var32;
                pl.a(var8, var36.length, 0, param1, 0, var7, t.field_a[1][var3_int], t.field_a[2][var3_int], t.field_a[0][var3_int].length, var37, -26182, t.field_a[0][var3_int]);
                var10 = var4[0];
                var11 = var5[0];
                var12 = var6[0];
                var13 = var36[0];
                var14 = var38[0];
                var15 = var37[0];
                var16 = fm.field_K[-1 + var3_int];
                var17 = 1;
                L3: while (true) {
                  if (32 < var17) {
                    var5 = var8;
                    var4 = var7;
                    var6 = var9;
                    var3_int++;
                    continue L2;
                  } else {
                    L4: {
                      var18 = var4[var17];
                      var19 = var5[var17];
                      var20 = var6[var17];
                      var21 = var36[var17];
                      var22 = var38[var17];
                      var23 = var37[var17];
                      if (0 >= var12) {
                        break L4;
                      } else {
                        if (0 >= var20) {
                          break L4;
                        } else {
                          if ((var15 ^ -1) >= -1) {
                            break L4;
                          } else {
                            if (-1 > (var23 ^ -1)) {
                              var24 = var16[-1 + var17];
                              var25 = 48 - -(var24.field_g * 208 / 4096);
                              qh.b(var10, var11, var18, var19, 1140241, var25);
                              qh.b(var10, var11, var13, var14, 1140241, var25);
                              qh.b(var18, var19, var21, var22, 1140241, var25);
                              qh.b(var13, var14, var21, var22, 1140241, var25);
                              break L4;
                            } else {
                              break L4;
                            }
                          }
                        }
                      }
                    }
                    var10 = var18;
                    var13 = var21;
                    var11 = var19;
                    var12 = var20;
                    var15 = var23;
                    var14 = var22;
                    var17++;
                    continue L3;
                  }
                }
              }
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L5: {
            var3 = decompiledCaughtException;
            stackIn_18_0 = (RuntimeException) (var3);

            stackIn_18_1 = new StringBuilder().append("fk.L(").append(param0).append(',');

            if (param1 == null) {
              stackIn_19_0 = (RuntimeException) ((Object) stackIn_18_0);
              stackIn_19_1 = (StringBuilder) ((Object) stackIn_18_1);
              stackIn_19_2 = "null";
              break L5;
            } else {
              stackIn_19_0 = (RuntimeException) ((Object) stackIn_18_0);
              stackIn_19_1 = (StringBuilder) ((Object) stackIn_18_1);
              stackIn_19_2 = "{...}";
              break L5;
            }
          }
          throw wm.a((Throwable) ((Object) stackIn_19_0), stackIn_19_2 + ')');
        }
    }

    private final void b(byte param0, int[] param1) {
        int var3_int = 0;
        int var4 = 0;
        int var5 = 0;
        int var6 = 0;
        RuntimeException stackIn_15_0 = null;
        StringBuilder stackIn_15_1 = null;
        RuntimeException stackIn_16_0 = null;
        StringBuilder stackIn_16_1 = null;
        String stackIn_16_2 = null;
        RuntimeException decompiledCaughtException = null;
        RuntimeException var3 = null;
        var6 = Pool.field_O;
        try {
          L0: {
            L1: {
              if (param0 == 44) {
                break L1;
              } else {
                field_eb = (String) null;
                break L1;
              }
            }
            var3_int = 0;
            L2: while (true) {
              if (ma.field_b.length <= var3_int) {
                break L0;
              } else {
                var4 = 0;
                L3: while (true) {
                  if (ma.field_b[var3_int].length <= var4) {
                    var3_int++;
                    continue L2;
                  } else {
                    var5 = 0;
                    L4: while (true) {
                      if (ma.field_b[var3_int][var4].length <= var5) {
                        var4++;
                        continue L3;
                      } else {
                        ma.field_b[var3_int][var4][var5].a((byte) 67);
                        var5++;
                        continue L4;
                      }
                    }
                  }
                }
              }
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L5: {
            var3 = decompiledCaughtException;
            stackIn_15_0 = (RuntimeException) (var3);

            stackIn_15_1 = new StringBuilder().append("fk.AA(").append(param0).append(',');

            if (param1 == null) {
              stackIn_16_0 = (RuntimeException) ((Object) stackIn_15_0);
              stackIn_16_1 = (StringBuilder) ((Object) stackIn_15_1);
              stackIn_16_2 = "null";
              break L5;
            } else {
              stackIn_16_0 = (RuntimeException) ((Object) stackIn_15_0);
              stackIn_16_1 = (StringBuilder) ((Object) stackIn_15_1);
              stackIn_16_2 = "{...}";
              break L5;
            }
          }
          throw wm.a((Throwable) ((Object) stackIn_16_0), stackIn_16_2 + ')');
        }
    }

    fk() {
        this.field_L = new int[2];
        this.field_R = new int[2];
        this.field_X = new int[2];
        this.field_M = new int[2];
    }

    private final void o(int param0) {
        int var2 = 0;
        int var3 = 0;
        int var4 = 0;
        int var5 = 0;
        int var6 = 0;
        int var7 = 0;
        int var8 = Pool.field_O;
        if (param0 != 10) {
            return;
        }
        ma.field_b = new bl[4][16][16];
        for (var2 = 0; -17 < (var2 ^ -1); var2++) {
            var3 = -128 + (var2 * 2048 - -256) / 128;
            var4 = ((1 + var2) * 2048 - 256) / 128 - 128;
            for (var5 = 0; (var5 ^ -1) > -17; var5++) {
                var6 = (256 + 2048 * var5) / 128 + -128;
                var7 = -128 + (-256 + (var5 + 1) * 2048) / 128;
                ma.field_b[0][var2][var5] = new bl(128, -var6, var3, 128, -var7, var3, 128, -var6, var4, 128, -var7, var4);
                ma.field_b[1][var2][var5] = new bl(-var6, -128, var3, -var7, -128, var3, -var6, -128, var4, -var7, -128, var4);
                ma.field_b[2][var2][var5] = new bl(-128, var6, var3, -128, var7, var3, -128, var6, var4, -128, var7, var4);
                ma.field_b[3][var2][var5] = new bl(var6, 128, var3, var7, 128, var3, var6, 128, var4, var7, 128, var4);
            }
        }
    }

    final void a(boolean param0) {
        if (!(null == this.field_bb)) {
            kb.a((byte) -110, this.field_bb);
            this.field_bb = null;
        }
        if (param0) {
            this.field_T = (jg) null;
        }
    }

    final void a(byte param0, boolean param1, int param2) {
        int var4 = 81 / ((param0 - -47) / 45);
        if (9 == param2) {
            if (this.field_bb == null) {
            } else {
                if (param1) {
                    this.field_bb.c(64);
                } else {
                    this.field_bb.c(4096);
                }
                this.field_bb = null;
            }
        }
    }

    static {
        field_eb = "Members' Benefits";
        field_gb = null;
        field_V = 2;
    }
}
