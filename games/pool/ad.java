/*
 * Decompiled by CFR-JS 0.4.0.
 */
final class ad {
    private int[] field_f;
    int[] field_s;
    int field_d;
    int[] field_z;
    private int field_g;
    int[] field_i;
    byte[][] field_a;
    int[] field_w;
    int field_h;
    int field_v;
    private int[][] field_j;
    private byte[] field_q;
    wi field_p;
    int[] field_m;
    int[][] field_k;
    static ko field_l;
    wi[] field_n;
    static String field_e;
    static int field_x;
    static int[] field_o;
    static String field_b;
    static dd field_y;
    static int field_r;
    static int field_c;
    static String field_t;
    static int field_u;

    final static void a(byte param0, java.math.BigInteger param1, ge param2, java.math.BigInteger param3, ge param4) {
        try {
            lf.a(param3, param2.field_t, param1, param4, param2.field_v, 0, 0);
            if (param0 != 93) {
                ad.a(122, 64, 86);
            }
        } catch (RuntimeException runtimeException) {
            throw wm.a((Throwable) ((Object) runtimeException), "ad.B(" + param0 + ',' + (param1 != null ? "{...}" : "null") + ',' + (param2 != null ? "{...}" : "null") + ',' + (param3 != null ? "{...}" : "null") + ',' + (param4 != null ? "{...}" : "null") + ')');
        }
    }

    public static void a(int param0) {
        field_o = null;
        if (param0 != 20592) {
            return;
        }
        field_t = null;
        field_y = null;
        field_b = null;
        field_e = null;
        field_l = null;
    }

    private final void a(byte[] param0, int param1) {
        int dupTemp$0 = 0;
        int dupTemp$1 = 0;
        int[] array$2 = null;
        int dupTemp$3 = 0;
        int[] dupTemp$4 = null;
        int[] array$5 = null;
        int dupTemp$6 = 0;
        int[] dupTemp$7 = null;
        int[] array$8 = null;
        int stackIn_11_0 = 0;
        int stackIn_17_0 = 0;
        RuntimeException stackIn_94_0 = null;
        StringBuilder stackIn_94_1 = null;
        RuntimeException stackIn_95_0 = null;
        StringBuilder stackIn_95_1 = null;
        String stackIn_95_2 = null;
        RuntimeException decompiledCaughtException = null;
        RuntimeException var3 = null;
        int var4 = 0;
        int var5 = 0;
        int var6 = 0;
        int var7 = 0;
        int var8 = 0;
        int var9 = 0;
        int var10 = 0;
        int var11 = 0;
        int var12 = 0;
        int var13 = 0;
        int var14 = 0;
        int var15 = 0;
        int var16 = 0;
        ge var17 = null;
        byte[] var21 = null;
        var16 = Pool.field_O;
        try {
          L0: {
            L1: {
              var17 = new ge(wp.a(0, param0));
              var4 = var17.g(param1 ^ 105);
              if (-6 < (var4 ^ -1)) {
                break L1;
              } else {
                if (var4 <= 7) {
                  L2: {
                    if (var4 >= 6) {
                      this.field_h = var17.b(true);
                      break L2;
                    } else {
                      this.field_h = 0;
                      break L2;
                    }
                  }
                  L3: {
                    var5 = var17.g(-60);
                    if ((var5 & 1) == 0) {
                      stackIn_11_0 = 0;
                      break L3;
                    } else {
                      stackIn_11_0 = 1;
                      break L3;
                    }
                  }
                  L4: {
                    var6 = stackIn_11_0;
                    if (var4 >= 7) {
                      this.field_g = var17.c(99);
                      break L4;
                    } else {
                      this.field_g = var17.d(-1034);
                      break L4;
                    }
                  }
                  L5: {
                    if (0 == (var5 & 2)) {
                      stackIn_17_0 = 0;
                      break L5;
                    } else {
                      stackIn_17_0 = 1;
                      break L5;
                    }
                  }
                  L6: {
                    var7 = stackIn_17_0;
                    var8 = 0;
                    this.field_m = new int[this.field_g];
                    var9 = param1;
                    if (7 <= var4) {
                      var10 = 0;
                      L7: while (true) {
                        if (var10 >= this.field_g) {
                          break L6;
                        } else {
                          L8: {
                            dupTemp$0 = var8 + var17.c(cq.a(param1, 82));
                            var8 = dupTemp$0;
                            this.field_m[var10] = dupTemp$0;
                            if ((this.field_m[var10] ^ -1) < (var9 ^ -1)) {
                              var9 = this.field_m[var10];
                              break L8;
                            } else {
                              break L8;
                            }
                          }
                          var10++;
                          continue L7;
                        }
                      }
                    } else {
                      var10 = 0;
                      L9: while (true) {
                        if (this.field_g <= var10) {
                          break L6;
                        } else {
                          dupTemp$1 = var8 + var17.d(cq.a(param1, 1033));
                          var8 = dupTemp$1;
                          this.field_m[var10] = dupTemp$1;
                          if ((this.field_m[var10] ^ -1) < (var9 ^ -1)) {
                            var9 = this.field_m[var10];
                            var10++;
                            continue L9;
                          } else {
                            var10++;
                            continue L9;
                          }
                        }
                      }
                    }
                  }
                  L10: {
                    this.field_d = 1 + var9;
                    if (var7 == 0) {
                      break L10;
                    } else {
                      this.field_a = new byte[this.field_d][];
                      break L10;
                    }
                  }
                  L11: {
                    this.field_k = new int[this.field_d][];
                    this.field_s = new int[this.field_d];
                    this.field_w = new int[this.field_d];
                    this.field_i = new int[this.field_d];
                    this.field_z = new int[this.field_d];
                    if (var6 == 0) {
                      break L11;
                    } else {
                      this.field_f = new int[this.field_d];
                      var10 = 0;
                      L12: while (true) {
                        if (this.field_d <= var10) {
                          var10 = 0;
                          L13: while (true) {
                            if (this.field_g <= var10) {
                              this.field_p = new wi(this.field_f);
                              break L11;
                            } else {
                              this.field_f[this.field_m[var10]] = var17.b(true);
                              var10++;
                              continue L13;
                            }
                          }
                        } else {
                          this.field_f[var10] = -1;
                          var10++;
                          continue L12;
                        }
                      }
                    }
                  }
                  var10 = 0;
                  L14: while (true) {
                    if (this.field_g <= var10) {
                      L15: {
                        if (var7 == 0) {
                          break L15;
                        } else {
                          var10 = 0;
                          L16: while (true) {
                            if (var10 >= this.field_g) {
                              break L15;
                            } else {
                              var21 = new byte[64];
                              var17.a(0, (byte) -118, var21, 64);
                              this.field_a[this.field_m[var10]] = var21;
                              var10++;
                              continue L16;
                            }
                          }
                        }
                      }
                      var10 = 0;
                      L17: while (true) {
                        if (this.field_g <= var10) {
                          L18: {
                            if (var4 >= 7) {
                              var10 = 0;
                              L19: while (true) {
                                if (this.field_g <= var10) {
                                  var10 = 0;
                                  L20: while (true) {
                                    if (var10 >= this.field_g) {
                                      break L18;
                                    } else {
                                      var11 = this.field_m[var10];
                                      var8 = 0;
                                      var12 = this.field_s[var11];
                                      array$2 = new int[var12];
                                      this.field_k[var11] = array$2;
                                      var13 = -1;
                                      var14 = 0;
                                      L21: while (true) {
                                        if (var14 >= var12) {
                                          L22: {
                                            this.field_z[var11] = var13 - -1;
                                            if (1 + var13 == var12) {
                                              this.field_k[var11] = null;
                                              break L22;
                                            } else {
                                              break L22;
                                            }
                                          }
                                          var10++;
                                          continue L20;
                                        } else {
                                          L23: {
                                            dupTemp$3 = var8 + var17.c(81);
                                            var8 = dupTemp$3;
                                            dupTemp$4 = this.field_k[var11];
                                            dupTemp$4[var14] = dupTemp$3;
                                            var15 = dupTemp$3;
                                            if ((var13 ^ -1) > (var15 ^ -1)) {
                                              var13 = var15;
                                              break L23;
                                            } else {
                                              break L23;
                                            }
                                          }
                                          var14++;
                                          continue L21;
                                        }
                                      }
                                    }
                                  }
                                } else {
                                  this.field_s[this.field_m[var10]] = var17.c(cq.a(param1, -103));
                                  var10++;
                                  continue L19;
                                }
                              }
                            } else {
                              var10 = 0;
                              L24: while (true) {
                                if (var10 >= this.field_g) {
                                  var10 = 0;
                                  L25: while (true) {
                                    if (var10 >= this.field_g) {
                                      break L18;
                                    } else {
                                      var11 = this.field_m[var10];
                                      var8 = 0;
                                      var12 = this.field_s[var11];
                                      var13 = -1;
                                      array$5 = new int[var12];
                                      this.field_k[var11] = array$5;
                                      var14 = 0;
                                      L26: while (true) {
                                        if (var14 >= var12) {
                                          L27: {
                                            this.field_z[var11] = var13 + 1;
                                            if (var12 != 1 + var13) {
                                              break L27;
                                            } else {
                                              this.field_k[var11] = null;
                                              break L27;
                                            }
                                          }
                                          var10++;
                                          continue L25;
                                        } else {
                                          dupTemp$6 = var8 + var17.d(-1034);
                                          var8 = dupTemp$6;
                                          dupTemp$7 = this.field_k[var11];
                                          dupTemp$7[var14] = dupTemp$6;
                                          var15 = dupTemp$6;
                                          if (var13 < var15) {
                                            var13 = var15;
                                            var14++;
                                            continue L26;
                                          } else {
                                            var14++;
                                            continue L26;
                                          }
                                        }
                                      }
                                    }
                                  }
                                } else {
                                  this.field_s[this.field_m[var10]] = var17.d(param1 + -1033);
                                  var10++;
                                  continue L24;
                                }
                              }
                            }
                          }
                          L28: {
                            if (var6 == 0) {
                              break L28;
                            } else {
                              this.field_n = new wi[1 + var9];
                              this.field_j = new int[var9 - -1][];
                              var10 = 0;
                              L29: while (true) {
                                if (var10 >= this.field_g) {
                                  break L28;
                                } else {
                                  var11 = this.field_m[var10];
                                  var12 = this.field_s[var11];
                                  array$8 = new int[this.field_z[var11]];
                                  this.field_j[var11] = array$8;
                                  var13 = 0;
                                  L30: while (true) {
                                    if (this.field_z[var11] <= var13) {
                                      var13 = 0;
                                      L31: while (true) {
                                        if (var13 >= var12) {
                                          this.field_n[var11] = new wi(this.field_j[var11]);
                                          var10++;
                                          continue L29;
                                        } else {
                                          L32: {
                                            if (this.field_k[var11] != null) {
                                              var14 = this.field_k[var11][var13];
                                              break L32;
                                            } else {
                                              var14 = var13;
                                              break L32;
                                            }
                                          }
                                          this.field_j[var11][var14] = var17.b(true);
                                          var13++;
                                          continue L31;
                                        }
                                      }
                                    } else {
                                      this.field_j[var11][var13] = -1;
                                      var13++;
                                      continue L30;
                                    }
                                  }
                                }
                              }
                            }
                          }
                          break L0;
                        } else {
                          this.field_i[this.field_m[var10]] = var17.b(true);
                          var10++;
                          continue L17;
                        }
                      }
                    } else {
                      this.field_w[this.field_m[var10]] = var17.b(true);
                      var10++;
                      continue L14;
                    }
                  }
                } else {
                  break L1;
                }
              }
            }
            throw new RuntimeException();
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L33: {
            var3 = decompiledCaughtException;
            stackIn_94_0 = (RuntimeException) (var3);

            stackIn_94_1 = new StringBuilder().append("ad.D(");

            if (param0 == null) {
              stackIn_95_0 = (RuntimeException) ((Object) stackIn_94_0);
              stackIn_95_1 = (StringBuilder) ((Object) stackIn_94_1);
              stackIn_95_2 = "null";
              break L33;
            } else {
              stackIn_95_0 = (RuntimeException) ((Object) stackIn_94_0);
              stackIn_95_1 = (StringBuilder) ((Object) stackIn_94_1);
              stackIn_95_2 = "{...}";
              break L33;
            }
          }
          throw wm.a((Throwable) ((Object) stackIn_95_0), stackIn_95_2 + ',' + param1 + ')');
        }
    }

    ad(byte[] param0, int param1, byte[] param2) {
        int var4_int = 0;
        try {
            this.field_v = qo.a(param0.length, param0, 0);
            if (this.field_v != param1) {
                throw new RuntimeException();
            }
            if (param2 != null) {
                if (param2.length != 64) {
                    throw new RuntimeException();
                }
                this.field_q = jr.a(param0.length, (byte) -51, param0, 0);
                for (var4_int = 0; var4_int < 64; var4_int++) {
                    if (param2[var4_int] != this.field_q[var4_int]) {
                        throw new RuntimeException();
                    }
                }
            }
            this.a(param0, -1);
        } catch (RuntimeException runtimeException) {
            throw wm.a((Throwable) ((Object) runtimeException), "ad.<init>(" + (param0 != null ? "{...}" : "null") + ',' + param1 + ',' + (param2 != null ? "{...}" : "null") + ')');
        }
    }

    final static dd a(int param0, int param1, int param2) {
        int var4 = 0;
        if (param2 != 7) {
            ad.a(82);
        }
        dd var5 = new dd(param1, param1);
        dd var3 = var5;
        for (var4 = 0; var4 < var3.field_D.length; var4++) {
            var5.field_D[var4] = param0;
        }
        return var3;
    }

    static {
        field_l = new ko();
        field_e = "Cancel";
        field_o = new int[]{2, 20, 21, 49, 5, 3, 9};
        field_b = "<%0> gained 1 life!";
        field_t = "Create a free Account";
    }
}
