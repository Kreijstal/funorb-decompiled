/*
 * Decompiled by CFR-JS 0.4.0.
 */
abstract class tm {
    static vh field_d;
    eg[] field_a;
    static int field_f;
    boolean field_g;
    nc[] field_j;
    dd[] field_e;
    private int[][] field_k;
    static ko field_i;
    static ea field_b;
    static cd field_h;
    static int[] field_c;

    void a(int[] param0, int[] param1, int param2, int param3) {
        try {
            this.field_j[param2].b(param1, param0);
            if (param3 != -3637) {
                this.a(-49, false);
            }
        } catch (RuntimeException runtimeException) {
            throw wm.a((Throwable) ((Object) runtimeException), "tm.DA(" + (param0 != null ? "{...}" : "null") + ',' + (param1 != null ? "{...}" : "null") + ',' + param2 + ',' + param3 + ')');
        }
    }

    String a(int param0, byte param1) {
        if (param1 <= 108) {
            return (String) null;
        }
        return this.a((byte) 102, param0);
    }

    abstract int a(int param0, boolean param1);

    void a(int param0, int param1, int[] param2, int[] param3, boolean param4) {
        if (param0 != -1912602369) {
            return;
        }
        try {
            this.a(param4, param2, param1, param3, -20263);
            this.field_j[param1].b(param3, param2);
        } catch (RuntimeException runtimeException) {
            throw wm.a((Throwable) ((Object) runtimeException), "tm.K(" + param0 + ',' + param1 + ',' + (param2 != null ? "{...}" : "null") + ',' + (param3 != null ? "{...}" : "null") + ',' + param4 + ')');
        }
    }

    private final void f(int param0) {
        int var2 = 0;
        int var3 = Pool.field_O;
        this.field_k = new int[16][];
        if (param0 > -124) {
            this.field_j = (nc[]) null;
        }
        for (var2 = 0; var2 < this.field_k.length; var2++) {
            this.field_k[var2] = ta.a(new int[]{0, 0, 128, this.a(var2, true)}, -119);
        }
    }

    final static hj a(int param0, int param1) {
        hj[] var2;
        int var3;
        int var4;
        int var5;
        hj[] var6;
        var5 = Pool.field_O;
        var6 = lm.a((byte) -32);
        var2 = var6;
        var3 = 0;
        var4 = -81 / ((param1 - -69) / 37);
        L0: while (true) {
          if (var3 < var6.length) {
            if (var6[var3].field_i != param0) {
              var3++;
              continue L0;
            } else {
              return var6[var3];
            }
          } else {
            return null;
          }
        }
    }

    final void a(boolean param0, int[] param1, int param2, int[] param3, int param4) {
        int[] stackIn_10_0 = null;
        RuntimeException stackIn_18_0 = null;
        StringBuilder stackIn_18_1 = null;
        RuntimeException stackIn_19_0 = null;
        StringBuilder stackIn_19_1 = null;
        String stackIn_19_2 = null;
        StringBuilder stackIn_21_1 = null;
        StringBuilder stackIn_22_1 = null;
        String stackIn_22_2 = null;
        int decompiledRegionSelector0 = 0;
        RuntimeException decompiledCaughtException = null;
        int var6_int = 0;
        RuntimeException var6 = null;
        int var7 = 0;
        int var8 = 0;
        int var9 = 0;
        int var10 = 0;
        int var11 = 0;
        int[] var12 = null;
        int var13 = 0;
        int[] var14 = null;
        int[] var15 = null;
        try {
          L0: {
            if (-1 > (no.field_n[param2] ^ -1)) {
              L1: {
                var8 = -param3[2] + param1[2];
                var6_int = param1[0] - param3[0];
                var7 = param1[1] + -param3[1];
                var10 = param3[8] * var8 + (param3[7] * var7 + var6_int * param3[6]) >> -273628317;
                var9 = param3[4] * var7 + (param3[3] * var6_int + var8 * param3[5]) >> -1680844509;
                if (param4 == -20263) {
                  break L1;
                } else {
                  this.field_a = (eg[]) null;
                  break L1;
                }
              }
              var8 = var8 * param3[11] + param3[9] * var6_int - -(var7 * param3[10]) >> 2020877808;
              if (-1 <= (var8 ^ -1)) {
                decompiledRegionSelector0 = 1;
                break L0;
              } else {
                L2: {
                  var7 = (qh.field_c + fb.field_d << 563442148) - -(var10 / var8);
                  var6_int = (qh.field_b - -fb.field_q << -930305852) - -(var9 / var8);
                  var11 = 32 + 589824 / var8;
                  if (!param0) {
                    stackIn_10_0 = ff.field_h;
                    break L2;
                  } else {
                    stackIn_10_0 = ff.field_i;
                    break L2;
                  }
                }
                var15 = stackIn_10_0;
                var14 = var15;
                var12 = var14;
                var13 = 0;
                L3: while (true) {
                  if (var13 >= var15.length) {
                    L4: {
                      if (2147483647L <= 64L * ((long)var11 * (long)var11)) {
                        break L4;
                      } else {
                        qh.a(var6_int, var7, var11, 64, ff.field_a);
                        break L4;
                      }
                    }
                    decompiledRegionSelector0 = 2;
                    break L0;
                  } else {
                    ff.field_a[var13] = md.a(md.a(rb.b(no.field_n[param2] * rb.b(var15[var13], 65280) >>> 198123656, 973143808), rb.b(-16776974, rb.b(16711680, var15[var13]) * no.field_n[param2]) >>> 377159592), rb.b(-1912602369, no.field_n[param2] * rb.b(255, var15[var13]) >>> 1400849512));
                    var13++;
                    continue L3;
                  }
                }
              }
            } else {
              decompiledRegionSelector0 = 0;
              break L0;
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L5: {
            var6 = decompiledCaughtException;
            stackIn_18_0 = (RuntimeException) (var6);

            stackIn_18_1 = new StringBuilder().append("tm.BA(").append(param0).append(',');

            if (param1 == null) {
              stackIn_19_0 = (RuntimeException) ((Object) stackIn_18_0);
              stackIn_19_1 = (StringBuilder) ((Object) stackIn_18_1);
              stackIn_19_2 = "null";
              break L5;
            } else {
              stackIn_19_0 = (RuntimeException) ((Object) stackIn_18_0);
              stackIn_19_1 = (StringBuilder) ((Object) stackIn_18_1);
              stackIn_19_2 = "{...}";
              break L5;
            }
          }
          L6: {


            stackIn_21_1 = ((StringBuilder) (Object) stackIn_19_1).append(stackIn_19_2).append(',').append(param2).append(',');

            if (param3 == null) {
              stackIn_19_0 = (RuntimeException) ((Object) stackIn_19_0);
              stackIn_22_1 = (StringBuilder) ((Object) stackIn_21_1);
              stackIn_22_2 = "null";
              break L6;
            } else {
              stackIn_19_0 = (RuntimeException) ((Object) stackIn_19_0);
              stackIn_22_1 = (StringBuilder) ((Object) stackIn_21_1);
              stackIn_22_2 = "{...}";
              break L6;
            }
          }
          throw wm.a((Throwable) ((Object) stackIn_19_0), stackIn_22_2 + ',' + param4 + ')');
        }
        if (decompiledRegionSelector0 == 0) {
          return;
        } else {
          if (decompiledRegionSelector0 == 1) {
            return;
          } else {
            return;
          }
        }
    }

    final eg g(int param0) {
        int var3;
        int var4;
        int var5;
        int var6;
        double var7;
        double var9;
        double var11;
        double var13;
        int var15;
        double var16;
        double var18;
        double var20;
        double var22;
        int var24;
        int var25;
        int var26;
        int var27;
        int var28;
        eg var29;
        L0: {
          var28 = Pool.field_O;
          var29 = new eg((3 + ne.field_v) * (hr.field_b - -1), 2 * (hr.field_b * ne.field_v), ne.field_v);
          if (param0 == -930305852) {
            break L0;
          } else {
            field_h = (cd) null;
            break L0;
          }
        }
        var3 = var29.a(-383, -383, 384);
        var4 = var29.a(384, -383, 384);
        var5 = var29.a(-383, 384, 384);
        var29.a((short)var3, (short)var4, (short)var5);
        var6 = 0;
        L1: while (true) {
          if (var6 >= hr.field_b) {
            var29.e();
            return var29;
          } else {
            var7 = Math.cos((double)var6 * 3.141592653589793 / (double)hr.field_b) * 8.0;
            var9 = 8.0 * Math.sin(3.141592653589793 * (double)var6 / (double)hr.field_b);
            var11 = 8.0 * Math.cos((double)(1 + var6) * 3.141592653589793 / (double)hr.field_b);
            var13 = Math.sin((double)(var6 + 1) * 3.141592653589793 / (double)hr.field_b) * 8.0;
            var15 = 0;
            L2: while (true) {
              if (var15 >= ne.field_v) {
                var6++;
                continue L1;
              } else {
                L3: {
                  var16 = Math.cos(3.141592653589793 * (double)(2 * var15) / (double)ne.field_v);
                  var18 = Math.sin(3.141592653589793 * (double)(var15 * 2) / (double)ne.field_v);
                  var20 = Math.cos(3.141592653589793 * (double)(2 + var15 * 2) / (double)ne.field_v);
                  var22 = Math.sin((double)(2 * (var15 - -1)) * 3.141592653589793 / (double)ne.field_v);
                  var24 = var29.a(cr.a(4885, var16 * var9), cr.a(4885, var9 * var18), cr.a(4885, var7));
                  var25 = var29.a(cr.a(4885, var20 * var9), cr.a(param0 + 930310737, var9 * var22), cr.a(param0 ^ -930300975, var7));
                  var26 = var29.a(cr.a(4885, var13 * var16), cr.a(4885, var13 * var18), cr.a(4885, var11));
                  var27 = var29.a(cr.a(4885, var20 * var13), cr.a(4885, var13 * var22), cr.a(4885, var11));
                  if (var24 != var25) {
                    var29.a(var24, var26, var25, (byte) 0, (short) 96, this.a(true));
                    break L3;
                  } else {
                    break L3;
                  }
                }
                if (var26 != var27) {
                  var29.a(var27, var25, var26, (byte) 0, (short) 96, this.a(true));
                  var15++;
                  continue L2;
                } else {
                  var15++;
                  continue L2;
                }
              }
            }
          }
        }
    }

    final void a(int param0) {
        if (this.field_a != null) {
            this.field_j = new nc[this.field_a.length];
        }
        if (param0 != 7096) {
            this.field_k = (int[][]) null;
        }
        this.f(-128);
    }

    public static void d(int param0) {
        field_h = null;
        if (param0 != -22799) {
            field_i = (ko) null;
        }
        field_i = null;
        field_b = null;
        field_c = null;
        field_d = null;
    }

    int b(int param0) {
        if (param0 != 384) {
            this.field_g = true;
        }
        return this.field_j.length;
    }

    void a(int param0, di param1) {
        try {
            if (param0 != 198123656) {
                this.h(32);
            }
        } catch (RuntimeException runtimeException) {
            throw wm.a((Throwable) ((Object) runtimeException), "tm.D(" + param0 + ',' + (param1 != null ? "{...}" : "null") + ')');
        }
    }

    int[] b(int param0, int param1) {
        if (param1 != 0) {
            return (int[]) null;
        }
        return this.field_k[param0];
    }

    void a(di param0, byte param1) {
        try {
            if (param1 != 117) {
                this.b(-29, 80);
            }
        } catch (RuntimeException runtimeException) {
            throw wm.a((Throwable) ((Object) runtimeException), "tm.C(" + (param0 != null ? "{...}" : "null") + ',' + param1 + ')');
        }
    }

    final String a(int param0, sq param1, byte param2) {
        RuntimeException var4 = null;
        String stackIn_4_0 = null;
        String stackIn_7_0 = null;
        String stackIn_12_0 = null;
        String stackIn_14_0 = null;
        RuntimeException stackIn_17_0 = null;
        StringBuilder stackIn_17_1 = null;
        RuntimeException stackIn_18_0 = null;
        StringBuilder stackIn_18_1 = null;
        String stackIn_18_2 = null;
        int decompiledRegionSelector0 = 0;
        RuntimeException decompiledCaughtException = null;
        try {
          L0: {
            L1: {
              if (param2 == -111) {
                break L1;
              } else {
                this.field_k = (int[][]) null;
                break L1;
              }
            }
            if (!(param1 instanceof fj)) {
              if (!this.c(2)) {
                L2: {
                  if (0 == param0) {
                    break L2;
                  } else {
                    if (param0 != 8) {
                      stackIn_14_0 = this.a((byte) 33, param0 >> 604778307 & 1);
                      decompiledRegionSelector0 = 3;
                      break L0;
                    } else {
                      break L2;
                    }
                  }
                }
                stackIn_12_0 = ho.field_u[param0];
                decompiledRegionSelector0 = 2;
                break L0;
              } else {
                stackIn_7_0 = ho.field_u[param0];
                decompiledRegionSelector0 = 1;
                break L0;
              }
            } else {
              stackIn_4_0 = or.field_k[param0];
              decompiledRegionSelector0 = 0;
              break L0;
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L3: {
            var4 = decompiledCaughtException;
            stackIn_17_0 = (RuntimeException) (var4);

            stackIn_17_1 = new StringBuilder().append("tm.R(").append(param0).append(',');

            if (param1 == null) {
              stackIn_18_0 = (RuntimeException) ((Object) stackIn_17_0);
              stackIn_18_1 = (StringBuilder) ((Object) stackIn_17_1);
              stackIn_18_2 = "null";
              break L3;
            } else {
              stackIn_18_0 = (RuntimeException) ((Object) stackIn_17_0);
              stackIn_18_1 = (StringBuilder) ((Object) stackIn_17_1);
              stackIn_18_2 = "{...}";
              break L3;
            }
          }
          throw wm.a((Throwable) ((Object) stackIn_18_0), stackIn_18_2 + ',' + param2 + ')');
        }
        if (decompiledRegionSelector0 == 0) {
          return stackIn_4_0;
        } else {
          if (decompiledRegionSelector0 == 1) {
            return stackIn_7_0;
          } else {
            if (decompiledRegionSelector0 == 2) {
              return stackIn_12_0;
            } else {
              return stackIn_14_0;
            }
          }
        }
    }

    void h(int param0) {
        int var2 = -55 % ((29 - param0) / 44);
    }

    abstract String a(byte param0, int param1);

    void a(boolean param0, int[] param1, int param2) {
        RuntimeException stackIn_8_0 = null;
        StringBuilder stackIn_8_1 = null;
        RuntimeException stackIn_9_0 = null;
        StringBuilder stackIn_9_1 = null;
        String stackIn_9_2 = null;
        RuntimeException decompiledCaughtException = null;
        RuntimeException var4 = null;
        try {
          L0: {
            L1: {
              if (!param0) {
                break L1;
              } else {
                this.field_a = (eg[]) null;
                break L1;
              }
            }
            L2: {
              if (!this.field_g) {
                this.field_j[param2] = (nc) ((Object) kh.field_gc.b(param1, this.field_a[param2], -25431));
                break L2;
              } else {
                this.field_j[param2] = (nc) ((Object) kh.field_gc.a(param1, (byte) 103, this.field_a[param2]));
                break L2;
              }
            }
            break L0;
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L3: {
            var4 = decompiledCaughtException;
            stackIn_8_0 = (RuntimeException) (var4);

            stackIn_8_1 = new StringBuilder().append("tm.O(").append(param0).append(',');

            if (param1 == null) {
              stackIn_9_0 = (RuntimeException) ((Object) stackIn_8_0);
              stackIn_9_1 = (StringBuilder) ((Object) stackIn_8_1);
              stackIn_9_2 = "null";
              break L3;
            } else {
              stackIn_9_0 = (RuntimeException) ((Object) stackIn_8_0);
              stackIn_9_1 = (StringBuilder) ((Object) stackIn_8_1);
              stackIn_9_2 = "{...}";
              break L3;
            }
          }
          throw wm.a((Throwable) ((Object) stackIn_9_0), stackIn_9_2 + ',' + param2 + ')');
        }
    }

    final void e(int param0) {
        if (param0 != -26707) {
            int[] var3 = (int[]) null;
            this.a(true, (int[]) null, 13, (int[]) null, -85);
        }
    }

    abstract boolean c(int param0);

    private final byte a(boolean param0) {
        if (!param0) {
            return (byte) -68;
        }
        return (byte) 0;
    }

    void a(int param0, al param1) {
        try {
            if (param0 != 2147483647) {
                this.field_j = (nc[]) null;
            }
        } catch (RuntimeException runtimeException) {
            throw wm.a((Throwable) ((Object) runtimeException), "tm.AA(" + param0 + ',' + (param1 != null ? "{...}" : "null") + ')');
        }
    }

    tm() {
        this.field_g = true;
    }

    static {
        field_i = new ko();
        field_b = null;
        field_h = new cd();
    }
}
