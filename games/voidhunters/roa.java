/*
 * Decompiled by CFR-JS 0.4.0.
 */
final class roa extends ka {
    private int field_Q;
    private int[][] field_a;
    private short[] field_i;
    private glb field_C;
    private int[] field_l;
    private float[] field_D;
    private int field_v;
    private int field_B;
    private int field_L;
    private int[] field_j;
    private int[] field_g;
    private boolean field_m;
    private glb field_h;
    private int field_z;
    private short[] field_r;
    private int[] field_K;
    private short[] field_e;
    private short[] field_d;
    private int field_I;
    private int field_M;
    private int field_u;
    private float[] field_y;
    private int[] field_t;
    private short[] field_c;
    private short[] field_x;
    private glb field_o;
    private int[] field_p;
    private int field_P;
    private byte[] field_b;
    private int field_G;
    private short[] field_O;
    private short[] field_A;
    private short[] field_f;
    private short[] field_J;
    private gib[] field_E;
    private short[] field_N;
    private rv field_H;
    private int[] field_w;
    private glb field_q;
    private short[] field_F;
    private byte[] field_n;
    private lga[] field_k;
    private int field_s;

    final int HA() {
        if (!this.field_m) {
            this.b(-27337);
        }
        return this.field_z;
    }

    final void H(int param0, int param1, int param2) {
        int var4 = 0;
        int var5 = 0;
        var5 = VoidHunters.field_G;
        var4 = 0;
        L0: while (true) {
          if (var4 >= this.field_P) {
            this.a((byte) -53);
            this.field_m = false;
            return;
          } else {
            L1: {
              if (param0 == 0) {
                break L1;
              } else {
                this.field_K[var4] = this.field_K[var4] + param0;
                break L1;
              }
            }
            L2: {
              if (-1 != (param1 ^ -1)) {
                this.field_j[var4] = this.field_j[var4] + param1;
                break L2;
              } else {
                break L2;
              }
            }
            if (0 != param2) {
              this.field_p[var4] = this.field_p[var4] + param2;
              var4++;
              continue L0;
            } else {
              var4++;
              continue L0;
            }
          }
        }
    }

    private final void a(byte param0) {
        short discarded$0 = 0;
        if (this.field_o != null) {
        }
        if (param0 > -42) {
            discarded$0 = this.a(73, 73, 17, 37, 0.2685146629810333f, (cbb) null, -53, 101, -66L, 29, 0.045781638473272324f);
        }
    }

    roa(pf param0, cbb param1, int param2, int param3, int param4, int param5) {
        this(param0, param2, param5, true, false);
        int[][] discarded$5 = null;
        int[][] discarded$6 = null;
        int[] array$7 = null;
        vib dupTemp$8 = null;
        int fieldTemp$9 = 0;
        RuntimeException var7 = null;
        int[] var8 = null;
        int var9_int = 0;
        long[] var9 = null;
        int var10 = 0;
        opa var10_ref_opa = null;
        int var11_int = 0;
        hu[] var11 = null;
        int var12 = 0;
        Object var13_ref = null;
        int var13 = 0;
        int var14 = 0;
        int var15_int = 0;
        vib[] var15 = null;
        int var16 = 0;
        int var17_int = 0;
        int[] var17 = null;
        int var18 = 0;
        int var19 = 0;
        bqa var20_ref_bqa = null;
        int var20 = 0;
        sr var21_ref_sr = null;
        int var21 = 0;
        int var22 = 0;
        float var22_float = 0.0f;
        int var23_int = 0;
        float var23 = 0.0f;
        int var24_int = 0;
        float var24 = 0.0f;
        int var25_int = 0;
        float var25 = 0.0f;
        int var26_int = 0;
        float var26 = 0.0f;
        int var27_int = 0;
        float var27 = 0.0f;
        int var28 = 0;
        int var29 = 0;
        int var30 = 0;
        int var31 = 0;
        vib var31_ref_vib = null;
        int var32_int = 0;
        long var32 = 0L;
        vib var32_ref_vib = null;
        int var33_int = 0;
        long var33 = 0L;
        int var34 = 0;
        int var35 = 0;
        int var36 = 0;
        int var37 = 0;
        float var38 = 0.0f;
        float var39 = 0.0f;
        int var39_int = 0;
        float var40 = 0.0f;
        float var41 = 0.0f;
        float var42 = 0.0f;
        float var43 = 0.0f;
        int var43_int = 0;
        float var44 = 0.0f;
        int var44_int = 0;
        float var45 = 0.0f;
        int var45_int = 0;
        float var46 = 0.0f;
        int var46_int = 0;
        float var47 = 0.0f;
        int var47_int = 0;
        float var48 = 0.0f;
        int var48_int = 0;
        float var49 = 0.0f;
        int var49_int = 0;
        float var50 = 0.0f;
        int var50_int = 0;
        float var51 = 0.0f;
        int var51_int = 0;
        float var52 = 0.0f;
        float var53 = 0.0f;
        float var54 = 0.0f;
        float var55 = 0.0f;
        float var56 = 0.0f;
        float var57 = 0.0f;
        float var58 = 0.0f;
        float var59 = 0.0f;
        float var60 = 0.0f;
        float var61 = 0.0f;
        float var62 = 0.0f;
        d var64 = null;
        int[] var65 = null;
        opa var66 = null;
        opa var67 = null;
        long[] var68 = null;
        bqa var69 = null;
        sr var70 = null;
        hu var71 = null;
        hu var72 = null;
        hu var73 = null;
        opa var74 = null;
        kua var75 = null;
        hu var77 = null;
        hu var78 = null;
        hu var79 = null;
        int[] var80 = null;
        int[] var81 = null;
        long[] var82 = null;
        int[] var84 = null;
        float[] var86 = null;
        int stackIn_15_0 = 0;
        int stackIn_43_0 = 0;
        int stackIn_62_0 = 0;
        int stackIn_63_0 = 0;
        int stackIn_64_0 = 0;
        int stackIn_64_1 = 0;
        int stackIn_85_0 = 0;
        int stackIn_101_0 = 0;
        int stackIn_154_0 = 0;
        int stackIn_155_0 = 0;
        int stackIn_156_0 = 0;
        int stackIn_156_1 = 0;
        RuntimeException stackIn_212_0 = null;
        StringBuilder stackIn_212_1 = null;
        RuntimeException stackIn_213_0 = null;
        StringBuilder stackIn_213_1 = null;
        RuntimeException stackIn_214_0 = null;
        StringBuilder stackIn_214_1 = null;
        String stackIn_214_2 = null;
        RuntimeException stackIn_215_0 = null;
        StringBuilder stackIn_215_1 = null;
        RuntimeException stackIn_216_0 = null;
        StringBuilder stackIn_216_1 = null;
        RuntimeException stackIn_217_0 = null;
        StringBuilder stackIn_217_1 = null;
        String stackIn_217_2 = null;
        Throwable caughtException = null;
        RuntimeException decompiledCaughtException = null;
        int stackOut_14_0 = 0;
        int stackOut_13_0 = 0;
        int stackOut_61_0 = 0;
        int stackOut_63_0 = 0;
        int stackOut_63_1 = 0;
        int stackOut_62_0 = 0;
        byte stackOut_62_1 = 0;
        int stackOut_100_0 = 0;
        short stackOut_99_0 = 0;
        int stackOut_153_0 = 0;
        int stackOut_155_0 = 0;
        int stackOut_155_1 = 0;
        int stackOut_154_0 = 0;
        int stackOut_154_1 = 0;
        int stackOut_84_0 = 0;
        byte stackOut_83_0 = 0;
        int stackOut_36_0 = 0;
        int stackOut_41_0 = 0;
        int stackOut_40_0 = 0;
        int stackOut_38_0 = 0;
        RuntimeException stackOut_211_0 = null;
        StringBuilder stackOut_211_1 = null;
        RuntimeException stackOut_213_0 = null;
        StringBuilder stackOut_213_1 = null;
        String stackOut_213_2 = null;
        RuntimeException stackOut_212_0 = null;
        StringBuilder stackOut_212_1 = null;
        String stackOut_212_2 = null;
        RuntimeException stackOut_214_0 = null;
        StringBuilder stackOut_214_1 = null;
        RuntimeException stackOut_216_0 = null;
        StringBuilder stackOut_216_1 = null;
        String stackOut_216_2 = null;
        RuntimeException stackOut_215_0 = null;
        StringBuilder stackOut_215_1 = null;
        String stackOut_215_2 = null;
        try {
          L0: {
            var64 = param0.field_a;
            var81 = new int[param1.field_j];
            var65 = var81;
            var8 = var65;
            this.field_l = new int[param1.field_D - -1];
            var9_int = 0;
            L1: while (true) {
              if (param1.field_j <= var9_int) {
                L2: {
                  this.field_Q = this.field_v;
                  var82 = new long[this.field_v];
                  var68 = var82;
                  var9 = var68;
                  if (-1 == (this.field_I & 256 ^ -1)) {
                    stackOut_14_0 = 0;
                    stackIn_15_0 = stackOut_14_0;
                    break L2;
                  } else {
                    stackOut_13_0 = 1;
                    stackIn_15_0 = stackOut_13_0;
                    break L2;
                  }
                }
                var10 = stackIn_15_0;
                var11_int = 0;
                L3: while (true) {
                  if (this.field_v <= var11_int) {
                    L4: {
                      kn.a((byte) -24, var81, var82);
                      this.field_P = param1.field_D;
                      this.field_j = param1.field_G;
                      this.field_p = param1.field_x;
                      this.field_K = param1.field_v;
                      var11 = new hu[this.field_P];
                      if (null == param1.field_g) {
                        break L4;
                      } else {
                        this.field_u = param1.field_g.length;
                        this.field_k = new lga[this.field_u];
                        this.field_E = new gib[this.field_u];
                        var12 = 0;
                        L5: while (true) {
                          if (this.field_u <= var12) {
                            break L4;
                          } else {
                            var69 = param1.field_g[var12];
                            var70 = jm.a(var69.field_e, 0);
                            var15_int = -1;
                            var16 = 0;
                            L6: while (true) {
                              L7: {
                                if (var16 >= this.field_v) {
                                  break L7;
                                } else {
                                  if (var81[var16] != var69.field_b) {
                                    var16++;
                                    continue L6;
                                  } else {
                                    var15_int = var16;
                                    break L7;
                                  }
                                }
                              }
                              if ((var15_int ^ -1) != 0) {
                                L8: {
                                  var16 = 16777215 & klb.field_o[param1.field_b[var69.field_b] & 65535];
                                  stackOut_61_0 = var16;
                                  stackIn_63_0 = stackOut_61_0;
                                  stackIn_62_0 = stackOut_61_0;
                                  if (param1.field_r == null) {
                                    stackOut_63_0 = stackIn_63_0;
                                    stackOut_63_1 = 0;
                                    stackIn_64_0 = stackOut_63_0;
                                    stackIn_64_1 = stackOut_63_1;
                                    break L8;
                                  } else {
                                    stackOut_62_0 = stackIn_62_0;
                                    stackOut_62_1 = param1.field_r[var69.field_b];
                                    stackIn_64_0 = stackOut_62_0;
                                    stackIn_64_1 = stackOut_62_1;
                                    break L8;
                                  }
                                }
                                var16 = stackIn_64_0 | -stackIn_64_1 + 255 << -194885032;
                                this.field_k[var12] = new lga(var15_int, (int) param1.field_k[var69.field_b], (int) param1.field_o[var69.field_b], (int) param1.field_K[var69.field_b], var70.field_g, var70.field_b, var70.field_e, var70.field_d, var70.field_f, var70.field_a, var70.field_c, var69.field_c);
                                this.field_E[var12] = new gib(var16);
                                var12++;
                                continue L5;
                              } else {
                                throw new RuntimeException();
                              }
                            }
                          }
                        }
                      }
                    }
                    L9: {
                      var12 = 3 * this.field_v;
                      this.field_e = new short[this.field_v];
                      this.field_A = new short[var12];
                      bsb.field_b = new long[var12];
                      this.field_x = new short[var12];
                      this.field_y = new float[var12];
                      this.field_F = new short[var12];
                      this.field_r = new short[var12];
                      this.field_i = new short[this.field_v];
                      this.field_n = new byte[this.field_v];
                      this.field_J = new short[var12];
                      this.field_f = new short[var12];
                      this.field_O = new short[this.field_v];
                      this.field_N = new short[this.field_v];
                      this.field_b = new byte[var12];
                      this.field_D = new float[var12];
                      this.field_c = new short[this.field_v];
                      if (param1.field_w == null) {
                        break L9;
                      } else {
                        this.field_d = new short[this.field_v];
                        break L9;
                      }
                    }
                    var13 = 0;
                    var14 = 0;
                    L10: while (true) {
                      if (var14 >= param1.field_D) {
                        this.field_l[param1.field_D] = var13;
                        var75 = ao.a(param1, var81, this.field_v, -3);
                        var15 = new vib[param1.field_j];
                        var16 = 0;
                        L11: while (true) {
                          if (var16 >= param1.field_j) {
                            var16 = 0;
                            L12: while (true) {
                              if (this.field_v <= var16) {
                                L13: {
                                  if (-1 <= (this.field_Q ^ -1)) {
                                    break L13;
                                  } else {
                                    var16 = 1;
                                    var17_int = this.field_O[0];
                                    var18 = 0;
                                    L14: while (true) {
                                      if (this.field_Q <= var18) {
                                        this.field_w = new int[var16];
                                        this.field_g = new int[1 + var16];
                                        this.field_t = new int[var16];
                                        this.field_g[0] = 0;
                                        var18 = this.field_L;
                                        var16 = 0;
                                        var19 = 0;
                                        var17_int = this.field_O[0];
                                        var20 = 0;
                                        L15: while (true) {
                                          if (var20 >= this.field_Q) {
                                            this.field_t[var16] = var18;
                                            this.field_w[var16] = var19 + -var18 + 1;
                                            var16++;
                                            this.field_g[var16] = this.field_Q;
                                            break L13;
                                          } else {
                                            L16: {
                                              var21 = this.field_O[var20];
                                              if (var21 == var17_int) {
                                                break L16;
                                              } else {
                                                this.field_t[var16] = var18;
                                                this.field_w[var16] = var19 + -var18 + 1;
                                                var16++;
                                                this.field_g[var16] = var20;
                                                var18 = this.field_L;
                                                var19 = 0;
                                                var17_int = var21;
                                                break L16;
                                              }
                                            }
                                            L17: {
                                              var22 = this.field_N[var20];
                                              if (var22 <= var19) {
                                                break L17;
                                              } else {
                                                var19 = var22;
                                                break L17;
                                              }
                                            }
                                            L18: {
                                              if (var18 <= var22) {
                                                break L18;
                                              } else {
                                                var18 = var22;
                                                break L18;
                                              }
                                            }
                                            L19: {
                                              var22 = this.field_e[var20];
                                              if (var18 <= var22) {
                                                break L19;
                                              } else {
                                                var18 = var22;
                                                break L19;
                                              }
                                            }
                                            L20: {
                                              if (var22 <= var19) {
                                                break L20;
                                              } else {
                                                var19 = var22;
                                                break L20;
                                              }
                                            }
                                            L21: {
                                              var22 = this.field_i[var20];
                                              if (var18 <= var22) {
                                                break L21;
                                              } else {
                                                var18 = var22;
                                                break L21;
                                              }
                                            }
                                            L22: {
                                              if (var22 <= var19) {
                                                break L22;
                                              } else {
                                                var19 = var22;
                                                break L22;
                                              }
                                            }
                                            var20++;
                                            continue L15;
                                          }
                                        }
                                      } else {
                                        L23: {
                                          var19 = this.field_O[var18];
                                          if (var19 == var17_int) {
                                            break L23;
                                          } else {
                                            var17_int = var19;
                                            var16++;
                                            break L23;
                                          }
                                        }
                                        var18++;
                                        continue L14;
                                      }
                                    }
                                  }
                                }
                                L24: {
                                  bsb.field_b = null;
                                  this.field_A = rh.a(this.field_L, this.field_A, (byte) 53);
                                  this.field_J = rh.a(this.field_L, this.field_J, (byte) 108);
                                  this.field_F = rh.a(this.field_L, this.field_F, (byte) 69);
                                  this.field_r = rh.a(this.field_L, this.field_r, (byte) 50);
                                  this.field_x = rh.a(this.field_L, this.field_x, (byte) 59);
                                  this.field_b = aba.a(this.field_b, this.field_L, -28321);
                                  this.field_D = jp.a(this.field_D, this.field_L, 7353);
                                  this.field_y = jp.a(this.field_y, this.field_L, 7353);
                                  if (null == param1.field_p) {
                                    break L24;
                                  } else {
                                    if (!kg.a((byte) 11, this.field_s, param2)) {
                                      break L24;
                                    } else {
                                      discarded$5 = param1.a((byte) -37, false);
                                      break L24;
                                    }
                                  }
                                }
                                L25: {
                                  if (null == param1.field_g) {
                                    break L25;
                                  } else {
                                    if (!lnb.a(param2, this.field_s, false)) {
                                      break L25;
                                    } else {
                                      discarded$6 = param1.a(-126);
                                      break L25;
                                    }
                                  }
                                }
                                L26: {
                                  if (null == param1.field_y) {
                                    break L26;
                                  } else {
                                    if (!mhb.a(this.field_s, param2, -2679)) {
                                      break L26;
                                    } else {
                                      var16 = 0;
                                      var84 = new int[256];
                                      var80 = var84;
                                      var17 = var80;
                                      var18 = 0;
                                      L27: while (true) {
                                        if (this.field_v <= var18) {
                                          this.field_a = new int[1 + var16][];
                                          var18 = 0;
                                          L28: while (true) {
                                            if (var16 < var18) {
                                              var18 = 0;
                                              L29: while (true) {
                                                if (var18 >= this.field_v) {
                                                  break L26;
                                                } else {
                                                  L30: {
                                                    var19 = param1.field_y[var81[var18]];
                                                    if (-1 < (var19 ^ -1)) {
                                                      break L30;
                                                    } else {
                                                      var17[var19] = var17[var19] + 1;
                                                      this.field_a[var19][var17[var19]] = var18;
                                                      break L30;
                                                    }
                                                  }
                                                  var18++;
                                                  continue L29;
                                                }
                                              }
                                            } else {
                                              array$7 = new int[var84[var18]];
                                              this.field_a[var18] = array$7;
                                              var84[var18] = 0;
                                              var18++;
                                              continue L28;
                                            }
                                          }
                                        } else {
                                          L31: {
                                            var19 = param1.field_y[var81[var18]];
                                            if (0 > var19) {
                                              break L31;
                                            } else {
                                              L32: {
                                                if (var19 <= var16) {
                                                  break L32;
                                                } else {
                                                  var16 = var19;
                                                  break L32;
                                                }
                                              }
                                              var17[var19] = var17[var19] + 1;
                                              break L31;
                                            }
                                          }
                                          var18++;
                                          continue L27;
                                        }
                                      }
                                    }
                                  }
                                }
                                break L0;
                              } else {
                                L33: {
                                  var17_int = var81[var16];
                                  var18 = 65535 & param1.field_b[var17_int];
                                  if (param1.field_M == null) {
                                    var19 = -1;
                                    break L33;
                                  } else {
                                    var19 = param1.field_M[var17_int];
                                    break L33;
                                  }
                                }
                                L34: {
                                  if (param1.field_r != null) {
                                    var20 = param1.field_r[var17_int] & 255;
                                    break L34;
                                  } else {
                                    var20 = 0;
                                    break L34;
                                  }
                                }
                                L35: {
                                  if (null == param1.field_u) {
                                    stackOut_100_0 = -1;
                                    stackIn_101_0 = stackOut_100_0;
                                    break L35;
                                  } else {
                                    stackOut_99_0 = param1.field_u[var17_int];
                                    stackIn_101_0 = stackOut_99_0;
                                    break L35;
                                  }
                                }
                                L36: {
                                  var21 = stackIn_101_0;
                                  if (-1 == var21) {
                                    break L36;
                                  } else {
                                    if ((64 & this.field_s) == 0) {
                                      break L36;
                                    } else {
                                      var74 = var64.a(-20292, var21 & 65535);
                                      if (!var74.field_r) {
                                        break L36;
                                      } else {
                                        var21 = -1;
                                        break L36;
                                      }
                                    }
                                  }
                                }
                                L37: {
                                  var22_float = 0.0f;
                                  var23 = 0.0f;
                                  var24 = 0.0f;
                                  var25 = 0.0f;
                                  var26 = 0.0f;
                                  var27 = 0.0f;
                                  var28 = 0;
                                  var29 = 0;
                                  var30 = 0;
                                  if (0 == (var21 ^ -1)) {
                                    break L37;
                                  } else {
                                    if (var19 == -1) {
                                      var24 = 1.0f;
                                      var29 = 2;
                                      var22_float = 0.0f;
                                      var25 = 1.0f;
                                      var28 = 1;
                                      var27 = 0.0f;
                                      var26 = 0.0f;
                                      var23 = 1.0f;
                                      break L37;
                                    } else {
                                      var19 = var19 & 255;
                                      var31 = param1.field_e[var19];
                                      if (var31 == 0) {
                                        var32_int = param1.field_k[var17_int];
                                        var33_int = param1.field_o[var17_int];
                                        var34 = param1.field_K[var17_int];
                                        var35 = param1.field_t[var19];
                                        var36 = param1.field_i[var19];
                                        var37 = param1.field_H[var19];
                                        var38 = (float)param1.field_v[var35];
                                        var39 = (float)param1.field_G[var35];
                                        var40 = (float)param1.field_x[var35];
                                        var41 = (float)param1.field_v[var36] - var38;
                                        var42 = (float)param1.field_G[var36] - var39;
                                        var43 = -var40 + (float)param1.field_x[var36];
                                        var44 = -var38 + (float)param1.field_v[var37];
                                        var45 = (float)param1.field_G[var37] - var39;
                                        var46 = (float)param1.field_x[var37] - var40;
                                        var47 = (float)param1.field_v[var32_int] - var38;
                                        var48 = (float)param1.field_G[var32_int] - var39;
                                        var49 = (float)param1.field_x[var32_int] - var40;
                                        var50 = -var38 + (float)param1.field_v[var33_int];
                                        var51 = -var39 + (float)param1.field_G[var33_int];
                                        var52 = (float)param1.field_x[var33_int] - var40;
                                        var53 = -var38 + (float)param1.field_v[var34];
                                        var54 = -var39 + (float)param1.field_G[var34];
                                        var55 = -var40 + (float)param1.field_x[var34];
                                        var56 = -(var43 * var45) + var42 * var46;
                                        var57 = -(var41 * var46) + var44 * var43;
                                        var58 = var45 * var41 - var42 * var44;
                                        var59 = var45 * var58 - var57 * var46;
                                        var60 = -(var58 * var44) + var56 * var46;
                                        var61 = var57 * var44 - var45 * var56;
                                        var62 = 1.0f / (var42 * var60 + var41 * var59 + var43 * var61);
                                        var24 = (var52 * var61 + (var51 * var60 + var50 * var59)) * var62;
                                        var26 = var62 * (var55 * var61 + (var54 * var60 + var59 * var53));
                                        var22_float = var62 * (var49 * var61 + (var60 * var48 + var47 * var59));
                                        var60 = var56 * var43 - var41 * var58;
                                        var59 = -(var43 * var57) + var58 * var42;
                                        var61 = var57 * var41 - var56 * var42;
                                        var62 = 1.0f / (var46 * var61 + (var44 * var59 + var60 * var45));
                                        var25 = var62 * (var59 * var50 + var60 * var51 + var52 * var61);
                                        var23 = var62 * (var48 * var60 + var47 * var59 + var49 * var61);
                                        var27 = (var55 * var61 + (var54 * var60 + var59 * var53)) * var62;
                                        break L37;
                                      } else {
                                        var32_int = param1.field_k[var17_int];
                                        var33_int = param1.field_o[var17_int];
                                        var34 = param1.field_K[var17_int];
                                        var35 = var75.field_d[var19];
                                        var36 = var75.field_b[var19];
                                        var37 = var75.field_f[var19];
                                        var86 = var75.field_c[var19];
                                        var39_int = param1.field_n[var19];
                                        var40 = (float)param1.field_L[var19] / 256.0f;
                                        if (1 != var31) {
                                          if (2 == var31) {
                                            var41 = (float)param1.field_f[var19] / 256.0f;
                                            var42 = (float)param1.field_I[var19] / 256.0f;
                                            var43_int = param1.field_v[var33_int] - param1.field_v[var32_int];
                                            var44_int = -param1.field_G[var32_int] + param1.field_G[var33_int];
                                            var45_int = -param1.field_x[var32_int] + param1.field_x[var33_int];
                                            var46_int = -param1.field_v[var32_int] + param1.field_v[var34];
                                            var47_int = param1.field_G[var34] + -param1.field_G[var32_int];
                                            var48_int = param1.field_x[var34] + -param1.field_x[var32_int];
                                            var49_int = var48_int * var44_int + -(var47_int * var45_int);
                                            var50_int = var45_int * var46_int - var48_int * var43_int;
                                            var51_int = -(var44_int * var46_int) + var43_int * var47_int;
                                            var52 = 64.0f / (float)param1.field_F[var19];
                                            var53 = 64.0f / (float)param1.field_a[var19];
                                            var54 = 64.0f / (float)param1.field_C[var19];
                                            var55 = ((float)var51_int * var86[2] + ((float)var49_int * var86[0] + (float)var50_int * var86[1])) / var52;
                                            var56 = (var86[5] * (float)var51_int + ((float)var50_int * var86[4] + (float)var49_int * var86[3])) / var53;
                                            var57 = (var86[7] * (float)var50_int + (float)var49_int * var86[6] + (float)var51_int * var86[8]) / var54;
                                            var30 = mpa.a(-74, var56, var55, var57);
                                            dhb.a(var42, param1.field_x[var32_int], var41, (byte) 47, param1.field_v[var32_int], var37, param1.field_G[var32_int], var30, jqb.field_o, var35, var39_int, var86, var36, var40);
                                            var23 = jqb.field_o[1];
                                            var22_float = jqb.field_o[0];
                                            dhb.a(var42, param1.field_x[var33_int], var41, (byte) 81, param1.field_v[var33_int], var37, param1.field_G[var33_int], var30, jqb.field_o, var35, var39_int, var86, var36, var40);
                                            var24 = jqb.field_o[0];
                                            var25 = jqb.field_o[1];
                                            dhb.a(var42, param1.field_x[var34], var41, (byte) 39, param1.field_v[var34], var37, param1.field_G[var34], var30, jqb.field_o, var35, var39_int, var86, var36, var40);
                                            var26 = jqb.field_o[0];
                                            var27 = jqb.field_o[1];
                                            break L37;
                                          } else {
                                            if (var31 != 3) {
                                              break L37;
                                            } else {
                                              oma.a(var86, param1.field_x[var32_int], var40, jqb.field_o, var37, param1.field_v[var32_int], (byte) -82, var36, param1.field_G[var32_int], var39_int, var35);
                                              var23 = jqb.field_o[1];
                                              var22_float = jqb.field_o[0];
                                              oma.a(var86, param1.field_x[var33_int], var40, jqb.field_o, var37, param1.field_v[var33_int], (byte) -82, var36, param1.field_G[var33_int], var39_int, var35);
                                              var25 = jqb.field_o[1];
                                              var24 = jqb.field_o[0];
                                              oma.a(var86, param1.field_x[var34], var40, jqb.field_o, var37, param1.field_v[var34], (byte) -82, var36, param1.field_G[var34], var39_int, var35);
                                              var26 = jqb.field_o[0];
                                              var27 = jqb.field_o[1];
                                              if (-1 != (1 & var39_int ^ -1)) {
                                                L38: {
                                                  if (var25 - var23 <= 0.5f) {
                                                    if (var23 - var25 <= 0.5f) {
                                                      break L38;
                                                    } else {
                                                      var28 = 2;
                                                      var25 = var25 + 1.0f;
                                                      break L38;
                                                    }
                                                  } else {
                                                    var25 = var25 - 1.0f;
                                                    var28 = 1;
                                                    break L38;
                                                  }
                                                }
                                                if (0.5f >= -var23 + var27) {
                                                  if (0.5f >= -var27 + var23) {
                                                    break L37;
                                                  } else {
                                                    var27 = var27 + 1.0f;
                                                    var29 = 2;
                                                    break L37;
                                                  }
                                                } else {
                                                  var29 = 1;
                                                  var27 = var27 - 1.0f;
                                                  break L37;
                                                }
                                              } else {
                                                L39: {
                                                  if (0.5f < var24 - var22_float) {
                                                    var24 = var24 - 1.0f;
                                                    var28 = 1;
                                                    break L39;
                                                  } else {
                                                    if (-var24 + var22_float <= 0.5f) {
                                                      break L39;
                                                    } else {
                                                      var24 = var24 + 1.0f;
                                                      var28 = 2;
                                                      break L39;
                                                    }
                                                  }
                                                }
                                                if (0.5f < -var22_float + var26) {
                                                  var26 = var26 - 1.0f;
                                                  var29 = 1;
                                                  break L37;
                                                } else {
                                                  if (0.5f >= var22_float - var26) {
                                                    break L37;
                                                  } else {
                                                    var29 = 2;
                                                    var26 = var26 + 1.0f;
                                                    break L37;
                                                  }
                                                }
                                              }
                                            }
                                          }
                                        } else {
                                          var41 = (float)param1.field_C[var19] / 1024.0f;
                                          ac.a(var86, var36, var39_int, var40, param1.field_v[var32_int], param1.field_G[var32_int], (byte) -52, var41, var37, jqb.field_o, var35, param1.field_x[var32_int]);
                                          var22_float = jqb.field_o[0];
                                          var23 = jqb.field_o[1];
                                          ac.a(var86, var36, var39_int, var40, param1.field_v[var33_int], param1.field_G[var33_int], (byte) -52, var41, var37, jqb.field_o, var35, param1.field_x[var33_int]);
                                          var25 = jqb.field_o[1];
                                          var24 = jqb.field_o[0];
                                          ac.a(var86, var36, var39_int, var40, param1.field_v[var34], param1.field_G[var34], (byte) -52, var41, var37, jqb.field_o, var35, param1.field_x[var34]);
                                          var27 = jqb.field_o[1];
                                          var26 = jqb.field_o[0];
                                          var42 = var41 / 2.0f;
                                          if ((var39_int & 1) == 0) {
                                            L40: {
                                              if (var42 >= var24 - var22_float) {
                                                if (var42 >= -var24 + var22_float) {
                                                  break L40;
                                                } else {
                                                  var28 = 2;
                                                  var24 = var24 + var41;
                                                  break L40;
                                                }
                                              } else {
                                                var28 = 1;
                                                var24 = var24 - var41;
                                                break L40;
                                              }
                                            }
                                            if (var26 - var22_float <= var42) {
                                              if (var42 >= -var26 + var22_float) {
                                                break L37;
                                              } else {
                                                var29 = 2;
                                                var26 = var26 + var41;
                                                break L37;
                                              }
                                            } else {
                                              var29 = 1;
                                              var26 = var26 - var41;
                                              break L37;
                                            }
                                          } else {
                                            L41: {
                                              if (var42 < -var23 + var25) {
                                                var28 = 1;
                                                var25 = var25 - var41;
                                                break L41;
                                              } else {
                                                if (var42 >= var23 - var25) {
                                                  break L41;
                                                } else {
                                                  var28 = 2;
                                                  var25 = var25 + var41;
                                                  break L41;
                                                }
                                              }
                                            }
                                            if (var42 >= -var23 + var27) {
                                              if (var23 - var27 <= var42) {
                                                break L37;
                                              } else {
                                                var27 = var27 + var41;
                                                var29 = 2;
                                                break L37;
                                              }
                                            } else {
                                              var29 = 1;
                                              var27 = var27 - var41;
                                              break L37;
                                            }
                                          }
                                        }
                                      }
                                    }
                                  }
                                }
                                L42: {
                                  if (null != param1.field_E) {
                                    var31 = param1.field_E[var17_int];
                                    break L42;
                                  } else {
                                    var31 = 0;
                                    break L42;
                                  }
                                }
                                L43: {
                                  if (var31 == 0) {
                                    var32 = ((long)var20 + (long)(var30 << -1712201608) - -(long)(var18 << 1645891944) << -837908064) + (long)(var19 << 888740898);
                                    var34 = param1.field_k[var17_int];
                                    var35 = param1.field_o[var17_int];
                                    var36 = param1.field_K[var17_int];
                                    var77 = var11[var34];
                                    this.field_N[var16] = this.a(var77.field_d, var16, var34, 31809, var22_float, param1, var77.field_e, var77.field_c, var32, var77.field_a, var23);
                                    var78 = var11[var35];
                                    this.field_e[var16] = this.a(var78.field_d, var16, var35, 31809, var24, param1, var78.field_e, var78.field_c, var32 + (long)var28, var78.field_a, var25);
                                    var79 = var11[var36];
                                    this.field_i[var16] = this.a(var79.field_d, var16, var36, 31809, var26, param1, var79.field_e, var79.field_c, var32 + (long)var29, var79.field_a, var27);
                                    break L43;
                                  } else {
                                    if ((var31 ^ -1) != -2) {
                                      break L43;
                                    } else {
                                      L44: {
                                        var32_ref_vib = var15[var17_int];
                                        stackOut_153_0 = (256 + var32_ref_vib.field_c << -494158538) + (var19 << -2138106782);
                                        stackIn_155_0 = stackOut_153_0;
                                        stackIn_154_0 = stackOut_153_0;
                                        if ((var32_ref_vib.field_a ^ -1) < -1) {
                                          stackOut_155_0 = stackIn_155_0;
                                          stackOut_155_1 = 1024;
                                          stackIn_156_0 = stackOut_155_0;
                                          stackIn_156_1 = stackOut_155_1;
                                          break L44;
                                        } else {
                                          stackOut_154_0 = stackIn_154_0;
                                          stackOut_154_1 = 2048;
                                          stackIn_156_0 = stackOut_154_0;
                                          stackIn_156_1 = stackOut_154_1;
                                          break L44;
                                        }
                                      }
                                      var33 = (long)(stackIn_156_0 + (stackIn_156_1 + (var32_ref_vib.field_d - -256 << -1615583284))) + ((long)(var30 << -1958235016) - -(long)(var18 << 1692459624) + (long)var20 << -1711456160);
                                      this.field_N[var16] = this.a(var32_ref_vib.field_d, var16, (int) param1.field_k[var17_int], 31809, var22_float, param1, 0, var32_ref_vib.field_c, var33, var32_ref_vib.field_a, var23);
                                      this.field_e[var16] = this.a(var32_ref_vib.field_d, var16, (int) param1.field_o[var17_int], 31809, var24, param1, 0, var32_ref_vib.field_c, var33 + (long)var28, var32_ref_vib.field_a, var25);
                                      this.field_i[var16] = this.a(var32_ref_vib.field_d, var16, (int) param1.field_K[var17_int], 31809, var26, param1, 0, var32_ref_vib.field_c, var33 + (long)var29, var32_ref_vib.field_a, var27);
                                      break L43;
                                    }
                                  }
                                }
                                L45: {
                                  if (param1.field_r == null) {
                                    break L45;
                                  } else {
                                    this.field_n[var16] = param1.field_r[var17_int];
                                    break L45;
                                  }
                                }
                                L46: {
                                  if (null == param1.field_w) {
                                    break L46;
                                  } else {
                                    this.field_d[var16] = param1.field_w[var17_int];
                                    break L46;
                                  }
                                }
                                this.field_c[var16] = param1.field_b[var17_int];
                                this.field_O[var16] = (short)var21;
                                var16++;
                                continue L12;
                              }
                            }
                          } else {
                            var17_int = param1.field_k[var16];
                            var18 = param1.field_o[var16];
                            var19 = param1.field_K[var16];
                            var20 = this.field_K[var18] - this.field_K[var17_int];
                            var21 = -this.field_j[var17_int] + this.field_j[var18];
                            var22 = -this.field_p[var17_int] + this.field_p[var18];
                            var23_int = -this.field_K[var17_int] + this.field_K[var19];
                            var24_int = -this.field_j[var17_int] + this.field_j[var19];
                            var25_int = this.field_p[var19] - this.field_p[var17_int];
                            var26_int = -(var22 * var24_int) + var25_int * var21;
                            var27_int = -(var20 * var25_int) + var22 * var23_int;
                            var28 = -(var23_int * var21) + var20 * var24_int;
                            L47: while (true) {
                              L48: {
                                if (8192 < var26_int) {
                                  break L48;
                                } else {
                                  if (-8193 > (var27_int ^ -1)) {
                                    break L48;
                                  } else {
                                    if (var28 > 8192) {
                                      break L48;
                                    } else {
                                      if (var26_int < -8192) {
                                        break L48;
                                      } else {
                                        if (var27_int < -8192) {
                                          break L48;
                                        } else {
                                          if (var28 >= -8192) {
                                            L49: {
                                              var29 = (int)Math.sqrt((double)(var26_int * var26_int - -(var27_int * var27_int) + var28 * var28));
                                              if (-1 > (var29 ^ -1)) {
                                                break L49;
                                              } else {
                                                var29 = 1;
                                                break L49;
                                              }
                                            }
                                            L50: {
                                              var26_int = var26_int * 256 / var29;
                                              var27_int = var27_int * 256 / var29;
                                              var28 = 256 * var28 / var29;
                                              if (param1.field_E == null) {
                                                stackOut_84_0 = 0;
                                                stackIn_85_0 = stackOut_84_0;
                                                break L50;
                                              } else {
                                                stackOut_83_0 = param1.field_E[var16];
                                                stackIn_85_0 = stackOut_83_0;
                                                break L50;
                                              }
                                            }
                                            L51: {
                                              var30 = stackIn_85_0;
                                              if (var30 == 0) {
                                                var71 = var11[var17_int];
                                                var71.field_a = var71.field_a + var26_int;
                                                var71.field_c = var71.field_c + var28;
                                                var71.field_e = var71.field_e + 1;
                                                var71.field_d = var71.field_d + var27_int;
                                                var72 = var11[var18];
                                                var72.field_a = var72.field_a + var26_int;
                                                var72.field_e = var72.field_e + 1;
                                                var72.field_d = var72.field_d + var27_int;
                                                var72.field_c = var72.field_c + var28;
                                                var73 = var11[var19];
                                                var73.field_a = var73.field_a + var26_int;
                                                var73.field_d = var73.field_d + var27_int;
                                                var73.field_c = var73.field_c + var28;
                                                var73.field_e = var73.field_e + 1;
                                                break L51;
                                              } else {
                                                if (1 != var30) {
                                                  break L51;
                                                } else {
                                                  dupTemp$8 = new vib();
                                                  var15[var16] = dupTemp$8;
                                                  var31_ref_vib = dupTemp$8;
                                                  var31_ref_vib.field_d = var27_int;
                                                  var31_ref_vib.field_a = var26_int;
                                                  var31_ref_vib.field_c = var28;
                                                  break L51;
                                                }
                                              }
                                            }
                                            var16++;
                                            continue L11;
                                          } else {
                                            break L48;
                                          }
                                        }
                                      }
                                    }
                                  }
                                }
                              }
                              var28 = var28 >> 1;
                              var26_int = var26_int >> 1;
                              var27_int = var27_int >> 1;
                              continue L47;
                            }
                          }
                        }
                      } else {
                        var15_int = this.field_l[var14];
                        this.field_l[var14] = var13;
                        var11[var14] = new hu();
                        var13 = var13 + var15_int;
                        var14++;
                        continue L10;
                      }
                    }
                  } else {
                    L52: {
                      L53: {
                        var12 = var81[var11_int];
                        var13_ref = null;
                        var14 = 0;
                        var15_int = 0;
                        var16 = 0;
                        var17_int = 0;
                        if (param1.field_g == null) {
                          break L53;
                        } else {
                          var18 = 0;
                          var19 = 0;
                          L54: while (true) {
                            if (param1.field_g.length <= var19) {
                              if (var18 == 0) {
                                break L53;
                              } else {
                                var9[var11_int] = 9223372036854775807L;
                                this.field_Q = this.field_Q - 1;
                                break L52;
                              }
                            } else {
                              L55: {
                                var20_ref_bqa = param1.field_g[var19];
                                if (var12 != var20_ref_bqa.field_b) {
                                  break L55;
                                } else {
                                  L56: {
                                    var21_ref_sr = jm.a(var20_ref_bqa.field_e, 0);
                                    if (!var21_ref_sr.field_a) {
                                      break L56;
                                    } else {
                                      var18 = 1;
                                      break L56;
                                    }
                                  }
                                  if (0 == (var21_ref_sr.field_e ^ -1)) {
                                    break L55;
                                  } else {
                                    var66 = var64.a(-20292, var21_ref_sr.field_e);
                                    if (2 != var66.field_q) {
                                      break L55;
                                    } else {
                                      break L55;
                                    }
                                  }
                                }
                              }
                              var19++;
                              continue L54;
                            }
                          }
                        }
                      }
                      L57: {
                        var18 = -1;
                        if (param1.field_u == null) {
                          break L57;
                        } else {
                          var18 = param1.field_u[var12];
                          if (-1 == var18) {
                            break L57;
                          } else {
                            L58: {
                              var67 = var64.a(-20292, var18 & 65535);
                              var13_ref = var67;
                              if (-1 == (64 & this.field_s ^ -1)) {
                                break L58;
                              } else {
                                if (!var67.field_r) {
                                  break L58;
                                } else {
                                  var13_ref = null;
                                  var18 = -1;
                                  break L57;
                                }
                              }
                            }
                            var16 = var67.field_c;
                            var17_int = var67.field_n;
                            break L57;
                          }
                        }
                      }
                      L59: {
                        L60: {
                          if (null == param1.field_r) {
                            break L60;
                          } else {
                            if (param1.field_r[var12] == 0) {
                              break L60;
                            } else {
                              stackOut_36_0 = 1;
                              stackIn_43_0 = stackOut_36_0;
                              break L59;
                            }
                          }
                        }
                        if (var13_ref != null) {
                          if (((opa) (var13_ref)).field_q != 0) {
                            stackOut_41_0 = 1;
                            stackIn_43_0 = stackOut_41_0;
                            break L59;
                          } else {
                            stackOut_40_0 = 0;
                            stackIn_43_0 = stackOut_40_0;
                            break L59;
                          }
                        } else {
                          stackOut_38_0 = 0;
                          stackIn_43_0 = stackOut_38_0;
                          break L59;
                        }
                      }
                      L61: {
                        L62: {
                          var19 = stackIn_43_0;
                          if (var10 != 0) {
                            break L62;
                          } else {
                            if (var19 == 0) {
                              break L61;
                            } else {
                              break L62;
                            }
                          }
                        }
                        if (param1.field_m == null) {
                          break L61;
                        } else {
                          var14 = var14 + (param1.field_m[var12] << 1722994001);
                          break L61;
                        }
                      }
                      L63: {
                        if (var19 == 0) {
                          break L63;
                        } else {
                          var14 = var14 + 65536;
                          break L63;
                        }
                      }
                      var14 = var14 + (var16 << 1294821256 & 65280);
                      var14 = var14 + (var17_int & 255);
                      var15_int = var15_int + (-65536 & var18 << 1062597424);
                      var15_int = var15_int + (var11_int & 65535);
                      var9[var11_int] = (long)var15_int + ((long)var14 << 843010336);
                      break L52;
                    }
                    var11_int++;
                    continue L3;
                  }
                }
              } else {
                L64: {
                  L65: {
                    if (null == param1.field_E) {
                      break L65;
                    } else {
                      if (2 == param1.field_E[var9_int]) {
                        break L64;
                      } else {
                        break L65;
                      }
                    }
                  }
                  L66: {
                    if (null == param1.field_u) {
                      break L66;
                    } else {
                      if (param1.field_u[var9_int] == -1) {
                        break L66;
                      } else {
                        L67: {
                          var10_ref_opa = var64.a(-20292, 65535 & param1.field_u[var9_int]);
                          if ((this.field_s & 64) == 0) {
                            break L67;
                          } else {
                            if (var10_ref_opa.field_r) {
                              break L66;
                            } else {
                              break L67;
                            }
                          }
                        }
                        if (var10_ref_opa.field_m) {
                          break L64;
                        } else {
                          break L66;
                        }
                      }
                    }
                  }
                  fieldTemp$9 = this.field_v;
                  this.field_v = this.field_v + 1;
                  var8[fieldTemp$9] = var9_int;
                  this.field_l[param1.field_k[var9_int]] = this.field_l[param1.field_k[var9_int]] + 1;
                  this.field_l[param1.field_o[var9_int]] = this.field_l[param1.field_o[var9_int]] + 1;
                  this.field_l[param1.field_K[var9_int]] = this.field_l[param1.field_K[var9_int]] + 1;
                  break L64;
                }
                var9_int++;
                continue L1;
              }
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L68: {
            var7 = decompiledCaughtException;
            stackOut_211_0 = (RuntimeException) (var7);
            stackOut_211_1 = new StringBuilder().append("roa.<init>(");
            stackIn_213_0 = stackOut_211_0;
            stackIn_213_1 = stackOut_211_1;
            stackIn_212_0 = stackOut_211_0;
            stackIn_212_1 = stackOut_211_1;
            if (param0 == null) {
              stackOut_213_0 = (RuntimeException) ((Object) stackIn_213_0);
              stackOut_213_1 = (StringBuilder) ((Object) stackIn_213_1);
              stackOut_213_2 = "null";
              stackIn_214_0 = stackOut_213_0;
              stackIn_214_1 = stackOut_213_1;
              stackIn_214_2 = stackOut_213_2;
              break L68;
            } else {
              stackOut_212_0 = (RuntimeException) ((Object) stackIn_212_0);
              stackOut_212_1 = (StringBuilder) ((Object) stackIn_212_1);
              stackOut_212_2 = "{...}";
              stackIn_214_0 = stackOut_212_0;
              stackIn_214_1 = stackOut_212_1;
              stackIn_214_2 = stackOut_212_2;
              break L68;
            }
          }
          L69: {
            stackOut_214_0 = (RuntimeException) ((Object) stackIn_214_0);
            stackOut_214_1 = ((StringBuilder) (Object) stackIn_214_1).append(stackIn_214_2).append(',');
            stackIn_216_0 = stackOut_214_0;
            stackIn_216_1 = stackOut_214_1;
            stackIn_215_0 = stackOut_214_0;
            stackIn_215_1 = stackOut_214_1;
            if (param1 == null) {
              stackOut_216_0 = (RuntimeException) ((Object) stackIn_216_0);
              stackOut_216_1 = (StringBuilder) ((Object) stackIn_216_1);
              stackOut_216_2 = "null";
              stackIn_217_0 = stackOut_216_0;
              stackIn_217_1 = stackOut_216_1;
              stackIn_217_2 = stackOut_216_2;
              break L69;
            } else {
              stackOut_215_0 = (RuntimeException) ((Object) stackIn_215_0);
              stackOut_215_1 = (StringBuilder) ((Object) stackIn_215_1);
              stackOut_215_2 = "{...}";
              stackIn_217_0 = stackOut_215_0;
              stackIn_217_1 = stackOut_215_1;
              stackIn_217_2 = stackOut_215_2;
              break L69;
            }
          }
          throw rta.a((Throwable) ((Object) stackIn_217_0), stackIn_217_2 + ',' + param2 + ',' + param3 + ',' + param4 + ',' + param5 + ')');
        }
    }

    final int V() {
        if (!(this.field_m)) {
            this.b(-27337);
        }
        return this.field_B;
    }

    private final short a(int param0, int param1, int param2, int param3, float param4, cbb param5, int param6, int param7, long param8, int param9, float param10) {
        int fieldTemp$1 = 0;
        RuntimeException var13 = null;
        int var13_int = 0;
        int var14 = 0;
        int var15 = 0;
        int var16 = 0;
        int var17 = 0;
        int var18 = 0;
        int stackIn_3_0 = 0;
        short stackIn_11_0 = 0;
        short stackIn_14_0 = 0;
        RuntimeException stackIn_16_0 = null;
        StringBuilder stackIn_16_1 = null;
        RuntimeException stackIn_17_0 = null;
        StringBuilder stackIn_17_1 = null;
        RuntimeException stackIn_18_0 = null;
        StringBuilder stackIn_18_1 = null;
        String stackIn_18_2 = null;
        int decompiledRegionSelector0 = 0;
        RuntimeException decompiledCaughtException = null;
        short stackOut_10_0 = 0;
        short stackOut_13_0 = 0;
        int stackOut_2_0 = 0;
        RuntimeException stackOut_15_0 = null;
        StringBuilder stackOut_15_1 = null;
        RuntimeException stackOut_17_0 = null;
        StringBuilder stackOut_17_1 = null;
        String stackOut_17_2 = null;
        RuntimeException stackOut_16_0 = null;
        StringBuilder stackOut_16_1 = null;
        String stackOut_16_2 = null;
        var18 = VoidHunters.field_G;
        try {
          L0: {
            var13_int = this.field_l[param2];
            var14 = this.field_l[param2 + 1];
            var15 = 0;
            var16 = var13_int;
            if (param3 == 31809) {
              L1: while (true) {
                L2: {
                  if (var16 >= var14) {
                    break L2;
                  } else {
                    var17 = this.field_f[var16];
                    if (var17 == 0) {
                      var15 = var16;
                      break L2;
                    } else {
                      if (param8 == bsb.field_b[var16]) {
                        stackOut_10_0 = (short)(-1 + var17);
                        stackIn_11_0 = stackOut_10_0;
                        decompiledRegionSelector0 = 1;
                        break L0;
                      } else {
                        var16++;
                        continue L1;
                      }
                    }
                  }
                }
                this.field_f[var15] = (short)(this.field_L + 1);
                bsb.field_b[var15] = param8;
                this.field_J[this.field_L] = (short)param1;
                this.field_A[this.field_L] = (short)param2;
                this.field_F[this.field_L] = (short)param9;
                this.field_r[this.field_L] = (short)param0;
                this.field_x[this.field_L] = (short)param7;
                this.field_b[this.field_L] = (byte)param6;
                this.field_D[this.field_L] = param4;
                this.field_y[this.field_L] = param10;
                fieldTemp$1 = this.field_L;
                this.field_L = this.field_L + 1;
                stackOut_13_0 = (short)fieldTemp$1;
                stackIn_14_0 = stackOut_13_0;
                decompiledRegionSelector0 = 2;
                break L0;
              }
            } else {
              stackOut_2_0 = 108;
              stackIn_3_0 = stackOut_2_0;
              decompiledRegionSelector0 = 0;
              break L0;
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L3: {
            var13 = decompiledCaughtException;
            stackOut_15_0 = (RuntimeException) (var13);
            stackOut_15_1 = new StringBuilder().append("roa.C(").append(param0).append(',').append(param1).append(',').append(param2).append(',').append(param3).append(',').append(param4).append(',');
            stackIn_17_0 = stackOut_15_0;
            stackIn_17_1 = stackOut_15_1;
            stackIn_16_0 = stackOut_15_0;
            stackIn_16_1 = stackOut_15_1;
            if (param5 == null) {
              stackOut_17_0 = (RuntimeException) ((Object) stackIn_17_0);
              stackOut_17_1 = (StringBuilder) ((Object) stackIn_17_1);
              stackOut_17_2 = "null";
              stackIn_18_0 = stackOut_17_0;
              stackIn_18_1 = stackOut_17_1;
              stackIn_18_2 = stackOut_17_2;
              break L3;
            } else {
              stackOut_16_0 = (RuntimeException) ((Object) stackIn_16_0);
              stackOut_16_1 = (StringBuilder) ((Object) stackIn_16_1);
              stackOut_16_2 = "{...}";
              stackIn_18_0 = stackOut_16_0;
              stackIn_18_1 = stackOut_16_1;
              stackIn_18_2 = stackOut_16_2;
              break L3;
            }
          }
          throw rta.a((Throwable) ((Object) stackIn_18_0), stackIn_18_2 + ',' + param6 + ',' + param7 + ',' + param8 + ',' + param9 + ',' + param10 + ')');
        }
        if (decompiledRegionSelector0 == 0) {
          return (short) stackIn_3_0;
        } else {
          if (decompiledRegionSelector0 == 1) {
            return stackIn_11_0;
          } else {
            return stackIn_14_0;
          }
        }
    }

    final void O(int param0, int param1, int param2) {
        int var4 = 0;
        int var5 = 0;
        var5 = VoidHunters.field_G;
        var4 = 0;
        L0: while (true) {
          if (var4 >= this.field_P) {
            this.a((byte) -111);
            this.field_m = false;
            return;
          } else {
            L1: {
              if (-129 == (param0 ^ -1)) {
                break L1;
              } else {
                this.field_K[var4] = param0 * this.field_K[var4] >> -1677483993;
                break L1;
              }
            }
            L2: {
              if (param1 != 128) {
                this.field_j[var4] = this.field_j[var4] * param1 >> -2010093561;
                break L2;
              } else {
                break L2;
              }
            }
            if (param2 != 128) {
              this.field_p[var4] = param2 * this.field_p[var4] >> 248987111;
              var4++;
              continue L0;
            } else {
              var4++;
              continue L0;
            }
          }
        }
    }

    private final void b(int param0) {
        int var2 = 0;
        int var3 = 0;
        int var4 = 0;
        int var5 = 0;
        int var6 = 0;
        int var7 = 0;
        int var8 = 0;
        int var9 = 0;
        int var10 = 0;
        int var11 = 0;
        int var12 = 0;
        int var13 = 0;
        int var14 = 0;
        int var15 = 0;
        var15 = VoidHunters.field_G;
        var2 = 32767;
        var3 = 32767;
        var4 = 32767;
        var5 = -32768;
        var6 = -32768;
        var7 = -32768;
        var8 = 0;
        var9 = 0;
        var10 = 0;
        L0: while (true) {
          if (var10 >= this.field_P) {
            this.field_z = var4;
            this.field_M = var7;
            if (param0 == -27337) {
              this.field_B = var2;
              this.field_G = var5;
              this.field_m = true;
              return;
            } else {
              return;
            }
          } else {
            L1: {
              var11 = this.field_K[var10];
              var12 = this.field_j[var10];
              if (var3 <= var12) {
                break L1;
              } else {
                var3 = var12;
                break L1;
              }
            }
            L2: {
              if (var6 >= var12) {
                break L2;
              } else {
                var6 = var12;
                break L2;
              }
            }
            L3: {
              if (var11 <= var5) {
                break L3;
              } else {
                var5 = var11;
                break L3;
              }
            }
            L4: {
              var13 = this.field_p[var10];
              if (var11 < var2) {
                var2 = var11;
                break L4;
              } else {
                break L4;
              }
            }
            L5: {
              if (var13 > var7) {
                var7 = var13;
                break L5;
              } else {
                break L5;
              }
            }
            L6: {
              if (var13 >= var4) {
                break L6;
              } else {
                var4 = var13;
                break L6;
              }
            }
            L7: {
              var14 = var11 * var11 + var13 * var13;
              if (var14 <= var8) {
                break L7;
              } else {
                var8 = var14;
                break L7;
              }
            }
            var14 = var12 * var12 + (var13 * var13 + var11 * var11);
            if (var9 < var14) {
              var9 = var14;
              var10++;
              continue L0;
            } else {
              var10++;
              continue L0;
            }
          }
        }
    }

    final int G() {
        if (!this.field_m) {
            this.b(-27337);
        }
        return this.field_M;
    }

    final void a(int param0) {
        if (param0 != 26385) {
            this.field_x = (short[]) null;
        }
        if (!(this.field_o == null)) {
            this.field_o.a(26363);
        }
        if (!(null == this.field_q)) {
            this.field_q.a(26363);
        }
        if (null != this.field_C) {
            this.field_C.a(param0 ^ 490);
        }
        if (!(this.field_h == null)) {
            this.field_h.a(26363);
        }
        if (null != this.field_H) {
            this.field_H.a((byte) -35);
        }
    }

    final int RA() {
        if (!this.field_m) {
            this.b(-27337);
        }
        return this.field_G;
    }

    roa(pf param0, int param1, int param2, boolean param3, boolean param4) {
        RuntimeException var6 = null;
        RuntimeException stackIn_18_0 = null;
        StringBuilder stackIn_18_1 = null;
        RuntimeException stackIn_19_0 = null;
        StringBuilder stackIn_19_1 = null;
        RuntimeException stackIn_20_0 = null;
        StringBuilder stackIn_20_1 = null;
        String stackIn_20_2 = null;
        RuntimeException decompiledCaughtException = null;
        RuntimeException stackOut_17_0 = null;
        StringBuilder stackOut_17_1 = null;
        RuntimeException stackOut_19_0 = null;
        StringBuilder stackOut_19_1 = null;
        String stackOut_19_2 = null;
        RuntimeException stackOut_18_0 = null;
        StringBuilder stackOut_18_1 = null;
        String stackOut_18_2 = null;
        this.field_Q = 0;
        this.field_v = 0;
        this.field_P = 0;
        this.field_m = false;
        this.field_L = 0;
        try {
          L0: {
            L1: {
              L2: {
                this.field_I = param1;
                this.field_s = param2;
                if (param3) {
                  break L2;
                } else {
                  if (!rfb.a((byte) -89, this.field_I, this.field_s)) {
                    break L1;
                  } else {
                    break L2;
                  }
                }
              }
              this.field_o = new glb(lga.a(14216, this.field_s, this.field_I));
              break L1;
            }
            L3: {
              L4: {
                if (param3) {
                  break L4;
                } else {
                  if (!bdb.a(this.field_s, false, this.field_I)) {
                    break L3;
                  } else {
                    break L4;
                  }
                }
              }
              this.field_q = new glb(cma.a(this.field_s, this.field_I, -22785));
              break L3;
            }
            L5: {
              L6: {
                if (param3) {
                  break L6;
                } else {
                  if (!eja.a(this.field_I, (byte) -111, this.field_s)) {
                    break L5;
                  } else {
                    break L6;
                  }
                }
              }
              this.field_C = new glb(pf.a(0, this.field_I, this.field_s));
              break L5;
            }
            L7: {
              L8: {
                if (param3) {
                  break L8;
                } else {
                  if (!wh.a(this.field_s, (byte) 127, this.field_I)) {
                    break L7;
                  } else {
                    break L8;
                  }
                }
              }
              this.field_h = new glb(lbb.a(this.field_s, this.field_I, (byte) 30));
              break L7;
            }
            L9: {
              L10: {
                if (param3) {
                  break L10;
                } else {
                  if (!eta.a(this.field_s, -1104, this.field_I)) {
                    break L9;
                  } else {
                    break L10;
                  }
                }
              }
              this.field_H = new rv(iea.a(this.field_I, this.field_s, (byte) -106));
              break L9;
            }
            break L0;
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L11: {
            var6 = decompiledCaughtException;
            stackOut_17_0 = (RuntimeException) (var6);
            stackOut_17_1 = new StringBuilder().append("roa.<init>(");
            stackIn_19_0 = stackOut_17_0;
            stackIn_19_1 = stackOut_17_1;
            stackIn_18_0 = stackOut_17_0;
            stackIn_18_1 = stackOut_17_1;
            if (param0 == null) {
              stackOut_19_0 = (RuntimeException) ((Object) stackIn_19_0);
              stackOut_19_1 = (StringBuilder) ((Object) stackIn_19_1);
              stackOut_19_2 = "null";
              stackIn_20_0 = stackOut_19_0;
              stackIn_20_1 = stackOut_19_1;
              stackIn_20_2 = stackOut_19_2;
              break L11;
            } else {
              stackOut_18_0 = (RuntimeException) ((Object) stackIn_18_0);
              stackOut_18_1 = (StringBuilder) ((Object) stackIn_18_1);
              stackOut_18_2 = "{...}";
              stackIn_20_0 = stackOut_18_0;
              stackIn_20_1 = stackOut_18_1;
              stackIn_20_2 = stackOut_18_2;
              break L11;
            }
          }
          throw rta.a((Throwable) ((Object) stackIn_20_0), stackIn_20_2 + ',' + param1 + ',' + param2 + ',' + param3 + ',' + param4 + ')');
        }
    }

    static {
    }
}
