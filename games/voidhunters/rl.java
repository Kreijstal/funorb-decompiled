/*
 * Decompiled by CFR-JS 0.4.0.
 */
final class rl extends rqa {
    final static void a(boolean param0, int param1, int param2, int param3, boolean param4, int param5, int param6, boolean param7, boolean param8, boolean param9) {
        int var11;
        var11 = VoidHunters.field_G;
        if (!param7) {
          if (gp.field_p == dma.field_g) {
            L0: {
              if (dma.field_j == wjb.field_p) {
                break L0;
              } else {
                if (dma.field_j != jl.field_q.field_b) {
                  break L0;
                } else {
                  if (jl.field_q.field_d == dma.field_g) {
                    if (null == lqa.field_o) {
                      if (null == ohb.field_o) {
                        bp.a(5);
                        break L0;
                      } else {
                        ok.a((byte) 100, true);
                        break L0;
                      }
                    } else {
                      ok.a((byte) 100, false);
                      break L0;
                    }
                  } else {
                    break L0;
                  }
                }
              }
            }
            if (param7) {
              L1: {
                icb.field_o = ed.field_p;
                pra.a(param7, true);
                if (lia.field_b <= 0) {
                  break L1;
                } else {
                  fdb.a(param0, param7, param5, (byte) 100);
                  break L1;
                }
              }
              L2: {
                qi.field_p.field_F = 1;
                qi.field_p.field_w = mla.field_q.field_w;
                if (-1 > (nf.field_a ^ -1)) {
                  soa.a(param0, -99, param7, param5);
                  break L2;
                } else {
                  break L2;
                }
              }
              L3: {
                if ((tba.field_p ^ -1) < param1) {
                  fka.a(param5, (byte) 105, param8, param0, param7);
                  break L3;
                } else {
                  break L3;
                }
              }
              if (wo.field_o) {
                if (lqa.field_o.field_zb < lqa.field_o.field_Fb) {
                  tja.field_a.field_xb.field_R = null;
                  fh.field_f.field_y = true;
                  bm.a(param6, (byte) 1, tja.field_a, param4, param5);
                  me.a(param2, param6, param4, param9, param5, param3, (byte) 119);
                  bm.a(param6, (byte) 1, rga.field_f, param4, param5);
                  ls.field_o = ls.field_o + 1;
                  return;
                } else {
                  fh.field_f.field_y = false;
                  tja.field_a.field_xb.field_R = ukb.field_q;
                  igb.a(tja.field_a.field_wb, (byte) -93);
                  me.a(param2, param6, param4, param9, param5, param3, (byte) 119);
                  bm.a(param6, (byte) 1, rga.field_f, param4, param5);
                  ls.field_o = ls.field_o + 1;
                  return;
                }
              } else {
                tja.field_a.field_xb.field_R = null;
                fh.field_f.field_y = true;
                bm.a(param6, (byte) 1, tja.field_a, param4, param5);
                me.a(param2, param6, param4, param9, param5, param3, (byte) 119);
                bm.a(param6, (byte) 1, rga.field_f, param4, param5);
                ls.field_o = ls.field_o + 1;
                return;
              }
            } else {
              L4: {
                icb.field_o = (gp.field_p + -640) / 2;
                pra.a(param7, true);
                if (lia.field_b <= 0) {
                  break L4;
                } else {
                  fdb.a(param0, param7, param5, (byte) 100);
                  break L4;
                }
              }
              L5: {
                qi.field_p.field_F = 1;
                qi.field_p.field_w = mla.field_q.field_w;
                if (-1 > (nf.field_a ^ -1)) {
                  soa.a(param0, -99, param7, param5);
                  break L5;
                } else {
                  break L5;
                }
              }
              L6: {
                if ((tba.field_p ^ -1) < param1) {
                  fka.a(param5, (byte) 105, param8, param0, param7);
                  break L6;
                } else {
                  break L6;
                }
              }
              L7: {
                L8: {
                  if (!wo.field_o) {
                    break L8;
                  } else {
                    if (lqa.field_o.field_zb >= lqa.field_o.field_Fb) {
                      fh.field_f.field_y = false;
                      tja.field_a.field_xb.field_R = ukb.field_q;
                      igb.a(tja.field_a.field_wb, (byte) -93);
                      break L7;
                    } else {
                      break L8;
                    }
                  }
                }
                tja.field_a.field_xb.field_R = null;
                fh.field_f.field_y = true;
                bm.a(param6, (byte) 1, tja.field_a, param4, param5);
                break L7;
              }
              me.a(param2, param6, param4, param9, param5, param3, (byte) 119);
              bm.a(param6, (byte) 1, rga.field_f, param4, param5);
              ls.field_o = ls.field_o + 1;
              return;
            }
          } else {
            L9: {
              if (dma.field_j != jl.field_q.field_b) {
                break L9;
              } else {
                if (jl.field_q.field_d == dma.field_g) {
                  if (null == lqa.field_o) {
                    if (null == ohb.field_o) {
                      bp.a(5);
                      break L9;
                    } else {
                      ok.a((byte) 100, true);
                      break L9;
                    }
                  } else {
                    ok.a((byte) 100, false);
                    break L9;
                  }
                } else {
                  break L9;
                }
              }
            }
            if (!param7) {
              L10: {
                icb.field_o = (gp.field_p + -640) / 2;
                pra.a(param7, true);
                if (lia.field_b <= 0) {
                  break L10;
                } else {
                  fdb.a(param0, param7, param5, (byte) 100);
                  break L10;
                }
              }
              L11: {
                qi.field_p.field_F = 1;
                qi.field_p.field_w = mla.field_q.field_w;
                if (-1 > (nf.field_a ^ -1)) {
                  soa.a(param0, -99, param7, param5);
                  break L11;
                } else {
                  break L11;
                }
              }
              L12: {
                if ((tba.field_p ^ -1) < param1) {
                  fka.a(param5, (byte) 105, param8, param0, param7);
                  break L12;
                } else {
                  break L12;
                }
              }
              L13: {
                L14: {
                  if (!wo.field_o) {
                    break L14;
                  } else {
                    if (lqa.field_o.field_zb >= lqa.field_o.field_Fb) {
                      fh.field_f.field_y = false;
                      tja.field_a.field_xb.field_R = ukb.field_q;
                      igb.a(tja.field_a.field_wb, (byte) -93);
                      break L13;
                    } else {
                      break L14;
                    }
                  }
                }
                tja.field_a.field_xb.field_R = null;
                fh.field_f.field_y = true;
                bm.a(param6, (byte) 1, tja.field_a, param4, param5);
                break L13;
              }
              me.a(param2, param6, param4, param9, param5, param3, (byte) 119);
              bm.a(param6, (byte) 1, rga.field_f, param4, param5);
              ls.field_o = ls.field_o + 1;
              return;
            } else {
              L15: {
                icb.field_o = ed.field_p;
                pra.a(param7, true);
                if (lia.field_b <= 0) {
                  break L15;
                } else {
                  fdb.a(param0, param7, param5, (byte) 100);
                  break L15;
                }
              }
              L16: {
                qi.field_p.field_F = 1;
                qi.field_p.field_w = mla.field_q.field_w;
                if (-1 > (nf.field_a ^ -1)) {
                  soa.a(param0, -99, param7, param5);
                  break L16;
                } else {
                  break L16;
                }
              }
              L17: {
                if ((tba.field_p ^ -1) < param1) {
                  fka.a(param5, (byte) 105, param8, param0, param7);
                  break L17;
                } else {
                  break L17;
                }
              }
              L18: {
                L19: {
                  if (!wo.field_o) {
                    break L19;
                  } else {
                    if (lqa.field_o.field_zb >= lqa.field_o.field_Fb) {
                      fh.field_f.field_y = false;
                      tja.field_a.field_xb.field_R = ukb.field_q;
                      igb.a(tja.field_a.field_wb, (byte) -93);
                      break L18;
                    } else {
                      break L19;
                    }
                  }
                }
                tja.field_a.field_xb.field_R = null;
                fh.field_f.field_y = true;
                bm.a(param6, (byte) 1, tja.field_a, param4, param5);
                break L18;
              }
              me.a(param2, param6, param4, param9, param5, param3, (byte) 119);
              bm.a(param6, (byte) 1, rga.field_f, param4, param5);
              ls.field_o = ls.field_o + 1;
              return;
            }
          }
        } else {
          L20: {
            icb.field_o = ed.field_p;
            pra.a(param7, true);
            if (lia.field_b <= 0) {
              break L20;
            } else {
              fdb.a(param0, param7, param5, (byte) 100);
              break L20;
            }
          }
          L21: {
            qi.field_p.field_F = 1;
            qi.field_p.field_w = mla.field_q.field_w;
            if (-1 > (nf.field_a ^ -1)) {
              soa.a(param0, -99, param7, param5);
              break L21;
            } else {
              break L21;
            }
          }
          L22: {
            if ((tba.field_p ^ -1) < param1) {
              fka.a(param5, (byte) 105, param8, param0, param7);
              break L22;
            } else {
              break L22;
            }
          }
          L23: {
            L24: {
              if (!wo.field_o) {
                break L24;
              } else {
                if (lqa.field_o.field_zb >= lqa.field_o.field_Fb) {
                  fh.field_f.field_y = false;
                  tja.field_a.field_xb.field_R = ukb.field_q;
                  igb.a(tja.field_a.field_wb, (byte) -93);
                  break L23;
                } else {
                  break L24;
                }
              }
            }
            tja.field_a.field_xb.field_R = null;
            fh.field_f.field_y = true;
            bm.a(param6, (byte) 1, tja.field_a, param4, param5);
            break L23;
          }
          me.a(param2, param6, param4, param9, param5, param3, (byte) 119);
          bm.a(param6, (byte) 1, rga.field_f, param4, param5);
          ls.field_o = ls.field_o + 1;
          return;
        }
    }

    rl(String param0, Class[] param1, String param2) {
        super(param0, param1, param2);
    }

    final nc a(nc[] param0, int param1) {
        RuntimeException var3 = null;
        nc stackIn_3_0 = null;
        RuntimeException stackIn_6_0 = null;
        StringBuilder stackIn_6_1 = null;
        RuntimeException stackIn_7_0 = null;
        StringBuilder stackIn_7_1 = null;
        String stackIn_7_2 = null;
        RuntimeException decompiledCaughtException = null;
        try {
          L0: {
            L1: {
              if (param1 < -119) {
                break L1;
              } else {
                rl.b(true);
                break L1;
              }
            }
            stackIn_3_0 = new nc(fua.field_o);
            break L0;
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L2: {
            var3 = decompiledCaughtException;
            stackIn_6_0 = (RuntimeException) (var3);

            stackIn_6_1 = new StringBuilder().append("rl.A(");

            if (param0 == null) {
              stackIn_7_0 = (RuntimeException) ((Object) stackIn_6_0);
              stackIn_7_1 = (StringBuilder) ((Object) stackIn_6_1);
              stackIn_7_2 = "null";
              break L2;
            } else {
              stackIn_7_0 = (RuntimeException) ((Object) stackIn_6_0);
              stackIn_7_1 = (StringBuilder) ((Object) stackIn_6_1);
              stackIn_7_2 = "{...}";
              break L2;
            }
          }
          throw rta.a((Throwable) ((Object) stackIn_7_0), stackIn_7_2 + ',' + param1 + ')');
        }
        return stackIn_3_0;
    }

    final static void b(boolean param0) {
        if (!param0) {
          if (!ar.field_d) {
            if (fjb.field_c != null) {
              if (fjb.field_c.field_d) {
                fab.a(-29365, fjb.field_c.field_f);
                fjb.field_c = null;
                ar.field_d = true;
                ilb.a((byte) -114);
                bjb.a(15334);
                return;
              } else {
                return;
              }
            } else {
              fjb.field_c = drb.a(true, 6);
              if (fjb.field_c.field_d) {
                fab.a(-29365, fjb.field_c.field_f);
                fjb.field_c = null;
                ar.field_d = true;
                ilb.a((byte) -114);
                bjb.a(15334);
                return;
              } else {
                return;
              }
            }
          } else {
            return;
          }
        } else {
          rl.a(true, 69, -111, -75, true, 23, 116, false, false, true);
          if (!ar.field_d) {
            if (fjb.field_c == null) {
              fjb.field_c = drb.a(true, 6);
              if (!fjb.field_c.field_d) {
                return;
              } else {
                fab.a(-29365, fjb.field_c.field_f);
                fjb.field_c = null;
                ar.field_d = true;
                ilb.a((byte) -114);
                bjb.a(15334);
                return;
              }
            } else {
              if (fjb.field_c.field_d) {
                fab.a(-29365, fjb.field_c.field_f);
                fjb.field_c = null;
                ar.field_d = true;
                ilb.a((byte) -114);
                bjb.a(15334);
                return;
              } else {
                return;
              }
            }
          } else {
            return;
          }
        }
    }

    static {
    }
}
