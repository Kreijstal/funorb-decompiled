/*
 * Decompiled by CFR-JS 0.4.0.
 */
final class qa {
    static bd[] field_g;
    static int[] field_d;
    static String field_a;
    static String field_f;
    static int field_e;
    static bd field_c;
    static int field_b;

    final static bd[] a(int param0) {
        bd[] var1;
        int var2;
        int var3;
        byte[] var5;
        int var6;
        int[] var6_ref_int__;
        int var7;
        int var8;
        byte[] var9;
        int[] var10;
        byte[] var11;
        int[] var13;
        byte[] var15;
        int[] var17;
        byte[] var21;
        int[] var22;
        var8 = HostileSpawn.field_I ? 1 : 0;
        var1 = new bd[sc.field_c];
        var2 = 0;
        L0: while (true) {
          if (var2 >= sc.field_c) {
            L1: {
              if (param0 == 26627) {
                break L1;
              } else {
                qa.a(-20, 98);
                break L1;
              }
            }
            uj.d(param0 ^ -26740);
            return var1;
          } else {
            var3 = hd.field_r[var2] * sf.field_X[var2];
            var21 = ja.field_e[var2];
            if (!qj.field_k[var2]) {
              var10 = new int[var3];
              var22 = var10;
              var6 = 0;
              L2: while (true) {
                if (var6 >= var3) {
                  var1[var2] = new bd(ra.field_a, vc.field_e, th.field_Jb[var2], nh.field_O[var2], sf.field_X[var2], hd.field_r[var2], var22);
                  var2++;
                  continue L0;
                } else {
                  var10[var6] = bf.field_d[ua.a((int) var21[var6], 255)];
                  var6++;
                  continue L2;
                }
              }
            } else {
              var15 = vh.field_a[var2];
              var11 = var15;
              var9 = var11;
              var5 = var9;
              var17 = new int[var3];
              var13 = var17;
              var6_ref_int__ = var13;
              var7 = 0;
              L3: while (true) {
                if (var7 >= var3) {
                  var1[var2] = (bd) ((Object) new gk(ra.field_a, vc.field_e, th.field_Jb[var2], nh.field_O[var2], sf.field_X[var2], hd.field_r[var2], var17));
                  var2++;
                  continue L0;
                } else {
                  var6_ref_int__[var7] = ll.a(ua.a(-16777216, var15[var7] << -528475528), bf.field_d[ua.a((int) var21[var7], 255)]);
                  var7++;
                  continue L3;
                }
              }
            }
          }
        }
    }

    final static void a(int param0, int param1, int param2, int param3, int param4, int param5, int param6) {
        int stackIn_72_0 = 0;
        int stackIn_75_0 = 0;
        int stackIn_78_0 = 0;
        int stackIn_81_0 = 0;
        int stackIn_84_0 = 0;
        int stackIn_87_0 = 0;
        int stackIn_90_0 = 0;
        int stackIn_93_0 = 0;
        RuntimeException decompiledCaughtException = null;
        int var7_int = 0;
        RuntimeException var7 = null;
        int var8 = 0;
        int var9 = 0;
        int var10 = 0;
        int var11 = 0;
        int[] var12 = null;
        boolean[] var13 = null;
        boolean[] var14 = null;
        int var16 = 0;
        int var17 = 0;
        int var18 = 0;
        int var19 = 0;
        int var20 = 0;
        int var21 = 0;
        int var22 = 0;
        int var23 = 0;
        int var24 = 0;
        int var25 = 0;
        int var26 = 0;
        bd var27 = null;
        int var28 = 0;
        int var29 = 0;
        int var30 = 0;
        String var31 = null;
        int[] var33 = null;
        int[] var35 = null;
        int[] var37 = null;
        var30 = HostileSpawn.field_I ? 1 : 0;
        try {
          L0: {
            L1: {
              var10 = uj.field_p.field_e;
              if (param1 > 13) {
                break L1;
              } else {
                var31 = (String) null;
                qa.a(true, (byte) -106, (String) null);
                break L1;
              }
            }
            var11 = uj.field_p.field_g;
            var35 = ln.field_a.field_y;
            var33 = var35;
            var12 = var33;
            var13 = ln.field_a.field_B;
            var14 = ln.field_a.field_r;
            var37 = ln.field_a.field_o;
            var16 = param2;
            L2: while (true) {
              if (param4 < var16) {
                break L0;
              } else {
                var7_int = param5 * var16 - -param0;
                var9 = var16 * 24 - var11;
                var17 = param0;
                L3: while (true) {
                  if (param3 < var17) {
                    var16++;
                    continue L2;
                  } else {
                    L4: {
                      L5: {
                        var8 = var17 * 24 + -var10;
                        var7_int++;
                        var18 = sa.a((byte) -128, var7_int);
                        var19 = var37[var7_int];
                        var20 = 250 - var18 * 10;
                        var23 = var18;
                        var22 = var18;
                        var21 = var18;
                        var24 = var18;
                        if (-1 == (r.field_a ^ -1)) {
                          break L5;
                        } else {
                          if (var19 < 0) {
                            var25 = ee.a(var18, -param5 + var7_int - 1, false);
                            var21 = var21 + var25;
                            var25 = ee.a(var18, var7_int + 0 + -param5, false);
                            var22 = var22 + var25;
                            var21 = var21 + var25;
                            var25 = ee.a(var18, 1 + (var7_int - param5), false);
                            var22 = var22 + var25;
                            var25 = ee.a(var18, -1 + var7_int, false);
                            var23 = var23 + var25;
                            var21 = var21 + var25;
                            var25 = ee.a(var18, 1 + var7_int, false);
                            var22 = var22 + var25;
                            var24 = var24 + var25;
                            var25 = ee.a(var18, var7_int + (-1 + param5), false);
                            var23 = var23 + var25;
                            var25 = ee.a(var18, param5 + var7_int - 0, false);
                            var23 = var23 + var25;
                            var24 = var24 + var25;
                            var25 = ee.a(var18, param5 + (1 + var7_int), false);
                            var24 = var24 + var25;
                            break L4;
                          } else {
                            break L5;
                          }
                        }
                      }
                      var25 = sa.a((byte) -128, var7_int - (1 - -param5));
                      var21 = var21 + var25;
                      var25 = sa.a((byte) -126, -param5 + (var7_int - 0));
                      var22 = var22 + var25;
                      var21 = var21 + var25;
                      var25 = sa.a((byte) -121, 1 + var7_int - param5);
                      var22 = var22 + var25;
                      var25 = sa.a((byte) -113, -1 + var7_int);
                      var23 = var23 + var25;
                      var21 = var21 + var25;
                      var25 = sa.a((byte) -105, 1 + var7_int);
                      var22 = var22 + var25;
                      var24 = var24 + var25;
                      var25 = sa.a((byte) -111, param5 + (var7_int + -1));
                      var23 = var23 + var25;
                      var25 = sa.a((byte) -117, 0 + var7_int - -param5);
                      var24 = var24 + var25;
                      var23 = var23 + var25;
                      var25 = sa.a((byte) -127, param5 + var7_int - -1);
                      var24 = var24 + var25;
                      break L4;
                    }
                    L6: {
                      L7: {
                        L8: {
                          var21 = var21 >> 2;
                          var22 = var22 >> 2;
                          var24 = var24 >> 2;
                          var23 = var23 >> 2;
                          var21 = var21 * 10;
                          var23 = var23 * 10;
                          var22 = var22 * 10;
                          var24 = var24 * 10;
                          var27 = pi.field_a[2];
                          if (-10 < (var35[var7_int] ^ -1)) {
                            break L8;
                          } else {
                            if ((var35[var7_int] ^ -1) >= -14) {
                              break L7;
                            } else {
                              break L8;
                            }
                          }
                        }
                        L9: {
                          if ((var35[var7_int] ^ -1) > -17) {
                            break L9;
                          } else {
                            if (17 >= var35[var7_int]) {
                              break L7;
                            } else {
                              break L9;
                            }
                          }
                        }
                        L10: {
                          if (-23 < (var35[var7_int] ^ -1)) {
                            break L10;
                          } else {
                            if ((var35[var7_int] ^ -1) >= -24) {
                              break L7;
                            } else {
                              break L10;
                            }
                          }
                        }
                        L11: {
                          if (-31 < (var35[var7_int] ^ -1)) {
                            break L11;
                          } else {
                            if (-32 <= (var35[var7_int] ^ -1)) {
                              break L7;
                            } else {
                              break L11;
                            }
                          }
                        }
                        if ((var35[var7_int] ^ -1) > -39) {
                          break L6;
                        } else {
                          if (39 >= var35[var7_int]) {
                            break L7;
                          } else {
                            break L6;
                          }
                        }
                      }
                      var27 = fa.field_d[var35[var7_int]];
                      break L6;
                    }
                    L12: {
                      L13: {
                        if ((var21 ^ -1) < -1) {
                          break L13;
                        } else {
                          if (-1 > (var22 ^ -1)) {
                            break L13;
                          } else {
                            if (var23 > 0) {
                              break L13;
                            } else {
                              if (-1 <= (var24 ^ -1)) {
                                si.a(var8, var9, 24, 24, 0, 255);
                                break L12;
                              } else {
                                break L13;
                              }
                            }
                          }
                        }
                      }
                      L14: {
                        if (var22 != var21) {
                          break L14;
                        } else {
                          if (var21 != var23) {
                            break L14;
                          } else {
                            if (var24 == var21) {
                              si.a(var8, var9, 24, 24, 0, 256 - var21);
                              break L12;
                            } else {
                              break L14;
                            }
                          }
                        }
                      }
                      eh.a(var8, var9, var21, var22, var23, var24);
                      break L12;
                    }
                    L15: {
                      L16: {
                        if (var19 < 0) {
                          break L16;
                        } else {
                          if (-1 > (ak.field_b[var19].field_a ^ -1)) {
                            if (r.field_a == 0) {
                              if (4 == var19 >> -941592216) {
                                break L15;
                              } else {
                                break L15;
                              }
                            } else {
                              break L15;
                            }
                          } else {
                            break L16;
                          }
                        }
                      }
                      L17: {
                        if (var13[var7_int]) {
                          break L17;
                        } else {
                          if (-1 < (rl.field_c ^ -1)) {
                            break L17;
                          } else {
                            if (-3 > (rl.field_c ^ -1)) {
                              break L17;
                            } else {
                              if (bm.field_c == 0) {
                                L18: {
                                  L19: {
                                    if (-3 != (rl.field_c ^ -1)) {
                                      break L19;
                                    } else {
                                      if (-1 != (var35[var7_int] ^ -1)) {
                                        break L18;
                                      } else {
                                        break L19;
                                      }
                                    }
                                  }
                                  if (var12[-1 + var7_int] != 0) {
                                    break L18;
                                  } else {
                                    if (-1 != (var12[1 + var7_int] ^ -1)) {
                                      break L18;
                                    } else {
                                      if (-1 != (var12[var7_int + -param5] ^ -1)) {
                                        break L18;
                                      } else {
                                        if (var12[var7_int + param5] != 0) {
                                          break L18;
                                        } else {
                                          if (0 != var12[-1 + (var7_int + -param5)]) {
                                            break L18;
                                          } else {
                                            if (0 != var12[var7_int - -1 - param5]) {
                                              break L18;
                                            } else {
                                              if (0 != var12[var7_int + param5 - 1]) {
                                                break L18;
                                              } else {
                                                if (var12[-param5 + 1 + var7_int] != 0) {
                                                  break L18;
                                                } else {
                                                  break L15;
                                                }
                                              }
                                            }
                                          }
                                        }
                                      }
                                    }
                                  }
                                }
                                if (var18 != 0) {
                                  break L15;
                                } else {
                                  L20: {
                                    if (var12[-param5 + (-1 + var7_int)] != 0) {
                                      stackIn_72_0 = -25;
                                      break L20;
                                    } else {
                                      stackIn_72_0 = 25;
                                      break L20;
                                    }
                                  }
                                  L21: {
                                    var25 = stackIn_72_0;
                                    var24 = var18;
                                    var22 = var18;
                                    var23 = var18;
                                    var21 = var18;
                                    var21 = var21 + var25;
                                    if (-1 == (var12[var7_int - 0 - param5] ^ -1)) {
                                      stackIn_75_0 = 25;
                                      break L21;
                                    } else {
                                      stackIn_75_0 = -25;
                                      break L21;
                                    }
                                  }
                                  L22: {
                                    var25 = stackIn_75_0;
                                    var22 = var22 + var25;
                                    var21 = var21 + var25;
                                    if (var12[1 + (var7_int - param5)] == 0) {
                                      stackIn_78_0 = 25;
                                      break L22;
                                    } else {
                                      stackIn_78_0 = -25;
                                      break L22;
                                    }
                                  }
                                  L23: {
                                    var25 = stackIn_78_0;
                                    var22 = var22 + var25;
                                    if (var12[-1 + var7_int] == 0) {
                                      stackIn_81_0 = 25;
                                      break L23;
                                    } else {
                                      stackIn_81_0 = -25;
                                      break L23;
                                    }
                                  }
                                  L24: {
                                    var25 = stackIn_81_0;
                                    var23 = var23 + var25;
                                    var21 = var21 + var25;
                                    if (var12[1 + var7_int] == 0) {
                                      stackIn_84_0 = 25;
                                      break L24;
                                    } else {
                                      stackIn_84_0 = -25;
                                      break L24;
                                    }
                                  }
                                  L25: {
                                    var25 = stackIn_84_0;
                                    var24 = var24 + var25;
                                    var22 = var22 + var25;
                                    if (var12[var7_int - (1 + -param5)] == 0) {
                                      stackIn_87_0 = 25;
                                      break L25;
                                    } else {
                                      stackIn_87_0 = -25;
                                      break L25;
                                    }
                                  }
                                  L26: {
                                    var25 = stackIn_87_0;
                                    var23 = var23 + var25;
                                    if (var12[0 + var7_int + param5] != 0) {
                                      stackIn_90_0 = -25;
                                      break L26;
                                    } else {
                                      stackIn_90_0 = 25;
                                      break L26;
                                    }
                                  }
                                  L27: {
                                    var25 = stackIn_90_0;
                                    var24 = var24 + var25;
                                    var23 = var23 + var25;
                                    if (0 == var12[var7_int - -1 - -param5]) {
                                      stackIn_93_0 = 25;
                                      break L27;
                                    } else {
                                      stackIn_93_0 = -25;
                                      break L27;
                                    }
                                  }
                                  L28: {
                                    var25 = stackIn_93_0;
                                    var24 = var24 + var25;
                                    var23 = var23 >> 2;
                                    var22 = var22 >> 2;
                                    var21 = var21 >> 2;
                                    var24 = var24 >> 2;
                                    if (0 > var22) {
                                      var22 = 0;
                                      break L28;
                                    } else {
                                      break L28;
                                    }
                                  }
                                  L29: {
                                    if (var23 >= 0) {
                                      break L29;
                                    } else {
                                      var23 = 0;
                                      break L29;
                                    }
                                  }
                                  L30: {
                                    if (-1 >= (var21 ^ -1)) {
                                      break L30;
                                    } else {
                                      var21 = 0;
                                      break L30;
                                    }
                                  }
                                  L31: {
                                    var21 = 250 + -(var21 * 10);
                                    var22 = 250 - 10 * var22;
                                    var23 = -(var23 * 10) + 250;
                                    if (var24 < 0) {
                                      var24 = 0;
                                      break L31;
                                    } else {
                                      break L31;
                                    }
                                  }
                                  L32: {
                                    var24 = 250 + -(var24 * 10);
                                    if (var21 != var22) {
                                      break L32;
                                    } else {
                                      if (var23 != var21) {
                                        break L32;
                                      } else {
                                        if (var24 != var21) {
                                          break L32;
                                        } else {
                                          si.a(var8, var9, 24, 24, 0, var21);
                                          break L15;
                                        }
                                      }
                                    }
                                  }
                                  var28 = 0;
                                  L33: while (true) {
                                    if (-25 >= (var28 ^ -1)) {
                                      break L15;
                                    } else {
                                      var26 = (24 - var28) * var23 + var28 * var24 >> 584784900;
                                      var25 = var21 * (24 + -var28) + var22 * var28 >> -616109340;
                                      var29 = 0;
                                      L34: while (true) {
                                        if (24 <= var29) {
                                          var28 += 6;
                                          continue L33;
                                        } else {
                                          var20 = (24 + -var29) * var25 + var26 * var29 >> 1686761252;
                                          si.a(var8 + var28, var29 + var9, 6, 6, 0, var20);
                                          var29 += 6;
                                          continue L34;
                                        }
                                      }
                                    }
                                  }
                                }
                              } else {
                                break L17;
                              }
                            }
                          }
                        }
                      }
                      if (var21 != 0) {
                        break L15;
                      } else {
                        if (var22 != 0) {
                          break L15;
                        } else {
                          if (0 != var23) {
                            break L15;
                          } else {
                            if (var24 != 0) {
                              break L15;
                            } else {
                              L35: {
                                if (var13[var7_int]) {
                                  break L35;
                                } else {
                                  if (!var14[var7_int]) {
                                    break L15;
                                  } else {
                                    break L35;
                                  }
                                }
                              }
                              var27.a(var8, var9, 24, 24);
                              break L15;
                            }
                          }
                        }
                      }
                    }
                    var17++;
                    continue L3;
                  }
                }
              }
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          var7 = decompiledCaughtException;
          throw wg.a((Throwable) ((Object) var7), "qa.B(" + param0 + ',' + param1 + ',' + param2 + ',' + param3 + ',' + param4 + ',' + param5 + ',' + param6 + ')');
        }
    }

    final static int a(int param0, int param1) {
        param0--;
        param0 = param0 | param0 >>> 510507425;
        param0 = param0 | param0 >>> 1601519202;
        param0 = param0 | param0 >>> -277196572;
        param0 = param0 | param0 >>> -2047848920;
        if (param1 != -1) {
            qa.a(-95, 16, -125, 100, -24, -56, 98);
        }
        param0 = param0 | param0 >>> -138262160;
        return 1 + param0;
    }

    final static void a(boolean param0, byte param1, String param2) {
        try {
            md.field_i = param0;
            eh.field_e = true;
            if (param1 > -19) {
                field_b = -46;
            }
            jj.field_d = new dg(im.field_e, pe.field_eb, param2, li.field_o, md.field_i);
            im.field_e.c(jj.field_d, 101);
        } catch (RuntimeException runtimeException) {
            throw wg.a((Throwable) ((Object) runtimeException), "qa.G(" + param0 + ',' + param1 + ',' + (param2 != null ? "{...}" : "null") + ')');
        }
    }

    final static void a(boolean param0) {
        if (-tk.field_d + 0 != tk.field_e && tk.field_e != 250 + -tk.field_d) {
        }
        tk.field_e = tk.field_e + 1;
        if (!param0) {
            int[] var2 = (int[]) null;
            qa.a((int[]) null, -44, true);
        }
    }

    public static void b(boolean param0) {
        field_d = null;
        field_a = null;
        field_g = null;
        field_f = null;
        field_c = null;
        if (!param0) {
            field_g = (bd[]) null;
        }
    }

    final static boolean a(int[] param0, int param1, boolean param2) {
        RuntimeException var3 = null;
        int stackIn_5_0 = 0;
        RuntimeException stackIn_8_0 = null;
        StringBuilder stackIn_8_1 = null;
        RuntimeException stackIn_9_0 = null;
        StringBuilder stackIn_9_1 = null;
        String stackIn_9_2 = null;
        RuntimeException decompiledCaughtException = null;
        try {
          L0: {
            L1: {
              if (param2) {
                break L1;
              } else {
                qa.a(true);
                break L1;
              }
            }
            L2: {
              if (0 != (param0[param1 >> 1846534725] & 1 << (31 & param1))) {
                stackIn_5_0 = 0;
                break L2;
              } else {
                stackIn_5_0 = 1;
                break L2;
              }
            }
            break L0;
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L3: {
            var3 = decompiledCaughtException;
            stackIn_8_0 = (RuntimeException) (var3);

            stackIn_8_1 = new StringBuilder().append("qa.D(");

            if (param0 == null) {
              stackIn_9_0 = (RuntimeException) ((Object) stackIn_8_0);
              stackIn_9_1 = (StringBuilder) ((Object) stackIn_8_1);
              stackIn_9_2 = "null";
              break L3;
            } else {
              stackIn_9_0 = (RuntimeException) ((Object) stackIn_8_0);
              stackIn_9_1 = (StringBuilder) ((Object) stackIn_8_1);
              stackIn_9_2 = "{...}";
              break L3;
            }
          }
          throw wg.a((Throwable) ((Object) stackIn_9_0), stackIn_9_2 + ',' + param1 + ',' + param2 + ')');
        }
        return stackIn_5_0 != 0;
    }

    static {
        field_a = "If I find whoever is leaving all these crates here ...";
        field_b = 0;
    }
}
