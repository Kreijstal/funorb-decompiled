/*
 * Decompiled by CFR-JS 0.4.0.
 */
final class ae {
    private int field_k;
    static String[] field_a;
    static String field_h;
    private int field_d;
    int field_i;
    int field_b;
    private int field_j;
    private boolean field_f;
    private int field_g;
    static String field_c;
    static String field_e;

    final void d(int param0) {
        this.field_d = 0;
        this.field_k = 0;
        if (-1 == (this.field_g ^ -1)) {
            if (!((mm.field_t ^ -1) != -99)) {
                if (!(0 < this.field_b)) {
                    this.field_b = this.field_i;
                }
                this.field_b = this.field_b - 1;
                this.field_f = false;
            }
        }
        if (this.field_g == 0) {
            this.field_d = mm.field_t;
        }
        if (param0 == this.field_g && 99 == mm.field_t) {
            this.field_b = this.field_b + 1;
            if (this.field_b >= this.field_i) {
                this.field_b = 0;
            }
            this.field_f = false;
        }
    }

    final boolean b(int param0) {
        if (param0 != -1) {
            this.field_k = -104;
        }
        return this.field_d == 102 ? true : false;
    }

    public static void a(byte param0) {
        field_h = null;
        if (param0 != -4) {
            return;
        }
        field_e = null;
        field_c = null;
        field_a = null;
    }

    final static void a(int param0, int param1) {
        to.field_g = sj.field_cb[param0];
        if (param1 != -5) {
            return;
        }
        ec.field_r = lc.field_k[param0];
        ki.field_c = qh.field_R[param0];
    }

    final void a(int param0, byte param1) {
        L0: {
          this.field_d = 0;
          this.field_k = 0;
          if (this.field_g != 0) {
            break L0;
          } else {
            if (-97 != (mm.field_t ^ -1)) {
              break L0;
            } else {
              L1: {
                if ((this.field_b ^ -1) >= -1) {
                  this.field_b = this.field_i;
                  break L1;
                } else {
                  break L1;
                }
              }
              this.field_f = false;
              this.field_b = this.field_b - 1;
              break L0;
            }
          }
        }
        L2: {
          if (-1 == (this.field_g ^ -1)) {
            this.field_d = mm.field_t;
            break L2;
          } else {
            break L2;
          }
        }
        L3: {
          if (-1 != (this.field_g ^ -1)) {
            break L3;
          } else {
            if ((mm.field_t ^ -1) != -98) {
              break L3;
            } else {
              L4: {
                this.field_b = this.field_b + 1;
                if (this.field_i <= this.field_b) {
                  this.field_b = 0;
                  break L4;
                } else {
                  break L4;
                }
              }
              this.field_f = false;
              break L3;
            }
          }
        }
        if (param1 == -107) {
          L5: {
            if (-1 != (this.field_g ^ -1)) {
              break L5;
            } else {
              L6: {
                if (98 == mm.field_t) {
                  break L6;
                } else {
                  if (-100 == (mm.field_t ^ -1)) {
                    break L6;
                  } else {
                    break L5;
                  }
                }
              }
              L7: {
                if (-1 < (this.field_b ^ -1)) {
                  this.field_b = param0;
                  break L7;
                } else {
                  break L7;
                }
              }
              this.field_f = false;
              break L5;
            }
          }
          return;
        } else {
          return;
        }
    }

    final boolean b(byte param0) {
        int var2 = -93 / ((param0 - 59) / 62);
        return (this.field_g ^ -1) != -1 ? true : false;
    }

    final boolean c(byte param0) {
        int var2 = -70 % ((param0 - -30) / 56);
        return this.field_d == 97 ? true : false;
    }

    final boolean a(int param0) {
        if (param0 != 0) {
            this.d(43);
        }
        return this.field_d == 96 ? true : false;
    }

    final void a(boolean param0, boolean param1, int param2, int param3) {
        this.field_f = param1 ? true : false;
        this.field_g = 0;
        if (this.field_f) {
            this.field_b = param3;
        } else {
            this.field_b = param2;
        }
        if (param0) {
            this.c((byte) 33);
        }
    }

    final boolean a(boolean param0) {
        if (param0) {
            ae.a(53, 54);
        }
        return -104 == (this.field_d ^ -1) ? true : false;
    }

    final void b(int param0, int param1) {
        if (param1 != 3) {
            field_e = (String) null;
        }
        if (-1 == (this.field_g ^ -1)) {
            this.field_b = param0;
            this.field_f = false;
        }
    }

    final static void a(boolean param0, int param1, int param2) {
        int stackIn_32_0 = 0;
        int stackIn_35_0 = 0;
        String[] stackIn_42_0 = null;
        int stackIn_42_1 = 0;
        String[] stackIn_43_0 = null;
        int stackIn_43_1 = 0;
        String stackIn_43_2 = null;
        String[] stackIn_47_0 = null;
        int stackIn_47_1 = 0;
        int stackIn_47_2 = 0;
        int stackIn_47_3 = 0;
        String stackIn_50_2 = null;
        int stackIn_93_0 = 0;
        int stackIn_114_0 = 0;
        int stackIn_115_0 = 0;
        int stackIn_115_1 = 0;
        Throwable caughtException = null;
        RuntimeException decompiledCaughtException = null;
        int var3_int = 0;
        RuntimeException var3 = null;
        int var4 = 0;
        String[] var4_ref_String__ = null;
        int var5 = 0;
        int var6 = 0;
        int var7 = 0;
        int var8 = 0;
        int var9 = 0;
        String[] var10 = null;
        String var11 = null;
        String var12 = null;
        var9 = DungeonAssault.field_K;
        try {
          L0: {
            L1: {
              nk.field_Hb = true;
              kh.field_t = param1;
              if (kh.field_t != 0) {
                if (-2 == (kh.field_t ^ -1)) {
                  var3_int = og.a(n.field_c, (byte) -114, qj.field_e, wa.field_l, ql.field_k);
                  var4 = var3_int + 2;
                  to.field_c = new int[var4];
                  go.field_A = new String[var4];
                  var5 = 0;
                  L2: while (true) {
                    if (var4 <= var5) {
                      n.field_g = new int[1];
                      var5 = 0;
                      L3: while (true) {
                        if (var5 >= var3_int) {
                          go.field_A[-2 + var4] = "";
                          go.field_A[var4 + -1] = sp.field_b;
                          to.field_c[var4 - 1] = 0;
                          n.field_g[0] = 5;
                          break L1;
                        } else {
                          go.field_A[var5] = qj.field_e[var5];
                          var5++;
                          continue L3;
                        }
                      }
                    } else {
                      to.field_c[var5] = -1;
                      var5++;
                      continue L2;
                    }
                  }
                } else {
                  if (-3 != (kh.field_t ^ -1)) {
                    if (kh.field_t != 3) {
                      if ((kh.field_t ^ -1) != -5) {
                        if (-6 != (kh.field_t ^ -1)) {
                          throw new IllegalArgumentException();
                        } else {
                          var3_int = og.a(n.field_c, (byte) -68, qj.field_e, wa.field_l, id.field_g);
                          var4 = 3 + var3_int;
                          to.field_c = new int[var4];
                          go.field_A = new String[var4];
                          var5 = 0;
                          L4: while (true) {
                            if (var4 <= var5) {
                              n.field_g = new int[2];
                              var5 = 0;
                              L5: while (true) {
                                if (var3_int <= var5) {
                                  go.field_A[var4 - 3] = "";
                                  go.field_A[-2 + var4] = fc.field_j;
                                  to.field_c[var4 + -2] = 0;
                                  n.field_g[0] = 3;
                                  go.field_A[-1 + var4] = sp.field_b;
                                  to.field_c[-1 + var4] = 1;
                                  n.field_g[1] = 5;
                                  break L1;
                                } else {
                                  go.field_A[var5] = qj.field_e[var5];
                                  var5++;
                                  continue L5;
                                }
                              }
                            } else {
                              to.field_c[var5] = -1;
                              var5++;
                              continue L4;
                            }
                          }
                        }
                      } else {
                        var3_int = og.a(n.field_c, (byte) 101, qj.field_e, wa.field_l, jg.field_b);
                        var4 = var3_int + 2;
                        to.field_c = new int[var4];
                        go.field_A = new String[var4];
                        var5 = 0;
                        L6: while (true) {
                          if (var5 >= var4) {
                            n.field_g = new int[1];
                            var5 = 0;
                            L7: while (true) {
                              if (var5 >= var3_int) {
                                go.field_A[var4 + -2] = "";
                                go.field_A[var4 - 1] = sp.field_b;
                                to.field_c[var4 - 1] = 0;
                                n.field_g[0] = 5;
                                break L1;
                              } else {
                                go.field_A[var5] = qj.field_e[var5];
                                var5++;
                                continue L7;
                              }
                            }
                          } else {
                            to.field_c[var5] = -1;
                            var5++;
                            continue L6;
                          }
                        }
                      }
                    } else {
                      L8: {
                        if (i.field_A.field_j) {
                          var3_int = og.a(n.field_c, (byte) 120, qj.field_e, wa.field_l, fg.field_g);
                          break L8;
                        } else {
                          var3_int = og.a(n.field_c, (byte) -46, qj.field_e, wa.field_l, oh.field_a);
                          break L8;
                        }
                      }
                      var4 = 2 - -var3_int;
                      go.field_A = new String[var4];
                      to.field_c = new int[var4];
                      var5 = 0;
                      L9: while (true) {
                        if (var5 >= var4) {
                          n.field_g = new int[1];
                          var5 = 0;
                          L10: while (true) {
                            if (var3_int <= var5) {
                              go.field_A[var4 - 2] = "";
                              go.field_A[-1 + var4] = sp.field_b;
                              to.field_c[-1 + var4] = 0;
                              n.field_g[0] = 5;
                              break L1;
                            } else {
                              go.field_A[var5] = qj.field_e[var5];
                              var5++;
                              continue L10;
                            }
                          }
                        } else {
                          to.field_c[var5] = -1;
                          var5++;
                          continue L9;
                        }
                      }
                    }
                  } else {
                    var3_int = og.a(n.field_c, (byte) 125, qj.field_e, wa.field_l, jk.a(new String[]{"<br><%0><br>"}, ef.field_b, param2 ^ -30497));
                    var4 = -1;
                    var5 = 0;
                    L11: while (true) {
                      L12: {
                        if (var3_int <= var5) {
                          break L12;
                        } else {
                          if ("<%0>".equals(qj.field_e[var5])) {
                            var4 = var5;
                            break L12;
                          } else {
                            var5++;
                            continue L11;
                          }
                        }
                      }
                      if (var4 == -1) {
                        throw new IllegalStateException();
                      } else {
                        sn.field_a = new String[var4];
                        cj.a(qj.field_e, 0, sn.field_a, 0, var4);
                        cp.field_Q = new String[-1 + (-var4 + var3_int)];
                        cj.a(qj.field_e, var4 - -1, cp.field_Q, 0, var3_int - var4 - 1);
                        var4 = -1;
                        var3_int = og.a(n.field_c, (byte) 84, qj.field_e, wa.field_l, jk.a(new String[]{"<br><%0><br>"}, bn.field_i, param2 ^ -30497));
                        var5 = 0;
                        L13: while (true) {
                          L14: {
                            if (var5 >= var3_int) {
                              break L14;
                            } else {
                              if ("<%0>".equals(qj.field_e[var5])) {
                                var4 = var5;
                                break L14;
                              } else {
                                var5++;
                                continue L13;
                              }
                            }
                          }
                          if (var4 == -1) {
                            throw new IllegalStateException();
                          } else {
                            L15: {
                              mo.field_g = new String[var4];
                              cj.a(qj.field_e, 0, mo.field_g, 0, var4);
                              fd.field_b = new String[-1 + -var4 + var3_int];
                              cj.a(qj.field_e, var4 + 1, fd.field_b, 0, -1 + var3_int - var4);
                              if (sn.field_a.length < mo.field_g.length) {
                                stackIn_32_0 = mo.field_g.length;
                                break L15;
                              } else {
                                stackIn_32_0 = sn.field_a.length;
                                break L15;
                              }
                            }
                            L16: {
                              var5 = stackIn_32_0;
                              if (cp.field_Q.length < fd.field_b.length) {
                                stackIn_35_0 = fd.field_b.length;
                                break L16;
                              } else {
                                stackIn_35_0 = cp.field_Q.length;
                                break L16;
                              }
                            }
                            var6 = stackIn_35_0;
                            var7 = var6 + (7 + var5);
                            go.field_A = new String[var7];
                            to.field_c = new int[var7];
                            var8 = 0;
                            L17: while (true) {
                              if (var7 <= var8) {
                                to.field_c[1] = 0;
                                go.field_A[0] = mj.field_b;
                                n.field_g = new int[2];
                                go.field_A[1] = lg.field_l;
                                go.field_A[5] = "";
                                n.field_g[0] = 5;
                                go.field_A[2] = ef.field_c;
                                to.field_c[3] = 1;
                                go.field_A[3] = jg.field_d;
                                n.field_g[1] = 2;
                                go.field_A[4] = qp.field_e;
                                var8 = 0;
                                L18: while (true) {
                                  if (var8 >= var5) {
                                    go.field_A[6 - -var5] = null;
                                    to.field_c[6 - -var5] = -2;
                                    var8 = 0;
                                    L19: while (true) {
                                      if (var6 <= var8) {
                                        kg.field_d = kd.c(-2456);
                                        break L1;
                                      } else {
                                        stackIn_47_0 = go.field_A;
                                        stackIn_47_1 = 7 - (-var5 - var8);
                                        stackIn_47_2 = fd.field_b.length;
                                        stackIn_47_3 = var8;
                                        L20: {




                                          if (stackIn_47_2 <= stackIn_47_3) {
                                            stackIn_47_0 = (String[]) ((Object) stackIn_47_0);

                                            stackIn_50_2 = "";
                                            break L20;
                                          } else {
                                            stackIn_47_0 = (String[]) ((Object) stackIn_47_0);

                                            stackIn_50_2 = fd.field_b[var8];
                                            break L20;
                                          }
                                        }
                                        stackIn_47_0[stackIn_47_1] = stackIn_50_2;
                                        var8++;
                                        continue L19;
                                      }
                                    }
                                  } else {
                                    L21: {
                                      stackIn_42_0 = go.field_A;

                                      stackIn_42_1 = var8 + 6;

                                      if (-1 >= (mo.field_g.length + (var8 + -var5) ^ -1)) {
                                        stackIn_43_0 = (String[]) ((Object) stackIn_42_0);
                                        stackIn_43_1 = stackIn_42_1;
                                        stackIn_43_2 = mo.field_g[mo.field_g.length + var8 + -var5];
                                        break L21;
                                      } else {
                                        stackIn_43_0 = (String[]) ((Object) stackIn_42_0);
                                        stackIn_43_1 = stackIn_42_1;
                                        stackIn_43_2 = "";
                                        break L21;
                                      }
                                    }
                                    stackIn_43_0[stackIn_43_1] = stackIn_43_2;
                                    var8++;
                                    continue L18;
                                  }
                                }
                              } else {
                                to.field_c[var8] = -1;
                                var8++;
                                continue L17;
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              } else {
                var3_int = og.a(n.field_c, (byte) 96, qj.field_e, wa.field_l, ql.field_k);
                var4 = var3_int + 3;
                go.field_A = new String[var4];
                to.field_c = new int[var4];
                var5 = 0;
                L22: while (true) {
                  if (var5 >= var4) {
                    n.field_g = new int[2];
                    var5 = 0;
                    L23: while (true) {
                      if (var3_int <= var5) {
                        go.field_A[var4 + -3] = "";
                        go.field_A[var4 - 2] = mn.field_a;
                        to.field_c[var4 - 2] = 0;
                        n.field_g[0] = 4;
                        go.field_A[-1 + var4] = sp.field_b;
                        to.field_c[var4 + -1] = 1;
                        n.field_g[1] = 5;
                        break L1;
                      } else {
                        go.field_A[var5] = qj.field_e[var5];
                        var5++;
                        continue L23;
                      }
                    }
                  } else {
                    to.field_c[var5] = -1;
                    var5++;
                    continue L22;
                  }
                }
              }
            }
            L24: {
              d.field_g.field_i = n.field_g.length;
              var3_int = 0;
              var4 = 0;
              if (param2 == -1) {
                break L24;
              } else {
                field_e = (String) null;
                break L24;
              }
            }
            L25: while (true) {
              if (var4 >= go.field_A.length) {
                L26: {
                  if ((kh.field_t ^ -1) != -3) {
                    break L26;
                  } else {
                    var10 = sn.field_a;
                    var4_ref_String__ = var10;
                    var5 = 0;
                    L27: while (true) {
                      if (var10.length <= var5) {
                        var4_ref_String__ = cp.field_Q;
                        var5 = 0;
                        L28: while (true) {
                          if (var5 >= var4_ref_String__.length) {
                            break L26;
                          } else {
                            L29: {
                              var12 = var4_ref_String__[var5];
                              var7 = og.a(false, var12, -30633);
                              if ((var3_int ^ -1) > (var7 ^ -1)) {
                                var3_int = var7;
                                break L29;
                              } else {
                                break L29;
                              }
                            }
                            var5++;
                            continue L28;
                          }
                        }
                      } else {
                        L30: {
                          var11 = var10[var5];
                          var7 = og.a(false, var11, param2 + -30632);
                          if ((var7 ^ -1) < (var3_int ^ -1)) {
                            var3_int = var7;
                            break L30;
                          } else {
                            break L30;
                          }
                        }
                        var5++;
                        continue L27;
                      }
                    }
                  }
                }
                d.field_e = vo.field_r - (-var3_int - -(var3_int >> 1615599265));
                jn.field_d = (ci.field_j + h.field_q << -1293823903) * d.field_g.field_i;
                bf.field_b = -(var3_int >> 1545475361) + vo.field_r;
                var4 = 0;
                L31: while (true) {
                  if (go.field_A.length <= var4) {
                    jm.field_c = -(jn.field_d >> 1038824001) + bl.field_p;
                    dg.field_L = new int[go.field_A.length][];
                    var4 = 0;
                    var5 = jm.field_c;
                    L32: while (true) {
                      if (go.field_A.length <= var4) {
                        L33: {
                          if (kh.field_t == 2) {
                            d.field_g.a(false, param0, -1, -1);
                            break L33;
                          } else {
                            d.field_g.a(false, param0, 0, ab.a(-776726687, hj.field_S, eh.field_h));
                            break L33;
                          }
                        }
                        break L0;
                      } else {
                        L34: {
                          var6 = to.field_c[var4];
                          if (0 <= var6) {
                            var7 = og.a(true, go.field_A[var4], param2 ^ 30632);
                            var8 = -(var7 >> 1335616385) + vo.field_r;
                            var5 = var5 + ci.field_j;
                            dg.field_L[var4] = new int[4];
                            dg.field_L[var4][0] = -oh.field_t + var8;
                            dg.field_L[var4][1] = var5;
                            dg.field_L[var4][2] = var7 + (oh.field_t << -818253023);
                            var5 = var5 + (mj.field_g + ((h.field_q << 1815351009) + ci.field_j));
                            dg.field_L[var4][3] = (h.field_q << 1805199777) + mj.field_g;
                            break L34;
                          } else {
                            var5 = var5 + ag.field_a;
                            break L34;
                          }
                        }
                        var4++;
                        continue L32;
                      }
                    }
                  } else {
                    L35: {
                      stackIn_114_0 = jn.field_d;

                      if ((to.field_c[var4] ^ -1) > -1) {
                        stackIn_115_0 = stackIn_114_0;
                        stackIn_115_1 = ag.field_a;
                        break L35;
                      } else {
                        stackIn_115_0 = stackIn_114_0;
                        stackIn_115_1 = mj.field_g;
                        break L35;
                      }
                    }
                    jn.field_d = stackIn_115_0 + stackIn_115_1;
                    var4++;
                    continue L31;
                  }
                }
              } else {
                L36: {
                  if (to.field_c[var4] < 0) {
                    stackIn_93_0 = 0;
                    break L36;
                  } else {
                    stackIn_93_0 = 1;
                    break L36;
                  }
                }
                L37: {
                  var5 = og.a(stackIn_93_0 != 0, go.field_A[var4], param2 ^ 30632);
                  if (var5 > var3_int) {
                    var3_int = var5;
                    break L37;
                  } else {
                    break L37;
                  }
                }
                var4++;
                continue L25;
              }
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          var3 = decompiledCaughtException;
          throw vk.a((Throwable) ((Object) var3), "ae.D(" + param0 + ',' + param1 + ',' + param2 + ')');
        }
    }

    final void e(int param0) {
        this.field_d = 0;
        if (param0 < 88) {
            this.field_g = -80;
        }
        this.field_k = 0;
        if (!(this.field_g != 0)) {
            this.field_d = mm.field_t;
        }
    }

    final void a(int param0, int param1, int param2) {
        if (param2 >= this.field_i) {
          throw new IllegalArgumentException();
        } else {
          L0: {
            if (param0 == -7734) {
              break L0;
            } else {
              this.field_f = true;
              break L0;
            }
          }
          if (this.field_i > param1) {
            L1: {
              this.field_d = 0;
              this.field_k = 0;
              if (0 != no.field_d) {
                this.field_g = no.field_d;
                this.field_f = true;
                this.field_b = param1;
                this.field_k = no.field_d;
                this.field_j = e.field_e;
                break L1;
              } else {
                break L1;
              }
            }
            L2: {
              if (this.field_g == 0) {
                if (no.field_d != 0) {
                  break L2;
                } else {
                  if (la.field_g != 0) {
                    break L2;
                  } else {
                    this.field_g = 0;
                    break L2;
                  }
                }
              } else {
                if (no.field_d != 0) {
                  break L2;
                } else {
                  if (la.field_g != 0) {
                    break L2;
                  } else {
                    this.field_g = 0;
                    break L2;
                  }
                }
              }
            }
            L3: {
              if (this.field_g != 0) {
                break L3;
              } else {
                L4: {
                  if (this.field_f) {
                    break L4;
                  } else {
                    if (!cp.field_U) {
                      break L3;
                    } else {
                      break L4;
                    }
                  }
                }
                if ((param2 ^ -1) > -1) {
                  if (this.field_f) {
                    this.field_b = -1;
                    break L3;
                  } else {
                    break L3;
                  }
                } else {
                  L5: {
                    if (this.field_b == param2) {
                      break L5;
                    } else {
                      break L5;
                    }
                  }
                  this.field_f = true;
                  this.field_b = param2;
                  break L3;
                }
              }
            }
            return;
          } else {
            throw new IllegalArgumentException();
          }
        }
    }

    final boolean c(int param0) {
        int var2;
        int stackIn_6_0 = 0;
        L0: {
          L1: {
            var2 = -24 / ((param0 - -29) / 40);
            if (this.field_k != 0) {
              break L1;
            } else {
              if (-85 == (this.field_d ^ -1)) {
                break L1;
              } else {
                if (83 != this.field_d) {
                  stackIn_6_0 = 0;
                  break L0;
                } else {
                  break L1;
                }
              }
            }
          }
          stackIn_6_0 = 1;
          break L0;
        }
        return stackIn_6_0 != 0;
    }

    ae(int param0) {
        this.field_b = 0;
        this.field_f = false;
        this.field_i = param0;
    }

    static {
        field_h = "By clicking Create, you agree to the <%0><hotspot=0>Terms of Use</hotspot><%1> and <%0><hotspot=1>Privacy Policy</hotspot><%1>.";
        field_c = "Dwarf Renegade";
        field_e = "HEAL";
    }
}
