/*
 * Decompiled by CFR-JS 0.4.0.
 */
abstract class in extends kg {
    private int field_r;
    String field_p;
    private int field_q;
    static og field_C;
    private int field_z;
    private boolean field_u;
    private int field_F;
    static boolean field_E;
    private int field_x;
    private boolean field_s;
    private long field_A;
    static pp field_y;
    private boolean field_v;
    private int field_w;
    static cn field_D;
    static int[] field_B;
    static String field_t;

    private final void a(String param0, int param1, boolean param2, int param3, int param4) {
        Object stackIn_8_0 = null;
        Object stackIn_9_0 = null;
        Object stackIn_10_0 = null;
        int stackIn_10_1 = 0;
        RuntimeException stackIn_27_0 = null;
        StringBuilder stackIn_27_1 = null;
        RuntimeException stackIn_28_0 = null;
        StringBuilder stackIn_28_1 = null;
        String stackIn_28_2 = null;
        int decompiledRegionSelector0 = 0;
        int decompiledRegionSelector1 = 0;
        Throwable decompiledCaughtException = null;
        Exception var6 = null;
        RuntimeException var6_ref = null;
        String var7 = null;
        String var8 = null;
        String var9 = null;
        String var10 = null;
        String var11 = null;
        try {
          L0: {
            try {
              L1: {
                if (!this.b(-126)) {
                  decompiledRegionSelector0 = 0;
                  break L1;
                } else {
                  this.field_p = this.getCodeBase().getHost();
                  var11 = this.field_p.toLowerCase();
                  if (param2) {
                    L2: {
                      L3: {
                        stackIn_8_0 = this;

                        if (var11.equals("jagex.com")) {
                          break L3;
                        } else {
                          stackIn_9_0 = this;

                          if (!var11.endsWith(".jagex.com")) {
                            stackIn_10_0 = this;
                            stackIn_10_1 = 0;
                            break L2;
                          } else {
                            stackIn_8_0 = this;
                            break L3;
                          }
                        }
                      }
                      stackIn_10_0 = this;
                      stackIn_10_1 = 1;
                      break L2;
                    }
                    L4: {
                      ((in) (this)).field_u = stackIn_10_1 != 0;
                      this.field_z = Integer.parseInt(this.getParameter("gameport1"));
                      this.field_F = Integer.parseInt(this.getParameter("gameport2"));
                      var7 = this.getParameter("servernum");
                      if (var7 != null) {
                        this.field_r = Integer.parseInt(var7);
                        break L4;
                      } else {
                        break L4;
                      }
                    }
                    L5: {
                      this.field_w = Integer.parseInt(this.getParameter("gamecrc"));
                      this.field_A = Long.parseLong(this.getParameter("instanceid"));
                      this.field_s = this.getParameter("member").equals("yes");
                      var8 = this.getParameter("lang");
                      if (var8 != null) {
                        this.field_x = Integer.parseInt(var8);
                        break L5;
                      } else {
                        break L5;
                      }
                    }
                    L6: {
                      if (this.field_x < 5) {
                        break L6;
                      } else {
                        this.field_x = 0;
                        break L6;
                      }
                    }
                    L7: {
                      var9 = this.getParameter("affid");
                      if (var9 != null) {
                        this.field_q = Integer.parseInt(var9);
                        break L7;
                      } else {
                        break L7;
                      }
                    }
                    f.field_d = Boolean.valueOf(this.getParameter("simplemode")).booleanValue();
                    this.a(32, (byte) 107, param0, param4, param1, param3, this.field_w);
                    decompiledRegionSelector0 = 2;
                    break L1;
                  } else {
                    decompiledRegionSelector0 = 1;
                    break L1;
                  }
                }
              }
            } catch (java.lang.Exception decompiledCaughtParameter0) {
              decompiledCaughtException = decompiledCaughtParameter0;
              L8: {
                var6 = (Exception) (Object) decompiledCaughtException;
                var10 = (String) null;
                sm.a((Throwable) ((Object) var6), 1, (String) null);
                this.a((byte) -112, "crash");
                decompiledRegionSelector0 = 2;
                break L8;
              }
            }
            if (decompiledRegionSelector0 == 0) {
              decompiledRegionSelector1 = 0;
              break L0;
            } else {
              if (decompiledRegionSelector0 == 1) {
                decompiledRegionSelector1 = 1;
                break L0;
              } else {
                decompiledRegionSelector1 = 2;
                break L0;
              }
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter1) {
          decompiledCaughtException = decompiledCaughtParameter1;
          L9: {
            var6_ref = (RuntimeException) (Object) decompiledCaughtException;
            stackIn_27_0 = (RuntimeException) (var6_ref);

            stackIn_27_1 = new StringBuilder().append("in.Q(");

            if (param0 == null) {
              stackIn_28_0 = (RuntimeException) ((Object) stackIn_27_0);
              stackIn_28_1 = (StringBuilder) ((Object) stackIn_27_1);
              stackIn_28_2 = "null";
              break L9;
            } else {
              stackIn_28_0 = (RuntimeException) ((Object) stackIn_27_0);
              stackIn_28_1 = (StringBuilder) ((Object) stackIn_27_1);
              stackIn_28_2 = "{...}";
              break L9;
            }
          }
          throw vk.a((Throwable) ((Object) stackIn_28_0), stackIn_28_2 + ',' + param1 + ',' + param2 + ',' + param3 + ',' + param4 + ')');
        }
        if (decompiledRegionSelector1 == 0) {
          return;
        } else {
          if (decompiledRegionSelector1 == 1) {
            return;
          } else {
            return;
          }
        }
    }

    final void g(byte param0) {
        int discarded$0 = 0;
        int discarded$1 = 0;
        Object stackIn_4_0 = null;
        int stackIn_4_1 = 0;
        Object stackIn_5_0 = null;
        int stackIn_5_1 = 0;
        int stackIn_5_2 = 0;
        int var3;
        var3 = DungeonAssault.field_K;
        if (param0 != 2) {
          return;
        } else {
          L0: {
            if (!vk.a(22084)) {
              if (dk.field_c < 10) {
                break L0;
              } else {
                if (!l.a(true)) {
                  dh.b(-128);
                  break L0;
                } else {
                  if (kh.field_n != 0) {
                    td.a(pj.field_r, param0 + 119);
                    break L0;
                  } else {
                    discarded$0 = this.a(false, false, false);
                    break L0;
                  }
                }
              }
            } else {
              L1: {
                stackIn_4_0 = this;

                stackIn_4_1 = 0;

                if (null == cm.field_R) {
                  stackIn_5_0 = this;
                  stackIn_5_1 = stackIn_4_1;
                  stackIn_5_2 = 0;
                  break L1;
                } else {
                  stackIn_5_0 = this;
                  stackIn_5_1 = stackIn_4_1;
                  stackIn_5_2 = 1;
                  break L1;
                }
              }
              discarded$1 = this.a(stackIn_5_1 != 0, stackIn_5_2 != 0, false);
              break L0;
            }
          }
          return;
        }
    }

    public static void e(int param0) {
        field_C = null;
        field_t = null;
        field_D = null;
        field_y = null;
        field_B = null;
        if (param0 != -1) {
            in.e(-36);
        }
    }

    final static int a(int param0, int param1) {
        param0 = (1431655765 & param0 >>> 449097697) + (param0 & 1431655765);
        if (param1 != 8) {
            in.e(-51);
        }
        param0 = (param0 >>> -1538636638 & -1288490189) + (param0 & 858993459);
        param0 = param0 - -(param0 >>> 627652708) & 252645135;
        param0 = param0 + (param0 >>> 819635336);
        param0 = param0 + (param0 >>> -823313680);
        return 255 & param0;
    }

    final int b(byte param0, boolean param1) {
        int discarded$0 = 0;
        if (param0 != 82) {
            discarded$0 = this.g(12);
        }
        return this.a(false, param1, true);
    }

    final void a(boolean param0, int param1, int param2, int param3, int param4, int param5, int param6, int param7) {
        java.awt.Frame var10 = new java.awt.Frame("Jagex");
        var10.pack();
        var10.dispose();
        this.setBackground(java.awt.Color.black);
        hc.field_d = this.field_x;
        ae.a(hc.field_d, -5);
        dj.a(param1, 5000, this.field_A, this.field_z, this.field_p, this.field_w, hc.field_d, this.field_F, this.field_r, this.field_s, param6, param0, (byte) -27, i.field_A);
        an.a(this.field_r, this.field_F, -69, this.field_z, this.field_p, hc.field_d, this.field_w, param1, i.field_A);
        uf.a(param6 ^ 5031);
        bn.field_j = pm.a((byte) 109);
        uk.a(c.field_h, (byte) -126);
        mh.field_a = param5;
        s.field_a = param7;
        bh.field_P = param2;
        pm.field_m = param3;
        sp.field_h = param4;
        this.e((byte) 68);
        ch.a((byte) -110);
    }

    final int f(byte param0) {
        int discarded$0 = 0;
        int var2 = this.g(11);
        if (param0 < 86) {
            discarded$0 = this.g(-68);
        }
        if (-1 == (var2 ^ -1) || (var2 ^ -1) == -2) {
            if (!(!pj.field_w[1])) {
                f.a(2, (byte) -50);
            }
            if (!(!pj.field_w[2])) {
                fl.a((byte) -93, 3);
            }
            if (pj.field_w[3]) {
                ep.a((byte) 127, 4);
            }
            if (!(!pj.field_w[4])) {
                ko.a(true, 5);
            }
            if (!(!pj.field_w[5])) {
                ol.a(8, 6);
            }
            if (!(!pj.field_w[6])) {
                tc.a(7, (byte) -112);
            }
            if (pj.field_w[8]) {
                hj.g(-59);
            }
        }
        return var2;
    }

    private final void e(byte param0) {
        uo.field_c[2] = -2;
        uo.field_c[13] = -1;
        uo.field_c[5] = -1;
        uo.field_c[7] = -1;
        uo.field_c[11] = -1;
        uo.field_c[10] = -1;
        uo.field_c[12] = -1;
        uo.field_c[16] = -1;
        uo.field_c[1] = 16;
        uo.field_c[9] = -1;
        uo.field_c[18] = 1;
        if (param0 != 68) {
            this.field_A = -61L;
        }
        uo.field_c[17] = -1;
        uo.field_c[8] = -2;
        uo.field_c[6] = -2;
        uo.field_c[3] = -1;
        uo.field_c[4] = -1;
    }

    final static void a(int param0, int param1, int param2, int param3, int param4) {
        int stackIn_4_0 = 0;
        int stackIn_7_0 = 0;
        int stackIn_10_0 = 0;
        int stackIn_13_0 = 0;
        int decompiledRegionSelector0 = 0;
        RuntimeException decompiledCaughtException = null;
        int var5_int = 0;
        RuntimeException var5 = null;
        int var6 = 0;
        int var7 = 0;
        int var8 = 0;
        int var9 = 0;
        int var10 = 0;
        int var11 = 0;
        int var12 = 0;
        int var13 = 0;
        var13 = DungeonAssault.field_K;
        try {
          L0: {
            L1: {
              var5_int = param2 + param0;
              var6 = param3 + param1;
              if (gf.field_j >= param0) {
                stackIn_4_0 = gf.field_j;
                break L1;
              } else {
                stackIn_4_0 = param0;
                break L1;
              }
            }
            L2: {
              var7 = stackIn_4_0;
              if (param3 > gf.field_f) {
                stackIn_7_0 = param3;
                break L2;
              } else {
                stackIn_7_0 = gf.field_f;
                break L2;
              }
            }
            L3: {
              var8 = stackIn_7_0;
              if (gf.field_h > var5_int) {
                stackIn_10_0 = var5_int;
                break L3;
              } else {
                stackIn_10_0 = gf.field_h;
                break L3;
              }
            }
            L4: {
              var9 = stackIn_10_0;
              if (var6 >= gf.field_e) {
                stackIn_13_0 = gf.field_e;
                break L4;
              } else {
                stackIn_13_0 = var6;
                break L4;
              }
            }
            L5: {
              var10 = stackIn_13_0;
              if (gf.field_j > param0) {
                break L5;
              } else {
                if (gf.field_h > param0) {
                  var11 = var8 * gf.field_i + param0;
                  var12 = var10 + (1 - var8) >> 70561761;
                  L6: while (true) {
                    var12--;
                    if (0 > var12) {
                      break L5;
                    } else {
                      gf.field_b[var11] = 16777215;
                      var11 = var11 + 2 * gf.field_i;
                      continue L6;
                    }
                  }
                } else {
                  break L5;
                }
              }
            }
            if (param4 == 1) {
              L7: {
                if (gf.field_f > param3) {
                  break L7;
                } else {
                  if (gf.field_e <= var6) {
                    break L7;
                  } else {
                    var11 = param3 * gf.field_i + var7;
                    var12 = -var7 + 1 + var9 >> 1879008897;
                    L8: while (true) {
                      var12--;
                      if ((var12 ^ -1) > -1) {
                        break L7;
                      } else {
                        gf.field_b[var11] = 16777215;
                        var11 += 2;
                        continue L8;
                      }
                    }
                  }
                }
              }
              L9: {
                if (gf.field_j > var5_int) {
                  break L9;
                } else {
                  if (var5_int >= gf.field_h) {
                    break L9;
                  } else {
                    var11 = (var8 + (1 & var5_int + -param0)) * gf.field_i - -var5_int;
                    var12 = var10 + 1 - var8 >> -454410015;
                    L10: while (true) {
                      var12--;
                      if ((var12 ^ -1) > -1) {
                        break L9;
                      } else {
                        gf.field_b[var11] = 16777215;
                        var11 = var11 + 2 * gf.field_i;
                        continue L10;
                      }
                    }
                  }
                }
              }
              L11: {
                if (gf.field_f > param3) {
                  break L11;
                } else {
                  if (gf.field_e <= var6) {
                    break L11;
                  } else {
                    var11 = var7 + var6 * gf.field_i - -(-param3 + var6 & 1);
                    var12 = -var7 + (var9 + 1) >> 757868257;
                    L12: while (true) {
                      var12--;
                      if ((var12 ^ -1) > -1) {
                        break L11;
                      } else {
                        gf.field_b[var11] = 16777215;
                        var11 += 2;
                        continue L12;
                      }
                    }
                  }
                }
              }
              decompiledRegionSelector0 = 1;
              break L0;
            } else {
              decompiledRegionSelector0 = 0;
              break L0;
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          var5 = decompiledCaughtException;
          throw vk.a((Throwable) ((Object) var5), "in.FA(" + param0 + ',' + param1 + ',' + param2 + ',' + param3 + ',' + param4 + ')');
        }
        if (decompiledRegionSelector0 == 0) {
          return;
        } else {
          return;
        }
    }

    final void d(boolean param0) {
        int stackIn_16_0 = 0;
        int stackIn_32_0 = 0;
        int var2;
        la var3;
        int var4;
        L0: {
          var4 = DungeonAssault.field_K;
          var2 = hc.field_c;
          if (var2 >= 64) {
            break L0;
          } else {
            if (pj.field_w[var2]) {
              L1: {
                if (param0) {
                  break L1;
                } else {
                  this.field_s = false;
                  break L1;
                }
              }
              if (0 == var2) {
                return;
              } else {
                L2: {
                  if (-2 != (var2 ^ -1)) {
                    if (var2 == 2) {
                      ui.a(30592);
                      break L2;
                    } else {
                      if (-4 != (var2 ^ -1)) {
                        if (var2 != 4) {
                          if (-6 != (var2 ^ -1)) {
                            if (6 == var2) {
                              bf.b(19664);
                              break L2;
                            } else {
                              if (-8 != (var2 ^ -1)) {
                                if (-9 != (var2 ^ -1)) {
                                  if ((var2 ^ -1) != -17) {
                                    L3: {
                                      if (-12 == (var2 ^ -1)) {
                                        break L3;
                                      } else {
                                        if (-13 != (var2 ^ -1)) {
                                          if ((var2 ^ -1) == -14) {
                                            lf.d((byte) 123);
                                            break L2;
                                          } else {
                                            if (17 != var2) {
                                              if (18 != var2) {
                                                sm.a((Throwable) null, 1, "MGS1: " + sj.l(192));
                                                tl.a(124);
                                                break L2;
                                              } else {
                                                rc.c(16777215);
                                                break L2;
                                              }
                                            } else {
                                              this.d(-28859);
                                              break L2;
                                            }
                                          }
                                        } else {
                                          break L3;
                                        }
                                      }
                                    }
                                    L4: {
                                      if (var2 != 12) {
                                        stackIn_32_0 = 0;
                                        break L4;
                                      } else {
                                        stackIn_32_0 = 1;
                                        break L4;
                                      }
                                    }
                                    var3 = d.a(stackIn_32_0 != 0, (byte) 118);
                                    jm.a(var3, (byte) 116);
                                    break L2;
                                  } else {
                                    ih.a((byte) 59);
                                    break L2;
                                  }
                                } else {
                                  hb.a(-5, ra.field_c, i.field_A, nj.field_b);
                                  break L2;
                                }
                              } else {
                                this.f(8);
                                break L2;
                              }
                            }
                          } else {
                            rm.q(-5705);
                            break L2;
                          }
                        } else {
                          L5: {
                            if (param0) {
                              stackIn_16_0 = 0;
                              break L5;
                            } else {
                              stackIn_16_0 = 1;
                              break L5;
                            }
                          }
                          hd.a(stackIn_16_0 != 0);
                          break L2;
                        }
                      } else {
                        pk.d(126);
                        break L2;
                      }
                    }
                  } else {
                    lg.a(72);
                    break L2;
                  }
                }
                return;
              }
            } else {
              break L0;
            }
          }
        }
        sm.a((Throwable) null, 1, "MGS2: " + sj.l(192));
        tl.a(-48);
    }

    private final void d(int param0) {
        if (param0 != -28859) {
            this.f((byte) 39);
        }
        int var2 = ra.field_c.c(true);
        int var3 = -1 != (var2 & 1 ^ -1) ? 1 : 0;
        int var4 = nj.field_b - 1;
        byte[] var5 = new byte[var4];
        ra.field_c.a(0, (byte) 95, var4, var5);
        oe.a((byte) 121, var3 != 0, m.c(true), dj.a(var5, false));
    }

    private final int a(boolean param0, boolean param1, boolean param2) {
        try {
            int discarded$0 = 0;
            boolean stackIn_7_0 = false;
            cd stackIn_7_1 = null;
            boolean stackIn_8_0 = false;
            cd stackIn_8_1 = null;
            int stackIn_8_2 = 0;
            int stackIn_47_0 = 0;
            int stackIn_48_0 = 0;
            int stackIn_48_1 = 0;
            Throwable decompiledCaughtException = null;
            int var4 = 0;
            int var5_int = 0;
            Exception var5 = null;
            String var7 = null;
            int var8 = 0;
            String var9 = null;
            String var10 = null;
            Boolean var11 = null;
            ec var12 = null;
            L0: {
              var8 = DungeonAssault.field_K;
              if (!param0) {
                break L0;
              } else {
                discarded$0 = this.a(false, false, false);
                break L0;
              }
            }
            var4 = o.a(16711680, hc.field_d, param1, pj.field_r);
            if (-1 != (var4 ^ -1)) {
              L1: {
                if (var4 != 1) {
                  break L1;
                } else {
                  L2: {
                    stackIn_7_0 = param0;

                    stackIn_7_1 = ln.a((byte) -120);

                    if (param0) {
                      stackIn_8_0 = stackIn_7_0;
                      stackIn_8_1 = (cd) ((Object) stackIn_7_1);
                      stackIn_8_2 = 0;
                      break L2;
                    } else {
                      stackIn_8_0 = stackIn_7_0;
                      stackIn_8_1 = (cd) ((Object) stackIn_7_1);
                      stackIn_8_2 = 1;
                      break L2;
                    }
                  }
                  L3: {
                    var5_int = qh.a(stackIn_8_0, stackIn_8_1, om.a(stackIn_8_2 != 0));
                    if (-1 != var5_int) {
                      ri.a(tp.field_v, mf.field_c, var5_int, -15);
                      mf.field_c = null;
                      tp.field_v = null;
                      break L3;
                    } else {
                      break L3;
                    }
                  }
                  var11 = fb.a((byte) 92);
                  if (var11 != null) {
                    ig.a(-99, var11.booleanValue());
                    break L1;
                  } else {
                    break L1;
                  }
                }
              }
              L4: {
                if ((var4 ^ -1) != -3) {
                  break L4;
                } else {
                  var5_int = qm.a(this.field_q, rn.a(-87), tp.c((byte) -73), em.e(7142), vl.c(-29455), 57, co.e((byte) 57));
                  if (-1 == var5_int) {
                    break L4;
                  } else {
                    gj.a(var5_int, 29120, mf.field_c, tp.field_v);
                    tp.field_v = null;
                    mf.field_c = null;
                    break L4;
                  }
                }
              }
              L5: {
                if (3 == var4) {
                  L6: {
                    if (gm.field_c == -1) {
                      break L6;
                    } else {
                      if (-1 == (gm.field_c ^ -1)) {
                        break L6;
                      } else {
                        gm.field_c = -1;
                        en.a((byte) -54);
                        break L6;
                      }
                    }
                  }
                  if (param2) {
                    hh.field_c = false;
                    break L5;
                  } else {
                    var5_int = bh.a(this.field_u, em.e(7142), 106, this.field_q, false, vl.c(-29455));
                    if (var5_int != -1) {
                      if ((var5_int ^ -1) == -1) {
                        ne.field_g = hg.field_s;
                        bl.b((byte) -97);
                        kh.field_n = 10;
                        tl.field_l = false;
                        break L5;
                      } else {
                        nf.a(var5_int, true, tp.field_v);
                        tp.field_v = null;
                        break L5;
                      }
                    } else {
                      break L5;
                    }
                  }
                } else {
                  break L5;
                }
              }
              L7: {
                if (4 == var4) {
                  if (!ld.field_g) {
                    tl.field_l = true;
                    kh.field_n = 10;
                    break L7;
                  } else {
                    pd.a(5189, m.c(true));
                    break L7;
                  }
                } else {
                  break L7;
                }
              }
              L8: {
                if (-6 != (var4 ^ -1)) {
                  break L8;
                } else {
                  eg.a((byte) -37, m.c(true));
                  break L8;
                }
              }
              L9: {
                if (6 != var4) {
                  break L9;
                } else {
                  if (oj.field_b) {
                    kh.field_n = 10;
                    break L9;
                  } else {
                    break L9;
                  }
                }
              }
              L10: {
                if ((var4 ^ -1) == -8) {
                  pa.a(123, m.c(true));
                  break L10;
                } else {
                  break L10;
                }
              }
              L11: {
                if (8 == var4) {
                  L12: {
                    stackIn_47_0 = 5189;

                    if (param0) {
                      stackIn_48_0 = stackIn_47_0;
                      stackIn_48_1 = 0;
                      break L12;
                    } else {
                      stackIn_48_0 = stackIn_47_0;
                      stackIn_48_1 = 1;
                      break L12;
                    }
                  }
                  pd.a(stackIn_48_0, m.c(stackIn_48_1 != 0));
                  break L11;
                } else {
                  break L11;
                }
              }
              L13: {
                if (9 != var4) {
                  break L13;
                } else {
                  jl.a(m.c(true), false);
                  break L13;
                }
              }
              L14: {
                if (10 == var4) {
                  ac.field_B.d((byte) -90, 17);
                  break L14;
                } else {
                  break L14;
                }
              }
              L15: {
                if ((var4 ^ -1) == -12) {
                  fk.a(84, m.c(true));
                  break L15;
                } else {
                  break L15;
                }
              }
              L16: {
                if ((var4 ^ -1) != -13) {
                  break L16;
                } else {
                  oe.a(ao.a(-21), m.c(true), (byte) 94);
                  break L16;
                }
              }
              L17: {
                if (13 != var4) {
                  break L17;
                } else {
                  try {
                    L18: {
                      L19: {
                        if (null != hl.field_i) {
                          break L19;
                        } else {
                          hl.field_i = new tl(i.field_A, new java.net.URL(this.getCodeBase(), "countrylist.ws"), 5000);
                          break L19;
                        }
                      }
                      L20: {
                        if (!hl.field_i.d(34)) {
                          break L20;
                        } else {
                          L21: {
                            var12 = hl.field_i.c(3);
                            if (var12 == null) {
                              var9 = (String) null;
                              rg.a(15, (String) null);
                              break L21;
                            } else {
                              var7 = mk.a((byte) -24, var12.field_m, var12.field_o, 0);
                              rg.a(15, var7);
                              break L21;
                            }
                          }
                          hl.field_i = null;
                          break L20;
                        }
                      }
                      break L18;
                    }
                  } catch (java.lang.Exception decompiledCaughtParameter0) {
                    decompiledCaughtException = decompiledCaughtParameter0;
                    L22: {
                      var5 = (Exception) (Object) decompiledCaughtException;
                      sm.a((Throwable) ((Object) var5), 1, "S1");
                      var10 = (String) null;
                      rg.a(15, (String) null);
                      hl.field_i = null;
                      break L22;
                    }
                  }
                  break L17;
                }
              }
              L23: {
                if ((var4 ^ -1) == -16) {
                  kh.field_n = 10;
                  break L23;
                } else {
                  break L23;
                }
              }
              if (-17 == (var4 ^ -1)) {
                return 1;
              } else {
                if (17 != var4) {
                  return 0;
                } else {
                  return 2;
                }
              }
            } else {
              throw new IllegalStateException();
            }
        } catch (RuntimeException | Error decompiledUncheckedException) {
            throw decompiledUncheckedException;
        } catch (Throwable decompiledCheckedException) {
            throw new RuntimeException(decompiledCheckedException);
        }
    }

    private final void f(int param0) {
        if (param0 != 8) {
            return;
        }
        String var2 = eg.a(-96);
        sm.a(m.c(true), 31500, var2);
    }

    final void a(int param0, String param1, int param2) {
        if (param2 != 0) {
            return;
        }
        try {
            this.a(param1, 640, true, param0, 480);
        } catch (RuntimeException runtimeException) {
            throw vk.a((Throwable) ((Object) runtimeException), "in.W(" + param0 + ',' + (param1 != null ? "{...}" : "null") + ',' + param2 + ')');
        }
    }

    protected in() {
    }

    final void a(boolean param0, byte param1) {
        int stackIn_26_0 = 0;
        int stackIn_102_0 = 0;
        int stackIn_103_0 = 0;
        int stackIn_103_1 = 0;
        String stackIn_105_2 = null;
        String stackIn_106_2;
        int stackIn_106_3;
        int stackIn_135_0 = 0;
        int var3;
        java.awt.Dimension var4;
        int var5;
        java.awt.Container var6;
        L0: {
          var5 = DungeonAssault.field_K;
          if (null != il.field_c) {
            L1: {
              if (cm.field_R == null) {
                var6 = ua.a(0);
                var4 = var6.getSize();
                il.field_c.a(var4.height, true, var4.width);
                break L1;
              } else {
                break L1;
              }
            }
            il.field_c.e(6731);
            break L0;
          } else {
            break L0;
          }
        }
        L2: {
          bp.b(94);
          ke.b(param1 + 74);
          if (param1 == -74) {
            break L2;
          } else {
            this.field_q = -128;
            break L2;
          }
        }
        L3: {
          if (ao.a((byte) -116)) {
            break L3;
          } else {
            if ((kh.field_n ^ -1) != -12) {
              cp.f(-30208);
              break L3;
            } else {
              break L3;
            }
          }
        }
        L4: {
          if (bn.field_j == null) {
            break L4;
          } else {
            pj.field_r = bn.field_j.a(6);
            break L4;
          }
        }
        L5: {
          if (!dh.a((byte) -120)) {
            break L5;
          } else {
            L6: {
              var3 = 1200 * eg.c(1);
              if (this.field_v) {
                break L6;
              } else {
                if (ib.b((byte) 31) <= var3) {
                  break L5;
                } else {
                  if ((var3 ^ -1) <= (mn.a((byte) 106) ^ -1)) {
                    break L5;
                  } else {
                    break L6;
                  }
                }
              }
            }
            this.field_v = false;
            tl.a(122);
            l.a(param1 + 73);
            nf.a(2, true, sj.field_V);
            ql.a((byte) -105);
            hh.field_c = true;
            ab.field_a = 15000L + kd.c(-2456);
            break L5;
          }
        }
        L7: {
          L8: {
            if ((gm.field_c ^ -1) == 0) {
              break L8;
            } else {
              if ((gm.field_c ^ -1) == -1) {
                break L8;
              } else {
                break L7;
              }
            }
          }
          L9: {
            if ((gm.field_c ^ -1) != 0) {
              stackIn_26_0 = 0;
              break L9;
            } else {
              stackIn_26_0 = 1;
              break L9;
            }
          }
          L10: {
            var3 = stackIn_26_0;
            gm.field_c = fp.a((byte) 70);
            if (var3 == 0) {
              break L10;
            } else {
              if (0 != gm.field_c) {
                break L10;
              } else {
                if (11 != kh.field_n) {
                  break L10;
                } else {
                  if (!dm.c(param1 + -7)) {
                    bl.b((byte) -97);
                    break L10;
                  } else {
                    break L10;
                  }
                }
              }
            }
          }
          if ((gm.field_c ^ -1) == 0) {
            break L7;
          } else {
            if (-1 == (gm.field_c ^ -1)) {
              break L7;
            } else {
              ab.field_a = 15000L + kd.c(param1 ^ 2526);
              break L7;
            }
          }
        }
        L11: {
          if (-1 == gm.field_c) {
            break L11;
          } else {
            if (-1 == (gm.field_c ^ -1)) {
              break L11;
            } else {
              if (10 <= dk.field_c) {
                if (-11 >= (kh.field_n ^ -1)) {
                  L12: {
                    l.a(param1 + 73);
                    if (3 != gm.field_c) {
                      if ((gm.field_c ^ -1) == -5) {
                        nf.a(256, true, sf.field_e);
                        break L12;
                      } else {
                        if ((gm.field_c ^ -1) == -3) {
                          nf.a(256, true, ul.field_b);
                          break L12;
                        } else {
                          if ((gm.field_c ^ -1) != -6) {
                            nf.a(256, true, b.field_b);
                            break L12;
                          } else {
                            nf.a(5, true, ed.field_W);
                            break L12;
                          }
                        }
                      }
                    } else {
                      nf.a(256, true, ck.field_D);
                      break L12;
                    }
                  }
                  hh.field_c = true;
                  break L11;
                } else {
                  break L11;
                }
              } else {
                if (3 == gm.field_c) {
                  this.a((byte) -112, "js5crc");
                  break L11;
                } else {
                  if (gm.field_c == 4) {
                    this.a((byte) -112, "js5io");
                    break L11;
                  } else {
                    if (2 != gm.field_c) {
                      if (gm.field_c != 5) {
                        this.a((byte) -112, "js5connect");
                        break L11;
                      } else {
                        this.a((byte) -112, "outofdate");
                        break L11;
                      }
                    } else {
                      this.a((byte) -112, "js5connect_full");
                      break L11;
                    }
                  }
                }
              }
            }
          }
        }
        L13: {
          L14: {
            L15: {
              if (0 == (gm.field_c ^ -1)) {
                break L15;
              } else {
                if (0 != gm.field_c) {
                  break L14;
                } else {
                  break L15;
                }
              }
            }
            if (!dm.c(param1 + -43)) {
              break L13;
            } else {
              break L14;
            }
          }
          if ((kd.c(param1 + -2382) ^ -1L) <= (ab.field_a ^ -1L)) {
            hh.field_c = false;
            if (0 == (gm.field_c ^ -1)) {
              break L13;
            } else {
              if (0 == gm.field_c) {
                break L13;
              } else {
                gm.field_c = -1;
                en.a((byte) -124);
                break L13;
              }
            }
          } else {
            break L13;
          }
        }
        L16: {
          if (-1 != (gm.field_c ^ -1)) {
            break L16;
          } else {
            if (!dm.c(-125)) {
              u.field_O = false;
              break L16;
            } else {
              break L16;
            }
          }
        }
        L17: {
          if (0 != dk.field_c) {
            break L17;
          } else {
            if (ln.a(255)) {
              dk.field_c = 1;
              break L17;
            } else {
              break L17;
            }
          }
        }
        L18: {
          if ((dk.field_c ^ -1) != -2) {
            break L18;
          } else {
            L19: {
              if (hc.field_d == 0) {
                break L19;
              } else {
                ia.field_U = mk.a(mh.field_a, (byte) 34);
                break L19;
              }
            }
            fi.field_u = qd.a(false, true, s.field_a, (byte) -31, 1);
            kg.field_b = qd.a(false, true, pm.field_m, (byte) -31, 1);
            ib.field_b = qd.a(false, true, sp.field_h, (byte) -31, 1);
            dk.field_c = 2;
            gh.field_c = kg.field_b;
            uc.field_I = fi.field_u;
            break L18;
          }
        }
        L20: {
          if (dk.field_c == 2) {
            L21: {
              if (null != ia.field_U) {
                if (!ia.field_U.a(param1 + -14280)) {
                  break L21;
                } else {
                  if (ia.field_U.c("", param1 ^ -74)) {
                    if (!ia.field_U.a(false, "")) {
                      break L21;
                    } else {
                      ng.a(param1 ^ 27014, ia.field_U);
                      ia.field_U = null;
                      nf.d(0);
                      break L21;
                    }
                  } else {
                    ia.field_U = null;
                    break L21;
                  }
                }
              } else {
                break L21;
              }
            }
            if (ia.field_U != null) {
              break L20;
            } else {
              dk.field_c = 3;
              break L20;
            }
          } else {
            break L20;
          }
        }
        L22: {
          if ((dk.field_c ^ -1) == -4) {
            if (!ql.a(kg.field_b, ib.field_b, (byte) 61, fi.field_u)) {
              break L22;
            } else {
              if (fl.a(-43, ib.field_b)) {
                L23: {
                  L24: {
                    jm.a(false);
                    am.b(112);
                    lj.field_e = bg.field_t;
                    oj.field_b = false;
                    h.a(param1 ^ -8099, fi.field_u, ib.field_b, ld.field_g, kg.field_b);
                    if (np.field_D) {
                      break L24;
                    } else {
                      if (null == mi.field_d) {
                        break L23;
                      } else {
                        break L24;
                      }
                    }
                  }
                  L25: {
                    stackIn_102_0 = 8192;

                    if (np.field_D) {
                      stackIn_103_0 = stackIn_102_0;
                      stackIn_103_1 = 0;
                      break L25;
                    } else {
                      stackIn_103_0 = stackIn_102_0;
                      stackIn_103_1 = 1;
                      break L25;
                    }
                  }
                  L26: {




                    stackIn_105_2 = mi.field_d;

                    if (np.field_D) {


                      stackIn_106_2 = (String) ((Object) stackIn_105_2);
                      stackIn_106_3 = 0;
                      break L26;
                    } else {


                      stackIn_106_2 = (String) ((Object) stackIn_105_2);
                      stackIn_106_3 = 1;
                      break L26;
                    }
                  }
                  hd.a(stackIn_103_0, stackIn_103_1 != 0, stackIn_106_2, stackIn_106_3 != 0);
                  break L23;
                }
                L27: {
                  if (!f.field_d) {
                    break L27;
                  } else {
                    kd.a(4);
                    break L27;
                  }
                }
                L28: {
                  if (je.field_c != null) {
                    break L28;
                  } else {
                    je.field_c = fb.b((byte) 127);
                    tb.field_g = ab.a((byte) -97);
                    break L28;
                  }
                }
                nh.a((byte) -96, ib.field_b, je.field_c, tb.field_g);
                ib.field_b = null;
                fi.field_u = null;
                kg.field_b = null;
                qj.a((java.applet.Applet) (this), (byte) 89);
                nf.d(0);
                dk.field_c = 10;
                break L22;
              } else {
                break L22;
              }
            }
          } else {
            break L22;
          }
        }
        L29: {
          if (dk.field_c != 10) {
            break L29;
          } else {
            L30: {
              if (hc.field_d != 0) {
                ec.field_k = mk.a(bh.field_P, (byte) 34);
                break L30;
              } else {
                break L30;
              }
            }
            dk.field_c = 11;
            break L29;
          }
        }
        L31: {
          if (-12 != (dk.field_c ^ -1)) {
            break L31;
          } else {
            L32: {
              if (null == ec.field_k) {
                break L32;
              } else {
                L33: {
                  if (!ec.field_k.a(-14354)) {
                    break L33;
                  } else {
                    if (!ec.field_k.a((byte) 59)) {
                      break L33;
                    } else {
                      break L32;
                    }
                  }
                }
                tp.a(10, 0.0f, lk.a(ki.field_c, ec.field_k, ec.field_r, (byte) -125));
                break L31;
              }
            }
            ld.field_i = true;
            dk.field_c = 12;
            break L31;
          }
        }
        L34: {
          if ((dk.field_c ^ -1) != -13) {
            break L34;
          } else {
            if (!ld.field_i) {
              dk.field_c = 13;
              break L34;
            } else {
              break L34;
            }
          }
        }
        L35: {
          if ((dk.field_c ^ -1) == -14) {
            L36: {
              var3 = 1;
              if (be.field_s != null) {
                L37: {
                  if (!be.field_s.a(false)) {
                    stackIn_135_0 = 0;
                    break L37;
                  } else {
                    stackIn_135_0 = 1;
                    break L37;
                  }
                }
                var3 = stackIn_135_0;
                tp.a(10, be.field_s.field_j, be.field_s.field_b);
                break L36;
              } else {
                break L36;
              }
            }
            if (var3 == 0) {
              break L35;
            } else {
              dk.field_c = 20;
              break L35;
            }
          } else {
            break L35;
          }
        }
        L38: {
          if (param0) {
            break L38;
          } else {
            if (!ve.field_a) {
              break L38;
            } else {
              pd.a(c.field_h, -32);
              this.d((byte) 121);
              uk.a(c.field_h, (byte) -124);
              break L38;
            }
          }
        }
        L39: {
          if (!pj.field_w[8]) {
            break L39;
          } else {
            ta.b(12841);
            break L39;
          }
        }
    }

    private final int g(int param0) {
        int var2;
        if (param0 == 11) {
          if (!this.field_i) {
            if (!dm.c(-118)) {
              return -1;
            } else {
              if (!hh.field_c) {
                var2 = bh.a(this.field_u, em.e(7142), 116, this.field_q, true, vl.c(-29455));
                if ((var2 ^ -1) == 0) {
                  return -1;
                } else {
                  L0: {
                    if (0 == var2) {
                      break L0;
                    } else {
                      if ((var2 ^ -1) == -2) {
                        break L0;
                      } else {
                        L1: {
                          if (rm.field_P) {
                            break L1;
                          } else {
                            this.a((byte) -112, "reconnect");
                            break L1;
                          }
                        }
                        l.a(-1);
                        nf.a(var2, true, tp.field_v);
                        hh.field_c = true;
                        ab.field_a = 15000L + kd.c(-2456);
                        return var2;
                      }
                    }
                  }
                  L2: {
                    if (kh.field_n != 11) {
                      break L2;
                    } else {
                      if (gm.field_c == 0) {
                        bl.b((byte) -97);
                        break L2;
                      } else {
                        break L2;
                      }
                    }
                  }
                  return var2;
                }
              } else {
                return -1;
              }
            }
          } else {
            return -1;
          }
        } else {
          return 24;
        }
    }

    final void b(boolean param0, byte param1) {
        pj.field_w[16] = true;
        if (param1 >= -33) {
            this.field_F = 36;
        }
        pj.field_w[7] = true;
        pj.field_w[18] = true;
        pj.field_w[3] = true;
        pj.field_w[8] = param0;
        pj.field_w[17] = true;
        pj.field_w[0] = true;
    }

    static {
        field_C = new og();
        field_B = new int[]{2, 13, 14, 8, 28, 7, 11};
        field_t = "Nimble even by the standards of his dexterous race, the saurus slink can dodge traps and attacks with equal ease.";
    }
}
