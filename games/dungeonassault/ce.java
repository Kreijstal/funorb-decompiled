/*
 * Decompiled by CFR-JS 0.4.0.
 */
final class ce extends kl {
    private pp field_n;
    static eh field_v;
    static long field_s;
    private pp field_r;
    static int field_p;
    static String[] field_q;
    static String field_m;
    static String field_t;
    static int field_x;
    static String[] field_o;
    static int field_w;
    StringBuilder field_u;

    public static void d(byte param0) {
        field_m = null;
        field_v = null;
        field_o = null;
        if (param0 != 58) {
          field_p = 90;
          field_t = null;
          field_q = null;
          return;
        } else {
          field_t = null;
          field_q = null;
          return;
        }
    }

    final void b(int param0) {
        String var9 = null;
        String var7 = null;
        int var8 = 0;
        super.b(-51);
        int var2 = (this.field_a >> -1449249503) + this.field_k;
        int var3 = this.field_j - -10;
        ne.field_c.c(hb.field_l.toUpperCase(), var2, var3 - -ne.field_c.field_H, 16777215, -1);
        var3 = var3 + ne.field_c.field_H;
        la.field_b.a(hl.field_k, 20 + this.field_k, var3, -40 + this.field_a, this.field_i + this.field_j + -var3 - (24 - -(jm.field_i.field_v * 2)), 16777215, -1, 1, 1, la.field_b.field_H);
        int var4 = this.field_k + (this.field_a + -200 >> 1034248321);
        int var5 = 60 + var3;
        int var6 = -121 / ((41 - param0) / 63);
        gf.b(-4 + var4, -4 + var5, 208, 28, 0);
        rp.a(4, 200, (byte) 108, var4, 20, kc.field_S, var5);
        if (this.field_u != null) {
            var9 = this.field_u.toString();
            var7 = var9;
            if (-1 == (1 & mo.field_a / 25 ^ -1)) {
                var7 = var9 + "|";
            }
            var8 = this.field_u.length() <= 12 ? 16777215 : 16711680;
            la.field_b.b(var7, var4, (la.field_b.field_H + 20 >> -1174881279) + var5, var8, -1);
        }
    }

    final void b(boolean param0) {
        StringBuilder discarded$0 = null;
        int var3;
        var3 = DungeonAssault.field_K;
        if (-85 != (mm.field_t ^ -1)) {
          if (-14 != (mm.field_t ^ -1)) {
            if (85 == mm.field_t) {
              if (this.field_u.length() > 0) {
                am.a(this.field_u, ' ', -1 + this.field_u.length(), false);
                if (param0) {
                  field_v = (eh) null;
                  return;
                } else {
                  return;
                }
              } else {
                if (param0) {
                  field_v = (eh) null;
                  return;
                } else {
                  return;
                }
              }
            } else {
              if (-13 < (this.field_u.length() ^ -1)) {
                if (pe.a((CharSequence) ((Object) this.field_u), ob.field_Yb, (byte) -105)) {
                  discarded$0 = this.field_u.append(ob.field_Yb);
                  if (param0) {
                    field_v = (eh) null;
                    return;
                  } else {
                    return;
                  }
                } else {
                  if (param0) {
                    field_v = (eh) null;
                    return;
                  } else {
                    return;
                  }
                }
              } else {
                if (param0) {
                  field_v = (eh) null;
                  return;
                } else {
                  return;
                }
              }
            }
          } else {
            this.field_u = null;
            this.field_d = true;
            if (!param0) {
              return;
            } else {
              field_v = (eh) null;
              return;
            }
          }
        } else {
          this.field_d = true;
          if (!param0) {
            return;
          } else {
            field_v = (eh) null;
            return;
          }
        }
    }

    final void a(boolean param0) {
        super.a(param0);
        if (!(null == hh.field_s)) {
            if (!(this.field_n != hh.field_s)) {
                this.field_u = null;
            }
            this.field_d = true;
        }
    }

    ce(String param0) {
        int var2_int = 0;
        try {
            this.field_u = new StringBuilder(param0);
            var2_int = 88 + 2 * jm.field_i.field_v + oj.a(hl.field_k, -1, new int[]{300}, la.field_b) * la.field_b.field_H;
            this.a(350, 0, var2_int);
            this.field_d = false;
            this.field_r = tn.a(this.field_k - -10 + (-10 + (this.field_a >> -1528287295) + -jm.field_i.field_y >> -1813890591), this.field_i + (this.field_j + -16) + -jm.field_i.field_v, ne.field_c, jm.field_i, -85, hb.field_l);
            this.field_n = tn.a((-jm.field_i.field_y + (this.field_a >> -1154861215) + -10 >> -536053375) + (this.field_a >> -319598591) + this.field_k, this.field_i + this.field_j + (-16 - jm.field_i.field_v), ne.field_c, jm.field_i, -125, be.field_r);
            this.field_f.a(this.field_r, false);
            this.field_f.a(this.field_n, false);
        } catch (RuntimeException runtimeException) {
            throw vk.a((Throwable) ((Object) runtimeException), "ce.<init>(" + (param0 != null ? "{...}" : "null") + ')');
        }
    }

    static {
        field_m = "Snakes";
        field_t = "Enter a password for this account. Try to pick a strong password that can't easily be guessed.";
        field_o = new String[]{"Goblin Chieftain", "Orc Warlord", "Raider Newt", "Raider Lizard", "Raider Drake", "Raider Dragon", "Feared Raider", "Heroic Raider", "Defender Newt", "Defender Lizard", "Defender Drake", "Defender Dragon", "Vengeful Dragon", "Tyrannical Dragon", "Cartographer", "Renowned Dragon", "Persistence", "Price of Greed", "Bounty Hunter", "Great Raider", "Great Defender", "Plunder", "Consummate Dragon", "Glorious Raider", "Glorious Defender", "Gold Standing", "Dragon's Engineering", "Dragon's Empathy", "Draconic Heist", "Ransack", "Arcane Mastery", "Majestic Raider", "Majestic Defender", "Platinum Standing", "Dragon's Determination", "Dragon's Scheming", "Dragon's Daring", "Dragon's Triumph", "Aristocrat", "Dragon's Wrath", "Dragon's Hoard", "Dragon's Riches", "Creature of Habit", "Dungeon Assault Bronze", "Dungeon Assault Silver", "Dungeon Assault Gold"};
        field_q = new String[]{"<%0> the Black", "<%0> the White", "<%0> the Good", "<%0> the Evil", "<%0> Flameclaw", "<%0> Ironscale", "<%0> Orcbane", "<%0> the Ascendant", "<%0> the Wanderer", "<%0>, Winged Terror", "<%0> the Deceitful", "<%0>, Disciple of Night", "<%0>, Servant of Dawn", "<%0> Demonfire", "<%0>, Kindler of Hatred", "<%0>, Agent of Malice"};
    }
}
