/*
 * Decompiled by CFR-JS 0.4.0.
 */
import java.io.*;

final class hd {
    private int field_d;
    private int field_l;
    private long field_k;
    private byte[] field_n;
    static java.lang.reflect.Constructor field_h;
    private long field_a;
    private long field_c;
    private long field_b;
    private qp field_f;
    static int[] field_j;
    private long field_i;
    private long field_p;
    private byte[] field_o;
    static java.lang.reflect.Method field_e;
    static String[] field_m;
    static od field_g;

    final void c(byte param0) throws IOException {
        this.d((byte) -117);
        int var2 = 44 % ((19 - param0) / 58);
        this.field_f.a(0);
    }

    private final void d(byte param0) throws IOException {
        int var2;
        long var3;
        long var5;
        int var7;
        int var8;
        L0: {
          var8 = DungeonAssault.field_K;
          var2 = 81 % ((param0 - 50) / 55);
          if (-1L != this.field_c) {
            L1: {
              if ((this.field_a ^ -1L) == (this.field_c ^ -1L)) {
                break L1;
              } else {
                this.field_f.a(this.field_c, 18);
                this.field_a = this.field_c;
                break L1;
              }
            }
            L2: {
              this.field_f.a(0, 105, this.field_n, this.field_d);
              this.field_a = this.field_a + (long)this.field_d;
              if ((this.field_i ^ -1L) <= (this.field_a ^ -1L)) {
                break L2;
              } else {
                this.field_i = this.field_a;
                break L2;
              }
            }
            L3: {
              L4: {
                var3 = -1L;
                if (this.field_c < this.field_b) {
                  break L4;
                } else {
                  if (((long)this.field_l + this.field_b ^ -1L) >= (this.field_c ^ -1L)) {
                    break L4;
                  } else {
                    var3 = this.field_c;
                    break L3;
                  }
                }
              }
              if (this.field_b < this.field_c) {
                break L3;
              } else {
                if ((this.field_b ^ -1L) <= ((long)this.field_d + this.field_c ^ -1L)) {
                  break L3;
                } else {
                  var3 = this.field_b;
                  break L3;
                }
              }
            }
            L5: {
              L6: {
                var5 = -1L;
                if ((this.field_b ^ -1L) <= ((long)this.field_d + this.field_c ^ -1L)) {
                  break L6;
                } else {
                  if (((long)this.field_l + this.field_b ^ -1L) <= ((long)this.field_d + this.field_c ^ -1L)) {
                    var5 = this.field_c + (long)this.field_d;
                    break L5;
                  } else {
                    break L6;
                  }
                }
              }
              if (((long)this.field_l + this.field_b ^ -1L) >= (this.field_c ^ -1L)) {
                break L5;
              } else {
                if (((long)this.field_l + this.field_b ^ -1L) < ((long)this.field_d + this.field_c ^ -1L)) {
                  break L5;
                } else {
                  var5 = (long)this.field_l + this.field_b;
                  break L5;
                }
              }
            }
            L7: {
              if (var3 <= -1L) {
                break L7;
              } else {
                if ((var5 ^ -1L) >= (var3 ^ -1L)) {
                  break L7;
                } else {
                  var7 = (int)(var5 + -var3);
                  cj.a(this.field_n, (int)(var3 + -this.field_c), this.field_o, (int)(var3 - this.field_b), var7);
                  break L7;
                }
              }
            }
            this.field_d = 0;
            this.field_c = -1L;
            break L0;
          } else {
            break L0;
          }
        }
    }

    public static void b(byte param0) {
        field_h = null;
        int var1 = 33 % ((param0 - 52) / 62);
        field_e = null;
        field_j = null;
        field_m = null;
        field_g = null;
    }

    final static void a(String param0, java.awt.Color param1, boolean param2, byte param3, int param4) {
        RuntimeException stackIn_23_0 = null;
        StringBuilder stackIn_23_1 = null;
        RuntimeException stackIn_24_0 = null;
        StringBuilder stackIn_24_1 = null;
        String stackIn_24_2 = null;
        StringBuilder stackIn_26_1 = null;
        StringBuilder stackIn_27_1 = null;
        String stackIn_27_2 = null;
        Throwable decompiledCaughtException = null;
        Exception var5 = null;
        RuntimeException var5_ref = null;
        Exception var6 = null;
        int var7 = 0;
        int var8 = 0;
        java.awt.Graphics var9 = null;
        java.awt.Graphics var10 = null;
        try {
          L0: {
            try {
              L1: {
                L2: {
                  var9 = c.field_h.getGraphics();
                  if (td.field_a != null) {
                    break L2;
                  } else {
                    td.field_a = new java.awt.Font("Helvetica", 1, 13);
                    break L2;
                  }
                }
                L3: {
                  if (param2) {
                    var9.setColor(java.awt.Color.black);
                    var9.fillRect(0, 0, up.field_d, eg.field_d);
                    break L3;
                  } else {
                    break L3;
                  }
                }
                L4: {
                  if (param1 == null) {
                    param1 = new java.awt.Color(140, 17, 17);
                    break L4;
                  } else {
                    break L4;
                  }
                }
                try {
                  L5: {
                    L6: {
                      if (bb.field_e != null) {
                        break L6;
                      } else {
                        bb.field_e = c.field_h.createImage(304, 34);
                        break L6;
                      }
                    }
                    L7: {
                      var10 = bb.field_e.getGraphics();
                      var10.setColor(param1);
                      var10.drawRect(0, 0, 303, 33);
                      var10.fillRect(2, 2, param4 * 3, 30);
                      var10.setColor(java.awt.Color.black);
                      var10.drawRect(1, 1, 301, 31);
                      var10.fillRect(2 + param4 * 3, 2, 300 + -(param4 * 3), 30);
                      var10.setFont(td.field_a);
                      if (param3 == 105) {
                        break L7;
                      } else {
                        field_g = (od) null;
                        break L7;
                      }
                    }
                    var10.setColor(java.awt.Color.white);
                    var10.drawString(param0, (304 - param0.length() * 6) / 2, 22);
                    var9.drawImage(bb.field_e, up.field_d / 2 + -152, eg.field_d / 2 - 18, (java.awt.image.ImageObserver) null);
                    break L5;
                  }
                } catch (java.lang.Exception decompiledCaughtParameter0) {
                  decompiledCaughtException = decompiledCaughtParameter0;
                  L8: {
                    var6 = (Exception) (Object) decompiledCaughtException;
                    var7 = -152 + up.field_d / 2;
                    var8 = eg.field_d / 2 - 18;
                    var9.setColor(param1);
                    var9.drawRect(var7, var8, 303, 33);
                    var9.fillRect(2 + var7, var8 + 2, param4 * 3, 30);
                    var9.setColor(java.awt.Color.black);
                    var9.drawRect(var7 - -1, var8 + 1, 301, 31);
                    var9.fillRect(2 + (var7 + param4 * 3), var8 + 2, -(param4 * 3) + 300, 30);
                    var9.setFont(td.field_a);
                    var9.setColor(java.awt.Color.white);
                    var9.drawString(param0, var7 + (-(6 * param0.length()) + 304) / 2, 22 + var8);
                    break L8;
                  }
                }
                L9: {
                  if (bb.field_f != null) {
                    var9.setFont(td.field_a);
                    var9.setColor(java.awt.Color.white);
                    var9.drawString(bb.field_f, up.field_d / 2 - 6 * bb.field_f.length() / 2, -26 + eg.field_d / 2);
                    break L9;
                  } else {
                    break L9;
                  }
                }
                break L1;
              }
            } catch (java.lang.Exception decompiledCaughtParameter1) {
              decompiledCaughtException = decompiledCaughtParameter1;
              L10: {
                var5 = (Exception) (Object) decompiledCaughtException;
                c.field_h.repaint();
                break L10;
              }
            }
            break L0;
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter2) {
          decompiledCaughtException = decompiledCaughtParameter2;
          L11: {
            var5_ref = (RuntimeException) (Object) decompiledCaughtException;
            stackIn_23_0 = (RuntimeException) (var5_ref);

            stackIn_23_1 = new StringBuilder().append("hd.M(");

            if (param0 == null) {
              stackIn_24_0 = (RuntimeException) ((Object) stackIn_23_0);
              stackIn_24_1 = (StringBuilder) ((Object) stackIn_23_1);
              stackIn_24_2 = "null";
              break L11;
            } else {
              stackIn_24_0 = (RuntimeException) ((Object) stackIn_23_0);
              stackIn_24_1 = (StringBuilder) ((Object) stackIn_23_1);
              stackIn_24_2 = "{...}";
              break L11;
            }
          }
          L12: {


            stackIn_26_1 = ((StringBuilder) (Object) stackIn_24_1).append(stackIn_24_2).append(',');

            if (param1 == null) {
              stackIn_24_0 = (RuntimeException) ((Object) stackIn_24_0);
              stackIn_27_1 = (StringBuilder) ((Object) stackIn_26_1);
              stackIn_27_2 = "null";
              break L12;
            } else {
              stackIn_24_0 = (RuntimeException) ((Object) stackIn_24_0);
              stackIn_27_1 = (StringBuilder) ((Object) stackIn_26_1);
              stackIn_27_2 = "{...}";
              break L12;
            }
          }
          throw vk.a((Throwable) ((Object) stackIn_24_0), stackIn_27_2 + ',' + param2 + ',' + param3 + ',' + param4 + ')');
        }
    }

    final static void a(int param0, boolean param1, String param2, boolean param3) {
        tn.a(-1);
        lg.field_w.h(1);
        wo.field_e = new dn(co.field_C, (String) null, qc.field_t, param3, param1);
        if (param0 != 8192) {
            return;
        }
        try {
            ta.field_a = new jk(lg.field_w, wo.field_e);
            lg.field_w.a(ta.field_a, -1);
        } catch (RuntimeException runtimeException) {
            throw vk.a((Throwable) ((Object) runtimeException), "hd.A(" + param0 + ',' + param1 + ',' + (param2 != null ? "{...}" : "null") + ',' + param3 + ')');
        }
    }

    final static void a(boolean param0) {
        int decompiledRegionSelector0 = 0;
        RuntimeException decompiledCaughtException = null;
        RuntimeException var1 = null;
        int var2 = 0;
        int var3 = 0;
        int var4 = 0;
        kj var4_ref_kj = null;
        mm var5 = null;
        int var5_int = 0;
        int[] var6 = null;
        int var7 = 0;
        int var8 = 0;
        wj var9 = null;
        int[] var10 = null;
        int[] var11 = null;
        var8 = DungeonAssault.field_K;
        try {
          L0: {
            L1: {
              if (!param0) {
                break L1;
              } else {
                field_j = (int[]) null;
                break L1;
              }
            }
            L2: {
              var9 = ra.field_c;
              var2 = var9.c(true);
              var3 = var9.c(true);
              if (-1 != (var2 ^ -1)) {
                if (1 != var2) {
                  sm.a((Throwable) null, 1, "LR1: " + sj.l(192));
                  tl.a(-51);
                  break L2;
                } else {
                  var4 = var9.l(0);
                  var5 = (mm) ((Object) m.field_a.e(-24172));
                  L3: while (true) {
                    L4: {
                      if (var5 == null) {
                        break L4;
                      } else {
                        L5: {
                          if (var3 != var5.field_p) {
                            break L5;
                          } else {
                            if (var5.field_r != var4) {
                              break L5;
                            } else {
                              break L4;
                            }
                          }
                        }
                        var5 = (mm) ((Object) m.field_a.a(4));
                        continue L3;
                      }
                    }
                    if (var5 == null) {
                      tl.a(-23);
                      decompiledRegionSelector0 = 2;
                      break L0;
                    } else {
                      var5.a(false);
                      break L2;
                    }
                  }
                }
              } else {
                var4_ref_kj = (kj) ((Object) cc.field_i.e(-24172));
                if (var4_ref_kj != null) {
                  L6: {
                    var5_int = nj.field_b + -var9.field_o;
                    var11 = var4_ref_kj.field_l;
                    var10 = var11;
                    var6 = var10;
                    if (var5_int <= var11.length << 1063189954) {
                      break L6;
                    } else {
                      var5_int = var11.length << -213987646;
                      break L6;
                    }
                  }
                  var7 = 0;
                  L7: while (true) {
                    if (var5_int <= var7) {
                      var4_ref_kj.a(false);
                      break L2;
                    } else {
                      var6[var7 >> -1345301182] = var6[var7 >> -1345301182] + (var9.c(true) << nb.a(768, var7 << 1826925032));
                      var7++;
                      continue L7;
                    }
                  }
                } else {
                  tl.a(-21);
                  decompiledRegionSelector0 = 0;
                  break L0;
                }
              }
            }
            decompiledRegionSelector0 = 1;
            break L0;
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          var1 = decompiledCaughtException;
          throw vk.a((Throwable) ((Object) var1), "hd.H(" + param0 + ')');
        }
        if (decompiledRegionSelector0 == 0) {
          return;
        } else {
          if (decompiledRegionSelector0 == 1) {
            return;
          } else {
            return;
          }
        }
    }

    final void a(long param0, int param1) throws IOException {
        if (param1 != -26296) {
            return;
        }
        if (0L > param0) {
            throw new IOException();
        }
        this.field_k = param0;
    }

    private final void a(int param0) throws IOException {
        int var2;
        int var3;
        int var4;
        int var5;
        L0: {
          var5 = DungeonAssault.field_K;
          this.field_l = 0;
          var2 = -89 / ((param0 - 3) / 56);
          if ((this.field_k ^ -1L) != (this.field_a ^ -1L)) {
            this.field_f.a(this.field_k, 18);
            this.field_a = this.field_k;
            break L0;
          } else {
            break L0;
          }
        }
        this.field_b = this.field_k;
        L1: while (true) {
          L2: {
            if (this.field_l >= this.field_o.length) {
              break L2;
            } else {
              L3: {
                var3 = -this.field_l + this.field_o.length;
                if (200000000 >= var3) {
                  break L3;
                } else {
                  var3 = 200000000;
                  break L3;
                }
              }
              var4 = this.field_f.a(this.field_l, -11274, var3, this.field_o);
              if (0 == (var4 ^ -1)) {
                break L2;
              } else {
                this.field_l = this.field_l + var4;
                this.field_a = this.field_a + (long)var4;
                continue L1;
              }
            }
          }
          return;
        }
    }

    final void a(int param0, int param1, byte[] param2, int param3) throws IOException {
        int incrementValue$0 = 0;
        RuntimeException stackIn_60_0 = null;
        StringBuilder stackIn_60_1 = null;
        RuntimeException stackIn_61_0 = null;
        StringBuilder stackIn_61_1 = null;
        String stackIn_61_2 = null;
        int decompiledRegionSelector0 = 0;
        int decompiledRegionSelector1 = 0;
        Throwable decompiledCaughtException = null;
        long var5_long = 0L;
        IOException var5 = null;
        RuntimeException var5_ref = null;
        int var7 = 0;
        int var8 = 0;
        int var9_int = 0;
        long var9 = 0L;
        long var11 = 0L;
        int var13 = 0;
        int var14 = 0;
        var14 = DungeonAssault.field_K;
        try {
          L0: {
            try {
              L1: {
                if (param2.length < param0 + param3) {
                  throw new ArrayIndexOutOfBoundsException(-param2.length + (param3 + param0));
                } else {
                  L2: {
                    if ((long)param1 == this.field_c) {
                      break L2;
                    } else {
                      if ((this.field_k ^ -1L) > (this.field_c ^ -1L)) {
                        break L2;
                      } else {
                        if (this.field_k + (long)param3 <= (long)this.field_d + this.field_c) {
                          cj.a(this.field_n, (int)(this.field_k - this.field_c), param2, param0, param3);
                          this.field_k = this.field_k + (long)param3;
                          decompiledRegionSelector0 = 0;
                          break L1;
                        } else {
                          break L2;
                        }
                      }
                    }
                  }
                  L3: {
                    var5_long = this.field_k;
                    var7 = param0;
                    var8 = param3;
                    if (this.field_k < this.field_b) {
                      break L3;
                    } else {
                      if (this.field_k >= (long)this.field_l + this.field_b) {
                        break L3;
                      } else {
                        L4: {
                          var9_int = (int)((long)this.field_l + (-this.field_k - -this.field_b));
                          if (param3 < var9_int) {
                            var9_int = param3;
                            break L4;
                          } else {
                            break L4;
                          }
                        }
                        cj.a(this.field_o, (int)(-this.field_b + this.field_k), param2, param0, var9_int);
                        param0 = param0 + var9_int;
                        this.field_k = this.field_k + (long)var9_int;
                        param3 = param3 - var9_int;
                        break L3;
                      }
                    }
                  }
                  L5: {
                    if (param3 > this.field_o.length) {
                      this.field_f.a(this.field_k, param1 + 19);
                      this.field_a = this.field_k;
                      L6: while (true) {
                        if (-1 <= (param3 ^ -1)) {
                          break L5;
                        } else {
                          var9_int = this.field_f.a(param0, -11274, param3, param2);
                          if ((var9_int ^ -1) != 0) {
                            this.field_a = this.field_a + (long)var9_int;
                            param3 = param3 - var9_int;
                            this.field_k = this.field_k + (long)var9_int;
                            param0 = param0 + var9_int;
                            continue L6;
                          } else {
                            break L5;
                          }
                        }
                      }
                    } else {
                      if (param3 > 0) {
                        L7: {
                          this.a(-57);
                          var9_int = param3;
                          if (var9_int > this.field_l) {
                            var9_int = this.field_l;
                            break L7;
                          } else {
                            break L7;
                          }
                        }
                        cj.a(this.field_o, 0, param2, param0, var9_int);
                        this.field_k = this.field_k + (long)var9_int;
                        param3 = param3 - var9_int;
                        param0 = param0 + var9_int;
                        break L5;
                      } else {
                        break L5;
                      }
                    }
                  }
                  L8: {
                    if ((this.field_c ^ -1L) == 0L) {
                      break L8;
                    } else {
                      L9: {
                        if (this.field_k >= this.field_c) {
                          break L9;
                        } else {
                          if (param3 <= 0) {
                            break L9;
                          } else {
                            L10: {
                              var9_int = param0 - -(int)(-this.field_k + this.field_c);
                              if (var9_int > param0 + param3) {
                                var9_int = param0 + param3;
                                break L10;
                              } else {
                                break L10;
                              }
                            }
                            L11: while (true) {
                              if (var9_int <= param0) {
                                break L9;
                              } else {
                                param3--;
                                incrementValue$0 = param0;
                                param0++;
                                param2[incrementValue$0] = (byte) 0;
                                this.field_k = this.field_k + 1L;
                                continue L11;
                              }
                            }
                          }
                        }
                      }
                      L12: {
                        L13: {
                          var9 = -1L;
                          if (this.field_c < var5_long) {
                            break L13;
                          } else {
                            if ((long)var8 + var5_long <= this.field_c) {
                              break L13;
                            } else {
                              var9 = this.field_c;
                              break L12;
                            }
                          }
                        }
                        if (var5_long < this.field_c) {
                          break L12;
                        } else {
                          if ((var5_long ^ -1L) <= (this.field_c + (long)this.field_d ^ -1L)) {
                            break L12;
                          } else {
                            var9 = var5_long;
                            break L12;
                          }
                        }
                      }
                      L14: {
                        L15: {
                          var11 = -1L;
                          if ((var5_long ^ -1L) <= (this.field_c - -(long)this.field_d ^ -1L)) {
                            break L15;
                          } else {
                            if ((long)var8 + var5_long < (long)this.field_d + this.field_c) {
                              break L15;
                            } else {
                              var11 = (long)this.field_d + this.field_c;
                              break L14;
                            }
                          }
                        }
                        if ((var5_long + (long)var8 ^ -1L) >= (this.field_c ^ -1L)) {
                          break L14;
                        } else {
                          if (((long)this.field_d + this.field_c ^ -1L) > (var5_long - -(long)var8 ^ -1L)) {
                            break L14;
                          } else {
                            var11 = (long)var8 + var5_long;
                            break L14;
                          }
                        }
                      }
                      if (0L <= (var9 ^ -1L)) {
                        break L8;
                      } else {
                        if ((var9 ^ -1L) <= (var11 ^ -1L)) {
                          break L8;
                        } else {
                          var13 = (int)(var11 + -var9);
                          cj.a(this.field_n, (int)(var9 + -this.field_c), param2, var7 + (int)(var9 - var5_long), var13);
                          if ((var11 ^ -1L) >= (this.field_k ^ -1L)) {
                            break L8;
                          } else {
                            param3 = (int)((long)param3 - (-this.field_k + var11));
                            this.field_k = var11;
                            break L8;
                          }
                        }
                      }
                    }
                  }
                  decompiledRegionSelector0 = 1;
                  break L1;
                }
              }
            } catch (java.io.IOException decompiledCaughtParameter0) {
              decompiledCaughtException = decompiledCaughtParameter0;
              var5 = (IOException) (Object) decompiledCaughtException;
              this.field_a = -1L;
              throw var5;
            }
            if (decompiledRegionSelector0 == 0) {
              decompiledRegionSelector1 = 1;
              break L0;
            } else {
              if (param3 > 0) {
                throw new EOFException();
              } else {
                decompiledRegionSelector1 = 0;
                break L0;
              }
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter1) {
          decompiledCaughtException = decompiledCaughtParameter1;
          L16: {
            var5_ref = (RuntimeException) (Object) decompiledCaughtException;
            stackIn_60_0 = (RuntimeException) (var5_ref);

            stackIn_60_1 = new StringBuilder().append("hd.L(").append(param0).append(',').append(param1).append(',');

            if (param2 == null) {
              stackIn_61_0 = (RuntimeException) ((Object) stackIn_60_0);
              stackIn_61_1 = (StringBuilder) ((Object) stackIn_60_1);
              stackIn_61_2 = "null";
              break L16;
            } else {
              stackIn_61_0 = (RuntimeException) ((Object) stackIn_60_0);
              stackIn_61_1 = (StringBuilder) ((Object) stackIn_60_1);
              stackIn_61_2 = "{...}";
              break L16;
            }
          }
          throw vk.a((Throwable) ((Object) stackIn_61_0), stackIn_61_2 + ',' + param3 + ')');
        }
        if (decompiledRegionSelector1 == 0) {
          return;
        } else {
          return;
        }
    }

    final void a(int param0, byte[] param1) throws IOException {
        try {
            this.a(param0, param0 ^ -1, param1, param1.length);
        } catch (RuntimeException runtimeException) {
            throw vk.a((Throwable) ((Object) runtimeException), "hd.C(" + param0 + ',' + (param1 != null ? "{...}" : "null") + ')');
        }
    }

    final static cf a(boolean param0, int param1, String param2) {
        va var3 = null;
        RuntimeException var3_ref = null;
        Object stackIn_2_0 = null;
        cf stackIn_5_0 = null;
        cf stackIn_7_0 = null;
        RuntimeException stackIn_10_0 = null;
        StringBuilder stackIn_10_1 = null;
        RuntimeException stackIn_11_0 = null;
        StringBuilder stackIn_11_1 = null;
        String stackIn_11_2 = null;
        int decompiledRegionSelector0 = 0;
        RuntimeException decompiledCaughtException = null;
        try {
          L0: {
            var3 = ci.a(param2, -10396);
            if (var3 != null) {
              if (!param0) {
                stackIn_7_0 = new cf(var3, param1);
                decompiledRegionSelector0 = 2;
                break L0;
              } else {
                stackIn_5_0 = (cf) null;
                decompiledRegionSelector0 = 1;
                break L0;
              }
            } else {
              stackIn_2_0 = null;
              decompiledRegionSelector0 = 0;
              break L0;
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L1: {
            var3_ref = decompiledCaughtException;
            stackIn_10_0 = (RuntimeException) (var3_ref);

            stackIn_10_1 = new StringBuilder().append("hd.G(").append(param0).append(',').append(param1).append(',');

            if (param2 == null) {
              stackIn_11_0 = (RuntimeException) ((Object) stackIn_10_0);
              stackIn_11_1 = (StringBuilder) ((Object) stackIn_10_1);
              stackIn_11_2 = "null";
              break L1;
            } else {
              stackIn_11_0 = (RuntimeException) ((Object) stackIn_10_0);
              stackIn_11_1 = (StringBuilder) ((Object) stackIn_10_1);
              stackIn_11_2 = "{...}";
              break L1;
            }
          }
          throw vk.a((Throwable) ((Object) stackIn_11_0), stackIn_11_2 + ')');
        }
        if (decompiledRegionSelector0 == 0) {
          return (cf) ((Object) stackIn_2_0);
        } else {
          if (decompiledRegionSelector0 == 1) {
            return stackIn_5_0;
          } else {
            return stackIn_7_0;
          }
        }
    }

    final void a(int param0, int param1, int param2, byte[] param3) throws IOException {
        RuntimeException stackIn_54_0 = null;
        StringBuilder stackIn_54_1 = null;
        RuntimeException stackIn_55_0 = null;
        StringBuilder stackIn_55_1 = null;
        String stackIn_55_2 = null;
        int decompiledRegionSelector0 = 0;
        int decompiledRegionSelector1 = 0;
        Throwable decompiledCaughtException = null;
        int var5_int = 0;
        long var5_long = 0L;
        IOException var5 = null;
        RuntimeException var5_ref = null;
        long var7 = 0L;
        int var9 = 0;
        int var10 = 0;
        var10 = DungeonAssault.field_K;
        try {
          L0: {
            try {
              L1: {
                L2: {
                  if (this.field_k - -(long)param2 > this.field_p) {
                    this.field_p = (long)param2 + this.field_k;
                    break L2;
                  } else {
                    break L2;
                  }
                }
                L3: {
                  if (param1 < -75) {
                    break L3;
                  } else {
                    hd.a(true);
                    break L3;
                  }
                }
                L4: {
                  if (-1L == this.field_c) {
                    break L4;
                  } else {
                    L5: {
                      if ((this.field_c ^ -1L) < (this.field_k ^ -1L)) {
                        break L5;
                      } else {
                        if (this.field_k > this.field_c + (long)this.field_d) {
                          break L5;
                        } else {
                          break L4;
                        }
                      }
                    }
                    this.d((byte) 115);
                    break L4;
                  }
                }
                L6: {
                  if ((this.field_c ^ -1L) == 0L) {
                    break L6;
                  } else {
                    if ((this.field_k - -(long)param2 ^ -1L) < ((long)this.field_n.length + this.field_c ^ -1L)) {
                      var5_int = (int)(this.field_c + -this.field_k + (long)this.field_n.length);
                      cj.a(param3, param0, this.field_n, (int)(this.field_k + -this.field_c), var5_int);
                      param0 = param0 + var5_int;
                      this.field_k = this.field_k + (long)var5_int;
                      param2 = param2 - var5_int;
                      this.field_d = this.field_n.length;
                      this.d((byte) -92);
                      break L6;
                    } else {
                      break L6;
                    }
                  }
                }
                if (this.field_n.length >= param2) {
                  if (param2 <= 0) {
                    decompiledRegionSelector0 = 1;
                    break L1;
                  } else {
                    L7: {
                      if ((this.field_c ^ -1L) == 0L) {
                        this.field_c = this.field_k;
                        break L7;
                      } else {
                        break L7;
                      }
                    }
                    L8: {
                      cj.a(param3, param0, this.field_n, (int)(-this.field_c + this.field_k), param2);
                      this.field_k = this.field_k + (long)param2;
                      if (-this.field_c + this.field_k > (long)this.field_d) {
                        this.field_d = (int)(this.field_k - this.field_c);
                        break L8;
                      } else {
                        break L8;
                      }
                    }
                    decompiledRegionSelector0 = 2;
                    break L1;
                  }
                } else {
                  L9: {
                    if ((this.field_k ^ -1L) == (this.field_a ^ -1L)) {
                      break L9;
                    } else {
                      this.field_f.a(this.field_k, 18);
                      this.field_a = this.field_k;
                      break L9;
                    }
                  }
                  L10: {
                    this.field_f.a(param0, 102, param3, param2);
                    this.field_a = this.field_a + (long)param2;
                    if (this.field_a > this.field_i) {
                      this.field_i = this.field_a;
                      break L10;
                    } else {
                      break L10;
                    }
                  }
                  L11: {
                    L12: {
                      var5_long = -1L;
                      var7 = -1L;
                      if ((this.field_k ^ -1L) > (this.field_b ^ -1L)) {
                        break L12;
                      } else {
                        if (this.field_k < (long)this.field_l + this.field_b) {
                          var5_long = this.field_k;
                          break L11;
                        } else {
                          break L12;
                        }
                      }
                    }
                    if ((this.field_k ^ -1L) < (this.field_b ^ -1L)) {
                      break L11;
                    } else {
                      if ((this.field_b ^ -1L) <= (this.field_k + (long)param2 ^ -1L)) {
                        break L11;
                      } else {
                        var5_long = this.field_b;
                        break L11;
                      }
                    }
                  }
                  L13: {
                    L14: {
                      if (((long)param2 + this.field_k ^ -1L) >= (this.field_b ^ -1L)) {
                        break L14;
                      } else {
                        if ((long)param2 + this.field_k > this.field_b - -(long)this.field_l) {
                          break L14;
                        } else {
                          var7 = this.field_k + (long)param2;
                          break L13;
                        }
                      }
                    }
                    if (this.field_k >= this.field_b - -(long)this.field_l) {
                      break L13;
                    } else {
                      if ((this.field_b - -(long)this.field_l ^ -1L) >= ((long)param2 + this.field_k ^ -1L)) {
                        var7 = (long)this.field_l + this.field_b;
                        break L13;
                      } else {
                        break L13;
                      }
                    }
                  }
                  L15: {
                    if (0L <= (var5_long ^ -1L)) {
                      break L15;
                    } else {
                      if ((var5_long ^ -1L) > (var7 ^ -1L)) {
                        var9 = (int)(var7 - var5_long);
                        cj.a(param3, (int)(-this.field_k + (var5_long + (long)param0)), this.field_o, (int)(var5_long + -this.field_b), var9);
                        break L15;
                      } else {
                        break L15;
                      }
                    }
                  }
                  this.field_k = this.field_k + (long)param2;
                  decompiledRegionSelector0 = 0;
                  break L1;
                }
              }
            } catch (java.io.IOException decompiledCaughtParameter0) {
              decompiledCaughtException = decompiledCaughtParameter0;
              var5 = (IOException) (Object) decompiledCaughtException;
              this.field_a = -1L;
              throw var5;
            }
            if (decompiledRegionSelector0 == 0) {
              decompiledRegionSelector1 = 0;
              break L0;
            } else {
              if (decompiledRegionSelector0 == 1) {
                decompiledRegionSelector1 = 1;
                break L0;
              } else {
                decompiledRegionSelector1 = 2;
                break L0;
              }
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter1) {
          decompiledCaughtException = decompiledCaughtParameter1;
          L16: {
            var5_ref = (RuntimeException) (Object) decompiledCaughtException;
            stackIn_54_0 = (RuntimeException) (var5_ref);

            stackIn_54_1 = new StringBuilder().append("hd.F(").append(param0).append(',').append(param1).append(',').append(param2).append(',');

            if (param3 == null) {
              stackIn_55_0 = (RuntimeException) ((Object) stackIn_54_0);
              stackIn_55_1 = (StringBuilder) ((Object) stackIn_54_1);
              stackIn_55_2 = "null";
              break L16;
            } else {
              stackIn_55_0 = (RuntimeException) ((Object) stackIn_54_0);
              stackIn_55_1 = (StringBuilder) ((Object) stackIn_54_1);
              stackIn_55_2 = "{...}";
              break L16;
            }
          }
          throw vk.a((Throwable) ((Object) stackIn_55_0), stackIn_55_2 + ')');
        }
        if (decompiledRegionSelector1 == 0) {
          return;
        } else {
          if (decompiledRegionSelector1 == 1) {
            return;
          } else {
            return;
          }
        }
    }

    hd(qp param0, int param1, int param2) throws IOException {
        long dupTemp$0 = 0L;
        this.field_b = -1L;
        this.field_d = 0;
        this.field_c = -1L;
        try {
            this.field_f = param0;
            dupTemp$0 = param0.a((byte) 91);
            this.field_i = dupTemp$0;
            this.field_p = dupTemp$0;
            this.field_n = new byte[param2];
            this.field_o = new byte[param1];
            this.field_k = 0L;
        } catch (RuntimeException runtimeException) {
            throw vk.a((Throwable) ((Object) runtimeException), "hd.<init>(" + (param0 != null ? "{...}" : "null") + ',' + param1 + ',' + param2 + ')');
        }
    }

    final long a(byte param0) {
        if (param0 != -92) {
            return -92L;
        }
        return this.field_p;
    }

    static {
        Class var2 = null;
        Class var0 = null;
        field_j = new int[8192];
        field_m = new String[]{"<%0> the Fierce", "<%0> the Cruel", "<%0> the Fell", "<%0> the Terrible", "<%0> the Savage", "<%0> the Raging", "<%0>, Bringer of Wrath", "<%0>, Lord of Chaos", "<%0> the Undefeated"};
        try {
            var2 = Class.forName("java.lang.ref.SoftReference");
            var0 = var2;
            field_h = var2.getConstructor(new Class[]{Object.class});
            Class[] var1 = (Class[]) null;
            field_e = var0.getMethod("get", (Class[]) null);
        } catch (ClassNotFoundException classNotFoundException) {
            field_h = null;
        } catch (NoSuchMethodException noSuchMethodException) {
            field_h = null;
        }
        field_g = new od();
    }
}
