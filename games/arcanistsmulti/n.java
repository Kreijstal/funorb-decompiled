/*
 * Decompiled by CFR-JS 0.4.0.
 */
final class n {
    static String field_c;
    private int[] field_f;
    static ri field_b;
    static String field_h;
    static int field_a;
    static kc field_g;
    static oc field_d;
    static String field_e;

    public static void a(int param0) {
        java.awt.Canvas var2;
        field_b = null;
        field_h = null;
        field_c = null;
        if (param0 != 8577) {
          var2 = (java.awt.Canvas) null;
          n.a((java.awt.Canvas) null, (byte) -117);
          field_g = null;
          field_e = null;
          field_d = null;
          return;
        } else {
          field_g = null;
          field_e = null;
          field_d = null;
          return;
        }
    }

    final static void a(java.awt.Canvas param0, byte param1) {
        hg.a(1, (java.awt.Component) ((Object) param0));
        if (param1 < 115) {
            return;
        }
        try {
            no.a((java.awt.Component) ((Object) param0), 0);
            if (null != nh.field_w) {
                nh.field_w.a(-15238, (java.awt.Component) ((Object) param0));
            }
        } catch (RuntimeException runtimeException) {
            throw aa.a((Throwable) ((Object) runtimeException), "n.D(" + (param0 != null ? "{...}" : "null") + ',' + param1 + ')');
        }
    }

    final static void a(int param0, int param1, int param2) {
        int var3;
        int var4;
        int var5;
        int var6;
        int var7;
        int var8;
        var8 = ArcanistsMulti.field_G ? 1 : 0;
        if (0 > gi.field_a) {
          return;
        } else {
          L0: {
            var3 = -135 + param0;
            var4 = param1 + -35;
            var5 = 256;
            if ((gi.field_a ^ -1) > -76) {
              var5 = (gi.field_a << 59308968) / 75;
              break L0;
            } else {
              break L0;
            }
          }
          L1: {
            if (gi.field_a <= 200) {
              break L1;
            } else {
              var5 = (-gi.field_a + 250 << -618842264) / 50;
              break L1;
            }
          }
          L2: {
            fk.a(rc.field_h, -23095);
            id.a();
            de.b();
            cg.f(252566882);
            if (var5 < 256) {
              de.e(0, 0, de.field_e, de.field_j, 0, -var5 + 256);
              break L2;
            } else {
              break L2;
            }
          }
          oo.c(-15405);
          if ((gi.field_a ^ -1) <= -151) {
            mi.field_E.c(15 + var3, var4 + 10, var5);
            var6 = -125 + gi.field_a;
            if (param2 == 1) {
              if (0 < var6) {
                if ((var6 ^ -1) > -51) {
                  if (-21 >= (var6 ^ -1)) {
                    if (30 <= var6) {
                      L3: {
                        var7 = (50 + -var6) * 256 / 20;
                        gi.field_k.b(var3, var4, var7);
                        var6 = gi.field_a + -140;
                        if (-1 > (var6 ^ -1)) {
                          L4: {
                            var7 = 256;
                            if (20 > var6) {
                              var7 = 256 * var6 / 20;
                              break L4;
                            } else {
                              break L4;
                            }
                          }
                          gm.field_l.c(var3 - -15, var4 + 10, var5 * var7 >> -446111992);
                          break L3;
                        } else {
                          break L3;
                        }
                      }
                      return;
                    } else {
                      L5: {
                        gi.field_k.b(var3, var4, 256);
                        var6 = gi.field_a + -140;
                        if (-1 > (var6 ^ -1)) {
                          L6: {
                            var7 = 256;
                            if (20 > var6) {
                              var7 = 256 * var6 / 20;
                              break L6;
                            } else {
                              break L6;
                            }
                          }
                          gm.field_l.c(var3 - -15, var4 + 10, var5 * var7 >> -446111992);
                          break L5;
                        } else {
                          break L5;
                        }
                      }
                      return;
                    }
                  } else {
                    L7: {
                      var7 = 256 * var6 / 20;
                      gi.field_k.b(var3, var4, var7);
                      var6 = gi.field_a + -140;
                      if (-1 > (var6 ^ -1)) {
                        L8: {
                          var7 = 256;
                          if (20 > var6) {
                            var7 = 256 * var6 / 20;
                            break L8;
                          } else {
                            break L8;
                          }
                        }
                        gm.field_l.c(var3 - -15, var4 + 10, var5 * var7 >> -446111992);
                        break L7;
                      } else {
                        break L7;
                      }
                    }
                    return;
                  }
                } else {
                  L9: {
                    var6 = gi.field_a + -140;
                    if (-1 > (var6 ^ -1)) {
                      L10: {
                        var7 = 256;
                        if (20 > var6) {
                          var7 = 256 * var6 / 20;
                          break L10;
                        } else {
                          break L10;
                        }
                      }
                      gm.field_l.c(var3 - -15, var4 + 10, var5 * var7 >> -446111992);
                      break L9;
                    } else {
                      break L9;
                    }
                  }
                  return;
                }
              } else {
                L11: {
                  var6 = gi.field_a + -140;
                  if (-1 > (var6 ^ -1)) {
                    L12: {
                      var7 = 256;
                      if (20 > var6) {
                        var7 = 256 * var6 / 20;
                        break L12;
                      } else {
                        break L12;
                      }
                    }
                    gm.field_l.c(var3 - -15, var4 + 10, var5 * var7 >> -446111992);
                    break L11;
                  } else {
                    break L11;
                  }
                }
                return;
              }
            } else {
              return;
            }
          } else {
            rc.field_h.b(var3, var4);
            var6 = -125 + gi.field_a;
            if (param2 == 1) {
              if (0 < var6) {
                if ((var6 ^ -1) > -51) {
                  if (-21 >= (var6 ^ -1)) {
                    if (30 <= var6) {
                      L13: {
                        var7 = (50 + -var6) * 256 / 20;
                        gi.field_k.b(var3, var4, var7);
                        var6 = gi.field_a + -140;
                        if (-1 > (var6 ^ -1)) {
                          L14: {
                            var7 = 256;
                            if (20 > var6) {
                              var7 = 256 * var6 / 20;
                              break L14;
                            } else {
                              break L14;
                            }
                          }
                          gm.field_l.c(var3 - -15, var4 + 10, var5 * var7 >> -446111992);
                          break L13;
                        } else {
                          break L13;
                        }
                      }
                      return;
                    } else {
                      L15: {
                        gi.field_k.b(var3, var4, 256);
                        var6 = gi.field_a + -140;
                        if (-1 > (var6 ^ -1)) {
                          L16: {
                            var7 = 256;
                            if (20 > var6) {
                              var7 = 256 * var6 / 20;
                              break L16;
                            } else {
                              break L16;
                            }
                          }
                          gm.field_l.c(var3 - -15, var4 + 10, var5 * var7 >> -446111992);
                          break L15;
                        } else {
                          break L15;
                        }
                      }
                      return;
                    }
                  } else {
                    L17: {
                      var7 = 256 * var6 / 20;
                      gi.field_k.b(var3, var4, var7);
                      var6 = gi.field_a + -140;
                      if (-1 > (var6 ^ -1)) {
                        L18: {
                          var7 = 256;
                          if (20 > var6) {
                            var7 = 256 * var6 / 20;
                            break L18;
                          } else {
                            break L18;
                          }
                        }
                        gm.field_l.c(var3 - -15, var4 + 10, var5 * var7 >> -446111992);
                        break L17;
                      } else {
                        break L17;
                      }
                    }
                    return;
                  }
                } else {
                  L19: {
                    var6 = gi.field_a + -140;
                    if (-1 > (var6 ^ -1)) {
                      L20: {
                        var7 = 256;
                        if (20 > var6) {
                          var7 = 256 * var6 / 20;
                          break L20;
                        } else {
                          break L20;
                        }
                      }
                      gm.field_l.c(var3 - -15, var4 + 10, var5 * var7 >> -446111992);
                      break L19;
                    } else {
                      break L19;
                    }
                  }
                  return;
                }
              } else {
                L21: {
                  var6 = gi.field_a + -140;
                  if (-1 > (var6 ^ -1)) {
                    L22: {
                      var7 = 256;
                      if (20 > var6) {
                        var7 = 256 * var6 / 20;
                        break L22;
                      } else {
                        break L22;
                      }
                    }
                    gm.field_l.c(var3 - -15, var4 + 10, var5 * var7 >> -446111992);
                    break L21;
                  } else {
                    break L21;
                  }
                }
                return;
              }
            } else {
              return;
            }
          }
        }
    }

    final int a(int param0, int param1) {
        int var3;
        int var4;
        int var5;
        if (param1 == -140) {
          var3 = (this.field_f.length >> 218784193) - 1;
          var4 = param0 & var3;
          L0: while (true) {
            var5 = this.field_f[1 + var4 - -var4];
            if ((var5 ^ -1) == 0) {
              return -1;
            } else {
              if (param0 != this.field_f[var4 - -var4]) {
                var4 = var4 - -1 & var3;
                continue L0;
              } else {
                return var5;
              }
            }
          }
        } else {
          return -4;
        }
    }

    n(int[] param0) {
        int var2_int = 0;
        int var3 = 0;
        int var4 = 0;
        RuntimeException stackIn_16_0 = null;
        StringBuilder stackIn_16_1 = null;
        RuntimeException stackIn_17_0 = null;
        StringBuilder stackIn_17_1 = null;
        String stackIn_17_2 = null;
        RuntimeException decompiledCaughtException = null;
        RuntimeException var2 = null;
        try {
          L0: {
            var2_int = 1;
            L1: while (true) {
              if ((param0.length >> -66246431) + param0.length < var2_int) {
                this.field_f = new int[var2_int + var2_int];
                var3 = 0;
                L2: while (true) {
                  if (var2_int + var2_int <= var3) {
                    var3 = 0;
                    L3: while (true) {
                      if (param0.length <= var3) {
                        break L0;
                      } else {
                        var4 = param0[var3] & var2_int - 1;
                        L4: while (true) {
                          if ((this.field_f[1 + (var4 + var4)] ^ -1) == 0) {
                            this.field_f[var4 - -var4] = param0[var3];
                            this.field_f[1 + var4 + var4] = var3;
                            var3++;
                            continue L3;
                          } else {
                            var4 = 1 + var4 & var2_int - 1;
                            continue L4;
                          }
                        }
                      }
                    }
                  } else {
                    this.field_f[var3] = -1;
                    var3++;
                    continue L2;
                  }
                }
              } else {
                var2_int = var2_int << 1;
                continue L1;
              }
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L5: {
            var2 = decompiledCaughtException;
            stackIn_16_0 = (RuntimeException) (var2);

            stackIn_16_1 = new StringBuilder().append("n.<init>(");

            if (param0 == null) {
              stackIn_17_0 = (RuntimeException) ((Object) stackIn_16_0);
              stackIn_17_1 = (StringBuilder) ((Object) stackIn_16_1);
              stackIn_17_2 = "null";
              break L5;
            } else {
              stackIn_17_0 = (RuntimeException) ((Object) stackIn_16_0);
              stackIn_17_1 = (StringBuilder) ((Object) stackIn_16_1);
              stackIn_17_2 = "{...}";
              break L5;
            }
          }
          throw aa.a((Throwable) ((Object) stackIn_17_0), stackIn_17_2 + ')');
        }
    }

    static {
        field_c = "RuneScape clan";
        field_h = "Unrated game";
        field_b = new ri();
        field_d = new oc();
        field_e = "Game interface:";
    }
}
