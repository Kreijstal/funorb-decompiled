/*
 * Decompiled by CFR-JS 0.4.0.
 */
final class gb {
    static int field_b;
    static int field_d;
    static String field_e;
    private int field_a;
    private qe field_c;
    static String field_h;
    static String[] field_g;
    private ek field_i;
    private int field_f;

    private final void a(nf param0, int param1) {
        RuntimeException stackIn_8_0 = null;
        StringBuilder stackIn_8_1 = null;
        RuntimeException stackIn_9_0 = null;
        StringBuilder stackIn_9_1 = null;
        String stackIn_9_2 = null;
        RuntimeException decompiledCaughtException = null;
        RuntimeException var3 = null;
        try {
          L0: {
            L1: {
              if (param1 == 0) {
                break L1;
              } else {
                field_g = (String[]) null;
                break L1;
              }
            }
            L2: {
              if (param0 != null) {
                param0.b(57);
                param0.c(-1);
                this.field_f = this.field_f + param0.field_m;
                break L2;
              } else {
                break L2;
              }
            }
            break L0;
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L3: {
            var3 = decompiledCaughtException;
            stackIn_8_0 = (RuntimeException) (var3);

            stackIn_8_1 = new StringBuilder().append("gb.H(");

            if (param0 == null) {
              stackIn_9_0 = (RuntimeException) ((Object) stackIn_8_0);
              stackIn_9_1 = (StringBuilder) ((Object) stackIn_8_1);
              stackIn_9_2 = "null";
              break L3;
            } else {
              stackIn_9_0 = (RuntimeException) ((Object) stackIn_8_0);
              stackIn_9_1 = (StringBuilder) ((Object) stackIn_8_1);
              stackIn_9_2 = "{...}";
              break L3;
            }
          }
          throw dd.a((Throwable) ((Object) stackIn_9_0), stackIn_9_2 + ',' + param1 + ')');
        }
    }

    private final void a(int param0, Object param1, int param2, long param3) {
        nf var6 = null;
        lc var6_ref = null;
        int var7 = OrbDefence.field_D ? 1 : 0;
        try {
            if (!(this.field_a >= param2)) {
                throw new IllegalStateException();
            }
            this.a(2, param3);
            this.field_f = this.field_f - param2;
            while (this.field_f < 0) {
                var6 = (nf) ((Object) this.field_c.a(-39));
                this.a(var6, param0 ^ 0);
            }
            var6_ref = new lc(param1, param2);
            this.field_i.a(var6_ref, param0 + -124, param3);
            this.field_c.a(var6_ref, 21693);
            ((nf) ((Object) var6_ref)).field_h = (long)param0;
        } catch (RuntimeException runtimeException) {
            throw dd.a((Throwable) ((Object) runtimeException), "gb.D(" + param0 + ',' + (param1 != null ? "{...}" : "null") + ',' + param2 + ',' + param3 + ')');
        }
    }

    final Object a(long param0, int param1) {
        lc var6 = null;
        nf var7 = (nf) ((Object) this.field_i.a(param0, param1 + -1));
        if (var7 == null) {
            return null;
        }
        Object var5 = var7.f(param1);
        if (!(var5 != null)) {
            var7.b(57);
            var7.c(-1);
            this.field_f = this.field_f + var7.field_m;
            return null;
        }
        if (var7.g(param1 ^ 9)) {
            var6 = new lc(var5, var7.field_m);
            this.field_i.a(var6, -128, var7.field_d);
            this.field_c.a(var6, 21693);
            ((nf) ((Object) var6)).field_h = 0L;
            var7.b(57);
            var7.c(-1);
        } else {
            this.field_c.a(var7, 21693);
            var7.field_h = 0L;
        }
        return var5;
    }

    private final void a(int param0, long param1) {
        nf var4 = (nf) ((Object) this.field_i.a(param1, -1));
        if (param0 != 2) {
            this.field_c = (qe) null;
        }
        this.a(var4, 0);
    }

    public static void a(int param0) {
        field_g = null;
        field_h = null;
        if (param0 != 2867) {
            gb.b(-52);
        }
        field_e = null;
    }

    final static void a(boolean param0, int param1, boolean param2) {
        int stackIn_4_0 = 0;
        int stackIn_33_0 = 0;
        int stackIn_36_0 = 0;
        String[] stackIn_43_0 = null;
        int stackIn_43_1 = 0;
        String[] stackIn_44_0 = null;
        int stackIn_44_1 = 0;
        String stackIn_44_2 = null;
        String[] stackIn_48_0 = null;
        int stackIn_48_1 = 0;
        int stackIn_48_2 = 0;
        int stackIn_48_3 = 0;
        String stackIn_51_2 = null;
        int stackIn_93_0 = 0;
        int stackIn_114_0 = 0;
        int stackIn_115_0 = 0;
        int stackIn_115_1 = 0;
        Throwable caughtException = null;
        RuntimeException decompiledCaughtException = null;
        int var3_int = 0;
        RuntimeException var3 = null;
        int var4 = 0;
        String[] var4_ref_String__ = null;
        int var5 = 0;
        int var6 = 0;
        int var7 = 0;
        int var8 = 0;
        int var9 = 0;
        String[] var10 = null;
        String var11 = null;
        String var12 = null;
        var9 = OrbDefence.field_D ? 1 : 0;
        try {
          L0: {
            L1: {
              kg.field_m = param1;
              if (!param0) {
                stackIn_4_0 = 0;
                break L1;
              } else {
                stackIn_4_0 = 1;
                break L1;
              }
            }
            L2: {
              pc.field_db = stackIn_4_0 != 0;
              if (kg.field_m == 0) {
                var3_int = th.a(jd.field_a, ta.field_y, li.field_c, vi.field_I, -60);
                var4 = var3_int + 3;
                ea.field_d = new String[var4];
                dd.field_B = new int[var4];
                var5 = 0;
                L3: while (true) {
                  if (var5 >= var4) {
                    gg.field_O = new int[2];
                    var5 = 0;
                    L4: while (true) {
                      if (var5 >= var3_int) {
                        ea.field_d[-3 + var4] = "";
                        ea.field_d[-2 + var4] = p.field_f;
                        dd.field_B[-2 + var4] = 0;
                        gg.field_O[0] = 4;
                        ea.field_d[-1 + var4] = th.field_d;
                        dd.field_B[var4 - 1] = 1;
                        gg.field_O[1] = 5;
                        break L2;
                      } else {
                        ea.field_d[var5] = li.field_c[var5];
                        var5++;
                        continue L4;
                      }
                    }
                  } else {
                    dd.field_B[var5] = -1;
                    var5++;
                    continue L3;
                  }
                }
              } else {
                if ((kg.field_m ^ -1) != -2) {
                  if (kg.field_m != 2) {
                    if (3 == kg.field_m) {
                      L5: {
                        if (wc.field_c.field_q) {
                          var3_int = th.a(jd.field_a, am.field_c, li.field_c, vi.field_I, -62);
                          break L5;
                        } else {
                          var3_int = th.a(jd.field_a, dg.field_C, li.field_c, vi.field_I, -98);
                          break L5;
                        }
                      }
                      var4 = 2 - -var3_int;
                      ea.field_d = new String[var4];
                      dd.field_B = new int[var4];
                      var5 = 0;
                      L6: while (true) {
                        if (var4 <= var5) {
                          gg.field_O = new int[1];
                          var5 = 0;
                          L7: while (true) {
                            if (var5 >= var3_int) {
                              ea.field_d[var4 + -2] = "";
                              ea.field_d[var4 + -1] = th.field_d;
                              dd.field_B[-1 + var4] = 0;
                              gg.field_O[0] = 5;
                              break L2;
                            } else {
                              ea.field_d[var5] = li.field_c[var5];
                              var5++;
                              continue L7;
                            }
                          }
                        } else {
                          dd.field_B[var5] = -1;
                          var5++;
                          continue L6;
                        }
                      }
                    } else {
                      if (4 != kg.field_m) {
                        if ((kg.field_m ^ -1) == -6) {
                          var3_int = th.a(jd.field_a, lh.field_a, li.field_c, vi.field_I, -52);
                          var4 = var3_int + 3;
                          dd.field_B = new int[var4];
                          ea.field_d = new String[var4];
                          var5 = 0;
                          L8: while (true) {
                            if (var5 >= var4) {
                              gg.field_O = new int[2];
                              var5 = 0;
                              L9: while (true) {
                                if (var5 >= var3_int) {
                                  ea.field_d[var4 - 3] = "";
                                  ea.field_d[var4 + -2] = ge.field_o;
                                  dd.field_B[-2 + var4] = 0;
                                  gg.field_O[0] = 3;
                                  ea.field_d[var4 - 1] = th.field_d;
                                  dd.field_B[-1 + var4] = 1;
                                  gg.field_O[1] = 5;
                                  break L2;
                                } else {
                                  ea.field_d[var5] = li.field_c[var5];
                                  var5++;
                                  continue L9;
                                }
                              }
                            } else {
                              dd.field_B[var5] = -1;
                              var5++;
                              continue L8;
                            }
                          }
                        } else {
                          throw new IllegalArgumentException();
                        }
                      } else {
                        var3_int = th.a(jd.field_a, fi.field_a, li.field_c, vi.field_I, -89);
                        var4 = var3_int + 2;
                        ea.field_d = new String[var4];
                        dd.field_B = new int[var4];
                        var5 = 0;
                        L10: while (true) {
                          if (var5 >= var4) {
                            gg.field_O = new int[1];
                            var5 = 0;
                            L11: while (true) {
                              if (var5 >= var3_int) {
                                ea.field_d[-2 + var4] = "";
                                ea.field_d[-1 + var4] = th.field_d;
                                dd.field_B[var4 + -1] = 0;
                                gg.field_O[0] = 5;
                                break L2;
                              } else {
                                ea.field_d[var5] = li.field_c[var5];
                                var5++;
                                continue L11;
                              }
                            }
                          } else {
                            dd.field_B[var5] = -1;
                            var5++;
                            continue L10;
                          }
                        }
                      }
                    }
                  } else {
                    var3_int = th.a(jd.field_a, t.a(-415993727, new String[]{"<br><%0><br>"}, mc.field_f), li.field_c, vi.field_I, -105);
                    var4 = -1;
                    var5 = 0;
                    L12: while (true) {
                      L13: {
                        if (var3_int <= var5) {
                          break L13;
                        } else {
                          if ("<%0>".equals(li.field_c[var5])) {
                            var4 = var5;
                            break L13;
                          } else {
                            var5++;
                            continue L12;
                          }
                        }
                      }
                      if ((var4 ^ -1) != 0) {
                        pj.field_o = new String[var4];
                        mk.a(li.field_c, 0, pj.field_o, 0, var4);
                        bc.field_t = new String[var3_int + -var4 + -1];
                        mk.a(li.field_c, 1 + var4, bc.field_t, 0, var3_int + (-var4 + -1));
                        var4 = -1;
                        var3_int = th.a(jd.field_a, t.a(-415993727, new String[]{"<br><%0><br>"}, qk.field_c), li.field_c, vi.field_I, -108);
                        var5 = 0;
                        L14: while (true) {
                          L15: {
                            if (var5 >= var3_int) {
                              break L15;
                            } else {
                              if ("<%0>".equals(li.field_c[var5])) {
                                var4 = var5;
                                break L15;
                              } else {
                                var5++;
                                continue L14;
                              }
                            }
                          }
                          if (-1 != var4) {
                            L16: {
                              fd.field_e = new String[var4];
                              mk.a(li.field_c, 0, fd.field_e, 0, var4);
                              sj.field_u = new String[-1 + var3_int - var4];
                              mk.a(li.field_c, var4 - -1, sj.field_u, 0, -1 + (-var4 + var3_int));
                              if (fd.field_e.length <= pj.field_o.length) {
                                stackIn_33_0 = pj.field_o.length;
                                break L16;
                              } else {
                                stackIn_33_0 = fd.field_e.length;
                                break L16;
                              }
                            }
                            L17: {
                              var5 = stackIn_33_0;
                              if (sj.field_u.length > bc.field_t.length) {
                                stackIn_36_0 = sj.field_u.length;
                                break L17;
                              } else {
                                stackIn_36_0 = bc.field_t.length;
                                break L17;
                              }
                            }
                            var6 = stackIn_36_0;
                            var7 = var6 + (var5 + 7);
                            dd.field_B = new int[var7];
                            ea.field_d = new String[var7];
                            var8 = 0;
                            L18: while (true) {
                              if (var7 <= var8) {
                                ea.field_d[0] = sa.field_g;
                                dd.field_B[1] = 0;
                                ea.field_d[1] = lc.field_s;
                                gg.field_O = new int[2];
                                ea.field_d[3] = mj.field_k;
                                dd.field_B[3] = 1;
                                ea.field_d[4] = te.field_J;
                                ea.field_d[2] = ii.field_a;
                                gg.field_O[1] = 2;
                                gg.field_O[0] = 5;
                                ea.field_d[5] = "";
                                var8 = 0;
                                L19: while (true) {
                                  if (var8 >= var5) {
                                    ea.field_d[var5 + 6] = null;
                                    dd.field_B[6 + var5] = -2;
                                    var8 = 0;
                                    L20: while (true) {
                                      if (var8 >= var6) {
                                        am.field_a = ji.b(111);
                                        break L2;
                                      } else {
                                        stackIn_48_0 = ea.field_d;
                                        stackIn_48_1 = 7 + var5 + var8;
                                        stackIn_48_2 = sj.field_u.length;
                                        stackIn_48_3 = var8;
                                        L21: {




                                          if (stackIn_48_2 > stackIn_48_3) {
                                            stackIn_48_0 = (String[]) ((Object) stackIn_48_0);

                                            stackIn_51_2 = sj.field_u[var8];
                                            break L21;
                                          } else {
                                            stackIn_48_0 = (String[]) ((Object) stackIn_48_0);

                                            stackIn_51_2 = "";
                                            break L21;
                                          }
                                        }
                                        stackIn_48_0[stackIn_48_1] = stackIn_51_2;
                                        var8++;
                                        continue L20;
                                      }
                                    }
                                  } else {
                                    L22: {
                                      stackIn_43_0 = ea.field_d;

                                      stackIn_43_1 = var8 + 6;

                                      if (0 <= fd.field_e.length + (var8 + -var5)) {
                                        stackIn_44_0 = (String[]) ((Object) stackIn_43_0);
                                        stackIn_44_1 = stackIn_43_1;
                                        stackIn_44_2 = fd.field_e[var8 + fd.field_e.length - var5];
                                        break L22;
                                      } else {
                                        stackIn_44_0 = (String[]) ((Object) stackIn_43_0);
                                        stackIn_44_1 = stackIn_43_1;
                                        stackIn_44_2 = "";
                                        break L22;
                                      }
                                    }
                                    stackIn_44_0[stackIn_44_1] = stackIn_44_2;
                                    var8++;
                                    continue L19;
                                  }
                                }
                              } else {
                                dd.field_B[var8] = -1;
                                var8++;
                                continue L18;
                              }
                            }
                          } else {
                            throw new IllegalStateException();
                          }
                        }
                      } else {
                        throw new IllegalStateException();
                      }
                    }
                  }
                } else {
                  var3_int = th.a(jd.field_a, ta.field_y, li.field_c, vi.field_I, -77);
                  var4 = var3_int + 2;
                  ea.field_d = new String[var4];
                  dd.field_B = new int[var4];
                  var5 = 0;
                  L23: while (true) {
                    if (var5 >= var4) {
                      gg.field_O = new int[1];
                      var5 = 0;
                      L24: while (true) {
                        if (var5 >= var3_int) {
                          ea.field_d[-2 + var4] = "";
                          ea.field_d[var4 + -1] = th.field_d;
                          dd.field_B[-1 + var4] = 0;
                          gg.field_O[0] = 5;
                          break L2;
                        } else {
                          ea.field_d[var5] = li.field_c[var5];
                          var5++;
                          continue L24;
                        }
                      }
                    } else {
                      dd.field_B[var5] = -1;
                      var5++;
                      continue L23;
                    }
                  }
                }
              }
            }
            ab.field_j.field_i = gg.field_O.length;
            var3_int = 0;
            var4 = 0;
            L25: while (true) {
              if (var4 >= ea.field_d.length) {
                L26: {
                  if ((kg.field_m ^ -1) == -3) {
                    var10 = pj.field_o;
                    var4_ref_String__ = var10;
                    var5 = 0;
                    L27: while (true) {
                      if (var5 >= var10.length) {
                        var4_ref_String__ = bc.field_t;
                        var5 = 0;
                        L28: while (true) {
                          if (var4_ref_String__.length <= var5) {
                            break L26;
                          } else {
                            L29: {
                              var12 = var4_ref_String__[var5];
                              var7 = hg.a(false, var12, 0);
                              if ((var7 ^ -1) < (var3_int ^ -1)) {
                                var3_int = var7;
                                break L29;
                              } else {
                                break L29;
                              }
                            }
                            var5++;
                            continue L28;
                          }
                        }
                      } else {
                        L30: {
                          var11 = var10[var5];
                          var7 = hg.a(false, var11, 0);
                          if (var3_int < var7) {
                            var3_int = var7;
                            break L30;
                          } else {
                            break L30;
                          }
                        }
                        var5++;
                        continue L27;
                      }
                    }
                  } else {
                    break L26;
                  }
                }
                ha.field_o = -(var3_int >> 515316481) + fd.field_c;
                lc.field_q = -(var3_int >> -1860705023) + (fd.field_c - -var3_int);
                rf.field_C = (gg.field_N + ol.field_b << 2036715393) * ab.field_j.field_i;
                var4 = 0;
                L31: while (true) {
                  if (var4 >= ea.field_d.length) {
                    sd.field_h = sf.field_T - (rf.field_C >> 1076635041);
                    qd.field_d = new int[ea.field_d.length][];
                    var4 = 0;
                    var5 = sd.field_h;
                    L32: while (true) {
                      if (ea.field_d.length <= var4) {
                        L33: {
                          if (kg.field_m == 2) {
                            ab.field_j.a(-1, -1, 0, param2);
                            break L33;
                          } else {
                            ab.field_j.a(0, jc.a(ua.field_a, (byte) 71, vl.field_d), 0, param2);
                            break L33;
                          }
                        }
                        break L0;
                      } else {
                        L34: {
                          var6 = dd.field_B[var4];
                          if (var6 < 0) {
                            var5 = var5 + d.field_j;
                            break L34;
                          } else {
                            var7 = hg.a(true, ea.field_d[var4], 0);
                            var8 = fd.field_c + -(var7 >> 1482773409);
                            var5 = var5 + gg.field_N;
                            qd.field_d[var4] = new int[4];
                            qd.field_d[var4][0] = var8 + -mj.field_f;
                            qd.field_d[var4][1] = var5;
                            qd.field_d[var4][2] = var7 - -(mj.field_f << -89946207);
                            var5 = var5 + ((ol.field_b << 910348769) - -gg.field_N - -el.field_D);
                            qd.field_d[var4][3] = el.field_D - -(ol.field_b << -1051744479);
                            break L34;
                          }
                        }
                        var4++;
                        continue L32;
                      }
                    }
                  } else {
                    L35: {
                      stackIn_114_0 = rf.field_C;

                      if (-1 >= (dd.field_B[var4] ^ -1)) {
                        stackIn_115_0 = stackIn_114_0;
                        stackIn_115_1 = el.field_D;
                        break L35;
                      } else {
                        stackIn_115_0 = stackIn_114_0;
                        stackIn_115_1 = d.field_j;
                        break L35;
                      }
                    }
                    rf.field_C = stackIn_115_0 + stackIn_115_1;
                    var4++;
                    continue L31;
                  }
                }
              } else {
                L36: {
                  if (dd.field_B[var4] < 0) {
                    stackIn_93_0 = 0;
                    break L36;
                  } else {
                    stackIn_93_0 = 1;
                    break L36;
                  }
                }
                var5 = hg.a(stackIn_93_0 != 0, ea.field_d[var4], 0);
                if (var3_int < var5) {
                  var3_int = var5;
                  var4++;
                  continue L25;
                } else {
                  var4++;
                  continue L25;
                }
              }
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          var3 = decompiledCaughtException;
          throw dd.a((Throwable) ((Object) var3), "gb.F(" + param0 + ',' + param1 + ',' + param2 + ')');
        }
    }

    final void a(Object param0, byte param1, long param2) {
        try {
            this.a(0, param0, 1, param2);
            int var5_int = -24 % ((-26 - param1) / 63);
        } catch (RuntimeException runtimeException) {
            throw dd.a((Throwable) ((Object) runtimeException), "gb.G(" + (param0 != null ? "{...}" : "null") + ',' + param1 + ',' + param2 + ')');
        }
    }

    final static void b(int param0) {
        field_h = cd.field_t.g(2);
        CharSequence var2 = (CharSequence) ((Object) field_h);
        sg.field_E = ua.a(-88, var2);
        if (param0 > -27) {
            gb.b(-121);
        }
    }

    private gb() throws Throwable {
        throw new Error();
    }

    static {
        field_e = "WAVE";
        field_g = new String[]{"First Fight", "Pincer", "Breakout", "Dominion", "Gridlock", "Orb Central"};
    }
}
