/*
 * Decompiled by CFR-JS 0.4.0.
 */
class e {
    private static int field_l;
    private boolean field_i;
    static boolean field_s;
    private wf field_t;
    private int field_e;
    private long field_p;
    int[] field_m;
    private static dd field_k;
    static int field_r;
    private int field_b;
    private int field_g;
    private int field_a;
    private boolean field_q;
    private long field_h;
    private wf[] field_c;
    private int field_d;
    private int field_u;
    private int field_f;
    private wf[] field_o;
    private int field_n;
    private long field_j;

    final synchronized void d() {
        try {
            this.field_q = true;
            try {
                this.c();
            } catch (Exception exception) {
                this.g();
                this.field_h = fa.a(121) + 2000L;
            }
        } catch (RuntimeException | Error decompiledUncheckedException) {
            throw decompiledUncheckedException;
        } catch (Throwable decompiledCheckedException) {
            throw new RuntimeException(decompiledCheckedException);
        }
    }

    public static void h() {
        field_k = null;
    }

    private final void a(int[] param0, int param1) {
        int var3;
        int var4;
        int var5;
        int var6;
        int var7_int;
        Object var7;
        int var8_int;
        wf[] var8;
        int var9;
        Object var10;
        wf var11;
        ic var12;
        int var13;
        wf var14;
        int var15_int;
        wf var15;
        L0: {
          var3 = param1;
          if (!field_s) {
            break L0;
          } else {
            var3 = var3 << 1;
            break L0;
          }
        }
        L1: {
          cb.a(param0, 0, var3);
          this.field_b = this.field_b - param1;
          if (this.field_t == null) {
            break L1;
          } else {
            if (this.field_b > 0) {
              break L1;
            } else {
              this.field_b = this.field_b + (field_r >> 4);
              e.b(this.field_t);
              this.a(this.field_t, this.field_t.a());
              var4 = 0;
              var5 = 255;
              var6 = 7;
              L2: while (true) {
                L3: {
                  if (var5 == 0) {
                    break L3;
                  } else {
                    L4: {
                      if (var6 >= 0) {
                        var7_int = var6;
                        var8_int = 0;
                        break L4;
                      } else {
                        var7_int = var6 & 3;
                        var8_int = -(var6 >> 2);
                        break L4;
                      }
                    }
                    var9 = var5 >>> var7_int & 286331153;
                    L5: while (true) {
                      if (var9 != 0) {
                        L6: {
                          if ((var9 & 1) != 0) {
                            var5 = var5 & (1 << var7_int ^ -1);
                            var10 = null;
                            var11 = this.field_c[var7_int];
                            var14 = var11;
                            var14 = var11;
                            L7: while (true) {
                              if (var11 == null) {
                                break L6;
                              } else {
                                L8: {
                                  var12 = var11.field_j;
                                  if (var12 == null) {
                                    break L8;
                                  } else {
                                    if (var12.field_g <= var8_int) {
                                      break L8;
                                    } else {
                                      var5 = var5 | 1 << var7_int;
                                      var10 = var11;
                                      var11 = var11.field_h;
                                      continue L7;
                                    }
                                  }
                                }
                                L9: {
                                  var11.field_i = true;
                                  var13 = var11.b();
                                  var4 = var4 + var13;
                                  if (var12 == null) {
                                    break L9;
                                  } else {
                                    var12.field_g = var12.field_g + var13;
                                    break L9;
                                  }
                                }
                                if (var4 < this.field_e) {
                                  L10: {
                                    var14 = var11.d();
                                    if (var14 == null) {
                                      break L10;
                                    } else {
                                      var15_int = var11.field_g;
                                      L11: while (true) {
                                        if (var14 == null) {
                                          break L10;
                                        } else {
                                          this.a(var14, var15_int * var14.a() >> 8);
                                          var14 = var11.c();
                                          continue L11;
                                        }
                                      }
                                    }
                                  }
                                  L12: {
                                    var15 = var11.field_h;
                                    var11.field_h = null;
                                    if (var10 != null) {
                                      ((wf) (var10)).field_h = var15;
                                      break L12;
                                    } else {
                                      this.field_c[var7_int] = var15;
                                      break L12;
                                    }
                                  }
                                  L13: {
                                    if (var15 != null) {
                                      break L13;
                                    } else {
                                      this.field_o[var7_int] = (wf) (var10);
                                      break L13;
                                    }
                                  }
                                  var11 = var15;
                                  continue L7;
                                } else {
                                  break L3;
                                }
                              }
                            }
                          } else {
                            break L6;
                          }
                        }
                        var7_int += 4;
                        var8_int++;
                        var9 = var9 >>> 4;
                        continue L5;
                      } else {
                        var6--;
                        continue L2;
                      }
                    }
                  }
                }
                var6 = 0;
                L14: while (true) {
                  if (var6 >= 8) {
                    break L1;
                  } else {
                    var7 = this.field_c[var6];
                    var8 = this.field_c;
                    var9 = var6;
                    this.field_o[var6] = null;
                    var8[var9] = null;
                    L15: while (true) {
                      if (var7 == null) {
                        var6++;
                        continue L14;
                      } else {
                        var10 = ((wf) (var7)).field_h;
                        ((wf) (var7)).field_h = null;
                        var7 = var10;
                        continue L15;
                      }
                    }
                  }
                }
              }
            }
          }
        }
        L16: {
          if (this.field_b >= 0) {
            break L16;
          } else {
            this.field_b = 0;
            break L16;
          }
        }
        L17: {
          if (this.field_t == null) {
            break L17;
          } else {
            this.field_t.a(param0, 0, param1);
            break L17;
          }
        }
        this.field_p = fa.a(-49);
    }

    int b() throws Exception {
        return this.field_d;
    }

    void b(int param0) throws Exception {
    }

    void c() throws Exception {
    }

    final synchronized void a() {
        int var1;
        int var2;
        L0: {
          if (field_k == null) {
            break L0;
          } else {
            var1 = 1;
            var2 = 0;
            L1: while (true) {
              if (var2 >= 2) {
                if (var1 == 0) {
                  break L0;
                } else {
                  field_k.field_c = true;
                  L2: while (true) {
                    if (!field_k.field_b) {
                      field_k = null;
                      break L0;
                    } else {
                      wh.a(50L, 19406);
                      continue L2;
                    }
                  }
                }
              } else {
                L3: {
                  if (field_k.field_a[var2] != this) {
                    break L3;
                  } else {
                    field_k.field_a[var2] = null;
                    break L3;
                  }
                }
                if (field_k.field_a[var2] != null) {
                  var1 = 0;
                  var2++;
                  continue L1;
                } else {
                  var2++;
                  continue L1;
                }
              }
            }
          }
        }
        this.g();
        this.field_m = null;
        this.field_i = true;
    }

    final static void a(int param0, boolean param1, int param2) {
        if (param0 < 8000 || param0 > 48000) {
            throw new IllegalArgumentException();
        }
        field_r = param0;
        field_s = param1 ? true : false;
        field_l = param2;
    }

    private final static void b(wf param0) {
        param0.field_i = false;
        if (param0.field_j != null) {
            param0.field_j.field_g = 0;
        }
        wf var1 = param0.d();
        while (var1 != null) {
            e.b(var1);
            var1 = param0.c();
        }
    }

    final synchronized void e() {
        try {
            int decompiledRegionSelector0 = 0;
            Throwable decompiledCaughtException = null;
            long var1 = 0L;
            Exception var3 = null;
            int var3_int = 0;
            int var4 = 0;
            if (!this.field_i) {
              var1 = fa.a(-114);
              try {
                L0: {
                  L1: {
                    if (var1 <= this.field_p + 6000L) {
                      break L1;
                    } else {
                      this.field_p = var1 - 6000L;
                      break L1;
                    }
                  }
                  L2: while (true) {
                    if (var1 <= this.field_p + 5000L) {
                      break L0;
                    } else {
                      this.a(256);
                      this.field_p = this.field_p + (long)(256000 / field_r);
                      var1 = fa.a(-87);
                      continue L2;
                    }
                  }
                }
              } catch (java.lang.Exception decompiledCaughtParameter0) {
                decompiledCaughtException = decompiledCaughtParameter0;
                L3: {
                  var3 = (Exception) (Object) decompiledCaughtException;
                  this.field_p = var1;
                  break L3;
                }
              }
              if (this.field_m != null) {
                try {
                  L4: {
                    L5: {
                      if (this.field_h == 0L) {
                        break L5;
                      } else {
                        if (var1 >= this.field_h) {
                          this.b(this.field_d);
                          this.field_h = 0L;
                          this.field_q = true;
                          break L5;
                        } else {
                          decompiledRegionSelector0 = 0;
                          break L4;
                        }
                      }
                    }
                    L6: {
                      var3_int = this.b();
                      if (this.field_a - var3_int <= this.field_u) {
                        break L6;
                      } else {
                        this.field_u = this.field_a - var3_int;
                        break L6;
                      }
                    }
                    L7: {
                      var4 = this.field_g + this.field_n;
                      if (var4 + 256 <= 16384) {
                        break L7;
                      } else {
                        var4 = 16128;
                        break L7;
                      }
                    }
                    L8: {
                      if (var4 + 256 <= this.field_d) {
                        break L8;
                      } else {
                        L9: {
                          this.field_d = this.field_d + 1024;
                          if (this.field_d <= 16384) {
                            break L9;
                          } else {
                            this.field_d = 16384;
                            break L9;
                          }
                        }
                        this.g();
                        this.b(this.field_d);
                        var3_int = 0;
                        this.field_q = true;
                        if (var4 + 256 <= this.field_d) {
                          break L8;
                        } else {
                          var4 = this.field_d - 256;
                          this.field_n = var4 - this.field_g;
                          break L8;
                        }
                      }
                    }
                    L10: while (true) {
                      if (var3_int >= var4) {
                        L11: {
                          if (var1 <= this.field_j) {
                            break L11;
                          } else {
                            L12: {
                              if (this.field_q) {
                                this.field_q = false;
                                break L12;
                              } else {
                                L13: {
                                  if (this.field_u != 0) {
                                    break L13;
                                  } else {
                                    if (this.field_f != 0) {
                                      break L13;
                                    } else {
                                      this.g();
                                      this.field_h = var1 + 2000L;
                                      decompiledRegionSelector0 = 1;
                                      break L4;
                                    }
                                  }
                                }
                                this.field_n = Math.min(this.field_f, this.field_u);
                                this.field_f = this.field_u;
                                break L12;
                              }
                            }
                            this.field_u = 0;
                            this.field_j = var1 + 2000L;
                            break L11;
                          }
                        }
                        this.field_a = var3_int;
                        decompiledRegionSelector0 = 2;
                        break L4;
                      } else {
                        this.a(this.field_m, 256);
                        this.f();
                        var3_int += 256;
                        continue L10;
                      }
                    }
                  }
                } catch (java.lang.Exception decompiledCaughtParameter1) {
                  decompiledCaughtException = decompiledCaughtParameter1;
                  L14: {
                    var3 = (Exception) (Object) decompiledCaughtException;
                    this.g();
                    this.field_h = var1 + 2000L;
                    decompiledRegionSelector0 = 2;
                    break L14;
                  }
                }
                if (decompiledRegionSelector0 == 0) {
                  return;
                } else {
                  if (decompiledRegionSelector0 == 1) {
                    return;
                  } else {
                    return;
                  }
                }
              } else {
                return;
              }
            } else {
              return;
            }
        } catch (RuntimeException | Error decompiledUncheckedException) {
            throw decompiledUncheckedException;
        } catch (Throwable decompiledCheckedException) {
            throw new RuntimeException(decompiledCheckedException);
        }
    }

    final static e a(si param0, java.awt.Component param1, int param2, int param3) {
        try {
            ia var4 = null;
            Throwable var4_ref = null;
            ia var5 = null;
            ia stackIn_10_0 = null;
            int stackIn_10_1 = 0;
            ia stackIn_11_0 = null;
            int stackIn_11_1 = 0;
            int stackIn_11_2 = 0;
            ia stackIn_21_0 = null;
            Throwable decompiledCaughtException = null;
            if (field_r != 0) {
              L0: {
                if (param2 < 0) {
                  break L0;
                } else {
                  if (param2 < 2) {
                    L1: {
                      if (param3 >= 256) {
                        break L1;
                      } else {
                        param3 = 256;
                        break L1;
                      }
                    }
                    try {
                      L2: {
                        L3: {
                          var5 = new ia();
                          var4 = var5;
                          stackIn_10_0 = (ia) (var4);

                          stackIn_10_1 = 256;

                          if (!field_s) {
                            stackIn_11_0 = (ia) ((Object) stackIn_10_0);
                            stackIn_11_1 = stackIn_10_1;
                            stackIn_11_2 = 1;
                            break L3;
                          } else {
                            stackIn_11_0 = (ia) ((Object) stackIn_10_0);
                            stackIn_11_1 = stackIn_10_1;
                            stackIn_11_2 = 2;
                            break L3;
                          }
                        }
                        L4: {
                          ((e) ((Object) stackIn_11_0)).field_m = new int[stackIn_11_1 * stackIn_11_2];
                          ((e) ((Object) var4)).field_g = param3;
                          ((e) ((Object) var4)).a(param1);
                          ((e) ((Object) var4)).field_d = (param3 & -1024) + 1024;
                          if (((e) ((Object) var4)).field_d <= 16384) {
                            break L4;
                          } else {
                            ((e) ((Object) var4)).field_d = 16384;
                            break L4;
                          }
                        }
                        L5: {
                          ((e) ((Object) var4)).b(((e) ((Object) var4)).field_d);
                          if (field_l <= 0) {
                            break L5;
                          } else {
                            if (field_k != null) {
                              break L5;
                            } else {
                              field_k = new dd();
                              field_k.field_d = param0;
                              param0.a(field_l, (byte) 126, (Runnable) ((Object) field_k));
                              break L5;
                            }
                          }
                        }
                        L6: {
                          if (field_k == null) {
                            break L6;
                          } else {
                            if (field_k.field_a[param2] == null) {
                              field_k.field_a[param2] = (e) ((Object) var5);
                              break L6;
                            } else {
                              throw new IllegalArgumentException();
                            }
                          }
                        }
                        stackIn_21_0 = (ia) (var4);
                        break L2;
                      }
                    } catch (java.lang.Throwable decompiledCaughtParameter0) {
                      decompiledCaughtException = decompiledCaughtParameter0;
                      var4_ref = decompiledCaughtException;
                      return new e();
                    }
                    return (e) ((Object) stackIn_21_0);
                  } else {
                    break L0;
                  }
                }
              }
              throw new IllegalArgumentException();
            } else {
              throw new IllegalStateException();
            }
        } catch (RuntimeException | Error decompiledUncheckedException) {
            throw decompiledUncheckedException;
        } catch (Throwable decompiledCheckedException) {
            throw new RuntimeException(decompiledCheckedException);
        }
    }

    private final void a(int param0) {
        this.field_b = this.field_b - param0;
        if (this.field_b < 0) {
            this.field_b = 0;
        }
        if (this.field_t != null) {
            this.field_t.a(param0);
            return;
        }
    }

    void g() {
    }

    void f() throws Exception {
    }

    private final void a(wf param0, int param1) {
        int var3 = param1 >> 5;
        wf var4 = this.field_o[var3];
        if (var4 == null) {
            this.field_c[var3] = param0;
        } else {
            var4.field_h = param0;
        }
        this.field_o[var3] = param0;
        param0.field_g = param1;
    }

    void a(java.awt.Component param0) throws Exception {
    }

    final synchronized void a(wf param0) {
        this.field_t = param0;
    }

    e() {
        this.field_i = false;
        this.field_e = 32;
        this.field_p = fa.a(-78);
        this.field_q = true;
        this.field_b = 0;
        this.field_c = new wf[8];
        this.field_a = 0;
        this.field_f = 0;
        this.field_o = new wf[8];
        this.field_j = 0L;
        this.field_h = 0L;
        this.field_u = 0;
    }
}
