/*
 * Decompiled by CFR-JS 0.4.0.
 */
import java.io.*;

final class ti extends ba implements wc {
    static int field_O;
    private uh field_N;
    private int field_M;

    final String d(int param0) {
        lk var3;
        if (param0 == 0) {
          if (this.field_g) {
            if (null == this.field_q) {
              return null;
            } else {
              ll.a(uc.field_C - (this.field_M + -this.field_k), true, ll.field_y);
              return this.field_q;
            }
          } else {
            return null;
          }
        } else {
          var3 = (lk) null;
          this.a(57, 60, -66, (lk) null);
          if (this.field_g) {
            if (null == this.field_q) {
              return null;
            } else {
              ll.a(uc.field_C - (this.field_M + -this.field_k), true, ll.field_y);
              return this.field_q;
            }
          } else {
            return null;
          }
        }
    }

    final void a(int param0, int param1, int param2, lk param3) {
        try {
            super.a(param0 ^ 0, param1, param2, param3);
            if (param0 != -1) {
                field_O = 53;
            }
            this.field_M = -param1 + (uc.field_C - this.field_r);
        } catch (RuntimeException runtimeException) {
            throw ii.a((Throwable) ((Object) runtimeException), "ti.P(" + param0 + ',' + param1 + ',' + param2 + ',' + (param3 != null ? "{...}" : "null") + ')');
        }
    }

    final void a(uh param0, boolean param1) {
        try {
            if (!param1) {
                String var4 = (String) null;
                ti.a((Throwable) null, (byte) 122, (String) null);
            }
            this.field_N = param0;
        } catch (RuntimeException runtimeException) {
            throw ii.a((Throwable) ((Object) runtimeException), "ti.N(" + (param0 != null ? "{...}" : "null") + ',' + param1 + ')');
        }
    }

    final void h(byte param0) {
        super.h(param0);
        if (!(null == this.field_N)) {
            this.field_N.d((byte) 115);
        }
    }

    final static void a(Throwable param0, byte param1, String param2) {
        try {
            si stackIn_15_0;
            java.net.URL stackIn_15_1;
            java.net.URL stackIn_15_2;
            java.net.URL stackIn_15_3;
            StringBuilder stackIn_15_4;
            si stackIn_16_0;
            java.net.URL stackIn_16_1;
            java.net.URL stackIn_16_2;
            java.net.URL stackIn_16_3;
            StringBuilder stackIn_16_4;
            String stackIn_16_5;
            int decompiledRegionSelector0 = 0;
            Throwable decompiledCaughtException = null;
            String var3 = null;
            Exception var3_ref = null;
            mk var4 = null;
            DataInputStream var5 = null;
            int var6 = 0;
            String var7 = null;
            String var8 = null;
            String var9 = null;
            String var10 = null;
            var6 = Bounce.field_N;
            try {
              L0: {
                L1: {
                  var3 = "";
                  if (param0 != null) {
                    var3 = ae.a((byte) -114, param0);
                    break L1;
                  } else {
                    break L1;
                  }
                }
                L2: {
                  if (param2 != null) {
                    L3: {
                      if (param0 == null) {
                        break L3;
                      } else {
                        var3 = var3 + " | ";
                        break L3;
                      }
                    }
                    var3 = var3 + param2;
                    break L2;
                  } else {
                    break L2;
                  }
                }
                if (param1 == 100) {
                  ej.a(param1 ^ 0, var3);
                  var7 = ak.a(param1 + -57, ":", "%3a", var3);
                  var8 = ak.a(15, "@", "%40", var7);
                  var9 = ak.a(88, "&", "%26", var8);
                  var10 = ak.a(param1 ^ 48, "#", "%23", var9);
                  if (gk.field_j != null) {
                    L4: {
                      stackIn_15_0 = jj.field_a;

                      stackIn_15_1 = null;

                      stackIn_15_2 = null;

                      stackIn_15_3 = gk.field_j.getCodeBase();

                      stackIn_15_4 = new StringBuilder().append("clienterror.ws?c=").append(nd.field_E).append("&u=");

                      if (ke.field_m != null) {
                        stackIn_16_0 = (si) ((Object) stackIn_15_0);
                        stackIn_16_1 = null;
                        stackIn_16_2 = null;
                        stackIn_16_3 = (java.net.URL) ((Object) stackIn_15_3);
                        stackIn_16_4 = (StringBuilder) ((Object) stackIn_15_4);
                        stackIn_16_5 = ke.field_m;
                        break L4;
                      } else {
                        stackIn_16_0 = (si) ((Object) stackIn_15_0);
                        stackIn_16_1 = null;
                        stackIn_16_2 = null;
                        stackIn_16_3 = (java.net.URL) ((Object) stackIn_15_3);
                        stackIn_16_4 = (StringBuilder) ((Object) stackIn_15_4);
                        stackIn_16_5 = "" + al.field_h;
                        break L4;
                      }
                    }
                    var4 = ((si) (Object) stackIn_16_0).a(new java.net.URL(stackIn_16_3, stackIn_16_5 + "&v1=" + si.field_r + "&v2=" + si.field_d + "&e=" + var10), false);
                    L5: while (true) {
                      if (0 != var4.field_f) {
                        if (1 == var4.field_f) {
                          var5 = (DataInputStream) (var4.field_e);
                          var5.read();
                          var5.close();
                          decompiledRegionSelector0 = 2;
                          break L0;
                        } else {
                          return;
                        }
                      } else {
                        wh.a(1L, 19406);
                        continue L5;
                      }
                    }
                  } else {
                    decompiledRegionSelector0 = 1;
                    break L0;
                  }
                } else {
                  decompiledRegionSelector0 = 0;
                  break L0;
                }
              }
            } catch (java.lang.Exception decompiledCaughtParameter0) {
              decompiledCaughtException = decompiledCaughtParameter0;
              var3_ref = (Exception) (Object) decompiledCaughtException;
              return;
            }
            if (decompiledRegionSelector0 == 0) {
              return;
            } else {
              if (decompiledRegionSelector0 == 1) {
                return;
              } else {
                return;
              }
            }
        } catch (RuntimeException | Error decompiledUncheckedException) {
            throw decompiledUncheckedException;
        } catch (Throwable decompiledCheckedException) {
            throw new RuntimeException(decompiledCheckedException);
        }
    }

    final static void l(int param0) {
        int stackIn_20_0 = 0;
        String stackIn_26_0 = null;
        int stackIn_30_0 = 0;
        String stackIn_36_0 = null;
        int stackIn_42_0 = 0;
        oh stackIn_47_0 = null;
        int decompiledRegionSelector0 = 0;
        RuntimeException decompiledCaughtException = null;
        int var1_int = 0;
        RuntimeException var1 = null;
        int var2 = 0;
        long var3_long = 0L;
        int var3 = 0;
        int var4 = 0;
        int var5 = 0;
        String var6 = null;
        int var7 = 0;
        int var8 = 0;
        oh var9 = null;
        int var10 = 0;
        var10 = Bounce.field_N;
        try {
          L0: {
            var1_int = jl.field_a;
            if (param0 == 6) {
              L1: {
                var2 = 0;
                if (ne.field_u != 2) {
                  break L1;
                } else {
                  var3_long = fa.a(-23) + -gd.field_a;
                  var2 = (int)((10999L + -var3_long) / 1000L);
                  if (0 > var2) {
                    var2 = 0;
                    break L1;
                  } else {
                    break L1;
                  }
                }
              }
              var3 = 0;
              L2: while (true) {
                if (se.field_E.length <= var3) {
                  decompiledRegionSelector0 = 1;
                  break L0;
                } else {
                  L3: {
                    var4 = m.field_a[var3];
                    if (var4 < 0) {
                      var5 = bc.field_a;
                      break L3;
                    } else {
                      if (var4 != cf.field_f.field_b) {
                        var5 = de.field_f;
                        break L3;
                      } else {
                        var5 = ag.field_c;
                        break L3;
                      }
                    }
                  }
                  L4: {
                    var6 = se.field_E[var3];
                    if (-3 != (ne.field_u ^ -1)) {
                      break L4;
                    } else {
                      if ((var2 ^ -1) == -2) {
                        L5: {
                          if (t.field_r.length > eb.field_E.length) {
                            stackIn_20_0 = t.field_r.length;
                            break L5;
                          } else {
                            stackIn_20_0 = eb.field_E.length;
                            break L5;
                          }
                        }
                        L6: {
                          var7 = stackIn_20_0;
                          if (6 > var3) {
                            break L6;
                          } else {
                            if (var3 < var7 + 6) {
                              L7: {
                                if ((eb.field_E.length + (var3 + -6 - var7) ^ -1) > -1) {
                                  stackIn_26_0 = "";
                                  break L7;
                                } else {
                                  stackIn_26_0 = eb.field_E[-var7 + (var3 + -6 - -eb.field_E.length)];
                                  break L7;
                                }
                              }
                              var6 = stackIn_26_0;
                              break L6;
                            } else {
                              break L6;
                            }
                          }
                        }
                        L8: {
                          if (ll.field_E.length >= vb.field_k.length) {
                            stackIn_30_0 = ll.field_E.length;
                            break L8;
                          } else {
                            stackIn_30_0 = vb.field_k.length;
                            break L8;
                          }
                        }
                        var8 = stackIn_30_0;
                        if (var3 < 7 - -var7) {
                          break L4;
                        } else {
                          if (var3 < var8 + (var7 + 7)) {
                            L9: {
                              if (var3 - 7 + -var7 < ll.field_E.length) {
                                stackIn_36_0 = ll.field_E[-7 + var3 + -var7];
                                break L9;
                              } else {
                                stackIn_36_0 = "";
                                break L9;
                              }
                            }
                            var6 = stackIn_36_0;
                            break L4;
                          } else {
                            break L4;
                          }
                        }
                      } else {
                        break L4;
                      }
                    }
                  }
                  L10: {
                    if (-2 != var4) {
                      break L10;
                    } else {
                      var6 = Integer.toString(var2);
                      break L10;
                    }
                  }
                  L11: {
                    if ((var4 ^ -1) > -1) {
                      stackIn_42_0 = 0;
                      break L11;
                    } else {
                      stackIn_42_0 = 1;
                      break L11;
                    }
                  }
                  L12: {
                    var7 = vh.a(stackIn_42_0 != 0, var6, true);
                    var8 = bi.field_h + -(var7 >> 1449418177);
                    if (-1 >= (var4 ^ -1)) {
                      L13: {
                        var1_int = var1_int + hj.field_a;
                        if (cf.field_f.field_b != var4) {
                          stackIn_47_0 = d.field_F;
                          break L13;
                        } else {
                          stackIn_47_0 = le.field_a;
                          break L13;
                        }
                      }
                      L14: {
                        var9 = stackIn_47_0;
                        if (var9 != null) {
                          var9.a(var1_int, -kk.field_z + var8, (kk.field_z << -88972767) + var7, param0 ^ -21909, (cl.field_m << 51936449) + pb.field_j);
                          break L14;
                        } else {
                          break L14;
                        }
                      }
                      var1_int = var1_int + cl.field_m;
                      break L12;
                    } else {
                      break L12;
                    }
                  }
                  L15: {
                    if (var4 < 0) {
                      ba.field_E.b(var6, var8, qd.field_b + var1_int, var5, -1);
                      var1_int = var1_int + k.field_D;
                      break L15;
                    } else {
                      jc.field_h.b(var6, var8, ed.field_d + var1_int, var5, -1);
                      var1_int = var1_int + (cl.field_m - -hj.field_a + pb.field_j);
                      break L15;
                    }
                  }
                  var3++;
                  continue L2;
                }
              }
            } else {
              decompiledRegionSelector0 = 0;
              break L0;
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          var1 = decompiledCaughtException;
          throw ii.a((Throwable) ((Object) var1), "ti.Q(" + param0 + ')');
        }
        if (decompiledRegionSelector0 == 0) {
          return;
        } else {
          return;
        }
    }

    public final uh a(boolean param0) {
        if (!param0) {
            return (uh) null;
        }
        return this.field_N;
    }

    ti(String param0, sk param1, int param2) {
        super(param0, param1, param2);
    }

    final static void a(int param0, int param1, int param2, byte param3) {
        bc.field_a = param2;
        de.field_f = param0;
        ag.field_c = param1;
        if (param3 != 102) {
            field_O = 92;
        }
    }

    static {
    }
}
