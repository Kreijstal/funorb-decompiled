/*
 * Decompiled by CFR-JS 0.4.0.
 */
final class w extends pb {
    private int field_o;
    private hg field_l;
    static String field_m;
    static int field_n;

    w(int param0) {
        super(param0);
    }

    final void b(int[] param0, int param1) {
        try {
            this.field_l = new hg(param0);
            if (param1 != 8) {
                int[] var4 = (int[]) null;
                this.b((int[]) null, 66);
            }
        } catch (RuntimeException runtimeException) {
            throw ma.a((Throwable) ((Object) runtimeException), "w.QA(" + (param0 != null ? "{...}" : "null") + ',' + param1 + ')');
        }
    }

    final void g(int param0) {
        this.field_i = (7 + this.field_o) / param0;
    }

    final void f(boolean param0) {
        if (!param0) {
            field_m = (String) null;
        }
        this.field_o = this.field_i * 8;
    }

    final int g(byte param0) {
        int var2 = 118 % ((39 - param0) / 51);
        int fieldTemp$0 = this.field_i;
        this.field_i = this.field_i + 1;
        return this.field_g[fieldTemp$0] + -this.field_l.b(-1427) & 255;
    }

    final void b(int param0, byte[] param1, int param2, int param3) {
        int fieldTemp$1 = 0;
        int var5_int = 0;
        int var6 = 0;
        RuntimeException stackIn_9_0 = null;
        StringBuilder stackIn_9_1 = null;
        RuntimeException stackIn_10_0 = null;
        StringBuilder stackIn_10_1 = null;
        String stackIn_10_2 = null;
        RuntimeException decompiledCaughtException = null;
        RuntimeException var5 = null;
        var6 = Main.field_T;
        try {
          L0: {
            var5_int = 0;
            L1: while (true) {
              if (var5_int >= param3) {
                L2: {
                  if (param2 == 28040) {
                    break L2;
                  } else {
                    this.field_l = (hg) null;
                    break L2;
                  }
                }
                break L0;
              } else {
                fieldTemp$1 = this.field_i;
                this.field_i = this.field_i + 1;
                param1[var5_int - -param0] = (byte)(this.field_g[fieldTemp$1] + -this.field_l.b(-1427));
                var5_int++;
                continue L1;
              }
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L3: {
            var5 = decompiledCaughtException;
            stackIn_9_0 = (RuntimeException) (var5);

            stackIn_9_1 = new StringBuilder().append("w.SA(").append(param0).append(',');

            if (param1 == null) {
              stackIn_10_0 = (RuntimeException) ((Object) stackIn_9_0);
              stackIn_10_1 = (StringBuilder) ((Object) stackIn_9_1);
              stackIn_10_2 = "null";
              break L3;
            } else {
              stackIn_10_0 = (RuntimeException) ((Object) stackIn_9_0);
              stackIn_10_1 = (StringBuilder) ((Object) stackIn_9_1);
              stackIn_10_2 = "{...}";
              break L3;
            }
          }
          throw ma.a((Throwable) ((Object) stackIn_10_0), stackIn_10_2 + ',' + param2 + ',' + param3 + ')');
        }
    }

    final static void a(int[] param0, int param1, int param2, int param3, boolean param4, int param5, int param6, int param7, int param8) {
        RuntimeException stackIn_77_0 = null;
        StringBuilder stackIn_77_1 = null;
        RuntimeException stackIn_78_0 = null;
        StringBuilder stackIn_78_1 = null;
        String stackIn_78_2 = null;
        int decompiledRegionSelector0 = 0;
        RuntimeException decompiledCaughtException = null;
        int var9_int = 0;
        RuntimeException var9 = null;
        int var10 = 0;
        int var11 = 0;
        int var12 = 0;
        int var13 = 0;
        int var14 = 0;
        int var15 = 0;
        int var16 = 0;
        int var17 = 0;
        int var18 = 0;
        int var19 = 0;
        var19 = Main.field_T;
        try {
          L0: {
            L1: {
              if (param8 < 0) {
                break L1;
              } else {
                if (param7 < jb.field_c) {
                  L2: {
                    if (param3 >= 0) {
                      break L2;
                    } else {
                      if (-1 >= (param5 ^ -1)) {
                        break L2;
                      } else {
                        if ((param6 ^ -1) <= -1) {
                          break L2;
                        } else {
                          decompiledRegionSelector0 = 1;
                          break L0;
                        }
                      }
                    }
                  }
                  L3: {
                    if (jb.field_i > param3) {
                      break L3;
                    } else {
                      if (jb.field_i > param5) {
                        break L3;
                      } else {
                        if (param6 < jb.field_i) {
                          break L3;
                        } else {
                          decompiledRegionSelector0 = 2;
                          break L0;
                        }
                      }
                    }
                  }
                  L4: {
                    var14 = -param7 + param8;
                    if (param7 != param2) {
                      L5: {
                        var10 = param3 << -1196895312;
                        var9_int = param3 << -1196895312;
                        var15 = -param7 + param2;
                        var11 = (-param3 + param5 << -1542351056) / var15;
                        var12 = (param6 - param3 << 526374192) / var14;
                        if (var12 <= var11) {
                          var13 = 1;
                          var16 = var11;
                          var11 = var12;
                          var12 = var16;
                          break L5;
                        } else {
                          var13 = 0;
                          break L5;
                        }
                      }
                      L6: {
                        L7: {
                          if ((param7 ^ -1) <= -1) {
                            break L7;
                          } else {
                            if ((param2 ^ -1) > -1) {
                              param7 = -param7 + param2;
                              var9_int = var9_int + var11 * param7;
                              var10 = var10 + param7 * var12;
                              param7 = param2;
                              break L6;
                            } else {
                              param7 = -param7;
                              var10 = var10 + param7 * var12;
                              var9_int = var9_int + param7 * var11;
                              param7 = 0;
                              break L7;
                            }
                          }
                        }
                        var16 = jb.field_e[param7];
                        L8: while (true) {
                          if (param2 <= param7) {
                            break L6;
                          } else {
                            L9: {
                              var17 = var9_int >> 1629721200;
                              if ((jb.field_i ^ -1) < (var17 ^ -1)) {
                                var18 = (var10 >> 1635339920) + -(var9_int >> 115586896);
                                if (0 != var18) {
                                  L10: {
                                    if (jb.field_i > var18 + var17) {
                                      break L10;
                                    } else {
                                      var18 = -var17 + (jb.field_i + -1);
                                      break L10;
                                    }
                                  }
                                  if (-1 >= (var17 ^ -1)) {
                                    a.a(var16 + var17, param1, var18, param0, -1483723263);
                                    break L9;
                                  } else {
                                    a.a(var16, param1, var17 + var18, param0, -1483723263);
                                    break L9;
                                  }
                                } else {
                                  if ((var17 ^ -1) > -1) {
                                    break L9;
                                  } else {
                                    if (var17 < jb.field_i) {
                                      a.a(var16 + var17, param1, var18, param0, -1483723263);
                                      break L9;
                                    } else {
                                      break L9;
                                    }
                                  }
                                }
                              } else {
                                break L9;
                              }
                            }
                            param7++;
                            if (param7 >= jb.field_c) {
                              decompiledRegionSelector0 = 3;
                              break L0;
                            } else {
                              var9_int = var9_int + var11;
                              var16 = var16 + vj.field_l;
                              var10 = var10 + var12;
                              continue L8;
                            }
                          }
                        }
                      }
                      var16 = -param2 + param8;
                      if (var16 != 0) {
                        L11: {
                          var17 = param6 << 457183024;
                          if (var13 == 0) {
                            var9_int = param5 << 259350512;
                            break L11;
                          } else {
                            var10 = param5 << -758415184;
                            break L11;
                          }
                        }
                        var11 = (-var9_int + var17) / var16;
                        var12 = (-var10 + var17) / var16;
                        break L4;
                      } else {
                        var12 = 0;
                        var11 = 0;
                        break L4;
                      }
                    } else {
                      L12: {
                        if (param8 != param7) {
                          var15 = -param2 + param8;
                          if (param5 <= param3) {
                            var11 = (param6 + -param5 << 1448938576) / var15;
                            var12 = (param6 - param3 << -983630128) / var14;
                            var10 = param3 << -1122911280;
                            var9_int = param5 << -875206032;
                            break L12;
                          } else {
                            var12 = (param6 - param5 << 1915311568) / var15;
                            var11 = (-param3 + param6 << -1563375376) / var14;
                            var10 = param5 << -418156464;
                            var9_int = param3 << 1625150224;
                            break L12;
                          }
                        } else {
                          var10 = param5 << -171030864;
                          var9_int = param3 << -342130352;
                          var11 = 0;
                          var12 = 0;
                          break L12;
                        }
                      }
                      L13: {
                        if (-1 >= (param7 ^ -1)) {
                          break L13;
                        } else {
                          param7 = Math.min(-param7, -param7 + param2);
                          var9_int = var9_int + param7 * var11;
                          var10 = var10 + var12 * param7;
                          param7 = 0;
                          break L13;
                        }
                      }
                      var13 = 0;
                      break L4;
                    }
                  }
                  L14: {
                    if ((param7 ^ -1) <= -1) {
                      break L14;
                    } else {
                      param7 = -param7;
                      var10 = var10 + param7 * var12;
                      var9_int = var9_int + param7 * var11;
                      param7 = 0;
                      break L14;
                    }
                  }
                  var15 = jb.field_e[param7];
                  if (param4) {
                    L15: while (true) {
                      if (param7 >= param8) {
                        decompiledRegionSelector0 = 5;
                        break L0;
                      } else {
                        L16: {
                          var16 = var9_int >> -1047242832;
                          if (jb.field_i <= var16) {
                            break L16;
                          } else {
                            var17 = (var10 >> 807594160) + -(var9_int >> 1482364912);
                            if (-1 != (var17 ^ -1)) {
                              L17: {
                                if (jb.field_i > var17 + var16) {
                                  break L17;
                                } else {
                                  var17 = -1 + (jb.field_i + -var16);
                                  break L17;
                                }
                              }
                              if (0 <= var16) {
                                a.a(var15 + var16, param1, var17, param0, -1483723263);
                                break L16;
                              } else {
                                a.a(var15, param1, var17 + var16, param0, -1483723263);
                                break L16;
                              }
                            } else {
                              if (-1 < (var16 ^ -1)) {
                                break L16;
                              } else {
                                if (var16 < jb.field_i) {
                                  a.a(var16 - -var15, param1, var17, param0, -1483723263);
                                  break L16;
                                } else {
                                  break L16;
                                }
                              }
                            }
                          }
                        }
                        param7++;
                        if (param7 >= jb.field_c) {
                          decompiledRegionSelector0 = 6;
                          break L0;
                        } else {
                          var9_int = var9_int + var11;
                          var15 = var15 + vj.field_l;
                          var10 = var10 + var12;
                          continue L15;
                        }
                      }
                    }
                  } else {
                    decompiledRegionSelector0 = 4;
                    break L0;
                  }
                } else {
                  break L1;
                }
              }
            }
            decompiledRegionSelector0 = 0;
            break L0;
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L18: {
            var9 = decompiledCaughtException;
            stackIn_77_0 = (RuntimeException) (var9);

            stackIn_77_1 = new StringBuilder().append("w.PA(");

            if (param0 == null) {
              stackIn_78_0 = (RuntimeException) ((Object) stackIn_77_0);
              stackIn_78_1 = (StringBuilder) ((Object) stackIn_77_1);
              stackIn_78_2 = "null";
              break L18;
            } else {
              stackIn_78_0 = (RuntimeException) ((Object) stackIn_77_0);
              stackIn_78_1 = (StringBuilder) ((Object) stackIn_77_1);
              stackIn_78_2 = "{...}";
              break L18;
            }
          }
          throw ma.a((Throwable) ((Object) stackIn_78_0), stackIn_78_2 + ',' + param1 + ',' + param2 + ',' + param3 + ',' + param4 + ',' + param5 + ',' + param6 + ',' + param7 + ',' + param8 + ')');
        }
        if (decompiledRegionSelector0 == 0) {
          return;
        } else {
          if (decompiledRegionSelector0 == 1) {
            return;
          } else {
            if (decompiledRegionSelector0 == 2) {
              return;
            } else {
              if (decompiledRegionSelector0 == 3) {
                return;
              } else {
                if (decompiledRegionSelector0 == 4) {
                  return;
                } else {
                  if (decompiledRegionSelector0 == 5) {
                    return;
                  } else {
                    return;
                  }
                }
              }
            }
          }
        }
    }

    final void h(int param0, int param1) {
        if (param1 != 0) {
            return;
        }
        int fieldTemp$0 = this.field_i;
        this.field_i = this.field_i + 1;
        this.field_g[fieldTemp$0] = (byte)(param0 + this.field_l.b(-1427));
    }

    public static void h(int param0) {
        field_m = null;
        if (param0 <= 18) {
            w.a((int[]) null, -109, -96, -41, true, 32, 55, -64, -115);
        }
    }

    w(byte[] param0) {
        super(param0);
    }

    final int g(int param0, int param1) {
        int incrementValue$0 = 0;
        int var6 = Main.field_T;
        if (param1 != -1) {
            return 42;
        }
        int var3 = this.field_o >> -679251997;
        int var4 = 8 + -(this.field_o & 7);
        this.field_o = this.field_o + param0;
        int var5 = 0;
        while (var4 < param0) {
            incrementValue$0 = var3;
            var3++;
            var5 = var5 + ((this.field_g[incrementValue$0] & sg.field_g[var4]) << param0 - var4);
            param0 = param0 - var4;
            var4 = 8;
        }
        if (var4 != param0) {
            var5 = var5 + (this.field_g[var3] >> -param0 + var4 & sg.field_g[param0]);
        } else {
            var5 = var5 + (this.field_g[var3] & sg.field_g[var4]);
        }
        return var5;
    }

    static {
        field_m = "Play the game without logging in just yet";
    }
}
