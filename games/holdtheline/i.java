/*
 * Decompiled by CFR-JS 0.4.0.
 */
final class i extends vn {
    static int[] field_j;
    static vi[] field_k;
    static int[] field_l;
    static java.awt.Image field_i;
    static kf field_m;

    final static void a(boolean param0, gn param1, gn param2, gn param3) {
        hj stackIn_3_0;
        hj stackIn_3_1;
        gn stackIn_3_2;
        String stackIn_3_3;
        hj stackIn_4_0;
        hj stackIn_4_1;
        gn stackIn_4_2;
        String stackIn_4_3;
        int stackIn_4_4;
        RuntimeException stackIn_19_0 = null;
        StringBuilder stackIn_19_1 = null;
        RuntimeException stackIn_20_0 = null;
        StringBuilder stackIn_20_1 = null;
        String stackIn_20_2 = null;
        StringBuilder stackIn_22_1 = null;
        StringBuilder stackIn_23_1 = null;
        String stackIn_23_2 = null;
        StringBuilder stackIn_25_1 = null;
        StringBuilder stackIn_26_1 = null;
        String stackIn_26_2 = null;
        int decompiledRegionSelector0 = 0;
        RuntimeException decompiledCaughtException = null;
        RuntimeException var4 = null;
        bm[] var5 = null;
        bm[][] var6 = null;
        int[][] var7 = null;
        int var8 = 0;
        int var9 = 0;
        bm[] var10 = null;
        hj var11 = null;
        int var11_int = 0;
        hj var12 = null;
        int var13 = 0;
        bm[] var14 = null;
        hj var15 = null;
        int[][] var16 = null;
        hj var17 = null;
        hj var18 = null;
        int[][] var19 = null;
        var13 = HoldTheLine.field_D;
        try {
          L0: {
            L1: {
              ja.field_Db = si.a(param3, 33, "commonui", "frame_top");
              te.field_b = si.a(param3, 33, "commonui", "frame_bottom");
              di.field_t = dl.a(param3, "commonui", false, "jagex_logo_grey");
              vm.field_Ab = si.a(param3, 33, "commonui", "button");
              ef.field_i = ce.a("validation", 17870, "commonui", param3);
              mo.field_w = (qi) ((Object) ga.a("commonui", "arezzo12", param1, (byte) 35, param3));
              hl.field_g = (qi) ((Object) ga.a("commonui", "arezzo14", param1, (byte) 35, param3));
              de.field_I = (qi) ((Object) ga.a("commonui", "arezzo14bold", param1, (byte) 35, param3));
              stackIn_3_0 = null;

              stackIn_3_1 = null;

              stackIn_3_2 = (gn) (param2);

              stackIn_3_3 = "";

              if (param0) {
                stackIn_4_0 = null;
                stackIn_4_1 = null;
                stackIn_4_2 = (gn) ((Object) stackIn_3_2);
                stackIn_4_3 = (String) ((Object) stackIn_3_3);
                stackIn_4_4 = 0;
                break L1;
              } else {
                stackIn_4_0 = null;
                stackIn_4_1 = null;
                stackIn_4_2 = (gn) ((Object) stackIn_3_2);
                stackIn_4_3 = (String) ((Object) stackIn_3_3);
                stackIn_4_4 = 1;
                break L1;
              }
            }
            var17 = new hj(((gn) (Object) stackIn_4_2).a(stackIn_4_3, stackIn_4_4 != 0, "button.gif"), (java.awt.Component) ((Object) kd.field_b));
            hc.a("dropdown", param3, -120, "commonui");
            var5 = oc.a("screen_options", param3, (byte) 32, "commonui");
            ef.field_g = new bm[4];
            ef.field_d = new bm[4];
            ug.field_d = new bm[4];
            var6 = new bm[][]{ef.field_d, ef.field_g, ug.field_d};
            var19 = new int[4][];
            var16 = var19;
            var7 = var16;
            var7[0] = var5[0].field_n;
            var8 = 1;
            L2: while (true) {
              if (var19.length <= var8) {
                var8 = var5[0].field_m[0];
                var19[2][var8] = 16777215;
                var19[1][var8] = 2394342;
                var19[3][var8] = 4767999;
                var9 = 0;
                L3: while (true) {
                  if ((var9 ^ -1) <= -4) {
                    var9 = var17.field_y;
                    lj.a((byte) -124);
                    var17.f();
                    tc.g(0, 0, tc.field_j, tc.field_c);
                    var15 = new hj(var9, var9);
                    var18 = var15;
                    var18.f();
                    if (param0) {
                      var17.b(0, 0);
                      var11 = new hj(var9, var9);
                      var11.f();
                      var17.b(-var17.field_s + var9, 0);
                      var12 = new hj(var17.field_s + -(2 * var9), var9);
                      var12.f();
                      var17.b(-var9, 0);
                      dm.b((byte) -10);
                      vm.field_Ab = new hj[]{var15, var12, var11};
                      decompiledRegionSelector0 = 1;
                      break L0;
                    } else {
                      decompiledRegionSelector0 = 0;
                      break L0;
                    }
                  } else {
                    var14 = var6[var9];
                    var10 = var14;
                    var11_int = 0;
                    L4: while (true) {
                      if (var14.length <= var11_int) {
                        var9++;
                        continue L3;
                      } else {
                        var14[var11_int] = rl.a(var19[var11_int], -116, var5[var9]);
                        var11_int++;
                        continue L4;
                      }
                    }
                  }
                }
              } else {
                var7[var8] = (int[]) ((Object) var19[0].clone());
                var8++;
                continue L2;
              }
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L5: {
            var4 = decompiledCaughtException;
            stackIn_19_0 = (RuntimeException) (var4);

            stackIn_19_1 = new StringBuilder().append("i.E(").append(param0).append(',');

            if (param1 == null) {
              stackIn_20_0 = (RuntimeException) ((Object) stackIn_19_0);
              stackIn_20_1 = (StringBuilder) ((Object) stackIn_19_1);
              stackIn_20_2 = "null";
              break L5;
            } else {
              stackIn_20_0 = (RuntimeException) ((Object) stackIn_19_0);
              stackIn_20_1 = (StringBuilder) ((Object) stackIn_19_1);
              stackIn_20_2 = "{...}";
              break L5;
            }
          }
          L6: {


            stackIn_22_1 = ((StringBuilder) (Object) stackIn_20_1).append(stackIn_20_2).append(',');

            if (param2 == null) {
              stackIn_20_0 = (RuntimeException) ((Object) stackIn_20_0);
              stackIn_23_1 = (StringBuilder) ((Object) stackIn_22_1);
              stackIn_23_2 = "null";
              break L6;
            } else {
              stackIn_20_0 = (RuntimeException) ((Object) stackIn_20_0);
              stackIn_23_1 = (StringBuilder) ((Object) stackIn_22_1);
              stackIn_23_2 = "{...}";
              break L6;
            }
          }
          L7: {


            stackIn_25_1 = ((StringBuilder) (Object) stackIn_23_1).append(stackIn_23_2).append(',');

            if (param3 == null) {
              stackIn_20_0 = (RuntimeException) ((Object) stackIn_20_0);
              stackIn_26_1 = (StringBuilder) ((Object) stackIn_25_1);
              stackIn_26_2 = "null";
              break L7;
            } else {
              stackIn_20_0 = (RuntimeException) ((Object) stackIn_20_0);
              stackIn_26_1 = (StringBuilder) ((Object) stackIn_25_1);
              stackIn_26_2 = "{...}";
              break L7;
            }
          }
          throw kk.a((Throwable) ((Object) stackIn_20_0), stackIn_26_2 + ')');
        }
        if (decompiledRegionSelector0 == 0) {
          return;
        } else {
          return;
        }
    }

    final static int a(gj param0, gj param1, int param2) {
        int stackIn_5_0 = 0;
        int stackIn_15_0 = 0;
        int stackIn_19_0 = 0;
        int stackIn_21_0 = 0;
        RuntimeException stackIn_24_0 = null;
        StringBuilder stackIn_24_1 = null;
        RuntimeException stackIn_25_0 = null;
        StringBuilder stackIn_25_1 = null;
        String stackIn_25_2 = null;
        StringBuilder stackIn_27_1 = null;
        StringBuilder stackIn_28_1 = null;
        String stackIn_28_2 = null;
        int decompiledRegionSelector0 = 0;
        RuntimeException decompiledCaughtException = null;
        RuntimeException var3 = null;
        int var4 = 0;
        float var5 = 0.0f;
        in var6 = null;
        in var7 = null;
        float var8 = 0.0f;
        float var9 = 0.0f;
        float var10 = 0.0f;
        qm var11 = null;
        qm var12 = null;
        int var13 = 0;
        wb var14 = null;
        try {
          L0: {
            L1: {
              var14 = new wb(param1.field_t, param0.field_t);
              var4 = var14.a(-113);
              var5 = (float)Math.sqrt((double)var4);
              var6 = var14.a(var5, (byte) 121);
              if (param2 == -6218) {
                break L1;
              } else {
                i.a((byte) -15, true);
                break L1;
              }
            }
            var7 = var6.a((byte) -41);
            var8 = (float)(-(param0.field_u * param0.field_u) + (var4 - -(param1.field_u * param1.field_u)));
            var8 = var8 / (var5 * 2.0f);
            var9 = -(var8 * var8) + (float)(param1.field_u * param1.field_u);
            if (0.0f > var9) {
              stackIn_5_0 = 0;
              decompiledRegionSelector0 = 0;
              break L0;
            } else {
              var10 = (float)Math.sqrt((double)var9);
              var6.a(var8, param2 ^ 6217);
              var7.a(var10, -1);
              var11 = new qm(param1.field_t);
              var11.a(var6, (byte) 109);
              if (0.0010000000474974513f > var9) {
                L2: {
                  mm.field_d = ha.a(param1.field_t, var11, false);
                  lk.field_b = ha.a(param0.field_t, var11, false);
                  if (!param1.a(mm.field_d, param2 ^ -10910)) {
                    break L2;
                  } else {
                    if (!param0.a(lk.field_b, param2 ^ -10910)) {
                      break L2;
                    } else {
                      stackIn_19_0 = 1;
                      decompiledRegionSelector0 = 2;
                      break L0;
                    }
                  }
                }
                stackIn_21_0 = 0;
                decompiledRegionSelector0 = 3;
                break L0;
              } else {
                L3: {
                  var12 = new qm(var11);
                  var11.a(var7, (byte) 122);
                  var12.a(var7, -13201);
                  mm.field_d = ha.a(param1.field_t, var11, false);
                  var13 = 0;
                  lk.field_b = ha.a(param0.field_t, var11, false);
                  if (!param1.a(mm.field_d, 13012)) {
                    break L3;
                  } else {
                    if (!param0.a(lk.field_b, 13012)) {
                      break L3;
                    } else {
                      var13++;
                      break L3;
                    }
                  }
                }
                L4: {
                  rb.field_g = ha.a(param1.field_t, var12, false);
                  cj.field_G = ha.a(param0.field_t, var12, false);
                  if (!param1.a(rb.field_g, param2 + 19230)) {
                    break L4;
                  } else {
                    if (!param0.a(cj.field_G, 13012)) {
                      break L4;
                    } else {
                      var13++;
                      if ((var13 ^ -1) != -2) {
                        break L4;
                      } else {
                        mm.field_d = rb.field_g;
                        lk.field_b = cj.field_G;
                        break L4;
                      }
                    }
                  }
                }
                stackIn_15_0 = var13;
                decompiledRegionSelector0 = 1;
                break L0;
              }
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L5: {
            var3 = decompiledCaughtException;
            stackIn_24_0 = (RuntimeException) (var3);

            stackIn_24_1 = new StringBuilder().append("i.A(");

            if (param0 == null) {
              stackIn_25_0 = (RuntimeException) ((Object) stackIn_24_0);
              stackIn_25_1 = (StringBuilder) ((Object) stackIn_24_1);
              stackIn_25_2 = "null";
              break L5;
            } else {
              stackIn_25_0 = (RuntimeException) ((Object) stackIn_24_0);
              stackIn_25_1 = (StringBuilder) ((Object) stackIn_24_1);
              stackIn_25_2 = "{...}";
              break L5;
            }
          }
          L6: {


            stackIn_27_1 = ((StringBuilder) (Object) stackIn_25_1).append(stackIn_25_2).append(',');

            if (param1 == null) {
              stackIn_25_0 = (RuntimeException) ((Object) stackIn_25_0);
              stackIn_28_1 = (StringBuilder) ((Object) stackIn_27_1);
              stackIn_28_2 = "null";
              break L6;
            } else {
              stackIn_25_0 = (RuntimeException) ((Object) stackIn_25_0);
              stackIn_28_1 = (StringBuilder) ((Object) stackIn_27_1);
              stackIn_28_2 = "{...}";
              break L6;
            }
          }
          throw kk.a((Throwable) ((Object) stackIn_25_0), stackIn_28_2 + ',' + param2 + ')');
        }
        if (decompiledRegionSelector0 == 0) {
          return stackIn_5_0;
        } else {
          if (decompiledRegionSelector0 == 1) {
            return stackIn_15_0;
          } else {
            if (decompiledRegionSelector0 == 2) {
              return stackIn_19_0;
            } else {
              return stackIn_21_0;
            }
          }
        }
    }

    final static void a(byte param0, boolean param1) {
        ej.field_b = dg.field_c;
        if (param0 <= 43) {
            String var3 = (String) null;
            i.a(100, -42, -85, (String) null, -48);
        }
        if (-14 == (dg.field_c ^ -1)) {
            ((ec) ((Object) l.field_a[dg.field_c])).d((byte) -120);
        } else {
            if (0 != (dg.field_c ^ -1)) {
                ((ti) ((Object) l.field_a[dg.field_c])).i(105);
            }
        }
        gg.field_L = param1;
        vg.field_G = 1;
    }

    final static tk a(int param0, int param1, int param2, String param3, int param4) {
        String var5 = null;
        RuntimeException var5_ref = null;
        String var6 = null;
        oa var7 = null;
        oa var8 = null;
        tk stackIn_3_0 = null;
        RuntimeException stackIn_6_0 = null;
        StringBuilder stackIn_6_1 = null;
        RuntimeException stackIn_7_0 = null;
        StringBuilder stackIn_7_1 = null;
        String stackIn_7_2 = null;
        RuntimeException decompiledCaughtException = null;
        try {
          L0: {
            L1: {
              var5 = param3 + "_accelerate+loop";
              if (param0 < -104) {
                break L1;
              } else {
                field_i = (java.awt.Image) null;
                break L1;
              }
            }
            var6 = param3 + "_decelerate";
            var7 = new oa(var5, param2);
            var8 = new oa(var6, param4);
            var7.field_m = oh.a(var7, param1, 89);
            var7.field_l = var7.field_n.length;
            stackIn_3_0 = new tk(var7, var8);
            break L0;
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L2: {
            var5_ref = decompiledCaughtException;
            stackIn_6_0 = (RuntimeException) (var5_ref);

            stackIn_6_1 = new StringBuilder().append("i.F(").append(param0).append(',').append(param1).append(',').append(param2).append(',');

            if (param3 == null) {
              stackIn_7_0 = (RuntimeException) ((Object) stackIn_6_0);
              stackIn_7_1 = (StringBuilder) ((Object) stackIn_6_1);
              stackIn_7_2 = "null";
              break L2;
            } else {
              stackIn_7_0 = (RuntimeException) ((Object) stackIn_6_0);
              stackIn_7_1 = (StringBuilder) ((Object) stackIn_6_1);
              stackIn_7_2 = "{...}";
              break L2;
            }
          }
          throw kk.a((Throwable) ((Object) stackIn_7_0), stackIn_7_2 + ',' + param4 + ')');
        }
        return stackIn_3_0;
    }

    final void a(int param0, int param1, int param2, int param3, int param4) {
        cg.a(param4, param0, param3, (byte) 41, param1);
        if (param2 != -51) {
            gn var7 = (gn) null;
            i.a(true, (gn) null, (gn) null, (gn) null);
        }
    }

    public static void a(byte param0) {
        field_m = null;
        if (param0 != -3) {
            i.a((byte) -104, true);
        }
        field_j = null;
        field_i = null;
        field_k = null;
        field_l = null;
    }

    i() {
    }

    static {
        field_j = fc.a(-45, 5, 36);
        field_l = new int[]{4, 4, 4};
    }
}
