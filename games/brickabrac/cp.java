/*
 * Decompiled by CFR-JS 0.4.0.
 */
final class cp extends h {
    private vl field_J;
    static mh field_K;
    static String field_G;
    static jp field_I;
    static String field_H;

    final static void a(boolean param0, int param1) {
        int var2;
        int var3;
        j var4;
        L0: {
          var3 = BrickABrac.field_J ? 1 : 0;
          if (param1 == 21543) {
            break L0;
          } else {
            var4 = (j) null;
            cp.a(103, 104, 91, (byte) -14, (j) null, 105, 90);
            break L0;
          }
        }
        L1: {
          if (0 >= go.field_c) {
            if (og.h(-26090)) {
              var2 = 0;
              break L1;
            } else {
              var2 = 1;
              break L1;
            }
          } else {
            L2: {
              if (null == jj.field_a) {
                ua.field_d = ff.a(0, 0, 640, 480, true, lp.field_ub);
                break L2;
              } else {
                ua.field_d = jj.field_a.a(true);
                qm.a(97, 2);
                break L2;
              }
            }
            if (ua.field_d == null) {
              var2 = 3;
              break L1;
            } else {
              var2 = 2;
              ap.a((byte) 117, ua.field_d);
              break L1;
            }
          }
        }
        L3: {
          if (null != jj.field_a) {
            break L3;
          } else {
            if (!gh.field_c) {
              break L3;
            } else {
              bq.a(param0, var2, (byte) 104);
              break L3;
            }
          }
        }
    }

    final void a(int param0, int param1, int param2, byte param3) {
        int var7 = BrickABrac.field_J ? 1 : 0;
        if (this.field_l != null) {
            this.field_l.a(true, param3 + -20607, param1, (oc) (this), param0);
        }
        kh var5 = new kh(this.field_J);
        oc var6 = (oc) ((Object) var5.a(true));
        while (var6 != null) {
            var6.a(param0 - -this.field_w, param1 - -this.field_o, param2, (byte) -11);
            var6 = (oc) ((Object) var5.d(-24706));
        }
        if (param3 != -11) {
            field_G = (String) null;
        }
    }

    public static void j(byte param0) {
        field_I = null;
        field_G = null;
        if (param0 > -91) {
            ta var2 = (ta) null;
            cp.a(-80, 69, (ta) null, 50, (ta) null, -32);
        }
        field_H = null;
        field_K = null;
    }

    final void a(int param0) {
        int var4 = BrickABrac.field_J ? 1 : 0;
        if (param0 > -119) {
            return;
        }
        kh var2 = new kh(this.field_J);
        jc var3 = (jc) ((Object) var2.a(240));
        while (var3 != null) {
            var3.field_N = false;
            var3 = (jc) ((Object) var2.b(7));
        }
        this.field_E = null;
    }

    final void a(int param0, oc param1) {
        jc var3 = null;
        try {
            if (!(param1 instanceof jc)) {
                throw new IllegalArgumentException();
            }
            var3 = (jc) ((Object) param1);
            if (param0 > -63) {
                cp.j((byte) -73);
            }
            this.field_J.a(true, var3);
            var3.field_N = true;
            var3.a(true, (oc) (this));
        } catch (RuntimeException runtimeException) {
            throw qb.a((Throwable) ((Object) runtimeException), "cp.M(" + param0 + ',' + (param1 != null ? "{...}" : "null") + ')');
        }
    }

    final void i(byte param0) {
        int var4 = BrickABrac.field_J ? 1 : 0;
        kh var2 = new kh(this.field_J);
        jc var3 = (jc) ((Object) var2.a(240));
        if (param0 != -36) {
            return;
        }
        while (var3 != null) {
            if (!(!var3.a(-94))) {
                var3.b((byte) 111);
            }
            var3 = (jc) ((Object) var2.b(param0 ^ -37));
        }
    }

    public cp() {
        super(0, 0, mn.field_y, r.field_w, (ub) null, (uh) null);
        this.field_J = new vl();
    }

    final static lo a(String param0, byte param1) {
        int var2_int = 0;
        RuntimeException var2 = null;
        String var3 = null;
        String var4 = null;
        lo var5 = null;
        lo stackIn_4_0 = null;
        lo stackIn_7_0 = null;
        lo stackIn_10_0 = null;
        lo stackIn_13_0 = null;
        lo stackIn_15_0 = null;
        RuntimeException stackIn_18_0 = null;
        StringBuilder stackIn_18_1 = null;
        RuntimeException stackIn_19_0 = null;
        StringBuilder stackIn_19_1 = null;
        String stackIn_19_2 = null;
        int decompiledRegionSelector0 = 0;
        RuntimeException decompiledCaughtException = null;
        try {
          L0: {
            L1: {
              if (param0 == null) {
                break L1;
              } else {
                if (param0.length() != 0) {
                  var2_int = param0.indexOf('@');
                  if (-1 != var2_int) {
                    var3 = param0.substring(0, var2_int);
                    var4 = param0.substring(var2_int - -1);
                    if (param1 < -16) {
                      var5 = h.a(var3, false);
                      if (var5 == null) {
                        stackIn_15_0 = ji.a(var4, 0);
                        decompiledRegionSelector0 = 4;
                        break L0;
                      } else {
                        stackIn_13_0 = (lo) (var5);
                        decompiledRegionSelector0 = 3;
                        break L0;
                      }
                    } else {
                      stackIn_10_0 = (lo) null;
                      decompiledRegionSelector0 = 2;
                      break L0;
                    }
                  } else {
                    stackIn_7_0 = la.field_a;
                    decompiledRegionSelector0 = 1;
                    break L0;
                  }
                } else {
                  break L1;
                }
              }
            }
            stackIn_4_0 = tl.field_b;
            decompiledRegionSelector0 = 0;
            break L0;
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L2: {
            var2 = decompiledCaughtException;
            stackIn_18_0 = (RuntimeException) (var2);

            stackIn_18_1 = new StringBuilder().append("cp.L(");

            if (param0 == null) {
              stackIn_19_0 = (RuntimeException) ((Object) stackIn_18_0);
              stackIn_19_1 = (StringBuilder) ((Object) stackIn_18_1);
              stackIn_19_2 = "null";
              break L2;
            } else {
              stackIn_19_0 = (RuntimeException) ((Object) stackIn_18_0);
              stackIn_19_1 = (StringBuilder) ((Object) stackIn_18_1);
              stackIn_19_2 = "{...}";
              break L2;
            }
          }
          throw qb.a((Throwable) ((Object) stackIn_19_0), stackIn_19_2 + ',' + param1 + ')');
        }
        if (decompiledRegionSelector0 == 0) {
          return stackIn_4_0;
        } else {
          if (decompiledRegionSelector0 == 1) {
            return stackIn_7_0;
          } else {
            if (decompiledRegionSelector0 == 2) {
              return stackIn_10_0;
            } else {
              if (decompiledRegionSelector0 == 3) {
                return stackIn_13_0;
              } else {
                return stackIn_15_0;
              }
            }
          }
        }
    }

    final static void a(int param0, int param1, ta param2, int param3, ta param4, int param5) {
        try {
            kn.field_y = param1;
            cn.field_c = param5;
            if (param3 <= 25) {
                cp.h(24);
            }
            cd.field_c = param4;
            jb.field_H = param0;
            c.field_l = param2;
        } catch (RuntimeException runtimeException) {
            throw qb.a((Throwable) ((Object) runtimeException), "cp.Q(" + param0 + ',' + param1 + ',' + (param2 != null ? "{...}" : "null") + ',' + param3 + ',' + (param4 != null ? "{...}" : "null") + ',' + param5 + ')');
        }
    }

    final void i(int param0) {
        int var5 = BrickABrac.field_J ? 1 : 0;
        int var3 = 18 / ((36 - param0) / 43);
        kh var2 = new kh(this.field_J);
        jc var4 = (jc) ((Object) var2.a(240));
        while (var4 != null) {
            if (var4.h(0)) {
                var4.b((byte) 111);
            }
            var4 = (jc) ((Object) var2.b(7));
        }
        this.field_E = (oc) ((Object) this.g(-126));
    }

    final static void h(int param0) {
        int stackIn_19_0 = 0;
        int stackIn_22_0 = 0;
        String stackIn_27_0 = null;
        String stackIn_33_0 = null;
        int stackIn_39_0 = 0;
        ta stackIn_44_0 = null;
        RuntimeException decompiledCaughtException = null;
        int var1_int = 0;
        RuntimeException var1 = null;
        int var2 = 0;
        long var3_long = 0L;
        int var3 = 0;
        int var4 = 0;
        int var5 = 0;
        String var6 = null;
        int var7 = 0;
        int var8 = 0;
        ta var9 = null;
        int var10 = 0;
        j var11 = null;
        var10 = BrickABrac.field_J ? 1 : 0;
        try {
          L0: {
            L1: {
              var1_int = jq.field_r;
              var2 = 0;
              if (param0 == -16546) {
                break L1;
              } else {
                var11 = (j) null;
                cp.a(-114, -15, 90, (byte) -109, (j) null, -85, -67);
                break L1;
              }
            }
            L2: {
              if (-3 != (ih.field_p ^ -1)) {
                break L2;
              } else {
                var3_long = -un.field_g + ue.a(false);
                var2 = (int)((-var3_long + 10999L) / 1000L);
                if (0 > var2) {
                  var2 = 0;
                  break L2;
                } else {
                  break L2;
                }
              }
            }
            var3 = 0;
            L3: while (true) {
              if (qh.field_i.length <= var3) {
                break L0;
              } else {
                L4: {
                  var4 = BrickABrac.field_K[var3];
                  if (-1 < (var4 ^ -1)) {
                    var5 = cc.field_H;
                    break L4;
                  } else {
                    if (mh.field_H.field_j != var4) {
                      var5 = eb.field_n;
                      break L4;
                    } else {
                      var5 = vq.field_A;
                      break L4;
                    }
                  }
                }
                L5: {
                  var6 = qh.field_i[var3];
                  if (2 != ih.field_p) {
                    break L5;
                  } else {
                    if (1 != var2) {
                      break L5;
                    } else {
                      L6: {
                        if (rg.field_k.length >= ik.field_b.length) {
                          stackIn_19_0 = rg.field_k.length;
                          break L6;
                        } else {
                          stackIn_19_0 = ik.field_b.length;
                          break L6;
                        }
                      }
                      L7: {
                        var7 = stackIn_19_0;
                        if (fk.field_h.length <= h.field_F.length) {
                          stackIn_22_0 = h.field_F.length;
                          break L7;
                        } else {
                          stackIn_22_0 = fk.field_h.length;
                          break L7;
                        }
                      }
                      L8: {
                        var8 = stackIn_22_0;
                        if ((var3 ^ -1) > -7) {
                          break L8;
                        } else {
                          if (6 - -var7 <= var3) {
                            break L8;
                          } else {
                            L9: {
                              if (-var7 + -6 + var3 - -rg.field_k.length < 0) {
                                stackIn_27_0 = "";
                                break L9;
                              } else {
                                stackIn_27_0 = rg.field_k[-var7 + var3 + (-6 + rg.field_k.length)];
                                break L9;
                              }
                            }
                            var6 = stackIn_27_0;
                            break L8;
                          }
                        }
                      }
                      if (7 + var7 > var3) {
                        break L5;
                      } else {
                        if (7 - (-var7 + -var8) <= var3) {
                          break L5;
                        } else {
                          L10: {
                            if (-var7 + (var3 - 7) < h.field_F.length) {
                              stackIn_33_0 = h.field_F[var3 + (-7 + -var7)];
                              break L10;
                            } else {
                              stackIn_33_0 = "";
                              break L10;
                            }
                          }
                          var6 = stackIn_33_0;
                          break L5;
                        }
                      }
                    }
                  }
                }
                L11: {
                  if (-2 != var4) {
                    break L11;
                  } else {
                    var6 = Integer.toString(var2);
                    break L11;
                  }
                }
                L12: {
                  if (0 > var4) {
                    stackIn_39_0 = 0;
                    break L12;
                  } else {
                    stackIn_39_0 = 1;
                    break L12;
                  }
                }
                L13: {
                  var7 = hm.a(stackIn_39_0 != 0, var6, 0);
                  var8 = wi.field_k + -(var7 >> 1509099745);
                  if (var4 >= 0) {
                    L14: {
                      var1_int = var1_int + kn.field_y;
                      if (mh.field_H.field_j != var4) {
                        stackIn_44_0 = cd.field_c;
                        break L14;
                      } else {
                        stackIn_44_0 = c.field_l;
                        break L14;
                      }
                    }
                    L15: {
                      var9 = stackIn_44_0;
                      if (var9 != null) {
                        var9.a((cn.field_c << 639194529) + var7, b.field_c + (jb.field_H << 857598337), -cn.field_c + var8, false, var1_int);
                        break L15;
                      } else {
                        break L15;
                      }
                    }
                    var1_int = var1_int + jb.field_H;
                    break L13;
                  } else {
                    break L13;
                  }
                }
                L16: {
                  if (-1 >= (var4 ^ -1)) {
                    kp.field_b.c(var6, var8, var1_int - -so.field_b, var5, -1);
                    var1_int = var1_int + (jb.field_H + (kn.field_y - -b.field_c));
                    break L16;
                  } else {
                    ni.field_Wb.c(var6, var8, var1_int + ae.field_f, var5, -1);
                    var1_int = var1_int + pe.field_s;
                    break L16;
                  }
                }
                var3++;
                continue L3;
              }
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          var1 = decompiledCaughtException;
          throw qb.a((Throwable) ((Object) var1), "cp.F(" + param0 + ')');
        }
    }

    final static void a(int param0, int param1, int param2, byte param3, j param4, int param5, int param6) {
        int stackIn_7_0 = 0;
        int stackIn_7_1 = 0;
        int stackIn_7_2 = 0;
        int stackIn_8_0 = 0;
        int stackIn_8_1 = 0;
        int stackIn_8_2 = 0;
        int stackIn_8_3 = 0;
        RuntimeException stackIn_21_0 = null;
        StringBuilder stackIn_21_1 = null;
        RuntimeException stackIn_22_0 = null;
        StringBuilder stackIn_22_1 = null;
        String stackIn_22_2 = null;
        int decompiledRegionSelector0 = 0;
        RuntimeException decompiledCaughtException = null;
        int var7_int = 0;
        RuntimeException var7 = null;
        int var8 = 0;
        jp var9 = null;
        try {
          L0: {
            if (-51 < (param4.field_U ^ -1)) {
              decompiledRegionSelector0 = 0;
              break L0;
            } else {
              L1: {
                var7_int = param4.field_U - 50;
                var8 = var7_int * 96 / 100;
                if (-97 <= (var8 ^ -1)) {
                  break L1;
                } else {
                  var8 = 96;
                  break L1;
                }
              }
              L2: {
                stackIn_7_0 = param0;

                stackIn_7_1 = param5;

                stackIn_7_2 = param1 / param6;

                if (-2 != (param6 ^ -1)) {
                  stackIn_8_0 = stackIn_7_0;
                  stackIn_8_1 = stackIn_7_1;
                  stackIn_8_2 = stackIn_7_2;
                  stackIn_8_3 = 0;
                  break L2;
                } else {
                  stackIn_8_0 = stackIn_7_0;
                  stackIn_8_1 = stackIn_7_1;
                  stackIn_8_2 = stackIn_7_2;
                  stackIn_8_3 = 35;
                  break L2;
                }
              }
              L3: {
                lb.d(stackIn_8_0, stackIn_8_1, stackIn_8_2, -stackIn_8_3 + param2 / param6, 0, var8);
                if (150 >= var7_int * 2) {
                  break L3;
                } else {
                  if ((param4.field_zb ^ -1) >= -1) {
                    break L3;
                  } else {
                    if (param4.field_zb + -1 < a.field_s.length) {
                      L4: {
                        var8 = -75 + var7_int;
                        if (50 < var8) {
                          var8 = 50;
                          break L4;
                        } else {
                          break L4;
                        }
                      }
                      var8 = 256 * var8 / 50;
                      var9 = a.field_s[param4.field_zb + -1];
                      var9.b(param0 + (param1 + -(param6 * var9.field_x)) / (param6 * 2), param5 + (param2 + -(var9.field_z * param6)) / (param6 * 2), var8);
                      lo.a(param1, var8, 60, param0, param6, param5, param2, param4);
                      break L3;
                    } else {
                      break L3;
                    }
                  }
                }
              }
              L5: {
                if (param3 >= 118) {
                  break L5;
                } else {
                  cp.a(true, 86);
                  break L5;
                }
              }
              decompiledRegionSelector0 = 1;
              break L0;
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L6: {
            var7 = decompiledCaughtException;
            stackIn_21_0 = (RuntimeException) (var7);

            stackIn_21_1 = new StringBuilder().append("cp.H(").append(param0).append(',').append(param1).append(',').append(param2).append(',').append(param3).append(',');

            if (param4 == null) {
              stackIn_22_0 = (RuntimeException) ((Object) stackIn_21_0);
              stackIn_22_1 = (StringBuilder) ((Object) stackIn_21_1);
              stackIn_22_2 = "null";
              break L6;
            } else {
              stackIn_22_0 = (RuntimeException) ((Object) stackIn_21_0);
              stackIn_22_1 = (StringBuilder) ((Object) stackIn_21_1);
              stackIn_22_2 = "{...}";
              break L6;
            }
          }
          throw qb.a((Throwable) ((Object) stackIn_22_0), stackIn_22_2 + ',' + param5 + ',' + param6 + ')');
        }
        if (decompiledRegionSelector0 == 0) {
          return;
        } else {
          return;
        }
    }

    final static eg b(int param0, int param1, int param2, int param3, int param4) {
        int var6 = BrickABrac.field_J ? 1 : 0;
        eg var5 = (eg) ((Object) qc.field_n.d(-91));
        while (var5 != null) {
            if ((var5.field_l ^ -1) == (param1 ^ -1)) {
                return var5;
            }
            var5 = (eg) ((Object) qc.field_n.a((byte) 116));
        }
        eg var7 = new eg();
        var5 = var7;
        var7.field_q = param3;
        var7.field_p = param2;
        if (param0 != -2) {
            return (eg) null;
        }
        var5.field_l = param1;
        qc.field_n.a(var5, (byte) 3);
        uo.a(var5, param4, (byte) 68);
        return var5;
    }

    final oc h(byte param0) {
        kh var2;
        jc var3;
        int var4;
        L0: {
          var4 = BrickABrac.field_J ? 1 : 0;
          var2 = new kh(this.field_J);
          if (param0 == 25) {
            break L0;
          } else {
            cp.j((byte) -34);
            break L0;
          }
        }
        var3 = (jc) ((Object) var2.a(param0 + 215));
        L1: while (true) {
          if (var3 != null) {
            if (!var3.field_N) {
              var3 = (jc) ((Object) var2.b(7));
              continue L1;
            } else {
              return var3.g((byte) -3);
            }
          } else {
            return null;
          }
        }
    }

    final jc g(int param0) {
        kh var2;
        jc var3;
        int var4;
        L0: {
          var4 = BrickABrac.field_J ? 1 : 0;
          if (param0 < -96) {
            break L0;
          } else {
            field_G = (String) null;
            break L0;
          }
        }
        var2 = new kh(this.field_J);
        var3 = (jc) ((Object) var2.a(240));
        L1: while (true) {
          if (var3 != null) {
            if (var3.field_N) {
              return var3;
            } else {
              var3 = (jc) ((Object) var2.b(7));
              continue L1;
            }
          } else {
            return null;
          }
        }
    }

    static {
        field_H = "<%0> would need a rating of <%1> to play with the current options.";
        field_G = "Confirm Password: ";
    }
}
