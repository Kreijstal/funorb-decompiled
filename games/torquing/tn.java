/*
 * Decompiled by CFR-JS 0.4.0.
 */
final class tn extends gf {
    static String field_m;
    static uh field_l;

    final static void a(int param0, int param1) {
        int var2;
        int var3;
        int var4;
        int var5;
        int var6;
        int var7;
        var7 = Torquing.field_u;
        var2 = 0;
        var3 = jh.field_z;
        if ((var3 ^ -1) > -6) {
          L0: {
            var2 = 8192 * var3 * var3 / 1100;
            var4 = 1;
            var5 = 0;
            if ((param1 ^ -1) != -4) {
              break L0;
            } else {
              var4 = -1;
              break L0;
            }
          }
          L1: {
            if (-2 != (param1 ^ -1)) {
              break L1;
            } else {
              var5 = 1;
              break L1;
            }
          }
          L2: {
            if (4 != param1) {
              break L2;
            } else {
              var4 = 1;
              var5 = 1;
              break L2;
            }
          }
          L3: {
            if ((param1 ^ -1) == -6) {
              var4 = -1;
              var5 = 1;
              break L3;
            } else {
              break L3;
            }
          }
          L4: {
            if (6 == param1) {
              var5 = -1;
              var4 = 1;
              break L4;
            } else {
              break L4;
            }
          }
          if ((param1 ^ -1) != -8) {
            if ((param1 ^ -1) == -9) {
              L5: {
                var5 = -1;
                var4 = -1;
                if (-12 != (param1 ^ -1)) {
                  break L5;
                } else {
                  var4 = -1;
                  break L5;
                }
              }
              L6: {
                if (12 != param1) {
                  break L6;
                } else {
                  var5 = -1;
                  var4 = -1;
                  break L6;
                }
              }
              L7: {
                var6 = -21 % ((param0 - 17) / 52);
                if (param1 == 13) {
                  var4 = 1;
                  var5 = -1;
                  break L7;
                } else {
                  break L7;
                }
              }
              L8: {
                if (-15 != (param1 ^ -1)) {
                  break L8;
                } else {
                  var5 = 1;
                  var4 = -1;
                  break L8;
                }
              }
              L9: {
                if (param1 != 15) {
                  break L9;
                } else {
                  var4 = 1;
                  var5 = 1;
                  break L9;
                }
              }
              mo.field_B = lp.a(var4 * var2, false, var5 * var2);
              return;
            } else {
              L10: {
                if (-12 != (param1 ^ -1)) {
                  break L10;
                } else {
                  var4 = -1;
                  break L10;
                }
              }
              L11: {
                if (12 != param1) {
                  break L11;
                } else {
                  var5 = -1;
                  var4 = -1;
                  break L11;
                }
              }
              L12: {
                var6 = -21 % ((param0 - 17) / 52);
                if (param1 == 13) {
                  var4 = 1;
                  var5 = -1;
                  break L12;
                } else {
                  break L12;
                }
              }
              L13: {
                if (-15 != (param1 ^ -1)) {
                  break L13;
                } else {
                  var5 = 1;
                  var4 = -1;
                  break L13;
                }
              }
              L14: {
                if (param1 != 15) {
                  break L14;
                } else {
                  var4 = 1;
                  var5 = 1;
                  break L14;
                }
              }
              mo.field_B = lp.a(var4 * var2, false, var5 * var2);
              return;
            }
          } else {
            L15: {
              var5 = -1;
              var4 = -1;
              if (-12 != (param1 ^ -1)) {
                break L15;
              } else {
                var4 = -1;
                break L15;
              }
            }
            L16: {
              if (12 != param1) {
                break L16;
              } else {
                var5 = -1;
                var4 = -1;
                break L16;
              }
            }
            L17: {
              var6 = -21 % ((param0 - 17) / 52);
              if (param1 == 13) {
                var4 = 1;
                var5 = -1;
                break L17;
              } else {
                break L17;
              }
            }
            L18: {
              if (-15 != (param1 ^ -1)) {
                break L18;
              } else {
                var5 = 1;
                var4 = -1;
                break L18;
              }
            }
            L19: {
              if (param1 != 15) {
                break L19;
              } else {
                var4 = 1;
                var5 = 1;
                break L19;
              }
            }
            mo.field_B = lp.a(var4 * var2, false, var5 * var2);
            return;
          }
        } else {
          if (var3 < 105) {
            L20: {
              var2 = (var3 * 16384 + -40960) / 220;
              var4 = 1;
              var5 = 0;
              if ((param1 ^ -1) != -4) {
                break L20;
              } else {
                var4 = -1;
                break L20;
              }
            }
            L21: {
              if (-2 != (param1 ^ -1)) {
                break L21;
              } else {
                var5 = 1;
                break L21;
              }
            }
            L22: {
              if (4 != param1) {
                break L22;
              } else {
                var4 = 1;
                var5 = 1;
                break L22;
              }
            }
            L23: {
              if ((param1 ^ -1) == -6) {
                var4 = -1;
                var5 = 1;
                break L23;
              } else {
                break L23;
              }
            }
            L24: {
              if (6 == param1) {
                var5 = -1;
                var4 = 1;
                break L24;
              } else {
                break L24;
              }
            }
            if ((param1 ^ -1) != -8) {
              if ((param1 ^ -1) == -9) {
                L25: {
                  var5 = -1;
                  var4 = -1;
                  if (-12 != (param1 ^ -1)) {
                    break L25;
                  } else {
                    var4 = -1;
                    break L25;
                  }
                }
                L26: {
                  if (12 != param1) {
                    break L26;
                  } else {
                    var5 = -1;
                    var4 = -1;
                    break L26;
                  }
                }
                L27: {
                  var6 = -21 % ((param0 - 17) / 52);
                  if (param1 == 13) {
                    var4 = 1;
                    var5 = -1;
                    break L27;
                  } else {
                    break L27;
                  }
                }
                L28: {
                  if (-15 != (param1 ^ -1)) {
                    break L28;
                  } else {
                    var5 = 1;
                    var4 = -1;
                    break L28;
                  }
                }
                L29: {
                  if (param1 != 15) {
                    break L29;
                  } else {
                    var4 = 1;
                    var5 = 1;
                    break L29;
                  }
                }
                mo.field_B = lp.a(var4 * var2, false, var5 * var2);
                return;
              } else {
                L30: {
                  if (-12 != (param1 ^ -1)) {
                    break L30;
                  } else {
                    var4 = -1;
                    break L30;
                  }
                }
                L31: {
                  if (12 != param1) {
                    break L31;
                  } else {
                    var5 = -1;
                    var4 = -1;
                    break L31;
                  }
                }
                L32: {
                  var6 = -21 % ((param0 - 17) / 52);
                  if (param1 == 13) {
                    var4 = 1;
                    var5 = -1;
                    break L32;
                  } else {
                    break L32;
                  }
                }
                L33: {
                  if (-15 != (param1 ^ -1)) {
                    break L33;
                  } else {
                    var5 = 1;
                    var4 = -1;
                    break L33;
                  }
                }
                L34: {
                  if (param1 != 15) {
                    break L34;
                  } else {
                    var4 = 1;
                    var5 = 1;
                    break L34;
                  }
                }
                mo.field_B = lp.a(var4 * var2, false, var5 * var2);
                return;
              }
            } else {
              L35: {
                var5 = -1;
                var4 = -1;
                if (-12 != (param1 ^ -1)) {
                  break L35;
                } else {
                  var4 = -1;
                  break L35;
                }
              }
              L36: {
                if (12 != param1) {
                  break L36;
                } else {
                  var5 = -1;
                  var4 = -1;
                  break L36;
                }
              }
              L37: {
                var6 = -21 % ((param0 - 17) / 52);
                if (param1 == 13) {
                  var4 = 1;
                  var5 = -1;
                  break L37;
                } else {
                  break L37;
                }
              }
              L38: {
                if (-15 != (param1 ^ -1)) {
                  break L38;
                } else {
                  var5 = 1;
                  var4 = -1;
                  break L38;
                }
              }
              L39: {
                if (param1 != 15) {
                  break L39;
                } else {
                  var4 = 1;
                  var5 = 1;
                  break L39;
                }
              }
              mo.field_B = lp.a(var4 * var2, false, var5 * var2);
              return;
            }
          } else {
            if ((var3 ^ -1) > -121) {
              L40: {
                var3 = -var3 + 120;
                var2 = 8192 - 8192 * var3 * var3 / 3300;
                var4 = 1;
                var5 = 0;
                if ((param1 ^ -1) != -4) {
                  break L40;
                } else {
                  var4 = -1;
                  break L40;
                }
              }
              L41: {
                if (-2 != (param1 ^ -1)) {
                  break L41;
                } else {
                  var5 = 1;
                  break L41;
                }
              }
              L42: {
                if (4 != param1) {
                  break L42;
                } else {
                  var4 = 1;
                  var5 = 1;
                  break L42;
                }
              }
              L43: {
                if ((param1 ^ -1) == -6) {
                  var4 = -1;
                  var5 = 1;
                  break L43;
                } else {
                  break L43;
                }
              }
              L44: {
                if (6 == param1) {
                  var5 = -1;
                  var4 = 1;
                  break L44;
                } else {
                  break L44;
                }
              }
              if ((param1 ^ -1) == -8) {
                L45: {
                  var5 = -1;
                  var4 = -1;
                  if (-12 != (param1 ^ -1)) {
                    break L45;
                  } else {
                    var4 = -1;
                    break L45;
                  }
                }
                L46: {
                  if (12 != param1) {
                    break L46;
                  } else {
                    var5 = -1;
                    var4 = -1;
                    break L46;
                  }
                }
                L47: {
                  var6 = -21 % ((param0 - 17) / 52);
                  if (param1 == 13) {
                    var4 = 1;
                    var5 = -1;
                    break L47;
                  } else {
                    break L47;
                  }
                }
                L48: {
                  if (-15 != (param1 ^ -1)) {
                    break L48;
                  } else {
                    var5 = 1;
                    var4 = -1;
                    break L48;
                  }
                }
                L49: {
                  if (param1 != 15) {
                    break L49;
                  } else {
                    var4 = 1;
                    var5 = 1;
                    break L49;
                  }
                }
                mo.field_B = lp.a(var4 * var2, false, var5 * var2);
                return;
              } else {
                L50: {
                  if ((param1 ^ -1) != -9) {
                    break L50;
                  } else {
                    var5 = -1;
                    var4 = -1;
                    break L50;
                  }
                }
                L51: {
                  if (-12 != (param1 ^ -1)) {
                    break L51;
                  } else {
                    var4 = -1;
                    break L51;
                  }
                }
                L52: {
                  if (12 != param1) {
                    break L52;
                  } else {
                    var5 = -1;
                    var4 = -1;
                    break L52;
                  }
                }
                L53: {
                  var6 = -21 % ((param0 - 17) / 52);
                  if (param1 == 13) {
                    var4 = 1;
                    var5 = -1;
                    break L53;
                  } else {
                    break L53;
                  }
                }
                L54: {
                  if (-15 != (param1 ^ -1)) {
                    break L54;
                  } else {
                    var5 = 1;
                    var4 = -1;
                    break L54;
                  }
                }
                L55: {
                  if (param1 != 15) {
                    break L55;
                  } else {
                    var4 = 1;
                    var5 = 1;
                    break L55;
                  }
                }
                mo.field_B = lp.a(var4 * var2, false, var5 * var2);
                return;
              }
            } else {
              L56: {
                var4 = 1;
                var5 = 0;
                if ((param1 ^ -1) != -4) {
                  break L56;
                } else {
                  var4 = -1;
                  break L56;
                }
              }
              L57: {
                if (-2 != (param1 ^ -1)) {
                  break L57;
                } else {
                  var5 = 1;
                  break L57;
                }
              }
              L58: {
                if (4 != param1) {
                  break L58;
                } else {
                  var4 = 1;
                  var5 = 1;
                  break L58;
                }
              }
              L59: {
                if ((param1 ^ -1) == -6) {
                  var4 = -1;
                  var5 = 1;
                  break L59;
                } else {
                  break L59;
                }
              }
              L60: {
                if (6 == param1) {
                  var5 = -1;
                  var4 = 1;
                  break L60;
                } else {
                  break L60;
                }
              }
              if ((param1 ^ -1) == -8) {
                L61: {
                  var5 = -1;
                  var4 = -1;
                  if (-12 != (param1 ^ -1)) {
                    break L61;
                  } else {
                    var4 = -1;
                    break L61;
                  }
                }
                L62: {
                  if (12 != param1) {
                    break L62;
                  } else {
                    var5 = -1;
                    var4 = -1;
                    break L62;
                  }
                }
                L63: {
                  var6 = -21 % ((param0 - 17) / 52);
                  if (param1 == 13) {
                    var4 = 1;
                    var5 = -1;
                    break L63;
                  } else {
                    break L63;
                  }
                }
                L64: {
                  if (-15 != (param1 ^ -1)) {
                    break L64;
                  } else {
                    var5 = 1;
                    var4 = -1;
                    break L64;
                  }
                }
                L65: {
                  if (param1 != 15) {
                    break L65;
                  } else {
                    var4 = 1;
                    var5 = 1;
                    break L65;
                  }
                }
                mo.field_B = lp.a(var4 * var2, false, var5 * var2);
                return;
              } else {
                L66: {
                  if ((param1 ^ -1) != -9) {
                    break L66;
                  } else {
                    var5 = -1;
                    var4 = -1;
                    break L66;
                  }
                }
                L67: {
                  if (-12 != (param1 ^ -1)) {
                    break L67;
                  } else {
                    var4 = -1;
                    break L67;
                  }
                }
                L68: {
                  if (12 != param1) {
                    break L68;
                  } else {
                    var5 = -1;
                    var4 = -1;
                    break L68;
                  }
                }
                L69: {
                  var6 = -21 % ((param0 - 17) / 52);
                  if (param1 == 13) {
                    var4 = 1;
                    var5 = -1;
                    break L69;
                  } else {
                    break L69;
                  }
                }
                L70: {
                  if (-15 != (param1 ^ -1)) {
                    break L70;
                  } else {
                    var5 = 1;
                    var4 = -1;
                    break L70;
                  }
                }
                L71: {
                  if (param1 != 15) {
                    break L71;
                  } else {
                    var4 = 1;
                    var5 = 1;
                    break L71;
                  }
                }
                mo.field_B = lp.a(var4 * var2, false, var5 * var2);
                return;
              }
            }
          }
        }
    }

    tn(tk param0) {
        super(param0);
    }

    public static void e(int param0) {
        if (param0 != 0) {
            tn.e(-1);
            field_l = null;
            field_m = null;
            return;
        }
        field_l = null;
        field_m = null;
    }

    final cd a(int param0, String param1) {
        int var3_int = 0;
        RuntimeException var3 = null;
        int stackIn_3_0 = 0;
        cd stackIn_6_0 = null;
        cd stackIn_9_0 = null;
        cd stackIn_11_0 = null;
        RuntimeException stackIn_14_0 = null;
        StringBuilder stackIn_14_1 = null;
        RuntimeException stackIn_15_0 = null;
        StringBuilder stackIn_15_1 = null;
        String stackIn_15_2 = null;
        int decompiledRegionSelector0 = 0;
        RuntimeException decompiledCaughtException = null;
        try {
          L0: {
            L1: {
              if (lg.a(param1, 98) != null) {
                stackIn_3_0 = 0;
                break L1;
              } else {
                stackIn_3_0 = 1;
                break L1;
              }
            }
            var3_int = stackIn_3_0;
            if (var3_int == 0) {
              stackIn_6_0 = so.field_u;
              decompiledRegionSelector0 = 0;
              break L0;
            } else {
              if (param0 > 62) {
                stackIn_11_0 = so.field_s;
                decompiledRegionSelector0 = 2;
                break L0;
              } else {
                stackIn_9_0 = (cd) null;
                decompiledRegionSelector0 = 1;
                break L0;
              }
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L2: {
            var3 = decompiledCaughtException;
            stackIn_14_0 = (RuntimeException) (var3);

            stackIn_14_1 = new StringBuilder().append("tn.F(").append(param0).append(',');

            if (param1 == null) {
              stackIn_15_0 = (RuntimeException) ((Object) stackIn_14_0);
              stackIn_15_1 = (StringBuilder) ((Object) stackIn_14_1);
              stackIn_15_2 = "null";
              break L2;
            } else {
              stackIn_15_0 = (RuntimeException) ((Object) stackIn_14_0);
              stackIn_15_1 = (StringBuilder) ((Object) stackIn_14_1);
              stackIn_15_2 = "{...}";
              break L2;
            }
          }
          throw rb.a((Throwable) ((Object) stackIn_15_0), stackIn_15_2 + ')');
        }
        if (decompiledRegionSelector0 == 0) {
          return stackIn_6_0;
        } else {
          if (decompiledRegionSelector0 == 1) {
            return stackIn_9_0;
          } else {
            return stackIn_11_0;
          }
        }
    }

    final String a(byte param0, String param1) {
        RuntimeException var3 = null;
        String stackIn_4_0 = null;
        String stackIn_6_0 = null;
        RuntimeException stackIn_9_0 = null;
        StringBuilder stackIn_9_1 = null;
        RuntimeException stackIn_10_0 = null;
        StringBuilder stackIn_10_1 = null;
        String stackIn_10_2 = null;
        int decompiledRegionSelector0 = 0;
        RuntimeException decompiledCaughtException = null;
        try {
          L0: {
            L1: {
              if (param0 == -121) {
                break L1;
              } else {
                field_m = (String) null;
                break L1;
              }
            }
            if (this.a(78, param1) != so.field_u) {
              stackIn_6_0 = cp.field_h;
              decompiledRegionSelector0 = 1;
              break L0;
            } else {
              stackIn_4_0 = pl.field_t;
              decompiledRegionSelector0 = 0;
              break L0;
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L2: {
            var3 = decompiledCaughtException;
            stackIn_9_0 = (RuntimeException) (var3);

            stackIn_9_1 = new StringBuilder().append("tn.H(").append(param0).append(',');

            if (param1 == null) {
              stackIn_10_0 = (RuntimeException) ((Object) stackIn_9_0);
              stackIn_10_1 = (StringBuilder) ((Object) stackIn_9_1);
              stackIn_10_2 = "null";
              break L2;
            } else {
              stackIn_10_0 = (RuntimeException) ((Object) stackIn_9_0);
              stackIn_10_1 = (StringBuilder) ((Object) stackIn_9_1);
              stackIn_10_2 = "{...}";
              break L2;
            }
          }
          throw rb.a((Throwable) ((Object) stackIn_10_0), stackIn_10_2 + ')');
        }
        if (decompiledRegionSelector0 == 0) {
          return stackIn_4_0;
        } else {
          return stackIn_6_0;
        }
    }

    static {
        field_m = "Accept";
    }
}
