/*
 * Decompiled by CFR-JS 0.4.0.
 */
final class uf {
    private int field_e;
    static int field_g;
    private int field_j;
    private float field_f;
    private int field_a;
    static String field_d;
    static int[][] field_b;
    static String field_c;
    static String field_i;
    private boolean field_k;
    static ja field_h;

    final void a(boolean param0) {
        this.field_k = param0 ? true : false;
        nm.b(-128, 59);
    }

    public static void b(int param0) {
        field_h = null;
        field_c = null;
        field_b = (int[][]) null;
        field_i = null;
        if (param0 != 16737894) {
            field_d = (String) null;
        }
        field_d = null;
    }

    final void a(nm param0, byte param1) {
        ri[] stackIn_3_0 = null;
        ri[] stackIn_4_0 = null;
        int stackIn_4_1 = 0;
        RuntimeException stackIn_10_0 = null;
        StringBuilder stackIn_10_1 = null;
        RuntimeException stackIn_11_0 = null;
        StringBuilder stackIn_11_1 = null;
        String stackIn_11_2 = null;
        RuntimeException decompiledCaughtException = null;
        RuntimeException var3 = null;
        try {
          L0: {
            L1: {
              if (0.0f > this.field_f) {
                break L1;
              } else {
                L2: {
                  stackIn_3_0 = pp.field_q;

                  if (!this.field_k) {
                    stackIn_4_0 = (ri[]) ((Object) stackIn_3_0);
                    stackIn_4_1 = (int)this.field_f;
                    break L2;
                  } else {
                    stackIn_4_0 = (ri[]) ((Object) stackIn_3_0);
                    stackIn_4_1 = 6;
                    break L2;
                  }
                }
                ((ri) (Object) stackIn_4_0[stackIn_4_1]).a(param0.c(this.field_e, -20126) - 12, -12 + param0.d(this.field_j, 111), this.field_a);
                break L1;
              }
            }
            L3: {
              if (param1 >= 33) {
                break L3;
              } else {
                this.field_a = -63;
                break L3;
              }
            }
            break L0;
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L4: {
            var3 = decompiledCaughtException;
            stackIn_10_0 = (RuntimeException) (var3);

            stackIn_10_1 = new StringBuilder().append("uf.A(");

            if (param0 == null) {
              stackIn_11_0 = (RuntimeException) ((Object) stackIn_10_0);
              stackIn_11_1 = (StringBuilder) ((Object) stackIn_10_1);
              stackIn_11_2 = "null";
              break L4;
            } else {
              stackIn_11_0 = (RuntimeException) ((Object) stackIn_10_0);
              stackIn_11_1 = (StringBuilder) ((Object) stackIn_10_1);
              stackIn_11_2 = "{...}";
              break L4;
            }
          }
          throw fa.a((Throwable) ((Object) stackIn_11_0), stackIn_11_2 + ',' + param1 + ')');
        }
    }

    final void a(int param0, int param1, int param2) {
        this.field_a = 256;
        this.field_f = (float)param0;
        this.field_j = param2;
        this.field_e = param1;
    }

    final void b(boolean param0) {
        this.field_k = false;
        if (!param0) {
            nm var3 = (nm) null;
            this.a((nm) null, (byte) 30);
        }
        nm.b(-126, 20);
    }

    final void a(int param0) {
        if (!(this.field_f == -1.0f)) {
            this.field_f = this.field_f + 0.5f;
            if (-26 > (this.field_a ^ -1)) {
                this.field_a = this.field_a - 25;
            }
            if (this.field_f > 5.0f) {
                this.field_f = -1.0f;
            }
        }
        if (param0 != -29987) {
            field_b = (int[][]) null;
        }
    }

    uf() {
        this.field_j = 0;
        this.field_e = 0;
        this.field_f = -1.0f;
        this.field_k = false;
    }

    final static void a(int param0, boolean param1, byte param2) {
        tq stackIn_5_0;
        int stackIn_5_1;
        boolean stackIn_5_2;
        int stackIn_5_3;
        tq stackIn_6_0 = null;
        int stackIn_6_1 = 0;
        boolean stackIn_6_2 = false;
        int stackIn_6_3 = 0;
        int stackIn_6_4 = 0;
        cj stackIn_23_0 = null;
        cj stackIn_24_0 = null;
        String stackIn_24_1 = null;
        int stackIn_27_0 = 0;
        int decompiledRegionSelector0 = 0;
        RuntimeException decompiledCaughtException = null;
        int var3_int = 0;
        RuntimeException var3 = null;
        th var4 = null;
        Object var5 = null;
        ml var6 = null;
        int var7 = 0;
        int var8 = 0;
        int var9 = 0;
        int var10 = 0;
        int var11 = 0;
        String var13_ref_String = null;
        int var13 = 0;
        int var14 = 0;
        int var15 = 0;
        jb var17 = null;
        jb var18 = null;
        var15 = ZombieDawnMulti.field_E ? 1 : 0;
        try {
          L0: {
            if (param2 == 39) {
              L1: {
                stackIn_5_0 = gl.field_L.field_Jb;

                stackIn_5_1 = 2;

                stackIn_5_2 = param1;

                stackIn_5_3 = 2 + jk.field_f;

                if (vc.field_d != gl.field_L.field_Jb) {
                  stackIn_6_0 = (tq) ((Object) stackIn_5_0);
                  stackIn_6_1 = stackIn_5_1;
                  stackIn_6_2 = stackIn_5_2;
                  stackIn_6_3 = stackIn_5_3;
                  stackIn_6_4 = 0;
                  break L1;
                } else {
                  stackIn_6_0 = (tq) ((Object) stackIn_5_0);
                  stackIn_6_1 = stackIn_5_1;
                  stackIn_6_2 = stackIn_5_2;
                  stackIn_6_3 = stackIn_5_3;
                  stackIn_6_4 = 1;
                  break L1;
                }
              }
              L2: {
                L3: {
                  L4: {
                    var3_int = ((tq) (Object) stackIn_6_0).a(stackIn_6_1, stackIn_6_2, stackIn_6_3, stackIn_6_4 != 0, param0 * (3 * jk.field_f + 6), -90) ? 1 : 0;
                    var4 = gl.field_L.field_Ib.field_Cb;
                    if (ge.field_zb == 2) {
                      break L4;
                    } else {
                      if ((ge.field_zb ^ -1) != -2) {
                        break L3;
                      } else {
                        break L4;
                      }
                    }
                  }
                  gl.field_L.field_Z = true;
                  gl.field_L.field_Jb.field_Jb.field_wb = null;
                  var5 = null;
                  var6 = (ml) ((Object) var4.c(43));
                  L5: while (true) {
                    L6: {
                      L7: {
                        if (var6 == null) {
                          break L7;
                        } else {
                          var7 = 0;
                          if (var15 != 0) {
                            break L6;
                          } else {
                            L8: {
                              if (var6.field_Cb == null) {
                                var6.field_Ib = new cj(0L, m.field_x);
                                var6.a((byte) 50, var6.field_Ib);
                                var6.field_Gb = new cj(0L, m.field_x);
                                var6.a((byte) 50, var6.field_Gb);
                                var7 = 1;
                                var6.f(-128);
                                break L8;
                              } else {
                                break L8;
                              }
                            }
                            L9: {
                              var6.field_zb = gl.field_L.field_Ib.field_zb;
                              var6.field_Ib.a(jk.field_f, 0, var6.field_zb, 0, -3344);
                              var8 = 0;
                              if (var6.field_Kb == null) {
                                break L9;
                              } else {
                                if (var6.field_Kb.equals("")) {
                                  break L9;
                                } else {
                                  var6.field_Gb.field_ob = 16737894;
                                  var6.field_Gb.field_rb = pa.field_T;
                                  var6.field_Gb.a(jk.field_f, 0, 3 + pa.field_T.field_z, 0, -3344);
                                  var8 = 1;
                                  break L9;
                                }
                              }
                            }
                            L10: {
                              var9 = 0;
                              var10 = var6.field_zb;
                              if (var8 != 0) {
                                var9 = 3 + pa.field_T.field_z;
                                var10 = var10 - var9;
                                break L10;
                              } else {
                                break L10;
                              }
                            }
                            L11: {
                              stackIn_23_0 = var6.field_Ib;

                              if (var10 > 0) {
                                stackIn_24_0 = (cj) ((Object) stackIn_23_0);
                                stackIn_24_1 = fm.a(var6.field_Ib.field_yb, var6.field_Hb, var10);
                                break L11;
                              } else {
                                stackIn_24_0 = (cj) ((Object) stackIn_23_0);
                                stackIn_24_1 = var6.field_Hb;
                                break L11;
                              }
                            }
                            L12: {
                              stackIn_24_0.field_wb = stackIn_24_1;
                              if (var6.field_Ib.field_wb.equals(var6.field_Hb)) {
                                stackIn_27_0 = 0;
                                break L12;
                              } else {
                                stackIn_27_0 = 1;
                                break L12;
                              }
                            }
                            L13: {
                              var11 = stackIn_27_0;
                              var6.field_Ib.a(jk.field_f, 0, var10, var9, param2 + -3383);
                              if (var3_int == 0) {
                                var6.field_I = -var6.field_z + jk.field_f;
                                break L13;
                              } else {
                                break L13;
                              }
                            }
                            L14: {
                              if (var7 != 0) {
                                gl.field_L.field_Ib.a(2, var6, 1300, (cj) (var5));
                                break L14;
                              } else {
                                break L14;
                              }
                            }
                            L15: {
                              L16: {
                                L17: {
                                  if (null == var6.field_Gb) {
                                    break L17;
                                  } else {
                                    if (var6.field_Gb.field_L) {
                                      break L16;
                                    } else {
                                      break L17;
                                    }
                                  }
                                }
                                if (!var6.field_Ib.field_L) {
                                  break L15;
                                } else {
                                  if (var11 == 0) {
                                    break L15;
                                  } else {
                                    ui.field_j = var6.field_Hb;
                                    if (var15 == 0) {
                                      break L15;
                                    } else {
                                      break L16;
                                    }
                                  }
                                }
                              }
                              ui.field_j = var6.field_Kb;
                              break L15;
                            }
                            L18: {
                              if (-1 != (var6.field_T ^ -1)) {
                                ve.a(0L, 196, var6.field_Hb, gl.field_L.field_Jb, var6.field_Pb, var6, -1, (int[]) null, -1);
                                var17 = pk.field_b;
                                var13_ref_String = qh.field_d;
                                var17.field_j.a((byte) -118, var13_ref_String, 7);
                                var18 = pk.field_b;
                                var13 = vb.field_f;
                                var14 = ka.field_t;
                                var18.field_j.b(4542, var13, 0, var14, 0);
                                break L18;
                              } else {
                                break L18;
                              }
                            }
                            var5 = var6;
                            var6 = (ml) ((Object) var4.b(6));
                            if (var15 == 0) {
                              continue L5;
                            } else {
                              break L7;
                            }
                          }
                        }
                      }
                      if (0 == gl.field_L.field_Gb.field_T) {
                        break L6;
                      } else {
                        gi.field_i = new dk(gl.field_L.field_Gb.field_w, gl.field_L.field_Gb.field_qb, gl.field_L.field_Gb.field_zb, gl.field_L.field_Gb.field_z, pl.field_K, kd.field_a, vo.field_m, vo.field_m);
                        ic.field_e = 2;
                        break L6;
                      }
                    }
                    L19: {
                      if (-1 == (gl.field_L.field_Hb.field_T ^ -1)) {
                        break L19;
                      } else {
                        gi.field_i = new dk(gl.field_L.field_Hb.field_w, gl.field_L.field_Hb.field_qb, gl.field_L.field_Hb.field_zb, gl.field_L.field_Hb.field_z, gf.field_d, kd.field_a, vo.field_m, vo.field_m);
                        ic.field_e = 3;
                        break L19;
                      }
                    }
                    if (var15 == 0) {
                      break L2;
                    } else {
                      break L3;
                    }
                  }
                }
                gl.field_L.field_Z = false;
                gl.field_L.field_Jb.field_Jb.field_wb = hj.field_e;
                ob.a(gl.field_L.field_Ib, 37);
                break L2;
              }
              decompiledRegionSelector0 = 1;
              break L0;
            } else {
              decompiledRegionSelector0 = 0;
              break L0;
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          var3 = decompiledCaughtException;
          throw fa.a((Throwable) ((Object) var3), "uf.E(" + param0 + ',' + param1 + ',' + param2 + ')');
        }
        if (decompiledRegionSelector0 == 0) {
          return;
        } else {
          return;
        }
    }

    static {
        field_d = "Enhance your technological skills.";
        field_i = "<%0> has resigned.";
        field_c = "Aggressive zombies get the brains.";
    }
}
