/*
 * Decompiled by CFR-JS 0.4.0.
 */
class fj {
    private um field_k;
    private static j field_b;
    static int field_r;
    static boolean field_u;
    int[] field_s;
    private static int field_d;
    private boolean field_p;
    private long field_g;
    private int field_q;
    private int field_l;
    private um[] field_j;
    private int field_e;
    private int field_a;
    private long field_o;
    private boolean field_t;
    private int field_m;
    private int field_h;
    private int field_c;
    private long field_n;
    private int field_i;
    private um[] field_f;

    void g() {
    }

    public static void d() {
        field_b = null;
    }

    void h() throws Exception {
    }

    final synchronized void f() {
        try {
            this.field_t = true;
            try {
                this.c();
            } catch (Exception exception) {
                this.g();
                this.field_o = sf.a((byte) 100) + 2000L;
            }
        } catch (RuntimeException | Error decompiledUncheckedException) {
            throw decompiledUncheckedException;
        } catch (Throwable decompiledCheckedException) {
            throw new RuntimeException(decompiledCheckedException);
        }
    }

    int b() throws Exception {
        return this.field_l;
    }

    final synchronized void e() {
        try {
            int decompiledRegionSelector0 = 0;
            Throwable decompiledCaughtException = null;
            long var1 = 0L;
            Exception var3 = null;
            int var3_int = 0;
            int var4 = 0;
            if (!this.field_p) {
              var1 = sf.a((byte) -55);
              try {
                L0: {
                  L1: {
                    if (var1 <= this.field_g + 6000L) {
                      break L1;
                    } else {
                      this.field_g = var1 - 6000L;
                      break L1;
                    }
                  }
                  L2: while (true) {
                    if (var1 <= this.field_g + 5000L) {
                      break L0;
                    } else {
                      this.a(256);
                      this.field_g = this.field_g + (long)(256000 / field_r);
                      var1 = sf.a((byte) 99);
                      continue L2;
                    }
                  }
                }
              } catch (java.lang.Exception decompiledCaughtParameter0) {
                decompiledCaughtException = decompiledCaughtParameter0;
                L3: {
                  var3 = (Exception) (Object) decompiledCaughtException;
                  this.field_g = var1;
                  break L3;
                }
              }
              if (this.field_s != null) {
                try {
                  L4: {
                    L5: {
                      if (this.field_o == 0L) {
                        break L5;
                      } else {
                        if (var1 >= this.field_o) {
                          this.b(this.field_l);
                          this.field_o = 0L;
                          this.field_t = true;
                          break L5;
                        } else {
                          decompiledRegionSelector0 = 0;
                          break L4;
                        }
                      }
                    }
                    L6: {
                      var3_int = this.b();
                      if (this.field_h - var3_int <= this.field_e) {
                        break L6;
                      } else {
                        this.field_e = this.field_h - var3_int;
                        break L6;
                      }
                    }
                    L7: {
                      var4 = this.field_m + this.field_i;
                      if (var4 + 256 <= 16384) {
                        break L7;
                      } else {
                        var4 = 16128;
                        break L7;
                      }
                    }
                    L8: {
                      if (var4 + 256 <= this.field_l) {
                        break L8;
                      } else {
                        L9: {
                          this.field_l = this.field_l + 1024;
                          if (this.field_l <= 16384) {
                            break L9;
                          } else {
                            this.field_l = 16384;
                            break L9;
                          }
                        }
                        this.g();
                        this.b(this.field_l);
                        var3_int = 0;
                        this.field_t = true;
                        if (var4 + 256 <= this.field_l) {
                          break L8;
                        } else {
                          var4 = this.field_l - 256;
                          this.field_i = var4 - this.field_m;
                          break L8;
                        }
                      }
                    }
                    L10: while (true) {
                      if (var3_int >= var4) {
                        L11: {
                          if (var1 <= this.field_n) {
                            break L11;
                          } else {
                            L12: {
                              if (this.field_t) {
                                this.field_t = false;
                                break L12;
                              } else {
                                L13: {
                                  if (this.field_e != 0) {
                                    break L13;
                                  } else {
                                    if (this.field_a != 0) {
                                      break L13;
                                    } else {
                                      this.g();
                                      this.field_o = var1 + 2000L;
                                      decompiledRegionSelector0 = 1;
                                      break L4;
                                    }
                                  }
                                }
                                this.field_i = Math.min(this.field_a, this.field_e);
                                this.field_a = this.field_e;
                                break L12;
                              }
                            }
                            this.field_e = 0;
                            this.field_n = var1 + 2000L;
                            break L11;
                          }
                        }
                        this.field_h = var3_int;
                        decompiledRegionSelector0 = 2;
                        break L4;
                      } else {
                        this.a(this.field_s, 256);
                        this.h();
                        var3_int += 256;
                        continue L10;
                      }
                    }
                  }
                } catch (java.lang.Exception decompiledCaughtParameter1) {
                  decompiledCaughtException = decompiledCaughtParameter1;
                  L14: {
                    var3 = (Exception) (Object) decompiledCaughtException;
                    this.g();
                    this.field_o = var1 + 2000L;
                    decompiledRegionSelector0 = 2;
                    break L14;
                  }
                }
                if (decompiledRegionSelector0 == 0) {
                  return;
                } else {
                  if (decompiledRegionSelector0 == 1) {
                    return;
                  } else {
                    return;
                  }
                }
              } else {
                return;
              }
            } else {
              return;
            }
        } catch (RuntimeException | Error decompiledUncheckedException) {
            throw decompiledUncheckedException;
        } catch (Throwable decompiledCheckedException) {
            throw new RuntimeException(decompiledCheckedException);
        }
    }

    private final static void a(um param0) {
        param0.field_n = false;
        if (param0.field_q != null) {
            param0.field_q.field_n = 0;
        }
        um var1 = param0.b();
        while (var1 != null) {
            fj.a(var1);
            var1 = param0.c();
        }
    }

    final synchronized void a() {
        int var1;
        int var2;
        L0: {
          if (field_b == null) {
            break L0;
          } else {
            var1 = 1;
            var2 = 0;
            L1: while (true) {
              if (var2 >= 2) {
                if (var1 == 0) {
                  break L0;
                } else {
                  field_b.field_b = true;
                  L2: while (true) {
                    if (!field_b.field_i) {
                      field_b = null;
                      break L0;
                    } else {
                      gf.a(50L, -10309);
                      continue L2;
                    }
                  }
                }
              } else {
                L3: {
                  if (field_b.field_g[var2] != this) {
                    break L3;
                  } else {
                    field_b.field_g[var2] = null;
                    break L3;
                  }
                }
                if (field_b.field_g[var2] != null) {
                  var1 = 0;
                  var2++;
                  continue L1;
                } else {
                  var2++;
                  continue L1;
                }
              }
            }
          }
        }
        this.g();
        this.field_s = null;
        this.field_p = true;
    }

    final static fj a(vf param0, java.awt.Component param1, int param2, int param3) {
        try {
            sm var4 = null;
            Throwable var4_ref = null;
            sm var5 = null;
            sm stackIn_10_0 = null;
            int stackIn_10_1 = 0;
            sm stackIn_11_0 = null;
            int stackIn_11_1 = 0;
            int stackIn_11_2 = 0;
            sm stackIn_21_0 = null;
            Throwable decompiledCaughtException = null;
            if (field_r != 0) {
              L0: {
                if (param2 < 0) {
                  break L0;
                } else {
                  if (param2 < 2) {
                    L1: {
                      if (param3 >= 256) {
                        break L1;
                      } else {
                        param3 = 256;
                        break L1;
                      }
                    }
                    try {
                      L2: {
                        L3: {
                          var5 = new sm();
                          var4 = var5;
                          stackIn_10_0 = (sm) (var4);

                          stackIn_10_1 = 256;

                          if (!field_u) {
                            stackIn_11_0 = (sm) ((Object) stackIn_10_0);
                            stackIn_11_1 = stackIn_10_1;
                            stackIn_11_2 = 1;
                            break L3;
                          } else {
                            stackIn_11_0 = (sm) ((Object) stackIn_10_0);
                            stackIn_11_1 = stackIn_10_1;
                            stackIn_11_2 = 2;
                            break L3;
                          }
                        }
                        L4: {
                          ((fj) ((Object) stackIn_11_0)).field_s = new int[stackIn_11_1 * stackIn_11_2];
                          ((fj) ((Object) var4)).field_m = param3;
                          ((fj) ((Object) var4)).a(param1);
                          ((fj) ((Object) var4)).field_l = (param3 & -1024) + 1024;
                          if (((fj) ((Object) var4)).field_l <= 16384) {
                            break L4;
                          } else {
                            ((fj) ((Object) var4)).field_l = 16384;
                            break L4;
                          }
                        }
                        L5: {
                          ((fj) ((Object) var4)).b(((fj) ((Object) var4)).field_l);
                          if (field_d <= 0) {
                            break L5;
                          } else {
                            if (field_b != null) {
                              break L5;
                            } else {
                              field_b = new j();
                              field_b.field_c = param0;
                              param0.a((Runnable) ((Object) field_b), (byte) 18, field_d);
                              break L5;
                            }
                          }
                        }
                        L6: {
                          if (field_b == null) {
                            break L6;
                          } else {
                            if (field_b.field_g[param2] == null) {
                              field_b.field_g[param2] = (fj) ((Object) var5);
                              break L6;
                            } else {
                              throw new IllegalArgumentException();
                            }
                          }
                        }
                        stackIn_21_0 = (sm) (var4);
                        break L2;
                      }
                    } catch (java.lang.Throwable decompiledCaughtParameter0) {
                      decompiledCaughtException = decompiledCaughtParameter0;
                      var4_ref = decompiledCaughtException;
                      return new fj();
                    }
                    return (fj) ((Object) stackIn_21_0);
                  } else {
                    break L0;
                  }
                }
              }
              throw new IllegalArgumentException();
            } else {
              throw new IllegalStateException();
            }
        } catch (RuntimeException | Error decompiledUncheckedException) {
            throw decompiledUncheckedException;
        } catch (Throwable decompiledCheckedException) {
            throw new RuntimeException(decompiledCheckedException);
        }
    }

    private final void a(int[] param0, int param1) {
        int var3;
        int var4;
        int var5;
        int var6;
        int var7_int;
        Object var7;
        int var8_int;
        um[] var8;
        int var9;
        Object var10;
        um var11;
        gg var12;
        int var13;
        um var14;
        int var15_int;
        um var15;
        L0: {
          var3 = param1;
          if (!field_u) {
            break L0;
          } else {
            var3 = var3 << 1;
            break L0;
          }
        }
        L1: {
          ai.a(param0, 0, var3);
          this.field_c = this.field_c - param1;
          if (this.field_k == null) {
            break L1;
          } else {
            if (this.field_c > 0) {
              break L1;
            } else {
              this.field_c = this.field_c + (field_r >> 4);
              fj.a(this.field_k);
              this.a(this.field_k, this.field_k.a());
              var4 = 0;
              var5 = 255;
              var6 = 7;
              L2: while (true) {
                L3: {
                  if (var5 == 0) {
                    break L3;
                  } else {
                    L4: {
                      if (var6 >= 0) {
                        var7_int = var6;
                        var8_int = 0;
                        break L4;
                      } else {
                        var7_int = var6 & 3;
                        var8_int = -(var6 >> 2);
                        break L4;
                      }
                    }
                    var9 = var5 >>> var7_int & 286331153;
                    L5: while (true) {
                      if (var9 != 0) {
                        L6: {
                          if ((var9 & 1) != 0) {
                            var5 = var5 & (1 << var7_int ^ -1);
                            var10 = null;
                            var11 = this.field_f[var7_int];
                            var14 = var11;
                            var14 = var11;
                            L7: while (true) {
                              if (var11 == null) {
                                break L6;
                              } else {
                                L8: {
                                  var12 = var11.field_q;
                                  if (var12 == null) {
                                    break L8;
                                  } else {
                                    if (var12.field_n <= var8_int) {
                                      break L8;
                                    } else {
                                      var5 = var5 | 1 << var7_int;
                                      var10 = var11;
                                      var11 = var11.field_o;
                                      continue L7;
                                    }
                                  }
                                }
                                L9: {
                                  var11.field_n = true;
                                  var13 = var11.d();
                                  var4 = var4 + var13;
                                  if (var12 == null) {
                                    break L9;
                                  } else {
                                    var12.field_n = var12.field_n + var13;
                                    break L9;
                                  }
                                }
                                if (var4 < this.field_q) {
                                  L10: {
                                    var14 = var11.b();
                                    if (var14 == null) {
                                      break L10;
                                    } else {
                                      var15_int = var11.field_p;
                                      L11: while (true) {
                                        if (var14 == null) {
                                          break L10;
                                        } else {
                                          this.a(var14, var15_int * var14.a() >> 8);
                                          var14 = var11.c();
                                          continue L11;
                                        }
                                      }
                                    }
                                  }
                                  L12: {
                                    var15 = var11.field_o;
                                    var11.field_o = null;
                                    if (var10 != null) {
                                      ((um) (var10)).field_o = var15;
                                      break L12;
                                    } else {
                                      this.field_f[var7_int] = var15;
                                      break L12;
                                    }
                                  }
                                  L13: {
                                    if (var15 != null) {
                                      break L13;
                                    } else {
                                      this.field_j[var7_int] = (um) (var10);
                                      break L13;
                                    }
                                  }
                                  var11 = var15;
                                  continue L7;
                                } else {
                                  break L3;
                                }
                              }
                            }
                          } else {
                            break L6;
                          }
                        }
                        var7_int += 4;
                        var8_int++;
                        var9 = var9 >>> 4;
                        continue L5;
                      } else {
                        var6--;
                        continue L2;
                      }
                    }
                  }
                }
                var6 = 0;
                L14: while (true) {
                  if (var6 >= 8) {
                    break L1;
                  } else {
                    var7 = this.field_f[var6];
                    var8 = this.field_f;
                    var9 = var6;
                    this.field_j[var6] = null;
                    var8[var9] = null;
                    L15: while (true) {
                      if (var7 == null) {
                        var6++;
                        continue L14;
                      } else {
                        var10 = ((um) (var7)).field_o;
                        ((um) (var7)).field_o = null;
                        var7 = var10;
                        continue L15;
                      }
                    }
                  }
                }
              }
            }
          }
        }
        L16: {
          if (this.field_c >= 0) {
            break L16;
          } else {
            this.field_c = 0;
            break L16;
          }
        }
        L17: {
          if (this.field_k == null) {
            break L17;
          } else {
            this.field_k.a(param0, 0, param1);
            break L17;
          }
        }
        this.field_g = sf.a((byte) 107);
    }

    void b(int param0) throws Exception {
    }

    private final void a(int param0) {
        this.field_c = this.field_c - param0;
        if (this.field_c < 0) {
            this.field_c = 0;
        }
        if (this.field_k != null) {
            this.field_k.a(param0);
            return;
        }
    }

    final synchronized void b(um param0) {
        this.field_k = param0;
    }

    final static void a(int param0, boolean param1, int param2) {
        if (param0 < 8000 || param0 > 48000) {
            throw new IllegalArgumentException();
        }
        field_r = param0;
        field_u = param1 ? true : false;
        field_d = param2;
    }

    void a(java.awt.Component param0) throws Exception {
    }

    private final void a(um param0, int param1) {
        int var3 = param1 >> 5;
        um var4 = this.field_j[var3];
        if (var4 == null) {
            this.field_f[var3] = param0;
        } else {
            var4.field_o = param0;
        }
        this.field_j[var3] = param0;
        param0.field_p = param1;
    }

    void c() throws Exception {
    }

    fj() {
        this.field_p = false;
        this.field_q = 32;
        this.field_g = sf.a((byte) 114);
        this.field_j = new um[8];
        this.field_o = 0L;
        this.field_e = 0;
        this.field_t = true;
        this.field_h = 0;
        this.field_n = 0L;
        this.field_a = 0;
        this.field_c = 0;
        this.field_f = new um[8];
    }
}
