/*
 * Decompiled by CFR-JS 0.4.0.
 */
class la {
    static boolean field_h;
    private sh field_o;
    int[] field_i;
    private int field_c;
    private boolean field_u;
    private long field_t;
    private static ec field_n;
    private static int field_k;
    static int field_a;
    private boolean field_b;
    private int field_q;
    private int field_g;
    private int field_j;
    private int field_m;
    private long field_r;
    private int field_s;
    private int field_f;
    private sh[] field_d;
    private long field_p;
    private int field_l;
    private sh[] field_e;

    final synchronized void a() {
        try {
            int decompiledRegionSelector0 = 0;
            Throwable decompiledCaughtException = null;
            long var1 = 0L;
            Exception var3 = null;
            int var3_int = 0;
            int var4 = 0;
            if (!this.field_u) {
              var1 = lj.a((byte) -51);
              try {
                L0: {
                  L1: {
                    if (var1 <= this.field_t + 6000L) {
                      break L1;
                    } else {
                      this.field_t = var1 - 6000L;
                      break L1;
                    }
                  }
                  L2: while (true) {
                    if (var1 <= this.field_t + 5000L) {
                      break L0;
                    } else {
                      this.a(256);
                      this.field_t = this.field_t + (long)(256000 / field_a);
                      var1 = lj.a((byte) -98);
                      continue L2;
                    }
                  }
                }
              } catch (java.lang.Exception decompiledCaughtParameter0) {
                decompiledCaughtException = decompiledCaughtParameter0;
                L3: {
                  var3 = (Exception) (Object) decompiledCaughtException;
                  this.field_t = var1;
                  break L3;
                }
              }
              if (this.field_i != null) {
                try {
                  L4: {
                    L5: {
                      if (this.field_r == 0L) {
                        break L5;
                      } else {
                        if (var1 >= this.field_r) {
                          this.b(this.field_l);
                          this.field_r = 0L;
                          this.field_b = true;
                          break L5;
                        } else {
                          decompiledRegionSelector0 = 0;
                          break L4;
                        }
                      }
                    }
                    L6: {
                      var3_int = this.h();
                      if (this.field_g - var3_int <= this.field_s) {
                        break L6;
                      } else {
                        this.field_s = this.field_g - var3_int;
                        break L6;
                      }
                    }
                    L7: {
                      var4 = this.field_f + this.field_q;
                      if (var4 + 256 <= 16384) {
                        break L7;
                      } else {
                        var4 = 16128;
                        break L7;
                      }
                    }
                    L8: {
                      if (var4 + 256 <= this.field_l) {
                        break L8;
                      } else {
                        L9: {
                          this.field_l = this.field_l + 1024;
                          if (this.field_l <= 16384) {
                            break L9;
                          } else {
                            this.field_l = 16384;
                            break L9;
                          }
                        }
                        this.c();
                        this.b(this.field_l);
                        var3_int = 0;
                        this.field_b = true;
                        if (var4 + 256 <= this.field_l) {
                          break L8;
                        } else {
                          var4 = this.field_l - 256;
                          this.field_q = var4 - this.field_f;
                          break L8;
                        }
                      }
                    }
                    L10: while (true) {
                      if (var3_int >= var4) {
                        L11: {
                          if (var1 <= this.field_p) {
                            break L11;
                          } else {
                            L12: {
                              if (this.field_b) {
                                this.field_b = false;
                                break L12;
                              } else {
                                L13: {
                                  if (this.field_s != 0) {
                                    break L13;
                                  } else {
                                    if (this.field_j != 0) {
                                      break L13;
                                    } else {
                                      this.c();
                                      this.field_r = var1 + 2000L;
                                      decompiledRegionSelector0 = 1;
                                      break L4;
                                    }
                                  }
                                }
                                this.field_q = Math.min(this.field_j, this.field_s);
                                this.field_j = this.field_s;
                                break L12;
                              }
                            }
                            this.field_s = 0;
                            this.field_p = var1 + 2000L;
                            break L11;
                          }
                        }
                        this.field_g = var3_int;
                        decompiledRegionSelector0 = 2;
                        break L4;
                      } else {
                        this.a(this.field_i, 256);
                        this.f();
                        var3_int += 256;
                        continue L10;
                      }
                    }
                  }
                } catch (java.lang.Exception decompiledCaughtParameter1) {
                  decompiledCaughtException = decompiledCaughtParameter1;
                  L14: {
                    var3 = (Exception) (Object) decompiledCaughtException;
                    this.c();
                    this.field_r = var1 + 2000L;
                    decompiledRegionSelector0 = 2;
                    break L14;
                  }
                }
                if (decompiledRegionSelector0 == 0) {
                  return;
                } else {
                  if (decompiledRegionSelector0 == 1) {
                    return;
                  } else {
                    return;
                  }
                }
              } else {
                return;
              }
            } else {
              return;
            }
        } catch (RuntimeException | Error decompiledUncheckedException) {
            throw decompiledUncheckedException;
        } catch (Throwable decompiledCheckedException) {
            throw new RuntimeException(decompiledCheckedException);
        }
    }

    void b(int param0) throws Exception {
    }

    void c() {
    }

    final static la a(ce param0, java.awt.Component param1, int param2, int param3) {
        try {
            kg var4 = null;
            Throwable var4_ref = null;
            kg var5 = null;
            kg stackIn_10_0 = null;
            int stackIn_10_1 = 0;
            kg stackIn_11_0 = null;
            int stackIn_11_1 = 0;
            int stackIn_11_2 = 0;
            kg stackIn_21_0 = null;
            Throwable decompiledCaughtException = null;
            if (field_a != 0) {
              L0: {
                if (param2 < 0) {
                  break L0;
                } else {
                  if (param2 < 2) {
                    L1: {
                      if (param3 >= 256) {
                        break L1;
                      } else {
                        param3 = 256;
                        break L1;
                      }
                    }
                    try {
                      L2: {
                        L3: {
                          var5 = new kg();
                          var4 = var5;
                          stackIn_10_0 = (kg) (var4);

                          stackIn_10_1 = 256;

                          if (!field_h) {
                            stackIn_11_0 = (kg) ((Object) stackIn_10_0);
                            stackIn_11_1 = stackIn_10_1;
                            stackIn_11_2 = 1;
                            break L3;
                          } else {
                            stackIn_11_0 = (kg) ((Object) stackIn_10_0);
                            stackIn_11_1 = stackIn_10_1;
                            stackIn_11_2 = 2;
                            break L3;
                          }
                        }
                        L4: {
                          ((la) ((Object) stackIn_11_0)).field_i = new int[stackIn_11_1 * stackIn_11_2];
                          ((la) ((Object) var4)).field_f = param3;
                          ((la) ((Object) var4)).a(param1);
                          ((la) ((Object) var4)).field_l = (param3 & -1024) + 1024;
                          if (((la) ((Object) var4)).field_l <= 16384) {
                            break L4;
                          } else {
                            ((la) ((Object) var4)).field_l = 16384;
                            break L4;
                          }
                        }
                        L5: {
                          ((la) ((Object) var4)).b(((la) ((Object) var4)).field_l);
                          if (field_k <= 0) {
                            break L5;
                          } else {
                            if (field_n != null) {
                              break L5;
                            } else {
                              field_n = new ec();
                              field_n.field_f = param0;
                              param0.a(4532, (Runnable) ((Object) field_n), field_k);
                              break L5;
                            }
                          }
                        }
                        L6: {
                          if (field_n == null) {
                            break L6;
                          } else {
                            if (field_n.field_c[param2] == null) {
                              field_n.field_c[param2] = (la) ((Object) var5);
                              break L6;
                            } else {
                              throw new IllegalArgumentException();
                            }
                          }
                        }
                        stackIn_21_0 = (kg) (var4);
                        break L2;
                      }
                    } catch (java.lang.Throwable decompiledCaughtParameter0) {
                      decompiledCaughtException = decompiledCaughtParameter0;
                      var4_ref = decompiledCaughtException;
                      return new la();
                    }
                    return (la) ((Object) stackIn_21_0);
                  } else {
                    break L0;
                  }
                }
              }
              throw new IllegalArgumentException();
            } else {
              throw new IllegalStateException();
            }
        } catch (RuntimeException | Error decompiledUncheckedException) {
            throw decompiledUncheckedException;
        } catch (Throwable decompiledCheckedException) {
            throw new RuntimeException(decompiledCheckedException);
        }
    }

    private final void a(sh param0, int param1) {
        int var3 = param1 >> 5;
        sh var4 = this.field_e[var3];
        if (var4 == null) {
            this.field_d[var3] = param0;
        } else {
            var4.field_l = param0;
        }
        this.field_e[var3] = param0;
        param0.field_k = param1;
    }

    int h() throws Exception {
        return this.field_l;
    }

    public static void b() {
        field_n = null;
    }

    final static void a(int param0, boolean param1, int param2) {
        if (param0 < 8000 || param0 > 48000) {
            throw new IllegalArgumentException();
        }
        field_a = param0;
        field_h = param1 ? true : false;
        field_k = param2;
    }

    private final void a(int[] param0, int param1) {
        int var3;
        int var4;
        int var5;
        int var6;
        int var7_int;
        Object var7;
        int var8_int;
        sh[] var8;
        int var9;
        Object var10;
        sh var11;
        jf var12;
        int var13;
        sh var14;
        int var15_int;
        sh var15;
        L0: {
          var3 = param1;
          if (!field_h) {
            break L0;
          } else {
            var3 = var3 << 1;
            break L0;
          }
        }
        L1: {
          dm.a(param0, 0, var3);
          this.field_m = this.field_m - param1;
          if (this.field_o == null) {
            break L1;
          } else {
            if (this.field_m > 0) {
              break L1;
            } else {
              this.field_m = this.field_m + (field_a >> 4);
              la.a(this.field_o);
              this.a(this.field_o, this.field_o.b());
              var4 = 0;
              var5 = 255;
              var6 = 7;
              L2: while (true) {
                L3: {
                  if (var5 == 0) {
                    break L3;
                  } else {
                    L4: {
                      if (var6 >= 0) {
                        var7_int = var6;
                        var8_int = 0;
                        break L4;
                      } else {
                        var7_int = var6 & 3;
                        var8_int = -(var6 >> 2);
                        break L4;
                      }
                    }
                    var9 = var5 >>> var7_int & 286331153;
                    L5: while (true) {
                      if (var9 != 0) {
                        L6: {
                          if ((var9 & 1) != 0) {
                            var5 = var5 & (1 << var7_int ^ -1);
                            var10 = null;
                            var11 = this.field_d[var7_int];
                            var14 = var11;
                            var14 = var11;
                            L7: while (true) {
                              if (var11 == null) {
                                break L6;
                              } else {
                                L8: {
                                  var12 = var11.field_i;
                                  if (var12 == null) {
                                    break L8;
                                  } else {
                                    if (var12.field_i <= var8_int) {
                                      break L8;
                                    } else {
                                      var5 = var5 | 1 << var7_int;
                                      var10 = var11;
                                      var11 = var11.field_l;
                                      continue L7;
                                    }
                                  }
                                }
                                L9: {
                                  var11.field_j = true;
                                  var13 = var11.a();
                                  var4 = var4 + var13;
                                  if (var12 == null) {
                                    break L9;
                                  } else {
                                    var12.field_i = var12.field_i + var13;
                                    break L9;
                                  }
                                }
                                if (var4 < this.field_c) {
                                  L10: {
                                    var14 = var11.d();
                                    if (var14 == null) {
                                      break L10;
                                    } else {
                                      var15_int = var11.field_k;
                                      L11: while (true) {
                                        if (var14 == null) {
                                          break L10;
                                        } else {
                                          this.a(var14, var15_int * var14.b() >> 8);
                                          var14 = var11.c();
                                          continue L11;
                                        }
                                      }
                                    }
                                  }
                                  L12: {
                                    var15 = var11.field_l;
                                    var11.field_l = null;
                                    if (var10 != null) {
                                      ((sh) (var10)).field_l = var15;
                                      break L12;
                                    } else {
                                      this.field_d[var7_int] = var15;
                                      break L12;
                                    }
                                  }
                                  L13: {
                                    if (var15 != null) {
                                      break L13;
                                    } else {
                                      this.field_e[var7_int] = (sh) (var10);
                                      break L13;
                                    }
                                  }
                                  var11 = var15;
                                  continue L7;
                                } else {
                                  break L3;
                                }
                              }
                            }
                          } else {
                            break L6;
                          }
                        }
                        var7_int += 4;
                        var8_int++;
                        var9 = var9 >>> 4;
                        continue L5;
                      } else {
                        var6--;
                        continue L2;
                      }
                    }
                  }
                }
                var6 = 0;
                L14: while (true) {
                  if (var6 >= 8) {
                    break L1;
                  } else {
                    var7 = this.field_d[var6];
                    var8 = this.field_d;
                    var9 = var6;
                    this.field_e[var6] = null;
                    var8[var9] = null;
                    L15: while (true) {
                      if (var7 == null) {
                        var6++;
                        continue L14;
                      } else {
                        var10 = ((sh) (var7)).field_l;
                        ((sh) (var7)).field_l = null;
                        var7 = var10;
                        continue L15;
                      }
                    }
                  }
                }
              }
            }
          }
        }
        L16: {
          if (this.field_m >= 0) {
            break L16;
          } else {
            this.field_m = 0;
            break L16;
          }
        }
        L17: {
          if (this.field_o == null) {
            break L17;
          } else {
            this.field_o.b(param0, 0, param1);
            break L17;
          }
        }
        this.field_t = lj.a((byte) -103);
    }

    final synchronized void d() {
        int var1;
        int var2;
        L0: {
          if (field_n == null) {
            break L0;
          } else {
            var1 = 1;
            var2 = 0;
            L1: while (true) {
              if (var2 >= 2) {
                if (var1 == 0) {
                  break L0;
                } else {
                  field_n.field_g = true;
                  L2: while (true) {
                    if (!field_n.field_h) {
                      field_n = null;
                      break L0;
                    } else {
                      vh.a(50L, 73);
                      continue L2;
                    }
                  }
                }
              } else {
                L3: {
                  if (field_n.field_c[var2] != this) {
                    break L3;
                  } else {
                    field_n.field_c[var2] = null;
                    break L3;
                  }
                }
                if (field_n.field_c[var2] != null) {
                  var1 = 0;
                  var2++;
                  continue L1;
                } else {
                  var2++;
                  continue L1;
                }
              }
            }
          }
        }
        this.c();
        this.field_i = null;
        this.field_u = true;
    }

    void a(java.awt.Component param0) throws Exception {
    }

    private final static void a(sh param0) {
        param0.field_j = false;
        if (param0.field_i != null) {
            param0.field_i.field_i = 0;
        }
        sh var1 = param0.d();
        while (var1 != null) {
            la.a(var1);
            var1 = param0.c();
        }
    }

    final synchronized void b(sh param0) {
        this.field_o = param0;
    }

    void e() throws Exception {
    }

    void f() throws Exception {
    }

    private final void a(int param0) {
        this.field_m = this.field_m - param0;
        if (this.field_m < 0) {
            this.field_m = 0;
        }
        if (this.field_o != null) {
            this.field_o.a(param0);
            return;
        }
    }

    final synchronized void g() {
        try {
            this.field_b = true;
            try {
                this.e();
            } catch (Exception exception) {
                this.c();
                this.field_r = lj.a((byte) -23) + 2000L;
            }
        } catch (RuntimeException | Error decompiledUncheckedException) {
            throw decompiledUncheckedException;
        } catch (Throwable decompiledCheckedException) {
            throw new RuntimeException(decompiledCheckedException);
        }
    }

    la() {
        this.field_c = 32;
        this.field_u = false;
        this.field_t = lj.a((byte) -104);
        this.field_j = 0;
        this.field_m = 0;
        this.field_g = 0;
        this.field_r = 0L;
        this.field_d = new sh[8];
        this.field_b = true;
        this.field_s = 0;
        this.field_e = new sh[8];
        this.field_p = 0L;
    }
}
