/*
 * Decompiled by CFR-JS 0.4.0.
 */
import java.io.*;

final class bf {
    private byte[] field_n;
    private long[] field_h;
    static qj field_o;
    private byte[] field_g;
    static String field_c;
    static String field_d;
    private long[] field_m;
    static String field_f;
    static int[] field_a;
    private int field_l;
    private long[] field_b;
    private int field_i;
    static ng[] field_e;
    private long[] field_k;
    private long[] field_j;

    final void a(byte[] param0, long param1, int param2) {
        RuntimeException stackIn_25_0 = null;
        StringBuilder stackIn_25_1 = null;
        RuntimeException stackIn_26_0 = null;
        StringBuilder stackIn_26_1 = null;
        String stackIn_26_2 = null;
        RuntimeException decompiledCaughtException = null;
        int var5_int = 0;
        RuntimeException var5 = null;
        int var6 = 0;
        int var7 = 0;
        int var8 = 0;
        long var9 = 0L;
        int var11 = 0;
        int var12 = 0;
        int var13 = 0;
        var13 = TrackController.field_F ? 1 : 0;
        try {
          L0: {
            L1: {
              var5_int = 0;
              var6 = -((int)param1 & 7) + 8 & 7;
              var7 = 7 & this.field_l;
              var9 = param1;
              if (param2 == 30654) {
                break L1;
              } else {
                this.field_g = (byte[]) null;
                break L1;
              }
            }
            var11 = 31;
            var12 = 0;
            L2: while (true) {
              if ((var11 ^ -1) > -1) {
                L3: while (true) {
                  if (param1 <= 8L) {
                    L4: {
                      if (-1L <= (param1 ^ -1L)) {
                        var8 = 0;
                        break L4;
                      } else {
                        var8 = 255 & param0[var5_int] << var6;
                        this.field_g[this.field_i] = (byte)ok.a((int) this.field_g[this.field_i], var8 >>> var7);
                        break L4;
                      }
                    }
                    L5: {
                      if (-9L >= ((long)var7 - -param1 ^ -1L)) {
                        L6: {
                          this.field_i = this.field_i + 1;
                          param1 = param1 - (long)(8 - var7);
                          this.field_l = this.field_l + (-var7 + 8);
                          if (-513 != (this.field_l ^ -1)) {
                            break L6;
                          } else {
                            this.b(-2);
                            this.field_l = 0;
                            this.field_i = 0;
                            break L6;
                          }
                        }
                        this.field_g[this.field_i] = (byte)tc.a(255, var8 << -var7 + 8);
                        this.field_l = this.field_l + (int)param1;
                        break L5;
                      } else {
                        this.field_l = (int)((long)this.field_l + param1);
                        break L5;
                      }
                    }
                    break L0;
                  } else {
                    L7: {
                      var8 = 255 & param0[var5_int] << var6 | (param0[var5_int + 1] & 255) >>> -var6 + 8;
                      if (var8 < 0) {
                        break L7;
                      } else {
                        if (256 <= var8) {
                          break L7;
                        } else {
                          L8: {
                            this.field_g[this.field_i] = (byte)ok.a((int) this.field_g[this.field_i], var8 >>> var7);
                            this.field_i = this.field_i + 1;
                            this.field_l = this.field_l + (8 - var7);
                            if (-513 != (this.field_l ^ -1)) {
                              break L8;
                            } else {
                              this.b(-2);
                              this.field_l = 0;
                              this.field_i = 0;
                              break L8;
                            }
                          }
                          this.field_g[this.field_i] = (byte)tc.a(255, var8 << 8 + -var7);
                          param1 = param1 - 8L;
                          this.field_l = this.field_l + var7;
                          var5_int++;
                          continue L3;
                        }
                      }
                    }
                    throw new RuntimeException("LOGIC ERROR");
                  }
                }
              } else {
                var12 = var12 + ((this.field_n[var11] & 255) - -(255 & (int)var9));
                this.field_n[var11] = (byte)var12;
                var12 = var12 >>> 8;
                var9 = var9 >>> 8;
                var11--;
                continue L2;
              }
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L9: {
            var5 = decompiledCaughtException;
            stackIn_25_0 = (RuntimeException) (var5);

            stackIn_25_1 = new StringBuilder().append("bf.F(");

            if (param0 == null) {
              stackIn_26_0 = (RuntimeException) ((Object) stackIn_25_0);
              stackIn_26_1 = (StringBuilder) ((Object) stackIn_25_1);
              stackIn_26_2 = "null";
              break L9;
            } else {
              stackIn_26_0 = (RuntimeException) ((Object) stackIn_25_0);
              stackIn_26_1 = (StringBuilder) ((Object) stackIn_25_1);
              stackIn_26_2 = "{...}";
              break L9;
            }
          }
          throw sl.a((Throwable) ((Object) stackIn_26_0), stackIn_26_2 + ',' + param1 + ',' + param2 + ')');
        }
    }

    public static void a(byte param0) {
        field_a = null;
        field_f = null;
        field_d = null;
        field_o = null;
        if (param0 >= -50) {
            field_d = (String) null;
        }
        field_e = null;
        field_c = null;
    }

    final static boolean a(boolean param0, byte param1) {
        long dupTemp$2 = 0L;
        L0: {
          if (null != ga.field_d) {
            break L0;
          } else {
            ga.field_d = gf.field_o.a(79, ke.field_c, dc.field_o);
            break L0;
          }
        }
        if (0 == ga.field_d.field_f) {
          return false;
        } else {
          L1: {
            dupTemp$2 = qg.a(false);
            qh.field_f = dupTemp$2;
            he.field_a = dupTemp$2;
            if ((ga.field_d.field_f ^ -1) != -2) {
              break L1;
            } else {
              break L1;
            }
          }
          jc.field_a = oc.field_m;
          L2: {
            if (param1 >= 31) {
              break L2;
            } else {
              field_a = (int[]) null;
              break L2;
            }
          }
          ga.field_d = null;
          return true;
        }
    }

    final void a(byte[] param0, int param1, int param2) {
        int fieldTemp$0 = 0;
        int fieldTemp$1 = 0;
        RuntimeException stackIn_16_0 = null;
        StringBuilder stackIn_16_1 = null;
        RuntimeException stackIn_17_0 = null;
        StringBuilder stackIn_17_1 = null;
        String stackIn_17_2 = null;
        int decompiledRegionSelector0 = 0;
        RuntimeException decompiledCaughtException = null;
        int var4_int = 0;
        RuntimeException var4 = null;
        int var5 = 0;
        long var6 = 0L;
        int var8 = 0;
        var8 = TrackController.field_F ? 1 : 0;
        try {
          L0: {
            this.field_g[this.field_i] = (byte)ok.a((int) this.field_g[this.field_i], 128 >>> tc.a(this.field_l, 7));
            this.field_i = this.field_i + 1;
            if (param2 == -513) {
              L1: {
                if (this.field_i > 32) {
                  L2: while (true) {
                    if ((this.field_i ^ -1) <= -65) {
                      this.b(-2);
                      this.field_i = 0;
                      break L1;
                    } else {
                      fieldTemp$0 = this.field_i;
                      this.field_i = this.field_i + 1;
                      this.field_g[fieldTemp$0] = (byte) 0;
                      continue L2;
                    }
                  }
                } else {
                  break L1;
                }
              }
              L3: while (true) {
                if ((this.field_i ^ -1) <= -33) {
                  pd.a(this.field_n, 0, this.field_g, 32, 32);
                  this.b(-2);
                  var4_int = 0;
                  var5 = param1;
                  L4: while (true) {
                    if (8 <= var4_int) {
                      decompiledRegionSelector0 = 1;
                      break L0;
                    } else {
                      var6 = this.field_k[var4_int];
                      param0[var5] = (byte)(int)(var6 >>> -636635080);
                      param0[1 + var5] = (byte)(int)(var6 >>> 1881326704);
                      param0[var5 + 2] = (byte)(int)(var6 >>> -1657883672);
                      param0[var5 - -3] = (byte)(int)(var6 >>> 281218208);
                      param0[var5 - -4] = (byte)(int)(var6 >>> -1031222888);
                      param0[var5 + 5] = (byte)(int)(var6 >>> -202686960);
                      param0[var5 + 6] = (byte)(int)(var6 >>> 2142168776);
                      param0[var5 - -7] = (byte)(int)var6;
                      var4_int++;
                      var5 += 8;
                      continue L4;
                    }
                  }
                } else {
                  fieldTemp$1 = this.field_i;
                  this.field_i = this.field_i + 1;
                  this.field_g[fieldTemp$1] = (byte) 0;
                  continue L3;
                }
              }
            } else {
              decompiledRegionSelector0 = 0;
              break L0;
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L5: {
            var4 = decompiledCaughtException;
            stackIn_16_0 = (RuntimeException) (var4);

            stackIn_16_1 = new StringBuilder().append("bf.D(");

            if (param0 == null) {
              stackIn_17_0 = (RuntimeException) ((Object) stackIn_16_0);
              stackIn_17_1 = (StringBuilder) ((Object) stackIn_16_1);
              stackIn_17_2 = "null";
              break L5;
            } else {
              stackIn_17_0 = (RuntimeException) ((Object) stackIn_16_0);
              stackIn_17_1 = (StringBuilder) ((Object) stackIn_16_1);
              stackIn_17_2 = "{...}";
              break L5;
            }
          }
          throw sl.a((Throwable) ((Object) stackIn_17_0), stackIn_17_2 + ',' + param1 + ',' + param2 + ')');
        }
        if (decompiledRegionSelector0 == 0) {
          return;
        } else {
          return;
        }
    }

    final void a(int param0) {
        int var2 = 0;
        int var3 = TrackController.field_F ? 1 : 0;
        for (var2 = 0; (var2 ^ -1) > -33; var2++) {
            this.field_n[var2] = (byte) 0;
        }
        this.field_g[0] = (byte) 0;
        this.field_i = 0;
        this.field_l = 0;
        if (param0 != -11) {
            return;
        }
        for (var2 = 0; (var2 ^ -1) > -9; var2++) {
            this.field_k[var2] = 0L;
        }
    }

    final static void a(byte param0, String param1) {
        try {
            kg.field_a = param1;
            if (param0 >= -23) {
                bf.a((byte) 1);
            }
        } catch (RuntimeException runtimeException) {
            throw sl.a((Throwable) ((Object) runtimeException), "bf.B(" + param0 + ',' + (param1 != null ? "{...}" : "null") + ')');
        }
    }

    private final void b(int param0) {
        long dupTemp$0 = 0L;
        long arrayValue$1 = 0L;
        int var2;
        int var3;
        int var4;
        int var5;
        int var6;
        int var7;
        var6 = TrackController.field_F ? 1 : 0;
        var2 = 0;
        var3 = 0;
        L0: while (true) {
          if (8 <= var2) {
            var2 = 0;
            L1: while (true) {
              if (8 <= var2) {
                var2 = 1;
                L2: while (true) {
                  if ((var2 ^ -1) < -11) {
                    var2 = 0;
                    L3: while (true) {
                      if (-9 >= (var2 ^ -1)) {
                        L4: {
                          if (param0 == -2) {
                            break L4;
                          } else {
                            this.field_i = 66;
                            break L4;
                          }
                        }
                        return;
                      } else {
                        this.field_k[var2] = ib.a(this.field_k[var2], ib.a(this.field_h[var2], this.field_m[var2]));
                        var2++;
                        continue L3;
                      }
                    }
                  } else {
                    var3 = 0;
                    L5: while (true) {
                      if ((var3 ^ -1) <= -9) {
                        var3 = 0;
                        L6: while (true) {
                          if (8 <= var3) {
                            this.field_j[0] = ib.a(this.field_j[0], lh.field_D[var2]);
                            var3 = 0;
                            L7: while (true) {
                              if (-9 >= (var3 ^ -1)) {
                                var3 = 0;
                                L8: while (true) {
                                  if (-9 >= (var3 ^ -1)) {
                                    var2++;
                                    continue L2;
                                  } else {
                                    this.field_m[var3] = this.field_b[var3];
                                    var3++;
                                    continue L8;
                                  }
                                }
                              } else {
                                this.field_b[var3] = this.field_j[var3];
                                var7 = 0;
                                var4 = var7;
                                var5 = 56;
                                L9: while (true) {
                                  if (8 <= var7) {
                                    var3++;
                                    continue L7;
                                  } else {
                                    this.field_b[var3] = ib.a(this.field_b[var3], lh.field_B[var7][tc.a((int)(this.field_m[tc.a(var3 - var7, 7)] >>> var5), 255)]);
                                    var5 -= 8;
                                    var7++;
                                    continue L9;
                                  }
                                }
                              }
                            }
                          } else {
                            this.field_j[var3] = this.field_b[var3];
                            var3++;
                            continue L6;
                          }
                        }
                      } else {
                        this.field_b[var3] = 0L;
                        var4 = 0;
                        var5 = 56;
                        L10: while (true) {
                          if (var4 >= 8) {
                            var3++;
                            continue L5;
                          } else {
                            this.field_b[var3] = ib.a(this.field_b[var3], lh.field_B[var4][tc.a(255, (int)(this.field_j[tc.a(7, var3 - var4)] >>> var5))]);
                            var4++;
                            var5 -= 8;
                            continue L10;
                          }
                        }
                      }
                    }
                  }
                }
              } else {
                dupTemp$0 = this.field_k[var2];
                arrayValue$1 = this.field_h[var2];
                this.field_j[var2] = dupTemp$0;
                this.field_m[var2] = ib.a(arrayValue$1, dupTemp$0);
                var2++;
                continue L1;
              }
            }
          } else {
            this.field_h[var2] = ib.a(ib.a(ib.a(ib.a(al.a(255L, (long)this.field_g[var3 - -4]) << -1035872296, ib.a(al.a((long)this.field_g[var3 + 3], 255L) << -1835657888, ib.a(al.a(280375465082880L, (long)this.field_g[var3 - -2] << 1101055080), ib.a(al.a((long)this.field_g[var3 - -1], 255L) << -2121321744, (long)this.field_g[var3] << 1548063032)))), al.a((long)this.field_g[var3 - -5] << 1132118544, 16711680L)), al.a((long)this.field_g[var3 + 6], 255L) << -1057440056), al.a((long)this.field_g[7 + var3], 255L));
            var3 += 8;
            var2++;
            continue L0;
          }
        }
    }

    bf() {
        this.field_n = new byte[32];
        this.field_m = new long[8];
        this.field_h = new long[8];
        this.field_g = new byte[64];
        this.field_l = 0;
        this.field_b = new long[8];
        this.field_i = 0;
        this.field_k = new long[8];
        this.field_j = new long[8];
    }

    static {
        field_d = "Checking";
        field_c = "Unfortunately your configuration doesn't support fullscreen mode. You could try restarting your browser and using the signed applet.";
        field_a = new int[4];
    }
}
