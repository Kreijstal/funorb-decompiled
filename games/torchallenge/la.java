/*
 * Decompiled by CFR-JS 0.4.0.
 */
import java.io.*;

final class la {
    private rk field_c;
    private int field_b;
    static oe field_f;
    private rk field_e;
    static kb field_a;
    static fj field_d;
    private int field_g;

    private final boolean a(int param0, int param1, int param2, byte[] param3, boolean param4) {
        try {
            int stackIn_8_0 = 0;
            int stackIn_14_0 = 0;
            int stackIn_18_0 = 0;
            int stackIn_38_0 = 0;
            int stackIn_44_0 = 0;
            int stackIn_66_0 = 0;
            int stackIn_69_0 = 0;
            RuntimeException stackIn_74_0 = null;
            StringBuilder stackIn_74_1 = null;
            RuntimeException stackIn_75_0 = null;
            StringBuilder stackIn_75_1 = null;
            String stackIn_75_2 = null;
            int decompiledRegionSelector0 = 0;
            int decompiledRegionSelector1 = 0;
            int decompiledRegionSelector2 = 0;
            Throwable decompiledCaughtException = null;
            Object var6 = null;
            RuntimeException var6_ref = null;
            int var7_int = 0;
            IOException var7 = null;
            int var8 = 0;
            int var9 = 0;
            int var10 = 0;
            int var11 = 0;
            int var12 = 0;
            int var13 = 0;
            EOFException var14 = null;
            int var16 = 0;
            var16 = TorChallenge.field_F ? 1 : 0;
            try {
              var6 = this.field_c;
              synchronized (var6) {
                try {
                  L0: {
                    L1: {
                      if (param4) {
                        if ((this.field_e.a(3) ^ -1L) > ((long)(6 * param2 - -6) ^ -1L)) {
                          stackIn_8_0 = 0;

                          decompiledRegionSelector2 = 0;
                          break L0;
                        } else {
                          L2: {
                            this.field_e.a((long)(6 * param2), 0);
                            this.field_e.a(0, 6, (byte) 72, wi.field_c);
                            var7_int = (65280 & wi.field_c[4] << 1445846824) + (((wi.field_c[3] & 255) << 824425008) + (255 & wi.field_c[5]));
                            if ((var7_int ^ -1) >= -1) {
                              break L2;
                            } else {
                              if (((long)var7_int ^ -1L) < (this.field_c.a(param1 ^ -20) / 520L ^ -1L)) {
                                break L2;
                              } else {
                                break L1;
                              }
                            }
                          }
                          stackIn_14_0 = 0;

                          decompiledRegionSelector2 = 1;
                          break L0;
                        }
                      } else {
                        var7_int = (int)((this.field_c.a(-124) - -519L) / 520L);
                        if (var7_int != 0) {
                          break L1;
                        } else {
                          var7_int = 1;
                          break L1;
                        }
                      }
                    }
                    wi.field_c[2] = (byte)param0;
                    wi.field_c[4] = (byte)(var7_int >> 1520647816);
                    wi.field_c[1] = (byte)(param0 >> -377548344);
                    wi.field_c[0] = (byte)(param0 >> 677735536);
                    wi.field_c[3] = (byte)(var7_int >> -1586241712);
                    wi.field_c[5] = (byte)var7_int;
                    this.field_e.a((long)(param2 * 6), 0);
                    this.field_e.a(-4573, 0, wi.field_c, 6);
                    var8 = 0;
                    if (param1 == -6) {
                      var9 = 0;
                      L3: while (true) {
                        L4: {
                          if (var8 >= param0) {
                            break L4;
                          } else {
                            L5: {
                              var10 = 0;
                              if (!param4) {
                                break L5;
                              } else {
                                L6: {
                                  this.field_c.a((long)(520 * var7_int), 0);
                                  if (param2 <= 65535) {
                                    try {
                                      L7: {
                                        this.field_c.a(0, 8, (byte) 29, wi.field_c);
                                        decompiledRegionSelector0 = 0;
                                        break L7;
                                      }
                                    } catch (java.io.EOFException decompiledCaughtParameter0) {
                                      decompiledCaughtException = decompiledCaughtParameter0;
                                      L8: {
                                        var14 = (EOFException) (Object) decompiledCaughtException;
                                        decompiledRegionSelector0 = 1;
                                        break L8;
                                      }
                                    }
                                    if (decompiledRegionSelector0 == 0) {
                                      var12 = (wi.field_c[2] << -1561527992 & 65280) + (255 & wi.field_c[3]);
                                      var13 = 255 & wi.field_c[7];
                                      var11 = (wi.field_c[1] & 255) + ((255 & wi.field_c[0]) << 1329152776);
                                      var10 = (16711680 & wi.field_c[4] << 334558416) - -(wi.field_c[5] << 1894404264 & 65280) + (255 & wi.field_c[6]);
                                      break L6;
                                    } else {
                                      break L4;
                                    }
                                  } else {
                                    try {
                                      L9: {
                                        this.field_c.a(0, 10, (byte) 68, wi.field_c);
                                        decompiledRegionSelector1 = 0;
                                        break L9;
                                      }
                                    } catch (java.io.EOFException decompiledCaughtParameter1) {
                                      decompiledCaughtException = decompiledCaughtParameter1;
                                      L10: {
                                        var14 = (EOFException) (Object) decompiledCaughtException;
                                        decompiledRegionSelector1 = 1;
                                        break L10;
                                      }
                                    }
                                    if (decompiledRegionSelector1 == 0) {
                                      var10 = (65280 & wi.field_c[7] << 1768307688) + ((255 & wi.field_c[6]) << -1821448464) - -(255 & wi.field_c[8]);
                                      var11 = (255 & wi.field_c[3]) + ((wi.field_c[2] & 255) << 348533928) + (wi.field_c[0] << -633665704 & -16777216) - -(16711680 & wi.field_c[1] << 861319536);
                                      var12 = (wi.field_c[5] & 255) + (65280 & wi.field_c[4] << -355662168);
                                      var13 = wi.field_c[9] & 255;
                                      break L6;
                                    } else {
                                      break L4;
                                    }
                                  }
                                }
                                L11: {
                                  if (param2 != var11) {
                                    break L11;
                                  } else {
                                    if (var9 != var12) {
                                      break L11;
                                    } else {
                                      if (this.field_b == var13) {
                                        L12: {
                                          if (0 > var10) {
                                            break L12;
                                          } else {
                                            if ((long)var10 <= this.field_c.a(-112) / 520L) {
                                              break L5;
                                            } else {
                                              break L12;
                                            }
                                          }
                                        }
                                        stackIn_44_0 = 0;

                                        decompiledRegionSelector2 = 4;
                                        break L0;
                                      } else {
                                        break L11;
                                      }
                                    }
                                  }
                                }
                                stackIn_38_0 = 0;

                                decompiledRegionSelector2 = 3;
                                break L0;
                              }
                            }
                            L13: {
                              if (0 == var10) {
                                L14: {
                                  param4 = false;
                                  var10 = (int)((this.field_c.a(-14) - -519L) / 520L);
                                  if (0 == var10) {
                                    var10++;
                                    break L14;
                                  } else {
                                    break L14;
                                  }
                                }
                                if (var10 != var7_int) {
                                  break L13;
                                } else {
                                  var10++;
                                  break L13;
                                }
                              } else {
                                break L13;
                              }
                            }
                            L15: {
                              if (-513 <= (param0 + -var8 ^ -1)) {
                                var10 = 0;
                                break L15;
                              } else {
                                break L15;
                              }
                            }
                            L16: {
                              if (-65536 > (param2 ^ -1)) {
                                L17: {
                                  wi.field_c[5] = (byte)var9;
                                  wi.field_c[6] = (byte)(var10 >> 1607141904);
                                  wi.field_c[7] = (byte)(var10 >> -729187832);
                                  wi.field_c[4] = (byte)(var9 >> 1266526952);
                                  wi.field_c[8] = (byte)var10;
                                  wi.field_c[9] = (byte)this.field_b;
                                  wi.field_c[2] = (byte)(param2 >> -1407949560);
                                  wi.field_c[3] = (byte)param2;
                                  wi.field_c[0] = (byte)(param2 >> 784929080);
                                  wi.field_c[1] = (byte)(param2 >> -935111568);
                                  this.field_c.a((long)(var7_int * 520), 0);
                                  this.field_c.a(-4573, 0, wi.field_c, 10);
                                  var11 = -var8 + param0;
                                  if (var11 <= 510) {
                                    break L17;
                                  } else {
                                    var11 = 510;
                                    break L17;
                                  }
                                }
                                this.field_c.a(-4573, var8, param3, var11);
                                var8 = var8 + var11;
                                break L16;
                              } else {
                                L18: {
                                  wi.field_c[3] = (byte)var9;
                                  wi.field_c[1] = (byte)param2;
                                  wi.field_c[5] = (byte)(var10 >> 522511400);
                                  wi.field_c[2] = (byte)(var9 >> -1687770648);
                                  wi.field_c[7] = (byte)this.field_b;
                                  wi.field_c[6] = (byte)var10;
                                  wi.field_c[4] = (byte)(var10 >> -1219796240);
                                  wi.field_c[0] = (byte)(param2 >> 636369480);
                                  this.field_c.a((long)(var7_int * 520), 0);
                                  this.field_c.a(param1 ^ 4569, 0, wi.field_c, 8);
                                  var11 = -var8 + param0;
                                  if ((var11 ^ -1) < -513) {
                                    var11 = 512;
                                    break L18;
                                  } else {
                                    break L18;
                                  }
                                }
                                this.field_c.a(-4573, var8, param3, var11);
                                var8 = var8 + var11;
                                break L16;
                              }
                            }
                            var7_int = var10;
                            var9++;
                            continue L3;
                          }
                        }
                        stackIn_66_0 = 1;

                        decompiledRegionSelector2 = 5;
                        break L0;
                      }
                    } else {
                      stackIn_18_0 = 0;

                      decompiledRegionSelector2 = 2;
                      break L0;
                    }
                  }
                } catch (java.io.IOException decompiledCaughtParameter2) {
                  decompiledCaughtException = decompiledCaughtParameter2;
                  var7 = (IOException) (Object) decompiledCaughtException;
                  stackIn_69_0 = 0;
                  return stackIn_69_0 != 0;
                }
                if (decompiledRegionSelector2 == 0) {
                  return stackIn_8_0 != 0;
                } else {
                  if (decompiledRegionSelector2 == 1) {
                    return stackIn_14_0 != 0;
                  } else {
                    if (decompiledRegionSelector2 == 2) {
                      return stackIn_18_0 != 0;
                    } else {
                      if (decompiledRegionSelector2 == 3) {
                        return stackIn_38_0 != 0;
                      } else {
                        if (decompiledRegionSelector2 == 4) {
                          return stackIn_44_0 != 0;
                        } else {
                          return stackIn_66_0 != 0;
                        }
                      }
                    }
                  }
                }
              }
            } catch (java.lang.RuntimeException decompiledCaughtParameter3) {
              decompiledCaughtException = decompiledCaughtParameter3;
              L19: {
                var6_ref = (RuntimeException) (Object) decompiledCaughtException;
                stackIn_74_0 = (RuntimeException) (var6_ref);

                stackIn_74_1 = new StringBuilder().append("la.D(").append(param0).append(',').append(param1).append(',').append(param2).append(',');

                if (param3 == null) {
                  stackIn_75_0 = (RuntimeException) ((Object) stackIn_74_0);
                  stackIn_75_1 = (StringBuilder) ((Object) stackIn_74_1);
                  stackIn_75_2 = "null";
                  break L19;
                } else {
                  stackIn_75_0 = (RuntimeException) ((Object) stackIn_74_0);
                  stackIn_75_1 = (StringBuilder) ((Object) stackIn_74_1);
                  stackIn_75_2 = "{...}";
                  break L19;
                }
              }
              throw oj.a((Throwable) ((Object) stackIn_75_0), stackIn_75_2 + ',' + param4 + ')');
            }
        } catch (RuntimeException | Error decompiledUncheckedException) {
            throw decompiledUncheckedException;
        } catch (Throwable decompiledCheckedException) {
            throw new RuntimeException(decompiledCheckedException);
        }
    }

    final byte[] a(int param0, int param1) {
        try {
            int incrementValue$2 = 0;
            Object var3 = null;
            Object var3_ref = null;
            int var4_int = 0;
            int var5 = 0;
            int var6 = 0;
            byte[] var7 = null;
            int var8 = 0;
            int var9 = 0;
            int var10 = 0;
            int var11 = 0;
            int var12 = 0;
            int var13 = 0;
            int var14 = 0;
            int var15 = 0;
            int var16 = 0;
            int var17 = 0;
            int var19 = 0;
            Object stackIn_9_0 = null;
            Object stackIn_37_0 = null;
            Object stackIn_43_0 = null;
            byte[] stackIn_49_0 = null;
            Object stackIn_52_0 = null;
            int decompiledRegionSelector0 = 0;
            Throwable decompiledCaughtException = null;
            IOException var4 = null;
            var19 = TorChallenge.field_F ? 1 : 0;
            var3_ref = this.field_c;
            synchronized (var3_ref) {
              try {
                L0: {
                  if (((long)(6 + 6 * param0) ^ -1L) >= (this.field_e.a(-114) ^ -1L)) {
                    this.field_e.a((long)(6 * param0), 0);
                    this.field_e.a(0, 6, (byte) 46, wi.field_c);
                    var4_int = (wi.field_c[2] & 255) + ((wi.field_c[0] & 255) << 1513119312) - -((wi.field_c[1] & 255) << -1861070616);
                    var5 = (wi.field_c[5] & 255) + ((255 & wi.field_c[4]) << 1612039272) + ((255 & wi.field_c[3]) << -1655868944);
                    var6 = -54 % ((-63 - param1) / 40);
                    if ((var4_int ^ -1) <= -1) {
                      if (var4_int <= this.field_g) {
                        if (-1 > (var5 ^ -1)) {
                          if ((this.field_c.a(-117) / 520L ^ -1L) <= ((long)var5 ^ -1L)) {
                            var7 = new byte[var4_int];
                            var8 = 0;
                            var9 = 0;
                            L1: while (true) {
                              if (var4_int <= var8) {
                                stackIn_49_0 = (byte[]) (var7);

                                decompiledRegionSelector0 = 3;
                                break L0;
                              } else {
                                if (0 != var5) {
                                  L2: {
                                    this.field_c.a((long)(var5 * 520), 0);
                                    var10 = var4_int + -var8;
                                    if ((param0 ^ -1) < -65536) {
                                      L3: {
                                        var15 = 10;
                                        if (var10 > 510) {
                                          var10 = 510;
                                          break L3;
                                        } else {
                                          break L3;
                                        }
                                      }
                                      this.field_c.a(0, var10 + var15, (byte) 69, wi.field_c);
                                      var12 = ((255 & wi.field_c[4]) << -532938840) - -(255 & wi.field_c[5]);
                                      var14 = 255 & wi.field_c[9];
                                      var11 = ((wi.field_c[2] & 255) << -973944504) + (((255 & wi.field_c[1]) << 1878434224) + ((wi.field_c[0] & 255) << 530339384)) + (255 & wi.field_c[3]);
                                      var13 = (wi.field_c[8] & 255) + (65280 & wi.field_c[7] << 2282376) + (16711680 & wi.field_c[6] << 1716154800);
                                      break L2;
                                    } else {
                                      L4: {
                                        if (512 >= var10) {
                                          break L4;
                                        } else {
                                          var10 = 512;
                                          break L4;
                                        }
                                      }
                                      var15 = 8;
                                      this.field_c.a(0, var15 + var10, (byte) 98, wi.field_c);
                                      var12 = (255 & wi.field_c[3]) + (65280 & wi.field_c[2] << 1093552008);
                                      var13 = (255 & wi.field_c[6]) + (((wi.field_c[4] & 255) << -770117584) + (wi.field_c[5] << 1225973512 & 65280));
                                      var14 = wi.field_c[7] & 255;
                                      var11 = (wi.field_c[1] & 255) + ((255 & wi.field_c[0]) << 365069896);
                                      break L2;
                                    }
                                  }
                                  if (var11 == param0) {
                                    if (var9 == var12) {
                                      if (var14 == this.field_b) {
                                        if (-1 >= (var13 ^ -1)) {
                                          if (this.field_c.a(23) / 520L >= (long)var13) {
                                            var16 = var10 + var15;
                                            var9++;
                                            var5 = var13;
                                            var17 = var15;
                                            L5: while (true) {
                                              if (var16 <= var17) {
                                                continue L1;
                                              } else {
                                                incrementValue$2 = var8;
                                                var8++;
                                                var7[incrementValue$2] = wi.field_c[var17];
                                                var17++;
                                                continue L5;
                                              }
                                            }
                                          } else {
                                            stackIn_43_0 = null;

                                            decompiledRegionSelector0 = 2;
                                            break L0;
                                          }
                                        } else {
                                          return null;
                                        }
                                      } else {
                                        stackIn_37_0 = null;

                                        decompiledRegionSelector0 = 1;
                                        break L0;
                                      }
                                    } else {
                                      return null;
                                    }
                                  } else {
                                    return null;
                                  }
                                } else {
                                  return null;
                                }
                              }
                            }
                          } else {
                            return null;
                          }
                        } else {
                          return null;
                        }
                      } else {
                        stackIn_9_0 = null;

                        decompiledRegionSelector0 = 0;
                        break L0;
                      }
                    } else {
                      return null;
                    }
                  } else {
                    return null;
                  }
                }
              } catch (java.io.IOException decompiledCaughtParameter0) {
                decompiledCaughtException = decompiledCaughtParameter0;
                var4 = (IOException) (Object) decompiledCaughtException;
                stackIn_52_0 = null;
                return (byte[]) ((Object) stackIn_52_0);
              }
              if (decompiledRegionSelector0 == 0) {
                return (byte[]) ((Object) stackIn_9_0);
              } else {
                if (decompiledRegionSelector0 == 1) {
                  return (byte[]) ((Object) stackIn_37_0);
                } else {
                  if (decompiledRegionSelector0 == 2) {
                    return (byte[]) ((Object) stackIn_43_0);
                  } else {
                    return stackIn_49_0;
                  }
                }
              }
            }
        } catch (RuntimeException | Error decompiledUncheckedException) {
            throw decompiledUncheckedException;
        } catch (Throwable decompiledCheckedException) {
            throw new RuntimeException(decompiledCheckedException);
        }
    }

    final static int a(qe param0, int param1) {
        int stackIn_3_0 = 0;
        int stackIn_38_0 = 0;
        RuntimeException stackIn_41_0 = null;
        StringBuilder stackIn_41_1 = null;
        RuntimeException stackIn_42_0 = null;
        StringBuilder stackIn_42_1 = null;
        String stackIn_42_2 = null;
        int decompiledRegionSelector0 = 0;
        RuntimeException decompiledCaughtException = null;
        int var2_int = 0;
        RuntimeException var2 = null;
        int var3 = 0;
        int var4 = 0;
        int var5 = 0;
        int var6 = 0;
        int var7 = 0;
        int var8 = 0;
        qe var9 = null;
        int var11 = 0;
        int var12 = 0;
        int var13 = 0;
        int var14 = 0;
        int var15 = 0;
        var14 = TorChallenge.field_F ? 1 : 0;
        try {
          L0: {
            if (0 < param0.field_G) {
              L1: {
                var2_int = 0;
                if (-1 <= (param0.field_Q ^ -1)) {
                  break L1;
                } else {
                  var2_int = 1;
                  break L1;
                }
              }
              L2: {
                if (param0.field_Q >= 0) {
                  break L2;
                } else {
                  var2_int = -1;
                  break L2;
                }
              }
              var3 = param0.field_B + param0.field_Q;
              var4 = param0.field_o;
              var5 = 89 / ((11 - param1) / 56);
              L3: while (true) {
                if (256 >= var3 * var2_int) {
                  L4: {
                    var6 = (-param0.field_o + var4) * var2_int;
                    if ((var4 ^ -1) <= -65) {
                      var4 -= 64;
                      break L4;
                    } else {
                      break L4;
                    }
                  }
                  L5: {
                    if (0 > var4) {
                      var4 += 64;
                      break L5;
                    } else {
                      break L5;
                    }
                  }
                  var11 = 0;
                  var12 = 0;
                  L6: while (true) {
                    if (var6 <= var12) {
                      stackIn_38_0 = 1;
                      decompiledRegionSelector0 = 1;
                      break L0;
                    } else {
                      L7: {
                        var7 = var2_int * var12 + param0.field_o;
                        var15 = var7 % 64;
                        var7 = var15;
                        if (-1 < (var15 ^ -1)) {
                          var15 += 64;
                          break L7;
                        } else {
                          break L7;
                        }
                      }
                      L8: {
                        if (null == n.field_kb[var15][param0.field_H]) {
                          break L8;
                        } else {
                          var13 = 0;
                          L9: while (true) {
                            if (n.field_kb[var15][param0.field_H].length <= var13) {
                              break L8;
                            } else {
                              L10: {
                                if (var13 < 0) {
                                  break L10;
                                } else {
                                  var8 = var13;
                                  if (eh.a(var2_int, -30917, n.field_kb[var15][param0.field_H], var8, param0)) {
                                    L11: {
                                      if (var11 == 0) {
                                        param0.field_Q = 8 * param0.field_Q / 10;
                                        var11 = 1;
                                        break L11;
                                      } else {
                                        break L11;
                                      }
                                    }
                                    if (n.field_kb[var15][param0.field_H][var8] < ea.field_g.length) {
                                      var9 = ea.field_g[n.field_kb[var15][param0.field_H][var8]];
                                      if (var9 == null) {
                                        break L10;
                                      } else {
                                        var9.field_Q = param0.field_Q;
                                        break L10;
                                      }
                                    } else {
                                      break L10;
                                    }
                                  } else {
                                    break L10;
                                  }
                                }
                              }
                              var13++;
                              continue L9;
                            }
                          }
                        }
                      }
                      var12++;
                      continue L6;
                    }
                  }
                } else {
                  var4 = var4 + var2_int;
                  var3 = var3 + var2_int * -256;
                  continue L3;
                }
              }
            } else {
              stackIn_3_0 = -1;
              decompiledRegionSelector0 = 0;
              break L0;
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L12: {
            var2 = decompiledCaughtException;
            stackIn_41_0 = (RuntimeException) (var2);

            stackIn_41_1 = new StringBuilder().append("la.B(");

            if (param0 == null) {
              stackIn_42_0 = (RuntimeException) ((Object) stackIn_41_0);
              stackIn_42_1 = (StringBuilder) ((Object) stackIn_41_1);
              stackIn_42_2 = "null";
              break L12;
            } else {
              stackIn_42_0 = (RuntimeException) ((Object) stackIn_41_0);
              stackIn_42_1 = (StringBuilder) ((Object) stackIn_41_1);
              stackIn_42_2 = "{...}";
              break L12;
            }
          }
          throw oj.a((Throwable) ((Object) stackIn_42_0), stackIn_42_2 + ',' + param1 + ')');
        }
        if (decompiledRegionSelector0 == 0) {
          return stackIn_3_0;
        } else {
          return stackIn_38_0;
        }
    }

    final boolean a(int param0, byte[] param1, int param2, byte param3) {
        Object var5 = null;
        RuntimeException var5_ref = null;
        int var6 = 0;
        Throwable var7 = null;
        int stackIn_9_0 = 0;
        RuntimeException stackIn_14_0 = null;
        StringBuilder stackIn_14_1 = null;
        RuntimeException stackIn_15_0 = null;
        StringBuilder stackIn_15_1 = null;
        String stackIn_15_2 = null;
        Throwable decompiledCaughtException = null;
        try {
          L0: {
            L1: {
              if (param3 >= 116) {
                break L1;
              } else {
                this.toString();
                break L1;
              }
            }
            var5 = this.field_c;
            synchronized (var5) {
              L2: {
                L3: {
                  if (param2 < 0) {
                    break L3;
                  } else {
                    if (param2 <= this.field_g) {
                      L4: {
                        var6 = this.a(param2, -6, param0, param1, true) ? 1 : 0;
                        if (var6 != 0) {
                          break L4;
                        } else {
                          var6 = this.a(param2, -6, param0, param1, false) ? 1 : 0;
                          break L4;
                        }
                      }
                      stackIn_9_0 = var6;
                      break L2;
                    } else {
                      break L3;
                    }
                  }
                }
                throw new IllegalArgumentException();
              }
            }
            break L0;
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L5: {
            var5_ref = (RuntimeException) (Object) decompiledCaughtException;
            stackIn_14_0 = (RuntimeException) (var5_ref);

            stackIn_14_1 = new StringBuilder().append("la.G(").append(param0).append(',');

            if (param1 == null) {
              stackIn_15_0 = (RuntimeException) ((Object) stackIn_14_0);
              stackIn_15_1 = (StringBuilder) ((Object) stackIn_14_1);
              stackIn_15_2 = "null";
              break L5;
            } else {
              stackIn_15_0 = (RuntimeException) ((Object) stackIn_14_0);
              stackIn_15_1 = (StringBuilder) ((Object) stackIn_14_1);
              stackIn_15_2 = "{...}";
              break L5;
            }
          }
          throw oj.a((Throwable) ((Object) stackIn_15_0), stackIn_15_2 + ',' + param2 + ',' + param3 + ')');
        }
        return stackIn_9_0 != 0;
    }

    public static void b(byte param0) {
        field_a = null;
        field_d = null;
        field_f = null;
        if (param0 != -64) {
            la.b((byte) -70);
        }
    }

    final static void a(byte param0) {
        int decompiledRegionSelector0 = 0;
        RuntimeException decompiledCaughtException = null;
        RuntimeException var1 = null;
        int var2 = 0;
        int[] var3 = null;
        int[] var4 = null;
        pa var5 = null;
        int var6 = 0;
        int var7 = 0;
        int var8 = 0;
        int[] var10 = null;
        int[] var11 = null;
        rd var12 = null;
        pa var13 = null;
        int[] var14 = null;
        ql var15 = null;
        Object var16 = null;
        ql var16_ref = null;
        int[] var17 = null;
        var16 = null;
        var8 = TorChallenge.field_F ? 1 : 0;
        try {
          L0: {
            L1: {
              var13 = id.field_b;
              if (param0 < -29) {
                break L1;
              } else {
                la.a((byte) 70);
                break L1;
              }
            }
            L2: {
              var2 = var13.j(-126);
              if (var2 != 0) {
                if (1 != var2) {
                  if ((var2 ^ -1) != -3) {
                    td.a("A1: " + ck.a(-13299), (Throwable) null, (byte) -74);
                    ob.b(-87);
                    break L2;
                  } else {
                    var16_ref = (ql) ((Object) ji.field_q.c((byte) 125));
                    if (var16_ref == null) {
                      ob.b(-107);
                      decompiledRegionSelector0 = 3;
                      break L0;
                    } else {
                      var16_ref.field_o = jj.b((byte) -24);
                      var16_ref.field_i = true;
                      var16_ref.field_m = var16_ref.field_o[0];
                      var16_ref.a(true);
                      break L2;
                    }
                  }
                } else {
                  var12 = (rd) ((Object) bb.field_d.c((byte) 113));
                  if (var12 != null) {
                    var12.a(true);
                    break L2;
                  } else {
                    ob.b(-93);
                    decompiledRegionSelector0 = 2;
                    break L0;
                  }
                }
              } else {
                var11 = jj.b((byte) -24);
                var17 = var11;
                var14 = var17;
                var3 = var14;
                var10 = var11;
                var4 = var10;
                var5 = var13;
                var6 = ((uf) ((Object) var5)).j(-95);
                var7 = 0;
                L3: while (true) {
                  if (var6 <= var7) {
                    var15 = (ql) ((Object) ji.field_q.c((byte) 119));
                    if (var15 == null) {
                      ob.b(-100);
                      decompiledRegionSelector0 = 0;
                      break L0;
                    } else {
                      var15.field_m = var17[0];
                      var15.field_o = var3;
                      var15.field_i = true;
                      var15.a(true);
                      break L2;
                    }
                  } else {
                    var10[var7] = ((uf) ((Object) var5)).i(38);
                    var7++;
                    continue L3;
                  }
                }
              }
            }
            decompiledRegionSelector0 = 1;
            break L0;
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          var1 = decompiledCaughtException;
          throw oj.a((Throwable) ((Object) var1), "la.F(" + param0 + ')');
        }
        if (decompiledRegionSelector0 == 0) {
          return;
        } else {
          if (decompiledRegionSelector0 == 1) {
            return;
          } else {
            if (decompiledRegionSelector0 == 2) {
              return;
            } else {
              return;
            }
          }
        }
    }

    public final String toString() {
        return "" + this.field_b;
    }

    final static String a(byte param0, int param1, char param2) {
        int var4 = 0;
        int var5 = TorChallenge.field_F ? 1 : 0;
        char[] var6 = new char[param1];
        char[] var3 = var6;
        if (param0 < 75) {
            field_a = (kb) null;
        }
        for (var4 = 0; param1 > var4; var4++) {
            var6[var4] = param2;
        }
        return new String(var6);
    }

    la(int param0, rk param1, rk param2, int param3) {
        this.field_c = null;
        this.field_e = null;
        this.field_g = 65000;
        try {
            this.field_g = param3;
            this.field_b = param0;
            this.field_e = param2;
            this.field_c = param1;
        } catch (RuntimeException runtimeException) {
            throw oj.a((Throwable) ((Object) runtimeException), "la.<init>(" + param0 + ',' + (param1 != null ? "{...}" : "null") + ',' + (param2 != null ? "{...}" : "null") + ',' + param3 + ')');
        }
    }

    static {
    }
}
