/*
 * Decompiled by CFR-JS 0.4.0.
 */
abstract class of {
    static rl field_c;
    static ka field_b;
    static boolean[][][] field_a;
    static int field_d;
    static int field_f;
    static int field_e;

    abstract void a(int param0, byte[] param1);

    final static byte[] a(byte param0, boolean param1, Object param2) {
        byte[] var3 = null;
        int var3_int = 0;
        RuntimeException var3_ref = null;
        of var4 = null;
        Object stackIn_2_0 = null;
        byte[] stackIn_7_0 = null;
        byte[] stackIn_9_0 = null;
        byte[] stackIn_13_0 = null;
        RuntimeException stackIn_17_0 = null;
        StringBuilder stackIn_17_1 = null;
        RuntimeException stackIn_18_0 = null;
        StringBuilder stackIn_18_1 = null;
        String stackIn_18_2 = null;
        int decompiledRegionSelector0 = 0;
        RuntimeException decompiledCaughtException = null;
        try {
          L0: {
            if (param2 != null) {
              if (param2 instanceof byte[]) {
                var3 = (byte[]) (param2);
                if (param1) {
                  stackIn_9_0 = ek.a(var3, -43);
                  decompiledRegionSelector0 = 2;
                  break L0;
                } else {
                  stackIn_7_0 = (byte[]) (var3);
                  decompiledRegionSelector0 = 1;
                  break L0;
                }
              } else {
                if (param2 instanceof of) {
                  var4 = (of) (param2);
                  stackIn_13_0 = var4.c(2);
                  decompiledRegionSelector0 = 3;
                  break L0;
                } else {
                  var3_int = -55 / ((65 - param0) / 60);
                  throw new IllegalArgumentException();
                }
              }
            } else {
              stackIn_2_0 = null;
              decompiledRegionSelector0 = 0;
              break L0;
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L1: {
            var3_ref = decompiledCaughtException;
            stackIn_17_0 = (RuntimeException) (var3_ref);

            stackIn_17_1 = new StringBuilder().append("of.D(").append(param0).append(',').append(param1).append(',');

            if (param2 == null) {
              stackIn_18_0 = (RuntimeException) ((Object) stackIn_17_0);
              stackIn_18_1 = (StringBuilder) ((Object) stackIn_17_1);
              stackIn_18_2 = "null";
              break L1;
            } else {
              stackIn_18_0 = (RuntimeException) ((Object) stackIn_17_0);
              stackIn_18_1 = (StringBuilder) ((Object) stackIn_17_1);
              stackIn_18_2 = "{...}";
              break L1;
            }
          }
          throw oj.a((Throwable) ((Object) stackIn_18_0), stackIn_18_2 + ')');
        }
        if (decompiledRegionSelector0 == 0) {
          return (byte[]) ((Object) stackIn_2_0);
        } else {
          if (decompiledRegionSelector0 == 1) {
            return stackIn_7_0;
          } else {
            if (decompiledRegionSelector0 == 2) {
              return stackIn_9_0;
            } else {
              return stackIn_13_0;
            }
          }
        }
    }

    final static dk b(int param0) {
        if (!(pf.field_f != null)) {
            pf.field_f = new dk();
            pf.field_f.a(oe.field_g, -22373);
            pf.field_f.field_e = 7697781;
            pf.field_f.field_r = 5;
            pf.field_f.field_o = 4;
            pf.field_f.field_h = 0;
            pf.field_f.field_t = ug.field_f;
            pf.field_f.field_k = 6;
            pf.field_f.field_n = 14;
            pf.field_f.field_p = 2763306;
        }
        int var1 = 24 % ((52 - param0) / 61);
        return pf.field_f;
    }

    public static void a(int param0) {
        field_c = null;
        field_b = null;
        field_a = (boolean[][][]) null;
        if (param0 != 8727) {
            of.a(85, -111, (byte) -85, true, -34);
        }
    }

    final static void a(int param0, int param1, byte param2, boolean param3, int param4) {
        int stackIn_17_0 = 0;
        int[][] stackIn_82_0;
        int stackIn_82_1;
        int stackIn_82_2;
        int stackIn_82_3;
        int[][] stackIn_83_0;
        int stackIn_83_1;
        int stackIn_83_2;
        int stackIn_83_3;
        int stackIn_83_4;
        int[][] stackIn_101_0;
        int stackIn_101_1;
        int stackIn_101_2;
        int stackIn_101_3;
        int[][] stackIn_102_0;
        int stackIn_102_1;
        int stackIn_102_2;
        int stackIn_102_3;
        int stackIn_102_4;
        int[][] stackIn_118_0;
        int stackIn_118_1;
        int stackIn_118_2;
        int stackIn_118_3;
        int[][] stackIn_119_0;
        int stackIn_119_1;
        int stackIn_119_2;
        int stackIn_119_3;
        int stackIn_119_4;
        int decompiledRegionSelector0 = 0;
        Throwable caughtException = null;
        RuntimeException decompiledCaughtException = null;
        int var5_int = 0;
        RuntimeException var5 = null;
        int var6 = 0;
        int var7 = 0;
        int var8 = 0;
        String var9 = null;
        int var10 = 0;
        int var11 = 0;
        int var12 = 0;
        int var13 = 0;
        int var14 = 0;
        int var15 = 0;
        int var16 = 0;
        int var17 = 0;
        int var18 = 0;
        qe var19 = null;
        int var20 = 0;
        int var21 = 0;
        int var22 = 0;
        int var23 = 0;
        int var24 = 0;
        int var25 = 0;
        int var26 = 0;
        int var27 = 0;
        int var28 = 0;
        String var29 = null;
        var28 = TorChallenge.field_F ? 1 : 0;
        try {
          L0: {
            L1: {
              var5_int = gh.field_x ? 1 : 0;
              var6 = bc.field_o;
              var7 = bb.field_h;
              if (var5_int != 0) {
                if (-1 != var6) {
                  if ((var6 ^ -1) != -14) {
                    L2: {
                      if ((var6 ^ -1) != -2) {
                        break L2;
                      } else {
                        if (var7 != -1) {
                          break L2;
                        } else {
                          stackIn_17_0 = 1;
                          break L1;
                        }
                      }
                    }
                    if (-2 == (var6 ^ -1)) {
                      if (var7 == 13) {
                        stackIn_17_0 = 1;
                        break L1;
                      } else {
                        stackIn_17_0 = 0;
                        break L1;
                      }
                    } else {
                      stackIn_17_0 = 0;
                      break L1;
                    }
                  } else {
                    stackIn_17_0 = 1;
                    break L1;
                  }
                } else {
                  stackIn_17_0 = 1;
                  break L1;
                }
              } else {
                stackIn_17_0 = 0;
                break L1;
              }
            }
            L3: {
              var8 = stackIn_17_0;
              if (var8 == 0) {
                break L3;
              } else {
                qg.a(sh.field_w);
                qg.a(0, 0, 320, 240);
                break L3;
              }
            }
            L4: {
              od.a(param0, param1, 1, 7, 6, lj.field_o[0]);
              if (var8 != 0) {
                qg.b(sh.field_w);
                break L4;
              } else {
                break L4;
              }
            }
            od.a(param0, param1 + (479 - lj.field_o[1].field_v), 1, 7, 6, lj.field_o[1]);
            uk.field_N.g(param0, param1);
            if (param2 >= 32) {
              L5: {
                L6: {
                  var10 = 5 + nh.field_db[0];
                  var11 = nh.field_db[1] - -5;
                  var12 = (-8 + nh.field_db[2]) / 5;
                  var14 = 0;
                  var15 = 0;
                  var16 = -1;
                  var17 = 0;
                  var18 = 24;
                  if (!param3) {
                    break L6;
                  } else {
                    L7: {
                      if (param4 == 0) {
                        break L7;
                      } else {
                        if ((param4 ^ -1) == -3) {
                          break L7;
                        } else {
                          break L6;
                        }
                      }
                    }
                    var20 = 0;
                    L8: while (true) {
                      if (var20 >= 5) {
                        break L5;
                      } else {
                        L9: {
                          if (0 < (1 << var20 & vi.field_a)) {
                            L10: {
                              var19 = ea.field_g[mh.field_d[var20]];
                              if (wa.field_pb[-(ea.field_e * 5) + var19.field_V]) {
                                break L10;
                              } else {
                                if (var15 != 0) {
                                  break L10;
                                } else {
                                  var17 = var19.field_D;
                                  var16 = var20;
                                  var15 = 1;
                                  break L10;
                                }
                              }
                            }
                            var14++;
                            break L9;
                          } else {
                            break L9;
                          }
                        }
                        var20++;
                        continue L8;
                      }
                    }
                  }
                }
                if (param3) {
                  break L5;
                } else {
                  var14 = ef.field_k % 300;
                  var14 = var14 / 50;
                  var17 = ef.field_k % 50;
                  var15 = 1;
                  break L5;
                }
              }
              L11: {
                L12: {
                  if (!param3) {
                    break L12;
                  } else {
                    L13: {
                      if (-1 == (param4 ^ -1)) {
                        break L13;
                      } else {
                        if (-3 != (param4 ^ -1)) {
                          break L12;
                        } else {
                          break L13;
                        }
                      }
                    }
                    var20 = 0;
                    L14: while (true) {
                      if ((var20 ^ -1) <= -6) {
                        break L11;
                      } else {
                        L15: {
                          if (-1 > (vi.field_a & 1 << var20 ^ -1)) {
                            L16: {
                              if (var15 == 0) {
                                break L16;
                              } else {
                                if (var16 == var20) {
                                  dl.field_j.a(var10 - -param0, param1 + var11, -(5 * var17) + 255, 16777215);
                                  break L15;
                                } else {
                                  break L16;
                                }
                              }
                            }
                            dl.field_j.g(param0 + var10, param1 + var11);
                            break L15;
                          } else {
                            break L15;
                          }
                        }
                        var10 = var10 + var12;
                        var20++;
                        continue L14;
                      }
                    }
                  }
                }
                if (!param3) {
                  var20 = 0;
                  L17: while (true) {
                    if (var20 >= var14 + -1) {
                      L18: {
                        if (var15 == 0) {
                          break L18;
                        } else {
                          if (0 < var14) {
                            dl.field_j.a(var10 - -param0, var11 + param1, 255 + -(5 * var17), 16777215);
                            break L11;
                          } else {
                            break L18;
                          }
                        }
                      }
                      if (-1 <= (var14 ^ -1)) {
                        break L11;
                      } else {
                        dl.field_j.g(param0 + var10, param1 + var11);
                        break L11;
                      }
                    } else {
                      dl.field_j.g(var10 + param0, var11 + param1);
                      var10 = var10 + var12;
                      var20++;
                      continue L17;
                    }
                  }
                } else {
                  break L11;
                }
              }
              L19: {
                L20: {
                  if (0 == param4) {
                    break L20;
                  } else {
                    if ((param4 ^ -1) == -2) {
                      break L20;
                    } else {
                      break L19;
                    }
                  }
                }
                L21: {
                  var20 = 0;
                  var21 = 0;
                  var22 = 0;
                  var23 = -1;
                  if (param3) {
                    var21 = wb.field_p.field_d;
                    var20 = wb.field_p.field_db;
                    var22 = jj.a(255);
                    if ((ii.field_i ^ -1) >= -1) {
                      break L21;
                    } else {
                      var23 = 255 * ii.field_i / 50;
                      break L21;
                    }
                  } else {
                    var21 = 5;
                    var20 = ef.field_k % 250 / 50;
                    var22 = ef.field_k % 300 / 50;
                    var20 = var21 - var20;
                    break L21;
                  }
                }
                var10 = 10 + ob.field_b[0];
                var11 = 10 + ob.field_b[1];
                var12 = (-8 + ob.field_b[2] + -20) / 9;
                var10 = var10 + (-(9 * var12) + (-28 + ob.field_b[2])) / 2;
                var13 = ob.field_b[3] - 20;
                var24 = 0;
                L22: while (true) {
                  L23: {
                    if (var24 >= var20) {
                      break L23;
                    } else {
                      if ((ad.field_b.length ^ -1) < (var24 ^ -1)) {
                        L24: {
                          L25: {
                            if (0 >= var23) {
                              break L25;
                            } else {
                              if (var24 >= be.field_d) {
                                mb.field_d[0].a(var10 - -param0, param1 + var11, var23, 16777215);
                                break L24;
                              } else {
                                break L25;
                              }
                            }
                          }
                          mb.field_d[0].g(var10 + param0, var11 - -param1);
                          break L24;
                        }
                        L26: {
                          stackIn_82_0 = ad.field_b[var24];

                          stackIn_82_1 = var11 + param1;

                          stackIn_82_2 = mb.field_d[0].field_v;

                          stackIn_82_3 = 4;

                          if (0 != de.field_d % 8) {
                            stackIn_83_0 = (int[][]) ((Object) stackIn_82_0);
                            stackIn_83_1 = stackIn_82_1;
                            stackIn_83_2 = stackIn_82_2;
                            stackIn_83_3 = stackIn_82_3;
                            stackIn_83_4 = 0;
                            break L26;
                          } else {
                            stackIn_83_0 = (int[][]) ((Object) stackIn_82_0);
                            stackIn_83_1 = stackIn_82_1;
                            stackIn_83_2 = stackIn_82_2;
                            stackIn_83_3 = stackIn_82_3;
                            stackIn_83_4 = 1;
                            break L26;
                          }
                        }
                        rd.a(stackIn_83_0, stackIn_83_1, stackIn_83_2, stackIn_83_3, stackIn_83_4 != 0, 4, -1012815519, mb.field_d[0].field_q, param0 + var10, 2);
                        var10 = var10 + (1 + var12);
                        var24++;
                        continue L22;
                      } else {
                        break L23;
                      }
                    }
                  }
                  var24 = var20;
                  L27: while (true) {
                    L28: {
                      if (var21 <= var24) {
                        break L28;
                      } else {
                        if ((ad.field_b.length ^ -1) < (var24 ^ -1)) {
                          mb.field_d[2].g(param0 + var10, param1 + var11);
                          var10 = var10 + (1 + var12);
                          var24++;
                          continue L27;
                        } else {
                          break L28;
                        }
                      }
                    }
                    var24 = 5 - var22 % 5;
                    var13 = mb.field_d[0].field_v;
                    var12 = mb.field_d[0].field_q;
                    if (var22 % 5 > 0) {
                      mb.field_d[0].c(-var12 + (-10 + ob.field_b[2] + (ob.field_b[0] - (-param0 + -var24))), var24 + (var11 - -param1), -(2 * var24) + var12, -(var24 * 2) + var13);
                      break L19;
                    } else {
                      break L19;
                    }
                  }
                }
              }
              L29: {
                if (param4 == 2) {
                  if (o.field_l > 0) {
                    var11 = ed.field_i[1] - -4 - (ec.field_a[1].field_q >> -537794942);
                    var12 = ed.field_i[2];
                    var10 = -(ec.field_a[1].field_q >> 1960341282) + ed.field_i[0] + 1;
                    var20 = 0;
                    L30: while (true) {
                      if (10 <= var20) {
                        break L29;
                      } else {
                        L31: {
                          if (db.field_B <= var20) {
                            ec.field_a[2].g(var10, var11);
                            break L31;
                          } else {
                            L32: {
                              ec.field_a[1].g(var10, var11);
                              stackIn_101_0 = p.field_f[var20];

                              stackIn_101_1 = (ec.field_a[1].field_q >> -1519351582) + var11;

                              stackIn_101_2 = ec.field_a[1].field_v >> -1753202527;

                              stackIn_101_3 = 4;

                              if (0 != de.field_d % 8) {
                                stackIn_102_0 = (int[][]) ((Object) stackIn_101_0);
                                stackIn_102_1 = stackIn_101_1;
                                stackIn_102_2 = stackIn_101_2;
                                stackIn_102_3 = stackIn_101_3;
                                stackIn_102_4 = 0;
                                break L32;
                              } else {
                                stackIn_102_0 = (int[][]) ((Object) stackIn_101_0);
                                stackIn_102_1 = stackIn_101_1;
                                stackIn_102_2 = stackIn_101_2;
                                stackIn_102_3 = stackIn_101_3;
                                stackIn_102_4 = 1;
                                break L32;
                              }
                            }
                            rd.a(stackIn_102_0, stackIn_102_1, stackIn_102_2, stackIn_102_3, stackIn_102_4 != 0, 4, -1012815519, ec.field_a[1].field_q >> -2037405471, var10 + (ec.field_a[1].field_q >> 360274978), 2);
                            break L31;
                          }
                        }
                        var10 = var10 + (ec.field_a[1].field_u - -1);
                        var20++;
                        continue L30;
                      }
                    }
                  } else {
                    break L29;
                  }
                } else {
                  break L29;
                }
              }
              L33: {
                if (1 == param4) {
                  L34: {
                    var11 = 6 + oa.field_a[1];
                    var20 = 8;
                    var10 = oa.field_a[0] - -7;
                    if (null != cj.field_f) {
                      rl.field_h[fh.field_e].g(var10, var11);
                      break L34;
                    } else {
                      break L34;
                    }
                  }
                  L35: {
                    var11 = oa.field_a[1] + (oa.field_a[3] - mb.field_d[1].field_v >> 1209342817);
                    var10 = oa.field_a[0] - (-10 - rl.field_h[fh.field_e].field_q);
                    var13 = oa.field_a[3];
                    var12 = -rl.field_h[fh.field_e].field_q + (oa.field_a[2] + -20 + -var20 + 1);
                    var21 = var12 / var20;
                    var22 = (-(var21 * var20) + var12) / 2 + var10;
                    var12 = var21 * var20;
                    var23 = 0;
                    var24 = 0;
                    if (cj.field_f == null) {
                      break L35;
                    } else {
                      var23 = cj.field_f.field_db;
                      var24 = cj.field_f.field_d;
                      if (var23 <= 0) {
                        var23 = 0;
                        break L35;
                      } else {
                        break L35;
                      }
                    }
                  }
                  var25 = var10;
                  qg.a(sh.field_w);
                  var26 = var22 - -((var20 + -1 + var12) * var23 / var24);
                  qg.a(var25, var11, var26, var11 + var13);
                  var27 = 0;
                  L36: while (true) {
                    if (var27 >= var20) {
                      var25 = var26;
                      qg.b(sh.field_w);
                      var26 = var20 + -1 + (var12 + var22);
                      qg.a(sh.field_w);
                      qg.a(var25, var11, var26, var13 + var11);
                      var22 = var10 + 1;
                      var27 = 0;
                      L37: while (true) {
                        if (var27 >= var20) {
                          qg.b(sh.field_w);
                          break L33;
                        } else {
                          mb.field_d[2].g(var22, var11);
                          var22 = var22 + (var21 - -1);
                          var27++;
                          continue L37;
                        }
                      }
                    } else {
                      L38: {
                        mb.field_d[1].g(var22, var11);
                        stackIn_118_0 = ad.field_b[var27];

                        stackIn_118_1 = var11;

                        stackIn_118_2 = mb.field_d[1].field_v;

                        stackIn_118_3 = 4;

                        if (de.field_d % 8 != 0) {
                          stackIn_119_0 = (int[][]) ((Object) stackIn_118_0);
                          stackIn_119_1 = stackIn_118_1;
                          stackIn_119_2 = stackIn_118_2;
                          stackIn_119_3 = stackIn_118_3;
                          stackIn_119_4 = 0;
                          break L38;
                        } else {
                          stackIn_119_0 = (int[][]) ((Object) stackIn_118_0);
                          stackIn_119_1 = stackIn_118_1;
                          stackIn_119_2 = stackIn_118_2;
                          stackIn_119_3 = stackIn_118_3;
                          stackIn_119_4 = 1;
                          break L38;
                        }
                      }
                      rd.a(stackIn_119_0, stackIn_119_1, stackIn_119_2, stackIn_119_3, stackIn_119_4 != 0, 4, -1012815519, mb.field_d[1].field_q, var22, 2);
                      var22 = var22 + (1 + var21);
                      var27++;
                      continue L36;
                    }
                  }
                } else {
                  break L33;
                }
              }
              L39: {
                var12 = oj.field_e[2];
                var11 = oj.field_e[1];
                var10 = oj.field_e[0];
                var13 = oj.field_e[3];
                var10 = 2 + (var10 + (var12 >> 1503696577) - -12);
                var18 = 24;
                var20 = 0;
                if (!param3) {
                  var20 = ef.field_k % 300;
                  var20 = var20 / 100;
                  var20 = 3 - var20;
                  break L39;
                } else {
                  var20 = gl.field_k;
                  break L39;
                }
              }
              L40: {
                L41: {
                  tf.field_x.a(Integer.toString(var20), var10 + param0, param1 + var11 + ((var13 - var18 >> 69337409) + 18), 0, -1);
                  if (param4 == 0) {
                    break L41;
                  } else {
                    if (param4 != 1) {
                      if (param4 != 2) {
                        break L40;
                      } else {
                        var11 = af.field_f[1];
                        var12 = af.field_f[2];
                        var9 = w.field_j + (o.field_l / 50 + 1);
                        var10 = af.field_f[0];
                        var13 = af.field_f[3];
                        tf.field_x.c(var9, 13 + var10, (var13 - var18 >> -658017055) + (var11 + 18), 0, -1);
                        break L40;
                      }
                    } else {
                      break L41;
                    }
                  }
                }
                L42: {
                  var21 = 0;
                  var22 = 0;
                  if (!param3) {
                    var21 = 100 * (ef.field_k % 1000);
                    var22 = ef.field_k % 500 / 200;
                    break L42;
                  } else {
                    var21 = tf.field_o;
                    var22 = ab.field_d;
                    break L42;
                  }
                }
                var10 = af.field_f[0];
                var9 = oi.field_h + var21;
                var11 = af.field_f[1];
                var13 = af.field_f[3];
                var12 = af.field_f[2];
                tf.field_x.c(var9, 13 + var10 - -param0, param1 + var11 - -(var13 - var18 >> 1510612161) - -18, 0, -1);
                if (1 < var22) {
                  var11 = ag.field_e.field_x + ((var13 - ag.field_e.field_K - ag.field_e.field_x >> 500098625) + var11);
                  var29 = vj.field_b + var22;
                  var10 = var12 + var10;
                  ag.field_e.b(var29, var10 + (param0 - 13), param1 + var11, 0, -1);
                  break L40;
                } else {
                  break L40;
                }
              }
              L43: {
                L44: {
                  L45: {
                    if (0 == param4) {
                      break L45;
                    } else {
                      if (param4 != 2) {
                        break L44;
                      } else {
                        break L45;
                      }
                    }
                  }
                  if (!param3) {
                    break L44;
                  } else {
                    TorChallenge.field_E = be.a(mj.field_b, dk.field_a, vj.field_c, param1 + rf.field_eb[vj.field_c][1], rf.field_eb[vj.field_c][0] + param0, 25, TorChallenge.field_E, hf.field_c);
                    break L43;
                  }
                }
                break L43;
              }
              decompiledRegionSelector0 = 1;
              break L0;
            } else {
              decompiledRegionSelector0 = 0;
              break L0;
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          var5 = decompiledCaughtException;
          throw oj.a((Throwable) ((Object) var5), "of.C(" + param0 + ',' + param1 + ',' + param2 + ',' + param3 + ',' + param4 + ')');
        }
        if (decompiledRegionSelector0 == 0) {
          return;
        } else {
          return;
        }
    }

    abstract byte[] c(int param0);

    static {
        field_b = new ka(160, 160);
        field_a = new boolean[][][]{new boolean[][]{new boolean[]{false, false, false, false, false, true}, new boolean[]{false}, new boolean[]{false}, new boolean[]{false, false, false, false, true, true}, new boolean[]{false}, new boolean[]{false}, new boolean[]{false}}, new boolean[][]{new boolean[]{false, false, false, false, false, true}, new boolean[]{false, false, false, false, true}, new boolean[]{false, false, false, true}, new boolean[]{false}, new boolean[]{false}, new boolean[]{false, false, false, false, false, false, true}, new boolean[]{false}}, new boolean[][]{new boolean[]{false, false, false, false, false, true}, new boolean[]{false}, new boolean[]{false}, new boolean[]{false}, new boolean[]{false, false, false, false, false, true}, new boolean[]{false}, new boolean[]{false, false, false, false, false, true}}};
        field_d = 65;
    }
}
