/*
 * Decompiled by CFR-JS 0.4.0.
 */
final class lr {
    static qr field_a;
    static String field_d;
    static byte[] field_c;
    static String field_f;
    static int[] field_b;
    static String field_e;

    final static lp a(ph param0, fs param1, byte param2, fs param3, fs[] param4) {
        String dupTemp$1 = null;
        RuntimeException var5 = null;
        kb var6 = null;
        fe var7 = null;
        ff var8 = null;
        int var9_int = 0;
        ff var9 = null;
        ff var10 = null;
        int var11 = 0;
        lp var12 = null;
        lp var13 = null;
        lp[] stackIn_3_0 = null;
        lp[] stackIn_4_0 = null;
        int stackIn_4_1 = 0;
        fe[] stackIn_6_0 = null;
        fe[] stackIn_7_0 = null;
        int stackIn_7_1 = 0;
        lp stackIn_9_0 = null;
        ff[][] stackIn_12_0 = null;
        ff[][] stackIn_13_0 = null;
        int stackIn_13_1 = 0;
        ff[][] stackIn_17_0 = null;
        ff[][] stackIn_18_0 = null;
        int stackIn_18_1 = 0;
        lp stackIn_20_0 = null;
        RuntimeException stackIn_23_0 = null;
        StringBuilder stackIn_23_1 = null;
        RuntimeException stackIn_24_0 = null;
        StringBuilder stackIn_24_1 = null;
        String stackIn_24_2 = null;
        StringBuilder stackIn_26_1 = null;
        StringBuilder stackIn_27_1 = null;
        String stackIn_27_2 = null;
        StringBuilder stackIn_29_1 = null;
        StringBuilder stackIn_30_1 = null;
        String stackIn_30_2 = null;
        StringBuilder stackIn_32_1 = null;
        StringBuilder stackIn_33_1 = null;
        String stackIn_33_2 = null;
        int decompiledRegionSelector0 = 0;
        RuntimeException decompiledCaughtException = null;
        var11 = ShatteredPlansClient.field_F ? 1 : 0;
        try {
          L0: {
            L1: {
              var12 = new lp(0, 0, 182, rg.field_a);
              var13 = var12;
              var13.field_h = param3;
              stackIn_3_0 = param0.field_h;

              if (param1.field_x < param3.field_x) {
                stackIn_4_0 = (lp[]) ((Object) stackIn_3_0);
                stackIn_4_1 = param3.field_x;
                break L1;
              } else {
                stackIn_4_0 = (lp[]) ((Object) stackIn_3_0);
                stackIn_4_1 = param3.field_x - -1;
                break L1;
              }
            }
            L2: {
              stackIn_4_0[stackIn_4_1] = var12;
              var6 = new kb(pf.field_a[param3.field_x].field_z * 3 / 4, -1 + (var13.field_i / 2 - rs.field_Cb.field_J), 182 + -(pf.field_a[param3.field_x].field_z * 3 / 4), param3.field_k, param3.field_t, 16777215, true);
              var13.a(var6, 8);
              var7 = new fe(3 * pf.field_a[param3.field_x].field_z / 4, var13.field_i / 2 - -1, 182 + -(3 * pf.field_a[param3.field_x].field_z / 4), rs.field_Cb.field_J, 8421504, md.i(0), eb.field_e, 3375155);
              var13.a(var7, param2 + -13);
              stackIn_6_0 = param0.field_i;

              if (param3.field_x >= param1.field_x) {
                stackIn_7_0 = (fe[]) ((Object) stackIn_6_0);
                stackIn_7_1 = -1 + param3.field_x;
                break L2;
              } else {
                stackIn_7_0 = (fe[]) ((Object) stackIn_6_0);
                stackIn_7_1 = param3.field_x;
                break L2;
              }
            }
            stackIn_7_0[stackIn_7_1] = var7;
            dupTemp$1 = re.a(jg.field_f, 4371, new String[]{param3.field_t});
            var7.field_o = dupTemp$1;
            var6.field_o = dupTemp$1;
            var13.field_o = dupTemp$1;
            if (param2 == 21) {
              L3: {
                bg.a(-2612, param4.length);
                var8 = new ff(3 * pf.field_a[param3.field_x].field_z / 4, 3 + var13.field_i / 2 + rs.field_Cb.field_J, ne.a(-1, false));
                var13.a(var8, 8);
                var8.field_o = re.a(jg.field_f, 4371, new String[]{param3.field_t});
                stackIn_12_0 = param0.field_f;

                if (param1.field_x < param3.field_x) {
                  stackIn_13_0 = (ff[][]) ((Object) stackIn_12_0);
                  stackIn_13_1 = param3.field_x;
                  break L3;
                } else {
                  stackIn_13_0 = (ff[][]) ((Object) stackIn_12_0);
                  stackIn_13_1 = 1 + param3.field_x;
                  break L3;
                }
              }
              stackIn_13_0[stackIn_13_1][0] = var8;
              var9_int = 1;
              L4: while (true) {
                if (param4.length + -1 <= var9_int) {
                  var9 = new ff(0, (var12.field_i - pf.field_a[param3.field_x].field_w) / 2, pf.field_a[param3.field_x]);
                  var13.a(var9, param2 ^ 29);
                  var9.field_o = re.a(jg.field_f, 4371, new String[]{param3.field_t});
                  stackIn_20_0 = (lp) (var13);
                  decompiledRegionSelector0 = 1;
                  break L0;
                } else {
                  L5: {
                    var10 = new ff(pf.field_a[param3.field_x].field_z + (2 - -dc.field_F.field_z) * var9_int, var13.field_i / 2 + rs.field_Cb.field_J + 3, wm.a(-1, param2 + -21));
                    var13.a(var10, 8);
                    var10.field_o = re.a(jg.field_f, 4371, new String[]{param3.field_t});
                    stackIn_17_0 = param0.field_f;

                    if (param3.field_x <= param1.field_x) {
                      stackIn_18_0 = (ff[][]) ((Object) stackIn_17_0);
                      stackIn_18_1 = 1 + param3.field_x;
                      break L5;
                    } else {
                      stackIn_18_0 = (ff[][]) ((Object) stackIn_17_0);
                      stackIn_18_1 = param3.field_x;
                      break L5;
                    }
                  }
                  stackIn_18_0[stackIn_18_1][var9_int] = var10;
                  var9_int++;
                  continue L4;
                }
              }
            } else {
              stackIn_9_0 = (lp) null;
              decompiledRegionSelector0 = 0;
              break L0;
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L6: {
            var5 = decompiledCaughtException;
            stackIn_23_0 = (RuntimeException) (var5);

            stackIn_23_1 = new StringBuilder().append("lr.E(");

            if (param0 == null) {
              stackIn_24_0 = (RuntimeException) ((Object) stackIn_23_0);
              stackIn_24_1 = (StringBuilder) ((Object) stackIn_23_1);
              stackIn_24_2 = "null";
              break L6;
            } else {
              stackIn_24_0 = (RuntimeException) ((Object) stackIn_23_0);
              stackIn_24_1 = (StringBuilder) ((Object) stackIn_23_1);
              stackIn_24_2 = "{...}";
              break L6;
            }
          }
          L7: {


            stackIn_26_1 = ((StringBuilder) (Object) stackIn_24_1).append(stackIn_24_2).append(',');

            if (param1 == null) {
              stackIn_24_0 = (RuntimeException) ((Object) stackIn_24_0);
              stackIn_27_1 = (StringBuilder) ((Object) stackIn_26_1);
              stackIn_27_2 = "null";
              break L7;
            } else {
              stackIn_24_0 = (RuntimeException) ((Object) stackIn_24_0);
              stackIn_27_1 = (StringBuilder) ((Object) stackIn_26_1);
              stackIn_27_2 = "{...}";
              break L7;
            }
          }
          L8: {


            stackIn_29_1 = ((StringBuilder) (Object) stackIn_27_1).append(stackIn_27_2).append(',').append(param2).append(',');

            if (param3 == null) {
              stackIn_24_0 = (RuntimeException) ((Object) stackIn_24_0);
              stackIn_30_1 = (StringBuilder) ((Object) stackIn_29_1);
              stackIn_30_2 = "null";
              break L8;
            } else {
              stackIn_24_0 = (RuntimeException) ((Object) stackIn_24_0);
              stackIn_30_1 = (StringBuilder) ((Object) stackIn_29_1);
              stackIn_30_2 = "{...}";
              break L8;
            }
          }
          L9: {


            stackIn_32_1 = ((StringBuilder) (Object) stackIn_30_1).append(stackIn_30_2).append(',');

            if (param4 == null) {
              stackIn_24_0 = (RuntimeException) ((Object) stackIn_24_0);
              stackIn_33_1 = (StringBuilder) ((Object) stackIn_32_1);
              stackIn_33_2 = "null";
              break L9;
            } else {
              stackIn_24_0 = (RuntimeException) ((Object) stackIn_24_0);
              stackIn_33_1 = (StringBuilder) ((Object) stackIn_32_1);
              stackIn_33_2 = "{...}";
              break L9;
            }
          }
          throw r.a((Throwable) ((Object) stackIn_24_0), stackIn_33_2 + ')');
        }
        if (decompiledRegionSelector0 == 0) {
          return stackIn_9_0;
        } else {
          return stackIn_20_0;
        }
    }

    public static void b(int param0) {
        field_d = null;
        field_a = null;
        field_f = null;
        field_b = null;
        field_e = null;
        field_c = null;
        if (param0 != 3) {
            field_c = (byte[]) null;
        }
    }

    final static void a(int param0, int param1, int[] param2, byte param3, Object[] param4) {
        int incrementValue$0 = 0;
        int stackIn_7_0 = 0;
        RuntimeException stackIn_16_0 = null;
        StringBuilder stackIn_16_1 = null;
        RuntimeException stackIn_17_0 = null;
        StringBuilder stackIn_17_1 = null;
        String stackIn_17_2 = null;
        StringBuilder stackIn_19_1 = null;
        StringBuilder stackIn_20_1 = null;
        String stackIn_20_2 = null;
        int decompiledRegionSelector0 = 0;
        RuntimeException decompiledCaughtException = null;
        int var5_int = 0;
        RuntimeException var5 = null;
        int var6 = 0;
        int var7 = 0;
        Object var8 = null;
        int var9 = 0;
        int var10 = 0;
        int var11 = 0;
        Object var12 = null;
        int var13 = 0;
        var13 = ShatteredPlansClient.field_F ? 1 : 0;
        try {
          L0: {
            if (param3 > 66) {
              L1: {
                if (param1 <= param0) {
                  break L1;
                } else {
                  L2: {
                    var5_int = (param0 - -param1) / 2;
                    var6 = param0;
                    var7 = param2[var5_int];
                    param2[var5_int] = param2[param1];
                    param2[param1] = var7;
                    var8 = param4[var5_int];
                    param4[var5_int] = param4[param1];
                    param4[param1] = var8;
                    if (var7 == 2147483647) {
                      stackIn_7_0 = 0;
                      break L2;
                    } else {
                      stackIn_7_0 = 1;
                      break L2;
                    }
                  }
                  var9 = stackIn_7_0;
                  var10 = param0;
                  L3: while (true) {
                    if (param1 <= var10) {
                      param2[param1] = param2[var6];
                      param2[var6] = var7;
                      param4[param1] = param4[var6];
                      param4[var6] = var8;
                      lr.a(param0, var6 + -1, param2, (byte) 69, param4);
                      lr.a(var6 + 1, param1, param2, (byte) 77, param4);
                      break L1;
                    } else {
                      if ((var7 - (var9 & var10) ^ -1) > (param2[var10] ^ -1)) {
                        var11 = param2[var10];
                        param2[var10] = param2[var6];
                        param2[var6] = var11;
                        var12 = param4[var10];
                        param4[var10] = param4[var6];
                        incrementValue$0 = var6;
                        var6++;
                        param4[incrementValue$0] = var12;
                        var10++;
                        continue L3;
                      } else {
                        var10++;
                        continue L3;
                      }
                    }
                  }
                }
              }
              decompiledRegionSelector0 = 1;
              break L0;
            } else {
              decompiledRegionSelector0 = 0;
              break L0;
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L4: {
            var5 = decompiledCaughtException;
            stackIn_16_0 = (RuntimeException) (var5);

            stackIn_16_1 = new StringBuilder().append("lr.A(").append(param0).append(',').append(param1).append(',');

            if (param2 == null) {
              stackIn_17_0 = (RuntimeException) ((Object) stackIn_16_0);
              stackIn_17_1 = (StringBuilder) ((Object) stackIn_16_1);
              stackIn_17_2 = "null";
              break L4;
            } else {
              stackIn_17_0 = (RuntimeException) ((Object) stackIn_16_0);
              stackIn_17_1 = (StringBuilder) ((Object) stackIn_16_1);
              stackIn_17_2 = "{...}";
              break L4;
            }
          }
          L5: {


            stackIn_19_1 = ((StringBuilder) (Object) stackIn_17_1).append(stackIn_17_2).append(',').append(param3).append(',');

            if (param4 == null) {
              stackIn_17_0 = (RuntimeException) ((Object) stackIn_17_0);
              stackIn_20_1 = (StringBuilder) ((Object) stackIn_19_1);
              stackIn_20_2 = "null";
              break L5;
            } else {
              stackIn_17_0 = (RuntimeException) ((Object) stackIn_17_0);
              stackIn_20_1 = (StringBuilder) ((Object) stackIn_19_1);
              stackIn_20_2 = "{...}";
              break L5;
            }
          }
          throw r.a((Throwable) ((Object) stackIn_17_0), stackIn_20_2 + ')');
        }
        if (decompiledRegionSelector0 == 0) {
          return;
        } else {
          return;
        }
    }

    final static String a(int param0, CharSequence[] param1) {
        RuntimeException var2 = null;
        String stackIn_2_0 = null;
        String stackIn_4_0 = null;
        RuntimeException stackIn_7_0 = null;
        StringBuilder stackIn_7_1 = null;
        RuntimeException stackIn_8_0 = null;
        StringBuilder stackIn_8_1 = null;
        String stackIn_8_2 = null;
        int decompiledRegionSelector0 = 0;
        RuntimeException decompiledCaughtException = null;
        try {
          L0: {
            if (param0 >= 63) {
              stackIn_4_0 = hm.a(param1, 0, param1.length, (byte) 28);
              decompiledRegionSelector0 = 1;
              break L0;
            } else {
              stackIn_2_0 = (String) null;
              decompiledRegionSelector0 = 0;
              break L0;
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L1: {
            var2 = decompiledCaughtException;
            stackIn_7_0 = (RuntimeException) (var2);

            stackIn_7_1 = new StringBuilder().append("lr.H(").append(param0).append(',');

            if (param1 == null) {
              stackIn_8_0 = (RuntimeException) ((Object) stackIn_7_0);
              stackIn_8_1 = (StringBuilder) ((Object) stackIn_7_1);
              stackIn_8_2 = "null";
              break L1;
            } else {
              stackIn_8_0 = (RuntimeException) ((Object) stackIn_7_0);
              stackIn_8_1 = (StringBuilder) ((Object) stackIn_7_1);
              stackIn_8_2 = "{...}";
              break L1;
            }
          }
          throw r.a((Throwable) ((Object) stackIn_8_0), stackIn_8_2 + ')');
        }
        if (decompiledRegionSelector0 == 0) {
          return stackIn_2_0;
        } else {
          return stackIn_4_0;
        }
    }

    final static int a(byte param0) {
        if (param0 != 27) {
            Object[] var2 = (Object[]) null;
            lr.a(-35, -18, (int[]) null, (byte) 95, (Object[]) null);
        }
        return tg.field_b.nextInt();
    }

    final synchronized static byte[] a(int param0, int param1) {
        int fieldTemp$3 = 0;
        int fieldTemp$4 = 0;
        int fieldTemp$5 = 0;
        int dupTemp$6 = 0;
        byte[][] arrayValue$7 = null;
        byte[] var2_ref_byte__;
        int var2;
        byte[] var3;
        L0: {
          if (-101 != (param0 ^ -1)) {
            break L0;
          } else {
            if (je.field_c <= 0) {
              break L0;
            } else {
              fieldTemp$3 = je.field_c - 1;
              je.field_c = je.field_c - 1;
              var2_ref_byte__ = qf.field_d[fieldTemp$3];
              qf.field_d[je.field_c] = null;
              return var2_ref_byte__;
            }
          }
        }
        L1: {
          if (param0 != 5000) {
            break L1;
          } else {
            if ((ik.field_j ^ -1) >= -1) {
              break L1;
            } else {
              fieldTemp$4 = ik.field_j - 1;
              ik.field_j = ik.field_j - 1;
              var2_ref_byte__ = mj.field_Bb[fieldTemp$4];
              mj.field_Bb[ik.field_j] = null;
              return var2_ref_byte__;
            }
          }
        }
        L2: {
          if (param0 != 30000) {
            break L2;
          } else {
            if (0 < en.field_a) {
              fieldTemp$5 = en.field_a - 1;
              en.field_a = en.field_a - 1;
              var2_ref_byte__ = jf.field_c[fieldTemp$5];
              jf.field_c[en.field_a] = null;
              return var2_ref_byte__;
            } else {
              break L2;
            }
          }
        }
        if (param1 > 67) {
          L3: {
            if (nn.field_c == null) {
              break L3;
            } else {
              var2 = 0;
              L4: while (true) {
                if (var2 >= nf.field_a.length) {
                  break L3;
                } else {
                  if (nf.field_a[var2] == param0) {
                    if (-1 > (tf.field_a[var2] ^ -1)) {
                      dupTemp$6 = tf.field_a[var2] - 1;
                      arrayValue$7 = nn.field_c[var2];
                      tf.field_a[var2] = dupTemp$6;
                      var3 = arrayValue$7[dupTemp$6];
                      nn.field_c[var2][tf.field_a[var2]] = null;
                      return var3;
                    } else {
                      var2++;
                      continue L4;
                    }
                  } else {
                    var2++;
                    continue L4;
                  }
                }
              }
            }
          }
          return new byte[param0];
        } else {
          return (byte[]) null;
        }
    }

    final static void a(int param0) {
        int decompiledRegionSelector0 = 0;
        RuntimeException decompiledCaughtException = null;
        RuntimeException var1 = null;
        int var2 = 0;
        int[] var3 = null;
        int[] var4 = null;
        sl var5 = null;
        int var6 = 0;
        int var7 = 0;
        int var8 = 0;
        Object[] var9 = null;
        int[] var10 = null;
        int[] var11 = null;
        lq var12 = null;
        sl var13 = null;
        int[] var14 = null;
        gb var15 = null;
        Object var16 = null;
        gb var16_ref = null;
        int[] var17 = null;
        var16 = null;
        var8 = ShatteredPlansClient.field_F ? 1 : 0;
        try {
          L0: {
            L1: {
              if (param0 >= 65) {
                break L1;
              } else {
                var9 = (Object[]) null;
                lr.a(121, -50, (int[]) null, (byte) -33, (Object[]) null);
                break L1;
              }
            }
            L2: {
              var13 = sa.field_a;
              var2 = var13.j(-100);
              if (var2 != 0) {
                if (var2 == 1) {
                  var12 = (lq) ((Object) rs.field_Bb.d(0));
                  if (var12 != null) {
                    var12.b((byte) -108);
                    break L2;
                  } else {
                    q.g(-100);
                    decompiledRegionSelector0 = 3;
                    break L0;
                  }
                } else {
                  if ((var2 ^ -1) == -3) {
                    var16_ref = (gb) ((Object) vl.field_o.d(0));
                    if (var16_ref == null) {
                      q.g(-120);
                      decompiledRegionSelector0 = 2;
                      break L0;
                    } else {
                      var16_ref.field_k = ae.a(-116);
                      var16_ref.field_h = var16_ref.field_k[0];
                      var16_ref.field_j = true;
                      var16_ref.field_l = true;
                      var16_ref.b((byte) -89);
                      break L2;
                    }
                  } else {
                    jq.a(-29901, (Throwable) null, "A1: " + w.a(16738));
                    q.g(-125);
                    break L2;
                  }
                }
              } else {
                var11 = ae.a(-128);
                var17 = var11;
                var14 = var17;
                var3 = var14;
                var10 = var11;
                var4 = var10;
                var5 = var13;
                var6 = ((ob) ((Object) var5)).j(-115);
                var7 = 0;
                L3: while (true) {
                  if (var7 >= var6) {
                    var15 = (gb) ((Object) vl.field_o.d(0));
                    if (var15 == null) {
                      q.g(-112);
                      decompiledRegionSelector0 = 0;
                      break L0;
                    } else {
                      var15.field_l = false;
                      var15.field_h = var17[0];
                      var15.field_j = true;
                      var15.field_k = var3;
                      var15.b((byte) -105);
                      break L2;
                    }
                  } else {
                    var10[var7] = ((ob) ((Object) var5)).a(16711680);
                    var7++;
                    continue L3;
                  }
                }
              }
            }
            decompiledRegionSelector0 = 1;
            break L0;
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          var1 = decompiledCaughtException;
          throw r.a((Throwable) ((Object) var1), "lr.G(" + param0 + ')');
        }
        if (decompiledRegionSelector0 == 0) {
          return;
        } else {
          if (decompiledRegionSelector0 == 1) {
            return;
          } else {
            if (decompiledRegionSelector0 == 2) {
              return;
            } else {
              return;
            }
          }
        }
    }

    final static void a(int param0, int param1, int param2, int param3, int param4, int[] param5, int param6, int param7, int param8, int param9, int param10, int param11, int param12, int param13, int param14, int param15, int param16) {
        int var17_int = 0;
        int var18 = 0;
        int var19 = 0;
        int var20 = 0;
        int var21 = 0;
        int var22 = 0;
        int var23 = 0;
        int var24 = 0;
        int var25 = 0;
        int var26 = 0;
        int var27 = 0;
        int var28 = 0;
        int var29 = 0;
        int var30 = 0;
        int var31 = 0;
        int var32 = 0;
        int var33 = 0;
        int var34 = 0;
        int var35 = 0;
        int var36 = 0;
        int var42 = 0;
        RuntimeException stackIn_79_0 = null;
        StringBuilder stackIn_79_1 = null;
        RuntimeException stackIn_80_0 = null;
        StringBuilder stackIn_80_1 = null;
        String stackIn_80_2 = null;
        int decompiledRegionSelector0 = 0;
        Throwable caughtException = null;
        RuntimeException decompiledCaughtException = null;
        RuntimeException var17 = null;
        int var37 = 0;
        int var38 = 0;
        int var39 = 0;
        int var40 = 0;
        int var41 = 0;
        var42 = ShatteredPlansClient.field_F ? 1 : 0;
        try {
          L0: {
            L1: {
              if (param8 < 0) {
                break L1;
              } else {
                if (param4 >= wn.field_e) {
                  break L1;
                } else {
                  L2: {
                    if (0 <= param3) {
                      break L2;
                    } else {
                      if ((param0 ^ -1) <= -1) {
                        break L2;
                      } else {
                        if (-1 >= (param14 ^ -1)) {
                          break L2;
                        } else {
                          decompiledRegionSelector0 = 1;
                          break L0;
                        }
                      }
                    }
                  }
                  L3: {
                    if (wn.field_c > param3) {
                      break L3;
                    } else {
                      if (wn.field_c > param0) {
                        break L3;
                      } else {
                        if (param14 >= wn.field_c) {
                          decompiledRegionSelector0 = 2;
                          break L0;
                        } else {
                          break L3;
                        }
                      }
                    }
                  }
                  if (param2 == -31232) {
                    L4: {
                      var34 = -param4 + param8;
                      if (param15 == param4) {
                        L5: {
                          if (param8 == param4) {
                            var27 = 0;
                            var32 = 0;
                            var18 = param0 << -190811280;
                            var29 = param10;
                            var23 = 0;
                            var17_int = param3 << -770714608;
                            var24 = 0;
                            var19 = 0;
                            var21 = param9;
                            var25 = param16;
                            var28 = 0;
                            var26 = param11;
                            var31 = 0;
                            var30 = param1;
                            var20 = 0;
                            var22 = param7;
                            break L5;
                          } else {
                            var35 = -param15 + param8;
                            if (param0 > param3) {
                              var23 = (-param9 + param6 << 92859536) / var34;
                              var32 = (-param1 + param13 << -1648936240) / var35;
                              var27 = (param12 + -param16 << -1201981872) / var34;
                              var29 = param10 << -570759568;
                              var30 = param1 << 2106587664;
                              var22 = param7 << 811540080;
                              var31 = (param13 - param10 << 1927064720) / var34;
                              var24 = (param6 + -param7 << -976621360) / var35;
                              var28 = (param12 - param11 << 150192272) / var35;
                              var26 = param11 << 466860176;
                              var25 = param16 << 1786464816;
                              var18 = param0 << 859123344;
                              var17_int = param3 << -965133200;
                              var21 = param9 << -886235056;
                              var19 = (param14 - param3 << -1829111088) / var34;
                              var20 = (param14 - param0 << -1492975632) / var35;
                              break L5;
                            } else {
                              var28 = (param12 + -param16 << 1924148528) / var34;
                              var30 = param10 << -351698576;
                              var18 = param3 << 864510064;
                              var29 = param1 << -1193920624;
                              var31 = (param13 - param1 << -1491639984) / var35;
                              var17_int = param0 << 1412710672;
                              var20 = (-param3 + param14 << -758043376) / var34;
                              var32 = (-param10 + param13 << -612022160) / var34;
                              var25 = param11 << -1141956144;
                              var27 = (-param11 + param12 << -657804976) / var35;
                              var21 = param7 << 552453296;
                              var24 = (param6 + -param9 << -1763552880) / var34;
                              var19 = (param14 - param0 << 988600304) / var35;
                              var22 = param9 << 108528016;
                              var26 = param16 << -397572304;
                              var23 = (param6 - param7 << 101507280) / var35;
                              break L5;
                            }
                          }
                        }
                        L6: {
                          if (-1 < (param4 ^ -1)) {
                            param4 = Math.min(-param4, param15 + -param4);
                            var22 = var22 + var24 * param4;
                            var17_int = var17_int + var19 * param4;
                            var25 = var25 + var27 * param4;
                            var18 = var18 + param4 * var20;
                            var29 = var29 + var31 * param4;
                            var30 = var30 + param4 * var32;
                            var26 = var26 + param4 * var28;
                            var21 = var21 + var23 * param4;
                            param4 = 0;
                            break L6;
                          } else {
                            break L6;
                          }
                        }
                        var33 = 0;
                        break L4;
                      } else {
                        L7: {
                          var26 = param16 << 1538891888;
                          var25 = param16 << 1538891888;
                          var18 = param3 << -1239184752;
                          var17_int = param3 << -1239184752;
                          var22 = param9 << 1948944432;
                          var21 = param9 << 1948944432;
                          var30 = param10 << 2031090768;
                          var29 = param10 << 2031090768;
                          var35 = param15 + -param4;
                          var20 = (param14 - param3 << 1515881296) / var34;
                          var19 = (-param3 + param0 << -1046664656) / var35;
                          if (var20 > var19) {
                            var27 = (param11 - param16 << 757364816) / var35;
                            var32 = (param13 - param10 << 1808751536) / var34;
                            var28 = (-param16 + param12 << 894898416) / var34;
                            var31 = (-param10 + param1 << -1025944528) / var35;
                            var33 = 0;
                            var23 = (param7 - param9 << 1217619056) / var35;
                            var24 = (-param9 + param6 << -1778608944) / var34;
                            break L7;
                          } else {
                            var36 = var19;
                            var19 = var20;
                            var20 = var36;
                            var27 = (param12 + -param16 << -1394305648) / var34;
                            var32 = (param1 + -param10 << -2017114128) / var35;
                            var24 = (-param9 + param7 << -1262119760) / var35;
                            var31 = (param13 + -param10 << -495556016) / var34;
                            var23 = (-param9 + param6 << 132799984) / var34;
                            var33 = 1;
                            var28 = (param11 - param16 << 108127568) / var35;
                            break L7;
                          }
                        }
                        L8: {
                          L9: {
                            if (param4 < 0) {
                              if (-1 >= (param15 ^ -1)) {
                                param4 = -param4;
                                var29 = var29 + param4 * var31;
                                var26 = var26 + var28 * param4;
                                var21 = var21 + param4 * var23;
                                var25 = var25 + param4 * var27;
                                var22 = var22 + param4 * var24;
                                var18 = var18 + param4 * var20;
                                var30 = var30 + var32 * param4;
                                var17_int = var17_int + var19 * param4;
                                param4 = 0;
                                break L9;
                              } else {
                                param4 = param15 - param4;
                                var17_int = var17_int + var19 * param4;
                                var30 = var30 + param4 * var32;
                                var29 = var29 + param4 * var31;
                                var22 = var22 + param4 * var24;
                                var26 = var26 + var28 * param4;
                                var18 = var18 + param4 * var20;
                                var21 = var21 + var23 * param4;
                                var25 = var25 + var27 * param4;
                                param4 = param15;
                                break L8;
                              }
                            } else {
                              break L9;
                            }
                          }
                          var36 = wn.field_h[param4];
                          L10: while (true) {
                            if (param4 >= param15) {
                              break L8;
                            } else {
                              L11: {
                                var37 = var17_int >> -1619936848;
                                if ((var37 ^ -1) <= (wn.field_c ^ -1)) {
                                  break L11;
                                } else {
                                  var38 = (var18 >> 1138101616) + -(var17_int >> -833669936);
                                  if (var38 == 0) {
                                    if (-1 < (var37 ^ -1)) {
                                      break L11;
                                    } else {
                                      if (var37 >= wn.field_c) {
                                        break L11;
                                      } else {
                                        jg.a(var38, var29, var37 - -var36, 0, var25, false, var21, 0, param5, 0);
                                        break L11;
                                      }
                                    }
                                  } else {
                                    L12: {
                                      var39 = (var22 - var21) / var38;
                                      var40 = (var26 - var25) / var38;
                                      var41 = (var30 + -var29) / var38;
                                      if (wn.field_c <= var38 + var37) {
                                        var38 = -1 + wn.field_c + -var37;
                                        break L12;
                                      } else {
                                        break L12;
                                      }
                                    }
                                    if (0 > var37) {
                                      jg.a(var38 + var37, var29 - var37 * var41, var36, var40, -(var40 * var37) + var25, false, -(var37 * var39) + var21, var41, param5, var39);
                                      break L11;
                                    } else {
                                      jg.a(var38, var29, var37 - -var36, var40, var25, false, var21, var41, param5, var39);
                                      break L11;
                                    }
                                  }
                                }
                              }
                              param4++;
                              if (wn.field_e <= param4) {
                                decompiledRegionSelector0 = 4;
                                break L0;
                              } else {
                                var21 = var21 + var23;
                                var29 = var29 + var31;
                                var17_int = var17_int + var19;
                                var18 = var18 + var20;
                                var22 = var22 + var24;
                                var25 = var25 + var27;
                                var36 = var36 + gf.field_b;
                                var26 = var26 + var28;
                                var30 = var30 + var32;
                                continue L10;
                              }
                            }
                          }
                        }
                        var36 = -param15 + param8;
                        if (0 != var36) {
                          L13: {
                            var37 = param14 << 1357667888;
                            var38 = param6 << -1666279632;
                            var39 = param12 << -1062583760;
                            if (var33 != 0) {
                              var30 = param1 << 2049596272;
                              var22 = param7 << -599495536;
                              var26 = param11 << 1200128656;
                              var18 = param0 << 226179920;
                              break L13;
                            } else {
                              var21 = param7 << 402861456;
                              var29 = param1 << 1789477520;
                              var17_int = param0 << -566761552;
                              var25 = param11 << 1538363568;
                              break L13;
                            }
                          }
                          var40 = param13 << -323385104;
                          var19 = (var37 - var17_int) / var36;
                          var27 = (-var25 + var39) / var36;
                          var23 = (-var21 + var38) / var36;
                          var28 = (-var26 + var39) / var36;
                          var31 = (var40 + -var29) / var36;
                          var32 = (-var30 + var40) / var36;
                          var24 = (-var22 + var38) / var36;
                          var20 = (-var18 + var37) / var36;
                          break L4;
                        } else {
                          var19 = 0;
                          var28 = 0;
                          var31 = 0;
                          var23 = 0;
                          var27 = 0;
                          var24 = 0;
                          var20 = 0;
                          var32 = 0;
                          break L4;
                        }
                      }
                    }
                    L14: {
                      if (param4 >= 0) {
                        break L14;
                      } else {
                        param4 = -param4;
                        var26 = var26 + param4 * var28;
                        var30 = var30 + param4 * var32;
                        var29 = var29 + param4 * var31;
                        var18 = var18 + var20 * param4;
                        var25 = var25 + param4 * var27;
                        var22 = var22 + var24 * param4;
                        var17_int = var17_int + var19 * param4;
                        var21 = var21 + param4 * var23;
                        param4 = 0;
                        break L14;
                      }
                    }
                    var35 = wn.field_h[param4];
                    L15: while (true) {
                      if (param4 >= param8) {
                        decompiledRegionSelector0 = 5;
                        break L0;
                      } else {
                        L16: {
                          var36 = var17_int >> -1938759440;
                          if (var36 >= wn.field_c) {
                            break L16;
                          } else {
                            var37 = -(var17_int >> 472276720) + (var18 >> 882296368);
                            if (-1 == (var37 ^ -1)) {
                              if (var36 < 0) {
                                break L16;
                              } else {
                                if (wn.field_c <= var36) {
                                  break L16;
                                } else {
                                  jg.a(var37, var29, var36 + var35, 0, var25, false, var21, 0, param5, 0);
                                  break L16;
                                }
                              }
                            } else {
                              L17: {
                                var38 = (-var21 + var22) / var37;
                                var39 = (-var25 + var26) / var37;
                                var40 = (-var29 + var30) / var37;
                                if (var37 + var36 < wn.field_c) {
                                  break L17;
                                } else {
                                  var37 = -var36 + (wn.field_c + -1);
                                  break L17;
                                }
                              }
                              if (-1 >= (var36 ^ -1)) {
                                jg.a(var37, var29, var36 - -var35, var39, var25, false, var21, var40, param5, var38);
                                break L16;
                              } else {
                                jg.a(var37 + var36, -(var36 * var40) + var29, var35, var39, var25 - var36 * var39, false, var21 - var36 * var38, var40, param5, var38);
                                break L16;
                              }
                            }
                          }
                        }
                        param4++;
                        if (param4 >= wn.field_e) {
                          decompiledRegionSelector0 = 6;
                          break L0;
                        } else {
                          var25 = var25 + var27;
                          var21 = var21 + var23;
                          var26 = var26 + var28;
                          var35 = var35 + gf.field_b;
                          var22 = var22 + var24;
                          var17_int = var17_int + var19;
                          var18 = var18 + var20;
                          var29 = var29 + var31;
                          var30 = var30 + var32;
                          continue L15;
                        }
                      }
                    }
                  } else {
                    decompiledRegionSelector0 = 3;
                    break L0;
                  }
                }
              }
            }
            decompiledRegionSelector0 = 0;
            break L0;
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L18: {
            var17 = decompiledCaughtException;
            stackIn_79_0 = (RuntimeException) (var17);

            stackIn_79_1 = new StringBuilder().append("lr.F(").append(param0).append(',').append(param1).append(',').append(param2).append(',').append(param3).append(',').append(param4).append(',');

            if (param5 == null) {
              stackIn_80_0 = (RuntimeException) ((Object) stackIn_79_0);
              stackIn_80_1 = (StringBuilder) ((Object) stackIn_79_1);
              stackIn_80_2 = "null";
              break L18;
            } else {
              stackIn_80_0 = (RuntimeException) ((Object) stackIn_79_0);
              stackIn_80_1 = (StringBuilder) ((Object) stackIn_79_1);
              stackIn_80_2 = "{...}";
              break L18;
            }
          }
          throw r.a((Throwable) ((Object) stackIn_80_0), stackIn_80_2 + ',' + param6 + ',' + param7 + ',' + param8 + ',' + param9 + ',' + param10 + ',' + param11 + ',' + param12 + ',' + param13 + ',' + param14 + ',' + param15 + ',' + param16 + ')');
        }
        if (decompiledRegionSelector0 == 0) {
          return;
        } else {
          if (decompiledRegionSelector0 == 1) {
            return;
          } else {
            if (decompiledRegionSelector0 == 2) {
              return;
            } else {
              if (decompiledRegionSelector0 == 3) {
                return;
              } else {
                if (decompiledRegionSelector0 == 4) {
                  return;
                } else {
                  if (decompiledRegionSelector0 == 5) {
                    return;
                  } else {
                    return;
                  }
                }
              }
            }
          }
        }
    }

    static {
        field_f = "End turn";
        field_d = "That name is not available";
        field_b = new int[]{-1, -1, 2, -1, -1, -1, 2, 3, 0, -1, 3, -1, 1, -1, -1, -1, -1, -1, 3, 3, 0, 2};
        field_e = "Rankings";
    }
}
