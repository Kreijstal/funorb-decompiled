/*
 * Decompiled by CFR-JS 0.4.0.
 */
class qr extends hm {
    static nk[] field_A;
    int field_I;
    private int field_E;
    boolean field_s;
    private int field_X;
    int field_N;
    bi field_Z;
    private int field_D;
    int field_x;
    int field_V;
    bi field_ob;
    int field_w;
    int field_kb;
    int field_t;
    int field_hb;
    int field_y;
    bi field_pb;
    int field_ub;
    int field_Y;
    private int field_tb;
    boolean field_r;
    int field_G;
    bi field_U;
    nq field_nb;
    int field_T;
    bi[] field_lb;
    bi field_fb;
    pf field_M;
    bi[] field_vb;
    int field_J;
    int field_W;
    String field_u;
    int field_jb;
    int field_gb;
    int field_rb;
    private int field_O;
    private boolean field_ib;
    private boolean field_H;
    private int field_cb;
    private int field_S;
    bi[] field_wb;
    int field_qb;
    int field_K;
    bi[] field_F;
    boolean field_P;
    bi[] field_sb;
    int field_B;
    int field_Q;
    static String field_R;
    private int field_eb;
    boolean field_bb;
    private boolean field_C;
    boolean field_v;
    String field_z;
    int field_db;
    int field_mb;
    int field_ab;
    boolean field_L;

    final static String a(String param0, byte param1, String param2, String param3) {
        StringBuilder discarded$0 = null;
        StringBuilder discarded$1 = null;
        StringBuilder discarded$2 = null;
        String stackIn_14_0 = null;
        RuntimeException stackIn_17_0 = null;
        StringBuilder stackIn_17_1 = null;
        RuntimeException stackIn_18_0 = null;
        StringBuilder stackIn_18_1 = null;
        String stackIn_18_2 = null;
        StringBuilder stackIn_20_1 = null;
        StringBuilder stackIn_21_1 = null;
        String stackIn_21_2 = null;
        StringBuilder stackIn_23_1 = null;
        StringBuilder stackIn_24_1 = null;
        String stackIn_24_2 = null;
        RuntimeException decompiledCaughtException = null;
        int var4_int = 0;
        RuntimeException var4 = null;
        int var5 = 0;
        int var6 = 0;
        int var7 = 0;
        int var8 = 0;
        int var9_int = 0;
        StringBuilder var9 = null;
        int var10 = 0;
        int var11 = 0;
        int var12 = 0;
        var12 = ShatteredPlansClient.field_F ? 1 : 0;
        try {
          L0: {
            L1: {
              var4_int = param0.length();
              if (param1 < -15) {
                break L1;
              } else {
                field_A = (nk[]) null;
                break L1;
              }
            }
            var5 = param3.length();
            var6 = param2.length();
            if (-1 != (var5 ^ -1)) {
              L2: {
                var7 = var4_int;
                var8 = -var5 + var6;
                if (0 != var8) {
                  var9_int = 0;
                  L3: while (true) {
                    var9_int = param0.indexOf(param3, var9_int);
                    if (var9_int < 0) {
                      break L2;
                    } else {
                      var7 = var7 + var8;
                      var9_int = var9_int + var5;
                      continue L3;
                    }
                  }
                } else {
                  break L2;
                }
              }
              var9 = new StringBuilder(var7);
              var10 = 0;
              L4: while (true) {
                var11 = param0.indexOf(param3, var10);
                if ((var11 ^ -1) > -1) {
                  discarded$0 = var9.append(param0.substring(var10));
                  stackIn_14_0 = var9.toString();
                  break L0;
                } else {
                  discarded$1 = var9.append(param0.substring(var10, var11));
                  discarded$2 = var9.append(param2);
                  var10 = var11 - -var5;
                  continue L4;
                }
              }
            } else {
              throw new IllegalArgumentException("Key cannot have zero length");
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L5: {
            var4 = decompiledCaughtException;
            stackIn_17_0 = (RuntimeException) (var4);

            stackIn_17_1 = new StringBuilder().append("qr.L(");

            if (param0 == null) {
              stackIn_18_0 = (RuntimeException) ((Object) stackIn_17_0);
              stackIn_18_1 = (StringBuilder) ((Object) stackIn_17_1);
              stackIn_18_2 = "null";
              break L5;
            } else {
              stackIn_18_0 = (RuntimeException) ((Object) stackIn_17_0);
              stackIn_18_1 = (StringBuilder) ((Object) stackIn_17_1);
              stackIn_18_2 = "{...}";
              break L5;
            }
          }
          L6: {


            stackIn_20_1 = ((StringBuilder) (Object) stackIn_18_1).append(stackIn_18_2).append(',').append(param1).append(',');

            if (param2 == null) {
              stackIn_18_0 = (RuntimeException) ((Object) stackIn_18_0);
              stackIn_21_1 = (StringBuilder) ((Object) stackIn_20_1);
              stackIn_21_2 = "null";
              break L6;
            } else {
              stackIn_18_0 = (RuntimeException) ((Object) stackIn_18_0);
              stackIn_21_1 = (StringBuilder) ((Object) stackIn_20_1);
              stackIn_21_2 = "{...}";
              break L6;
            }
          }
          L7: {


            stackIn_23_1 = ((StringBuilder) (Object) stackIn_21_1).append(stackIn_21_2).append(',');

            if (param3 == null) {
              stackIn_18_0 = (RuntimeException) ((Object) stackIn_18_0);
              stackIn_24_1 = (StringBuilder) ((Object) stackIn_23_1);
              stackIn_24_2 = "null";
              break L7;
            } else {
              stackIn_18_0 = (RuntimeException) ((Object) stackIn_18_0);
              stackIn_24_1 = (StringBuilder) ((Object) stackIn_23_1);
              stackIn_24_2 = "{...}";
              break L7;
            }
          }
          throw r.a((Throwable) ((Object) stackIn_18_0), stackIn_24_2 + ')');
        }
        return stackIn_14_0;
    }

    final static dc a(int param0, int param1, String[] param2, vr param3, int param4, int param5) {
        try {
            dc stackIn_17_0 = null;
            RuntimeException stackIn_22_0 = null;
            StringBuilder stackIn_22_1 = null;
            RuntimeException stackIn_23_0 = null;
            StringBuilder stackIn_23_1 = null;
            String stackIn_23_2 = null;
            StringBuilder stackIn_25_1 = null;
            StringBuilder stackIn_26_1 = null;
            String stackIn_26_2 = null;
            int decompiledRegionSelector0 = 0;
            Throwable decompiledCaughtException = null;
            RuntimeException var6 = null;
            int var7 = 0;
            int var8 = 0;
            int var9 = 0;
            int var10_int = 0;
            jn var10 = null;
            int var11 = 0;
            int var12 = 0;
            int var13 = 0;
            int var14 = 0;
            dc var15 = null;
            hs var16 = null;
            vl var17 = null;
            pc var18 = null;
            var14 = ShatteredPlansClient.field_F ? 1 : 0;
            try {
              L0: {
                L1: {
                  var15 = new dc(param0, param3, param4, param2);
                  var7 = param2.length;
                  var8 = var7 * md.field_J[param5];
                  if (param1 == 50) {
                    break L1;
                  } else {
                    qr.a(false, (byte) 41);
                    break L1;
                  }
                }
                var9 = 0;
                L2: while (true) {
                  if (50 > var9) {
                    try {
                      L3: {
                        L4: {
                          L5: {
                            var10_int = lr.a((byte) 27);
                            var11 = var8 * 100 / 45;
                            var12 = (int)Math.sqrt((double)var11);
                            var13 = param4;
                            if (var13 != 0) {
                              if ((var13 ^ -1) == -3) {
                                break L5;
                              } else {
                                if (1 == var13) {
                                  var17 = new vl(var10_int, var15.field_v.length);
                                  var15.field_g = var17.a((byte) -128);
                                  break L4;
                                } else {
                                  if ((var13 ^ -1) != -4) {
                                    throw new RuntimeException("Cannot generate map for unknown gametype " + param4);
                                  } else {
                                    var18 = new pc(var12, var12, var10_int, var8, var15.field_v.length);
                                    var15.field_g = var18.a((byte) -127);
                                    break L4;
                                  }
                                }
                              }
                            } else {
                              break L5;
                            }
                          }
                          var16 = new hs(var12, var12, var10_int, var8, var15.field_v.length);
                          var15.field_g = var16.a((byte) -126);
                          break L4;
                        }
                        var15.field_g.a(var15.field_v, param3, param1 + -46);
                        var15.e(param1 + -50);
                        var15.b(param1 + -151);
                        stackIn_17_0 = (dc) (var15);
                        decompiledRegionSelector0 = 0;
                        break L3;
                      }
                    } catch (jn decompiledCaughtParameter0) {
                      decompiledCaughtException = decompiledCaughtParameter0;
                      L6: {
                        var10 = (jn) (Object) decompiledCaughtException;
                        var9++;
                        decompiledRegionSelector0 = 1;
                        break L6;
                      }
                    }
                    if (decompiledRegionSelector0 == 0) {
                      break L0;
                    } else {
                      continue L2;
                    }
                  } else {
                    return null;
                  }
                }
              }
            } catch (java.lang.RuntimeException decompiledCaughtParameter1) {
              decompiledCaughtException = decompiledCaughtParameter1;
              L7: {
                var6 = (RuntimeException) (Object) decompiledCaughtException;
                stackIn_22_0 = (RuntimeException) (var6);

                stackIn_22_1 = new StringBuilder().append("qr.R(").append(param0).append(',').append(param1).append(',');

                if (param2 == null) {
                  stackIn_23_0 = (RuntimeException) ((Object) stackIn_22_0);
                  stackIn_23_1 = (StringBuilder) ((Object) stackIn_22_1);
                  stackIn_23_2 = "null";
                  break L7;
                } else {
                  stackIn_23_0 = (RuntimeException) ((Object) stackIn_22_0);
                  stackIn_23_1 = (StringBuilder) ((Object) stackIn_22_1);
                  stackIn_23_2 = "{...}";
                  break L7;
                }
              }
              L8: {


                stackIn_25_1 = ((StringBuilder) (Object) stackIn_23_1).append(stackIn_23_2).append(',');

                if (param3 == null) {
                  stackIn_23_0 = (RuntimeException) ((Object) stackIn_23_0);
                  stackIn_26_1 = (StringBuilder) ((Object) stackIn_25_1);
                  stackIn_26_2 = "null";
                  break L8;
                } else {
                  stackIn_23_0 = (RuntimeException) ((Object) stackIn_23_0);
                  stackIn_26_1 = (StringBuilder) ((Object) stackIn_25_1);
                  stackIn_26_2 = "{...}";
                  break L8;
                }
              }
              throw r.a((Throwable) ((Object) stackIn_23_0), stackIn_26_2 + ',' + param4 + ',' + param5 + ')');
            }
            return stackIn_17_0;
        } catch (RuntimeException | Error decompiledUncheckedException) {
            throw decompiledUncheckedException;
        } catch (Throwable decompiledCheckedException) {
            throw new RuntimeException(decompiledCheckedException);
        }
    }

    final int a(int param0, byte param1) {
        qr var4 = null;
        int var5 = 0;
        int var4_int = 0;
        int var6 = ShatteredPlansClient.field_F ? 1 : 0;
        int var3 = 0;
        if (null != this.field_z && this.field_nb != null) {
            var4_int = this.field_nb.c(this.field_z, param0);
            if (var4_int > var3) {
                var3 = var4_int;
            }
        }
        if (null != this.field_pb) {
            var4_int = this.field_pb.field_o;
            if (var4_int > var3) {
                var3 = var4_int;
            }
        }
        if (param1 >= -56) {
            this.a(false, false, true, 31, 106, -74, false, -100, false, false, -72);
        }
        if (!(null == this.field_M)) {
            var4 = (qr) ((Object) this.field_M.d(0));
            while (var4 != null) {
                var5 = var4.field_hb + var4.field_K;
                if (!(var3 >= var5)) {
                    var3 = var5;
                }
                var4 = (qr) ((Object) this.field_M.a((byte) -71));
            }
        }
        return var3;
    }

    final void a(int param0, qr param1) {
        if (!(null != this.field_M)) {
            this.field_M = new pf();
        }
        if (param0 != 4) {
            return;
        }
        try {
            this.field_M.a((byte) -113, param1);
        } catch (RuntimeException runtimeException) {
            throw r.a((Throwable) ((Object) runtimeException), "qr.DA(" + param0 + ',' + (param1 != null ? "{...}" : "null") + ')');
        }
    }

    final void a(int param0, qr param1, qr param2, byte param3) {
        RuntimeException stackIn_8_0 = null;
        StringBuilder stackIn_8_1 = null;
        RuntimeException stackIn_9_0 = null;
        StringBuilder stackIn_9_1 = null;
        String stackIn_9_2 = null;
        StringBuilder stackIn_11_1 = null;
        StringBuilder stackIn_12_1 = null;
        String stackIn_12_2 = null;
        RuntimeException decompiledCaughtException = null;
        RuntimeException var5 = null;
        try {
          L0: {
            L1: {
              if (param1 == null) {
                param2.field_w = 0;
                param2.field_db = 0;
                break L1;
              } else {
                param2.field_db = param1.field_mb + (param1.field_db - -param0);
                param2.field_w = param1.field_w - -param1.field_gb;
                break L1;
              }
            }
            L2: {
              if (param3 >= 38) {
                break L2;
              } else {
                this.field_s = false;
                break L2;
              }
            }
            break L0;
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L3: {
            var5 = decompiledCaughtException;
            stackIn_8_0 = (RuntimeException) (var5);

            stackIn_8_1 = new StringBuilder().append("qr.O(").append(param0).append(',');

            if (param1 == null) {
              stackIn_9_0 = (RuntimeException) ((Object) stackIn_8_0);
              stackIn_9_1 = (StringBuilder) ((Object) stackIn_8_1);
              stackIn_9_2 = "null";
              break L3;
            } else {
              stackIn_9_0 = (RuntimeException) ((Object) stackIn_8_0);
              stackIn_9_1 = (StringBuilder) ((Object) stackIn_8_1);
              stackIn_9_2 = "{...}";
              break L3;
            }
          }
          L4: {


            stackIn_11_1 = ((StringBuilder) (Object) stackIn_9_1).append(stackIn_9_2).append(',');

            if (param2 == null) {
              stackIn_9_0 = (RuntimeException) ((Object) stackIn_9_0);
              stackIn_12_1 = (StringBuilder) ((Object) stackIn_11_1);
              stackIn_12_2 = "null";
              break L4;
            } else {
              stackIn_9_0 = (RuntimeException) ((Object) stackIn_9_0);
              stackIn_12_1 = (StringBuilder) ((Object) stackIn_11_1);
              stackIn_12_2 = "{...}";
              break L4;
            }
          }
          throw r.a((Throwable) ((Object) stackIn_9_0), stackIn_12_2 + ',' + param3 + ')');
        }
    }

    final void b(boolean param0, int param1) {
        this.a(false, true, false, 0, 0, -2147483648, false, gf.field_k, false, param0, gf.field_b);
        int var3 = 114 % ((param1 - 25) / 62);
    }

    final static void a(boolean param0, byte param1) {
        jb.a(param0, (byte) -27);
        if (param1 <= 107) {
            vr var3 = (vr) null;
            qr.a(46, -4, (String[]) null, (vr) null, -66, -4);
        }
        mj.a(0, param0);
    }

    final void a(int param0, boolean param1, boolean param2, int param3) {
        int var5;
        int var6;
        int var7;
        int var8;
        int var9;
        qr var10_ref_qr;
        int var10;
        int var11;
        L0: {
          var11 = ShatteredPlansClient.field_F ? 1 : 0;
          var5 = -this.field_db;
          var6 = var5 + -this.field_w;
          var7 = this.field_mb;
          var8 = var7 - -this.field_gb;
          if (param2) {
            break L0;
          } else {
            var9 = 0;
            var10_ref_qr = (qr) ((Object) this.field_M.d(0));
            L1: while (true) {
              if (var10_ref_qr == null) {
                var8 = -param3 + var9;
                break L0;
              } else {
                var10_ref_qr.field_w = -var10_ref_qr.field_db + var9;
                var9 = var9 + (var10_ref_qr.field_mb - -var10_ref_qr.field_gb - -param3);
                var10_ref_qr = (qr) ((Object) this.field_M.a((byte) -71));
                continue L1;
              }
            }
          }
        }
        var9 = var8 - param0;
        if (!param1) {
          return;
        } else {
          L2: {
            if (var6 <= var9) {
              break L2;
            } else {
              var6 = var9;
              break L2;
            }
          }
          L3: {
            if ((var6 ^ -1) > -1) {
              var6 = 0;
              break L3;
            } else {
              break L3;
            }
          }
          L4: {
            var10 = -param0 + var7;
            if (var5 > var10) {
              var5 = var10;
              break L4;
            } else {
              break L4;
            }
          }
          L5: {
            if (-1 >= (var5 ^ -1)) {
              break L5;
            } else {
              var5 = 0;
              break L5;
            }
          }
          this.field_mb = var7;
          this.field_w = var5 + -var6;
          this.field_db = -var5;
          this.field_gb = -var7 + var8;
          return;
        }
    }

    final void e(byte param0) {
        qr var4 = null;
        int var3 = ShatteredPlansClient.field_F ? 1 : 0;
        if (!(this.field_M == null)) {
            var4 = (qr) ((Object) this.field_M.f(3725));
            while (var4 != null) {
                var4.field_H = true;
                var4.e((byte) -123);
                var4 = (qr) ((Object) this.field_M.b(-125));
            }
        }
        if (param0 >= -122) {
            this.field_pb = (bi) null;
        }
    }

    private final void a(int param0, int param1, int param2, boolean param3, int param4, int param5) {
        Object stackIn_33_0 = null;
        Object stackIn_34_0 = null;
        int stackIn_34_1 = 0;
        Object stackIn_38_0 = null;
        Object stackIn_39_0 = null;
        int stackIn_39_1 = 0;
        int var7;
        int var8;
        int var9;
        int var10;
        int var11;
        int var12;
        int var13;
        qr var14;
        int var15;
        L0: {
          var15 = ShatteredPlansClient.field_F ? 1 : 0;
          if (0 >= this.field_cb) {
            var7 = this.field_cb >> -713030942;
            break L0;
          } else {
            var7 = -(-this.field_cb >> -1856382302);
            break L0;
          }
        }
        L1: {
          this.field_cb = this.field_cb - var7;
          this.field_hb = this.field_hb + var7;
          if ((this.field_w ^ -1) < -1) {
            var7 = -(-this.field_w >> -695197470);
            break L1;
          } else {
            var7 = this.field_w >> 1441558562;
            break L1;
          }
        }
        L2: {
          this.field_db = this.field_db + var7;
          this.field_w = this.field_w - var7;
          var8 = 92 / ((-17 - param4) / 43);
          if (0 < this.field_S) {
            var7 = -(-this.field_S >> -1573206430);
            break L2;
          } else {
            var7 = this.field_S >> 1086523490;
            break L2;
          }
        }
        L3: {
          this.field_K = this.field_K + var7;
          this.field_S = this.field_S - var7;
          if (0 < this.field_gb) {
            var7 = -(-this.field_gb >> 1184728578);
            break L3;
          } else {
            var7 = this.field_gb >> -1535081822;
            break L3;
          }
        }
        L4: {
          L5: {
            this.field_gb = this.field_gb - var7;
            param3 = param3 & this.field_s;
            this.field_B = param0 + this.field_hb;
            this.field_mb = this.field_mb + var7;
            this.field_N = this.field_db + param1;
            var9 = gf.field_i;
            var10 = gf.field_a;
            var11 = gf.field_l;
            var12 = gf.field_f;
            gf.i(this.field_B, this.field_N, this.field_B - -this.field_K, this.field_N - -this.field_mb);
            var13 = 0;
            if (us.field_c) {
              break L5;
            } else {
              if (!param3) {
                break L5;
              } else {
                if (je.field_d == 0) {
                  break L5;
                } else {
                  if (gf.field_i > nh.field_p) {
                    break L5;
                  } else {
                    if (gf.field_l <= nh.field_p) {
                      break L5;
                    } else {
                      if (td.field_P < gf.field_a) {
                        break L5;
                      } else {
                        if (td.field_P >= gf.field_f) {
                          break L5;
                        } else {
                          this.field_Q = -this.field_B + nh.field_p;
                          var13 = 1;
                          this.field_X = -this.field_N + td.field_P;
                          this.field_rb = je.field_d;
                          this.field_ub = je.field_d;
                          break L4;
                        }
                      }
                    }
                  }
                }
              }
            }
          }
          L6: {
            if (je.field_d != 0) {
              this.field_ub = 0;
              break L6;
            } else {
              break L6;
            }
          }
          this.field_rb = 0;
          break L4;
        }
        L7: {
          if (-1 == (rf.field_n ^ -1)) {
            this.field_ub = 0;
            break L7;
          } else {
            break L7;
          }
        }
        L8: {
          L9: {
            stackIn_33_0 = this;

            if (pa.field_j) {
              break L9;
            } else {
              stackIn_33_0 = this;

              if (gf.field_i > pd.field_k) {
                break L9;
              } else {
                stackIn_33_0 = this;

                if (gf.field_l <= pd.field_k) {
                  break L9;
                } else {
                  stackIn_33_0 = this;

                  if (gf.field_a > bb.field_b) {
                    break L9;
                  } else {
                    stackIn_33_0 = this;

                    if (gf.field_f <= bb.field_b) {
                      break L9;
                    } else {
                      stackIn_34_0 = this;
                      stackIn_34_1 = 1;
                      break L8;
                    }
                  }
                }
              }
            }
          }
          stackIn_34_0 = this;
          stackIn_34_1 = 0;
          break L8;
        }
        L10: {
          L11: {
            ((qr) (this)).field_v = stackIn_34_1 != 0;
            stackIn_38_0 = this;

            if (!this.field_v) {
              break L11;
            } else {
              stackIn_38_0 = this;

              if (this.field_ub != rf.field_n) {
                break L11;
              } else {
                stackIn_38_0 = this;

                if (!param3) {
                  break L11;
                } else {
                  stackIn_39_0 = this;
                  stackIn_39_1 = 1;
                  break L10;
                }
              }
            }
          }
          stackIn_39_0 = this;
          stackIn_39_1 = 0;
          break L10;
        }
        L12: {
          ((qr) (this)).field_L = stackIn_39_1 != 0;
          if (null != this.field_M) {
            var14 = (qr) ((Object) this.field_M.f(3725));
            L13: while (true) {
              if (var14 == null) {
                break L12;
              } else {
                var14.a(this.field_B, this.field_N, this.field_K, param3, -93, this.field_mb);
                var14 = (qr) ((Object) this.field_M.b(-125));
                continue L13;
              }
            }
          } else {
            break L12;
          }
        }
        L14: {
          if (this.field_v) {
            pa.field_j = true;
            break L14;
          } else {
            break L14;
          }
        }
        L15: {
          if (var13 != 0) {
            us.field_c = true;
            break L15;
          } else {
            break L15;
          }
        }
        L16: {
          gf.d(var9, var10, var11, var12);
          if (this.field_ub == 0) {
            break L16;
          } else {
            if (this.field_P) {
              L17: {
                this.field_db = -param1 + (bb.field_b - this.field_X);
                this.field_hb = -param0 + -this.field_Q + pd.field_k;
                if (this.field_hb >= 0) {
                  break L17;
                } else {
                  this.field_hb = 0;
                  break L17;
                }
              }
              L18: {
                if (this.field_hb <= param2 - this.field_K) {
                  break L18;
                } else {
                  this.field_hb = -this.field_K + param2;
                  break L18;
                }
              }
              L19: {
                if (this.field_db < 0) {
                  this.field_db = 0;
                  break L19;
                } else {
                  break L19;
                }
              }
              L20: {
                this.field_cb = 0;
                if (this.field_db <= -this.field_mb + param5) {
                  break L20;
                } else {
                  this.field_db = param5 - this.field_mb;
                  break L20;
                }
              }
              this.field_B = param0 - -this.field_hb;
              this.field_w = 0;
              this.field_N = param1 + this.field_db;
              break L16;
            } else {
              break L16;
            }
          }
        }
    }

    final static void f(int param0) {
        ha.field_a = null;
        if (param0 > -40) {
            field_R = (String) null;
        }
        rm.field_a = null;
    }

    final void a(qr param0, boolean param1) {
        Object stackIn_100_0 = null;
        Object stackIn_101_0 = null;
        int stackIn_101_1 = 0;
        RuntimeException stackIn_107_0 = null;
        StringBuilder stackIn_107_1 = null;
        RuntimeException stackIn_108_0 = null;
        StringBuilder stackIn_108_1 = null;
        String stackIn_108_2 = null;
        RuntimeException decompiledCaughtException = null;
        RuntimeException var3 = null;
        try {
          L0: {
            L1: {
              if (param0 == null) {
                break L1;
              } else {
                L2: {
                  if (null != param0.field_z) {
                    this.field_z = param0.field_z;
                    break L2;
                  } else {
                    break L2;
                  }
                }
                L3: {
                  if (!param0.field_r) {
                    break L3;
                  } else {
                    this.field_r = param0.field_r;
                    break L3;
                  }
                }
                L4: {
                  if (null != param0.field_Z) {
                    this.field_Z = param0.field_Z;
                    break L4;
                  } else {
                    break L4;
                  }
                }
                L5: {
                  if (!param0.field_s) {
                    this.field_s = param0.field_s;
                    break L5;
                  } else {
                    break L5;
                  }
                }
                L6: {
                  if (param0.field_bb) {
                    this.field_bb = param0.field_bb;
                    break L6;
                  } else {
                    break L6;
                  }
                }
                L7: {
                  if (-1 == (param0.field_kb ^ -1)) {
                    break L7;
                  } else {
                    this.field_kb = param0.field_kb;
                    break L7;
                  }
                }
                L8: {
                  if (param0.field_lb != null) {
                    this.field_lb = param0.field_lb;
                    break L8;
                  } else {
                    break L8;
                  }
                }
                L9: {
                  if (param0.field_wb != null) {
                    this.field_wb = param0.field_wb;
                    break L9;
                  } else {
                    break L9;
                  }
                }
                L10: {
                  if (param0.field_H) {
                    this.field_H = param0.field_H;
                    break L10;
                  } else {
                    break L10;
                  }
                }
                L11: {
                  if ((param0.field_I ^ -1) != -257) {
                    this.field_I = param0.field_I;
                    break L11;
                  } else {
                    break L11;
                  }
                }
                L12: {
                  if (param0.field_qb >= 0) {
                    this.field_qb = param0.field_qb;
                    break L12;
                  } else {
                    break L12;
                  }
                }
                L13: {
                  if ((param0.field_jb ^ -1) <= -1) {
                    this.field_jb = param0.field_jb;
                    break L13;
                  } else {
                    break L13;
                  }
                }
                L14: {
                  if (param0.field_C) {
                    this.field_C = param0.field_C;
                    break L14;
                  } else {
                    break L14;
                  }
                }
                L15: {
                  if (null != param0.field_u) {
                    this.field_u = param0.field_u;
                    break L15;
                  } else {
                    break L15;
                  }
                }
                L16: {
                  if (null != param0.field_ob) {
                    this.field_ob = param0.field_ob;
                    break L16;
                  } else {
                    break L16;
                  }
                }
                L17: {
                  if (-1 >= (param0.field_W ^ -1)) {
                    this.field_W = param0.field_W;
                    break L17;
                  } else {
                    break L17;
                  }
                }
                L18: {
                  if ((param0.field_ab ^ -1) <= -1) {
                    this.field_ab = param0.field_ab;
                    break L18;
                  } else {
                    break L18;
                  }
                }
                L19: {
                  if (null == param0.field_vb) {
                    break L19;
                  } else {
                    this.field_vb = param0.field_vb;
                    break L19;
                  }
                }
                L20: {
                  if (null != param0.field_F) {
                    this.field_F = param0.field_F;
                    break L20;
                  } else {
                    break L20;
                  }
                }
                L21: {
                  if (null != param0.field_pb) {
                    this.field_pb = param0.field_pb;
                    break L21;
                  } else {
                    break L21;
                  }
                }
                L22: {
                  if (param0.field_fb == null) {
                    break L22;
                  } else {
                    this.field_fb = param0.field_fb;
                    break L22;
                  }
                }
                L23: {
                  if (param0.field_Y == 0) {
                    break L23;
                  } else {
                    this.field_Y = param0.field_Y;
                    break L23;
                  }
                }
                L24: {
                  if (param0.field_U != null) {
                    this.field_U = param0.field_U;
                    break L24;
                  } else {
                    break L24;
                  }
                }
                L25: {
                  if (0 == param0.field_x) {
                    break L25;
                  } else {
                    this.field_x = param0.field_x;
                    break L25;
                  }
                }
                L26: {
                  if (null == param0.field_nb) {
                    break L26;
                  } else {
                    this.field_nb = param0.field_nb;
                    break L26;
                  }
                }
                L27: {
                  if (null == param0.field_sb) {
                    break L27;
                  } else {
                    this.field_sb = param0.field_sb;
                    break L27;
                  }
                }
                L28: {
                  if (2147483647 == (param0.field_V ^ -1)) {
                    break L28;
                  } else {
                    this.field_V = param0.field_V;
                    break L28;
                  }
                }
                L29: {
                  if (-2147483648 == param0.field_T) {
                    break L29;
                  } else {
                    this.field_T = param0.field_T;
                    break L29;
                  }
                }
                L30: {
                  if (0 == param0.field_y) {
                    break L30;
                  } else {
                    this.field_y = param0.field_y;
                    break L30;
                  }
                }
                L31: {
                  if (param0.field_t == -2147483648) {
                    break L31;
                  } else {
                    this.field_t = param0.field_t;
                    break L31;
                  }
                }
                L32: {
                  if (-2147483648 != param0.field_O) {
                    this.field_O = param0.field_O;
                    break L32;
                  } else {
                    break L32;
                  }
                }
                L33: {
                  if (-1 != (param0.field_E ^ -1)) {
                    this.field_E = param0.field_E;
                    break L33;
                  } else {
                    break L33;
                  }
                }
                L34: {
                  if (0 == param0.field_tb) {
                    break L34;
                  } else {
                    this.field_tb = param0.field_tb;
                    break L34;
                  }
                }
                L35: {
                  if (!param0.field_P) {
                    break L35;
                  } else {
                    this.field_P = param0.field_P;
                    break L35;
                  }
                }
                L36: {
                  if (0 != param0.field_G) {
                    this.field_G = param0.field_G;
                    break L36;
                  } else {
                    break L36;
                  }
                }
                L37: {
                  if ((param0.field_J ^ -1) == 2147483647) {
                    break L37;
                  } else {
                    this.field_J = param0.field_J;
                    break L37;
                  }
                }
                L38: {
                  if ((param0.field_D ^ -1) != 2147483647) {
                    this.field_D = param0.field_D;
                    break L38;
                  } else {
                    break L38;
                  }
                }
                L39: {
                  L40: {
                    stackIn_100_0 = this;

                    if (!this.field_C) {
                      break L40;
                    } else {
                      stackIn_100_0 = this;

                      if (!param0.field_ib) {
                        break L40;
                      } else {
                        stackIn_101_0 = this;
                        stackIn_101_1 = 1;
                        break L39;
                      }
                    }
                  }
                  stackIn_101_0 = this;
                  stackIn_101_1 = 0;
                  break L39;
                }
                ((qr) (this)).field_ib = stackIn_101_1 != 0;
                break L1;
              }
            }
            L41: {
              if (!param1) {
                break L41;
              } else {
                this.a(true, -124);
                break L41;
              }
            }
            break L0;
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L42: {
            var3 = decompiledCaughtException;
            stackIn_107_0 = (RuntimeException) (var3);

            stackIn_107_1 = new StringBuilder().append("qr.P(");

            if (param0 == null) {
              stackIn_108_0 = (RuntimeException) ((Object) stackIn_107_0);
              stackIn_108_1 = (StringBuilder) ((Object) stackIn_107_1);
              stackIn_108_2 = "null";
              break L42;
            } else {
              stackIn_108_0 = (RuntimeException) ((Object) stackIn_107_0);
              stackIn_108_1 = (StringBuilder) ((Object) stackIn_107_1);
              stackIn_108_2 = "{...}";
              break L42;
            }
          }
          throw r.a((Throwable) ((Object) stackIn_108_0), stackIn_108_2 + ',' + param1 + ')');
        }
    }

    final void a(int param0, int param1, byte param2, int param3, int param4) {
        this.field_db = param3;
        this.field_hb = param4;
        if (param2 <= 38) {
            this.field_F = (bi[]) null;
        }
        this.field_gb = 0;
        this.field_cb = 0;
        this.field_w = 0;
        this.field_K = param0;
        this.field_mb = param1;
        this.field_S = 0;
    }

    final static void a(int[] param0, byte[] param1, byte[] param2, String[][] param3, String[] param4, int param5, String[][] param6, bc param7, bc param8, int param9, bc param10, bi[][] param11, int param12, boolean param13, bi[][] param14) {
        RuntimeException stackIn_4_0 = null;
        StringBuilder stackIn_4_1 = null;
        RuntimeException stackIn_5_0 = null;
        StringBuilder stackIn_5_1 = null;
        String stackIn_5_2 = null;
        StringBuilder stackIn_7_1 = null;
        StringBuilder stackIn_8_1 = null;
        String stackIn_8_2 = null;
        StringBuilder stackIn_10_1 = null;
        StringBuilder stackIn_11_1 = null;
        String stackIn_11_2 = null;
        StringBuilder stackIn_13_1 = null;
        StringBuilder stackIn_14_1 = null;
        String stackIn_14_2 = null;
        StringBuilder stackIn_16_1 = null;
        StringBuilder stackIn_17_1 = null;
        String stackIn_17_2 = null;
        StringBuilder stackIn_19_1 = null;
        StringBuilder stackIn_20_1 = null;
        String stackIn_20_2 = null;
        StringBuilder stackIn_22_1 = null;
        StringBuilder stackIn_23_1 = null;
        String stackIn_23_2 = null;
        StringBuilder stackIn_25_1 = null;
        StringBuilder stackIn_26_1 = null;
        String stackIn_26_2 = null;
        StringBuilder stackIn_28_1 = null;
        StringBuilder stackIn_29_1 = null;
        String stackIn_29_2 = null;
        StringBuilder stackIn_31_1 = null;
        StringBuilder stackIn_32_1 = null;
        String stackIn_32_2 = null;
        StringBuilder stackIn_34_1 = null;
        StringBuilder stackIn_35_1 = null;
        String stackIn_35_2 = null;
        RuntimeException decompiledCaughtException = null;
        int var15_int = 0;
        RuntimeException var15 = null;
        int[] var16 = null;
        nk[] var17 = null;
        try {
          L0: {
            var15_int = 87 / ((40 - param12) / 61);
            var17 = (nk[]) null;
            var16 = (int[]) null;
            ga.a((nk[]) null, param5, param10, (int[]) null, param11, param4, param9, param7, param1, param3, param8, param2, param13, param6, 16384, param0, param14);
            break L0;
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L1: {
            var15 = decompiledCaughtException;
            stackIn_4_0 = (RuntimeException) (var15);

            stackIn_4_1 = new StringBuilder().append("qr.FA(");

            if (param0 == null) {
              stackIn_5_0 = (RuntimeException) ((Object) stackIn_4_0);
              stackIn_5_1 = (StringBuilder) ((Object) stackIn_4_1);
              stackIn_5_2 = "null";
              break L1;
            } else {
              stackIn_5_0 = (RuntimeException) ((Object) stackIn_4_0);
              stackIn_5_1 = (StringBuilder) ((Object) stackIn_4_1);
              stackIn_5_2 = "{...}";
              break L1;
            }
          }
          L2: {


            stackIn_7_1 = ((StringBuilder) (Object) stackIn_5_1).append(stackIn_5_2).append(',');

            if (param1 == null) {
              stackIn_5_0 = (RuntimeException) ((Object) stackIn_5_0);
              stackIn_8_1 = (StringBuilder) ((Object) stackIn_7_1);
              stackIn_8_2 = "null";
              break L2;
            } else {
              stackIn_5_0 = (RuntimeException) ((Object) stackIn_5_0);
              stackIn_8_1 = (StringBuilder) ((Object) stackIn_7_1);
              stackIn_8_2 = "{...}";
              break L2;
            }
          }
          L3: {


            stackIn_10_1 = ((StringBuilder) (Object) stackIn_8_1).append(stackIn_8_2).append(',');

            if (param2 == null) {
              stackIn_5_0 = (RuntimeException) ((Object) stackIn_5_0);
              stackIn_11_1 = (StringBuilder) ((Object) stackIn_10_1);
              stackIn_11_2 = "null";
              break L3;
            } else {
              stackIn_5_0 = (RuntimeException) ((Object) stackIn_5_0);
              stackIn_11_1 = (StringBuilder) ((Object) stackIn_10_1);
              stackIn_11_2 = "{...}";
              break L3;
            }
          }
          L4: {


            stackIn_13_1 = ((StringBuilder) (Object) stackIn_11_1).append(stackIn_11_2).append(',');

            if (param3 == null) {
              stackIn_5_0 = (RuntimeException) ((Object) stackIn_5_0);
              stackIn_14_1 = (StringBuilder) ((Object) stackIn_13_1);
              stackIn_14_2 = "null";
              break L4;
            } else {
              stackIn_5_0 = (RuntimeException) ((Object) stackIn_5_0);
              stackIn_14_1 = (StringBuilder) ((Object) stackIn_13_1);
              stackIn_14_2 = "{...}";
              break L4;
            }
          }
          L5: {


            stackIn_16_1 = ((StringBuilder) (Object) stackIn_14_1).append(stackIn_14_2).append(',');

            if (param4 == null) {
              stackIn_5_0 = (RuntimeException) ((Object) stackIn_5_0);
              stackIn_17_1 = (StringBuilder) ((Object) stackIn_16_1);
              stackIn_17_2 = "null";
              break L5;
            } else {
              stackIn_5_0 = (RuntimeException) ((Object) stackIn_5_0);
              stackIn_17_1 = (StringBuilder) ((Object) stackIn_16_1);
              stackIn_17_2 = "{...}";
              break L5;
            }
          }
          L6: {


            stackIn_19_1 = ((StringBuilder) (Object) stackIn_17_1).append(stackIn_17_2).append(',').append(param5).append(',');

            if (param6 == null) {
              stackIn_5_0 = (RuntimeException) ((Object) stackIn_5_0);
              stackIn_20_1 = (StringBuilder) ((Object) stackIn_19_1);
              stackIn_20_2 = "null";
              break L6;
            } else {
              stackIn_5_0 = (RuntimeException) ((Object) stackIn_5_0);
              stackIn_20_1 = (StringBuilder) ((Object) stackIn_19_1);
              stackIn_20_2 = "{...}";
              break L6;
            }
          }
          L7: {


            stackIn_22_1 = ((StringBuilder) (Object) stackIn_20_1).append(stackIn_20_2).append(',');

            if (param7 == null) {
              stackIn_5_0 = (RuntimeException) ((Object) stackIn_5_0);
              stackIn_23_1 = (StringBuilder) ((Object) stackIn_22_1);
              stackIn_23_2 = "null";
              break L7;
            } else {
              stackIn_5_0 = (RuntimeException) ((Object) stackIn_5_0);
              stackIn_23_1 = (StringBuilder) ((Object) stackIn_22_1);
              stackIn_23_2 = "{...}";
              break L7;
            }
          }
          L8: {


            stackIn_25_1 = ((StringBuilder) (Object) stackIn_23_1).append(stackIn_23_2).append(',');

            if (param8 == null) {
              stackIn_5_0 = (RuntimeException) ((Object) stackIn_5_0);
              stackIn_26_1 = (StringBuilder) ((Object) stackIn_25_1);
              stackIn_26_2 = "null";
              break L8;
            } else {
              stackIn_5_0 = (RuntimeException) ((Object) stackIn_5_0);
              stackIn_26_1 = (StringBuilder) ((Object) stackIn_25_1);
              stackIn_26_2 = "{...}";
              break L8;
            }
          }
          L9: {


            stackIn_28_1 = ((StringBuilder) (Object) stackIn_26_1).append(stackIn_26_2).append(',').append(param9).append(',');

            if (param10 == null) {
              stackIn_5_0 = (RuntimeException) ((Object) stackIn_5_0);
              stackIn_29_1 = (StringBuilder) ((Object) stackIn_28_1);
              stackIn_29_2 = "null";
              break L9;
            } else {
              stackIn_5_0 = (RuntimeException) ((Object) stackIn_5_0);
              stackIn_29_1 = (StringBuilder) ((Object) stackIn_28_1);
              stackIn_29_2 = "{...}";
              break L9;
            }
          }
          L10: {


            stackIn_31_1 = ((StringBuilder) (Object) stackIn_29_1).append(stackIn_29_2).append(',');

            if (param11 == null) {
              stackIn_5_0 = (RuntimeException) ((Object) stackIn_5_0);
              stackIn_32_1 = (StringBuilder) ((Object) stackIn_31_1);
              stackIn_32_2 = "null";
              break L10;
            } else {
              stackIn_5_0 = (RuntimeException) ((Object) stackIn_5_0);
              stackIn_32_1 = (StringBuilder) ((Object) stackIn_31_1);
              stackIn_32_2 = "{...}";
              break L10;
            }
          }
          L11: {


            stackIn_34_1 = ((StringBuilder) (Object) stackIn_32_1).append(stackIn_32_2).append(',').append(param12).append(',').append(param13).append(',');

            if (param14 == null) {
              stackIn_5_0 = (RuntimeException) ((Object) stackIn_5_0);
              stackIn_35_1 = (StringBuilder) ((Object) stackIn_34_1);
              stackIn_35_2 = "null";
              break L11;
            } else {
              stackIn_5_0 = (RuntimeException) ((Object) stackIn_5_0);
              stackIn_35_1 = (StringBuilder) ((Object) stackIn_34_1);
              stackIn_35_2 = "{...}";
              break L11;
            }
          }
          throw r.a((Throwable) ((Object) stackIn_5_0), stackIn_35_2 + ')');
        }
    }

    qr(long param0, qr param1) {
        this(param0, param1, 0, 0, 0, 0, (String) null);
    }

    final int e(boolean param0) {
        if (param0) {
            this.a(55, true, true, 26);
        }
        return this.a(2147483647, (byte) -93);
    }

    final void a(boolean param0, int param1) {
        us.field_c = !param0 ? true : false;
        pa.field_j = !param0 ? true : false;
        if (param1 != -20429) {
            this.field_z = (String) null;
        }
        this.a(0, 0, gf.field_b, true, -98, gf.field_k);
    }

    private final void a(boolean param0, boolean param1, boolean param2, int param3, int param4, int param5, boolean param6, int param7, boolean param8, boolean param9, int param10) {
        int stackIn_7_0 = 0;
        int var12;
        int var13;
        int var14;
        int var15;
        bi[] var16;
        bi var17;
        int var18;
        int var19;
        int var20;
        int var21;
        int var22;
        int var23_int;
        String var23;
        qr var23_ref;
        int var24;
        int var25;
        String var26;
        L0: {
          var25 = ShatteredPlansClient.field_F ? 1 : 0;
          this.field_B = param3 + this.field_hb;
          this.field_N = param4 - -this.field_db;
          var12 = gf.field_i;
          var13 = gf.field_a;
          var14 = gf.field_l;
          var15 = gf.field_f;
          gf.i(this.field_B, this.field_N, this.field_B - -this.field_K, this.field_mb + this.field_N);
          param1 = param1 & this.field_s;
          if (param5 == -2147483648) {
            break L0;
          } else {
            qr.a((int[]) null, (byte[]) null, (byte[]) null, (String[][]) null, (String[]) null, 117, (String[][]) null, (bc) null, (bc) null, 119, (bc) null, (bi[][]) null, 82, false, (bi[][]) null);
            break L0;
          }
        }
        L1: {
          if (!this.field_H) {
            L2: {
              param6 = this.field_bb;
              param8 = this.field_L;
              if (this.field_ub == 0) {
                stackIn_7_0 = 0;
                break L2;
              } else {
                stackIn_7_0 = 1;
                break L2;
              }
            }
            param2 = stackIn_7_0 != 0;
            break L1;
          } else {
            break L1;
          }
        }
        L3: {
          var16 = this.field_F;
          var17 = this.field_pb;
          var18 = this.field_x;
          var19 = 0;
          if (!param1) {
            L4: {
              if (this.field_ob == null) {
                break L4;
              } else {
                var17 = this.field_ob;
                break L4;
              }
            }
            L5: {
              if ((this.field_W ^ -1) > -1) {
                break L5;
              } else {
                var18 = this.field_W;
                break L5;
              }
            }
            if (this.field_wb != null) {
              var16 = this.field_wb;
              break L3;
            } else {
              break L3;
            }
          } else {
            break L3;
          }
        }
        L6: {
          var20 = 0;
          if (param8) {
            L7: {
              if (this.field_D != -2147483648) {
                var19 = this.field_D;
                break L7;
              } else {
                break L7;
              }
            }
            L8: {
              if (null != this.field_fb) {
                var17 = this.field_fb;
                break L8;
              } else {
                break L8;
              }
            }
            L9: {
              if (this.field_lb != null) {
                var16 = this.field_lb;
                break L9;
              } else {
                break L9;
              }
            }
            L10: {
              if (this.field_jb >= 0) {
                var18 = this.field_jb;
                break L10;
              } else {
                break L10;
              }
            }
            if (-2147483648 == this.field_O) {
              break L6;
            } else {
              var20 = this.field_O;
              break L6;
            }
          } else {
            break L6;
          }
        }
        L11: {
          if (param2) {
            L12: {
              if (0 <= this.field_qb) {
                var18 = this.field_qb;
                break L12;
              } else {
                break L12;
              }
            }
            L13: {
              if (null != this.field_vb) {
                var16 = this.field_vb;
                break L13;
              } else {
                break L13;
              }
            }
            L14: {
              if (this.field_U != null) {
                var17 = this.field_U;
                break L14;
              } else {
                break L14;
              }
            }
            L15: {
              if (this.field_t == -2147483648) {
                break L15;
              } else {
                var19 = this.field_t;
                break L15;
              }
            }
            if ((this.field_J ^ -1) != 2147483647) {
              var20 = this.field_J;
              break L11;
            } else {
              break L11;
            }
          } else {
            break L11;
          }
        }
        L16: {
          if (!param6) {
            break L16;
          } else {
            L17: {
              if (2147483647 == (this.field_V ^ -1)) {
                break L17;
              } else {
                var19 = this.field_V;
                break L17;
              }
            }
            L18: {
              if ((this.field_T ^ -1) != 2147483647) {
                var20 = this.field_T;
                break L18;
              } else {
                break L18;
              }
            }
            L19: {
              if (null == this.field_sb) {
                break L19;
              } else {
                var16 = this.field_sb;
                break L19;
              }
            }
            L20: {
              if (null == this.field_Z) {
                break L20;
              } else {
                var17 = this.field_Z;
                break L20;
              }
            }
            if ((this.field_ab ^ -1) > -1) {
              break L16;
            } else {
              var18 = this.field_ab;
              break L16;
            }
          }
        }
        L21: {
          var21 = this.field_E + var19;
          var22 = this.field_tb + var20;
          if (this.field_r) {
            em.a(param7, param10, 7, param4, var16, param3);
            break L21;
          } else {
            em.a(this.field_mb, this.field_K, 7, this.field_N, var16, this.field_B);
            break L21;
          }
        }
        L22: {
          if (var17 != null) {
            L23: {
              var23_int = this.field_B + var21;
              if ((this.field_y ^ -1) == -2) {
                var23_int = var23_int + (-var17.field_o + this.field_K) / 2;
                break L23;
              } else {
                break L23;
              }
            }
            L24: {
              var24 = var22 + this.field_N;
              if (1 == this.field_G) {
                var24 = var24 + (this.field_mb - var17.field_p) / 2;
                break L24;
              } else {
                break L24;
              }
            }
            L25: {
              if (this.field_y == 2) {
                var23_int = var23_int + (-var17.field_o + this.field_K);
                break L25;
              } else {
                break L25;
              }
            }
            L26: {
              if ((this.field_G ^ -1) == -3) {
                var24 = var24 + (-var17.field_p + this.field_mb);
                break L26;
              } else {
                break L26;
              }
            }
            if (256 > this.field_eb) {
              var17.a(var23_int, var24, this.field_eb);
              break L22;
            } else {
              var17.f(var23_int, var24);
              break L22;
            }
          } else {
            break L22;
          }
        }
        L27: {
          if (null == this.field_z) {
            break L27;
          } else {
            if (null != this.field_nb) {
              L28: {
                var26 = this.field_z;
                var23 = var26;
                var23 = var26;
                var23 = var26;
                if (!param9) {
                  break L28;
                } else {
                  if (this.field_u != null) {
                    var23 = var26 + this.field_u;
                    break L28;
                  } else {
                    break L28;
                  }
                }
              }
              L29: {
                if (this.field_nb.c(var23) > -(2 * this.field_kb) + this.field_K) {
                  break L29;
                } else {
                  if ((var23.indexOf("<br>") ^ -1) > -1) {
                    L30: {
                      if ((this.field_G ^ -1) == -2) {
                        var22 = var22 + (-this.field_nb.field_q + this.field_mb + -this.field_nb.field_J) / 2;
                        break L30;
                      } else {
                        if (2 == this.field_G) {
                          var22 = var22 + (-this.field_nb.field_J + (-this.field_nb.field_q + this.field_mb));
                          break L30;
                        } else {
                          break L30;
                        }
                      }
                    }
                    if (-1 != (this.field_y ^ -1)) {
                      if (-2 == (this.field_y ^ -1)) {
                        this.field_nb.a(var23, var21 + this.field_B + this.field_kb - -((this.field_K - this.field_kb * 2) / 2), this.field_N + var22 + this.field_nb.field_J, var18, -1, this.field_I);
                        break L27;
                      } else {
                        if (this.field_y != 2) {
                          this.field_nb.a(var23, this.field_kb + this.field_B - -var21, var22 + this.field_N, this.field_K + -(2 * this.field_kb), this.field_mb, var18, -1, this.field_I, this.field_y, this.field_G, this.field_Y);
                          break L27;
                        } else {
                          this.field_nb.c(var23, this.field_K + (-(this.field_kb * 2) + var21) + (this.field_B + this.field_kb), var22 + (this.field_N - -this.field_nb.field_J), var18, -1, this.field_I);
                          break L27;
                        }
                      }
                    } else {
                      this.field_nb.d(var23, var21 + (this.field_B + this.field_kb), this.field_nb.field_J + var22 + this.field_N, var18, -1, this.field_I);
                      break L27;
                    }
                  } else {
                    break L29;
                  }
                }
              }
              this.field_nb.a(var23, var21 + (this.field_kb + this.field_B), this.field_N - -var22, this.field_K - 2 * this.field_kb, this.field_mb, var18, -1, this.field_I, this.field_y, this.field_G, this.field_Y);
              break L27;
            } else {
              break L27;
            }
          }
        }
        L31: {
          if (null == this.field_M) {
            break L31;
          } else {
            var23_ref = (qr) ((Object) this.field_M.d(0));
            L32: while (true) {
              if (var23_ref == null) {
                break L31;
              } else {
                var23_ref.a(this.field_ib, param1, param2, this.field_B - -var19, var20 + this.field_N, -2147483648, param6, this.field_mb, param8, param9, this.field_K);
                var23_ref = (qr) ((Object) this.field_M.a((byte) -71));
                continue L32;
              }
            }
          }
        }
        gf.d(var12, var13, var14, var15);
    }

    final static void a(bc param0, byte param1) {
        bi var6 = null;
        int var3 = 0;
        int var4 = 0;
        try {
            var6 = new bi(param0.a(true, "", "final_frame.jpg"), (java.awt.Component) ((Object) tm.field_i));
            var3 = var6.field_z;
            var4 = var6.field_w;
            wh.a(false);
            if (param1 <= 22) {
                vr var5 = (vr) null;
                qr.a(-15, 122, (String[]) null, (vr) null, -98, -12);
            }
            qa.field_q = new bi(var3, 3 * var4 / 4);
            qa.field_q.e();
            var6.c(0, 0);
            ak.field_z = new bi(var3, -qa.field_q.field_w + var4);
            ak.field_z.e();
            var6.c(0, -qa.field_q.field_w);
            ak.field_z.field_x = qa.field_q.field_w;
            ln.f((byte) 85);
        } catch (RuntimeException runtimeException) {
            throw r.a((Throwable) ((Object) runtimeException), "qr.T(" + (param0 != null ? "{...}" : "null") + ',' + param1 + ')');
        }
    }

    qr(long param0, qr param1, String param2) {
        this(param0, param1, 0, 0, 0, 0, param2);
    }

    public static void e(int param0) {
        field_A = null;
        if (param0 != -14115) {
            return;
        }
        field_R = null;
    }

    protected qr() {
        this.field_W = -1;
        this.field_T = -2147483648;
        this.field_I = 256;
        this.field_D = -2147483648;
        this.field_qb = -1;
        this.field_V = -2147483648;
        this.field_jb = -1;
        this.field_O = -2147483648;
        this.field_J = -2147483648;
        this.field_ib = false;
        this.field_s = true;
        this.field_C = false;
        this.field_eb = 256;
        this.field_ab = -1;
        this.field_t = -2147483648;
    }

    qr(long param0, qr param1, int param2, int param3, int param4, int param5, String param6) {
        RuntimeException runtimeException = null;
        RuntimeException stackIn_6_0 = null;
        StringBuilder stackIn_6_1 = null;
        RuntimeException stackIn_7_0 = null;
        StringBuilder stackIn_7_1 = null;
        String stackIn_7_2 = null;
        StringBuilder stackIn_9_1 = null;
        StringBuilder stackIn_10_1 = null;
        String stackIn_10_2 = null;
        RuntimeException decompiledCaughtException = null;
        this.field_W = -1;
        this.field_T = -2147483648;
        this.field_I = 256;
        this.field_D = -2147483648;
        this.field_qb = -1;
        this.field_V = -2147483648;
        this.field_jb = -1;
        this.field_O = -2147483648;
        this.field_J = -2147483648;
        this.field_ib = false;
        this.field_s = true;
        this.field_C = false;
        this.field_eb = 256;
        this.field_ab = -1;
        this.field_t = -2147483648;
        try {
          L0: {
            L1: {
              this.field_K = param4;
              this.field_hb = param2;
              this.field_mb = param5;
              this.field_b = param0;
              this.field_db = param3;
              this.a(param1, false);
              if (param6 == null) {
                break L1;
              } else {
                this.field_z = param6;
                break L1;
              }
            }
            break L0;
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L2: {
            runtimeException = decompiledCaughtException;
            stackIn_6_0 = (RuntimeException) (runtimeException);

            stackIn_6_1 = new StringBuilder().append("qr.<init>(").append(param0).append(',');

            if (param1 == null) {
              stackIn_7_0 = (RuntimeException) ((Object) stackIn_6_0);
              stackIn_7_1 = (StringBuilder) ((Object) stackIn_6_1);
              stackIn_7_2 = "null";
              break L2;
            } else {
              stackIn_7_0 = (RuntimeException) ((Object) stackIn_6_0);
              stackIn_7_1 = (StringBuilder) ((Object) stackIn_6_1);
              stackIn_7_2 = "{...}";
              break L2;
            }
          }
          L3: {


            stackIn_9_1 = ((StringBuilder) (Object) stackIn_7_1).append(stackIn_7_2).append(',').append(param2).append(',').append(param3).append(',').append(param4).append(',').append(param5).append(',');

            if (param6 == null) {
              stackIn_7_0 = (RuntimeException) ((Object) stackIn_7_0);
              stackIn_10_1 = (StringBuilder) ((Object) stackIn_9_1);
              stackIn_10_2 = "null";
              break L3;
            } else {
              stackIn_7_0 = (RuntimeException) ((Object) stackIn_7_0);
              stackIn_10_1 = (StringBuilder) ((Object) stackIn_9_1);
              stackIn_10_2 = "{...}";
              break L3;
            }
          }
          throw r.a((Throwable) ((Object) stackIn_7_0), stackIn_10_2 + ')');
        }
    }

    static {
        field_R = "Checking";
    }
}
