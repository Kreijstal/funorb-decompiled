/*
 * Decompiled by CFR-JS 0.4.0.
 */
final class jd {
    boolean field_b;
    private int field_h;
    private int field_f;
    private int field_a;
    static String[] field_d;
    private boolean field_c;
    int field_g;
    private int field_i;
    int field_e;

    final boolean b(byte param0) {
        int stackIn_4_0 = 0;
        int stackIn_8_0 = 0;
        int stackOut_7_0 = 0;
        int stackOut_6_0 = 0;
        int stackOut_3_0 = 0;
        int stackOut_2_0 = 0;
        if (param0 != -73) {
          L0: {
            this.a(false, -83, -3, 33);
            if (97 != this.field_h) {
              stackOut_7_0 = 0;
              stackIn_8_0 = stackOut_7_0;
              break L0;
            } else {
              stackOut_6_0 = 1;
              stackIn_8_0 = stackOut_6_0;
              break L0;
            }
          }
          return stackIn_8_0 != 0;
        } else {
          L1: {
            if (97 != this.field_h) {
              stackOut_3_0 = 0;
              stackIn_4_0 = stackOut_3_0;
              break L1;
            } else {
              stackOut_2_0 = 1;
              stackIn_4_0 = stackOut_2_0;
              break L1;
            }
          }
          return stackIn_4_0 != 0;
        }
    }

    final static void a(boolean param0, byte param1) {
        if (lfa.field_q > 0) {
          if (!eda.field_j) {
            L0: {
              if (-1 > (ld.field_p ^ -1)) {
                if (w.field_e) {
                  dg.g(0, 0, dg.field_i, i.field_a.field_M);
                  qw.field_b.a(param0, 7802);
                  break L0;
                } else {
                  if (param1 < 67) {
                    field_d = (String[]) null;
                    return;
                  } else {
                    return;
                  }
                }
              } else {
                if (dja.field_m <= 0) {
                  break L0;
                } else {
                  L1: {
                    if (w.field_e) {
                      dg.g(0, 0, dg.field_i, i.field_a.field_M);
                      qw.field_b.a(param0, 7802);
                      break L1;
                    } else {
                      break L1;
                    }
                  }
                  if (param1 < 67) {
                    field_d = (String[]) null;
                    return;
                  } else {
                    return;
                  }
                }
              }
            }
            if (param1 < 67) {
              field_d = (String[]) null;
              return;
            } else {
              return;
            }
          } else {
            L2: {
              dg.g(0, 0, dg.field_i, i.field_a.field_M);
              sb.field_n.a(param0, 7802);
              if (-1 > (ld.field_p ^ -1)) {
                if (w.field_e) {
                  dg.g(0, 0, dg.field_i, i.field_a.field_M);
                  qw.field_b.a(param0, 7802);
                  break L2;
                } else {
                  if (param1 < 67) {
                    field_d = (String[]) null;
                    return;
                  } else {
                    return;
                  }
                }
              } else {
                if (dja.field_m <= 0) {
                  break L2;
                } else {
                  L3: {
                    if (w.field_e) {
                      dg.g(0, 0, dg.field_i, i.field_a.field_M);
                      qw.field_b.a(param0, 7802);
                      break L3;
                    } else {
                      break L3;
                    }
                  }
                  L4: {
                    if (param1 >= 67) {
                      break L4;
                    } else {
                      field_d = (String[]) null;
                      break L4;
                    }
                  }
                  return;
                }
              }
            }
            if (param1 < 67) {
              field_d = (String[]) null;
              return;
            } else {
              return;
            }
          }
        } else {
          if (-1 <= (ld.field_p ^ -1)) {
            if (dja.field_m > 0) {
              L5: {
                if (w.field_e) {
                  dg.g(0, 0, dg.field_i, i.field_a.field_M);
                  qw.field_b.a(param0, 7802);
                  break L5;
                } else {
                  break L5;
                }
              }
              if (param1 < 67) {
                field_d = (String[]) null;
                return;
              } else {
                return;
              }
            } else {
              L6: {
                if (param1 >= 67) {
                  break L6;
                } else {
                  field_d = (String[]) null;
                  break L6;
                }
              }
              return;
            }
          } else {
            L7: {
              if (w.field_e) {
                dg.g(0, 0, dg.field_i, i.field_a.field_M);
                qw.field_b.a(param0, 7802);
                break L7;
              } else {
                break L7;
              }
            }
            L8: {
              if (param1 >= 67) {
                break L8;
              } else {
                field_d = (String[]) null;
                break L8;
              }
            }
            return;
          }
        }
    }

    final void b(int param0, int param1) {
        if (this.field_f != 0) {
          if (param0 <= 84) {
            this.field_i = -59;
            return;
          } else {
            return;
          }
        } else {
          this.field_g = param1;
          this.field_c = false;
          this.field_b = true;
          if (param0 > 84) {
            return;
          } else {
            this.field_i = -59;
            return;
          }
        }
    }

    final boolean b(int param0) {
        boolean discarded$11 = false;
        int stackIn_4_0 = 0;
        int stackIn_8_0 = 0;
        int stackOut_7_0 = 0;
        int stackOut_6_0 = 0;
        int stackOut_3_0 = 0;
        int stackOut_2_0 = 0;
        if (param0 != 0) {
          L0: {
            discarded$11 = this.c(-72);
            if (0 == this.field_f) {
              stackOut_7_0 = 0;
              stackIn_8_0 = stackOut_7_0;
              break L0;
            } else {
              stackOut_6_0 = 1;
              stackIn_8_0 = stackOut_6_0;
              break L0;
            }
          }
          return stackIn_8_0 != 0;
        } else {
          L1: {
            if (0 == this.field_f) {
              stackOut_3_0 = 0;
              stackIn_4_0 = stackOut_3_0;
              break L1;
            } else {
              stackOut_2_0 = 1;
              stackIn_4_0 = stackOut_2_0;
              break L1;
            }
          }
          return stackIn_4_0 != 0;
        }
    }

    final void a(int param0, int param1) {
        L0: {
          this.field_b = false;
          this.field_h = 0;
          this.field_a = 0;
          if (0 != this.field_f) {
            break L0;
          } else {
            if (96 == gf.field_k) {
              L1: {
                if (0 >= this.field_g) {
                  this.field_g = this.field_e;
                  break L1;
                } else {
                  break L1;
                }
              }
              this.field_c = false;
              this.field_b = true;
              this.field_g = this.field_g - 1;
              break L0;
            } else {
              L2: {
                if (-1 == (this.field_f ^ -1)) {
                  this.field_h = gf.field_k;
                  break L2;
                } else {
                  break L2;
                }
              }
              if (this.field_f == 0) {
                L3: {
                  if (-98 == (gf.field_k ^ -1)) {
                    L4: {
                      this.field_g = this.field_g + 1;
                      if (this.field_g >= this.field_e) {
                        this.field_g = 0;
                        break L4;
                      } else {
                        break L4;
                      }
                    }
                    this.field_b = true;
                    this.field_c = false;
                    break L3;
                  } else {
                    break L3;
                  }
                }
                if (this.field_f != 0) {
                  L5: {
                    if (param1 < -116) {
                      break L5;
                    } else {
                      this.a(true, -61, 69, -124);
                      break L5;
                    }
                  }
                  return;
                } else {
                  L6: {
                    L7: {
                      if (gf.field_k == 98) {
                        break L7;
                      } else {
                        if (gf.field_k == 99) {
                          break L7;
                        } else {
                          break L6;
                        }
                      }
                    }
                    this.field_b = true;
                    if (this.field_g >= 0) {
                      this.field_c = false;
                      break L6;
                    } else {
                      this.field_g = param0;
                      this.field_c = false;
                      break L6;
                    }
                  }
                  L8: {
                    if (param1 < -116) {
                      break L8;
                    } else {
                      this.a(true, -61, 69, -124);
                      break L8;
                    }
                  }
                  return;
                }
              } else {
                L9: {
                  if (this.field_f != 0) {
                    break L9;
                  } else {
                    L10: {
                      if (gf.field_k == 98) {
                        break L10;
                      } else {
                        if (gf.field_k == 99) {
                          break L10;
                        } else {
                          break L9;
                        }
                      }
                    }
                    this.field_b = true;
                    if (this.field_g >= 0) {
                      this.field_c = false;
                      break L9;
                    } else {
                      this.field_g = param0;
                      this.field_c = false;
                      break L9;
                    }
                  }
                }
                L11: {
                  if (param1 < -116) {
                    break L11;
                  } else {
                    this.a(true, -61, 69, -124);
                    break L11;
                  }
                }
                return;
              }
            }
          }
        }
        L12: {
          if (-1 == (this.field_f ^ -1)) {
            this.field_h = gf.field_k;
            break L12;
          } else {
            break L12;
          }
        }
        L13: {
          if (this.field_f != 0) {
            break L13;
          } else {
            if (-98 == (gf.field_k ^ -1)) {
              L14: {
                this.field_g = this.field_g + 1;
                if (this.field_g >= this.field_e) {
                  this.field_g = 0;
                  break L14;
                } else {
                  break L14;
                }
              }
              this.field_b = true;
              this.field_c = false;
              break L13;
            } else {
              if (this.field_f == 0) {
                if (gf.field_k != 98) {
                  if (gf.field_k == 99) {
                    this.field_b = true;
                    if (this.field_g >= 0) {
                      L15: {
                        this.field_c = false;
                        if (param1 < -116) {
                          break L15;
                        } else {
                          this.a(true, -61, 69, -124);
                          break L15;
                        }
                      }
                      return;
                    } else {
                      this.field_g = param0;
                      L16: {
                        this.field_c = false;
                        if (param1 < -116) {
                          break L16;
                        } else {
                          this.a(true, -61, 69, -124);
                          break L16;
                        }
                      }
                      return;
                    }
                  } else {
                    if (param1 >= -116) {
                      this.a(true, -61, 69, -124);
                      return;
                    } else {
                      return;
                    }
                  }
                } else {
                  this.field_b = true;
                  if (this.field_g >= 0) {
                    L17: {
                      this.field_c = false;
                      if (param1 < -116) {
                        break L17;
                      } else {
                        this.a(true, -61, 69, -124);
                        break L17;
                      }
                    }
                    return;
                  } else {
                    this.field_g = param0;
                    L18: {
                      this.field_c = false;
                      if (param1 < -116) {
                        break L18;
                      } else {
                        this.a(true, -61, 69, -124);
                        break L18;
                      }
                    }
                    return;
                  }
                }
              } else {
                if (param1 >= -116) {
                  this.a(true, -61, 69, -124);
                  return;
                } else {
                  return;
                }
              }
            }
          }
        }
        if (this.field_f != 0) {
          if (param1 >= -116) {
            this.a(true, -61, 69, -124);
            return;
          } else {
            return;
          }
        } else {
          L19: {
            if (gf.field_k == 98) {
              break L19;
            } else {
              if (gf.field_k == 99) {
                break L19;
              } else {
                if (param1 >= -116) {
                  this.a(true, -61, 69, -124);
                  return;
                } else {
                  return;
                }
              }
            }
          }
          this.field_b = true;
          if (this.field_g >= 0) {
            this.field_c = false;
            if (param1 >= -116) {
              this.a(true, -61, 69, -124);
              return;
            } else {
              return;
            }
          } else {
            this.field_g = param0;
            this.field_c = false;
            if (param1 >= -116) {
              this.a(true, -61, 69, -124);
              return;
            } else {
              return;
            }
          }
        }
    }

    final boolean d(int param0) {
        int stackIn_4_0 = 0;
        int stackIn_8_0 = 0;
        int stackOut_7_0 = 0;
        int stackOut_6_0 = 0;
        int stackOut_3_0 = 0;
        int stackOut_2_0 = 0;
        if (param0 != -9907) {
          L0: {
            this.field_b = false;
            if (102 != this.field_h) {
              stackOut_7_0 = 0;
              stackIn_8_0 = stackOut_7_0;
              break L0;
            } else {
              stackOut_6_0 = 1;
              stackIn_8_0 = stackOut_6_0;
              break L0;
            }
          }
          return stackIn_8_0 != 0;
        } else {
          L1: {
            if (102 != this.field_h) {
              stackOut_3_0 = 0;
              stackIn_4_0 = stackOut_3_0;
              break L1;
            } else {
              stackOut_2_0 = 1;
              stackIn_4_0 = stackOut_2_0;
              break L1;
            }
          }
          return stackIn_4_0 != 0;
        }
    }

    public static void a(byte param0) {
        if (param0 != 89) {
            return;
        }
        field_d = null;
    }

    final void a(boolean param0) {
        this.field_b = false;
        this.field_a = 0;
        if (!param0) {
          L0: {
            this.field_f = 60;
            this.field_h = 0;
            if (-1 == (this.field_f ^ -1)) {
              this.field_h = gf.field_k;
              break L0;
            } else {
              break L0;
            }
          }
          return;
        } else {
          L1: {
            this.field_h = 0;
            if (-1 == (this.field_f ^ -1)) {
              this.field_h = gf.field_k;
              break L1;
            } else {
              break L1;
            }
          }
          return;
        }
    }

    final boolean a(int param0) {
        int stackIn_4_0 = 0;
        int stackIn_8_0 = 0;
        int stackOut_7_0 = 0;
        int stackOut_6_0 = 0;
        int stackOut_3_0 = 0;
        int stackOut_2_0 = 0;
        if (param0 != -1) {
          L0: {
            this.field_a = -17;
            if ((this.field_h ^ -1) != -104) {
              stackOut_7_0 = 0;
              stackIn_8_0 = stackOut_7_0;
              break L0;
            } else {
              stackOut_6_0 = 1;
              stackIn_8_0 = stackOut_6_0;
              break L0;
            }
          }
          return stackIn_8_0 != 0;
        } else {
          L1: {
            if ((this.field_h ^ -1) != -104) {
              stackOut_3_0 = 0;
              stackIn_4_0 = stackOut_3_0;
              break L1;
            } else {
              stackOut_2_0 = 1;
              stackIn_4_0 = stackOut_2_0;
              break L1;
            }
          }
          return stackIn_4_0 != 0;
        }
    }

    final void b(boolean param0) {
        this.field_a = 0;
        this.field_b = false;
        this.field_h = 0;
        if (0 != this.field_f) {
          L0: {
            if (-1 != (this.field_f ^ -1)) {
              break L0;
            } else {
              if (98 == gf.field_k) {
                L1: {
                  if (this.field_g <= 0) {
                    this.field_g = this.field_e;
                    break L1;
                  } else {
                    break L1;
                  }
                }
                this.field_c = false;
                this.field_b = true;
                this.field_g = this.field_g - 1;
                break L0;
              } else {
                if (param0) {
                  if (0 == this.field_f) {
                    if (-100 == (gf.field_k ^ -1)) {
                      this.field_g = this.field_g + 1;
                      if (this.field_e > this.field_g) {
                        this.field_b = true;
                        this.field_c = false;
                        return;
                      } else {
                        this.field_g = 0;
                        this.field_b = true;
                        this.field_c = false;
                        return;
                      }
                    } else {
                      return;
                    }
                  } else {
                    return;
                  }
                } else {
                  this.b(true);
                  if (0 == this.field_f) {
                    if (-100 == (gf.field_k ^ -1)) {
                      this.field_g = this.field_g + 1;
                      if (this.field_e > this.field_g) {
                        this.field_b = true;
                        this.field_c = false;
                        return;
                      } else {
                        this.field_g = 0;
                        this.field_b = true;
                        this.field_c = false;
                        return;
                      }
                    } else {
                      return;
                    }
                  } else {
                    return;
                  }
                }
              }
            }
          }
          if (param0) {
            if (0 == this.field_f) {
              if (-100 == (gf.field_k ^ -1)) {
                this.field_g = this.field_g + 1;
                if (this.field_e > this.field_g) {
                  this.field_b = true;
                  this.field_c = false;
                  return;
                } else {
                  this.field_g = 0;
                  this.field_b = true;
                  this.field_c = false;
                  return;
                }
              } else {
                return;
              }
            } else {
              return;
            }
          } else {
            this.b(true);
            if (0 == this.field_f) {
              if (-100 == (gf.field_k ^ -1)) {
                this.field_g = this.field_g + 1;
                if (this.field_e > this.field_g) {
                  this.field_b = true;
                  this.field_c = false;
                  return;
                } else {
                  this.field_g = 0;
                  this.field_b = true;
                  this.field_c = false;
                  return;
                }
              } else {
                return;
              }
            } else {
              return;
            }
          }
        } else {
          this.field_h = gf.field_k;
          if (-1 == (this.field_f ^ -1)) {
            L2: {
              if (98 == gf.field_k) {
                L3: {
                  if (this.field_g <= 0) {
                    this.field_g = this.field_e;
                    break L3;
                  } else {
                    break L3;
                  }
                }
                this.field_c = false;
                this.field_b = true;
                this.field_g = this.field_g - 1;
                break L2;
              } else {
                break L2;
              }
            }
            if (!param0) {
              this.b(true);
              if (0 == this.field_f) {
                if (-100 == (gf.field_k ^ -1)) {
                  this.field_g = this.field_g + 1;
                  if (this.field_e > this.field_g) {
                    this.field_b = true;
                    this.field_c = false;
                    return;
                  } else {
                    this.field_g = 0;
                    this.field_b = true;
                    this.field_c = false;
                    return;
                  }
                } else {
                  return;
                }
              } else {
                return;
              }
            } else {
              if (0 == this.field_f) {
                if (-100 == (gf.field_k ^ -1)) {
                  this.field_g = this.field_g + 1;
                  if (this.field_e > this.field_g) {
                    this.field_b = true;
                    this.field_c = false;
                    return;
                  } else {
                    this.field_g = 0;
                    this.field_b = true;
                    this.field_c = false;
                    return;
                  }
                } else {
                  return;
                }
              } else {
                return;
              }
            }
          } else {
            L4: {
              if (param0) {
                break L4;
              } else {
                this.b(true);
                break L4;
              }
            }
            if (0 == this.field_f) {
              if (-100 == (gf.field_k ^ -1)) {
                this.field_g = this.field_g + 1;
                if (this.field_e > this.field_g) {
                  this.field_b = true;
                  this.field_c = false;
                  return;
                } else {
                  this.field_g = 0;
                  this.field_b = true;
                  this.field_c = false;
                  return;
                }
              } else {
                return;
              }
            } else {
              return;
            }
          }
        }
    }

    final boolean e(int param0) {
        if (param0 == 0) {
          if (this.field_a == 0) {
            if (-85 != (this.field_h ^ -1)) {
              if ((this.field_h ^ -1) == -84) {
                return true;
              } else {
                return false;
              }
            } else {
              return true;
            }
          } else {
            return true;
          }
        } else {
          return true;
        }
    }

    final void a(boolean param0, int param1, int param2, int param3) {
        Object stackIn_1_0 = null;
        Object stackIn_2_0 = null;
        Object stackIn_3_0 = null;
        int stackIn_3_1 = 0;
        Object stackOut_0_0 = null;
        Object stackOut_2_0 = null;
        int stackOut_2_1 = 0;
        Object stackOut_1_0 = null;
        int stackOut_1_1 = 0;
        L0: {
          stackOut_0_0 = this;
          stackIn_2_0 = stackOut_0_0;
          stackIn_1_0 = stackOut_0_0;
          if (!param0) {
            stackOut_2_0 = this;
            stackOut_2_1 = 0;
            stackIn_3_0 = stackOut_2_0;
            stackIn_3_1 = stackOut_2_1;
            break L0;
          } else {
            stackOut_1_0 = this;
            stackOut_1_1 = 1;
            stackIn_3_0 = stackOut_1_0;
            stackIn_3_1 = stackOut_1_1;
            break L0;
          }
        }
        ((jd) (this)).field_c = stackIn_3_1 != 0;
        if (param1 >= 43) {
          this.field_f = 0;
          if (!this.field_c) {
            this.field_g = param3;
            return;
          } else {
            this.field_g = param2;
            return;
          }
        } else {
          return;
        }
    }

    final void a(int param0, int param1, int param2) {
        if (this.field_e > param2) {
          if (param0 >= this.field_e) {
            throw new IllegalArgumentException();
          } else {
            if (param1 < -114) {
              L0: {
                this.field_a = 0;
                this.field_b = false;
                this.field_h = 0;
                if (lf.field_c != 0) {
                  this.field_g = param0;
                  this.field_c = true;
                  this.field_a = lf.field_c;
                  this.field_f = lf.field_c;
                  this.field_i = nb.field_r;
                  break L0;
                } else {
                  break L0;
                }
              }
              if (this.field_f != 0) {
                if (fia.field_t != 0) {
                  L1: {
                    if (-1 <= (this.field_i ^ -1)) {
                      this.field_i = kw.field_f;
                      break L1;
                    } else {
                      break L1;
                    }
                  }
                  L2: {
                    this.field_i = this.field_i - 1;
                    if (lf.field_c != 0) {
                      break L2;
                    } else {
                      if (-1 == (fia.field_t ^ -1)) {
                        this.field_f = 0;
                        break L2;
                      } else {
                        if (this.field_f == 0) {
                          if (this.field_c) {
                            if (param2 < 0) {
                              if (this.field_c) {
                                this.field_g = -1;
                                return;
                              } else {
                                return;
                              }
                            } else {
                              L3: {
                                if (param2 != this.field_g) {
                                  this.field_b = true;
                                  break L3;
                                } else {
                                  break L3;
                                }
                              }
                              this.field_g = param2;
                              this.field_c = true;
                              return;
                            }
                          } else {
                            if (dda.field_f) {
                              L4: {
                                if (param2 < 0) {
                                  if (!this.field_c) {
                                    break L4;
                                  } else {
                                    this.field_g = -1;
                                    break L4;
                                  }
                                } else {
                                  L5: {
                                    if (param2 != this.field_g) {
                                      this.field_b = true;
                                      break L5;
                                    } else {
                                      break L5;
                                    }
                                  }
                                  this.field_g = param2;
                                  this.field_c = true;
                                  break L4;
                                }
                              }
                              return;
                            } else {
                              return;
                            }
                          }
                        } else {
                          return;
                        }
                      }
                    }
                  }
                  if (this.field_f == 0) {
                    if (this.field_c) {
                      if (param2 < 0) {
                        if (this.field_c) {
                          this.field_g = -1;
                          return;
                        } else {
                          return;
                        }
                      } else {
                        L6: {
                          if (param2 != this.field_g) {
                            this.field_b = true;
                            break L6;
                          } else {
                            break L6;
                          }
                        }
                        this.field_g = param2;
                        this.field_c = true;
                        return;
                      }
                    } else {
                      if (dda.field_f) {
                        if (param2 < 0) {
                          if (this.field_c) {
                            this.field_g = -1;
                            return;
                          } else {
                            return;
                          }
                        } else {
                          L7: {
                            if (param2 != this.field_g) {
                              this.field_b = true;
                              break L7;
                            } else {
                              break L7;
                            }
                          }
                          this.field_g = param2;
                          this.field_c = true;
                          return;
                        }
                      } else {
                        return;
                      }
                    }
                  } else {
                    return;
                  }
                } else {
                  L8: {
                    if (lf.field_c != 0) {
                      break L8;
                    } else {
                      if (-1 == (fia.field_t ^ -1)) {
                        this.field_f = 0;
                        break L8;
                      } else {
                        L9: {
                          if (this.field_f != 0) {
                            break L9;
                          } else {
                            L10: {
                              if (this.field_c) {
                                break L10;
                              } else {
                                if (!dda.field_f) {
                                  break L9;
                                } else {
                                  break L10;
                                }
                              }
                            }
                            if (param2 < 0) {
                              if (!this.field_c) {
                                break L9;
                              } else {
                                this.field_g = -1;
                                break L9;
                              }
                            } else {
                              L11: {
                                if (param2 != this.field_g) {
                                  this.field_b = true;
                                  break L11;
                                } else {
                                  break L11;
                                }
                              }
                              this.field_g = param2;
                              this.field_c = true;
                              break L9;
                            }
                          }
                        }
                        return;
                      }
                    }
                  }
                  if (this.field_f == 0) {
                    if (this.field_c) {
                      if (param2 < 0) {
                        if (this.field_c) {
                          this.field_g = -1;
                          return;
                        } else {
                          return;
                        }
                      } else {
                        L12: {
                          if (param2 != this.field_g) {
                            this.field_b = true;
                            break L12;
                          } else {
                            break L12;
                          }
                        }
                        this.field_g = param2;
                        this.field_c = true;
                        return;
                      }
                    } else {
                      if (dda.field_f) {
                        if (param2 < 0) {
                          if (this.field_c) {
                            this.field_g = -1;
                            return;
                          } else {
                            return;
                          }
                        } else {
                          L13: {
                            if (param2 != this.field_g) {
                              this.field_b = true;
                              break L13;
                            } else {
                              break L13;
                            }
                          }
                          this.field_g = param2;
                          this.field_c = true;
                          return;
                        }
                      } else {
                        return;
                      }
                    }
                  } else {
                    return;
                  }
                }
              } else {
                L14: {
                  if (lf.field_c != 0) {
                    break L14;
                  } else {
                    if (-1 == (fia.field_t ^ -1)) {
                      this.field_f = 0;
                      break L14;
                    } else {
                      L15: {
                        if (this.field_f != 0) {
                          break L15;
                        } else {
                          L16: {
                            if (this.field_c) {
                              break L16;
                            } else {
                              if (!dda.field_f) {
                                break L15;
                              } else {
                                break L16;
                              }
                            }
                          }
                          if (param2 < 0) {
                            if (!this.field_c) {
                              break L15;
                            } else {
                              this.field_g = -1;
                              break L15;
                            }
                          } else {
                            L17: {
                              if (param2 != this.field_g) {
                                this.field_b = true;
                                break L17;
                              } else {
                                break L17;
                              }
                            }
                            this.field_g = param2;
                            this.field_c = true;
                            break L15;
                          }
                        }
                      }
                      return;
                    }
                  }
                }
                if (this.field_f == 0) {
                  if (this.field_c) {
                    if (param2 < 0) {
                      if (this.field_c) {
                        this.field_g = -1;
                        return;
                      } else {
                        return;
                      }
                    } else {
                      L18: {
                        if (param2 != this.field_g) {
                          this.field_b = true;
                          break L18;
                        } else {
                          break L18;
                        }
                      }
                      this.field_g = param2;
                      this.field_c = true;
                      return;
                    }
                  } else {
                    if (dda.field_f) {
                      L19: {
                        if (param2 < 0) {
                          if (!this.field_c) {
                            break L19;
                          } else {
                            this.field_g = -1;
                            break L19;
                          }
                        } else {
                          L20: {
                            if (param2 != this.field_g) {
                              this.field_b = true;
                              break L20;
                            } else {
                              break L20;
                            }
                          }
                          this.field_g = param2;
                          this.field_c = true;
                          break L19;
                        }
                      }
                      return;
                    } else {
                      return;
                    }
                  }
                } else {
                  return;
                }
              }
            } else {
              return;
            }
          }
        } else {
          throw new IllegalArgumentException();
        }
    }

    final boolean c(int param0) {
        int var2 = -34 % ((param0 - 70) / 41);
        return (this.field_h ^ -1) == -97 ? true : false;
    }

    jd(int param0) {
        this.field_c = false;
        this.field_g = 0;
        this.field_e = param0;
    }

    static {
        field_d = new String[]{"Move back to the previous menu level.", "Return to the top level of the menu.", "Auto-respond to the last thing in your chat window.", "Open the Quick Chat menu.", "Repeat the last thing you said.", "Close the Quick Chat menu."};
    }
}
