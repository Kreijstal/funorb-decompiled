/*
 * Decompiled by CFR-JS 0.4.0.
 */
import java.util.*;

final class fha extends at {
    private int field_m;
    static String field_k;
    static String[][] field_j;
    private kj field_l;

    public static void d(byte param0) {
        field_j = null;
        field_k = null;
        if (param0 == 9) {
            return;
        }
        fha.d((byte) 101);
    }

    final boolean c(byte param0) {
        int var2 = 0;
        L0: {
          var2 = 40 % ((71 - param0) / 47);
          if (0 == ((fha) this).field_m) {
            ((fha) this).field_l.a(0);
            break L0;
          } else {
            break L0;
          }
        }
        ((fha) this).field_m = ((fha) this).field_m + 1;
        if (((fha) this).field_m + 1 >= this.d(-18093)) {
          this.e(1);
          return true;
        } else {
          return false;
        }
    }

    final static void a(int param0, int param1, iga param2, boolean param3, boolean param4) {
        int var5 = 0;
        int var6 = 0;
        String var7 = null;
        uka.field_i[0] = ia.field_k.nextInt();
        uka.field_i[1] = ia.field_k.nextInt();
        dm.field_g.field_g = param1;
        uka.field_i[3] = (int)bi.field_j;
        uka.field_i[2] = (int)(bi.field_j >> 1977964704);
        dm.field_g.e(uka.field_i[0], param1 + -1615464796);
        dm.field_g.e(uka.field_i[1], param1 + -1615464796);
        dm.field_g.e(uka.field_i[2], -1615464796);
        dm.field_g.e(uka.field_i[3], -1615464796);
        qja.b(param1 ^ 24, dm.field_g);
        dm.field_g.b(param0, -125);
        param2.a(dm.field_g, false);
        sja.field_fb.field_g = 0;
        if (!param4) {
          L0: {
            sja.field_fb.d(16, 0);
            sja.field_fb.field_g = sja.field_fb.field_g + 2;
            var5 = sja.field_fb.field_g;
            sja.field_fb.e(cp.field_c, param1 + -1615464796);
            sja.field_fb.b(param1 + 1686281208, vi.field_a);
            var6 = 0;
            if (vs.field_c) {
              var6 = var6 | 1;
              break L0;
            } else {
              break L0;
            }
          }
          L1: {
            if (cr.field_b) {
              var6 = var6 | 4;
              break L1;
            } else {
              break L1;
            }
          }
          L2: {
            if (param3) {
              var6 = var6 | 8;
              break L2;
            } else {
              break L2;
            }
          }
          L3: {
            if (ij.field_m != null) {
              var6 = var6 | 16;
              break L3;
            } else {
              break L3;
            }
          }
          L4: {
            sja.field_fb.d(var6, 0);
            var7 = pi.a(11713, nia.d(param1 + -31768));
            if (var7 != null) {
              break L4;
            } else {
              var7 = "";
              break L4;
            }
          }
          sja.field_fb.a((byte) -50, var7);
          if (null == ij.field_m) {
            wd.a(sm.field_q, (byte) -126, (lu) (Object) sja.field_fb, dm.field_g, tg.field_p);
            sja.field_fb.a((byte) -88, sja.field_fb.field_g + -var5);
            hna.a(-1, 85);
            return;
          } else {
            sja.field_fb.a(ij.field_m, 86);
            wd.a(sm.field_q, (byte) -126, (lu) (Object) sja.field_fb, dm.field_g, tg.field_p);
            sja.field_fb.a((byte) -88, sja.field_fb.field_g + -var5);
            hna.a(-1, 85);
            return;
          }
        } else {
          L5: {
            sja.field_fb.d(18, param1 ^ 0);
            sja.field_fb.field_g = sja.field_fb.field_g + 2;
            var5 = sja.field_fb.field_g;
            sja.field_fb.e(cp.field_c, param1 + -1615464796);
            sja.field_fb.b(param1 + 1686281208, vi.field_a);
            var6 = 0;
            if (vs.field_c) {
              var6 = var6 | 1;
              break L5;
            } else {
              break L5;
            }
          }
          L6: {
            if (cr.field_b) {
              var6 = var6 | 4;
              break L6;
            } else {
              break L6;
            }
          }
          L7: {
            if (param3) {
              var6 = var6 | 8;
              break L7;
            } else {
              break L7;
            }
          }
          L8: {
            if (ij.field_m != null) {
              var6 = var6 | 16;
              break L8;
            } else {
              break L8;
            }
          }
          L9: {
            sja.field_fb.d(var6, 0);
            var7 = pi.a(11713, nia.d(param1 + -31768));
            if (var7 != null) {
              break L9;
            } else {
              var7 = "";
              break L9;
            }
          }
          sja.field_fb.a((byte) -50, var7);
          if (null == ij.field_m) {
            wd.a(sm.field_q, (byte) -126, (lu) (Object) sja.field_fb, dm.field_g, tg.field_p);
            sja.field_fb.a((byte) -88, sja.field_fb.field_g + -var5);
            hna.a(-1, 85);
            return;
          } else {
            sja.field_fb.a(ij.field_m, 86);
            wd.a(sm.field_q, (byte) -126, (lu) (Object) sja.field_fb, dm.field_g, tg.field_p);
            sja.field_fb.a((byte) -88, sja.field_fb.field_g + -var5);
            hna.a(-1, 85);
            return;
          }
        }
    }

    private final void e(int param0) {
        if (param0 == 1) {
            return;
        }
        ((fha) this).field_l = null;
    }

    private final int d(int param0) {
        if (param0 != -18093) {
            return -20;
        }
        return 10;
    }

    fha(kj param0) {
        ((fha) this).field_l = param0;
    }

    final static void a(int param0, int param1, int param2, int param3, int param4, int param5, int param6, int param7) {
        int var8 = 0;
        int var9 = 0;
        int var10 = 0;
        int var11 = 0;
        int var12 = 0;
        int var13 = 0;
        int var14 = 0;
        int var15 = 0;
        int var16 = 0;
        int var18 = 0;
        int[] var32 = null;
        int[] var37 = null;
        var18 = BachelorFridge.field_y;
        var12 = cr.a(an.field_q, param7, ha.field_n, 0);
        var13 = cr.a(an.field_q, param2, ha.field_n, 0);
        var14 = cr.a(eo.field_l, param4, pw.field_x, 0);
        if (param1 >= 43) {
          var15 = cr.a(eo.field_l, param0, pw.field_x, 0);
          var8 = cr.a(an.field_q, param6 + param7, ha.field_n, 0);
          var9 = cr.a(an.field_q, param2 - param6, ha.field_n, 0);
          var16 = var12;
          L0: while (true) {
            if (var8 <= var16) {
              var16 = var13;
              L1: while (true) {
                if (var16 <= var9) {
                  var10 = cr.a(eo.field_l, param6 + param4, pw.field_x, 0);
                  var11 = cr.a(eo.field_l, param0 - param6, pw.field_x, 0);
                  var16 = var8;
                  L2: while (true) {
                    if (var9 >= var16) {
                      var32 = tj.field_b[var16];
                      hba.a(param3, var14, var32, 7, var10);
                      hba.a(param5, var10, var32, 7, var11);
                      hba.a(param3, var11, var32, 7, var15);
                      var16++;
                      continue L2;
                    } else {
                      return;
                    }
                  }
                } else {
                  hba.a(param3, var14, tj.field_b[var16], 7, var15);
                  var16--;
                  continue L1;
                }
              }
            } else {
              hba.a(param3, var14, tj.field_b[var16], 7, var15);
              var16++;
              continue L0;
            }
          }
        } else {
          field_j = null;
          var15 = cr.a(eo.field_l, param0, pw.field_x, 0);
          var8 = cr.a(an.field_q, param6 + param7, ha.field_n, 0);
          var9 = cr.a(an.field_q, param2 - param6, ha.field_n, 0);
          var16 = var12;
          L3: while (true) {
            if (var8 <= var16) {
              var16 = var13;
              L4: while (true) {
                if (var16 <= var9) {
                  var10 = cr.a(eo.field_l, param6 + param4, pw.field_x, 0);
                  var11 = cr.a(eo.field_l, param0 - param6, pw.field_x, 0);
                  var16 = var8;
                  L5: while (true) {
                    if (var9 >= var16) {
                      var37 = tj.field_b[var16];
                      hba.a(param3, var14, var37, 7, var10);
                      hba.a(param5, var10, var37, 7, var11);
                      hba.a(param3, var11, var37, 7, var15);
                      var16++;
                      continue L5;
                    } else {
                      return;
                    }
                  }
                } else {
                  hba.a(param3, var14, tj.field_b[var16], 7, var15);
                  var16--;
                  continue L4;
                }
              }
            } else {
              hba.a(param3, var14, tj.field_b[var16], 7, var15);
              var16++;
              continue L3;
            }
          }
        }
    }

    static {
        $cfr$clinit();
    }

    private static void $cfr$clinit() {
        field_k = "Names can only contain letters, numbers, spaces and underscores";
    }
}
