/*
 * Decompiled by CFR-JS 0.4.0.
 */
final class lfa extends k {
    static int field_q;
    private int field_m;
    static sna field_l;
    static kv field_u;
    static ee field_s;
    private int field_p;
    private int field_n;
    private int field_v;
    private int field_t;
    private int field_k;
    private int field_r;
    static int field_o;

    public static void e(byte param0) {
        field_s = null;
        field_l = null;
        if (param0 != -128) {
            return;
        }
        field_u = null;
    }

    private final boolean b(int param0, int param1, int param2) {
        int var4;
        int var5;
        int stackIn_6_0 = 0;
        var4 = this.field_n * (param1 - param2) >> 774652908;
        var5 = am.field_R[(1047373 & var4 * 255) >> -1436660628];
        var5 = (var5 << 742446028) / this.field_n;
        if (param0 == 0) {
          L0: {
            L1: {
              var5 = (var5 << 1318065004) / this.field_p;
              var5 = var5 * this.field_m >> -1049884916;
              if (var5 <= param1 + param2) {
                break L1;
              } else {
                if (param2 - -param1 <= -var5) {
                  break L1;
                } else {
                  stackIn_6_0 = 1;
                  break L0;
                }
              }
            }
            stackIn_6_0 = 0;
            break L0;
          }
          return stackIn_6_0 != 0;
        } else {
          return true;
        }
    }

    final static boolean d(int param0) {
        int stackIn_18_0 = 0;
        int stackIn_31_0 = 0;
        int stackIn_37_0 = 0;
        int stackIn_45_0 = 0;
        int stackIn_51_0 = 0;
        int stackIn_61_0 = 0;
        int stackIn_65_0 = 0;
        int stackIn_69_0 = 0;
        int stackIn_73_0 = 0;
        int stackIn_82_0 = 0;
        int stackIn_88_0 = 0;
        int stackIn_91_0 = 0;
        int stackIn_95_0 = 0;
        int stackIn_101_0 = 0;
        int stackIn_107_0 = 0;
        int stackIn_109_0 = 0;
        int decompiledRegionSelector0 = 0;
        RuntimeException decompiledCaughtException = null;
        Object var1 = null;
        int var1_int = 0;
        int var2 = 0;
        int var3_int = 0;
        op var3 = null;
        int var4_int = 0;
        tk[] var4 = null;
        int var5 = 0;
        int var6 = 0;
        String[] var6_ref_String__ = null;
        int var7 = 0;
        int var8_int = 0;
        op var8 = null;
        int var9 = 0;
        Object var10 = null;
        int var11_int = 0;
        gj var11 = null;
        int var12 = 0;
        int var13 = 0;
        kv var14 = null;
        rk[] var15 = null;
        var13 = BachelorFridge.field_y;
        try {
          L0: {
            L1: {
              if ((ri.field_a ^ -1) == -58) {
                break L1;
              } else {
                if ((ri.field_a ^ -1) != -59) {
                  L2: {
                    var1 = null;
                    if (param0 <= -30) {
                      break L2;
                    } else {
                      field_l = (sna) null;
                      break L2;
                    }
                  }
                  L3: {
                    if (vs.field_d) {
                      var1 = dj.field_c;
                      break L3;
                    } else {
                      if (gja.field_n) {
                        var1 = sna.field_fb;
                        break L3;
                      } else {
                        break L3;
                      }
                    }
                  }
                  if (var1 != null) {
                    if (-74 == (ri.field_a ^ -1)) {
                      ((gj) (var1)).field_h.a(-2, ig.field_m);
                      ((gj) (var1)).l(-67);
                      ((gj) (var1)).d((byte) 77);
                      stackIn_31_0 = 1;
                      decompiledRegionSelector0 = 1;
                      break L0;
                    } else {
                      if (-75 != (ri.field_a ^ -1)) {
                        if ((ri.field_a ^ -1) == -61) {
                          L4: {
                            if (var1 != null) {
                              L5: {
                                var2 = ig.field_m.e((byte) 66);
                                if (-2 != (ig.field_m.b(16711935) ^ -1)) {
                                  stackIn_45_0 = 0;
                                  break L5;
                                } else {
                                  stackIn_45_0 = 1;
                                  break L5;
                                }
                              }
                              L6: {
                                var3_int = stackIn_45_0;
                                var4_int = ig.field_m.b(true);
                                var5 = ig.field_m.b(16711935);
                                var6 = ig.field_m.e((byte) 127);
                                if (0 == ((gj) (var1)).field_h.field_A) {
                                  if (0 != var2) {
                                    stackIn_51_0 = 1;
                                    break L6;
                                  } else {
                                    stackIn_51_0 = 0;
                                    break L6;
                                  }
                                } else {
                                  stackIn_51_0 = 0;
                                  break L6;
                                }
                              }
                              L7: {
                                var7 = stackIn_51_0;
                                ((gj) (var1)).field_h.a(var2, -16292, var3_int != 0);
                                ((gj) (var1)).field_h.field_Q = var5;
                                ((gj) (var1)).field_h.field_M = var4_int;
                                if (((gj) (var1)).field_h.field_o != 2) {
                                  break L7;
                                } else {
                                  ((gj) (var1)).c(640, var4_int, var5);
                                  break L7;
                                }
                              }
                              L8: {
                                if (var7 != 0) {
                                  ((gj) (var1)).l(-68);
                                  break L8;
                                } else {
                                  break L8;
                                }
                              }
                              L9: {
                                ((gj) (var1)).field_M = 0;
                                ((gj) (var1)).field_p = -500 + var6;
                                ((gj) (var1)).field_k = var6;
                                ((gj) (var1)).field_m = true;
                                ((gj) (var1)).field_Y = new sfa[7];
                                ((gj) (var1)).field_u = var6;
                                if (!((gj) (var1)).field_h.field_N) {
                                  ((gj) (var1)).f((byte) 50);
                                  break L9;
                                } else {
                                  break L9;
                                }
                              }
                              ((gj) (var1)).m(10);
                              break L4;
                            } else {
                              break L4;
                            }
                          }
                          stackIn_61_0 = 1;
                          decompiledRegionSelector0 = 3;
                          break L0;
                        } else {
                          if ((ri.field_a ^ -1) == -62) {
                            stackIn_65_0 = 1;
                            decompiledRegionSelector0 = 4;
                            break L0;
                          } else {
                            if (75 == ri.field_a) {
                              var2 = ig.field_m.e((byte) 121);
                              ((gj) (var1)).field_u = var2;
                              ((gj) (var1)).field_p = var2 + -500;
                              ((gj) (var1)).c(false);
                              stackIn_69_0 = 1;
                              decompiledRegionSelector0 = 5;
                              break L0;
                            } else {
                              if ((ri.field_a ^ -1) == -85) {
                                stackIn_73_0 = 1;
                                decompiledRegionSelector0 = 6;
                                break L0;
                              } else {
                                if ((ri.field_a ^ -1) == -63) {
                                  var2 = ig.field_m.b(true);
                                  var3 = ((gj) (var1)).field_h;
                                  var4 = new tk[var3.field_d];
                                  var5 = 0;
                                  L10: while (true) {
                                    if (var3.field_d <= var5) {
                                      L11: {
                                        var3.a(true, var2);
                                        var3.field_H = var4;
                                        if (0 != var3.field_A) {
                                          break L11;
                                        } else {
                                          var14 = (kv) null;
                                          eq.a((kv) null, (gj) (var1), (byte) -111);
                                          sia.field_h = false;
                                          lg.a(true, 3, false);
                                          break L11;
                                        }
                                      }
                                      stackIn_82_0 = 1;
                                      decompiledRegionSelector0 = 7;
                                      break L0;
                                    } else {
                                      var4[var5] = ika.a((byte) 85, ig.field_m);
                                      var5++;
                                      continue L10;
                                    }
                                  }
                                } else {
                                  if ((ri.field_a ^ -1) != -64) {
                                    if (ri.field_a != 64) {
                                      if (ri.field_a == 65) {
                                        ((gj) (var1)).field_h.field_s = ig.field_m.b(16711935);
                                        stackIn_95_0 = 1;
                                        decompiledRegionSelector0 = 10;
                                        break L0;
                                      } else {
                                        if (ri.field_a != 66) {
                                          if (-68 != (ri.field_a ^ -1)) {
                                            stackIn_109_0 = 0;
                                            decompiledRegionSelector0 = 13;
                                            break L0;
                                          } else {
                                            L12: {
                                              ((gj) (var1)).field_V = ig.field_m.b(16711935);
                                              if (vs.field_d) {
                                                hia.c((byte) -122);
                                                break L12;
                                              } else {
                                                break L12;
                                              }
                                            }
                                            stackIn_107_0 = 1;
                                            decompiledRegionSelector0 = 12;
                                            break L0;
                                          }
                                        } else {
                                          L13: {
                                            ((gj) (var1)).field_h.field_f = ig.field_m.b(16711935);
                                            if (vs.field_d) {
                                              hia.c((byte) -109);
                                              break L13;
                                            } else {
                                              break L13;
                                            }
                                          }
                                          stackIn_101_0 = 1;
                                          decompiledRegionSelector0 = 11;
                                          break L0;
                                        }
                                      }
                                    } else {
                                      ((gj) (var1)).field_h.field_v = ig.field_m.b(16711935);
                                      stackIn_91_0 = 1;
                                      decompiledRegionSelector0 = 9;
                                      break L0;
                                    }
                                  } else {
                                    L14: {
                                      ((gj) (var1)).field_h.field_h = ig.field_m.b(16711935);
                                      if (vs.field_d) {
                                        qi.a((byte) 120);
                                        break L14;
                                      } else {
                                        break L14;
                                      }
                                    }
                                    stackIn_88_0 = 1;
                                    decompiledRegionSelector0 = 8;
                                    break L0;
                                  }
                                }
                              }
                            }
                          }
                        }
                      } else {
                        L15: {
                          ((gj) (var1)).field_h.a(dw.field_f, true, ig.field_m);
                          if (!((gj) (var1)).field_h.field_U.e(12917)) {
                            ((gj) (var1)).field_g = ((cj) ((Object) ((gj) (var1)).field_h.field_U.a(6))).field_j;
                            break L15;
                          } else {
                            break L15;
                          }
                        }
                        ((gj) (var1)).field_m = false;
                        ((gj) (var1)).b((byte) -117);
                        ((gj) (var1)).field_cb = false;
                        ((gj) (var1)).field_e.a((byte) 75);
                        stackIn_37_0 = 1;
                        decompiledRegionSelector0 = 2;
                        break L0;
                      }
                    }
                  } else {
                    return false;
                  }
                } else {
                  break L1;
                }
              }
            }
            baa.e(17035);
            hba.a(true, wha.field_a);
            gs.field_f = null;
            var1_int = ig.field_m.b(16711935);
            var2 = ig.field_m.b(16711935);
            var3_int = ig.field_m.b(16711935);
            var4_int = ig.field_m.b(16711935);
            var5 = ig.field_m.b(16711935);
            var6_ref_String__ = new String[ig.field_m.b(16711935)];
            var7 = ig.field_m.b(true);
            var8_int = 0;
            L16: while (true) {
              if (var6_ref_String__.length <= var8_int) {
                L17: {
                  var8 = new op(var1_int, var2, var3_int, var4_int, var5, var6_ref_String__);
                  var9 = ig.field_m.e((byte) 80);
                  var8.a(-2, ig.field_m);
                  var10 = null;
                  if (var8.field_A != 0) {
                    break L17;
                  } else {
                    if (var7 < 0) {
                      break L17;
                    } else {
                      var11_int = ig.field_m.b(16711935);
                      var15 = new rk[var11_int];
                      var10 = var15;
                      var12 = 0;
                      L18: while (true) {
                        if (var12 >= var11_int) {
                          break L17;
                        } else {
                          var15[var12] = gka.a(ig.field_m, 255);
                          var12++;
                          continue L18;
                        }
                      }
                    }
                  }
                }
                L19: {
                  var11 = new gj(var8, var7, (rk[]) (var10), true);
                  var11.field_g = ig.field_m.f(51);
                  if (var8.field_A == 0) {
                    break L19;
                  } else {
                    ala.a(26219, iq.field_m);
                    var11.l(-60);
                    var11.field_k = var9;
                    var11.field_p = -500 + var9;
                    var11.field_u = var9;
                    var11.field_m = true;
                    break L19;
                  }
                }
                L20: {
                  var11.n(-57);
                  if (-58 == (ri.field_a ^ -1)) {
                    vs.field_d = true;
                    dj.field_c = var11;
                    hia.c((byte) -119);
                    qi.a((byte) 122);
                    dna.f((byte) -12);
                    break L20;
                  } else {
                    sna.field_fb = var11;
                    gja.field_n = true;
                    break L20;
                  }
                }
                pha.field_a = false;
                sia.field_h = true;
                kaa.field_m = -1;
                dh.a((byte) 58, var2);
                stackIn_18_0 = 1;
                decompiledRegionSelector0 = 0;
                break L0;
              } else {
                var6_ref_String__[var8_int] = ig.field_m.i(0);
                var8_int++;
                continue L16;
              }
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          var1 = decompiledCaughtException;
          throw pe.a((Throwable) (var1), "lfa.D(" + param0 + ')');
        }
        if (decompiledRegionSelector0 == 0) {
          return stackIn_18_0 != 0;
        } else {
          if (decompiledRegionSelector0 == 1) {
            return stackIn_31_0 != 0;
          } else {
            if (decompiledRegionSelector0 == 2) {
              return stackIn_37_0 != 0;
            } else {
              if (decompiledRegionSelector0 == 3) {
                return stackIn_61_0 != 0;
              } else {
                if (decompiledRegionSelector0 == 4) {
                  return stackIn_65_0 != 0;
                } else {
                  if (decompiledRegionSelector0 == 5) {
                    return stackIn_69_0 != 0;
                  } else {
                    if (decompiledRegionSelector0 == 6) {
                      return stackIn_73_0 != 0;
                    } else {
                      if (decompiledRegionSelector0 == 7) {
                        return stackIn_82_0 != 0;
                      } else {
                        if (decompiledRegionSelector0 == 8) {
                          return stackIn_88_0 != 0;
                        } else {
                          if (decompiledRegionSelector0 == 9) {
                            return stackIn_91_0 != 0;
                          } else {
                            if (decompiledRegionSelector0 == 10) {
                              return stackIn_95_0 != 0;
                            } else {
                              if (decompiledRegionSelector0 == 11) {
                                return stackIn_101_0 != 0;
                              } else {
                                if (decompiledRegionSelector0 == 12) {
                                  return stackIn_107_0 != 0;
                                } else {
                                  return stackIn_109_0 != 0;
                                }
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          }
        }
    }

    final static void a(int param0, ht param1, boolean param2, byte param3, jp param4, java.awt.Component param5) {
        try {
            if (param3 <= 25) {
                java.awt.Component var7 = (java.awt.Component) null;
                lfa.a(93, (ht) null, true, (byte) 13, (jp) null, (java.awt.Component) null);
            }
            efa.a(param0, param0, 1024, param2, param1, 19299, param5, param4);
        } catch (RuntimeException runtimeException) {
            throw pe.a((Throwable) ((Object) runtimeException), "lfa.C(" + param0 + ',' + (param1 != null ? "{...}" : "null") + ',' + param2 + ',' + param3 + ',' + (param4 != null ? "{...}" : "null") + ',' + (param5 != null ? "{...}" : "null") + ')');
        }
    }

    final void d(byte param0) {
        gea.a(4231);
        if (param0 != -68) {
            lu var3 = (lu) null;
            this.a((byte) -6, (lu) null, -101);
        }
    }

    final int[] a(int param0, int param1) {
        int stackIn_9_0 = 0;
        int stackIn_12_0 = 0;
        int stackIn_15_0 = 0;
        int stackIn_18_0 = 0;
        int stackIn_21_0 = 0;
        int stackIn_24_0 = 0;
        int stackIn_27_0 = 0;
        int stackIn_30_0 = 0;
        int[] stackIn_33_0 = null;
        int stackIn_33_1 = 0;
        int[] stackIn_34_0 = null;
        int stackIn_34_1 = 0;
        int stackIn_34_2 = 0;
        int[] var3;
        int var4;
        int var5;
        int var6;
        int var7;
        int var8;
        int var9;
        int var10;
        int var11;
        int[] var12;
        var11 = BachelorFridge.field_y;
        if (param1 == 0) {
          L0: {
            var12 = this.field_j.a(param0, -1);
            var3 = var12;
            if (this.field_j.field_m) {
              var4 = tj.field_f[param0] - 2048;
              var5 = 0;
              L1: while (true) {
                if (hh.field_d <= var5) {
                  break L0;
                } else {
                  L2: {
                    var6 = -2048 + jq.field_k[var5];
                    var7 = var6 + this.field_v;
                    if ((var7 ^ -1) <= 2047) {
                      stackIn_9_0 = var7;
                      break L2;
                    } else {
                      stackIn_9_0 = 4096 + var7;
                      break L2;
                    }
                  }
                  L3: {
                    var7 = stackIn_9_0;
                    if (-2049 <= (var7 ^ -1)) {
                      stackIn_12_0 = var7;
                      break L3;
                    } else {
                      stackIn_12_0 = var7 - 4096;
                      break L3;
                    }
                  }
                  L4: {
                    var7 = stackIn_12_0;
                    var8 = this.field_t + var4;
                    if ((var8 ^ -1) > 2047) {
                      stackIn_15_0 = 4096 + var8;
                      break L4;
                    } else {
                      stackIn_15_0 = var8;
                      break L4;
                    }
                  }
                  L5: {
                    var8 = stackIn_15_0;
                    if (2048 < var8) {
                      stackIn_18_0 = var8 - 4096;
                      break L5;
                    } else {
                      stackIn_18_0 = var8;
                      break L5;
                    }
                  }
                  L6: {
                    var8 = stackIn_18_0;
                    var9 = this.field_k + var6;
                    if (-2048 > var9) {
                      stackIn_21_0 = var9 - -4096;
                      break L6;
                    } else {
                      stackIn_21_0 = var9;
                      break L6;
                    }
                  }
                  L7: {
                    var9 = stackIn_21_0;
                    if (2048 >= var9) {
                      stackIn_24_0 = var9;
                      break L7;
                    } else {
                      stackIn_24_0 = -4096 + var9;
                      break L7;
                    }
                  }
                  L8: {
                    var9 = stackIn_24_0;
                    var10 = this.field_r + var4;
                    if ((var10 ^ -1) <= 2047) {
                      stackIn_27_0 = var10;
                      break L8;
                    } else {
                      stackIn_27_0 = 4096 + var10;
                      break L8;
                    }
                  }
                  L9: {
                    var10 = stackIn_27_0;
                    if (2048 >= var10) {
                      stackIn_30_0 = var10;
                      break L9;
                    } else {
                      stackIn_30_0 = var10 + -4096;
                      break L9;
                    }
                  }
                  L10: {
                    L11: {
                      var10 = stackIn_30_0;
                      stackIn_33_0 = (int[]) (var12);

                      stackIn_33_1 = var5;

                      if (this.b(0, var8, var7)) {
                        break L11;
                      } else {
                        stackIn_33_0 = (int[]) ((Object) stackIn_33_0);

                        if (this.a(var10, var9, -106)) {
                          break L11;
                        } else {
                          stackIn_34_0 = (int[]) ((Object) stackIn_33_0);
                          stackIn_34_1 = stackIn_33_1;
                          stackIn_34_2 = 0;
                          break L10;
                        }
                      }
                    }
                    stackIn_34_0 = (int[]) ((Object) stackIn_33_0);
                    stackIn_34_1 = stackIn_33_1;
                    stackIn_34_2 = 4096;
                    break L10;
                  }
                  stackIn_34_0[stackIn_34_1] = stackIn_34_2;
                  var5++;
                  continue L1;
                }
              }
            } else {
              break L0;
            }
          }
          return var12;
        } else {
          return (int[]) null;
        }
    }

    public lfa() {
        super(0, true);
        this.field_m = 4096;
        this.field_t = 0;
        this.field_k = 0;
        this.field_r = 2048;
        this.field_p = 8192;
        this.field_v = 2048;
        this.field_n = 12288;
    }

    final void a(byte param0, lu param1, int param2) {
        int var4_int = 0;
        int var5 = 0;
        RuntimeException stackIn_21_0 = null;
        StringBuilder stackIn_21_1 = null;
        RuntimeException stackIn_22_0 = null;
        StringBuilder stackIn_22_1 = null;
        String stackIn_22_2 = null;
        RuntimeException decompiledCaughtException = null;
        RuntimeException var4 = null;
        var5 = BachelorFridge.field_y;
        try {
          L0: {
            L1: {
              if (param0 == -71) {
                break L1;
              } else {
                this.field_v = -29;
                break L1;
              }
            }
            L2: {
              var4_int = param2;
              if (-1 == (var4_int ^ -1)) {
                this.field_v = param1.e((byte) 83);
                break L2;
              } else {
                if ((var4_int ^ -1) == -2) {
                  this.field_t = param1.e((byte) 102);
                  break L2;
                } else {
                  if (-3 == (var4_int ^ -1)) {
                    this.field_k = param1.e((byte) 115);
                    break L2;
                  } else {
                    if ((var4_int ^ -1) == -4) {
                      this.field_r = param1.e((byte) 67);
                      break L2;
                    } else {
                      if ((var4_int ^ -1) == -5) {
                        this.field_n = param1.e((byte) 45);
                        break L2;
                      } else {
                        if ((var4_int ^ -1) == -6) {
                          this.field_m = param1.e((byte) 118);
                          break L2;
                        } else {
                          if (6 == var4_int) {
                            this.field_p = param1.e((byte) 70);
                            break L2;
                          } else {
                            break L2;
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
            break L0;
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L3: {
            var4 = decompiledCaughtException;
            stackIn_21_0 = (RuntimeException) (var4);

            stackIn_21_1 = new StringBuilder().append("lfa.E(").append(param0).append(',');

            if (param1 == null) {
              stackIn_22_0 = (RuntimeException) ((Object) stackIn_21_0);
              stackIn_22_1 = (StringBuilder) ((Object) stackIn_21_1);
              stackIn_22_2 = "null";
              break L3;
            } else {
              stackIn_22_0 = (RuntimeException) ((Object) stackIn_21_0);
              stackIn_22_1 = (StringBuilder) ((Object) stackIn_21_1);
              stackIn_22_2 = "{...}";
              break L3;
            }
          }
          throw pe.a((Throwable) ((Object) stackIn_22_0), stackIn_22_2 + ',' + param2 + ')');
        }
    }

    private final boolean a(int param0, int param1, int param2) {
        int var4;
        int var5;
        lu var6;
        int stackIn_6_0 = 0;
        L0: {
          var4 = this.field_n * (param0 - -param1) >> -717313524;
          var5 = am.field_R[255 & var4 * 255 >> -2071325172];
          var5 = (var5 << 4658476) / this.field_n;
          if (param2 <= -29) {
            break L0;
          } else {
            var6 = (lu) null;
            this.a((byte) -2, (lu) null, 90);
            break L0;
          }
        }
        L1: {
          L2: {
            var5 = (var5 << -2087017620) / this.field_p;
            var5 = this.field_m * var5 >> 657207596;
            if (var5 <= param0 - param1) {
              break L2;
            } else {
              if (-var5 >= -param1 + param0) {
                break L2;
              } else {
                stackIn_6_0 = 1;
                break L1;
              }
            }
          }
          stackIn_6_0 = 0;
          break L1;
        }
        return stackIn_6_0 != 0;
    }

    static {
        field_o = 10;
    }
}
