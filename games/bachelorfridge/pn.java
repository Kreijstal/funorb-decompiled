/*
 * Decompiled by CFR-JS 0.4.0.
 */
abstract class pn {
    static String field_e;
    static int field_d;
    static int field_f;
    private pa[] field_c;
    ee[] field_b;
    kv[] field_a;

    private final ee a(int param0, byte param1) {
        pa discarded$0 = null;
        if (param1 != -78) {
            kv var4 = (kv) null;
            discarded$0 = this.a((kv) null, (kv) null, false, 60);
        }
        if (param0 < 0) {
            return null;
        }
        if ((param0 ^ -1) < -32) {
            return null;
        }
        return this.field_b[param0 - -120];
    }

    private final ee b(int param0, int param1, int param2) {
        if (-6 > (param1 ^ -1)) {
            return null;
        }
        int var4 = -127 / ((param2 - -49) / 34);
        if (param0 == 0) {
            return nca.field_a[param1];
        }
        if (-2 == (param0 ^ -1)) {
            return nca.field_a[32 + param1];
        }
        if (!(-3 != (param0 ^ -1))) {
            return nca.field_a[96 + param1];
        }
        return null;
    }

    final void a(jo param0, byte param1) {
        RuntimeException stackIn_31_0 = null;
        StringBuilder stackIn_31_1 = null;
        RuntimeException stackIn_32_0 = null;
        StringBuilder stackIn_32_1 = null;
        String stackIn_32_2 = null;
        RuntimeException decompiledCaughtException = null;
        RuntimeException var3 = null;
        int var4 = 0;
        int var5 = 0;
        int var6 = 0;
        aj var7 = null;
        aj var8 = null;
        aj var9 = null;
        var6 = BachelorFridge.field_y;
        try {
          L0: {
            L1: {
              if (-1 == param0.field_F) {
                break L1;
              } else {
                this.a(param0, -1);
                break L1;
              }
            }
            var4 = 0;
            L2: while (true) {
              if (param0.field_z <= var4) {
                var4 = 0;
                L3: while (true) {
                  if (var4 >= 2 + param0.field_z) {
                    var4 = 0;
                    L4: while (true) {
                      if (var4 >= param0.field_z) {
                        L5: {
                          if (param1 >= 97) {
                            break L5;
                          } else {
                            this.field_c = (pa[]) null;
                            break L5;
                          }
                        }
                        break L0;
                      } else {
                        var5 = 0;
                        L6: while (true) {
                          if (param0.field_a <= var5) {
                            var4++;
                            continue L4;
                          } else {
                            L7: {
                              var9 = param0.a(var4, (byte) 48, var5);
                              if (0 == (param0.field_I[var5][var4].field_a ^ -1)) {
                                break L7;
                              } else {
                                if (param0.field_I[var5][var4].field_d) {
                                  break L7;
                                } else {
                                  this.a(var4, var9, var5, (byte) -35, param0);
                                  break L7;
                                }
                              }
                            }
                            this.a(var5, 79, var4, param0, var9.field_c, var9.field_a);
                            var5++;
                            continue L6;
                          }
                        }
                      }
                    }
                  } else {
                    var5 = 0;
                    L8: while (true) {
                      if (var5 >= 2 + param0.field_a) {
                        var4++;
                        continue L3;
                      } else {
                        L9: {
                          var8 = param0.a(var4, (byte) -103, var5);
                          if ((param0.field_j[var5][var4].field_n ^ -1) != -2) {
                            break L9;
                          } else {
                            this.a(var5, false, var8.field_c, var4, param0, -32 + var8.field_a);
                            break L9;
                          }
                        }
                        var5++;
                        continue L8;
                      }
                    }
                  }
                }
              } else {
                var5 = 0;
                L10: while (true) {
                  if (var5 >= param0.field_a) {
                    var4++;
                    continue L2;
                  } else {
                    var7 = param0.a(var4, (byte) 45, var5);
                    this.a(var5, var4, param0, var7.field_a, var7.field_c, false);
                    var5++;
                    continue L10;
                  }
                }
              }
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L11: {
            var3 = decompiledCaughtException;
            stackIn_31_0 = (RuntimeException) (var3);

            stackIn_31_1 = new StringBuilder().append("pn.H(");

            if (param0 == null) {
              stackIn_32_0 = (RuntimeException) ((Object) stackIn_31_0);
              stackIn_32_1 = (StringBuilder) ((Object) stackIn_31_1);
              stackIn_32_2 = "null";
              break L11;
            } else {
              stackIn_32_0 = (RuntimeException) ((Object) stackIn_31_0);
              stackIn_32_1 = (StringBuilder) ((Object) stackIn_31_1);
              stackIn_32_2 = "{...}";
              break L11;
            }
          }
          throw pe.a((Throwable) ((Object) stackIn_32_0), stackIn_32_2 + ',' + param1 + ')');
        }
    }

    private final void a(jo param0, int param1) {
        int var3_int = 0;
        ee[] var4 = null;
        int var5 = 0;
        int var6 = 0;
        kv var7 = null;
        kv var8 = null;
        int var9 = 0;
        kv stackIn_16_0 = null;
        RuntimeException stackIn_20_0 = null;
        StringBuilder stackIn_20_1 = null;
        RuntimeException stackIn_21_0 = null;
        StringBuilder stackIn_21_1 = null;
        String stackIn_21_2 = null;
        RuntimeException decompiledCaughtException = null;
        RuntimeException var3 = null;
        var9 = BachelorFridge.field_y;
        try {
          L0: {
            L1: {
              var3_int = 0;
              if (1 == param0.field_F) {
                wf.a(54, (byte) -57);
                break L1;
              } else {
                break L1;
              }
            }
            L2: {
              if ((param0.field_F ^ -1) < -81) {
                var3_int = 8 * (-80 + param0.field_F);
                if (-256 > (var3_int ^ -1)) {
                  var3_int = 255;
                  break L2;
                } else {
                  break L2;
                }
              } else {
                break L2;
              }
            }
            L3: {
              var4 = iw.field_b;
              var5 = var4.length;
              this.field_c = new pa[var5];
              if (param1 == -1) {
                break L3;
              } else {
                this.field_a = (kv[]) null;
                break L3;
              }
            }
            var6 = 0;
            L4: while (true) {
              if (var6 >= var5) {
                break L0;
              } else {
                L5: {
                  if (var6 == 0) {
                    stackIn_16_0 = null;
                    break L5;
                  } else {
                    stackIn_16_0 = iw.field_b[var6 + -1].a();
                    break L5;
                  }
                }
                var7 = stackIn_16_0;
                var8 = iw.field_b[var6].a();
                this.field_c[var6] = this.a(var8, var7, false, var3_int);
                var6++;
                continue L4;
              }
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L6: {
            var3 = decompiledCaughtException;
            stackIn_20_0 = (RuntimeException) (var3);

            stackIn_20_1 = new StringBuilder().append("pn.N(");

            if (param0 == null) {
              stackIn_21_0 = (RuntimeException) ((Object) stackIn_20_0);
              stackIn_21_1 = (StringBuilder) ((Object) stackIn_20_1);
              stackIn_21_2 = "null";
              break L6;
            } else {
              stackIn_21_0 = (RuntimeException) ((Object) stackIn_20_0);
              stackIn_21_1 = (StringBuilder) ((Object) stackIn_20_1);
              stackIn_21_2 = "{...}";
              break L6;
            }
          }
          throw pe.a((Throwable) ((Object) stackIn_21_0), stackIn_21_2 + ',' + param1 + ')');
        }
    }

    private final ee a(int param0, int param1) {
        if (-6 > (param1 ^ -1)) {
            return null;
        }
        if (param0 != 21160) {
            return (ee) null;
        }
        return this.field_b[96 + param1];
    }

    private final void a(int param0, aj param1, int param2, byte param3, jo param4) {
        ee var9 = null;
        wia var10 = param4.field_i.field_h.field_a[param2][param0];
        int var7 = var10.field_n;
        if (param3 != -35) {
            field_e = (String) null;
        }
        int var8 = var10.field_j;
        if (37 == var7) {
            return;
        }
        try {
            var9 = this.a(var7, param0, param2, (byte) -122, var8, param4.field_i.field_h.field_E);
            if (!(var9 == null)) {
                var9.a(param1.field_c, param1.field_a, 104);
            }
            this.a(param2, (byte) 100, var8, param0, var7).a(param1.field_c, param1.field_a);
        } catch (RuntimeException runtimeException) {
            throw pe.a((Throwable) ((Object) runtimeException), "pn.R(" + param0 + ',' + (param1 != null ? "{...}" : "null") + ',' + param2 + ',' + param3 + ',' + (param4 != null ? "{...}" : "null") + ')');
        }
    }

    private final ee d(int param0, int param1) {
        if (!(param1 != 0)) {
            return nca.field_a[15];
        }
        int var3 = 57 % ((param0 - -41) / 36);
        if ((param1 ^ -1) == -2) {
            return nca.field_a[63];
        }
        if (param1 != 2) {
            return null;
        }
        return nca.field_a[111];
    }

    private final ee b(int param0, int param1) {
        if (param0 == (param1 ^ -1)) {
            return nca.field_a[14];
        }
        if (!((param1 ^ -1) != -2)) {
            return nca.field_a[62];
        }
        if (!(-3 != (param1 ^ -1))) {
            return nca.field_a[110];
        }
        return null;
    }

    private final ee a(int param0, int param1, int param2, byte param3, int param4, int param5) {
        if ((param0 ^ -1) == -7 || (param0 ^ -1) == -9 || 10 == param0 || -13 == (param0 ^ -1) || -15 == (param0 ^ -1)) {
            return this.b(param5, (param2 + param1 * 2) % 4, -85);
        }
        if (20 == param0) {
            return this.b(-1, param5);
        }
        int var7 = 39 / ((49 - param3) / 58);
        return null;
    }

    void b(jo param0, int param1) {
        ee discarded$0 = null;
        try {
            if (param1 != -13319) {
                discarded$0 = this.a(45, -37, 115, (byte) 34, -102, -47);
            }
        } catch (RuntimeException runtimeException) {
            throw pe.a((Throwable) ((Object) runtimeException), "pn.B(" + (param0 != null ? "{...}" : "null") + ',' + param1 + ')');
        }
    }

    private final void a(int param0, int param1, int param2, jo param3, int param4, int param5) {
        int stackIn_32_0 = 0;
        RuntimeException stackIn_49_0 = null;
        StringBuilder stackIn_49_1 = null;
        RuntimeException stackIn_50_0 = null;
        StringBuilder stackIn_50_1 = null;
        String stackIn_50_2 = null;
        int decompiledRegionSelector0 = 0;
        RuntimeException decompiledCaughtException = null;
        RuntimeException var7 = null;
        int var8 = 0;
        int var9 = 0;
        int var10 = 0;
        pa var11 = null;
        int var11_int = 0;
        int var12 = 0;
        int var13 = 0;
        int var14 = 0;
        int var15 = 0;
        int var16 = 0;
        int var17 = 0;
        int var18 = 0;
        int var19 = 0;
        int var20 = 0;
        int var21 = 0;
        int var22 = 0;
        wia var23 = null;
        var22 = BachelorFridge.field_y;
        try {
          L0: {
            if (param3.field_I[param0][param2].field_d) {
              decompiledRegionSelector0 = 0;
              break L0;
            } else {
              L1: {
                L2: {
                  var8 = 32 / ((-62 - param1) / 45);
                  var23 = param3.field_i.field_h.field_a[param0][param2];
                  var9 = var23.field_n;
                  if (var9 != 21) {
                    break L2;
                  } else {
                    if (param3.field_I[param0][param2].field_i) {
                      break L2;
                    } else {
                      this.field_b[109].a(param4, param5);
                      break L1;
                    }
                  }
                }
                L3: {
                  if ((var9 ^ -1) != -22) {
                    break L3;
                  } else {
                    if (!param3.field_I[param0][param2].field_i) {
                      break L3;
                    } else {
                      this.field_b[108].a(param4, param5);
                      break L1;
                    }
                  }
                }
                if (-23 != (var9 ^ -1)) {
                  L4: {
                    if (-24 == (var9 ^ -1)) {
                      break L4;
                    } else {
                      if (var9 == 24) {
                        break L4;
                      } else {
                        if (25 == var9) {
                          break L4;
                        } else {
                          if (26 != var9) {
                            break L1;
                          } else {
                            break L4;
                          }
                        }
                      }
                    }
                  }
                  this.a(param0, (byte) 83, 0, param2, 20).a(param4, param5);
                  break L1;
                } else {
                  this.d(-79, param3.field_i.field_h.field_E).a(param4, param5, 104);
                  this.field_b[103].a(param4, param5);
                  break L1;
                }
              }
              L5: {
                if (-35 == (var9 ^ -1)) {
                  ge.field_K.a(param4, param5);
                  break L5;
                } else {
                  if ((var9 ^ -1) == -36) {
                    il.field_b.a(param4, param5);
                    break L5;
                  } else {
                    if (-29 == (var9 ^ -1)) {
                      if (var23.field_h >= od.field_w[var9].field_e) {
                        kka.field_f.a(param4, param5 + -64);
                        break L5;
                      } else {
                        hna.field_h.a(param4, param5 + -64);
                        break L5;
                      }
                    } else {
                      if (27 == var9) {
                        L6: {
                          var10 = var23.field_j - 1;
                          if ((var10 ^ -1) > -1) {
                            break L6;
                          } else {
                            L7: {
                              if (iw.field_b.length <= var10) {
                                var10 = iw.field_b.length - 1;
                                break L7;
                              } else {
                                break L7;
                              }
                            }
                            var11 = this.field_c[var10];
                            var11.e(param4, param5);
                            break L6;
                          }
                        }
                        break L5;
                      } else {
                        if ((var9 ^ -1) != -37) {
                          if (30 != var9) {
                            if (-30 == (var9 ^ -1)) {
                              pk.field_b[(param2 + param0) % 7].a(param4, param5);
                              break L5;
                            } else {
                              if (-40 == (var9 ^ -1)) {
                                ip.field_f[0].e(32 + param4, -32 + param5);
                                break L5;
                              } else {
                                if (-38 != (var9 ^ -1)) {
                                  break L5;
                                } else {
                                  L8: {
                                    var10 = (param0 - -(47 * param2)) % 3;
                                    var11_int = (param0 * 342 - -(98 * param2)) % 32;
                                    var12 = (231 * param0 - -(167 * param2)) % 32;
                                    var13 = param4 - (-var11_int - (var12 + -32));
                                    var14 = -var12 + (param5 + var11_int);
                                    var15 = 32 + (1212 * param2 + 4642 * param0 & 31);
                                    var16 = 16 + (31 & param2 * 464 + 57 * param0);
                                    var17 = vr.field_b % var15;
                                    var18 = var17 * (4 * (var16 * (var15 + -var17))) / (var15 * var15);
                                    var19 = 32;
                                    var20 = 24;
                                    if (var17 << 321879937 > var15) {
                                      stackIn_32_0 = 1;
                                      break L8;
                                    } else {
                                      stackIn_32_0 = 0;
                                      break L8;
                                    }
                                  }
                                  var21 = stackIn_32_0;
                                  var20 = var20 - (3 * var18 >> 943059300);
                                  var19 = var19 - (var18 >> 183175522);
                                  nk.a(param3.field_y, -(var19 >> 481161313) + 64 + var13, -(var20 >> 547457217) + 36 + var14, var19, var20);
                                  eka.field_g[var10][var21].a(var13, -var18 + var14);
                                  break L5;
                                }
                              }
                            }
                          } else {
                            ld.field_l[(param2 + param0) % 7].a(param4, param5);
                            break L5;
                          }
                        } else {
                          pfa.field_g.a(param4, -64 + param5);
                          break L5;
                        }
                      }
                    }
                  }
                }
              }
              decompiledRegionSelector0 = 1;
              break L0;
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L9: {
            var7 = decompiledCaughtException;
            stackIn_49_0 = (RuntimeException) (var7);

            stackIn_49_1 = new StringBuilder().append("pn.O(").append(param0).append(',').append(param1).append(',').append(param2).append(',');

            if (param3 == null) {
              stackIn_50_0 = (RuntimeException) ((Object) stackIn_49_0);
              stackIn_50_1 = (StringBuilder) ((Object) stackIn_49_1);
              stackIn_50_2 = "null";
              break L9;
            } else {
              stackIn_50_0 = (RuntimeException) ((Object) stackIn_49_0);
              stackIn_50_1 = (StringBuilder) ((Object) stackIn_49_1);
              stackIn_50_2 = "{...}";
              break L9;
            }
          }
          throw pe.a((Throwable) ((Object) stackIn_50_0), stackIn_50_2 + ',' + param4 + ',' + param5 + ')');
        }
        if (decompiledRegionSelector0 == 0) {
          return;
        } else {
          return;
        }
    }

    private final pa a(kv param0, int param1, int param2) {
        int var4_int = 0;
        RuntimeException var4 = null;
        pa var5 = null;
        int[] var6 = null;
        int var7 = 0;
        int var8 = 0;
        int var9 = 0;
        jo var10 = null;
        pa stackIn_10_0 = null;
        RuntimeException stackIn_13_0 = null;
        StringBuilder stackIn_13_1 = null;
        RuntimeException stackIn_14_0 = null;
        StringBuilder stackIn_14_1 = null;
        String stackIn_14_2 = null;
        RuntimeException decompiledCaughtException = null;
        var9 = BachelorFridge.field_y;
        try {
          L0: {
            L1: {
              param0.g();
              var4_int = param0.field_v.length;
              if (param2 < -75) {
                break L1;
              } else {
                var10 = (jo) null;
                this.a((jo) null, (byte) 45);
                break L1;
              }
            }
            var5 = new pa(param0.field_n, param0.field_o);
            var6 = var5.field_v;
            var7 = 0;
            L2: while (true) {
              if (var4_int <= var7) {
                stackIn_10_0 = (pa) (var5);
                break L0;
              } else {
                L3: {
                  var8 = 16777215 & param0.field_v[var7];
                  if (-1 == (var8 ^ -1)) {
                    var6[var7] = 0;
                    break L3;
                  } else {
                    var6[var7] = mp.a(var8, param1 << -718974088);
                    break L3;
                  }
                }
                var7++;
                continue L2;
              }
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L4: {
            var4 = decompiledCaughtException;
            stackIn_13_0 = (RuntimeException) (var4);

            stackIn_13_1 = new StringBuilder().append("pn.BA(");

            if (param0 == null) {
              stackIn_14_0 = (RuntimeException) ((Object) stackIn_13_0);
              stackIn_14_1 = (StringBuilder) ((Object) stackIn_13_1);
              stackIn_14_2 = "null";
              break L4;
            } else {
              stackIn_14_0 = (RuntimeException) ((Object) stackIn_13_0);
              stackIn_14_1 = (StringBuilder) ((Object) stackIn_13_1);
              stackIn_14_2 = "{...}";
              break L4;
            }
          }
          throw pe.a((Throwable) ((Object) stackIn_14_0), stackIn_14_2 + ',' + param1 + ',' + param2 + ')');
        }
        return stackIn_10_0;
    }

    private final void a(int param0, int param1, jo param2, int param3, int param4, boolean param5) {
        ee discarded$0 = null;
        try {
            if (param5) {
                discarded$0 = this.a(116, -127, -22);
            }
            this.c(26728, (param0 + param1 * (param2.field_a - -2)) % 24).a(param4, param3);
        } catch (RuntimeException runtimeException) {
            throw pe.a((Throwable) ((Object) runtimeException), "pn.K(" + param0 + ',' + param1 + ',' + (param2 != null ? "{...}" : "null") + ',' + param3 + ',' + param4 + ',' + param5 + ')');
        }
    }

    final static String a(byte param0, int param1, String[] param2) {
        int var3_int = 0;
        RuntimeException var3 = null;
        String stackIn_4_0 = null;
        String stackIn_6_0 = null;
        RuntimeException stackIn_9_0 = null;
        StringBuilder stackIn_9_1 = null;
        RuntimeException stackIn_10_0 = null;
        StringBuilder stackIn_10_1 = null;
        String stackIn_10_2 = null;
        int decompiledRegionSelector0 = 0;
        RuntimeException decompiledCaughtException = null;
        try {
          L0: {
            L1: {
              var3_int = -29 % ((param0 - -48) / 46);
              if (0 > param1) {
                break L1;
              } else {
                if (param1 >= param2.length) {
                  break L1;
                } else {
                  if (param2[param1] == null) {
                    break L1;
                  } else {
                    stackIn_4_0 = param2[param1];
                    decompiledRegionSelector0 = 0;
                    break L0;
                  }
                }
              }
            }
            stackIn_6_0 = "Unknown: " + param1;
            decompiledRegionSelector0 = 1;
            break L0;
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L2: {
            var3 = decompiledCaughtException;
            stackIn_9_0 = (RuntimeException) (var3);

            stackIn_9_1 = new StringBuilder().append("pn.W(").append(param0).append(',').append(param1).append(',');

            if (param2 == null) {
              stackIn_10_0 = (RuntimeException) ((Object) stackIn_9_0);
              stackIn_10_1 = (StringBuilder) ((Object) stackIn_9_1);
              stackIn_10_2 = "null";
              break L2;
            } else {
              stackIn_10_0 = (RuntimeException) ((Object) stackIn_9_0);
              stackIn_10_1 = (StringBuilder) ((Object) stackIn_9_1);
              stackIn_10_2 = "{...}";
              break L2;
            }
          }
          throw pe.a((Throwable) ((Object) stackIn_10_0), stackIn_10_2 + ')');
        }
        if (decompiledRegionSelector0 == 0) {
          return stackIn_4_0;
        } else {
          return stackIn_6_0;
        }
    }

    private final pa a(kv param0, kv param1, boolean param2, int param3) {
        pa stackIn_5_0 = null;
        pa stackIn_7_0 = null;
        pa stackIn_10_0 = null;
        pa stackIn_26_0 = null;
        RuntimeException stackIn_29_0 = null;
        StringBuilder stackIn_29_1 = null;
        RuntimeException stackIn_30_0 = null;
        StringBuilder stackIn_30_1 = null;
        String stackIn_30_2 = null;
        StringBuilder stackIn_32_1 = null;
        StringBuilder stackIn_33_1 = null;
        String stackIn_33_2 = null;
        int decompiledRegionSelector0 = 0;
        RuntimeException decompiledCaughtException = null;
        int var5_int = 0;
        RuntimeException var5 = null;
        pa var6 = null;
        int[] var7 = null;
        int var8 = 0;
        int var9 = 0;
        int var10 = 0;
        int var11 = 0;
        int var12 = 0;
        int var13 = 0;
        int var14 = 0;
        int var15 = 0;
        int[] var16 = null;
        int[] var17 = null;
        var15 = BachelorFridge.field_y;
        try {
          L0: {
            if (param1 == null) {
              stackIn_7_0 = this.a(param0, param3, -108);
              decompiledRegionSelector0 = 1;
              break L0;
            } else {
              if (param0 == null) {
                stackIn_5_0 = this.a(param1, 255 + -param3, -115);
                decompiledRegionSelector0 = 0;
                break L0;
              } else {
                param1.g();
                param0.g();
                if (!param2) {
                  var5_int = param1.field_v.length;
                  if (param0.field_v.length != var5_int) {
                    throw new RuntimeException();
                  } else {
                    var6 = new pa(param1.field_n, param1.field_o);
                    var17 = var6.field_v;
                    var16 = var17;
                    var7 = var16;
                    var8 = 0;
                    L1: while (true) {
                      if (var5_int <= var8) {
                        stackIn_26_0 = (pa) (var6);
                        decompiledRegionSelector0 = 3;
                        break L0;
                      } else {
                        L2: {
                          L3: {
                            var9 = param1.field_v[var8] & 16777215;
                            var10 = 16777215 & param0.field_v[var8];
                            if (-1 != (var9 ^ -1)) {
                              break L3;
                            } else {
                              if (var10 != 0) {
                                break L3;
                              } else {
                                var17[var8] = 0;
                                break L2;
                              }
                            }
                          }
                          if (var9 != 0) {
                            if (-1 != (var10 ^ -1)) {
                              var11 = var9 & 16711935;
                              var12 = 65280 & var9;
                              var13 = var10 & 16711935;
                              var12 = var12 * (256 - param3);
                              var14 = var10 & 65280;
                              var13 = var13 * param3;
                              var11 = var11 * (-param3 + 256);
                              var11 = var11 & -16711936;
                              var12 = var12 & 16711680;
                              var13 = var13 & -16711936;
                              var14 = var14 * param3;
                              var14 = var14 & 16711680;
                              var7[var8] = mp.a(mp.a(var11 + var13, var14 + var12) >>> 157908296, -16777216);
                              break L2;
                            } else {
                              var7[var8] = mp.a(-param3 + 255 << -1210775240, var9);
                              break L2;
                            }
                          } else {
                            var7[var8] = mp.a(var10, param3 << 1567358616);
                            break L2;
                          }
                        }
                        var8++;
                        continue L1;
                      }
                    }
                  }
                } else {
                  stackIn_10_0 = (pa) null;
                  decompiledRegionSelector0 = 2;
                  break L0;
                }
              }
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L4: {
            var5 = decompiledCaughtException;
            stackIn_29_0 = (RuntimeException) (var5);

            stackIn_29_1 = new StringBuilder().append("pn.S(");

            if (param0 == null) {
              stackIn_30_0 = (RuntimeException) ((Object) stackIn_29_0);
              stackIn_30_1 = (StringBuilder) ((Object) stackIn_29_1);
              stackIn_30_2 = "null";
              break L4;
            } else {
              stackIn_30_0 = (RuntimeException) ((Object) stackIn_29_0);
              stackIn_30_1 = (StringBuilder) ((Object) stackIn_29_1);
              stackIn_30_2 = "{...}";
              break L4;
            }
          }
          L5: {


            stackIn_32_1 = ((StringBuilder) (Object) stackIn_30_1).append(stackIn_30_2).append(',');

            if (param1 == null) {
              stackIn_30_0 = (RuntimeException) ((Object) stackIn_30_0);
              stackIn_33_1 = (StringBuilder) ((Object) stackIn_32_1);
              stackIn_33_2 = "null";
              break L5;
            } else {
              stackIn_30_0 = (RuntimeException) ((Object) stackIn_30_0);
              stackIn_33_1 = (StringBuilder) ((Object) stackIn_32_1);
              stackIn_33_2 = "{...}";
              break L5;
            }
          }
          throw pe.a((Throwable) ((Object) stackIn_30_0), stackIn_33_2 + ',' + param2 + ',' + param3 + ')');
        }
        if (decompiledRegionSelector0 == 0) {
          return stackIn_5_0;
        } else {
          if (decompiledRegionSelector0 == 1) {
            return stackIn_7_0;
          } else {
            if (decompiledRegionSelector0 == 2) {
              return stackIn_10_0;
            } else {
              return stackIn_26_0;
            }
          }
        }
    }

    private final ee a(int param0, byte param1, int param2, int param3, int param4) {
        int var7 = 0;
        int var6 = 27 / ((-28 - param1) / 40);
        if ((param4 ^ -1) != -7 && (param4 ^ -1) != -9 && 10 != param4 && (param4 ^ -1) != -13 && param4 != 14) {
            if (-8 != (param4 ^ -1) && param4 != 9 && (param4 ^ -1) != -12 && param4 != 13 && param4 != 15) {
                if (20 != param4) {
                    if (38 != param4) {
                        if (param4 != 33) {
                            return null;
                        }
                        return fw.field_f;
                    }
                    var7 = param3 * param0 % 3;
                    return cba.field_c[var7];
                }
                return this.field_b[102];
            }
            return this.a(21160, (param0 + param3 * 2) % 4);
        }
        return this.a((byte) -18, (param0 + 2 * param3) % 4);
    }

    private final void a(int param0, boolean param1, int param2, int param3, jo param4, int param5) {
        Object stackIn_5_0 = null;
        int stackIn_5_1 = 0;
        Object stackIn_6_0 = null;
        int stackIn_6_1 = 0;
        int stackIn_6_2 = 0;
        Object stackIn_11_0 = null;
        int stackIn_11_1 = 0;
        int stackIn_11_2 = 0;
        Object stackIn_12_0 = null;
        int stackIn_12_1 = 0;
        int stackIn_12_2 = 0;
        int stackIn_12_3 = 0;
        Object stackIn_16_0 = null;
        int stackIn_16_1 = 0;
        Object stackIn_17_0 = null;
        int stackIn_17_1 = 0;
        int stackIn_17_2 = 0;
        Object stackIn_19_0 = null;
        int stackIn_19_1 = 0;
        Object stackIn_20_0 = null;
        int stackIn_20_1 = 0;
        int stackIn_20_2 = 0;
        Object stackIn_24_0 = null;
        int stackIn_24_1 = 0;
        Object stackIn_25_0 = null;
        int stackIn_25_1 = 0;
        int stackIn_25_2 = 0;
        Object stackIn_29_0 = null;
        int stackIn_29_1 = 0;
        Object stackIn_30_0 = null;
        int stackIn_30_1 = 0;
        int stackIn_30_2 = 0;
        Object stackIn_34_0 = null;
        int stackIn_34_1 = 0;
        Object stackIn_35_0 = null;
        int stackIn_35_1 = 0;
        int stackIn_35_2 = 0;
        Object stackIn_37_0 = null;
        Object stackIn_38_0 = null;
        int stackIn_38_1 = 0;
        Object stackIn_42_0 = null;
        int stackIn_42_1 = 0;
        int stackIn_42_2 = 0;
        Object stackIn_43_0 = null;
        int stackIn_43_1 = 0;
        int stackIn_43_2 = 0;
        int stackIn_43_3 = 0;
        Object stackIn_48_0 = null;
        Object stackIn_49_0 = null;
        int stackIn_49_1 = 0;
        Object stackIn_52_0 = null;
        int stackIn_52_1 = 0;
        Object stackIn_53_0 = null;
        int stackIn_53_1 = 0;
        int stackIn_53_2 = 0;
        Object stackIn_58_0 = null;
        int stackIn_58_1 = 0;
        Object stackIn_59_0 = null;
        int stackIn_59_1 = 0;
        int stackIn_59_2 = 0;
        RuntimeException stackIn_82_0 = null;
        StringBuilder stackIn_82_1 = null;
        RuntimeException stackIn_83_0 = null;
        StringBuilder stackIn_83_1 = null;
        String stackIn_83_2 = null;
        RuntimeException decompiledCaughtException = null;
        Object var7 = null;
        RuntimeException var7_ref = null;
        int var8 = 0;
        ee var9 = null;
        ee var10 = null;
        ee var11 = null;
        ee var12 = null;
        ee var13 = null;
        ee var14 = null;
        var8 = BachelorFridge.field_y;
        try {
          L0: {
            L1: {
              if (param4.field_j[param0][param3].field_n != 1) {
                break L1;
              } else {
                var7 = null;
                if (param4.field_j[param0][param3].field_k != 5) {
                  if (-11 != (param4.field_j[param0][param3].field_k ^ -1)) {
                    if (-16 == (param4.field_j[param0][param3].field_k ^ -1)) {
                      L2: {
                        stackIn_52_0 = this;

                        stackIn_52_1 = param4.field_i.field_h.field_E;

                        if (!param4.field_j[param0][param3].field_o) {
                          stackIn_53_0 = this;
                          stackIn_53_1 = stackIn_52_1;
                          stackIn_53_2 = 0;
                          break L2;
                        } else {
                          stackIn_53_0 = this;
                          stackIn_53_1 = stackIn_52_1;
                          stackIn_53_2 = 16;
                          break L2;
                        }
                      }
                      L3: {
                        var14 = this.a(stackIn_53_1, stackIn_53_2 + (param0 - -param3) % 4 * 5, 15392);
                        if (var14 != null) {
                          var14.a(param2, param5, 104);
                          break L3;
                        } else {
                          break L3;
                        }
                      }
                      L4: {
                        stackIn_58_0 = this;

                        stackIn_58_1 = 5 * ((param0 - -param3) % 4);

                        if (param4.field_j[param0][param3].field_o) {
                          stackIn_59_0 = this;
                          stackIn_59_1 = stackIn_58_1;
                          stackIn_59_2 = 16;
                          break L4;
                        } else {
                          stackIn_59_0 = this;
                          stackIn_59_1 = stackIn_58_1;
                          stackIn_59_2 = 0;
                          break L4;
                        }
                      }
                      this.a(stackIn_59_1 - -stackIn_59_2, (byte) -78).a(param2, param5);
                      break L1;
                    } else {
                      L5: {
                        stackIn_42_0 = this;

                        stackIn_42_1 = param4.field_i.field_h.field_E;

                        stackIn_42_2 = param4.field_j[param0][param3].field_k;

                        if (!param4.field_j[param0][param3].field_o) {
                          stackIn_43_0 = this;
                          stackIn_43_1 = stackIn_42_1;
                          stackIn_43_2 = stackIn_42_2;
                          stackIn_43_3 = 0;
                          break L5;
                        } else {
                          stackIn_43_0 = this;
                          stackIn_43_1 = stackIn_42_1;
                          stackIn_43_2 = stackIn_42_2;
                          stackIn_43_3 = 16;
                          break L5;
                        }
                      }
                      L6: {
                        var13 = this.a(stackIn_43_1, stackIn_43_2 + stackIn_43_3, 15392);
                        if (var13 != null) {
                          var13.a(param2, param5, 104);
                          break L6;
                        } else {
                          break L6;
                        }
                      }
                      L7: {
                        stackIn_48_0 = this;

                        if (param4.field_j[param0][param3].field_o) {
                          stackIn_49_0 = this;
                          stackIn_49_1 = 16;
                          break L7;
                        } else {
                          stackIn_49_0 = this;
                          stackIn_49_1 = 0;
                          break L7;
                        }
                      }
                      this.a(stackIn_49_1 + param4.field_j[param0][param3].field_k, (byte) -78).a(param2, param5);
                      break L1;
                    }
                  } else {
                    L8: {
                      stackIn_24_0 = this;

                      stackIn_24_1 = param4.field_i.field_h.field_E;

                      if (!param4.field_j[param0][param3].field_o) {
                        stackIn_25_0 = this;
                        stackIn_25_1 = stackIn_24_1;
                        stackIn_25_2 = 0;
                        break L8;
                      } else {
                        stackIn_25_0 = this;
                        stackIn_25_1 = stackIn_24_1;
                        stackIn_25_2 = 16;
                        break L8;
                      }
                    }
                    L9: {
                      var11 = this.a(stackIn_25_1, stackIn_25_2 + 8, 15392);
                      if (var11 == null) {
                        break L9;
                      } else {
                        var11.a(param2, param5, 104);
                        break L9;
                      }
                    }
                    L10: {
                      stackIn_29_0 = this;

                      stackIn_29_1 = param4.field_i.field_h.field_E;

                      if (param4.field_j[param0][param3].field_o) {
                        stackIn_30_0 = this;
                        stackIn_30_1 = stackIn_29_1;
                        stackIn_30_2 = 16;
                        break L10;
                      } else {
                        stackIn_30_0 = this;
                        stackIn_30_1 = stackIn_29_1;
                        stackIn_30_2 = 0;
                        break L10;
                      }
                    }
                    L11: {
                      var12 = this.a(stackIn_30_1, stackIn_30_2 + 2, 15392);
                      if (var12 == null) {
                        break L11;
                      } else {
                        var12.a(param2, param5, 104);
                        break L11;
                      }
                    }
                    L12: {
                      stackIn_34_0 = this;

                      stackIn_34_1 = 8;

                      if (param4.field_j[param0][param3].field_o) {
                        stackIn_35_0 = this;
                        stackIn_35_1 = stackIn_34_1;
                        stackIn_35_2 = 16;
                        break L12;
                      } else {
                        stackIn_35_0 = this;
                        stackIn_35_1 = stackIn_34_1;
                        stackIn_35_2 = 0;
                        break L12;
                      }
                    }
                    L13: {
                      this.a(stackIn_35_1 + stackIn_35_2, (byte) -78).a(param2, param5);
                      stackIn_37_0 = this;

                      if (!param4.field_j[param0][param3].field_o) {
                        stackIn_38_0 = this;
                        stackIn_38_1 = 0;
                        break L13;
                      } else {
                        stackIn_38_0 = this;
                        stackIn_38_1 = 16;
                        break L13;
                      }
                    }
                    this.a(stackIn_38_1 + 2, (byte) -78).a(param2, param5);
                    break L1;
                  }
                } else {
                  L14: {
                    stackIn_5_0 = this;

                    stackIn_5_1 = param4.field_i.field_h.field_E;

                    if (!param4.field_j[param0][param3].field_o) {
                      stackIn_6_0 = this;
                      stackIn_6_1 = stackIn_5_1;
                      stackIn_6_2 = 0;
                      break L14;
                    } else {
                      stackIn_6_0 = this;
                      stackIn_6_1 = stackIn_5_1;
                      stackIn_6_2 = 16;
                      break L14;
                    }
                  }
                  L15: {
                    var9 = this.a(stackIn_6_1, stackIn_6_2 + 4, 15392);
                    if (var9 != null) {
                      var9.a(param2, param5, 104);
                      break L15;
                    } else {
                      break L15;
                    }
                  }
                  L16: {
                    stackIn_11_0 = this;

                    stackIn_11_1 = param4.field_i.field_h.field_E;

                    stackIn_11_2 = 1;

                    if (param4.field_j[param0][param3].field_o) {
                      stackIn_12_0 = this;
                      stackIn_12_1 = stackIn_11_1;
                      stackIn_12_2 = stackIn_11_2;
                      stackIn_12_3 = 16;
                      break L16;
                    } else {
                      stackIn_12_0 = this;
                      stackIn_12_1 = stackIn_11_1;
                      stackIn_12_2 = stackIn_11_2;
                      stackIn_12_3 = 0;
                      break L16;
                    }
                  }
                  L17: {
                    var10 = this.a(stackIn_12_1, stackIn_12_2 + stackIn_12_3, 15392);
                    if (var10 == null) {
                      break L17;
                    } else {
                      var10.a(param2, param5, 104);
                      break L17;
                    }
                  }
                  L18: {
                    stackIn_16_0 = this;

                    stackIn_16_1 = 4;

                    if (param4.field_j[param0][param3].field_o) {
                      stackIn_17_0 = this;
                      stackIn_17_1 = stackIn_16_1;
                      stackIn_17_2 = 16;
                      break L18;
                    } else {
                      stackIn_17_0 = this;
                      stackIn_17_1 = stackIn_16_1;
                      stackIn_17_2 = 0;
                      break L18;
                    }
                  }
                  L19: {
                    this.a(stackIn_17_1 - -stackIn_17_2, (byte) -78).a(param2, param5);
                    stackIn_19_0 = this;

                    stackIn_19_1 = 1;

                    if (!param4.field_j[param0][param3].field_o) {
                      stackIn_20_0 = this;
                      stackIn_20_1 = stackIn_19_1;
                      stackIn_20_2 = 0;
                      break L19;
                    } else {
                      stackIn_20_0 = this;
                      stackIn_20_1 = stackIn_19_1;
                      stackIn_20_2 = 16;
                      break L19;
                    }
                  }
                  this.a(stackIn_20_1 + stackIn_20_2, (byte) -78).a(param2, param5);
                  break L1;
                }
              }
            }
            L20: {
              if (-30 != (param4.field_j[param0][param3].field_n ^ -1)) {
                break L20;
              } else {
                if (param4.field_j[param0][param3].field_k == 5) {
                  pk.field_b[4].a(param2, param5);
                  pk.field_b[1].a(param2, param5);
                  break L20;
                } else {
                  if (10 != param4.field_j[param0][param3].field_k) {
                    if (15 != param4.field_j[param0][param3].field_k) {
                      pk.field_b[param4.field_j[param0][param3].field_k].a(param2, param5);
                      break L20;
                    } else {
                      pk.field_b[5 * ((param3 + param0) % 4)].a(param2, param5);
                      break L20;
                    }
                  } else {
                    pk.field_b[8].a(param2, param5);
                    pk.field_b[2].a(param2, param5);
                    break L20;
                  }
                }
              }
            }
            L21: {
              if (!param1) {
                break L21;
              } else {
                this.field_c = (pa[]) null;
                break L21;
              }
            }
            L22: {
              if (30 == param4.field_j[param0][param3].field_n) {
                if (-6 == (param4.field_j[param0][param3].field_k ^ -1)) {
                  ld.field_l[4].a(param2, param5);
                  ld.field_l[1].a(param2, param5);
                  break L22;
                } else {
                  if (10 == param4.field_j[param0][param3].field_k) {
                    ld.field_l[8].a(param2, param5);
                    ld.field_l[2].a(param2, param5);
                    break L22;
                  } else {
                    if (-16 != (param4.field_j[param0][param3].field_k ^ -1)) {
                      ld.field_l[param4.field_j[param0][param3].field_k].a(param2, param5);
                      break L22;
                    } else {
                      ld.field_l[(param3 + param0) % 4 * 5].a(param2, param5);
                      break L22;
                    }
                  }
                }
              } else {
                break L22;
              }
            }
            break L0;
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L23: {
            var7_ref = decompiledCaughtException;
            stackIn_82_0 = (RuntimeException) (var7_ref);

            stackIn_82_1 = new StringBuilder().append("pn.G(").append(param0).append(',').append(param1).append(',').append(param2).append(',').append(param3).append(',');

            if (param4 == null) {
              stackIn_83_0 = (RuntimeException) ((Object) stackIn_82_0);
              stackIn_83_1 = (StringBuilder) ((Object) stackIn_82_1);
              stackIn_83_2 = "null";
              break L23;
            } else {
              stackIn_83_0 = (RuntimeException) ((Object) stackIn_82_0);
              stackIn_83_1 = (StringBuilder) ((Object) stackIn_82_1);
              stackIn_83_2 = "{...}";
              break L23;
            }
          }
          throw pe.a((Throwable) ((Object) stackIn_83_0), stackIn_83_2 + ',' + param5 + ')');
        }
    }

    private final ee c(int param0, int param1) {
        if (-1 < (param1 ^ -1)) {
            return null;
        }
        if (-24 > (param1 ^ -1)) {
            return null;
        }
        if (param0 != 26728) {
            return (ee) null;
        }
        return this.field_b[param1 - -64];
    }

    private final ee a(int param0, int param1, int param2) {
        if (param2 != 15392) {
            return (ee) null;
        }
        if (1 != param0) {
            return null;
        }
        return nca.field_a[64 + param1];
    }

    private final ee a(byte param0, int param1) {
        if (param1 > 5) {
            return null;
        }
        if (param0 != -18) {
            jo var4 = (jo) null;
            this.a(-77, false, -85, -1, (jo) null, -104);
        }
        return this.field_b[param1 + 88];
    }

    public static void a(byte param0) {
        field_e = null;
        int var1 = 125 % ((67 - param0) / 34);
    }

    static {
        field_e = "Chat";
        field_f = 0;
    }
}
