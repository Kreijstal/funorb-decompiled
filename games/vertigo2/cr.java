/*
 * Decompiled by CFR-JS 0.4.0.
 */
class cr extends fh {
    int field_K;
    private int field_zb;
    int field_ab;
    int field_H;
    private int field_hb;
    private int field_rb;
    int field_C;
    boolean field_jb;
    int field_kb;
    private int field_fb;
    er field_W;
    String field_J;
    boolean field_pb;
    int field_O;
    boolean field_X;
    er field_mb;
    int field_qb;
    int field_tb;
    private boolean field_nb;
    String field_S;
    private boolean field_sb;
    int field_R;
    int field_xb;
    int field_B;
    static int[] field_P;
    boolean field_G;
    int field_wb;
    int field_U;
    er[] field_Bb;
    static boolean[] field_eb;
    int field_bb;
    nj field_M;
    static String field_Z;
    int field_ob;
    boolean field_V;
    int field_Cb;
    static er field_E;
    int field_db;
    er[] field_T;
    er[] field_Q;
    int field_N;
    int field_Fb;
    int field_L;
    private int field_F;
    boolean field_lb;
    private int field_vb;
    int field_A;
    er[] field_gb;
    er[] field_cb;
    er field_I;
    er field_Ab;
    private boolean field_ib;
    cc field_z;
    int field_Gb;
    er field_Db;
    private int field_Eb;
    int field_D;
    private int field_yb;
    int field_Y;
    int field_ub;

    final void a(byte param0, cr param1, cr param2, int param3) {
        RuntimeException stackIn_8_0 = null;
        StringBuilder stackIn_8_1 = null;
        RuntimeException stackIn_9_0 = null;
        StringBuilder stackIn_9_1 = null;
        String stackIn_9_2 = null;
        StringBuilder stackIn_11_1 = null;
        StringBuilder stackIn_12_1 = null;
        String stackIn_12_2 = null;
        RuntimeException decompiledCaughtException = null;
        RuntimeException var5 = null;
        try {
          L0: {
            L1: {
              if (param0 > 2) {
                break L1;
              } else {
                this.a(22, -78, 66, true);
                break L1;
              }
            }
            L2: {
              if (param1 == null) {
                param2.field_N = 0;
                param2.field_L = 0;
                break L2;
              } else {
                param2.field_N = param3 + (param1.field_db + param1.field_N);
                param2.field_L = param1.field_kb + param1.field_L;
                break L2;
              }
            }
            break L0;
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L3: {
            var5 = decompiledCaughtException;
            stackIn_8_0 = (RuntimeException) (var5);

            stackIn_8_1 = new StringBuilder().append("cr.AA(").append(param0).append(',');

            if (param1 == null) {
              stackIn_9_0 = (RuntimeException) ((Object) stackIn_8_0);
              stackIn_9_1 = (StringBuilder) ((Object) stackIn_8_1);
              stackIn_9_2 = "null";
              break L3;
            } else {
              stackIn_9_0 = (RuntimeException) ((Object) stackIn_8_0);
              stackIn_9_1 = (StringBuilder) ((Object) stackIn_8_1);
              stackIn_9_2 = "{...}";
              break L3;
            }
          }
          L4: {


            stackIn_11_1 = ((StringBuilder) (Object) stackIn_9_1).append(stackIn_9_2).append(',');

            if (param2 == null) {
              stackIn_9_0 = (RuntimeException) ((Object) stackIn_9_0);
              stackIn_12_1 = (StringBuilder) ((Object) stackIn_11_1);
              stackIn_12_2 = "null";
              break L4;
            } else {
              stackIn_9_0 = (RuntimeException) ((Object) stackIn_9_0);
              stackIn_12_1 = (StringBuilder) ((Object) stackIn_11_1);
              stackIn_12_2 = "{...}";
              break L4;
            }
          }
          throw wn.a((Throwable) ((Object) stackIn_9_0), stackIn_12_2 + ',' + param3 + ')');
        }
    }

    final void a(int param0, int param1, int param2, boolean param3) {
        cr var10_ref_cr = null;
        int var9 = 0;
        int var11 = Vertigo2.field_L ? 1 : 0;
        int var5 = -this.field_N;
        int var6 = var5 + -this.field_L;
        int var7 = this.field_db;
        int var8 = var7 - -this.field_kb;
        if (!param3) {
            var9 = 0;
            var10_ref_cr = (cr) ((Object) this.field_M.a((byte) 100));
            while (var10_ref_cr != null) {
                var10_ref_cr.field_L = -var10_ref_cr.field_N + var9;
                var9 = var9 + (param2 + var10_ref_cr.field_kb + var10_ref_cr.field_db);
                var10_ref_cr = (cr) ((Object) this.field_M.b(126));
            }
            var8 = var9 + -param2;
        }
        var9 = -param0 + var8;
        if (var6 > var9) {
            var6 = var9;
        }
        if (!(var6 >= 0)) {
            var6 = 0;
        }
        int var10 = -param0 + var7;
        if (!(var5 <= var10)) {
            var5 = var10;
        }
        if (param1 > var5) {
            var5 = 0;
        }
        this.field_db = var7;
        this.field_L = -var6 + var5;
        this.field_kb = -var7 + var8;
        this.field_N = -var5;
    }

    final void a(cr param0, int param1) {
        Object stackIn_100_0 = null;
        Object stackIn_101_0 = null;
        int stackIn_101_1 = 0;
        RuntimeException stackIn_107_0 = null;
        StringBuilder stackIn_107_1 = null;
        RuntimeException stackIn_108_0 = null;
        StringBuilder stackIn_108_1 = null;
        String stackIn_108_2 = null;
        RuntimeException decompiledCaughtException = null;
        RuntimeException var3 = null;
        try {
          L0: {
            L1: {
              if (param0 == null) {
                break L1;
              } else {
                L2: {
                  if (param0.field_R != 0) {
                    this.field_R = param0.field_R;
                    break L2;
                  } else {
                    break L2;
                  }
                }
                L3: {
                  if (param0.field_Bb != null) {
                    this.field_Bb = param0.field_Bb;
                    break L3;
                  } else {
                    break L3;
                  }
                }
                L4: {
                  if (param0.field_A == 0) {
                    break L4;
                  } else {
                    this.field_A = param0.field_A;
                    break L4;
                  }
                }
                L5: {
                  if (!param0.field_lb) {
                    this.field_lb = param0.field_lb;
                    break L5;
                  } else {
                    break L5;
                  }
                }
                L6: {
                  if (null == param0.field_z) {
                    break L6;
                  } else {
                    this.field_z = param0.field_z;
                    break L6;
                  }
                }
                L7: {
                  if (null == param0.field_Q) {
                    break L7;
                  } else {
                    this.field_Q = param0.field_Q;
                    break L7;
                  }
                }
                L8: {
                  if (!param0.field_jb) {
                    break L8;
                  } else {
                    this.field_jb = param0.field_jb;
                    break L8;
                  }
                }
                L9: {
                  if (0 > param0.field_D) {
                    break L9;
                  } else {
                    this.field_D = param0.field_D;
                    break L9;
                  }
                }
                L10: {
                  if (param0.field_ib) {
                    this.field_ib = param0.field_ib;
                    break L10;
                  } else {
                    break L10;
                  }
                }
                L11: {
                  if (null == param0.field_I) {
                    break L11;
                  } else {
                    this.field_I = param0.field_I;
                    break L11;
                  }
                }
                L12: {
                  if (!param0.field_sb) {
                    break L12;
                  } else {
                    this.field_sb = param0.field_sb;
                    break L12;
                  }
                }
                L13: {
                  if (param0.field_V) {
                    this.field_V = param0.field_V;
                    break L13;
                  } else {
                    break L13;
                  }
                }
                L14: {
                  if (0 > param0.field_H) {
                    break L14;
                  } else {
                    this.field_H = param0.field_H;
                    break L14;
                  }
                }
                L15: {
                  if (param0.field_mb == null) {
                    break L15;
                  } else {
                    this.field_mb = param0.field_mb;
                    break L15;
                  }
                }
                L16: {
                  if (param0.field_cb != null) {
                    this.field_cb = param0.field_cb;
                    break L16;
                  } else {
                    break L16;
                  }
                }
                L17: {
                  if (param0.field_gb == null) {
                    break L17;
                  } else {
                    this.field_gb = param0.field_gb;
                    break L17;
                  }
                }
                L18: {
                  if (param0.field_Ab != null) {
                    this.field_Ab = param0.field_Ab;
                    break L18;
                  } else {
                    break L18;
                  }
                }
                L19: {
                  if (-257 == (param0.field_U ^ -1)) {
                    break L19;
                  } else {
                    this.field_U = param0.field_U;
                    break L19;
                  }
                }
                L20: {
                  if (null != param0.field_W) {
                    this.field_W = param0.field_W;
                    break L20;
                  } else {
                    break L20;
                  }
                }
                L21: {
                  if (null != param0.field_Db) {
                    this.field_Db = param0.field_Db;
                    break L21;
                  } else {
                    break L21;
                  }
                }
                L22: {
                  if (null != param0.field_S) {
                    this.field_S = param0.field_S;
                    break L22;
                  } else {
                    break L22;
                  }
                }
                L23: {
                  if (null != param0.field_T) {
                    this.field_T = param0.field_T;
                    break L23;
                  } else {
                    break L23;
                  }
                }
                L24: {
                  if (param0.field_K >= 0) {
                    this.field_K = param0.field_K;
                    break L24;
                  } else {
                    break L24;
                  }
                }
                L25: {
                  if (-1 != (param0.field_qb ^ -1)) {
                    this.field_qb = param0.field_qb;
                    break L25;
                  } else {
                    break L25;
                  }
                }
                L26: {
                  if (null != param0.field_J) {
                    this.field_J = param0.field_J;
                    break L26;
                  } else {
                    break L26;
                  }
                }
                L27: {
                  if (param0.field_B < 0) {
                    break L27;
                  } else {
                    this.field_B = param0.field_B;
                    break L27;
                  }
                }
                L28: {
                  if (param0.field_X) {
                    this.field_X = param0.field_X;
                    break L28;
                  } else {
                    break L28;
                  }
                }
                L29: {
                  if (param0.field_hb != 0) {
                    this.field_hb = param0.field_hb;
                    break L29;
                  } else {
                    break L29;
                  }
                }
                L30: {
                  if (-1 == (param0.field_ab ^ -1)) {
                    break L30;
                  } else {
                    this.field_ab = param0.field_ab;
                    break L30;
                  }
                }
                L31: {
                  if (-1 != (param0.field_rb ^ -1)) {
                    this.field_rb = param0.field_rb;
                    break L31;
                  } else {
                    break L31;
                  }
                }
                L32: {
                  if (param0.field_Cb != -2147483648) {
                    this.field_Cb = param0.field_Cb;
                    break L32;
                  } else {
                    break L32;
                  }
                }
                L33: {
                  if (param0.field_O != 0) {
                    this.field_O = param0.field_O;
                    break L33;
                  } else {
                    break L33;
                  }
                }
                L34: {
                  if (2147483647 != (param0.field_yb ^ -1)) {
                    this.field_yb = param0.field_yb;
                    break L34;
                  } else {
                    break L34;
                  }
                }
                L35: {
                  if (param0.field_Gb != -2147483648) {
                    this.field_Gb = param0.field_Gb;
                    break L35;
                  } else {
                    break L35;
                  }
                }
                L36: {
                  if (2147483647 != (param0.field_F ^ -1)) {
                    this.field_F = param0.field_F;
                    break L36;
                  } else {
                    break L36;
                  }
                }
                L37: {
                  if (2147483647 == (param0.field_Y ^ -1)) {
                    break L37;
                  } else {
                    this.field_Y = param0.field_Y;
                    break L37;
                  }
                }
                L38: {
                  if (param0.field_bb == -2147483648) {
                    break L38;
                  } else {
                    this.field_bb = param0.field_bb;
                    break L38;
                  }
                }
                L39: {
                  L40: {
                    stackIn_100_0 = this;

                    if (!this.field_sb) {
                      break L40;
                    } else {
                      stackIn_100_0 = this;

                      if (!param0.field_nb) {
                        break L40;
                      } else {
                        stackIn_101_0 = this;
                        stackIn_101_1 = 1;
                        break L39;
                      }
                    }
                  }
                  stackIn_101_0 = this;
                  stackIn_101_1 = 0;
                  break L39;
                }
                ((cr) (this)).field_nb = stackIn_101_1 != 0;
                break L1;
              }
            }
            L41: {
              if (param1 <= -4) {
                break L41;
              } else {
                this.field_Fb = 105;
                break L41;
              }
            }
            break L0;
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L42: {
            var3 = decompiledCaughtException;
            stackIn_107_0 = (RuntimeException) (var3);

            stackIn_107_1 = new StringBuilder().append("cr.Q(");

            if (param0 == null) {
              stackIn_108_0 = (RuntimeException) ((Object) stackIn_107_0);
              stackIn_108_1 = (StringBuilder) ((Object) stackIn_107_1);
              stackIn_108_2 = "null";
              break L42;
            } else {
              stackIn_108_0 = (RuntimeException) ((Object) stackIn_107_0);
              stackIn_108_1 = (StringBuilder) ((Object) stackIn_107_1);
              stackIn_108_2 = "{...}";
              break L42;
            }
          }
          throw wn.a((Throwable) ((Object) stackIn_108_0), stackIn_108_2 + ',' + param1 + ')');
        }
    }

    private final void a(boolean param0, int param1, boolean param2, int param3, int param4, boolean param5, int param6, boolean param7, boolean param8, int param9, boolean param10) {
        int stackIn_4_0 = 0;
        int var12;
        int var13;
        int var14;
        int var15;
        er[] var16;
        er var17;
        int var18;
        int var19;
        int var20;
        int var21;
        int var22;
        int var23;
        int var24_int;
        String var24;
        cr var24_ref;
        int var25;
        int var26;
        String var27;
        L0: {
          var26 = Vertigo2.field_L ? 1 : 0;
          this.field_ob = this.field_wb + param4;
          this.field_C = this.field_N + param3;
          var12 = bi.field_d;
          var13 = bi.field_f;
          var14 = bi.field_i;
          var15 = bi.field_c;
          bi.c(this.field_ob, this.field_C, this.field_Fb + this.field_ob, this.field_C - -this.field_db);
          param2 = param2 & this.field_lb;
          if (this.field_ib) {
            break L0;
          } else {
            L1: {
              param0 = this.field_pb;
              param7 = this.field_V;
              if (this.field_xb == 0) {
                stackIn_4_0 = 0;
                break L1;
              } else {
                stackIn_4_0 = 1;
                break L1;
              }
            }
            param10 = stackIn_4_0 != 0;
            break L0;
          }
        }
        L2: {
          var16 = this.field_Bb;
          var17 = this.field_Db;
          var18 = this.field_A;
          var19 = 0;
          var20 = 0;
          if (!param2) {
            L3: {
              if (0 <= this.field_D) {
                var18 = this.field_D;
                break L3;
              } else {
                break L3;
              }
            }
            L4: {
              if (this.field_I == null) {
                break L4;
              } else {
                var17 = this.field_I;
                break L4;
              }
            }
            if (this.field_gb != null) {
              var16 = this.field_gb;
              break L2;
            } else {
              break L2;
            }
          } else {
            break L2;
          }
        }
        L5: {
          var21 = -98 % ((param6 - -17) / 32);
          if (param0) {
            L6: {
              if (this.field_W != null) {
                var17 = this.field_W;
                break L6;
              } else {
                break L6;
              }
            }
            L7: {
              if (-2147483648 != this.field_yb) {
                var20 = this.field_yb;
                break L7;
              } else {
                break L7;
              }
            }
            L8: {
              if ((this.field_F ^ -1) != 2147483647) {
                var19 = this.field_F;
                break L8;
              } else {
                break L8;
              }
            }
            L9: {
              if (this.field_B < 0) {
                break L9;
              } else {
                var18 = this.field_B;
                break L9;
              }
            }
            if (this.field_Q != null) {
              var16 = this.field_Q;
              break L5;
            } else {
              break L5;
            }
          } else {
            break L5;
          }
        }
        L10: {
          if (!param10) {
            break L10;
          } else {
            L11: {
              if (this.field_cb != null) {
                var16 = this.field_cb;
                break L11;
              } else {
                break L11;
              }
            }
            L12: {
              if (this.field_Ab == null) {
                break L12;
              } else {
                var17 = this.field_Ab;
                break L12;
              }
            }
            L13: {
              if (0 <= this.field_H) {
                var18 = this.field_H;
                break L13;
              } else {
                break L13;
              }
            }
            L14: {
              if (-2147483648 != this.field_Gb) {
                var19 = this.field_Gb;
                break L14;
              } else {
                break L14;
              }
            }
            if (-2147483648 == this.field_bb) {
              break L10;
            } else {
              var20 = this.field_bb;
              break L10;
            }
          }
        }
        L15: {
          if (!param7) {
            break L15;
          } else {
            L16: {
              if ((this.field_Cb ^ -1) != 2147483647) {
                var19 = this.field_Cb;
                break L16;
              } else {
                break L16;
              }
            }
            L17: {
              if (-2147483648 != this.field_Y) {
                var20 = this.field_Y;
                break L17;
              } else {
                break L17;
              }
            }
            L18: {
              if (0 <= this.field_K) {
                var18 = this.field_K;
                break L18;
              } else {
                break L18;
              }
            }
            L19: {
              if (null == this.field_T) {
                break L19;
              } else {
                var16 = this.field_T;
                break L19;
              }
            }
            if (this.field_mb == null) {
              break L15;
            } else {
              var17 = this.field_mb;
              break L15;
            }
          }
        }
        L20: {
          var22 = var19 + this.field_hb;
          var23 = this.field_rb - -var20;
          if (this.field_jb) {
            bc.a(param9, param3, (byte) 102, var16, param1, param4);
            break L20;
          } else {
            bc.a(this.field_db, this.field_C, (byte) 82, var16, this.field_Fb, this.field_ob);
            break L20;
          }
        }
        L21: {
          if (var17 != null) {
            L22: {
              var24_int = this.field_ob - -var22;
              var25 = this.field_C + var23;
              if (this.field_O != 1) {
                break L22;
              } else {
                var24_int = var24_int + (-var17.field_x + this.field_Fb) / 2;
                break L22;
              }
            }
            L23: {
              if (-2 != (this.field_ab ^ -1)) {
                break L23;
              } else {
                var25 = var25 + (this.field_db + -var17.field_B) / 2;
                break L23;
              }
            }
            L24: {
              if ((this.field_O ^ -1) == -3) {
                var24_int = var24_int + (this.field_Fb + -var17.field_x);
                break L24;
              } else {
                break L24;
              }
            }
            L25: {
              if ((this.field_ab ^ -1) == -3) {
                var25 = var25 + (this.field_db - var17.field_B);
                break L25;
              } else {
                break L25;
              }
            }
            if (this.field_vb < 256) {
              var17.c(var24_int, var25, this.field_vb);
              break L21;
            } else {
              var17.e(var24_int, var25);
              break L21;
            }
          } else {
            break L21;
          }
        }
        L26: {
          if (null == this.field_S) {
            break L26;
          } else {
            if (null != this.field_z) {
              L27: {
                var27 = this.field_S;
                var24 = var27;
                var24 = var27;
                var24 = var27;
                if (!param5) {
                  break L27;
                } else {
                  if (this.field_J != null) {
                    var24 = var27 + this.field_J;
                    break L27;
                  } else {
                    break L27;
                  }
                }
              }
              L28: {
                if (this.field_z.c(var24) > this.field_Fb + -(2 * this.field_R)) {
                  break L28;
                } else {
                  if (-1 >= (var24.indexOf("<br>") ^ -1)) {
                    break L28;
                  } else {
                    L29: {
                      if (this.field_ab != 1) {
                        if (this.field_ab == 2) {
                          var23 = var23 + (this.field_db + -this.field_z.field_M - this.field_z.field_z);
                          break L29;
                        } else {
                          break L29;
                        }
                      } else {
                        var23 = var23 + (-this.field_z.field_M + (this.field_db - this.field_z.field_z)) / 2;
                        break L29;
                      }
                    }
                    if (this.field_O == 0) {
                      this.field_z.a(var24, this.field_R + (this.field_ob + var22), var23 + (this.field_C + this.field_z.field_z), var18, -1, this.field_U);
                      break L26;
                    } else {
                      if (1 == this.field_O) {
                        this.field_z.b(var24, var22 + this.field_ob - (-this.field_R - (-(2 * this.field_R) + this.field_Fb) / 2), this.field_z.field_z + var23 + this.field_C, var18, -1, this.field_U);
                        break L26;
                      } else {
                        if (2 != this.field_O) {
                          this.field_z.a(var24, var22 + this.field_ob + this.field_R, var23 + this.field_C, -(2 * this.field_R) + this.field_Fb, this.field_db, var18, -1, this.field_U, this.field_O, this.field_ab, this.field_qb);
                          break L26;
                        } else {
                          this.field_z.c(var24, -(this.field_R * 2) - -this.field_Fb + (this.field_ob - -this.field_R) + var22, this.field_z.field_z + this.field_C - -var23, var18, -1, this.field_U);
                          break L26;
                        }
                      }
                    }
                  }
                }
              }
              this.field_z.a(var24, this.field_R + (this.field_ob - -var22), this.field_C - -var23, -(2 * this.field_R) + this.field_Fb, this.field_db, var18, -1, this.field_U, this.field_O, this.field_ab, this.field_qb);
              break L26;
            } else {
              break L26;
            }
          }
        }
        L30: {
          if (null != this.field_M) {
            var24_ref = (cr) ((Object) this.field_M.a((byte) 100));
            L31: while (true) {
              if (var24_ref == null) {
                break L30;
              } else {
                var24_ref.a(param0, this.field_Fb, param2, var20 + this.field_C, var19 + this.field_ob, param5, -102, param7, this.field_nb, this.field_db, param10);
                var24_ref = (cr) ((Object) this.field_M.b(83));
                continue L31;
              }
            }
          } else {
            break L30;
          }
        }
        bi.g(var12, var13, var14, var15);
    }

    cr(long param0, cr param1) {
        this(param0, param1, 0, 0, 0, 0, (String) null);
    }

    final void b(boolean param0, int param1) {
        this.a(false, bi.field_e, true, param1, 0, param0, 41, false, false, bi.field_j, false);
    }

    final void a(int param0, int param1, int param2, int param3, int param4) {
        if (param0 <= 71) {
            cr.b(-45, 118, -102, 29, 12);
        }
        this.field_L = 0;
        this.field_db = param2;
        this.field_wb = param4;
        this.field_kb = 0;
        this.field_N = param3;
        this.field_Eb = 0;
        this.field_Fb = param1;
        this.field_zb = 0;
    }

    cr(long param0, cr param1, String param2) {
        this(param0, param1, 0, 0, 0, 0, param2);
    }

    final static void b(int param0, int param1, int param2, int param3, int param4) {
        if (param2 != -1) {
            String var6 = (String) null;
            cr.b((String) null, 22);
        }
        if (param3 >= ap.field_e && ib.field_a >= param3) {
            param0 = pk.a((byte) -14, cj.field_D, param0, ua.field_e);
            param1 = pk.a((byte) -14, cj.field_D, param1, ua.field_e);
            ce.a(param3, (byte) -76, param0, param4, param1);
        }
    }

    public static void a(boolean param0) {
        field_eb = null;
        field_E = null;
        field_Z = null;
        if (param0) {
            field_Z = (String) null;
        }
        field_P = null;
    }

    final void a(int param0, boolean param1) {
        gi.field_o = !param1 ? true : false;
        sj.field_e = !param1 ? true : false;
        this.a(param0 ^ 1074455266, param0, 0, bi.field_j, true, bi.field_e);
    }

    final int b(int param0, int param1) {
        cr var4 = null;
        int var5 = 0;
        int var4_int = 0;
        int var6 = Vertigo2.field_L ? 1 : 0;
        if (param0 != 2) {
            cr var7 = (cr) null;
            this.a((byte) -126, (cr) null, (cr) null, 126);
        }
        int var3 = 0;
        if (this.field_S != null && null != this.field_z) {
            var4_int = this.field_z.a(this.field_S, param1);
            if (var4_int > var3) {
                var3 = var4_int;
            }
        }
        if (!(this.field_Db == null)) {
            var4_int = this.field_Db.field_x;
            if (!(var3 >= var4_int)) {
                var3 = var4_int;
            }
        }
        if (!(this.field_M == null)) {
            var4 = (cr) ((Object) this.field_M.a((byte) 100));
            while (var4 != null) {
                var5 = var4.field_Fb + var4.field_wb;
                if (var3 < var5) {
                    var3 = var5;
                }
                var4 = (cr) ((Object) this.field_M.b(param0 + 120));
            }
        }
        return var3;
    }

    final int j(int param0) {
        if (param0 != -21391) {
            cr var3 = (cr) null;
            this.a((cr) null, 10);
        }
        return this.b(param0 + 21393, 2147483647);
    }

    final static wp b(String param0, int param1) {
        int var2_int = 0;
        RuntimeException var2 = null;
        int var3 = 0;
        int var4 = 0;
        int var5 = 0;
        Object stackIn_7_0 = null;
        wp stackIn_10_0 = null;
        RuntimeException stackIn_13_0 = null;
        StringBuilder stackIn_13_1 = null;
        RuntimeException stackIn_14_0 = null;
        StringBuilder stackIn_14_1 = null;
        String stackIn_14_2 = null;
        int decompiledRegionSelector0 = 0;
        RuntimeException decompiledCaughtException = null;
        var5 = Vertigo2.field_L ? 1 : 0;
        try {
          L0: {
            var2_int = param0.length();
            var3 = param1;
            L1: while (true) {
              if (var2_int <= var3) {
                stackIn_10_0 = bp.field_A;
                decompiledRegionSelector0 = 1;
                break L0;
              } else {
                var4 = param0.charAt(var3);
                if (var4 >= 48) {
                  if (var4 <= 57) {
                    var3++;
                    continue L1;
                  } else {
                    stackIn_7_0 = null;
                    decompiledRegionSelector0 = 0;
                    break L0;
                  }
                } else {
                  return null;
                }
              }
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L2: {
            var2 = decompiledCaughtException;
            stackIn_13_0 = (RuntimeException) (var2);

            stackIn_13_1 = new StringBuilder().append("cr.M(");

            if (param0 == null) {
              stackIn_14_0 = (RuntimeException) ((Object) stackIn_13_0);
              stackIn_14_1 = (StringBuilder) ((Object) stackIn_13_1);
              stackIn_14_2 = "null";
              break L2;
            } else {
              stackIn_14_0 = (RuntimeException) ((Object) stackIn_13_0);
              stackIn_14_1 = (StringBuilder) ((Object) stackIn_13_1);
              stackIn_14_2 = "{...}";
              break L2;
            }
          }
          throw wn.a((Throwable) ((Object) stackIn_14_0), stackIn_14_2 + ',' + param1 + ')');
        }
        if (decompiledRegionSelector0 == 0) {
          return (wp) ((Object) stackIn_7_0);
        } else {
          return stackIn_10_0;
        }
    }

    final void b(cr param0, int param1) {
        try {
            if (null == this.field_M) {
                this.field_M = new nj();
            }
            if (param1 < 117) {
                this.field_lb = false;
            }
            this.field_M.a(param0, false);
        } catch (RuntimeException runtimeException) {
            throw wn.a((Throwable) ((Object) runtimeException), "cr.N(" + (param0 != null ? "{...}" : "null") + ',' + param1 + ')');
        }
    }

    private final void a(int param0, int param1, int param2, int param3, boolean param4, int param5) {
        Object stackIn_34_0 = null;
        Object stackIn_35_0 = null;
        int stackIn_35_1 = 0;
        Object stackIn_39_0 = null;
        Object stackIn_40_0 = null;
        int stackIn_40_1 = 0;
        int var7;
        int var8;
        int var9;
        int var10;
        int var11;
        int var12;
        cr var13;
        int var14;
        L0: {
          var14 = Vertigo2.field_L ? 1 : 0;
          if ((this.field_zb ^ -1) >= -1) {
            var7 = this.field_zb >> -1508911486;
            break L0;
          } else {
            var7 = -(-this.field_zb >> 1074455266);
            break L0;
          }
        }
        L1: {
          this.field_zb = this.field_zb - var7;
          this.field_wb = this.field_wb + var7;
          if (0 >= this.field_L) {
            var7 = this.field_L >> 594564354;
            break L1;
          } else {
            var7 = -(-this.field_L >> 1360559010);
            break L1;
          }
        }
        L2: {
          this.field_N = this.field_N + var7;
          this.field_L = this.field_L - var7;
          if (this.field_Eb > 0) {
            var7 = -(-this.field_Eb >> 587232770);
            break L2;
          } else {
            var7 = this.field_Eb >> 2024085218;
            break L2;
          }
        }
        L3: {
          this.field_Eb = this.field_Eb - var7;
          if (param0 == 1074455266) {
            break L3;
          } else {
            this.a(58, -12, 77, false);
            break L3;
          }
        }
        L4: {
          this.field_Fb = this.field_Fb + var7;
          if (this.field_kb <= 0) {
            var7 = this.field_kb >> 2076940130;
            break L4;
          } else {
            var7 = -(-this.field_kb >> 1994999234);
            break L4;
          }
        }
        L5: {
          L6: {
            this.field_kb = this.field_kb - var7;
            this.field_ob = this.field_wb + param2;
            this.field_db = this.field_db + var7;
            this.field_C = this.field_N + param1;
            param4 = param4 & this.field_lb;
            var8 = bi.field_d;
            var9 = bi.field_f;
            var10 = bi.field_i;
            var11 = bi.field_c;
            bi.c(this.field_ob, this.field_C, this.field_Fb + this.field_ob, this.field_C + this.field_db);
            var12 = 0;
            if (sj.field_e) {
              break L6;
            } else {
              if (!param4) {
                break L6;
              } else {
                if (kf.field_c == 0) {
                  break L6;
                } else {
                  if (sd.field_N < bi.field_d) {
                    break L6;
                  } else {
                    if (sd.field_N >= bi.field_i) {
                      break L6;
                    } else {
                      if (no.field_e < bi.field_f) {
                        break L6;
                      } else {
                        if (no.field_e < bi.field_c) {
                          this.field_tb = kf.field_c;
                          this.field_ub = -this.field_ob + sd.field_N;
                          var12 = 1;
                          this.field_xb = kf.field_c;
                          this.field_fb = -this.field_C + no.field_e;
                          break L5;
                        } else {
                          break L6;
                        }
                      }
                    }
                  }
                }
              }
            }
          }
          this.field_tb = 0;
          if (kf.field_c != 0) {
            this.field_xb = 0;
            break L5;
          } else {
            break L5;
          }
        }
        L7: {
          if (-1 != (q.field_v ^ -1)) {
            break L7;
          } else {
            this.field_xb = 0;
            break L7;
          }
        }
        L8: {
          L9: {
            stackIn_34_0 = this;

            if (gi.field_o) {
              break L9;
            } else {
              stackIn_34_0 = this;

              if (bi.field_d > ed.field_n) {
                break L9;
              } else {
                stackIn_34_0 = this;

                if (ed.field_n >= bi.field_i) {
                  break L9;
                } else {
                  stackIn_34_0 = this;

                  if (gb.field_d < bi.field_f) {
                    break L9;
                  } else {
                    stackIn_34_0 = this;

                    if (bi.field_c <= gb.field_d) {
                      break L9;
                    } else {
                      stackIn_35_0 = this;
                      stackIn_35_1 = 1;
                      break L8;
                    }
                  }
                }
              }
            }
          }
          stackIn_35_0 = this;
          stackIn_35_1 = 0;
          break L8;
        }
        L10: {
          L11: {
            ((cr) (this)).field_G = stackIn_35_1 != 0;
            stackIn_39_0 = this;

            if (!this.field_G) {
              break L11;
            } else {
              stackIn_39_0 = this;

              if (q.field_v != this.field_xb) {
                break L11;
              } else {
                stackIn_39_0 = this;

                if (!param4) {
                  break L11;
                } else {
                  stackIn_40_0 = this;
                  stackIn_40_1 = 1;
                  break L10;
                }
              }
            }
          }
          stackIn_40_0 = this;
          stackIn_40_1 = 0;
          break L10;
        }
        L12: {
          ((cr) (this)).field_pb = stackIn_40_1 != 0;
          if (this.field_M == null) {
            break L12;
          } else {
            var13 = (cr) ((Object) this.field_M.c((byte) -12));
            L13: while (true) {
              if (var13 == null) {
                break L12;
              } else {
                var13.a(1074455266, this.field_C, this.field_ob, this.field_db, param4, this.field_Fb);
                var13 = (cr) ((Object) this.field_M.b(false));
                continue L13;
              }
            }
          }
        }
        L14: {
          if (var12 != 0) {
            sj.field_e = true;
            break L14;
          } else {
            break L14;
          }
        }
        L15: {
          if (this.field_G) {
            gi.field_o = true;
            break L15;
          } else {
            break L15;
          }
        }
        L16: {
          bi.g(var8, var9, var10, var11);
          if (-1 == (this.field_xb ^ -1)) {
            break L16;
          } else {
            if (this.field_X) {
              L17: {
                this.field_N = gb.field_d - this.field_fb + -param1;
                this.field_wb = -this.field_ub + (ed.field_n - param2);
                if ((this.field_wb ^ -1) > -1) {
                  this.field_wb = 0;
                  break L17;
                } else {
                  break L17;
                }
              }
              L18: {
                if (this.field_wb <= param5 + -this.field_Fb) {
                  break L18;
                } else {
                  this.field_wb = param5 + -this.field_Fb;
                  break L18;
                }
              }
              L19: {
                if (0 > this.field_N) {
                  this.field_N = 0;
                  break L19;
                } else {
                  break L19;
                }
              }
              L20: {
                if (param3 + -this.field_db >= this.field_N) {
                  break L20;
                } else {
                  this.field_N = -this.field_db + param3;
                  break L20;
                }
              }
              this.field_L = 0;
              this.field_zb = 0;
              this.field_ob = param2 + this.field_wb;
              this.field_C = this.field_N + param1;
              break L16;
            } else {
              break L16;
            }
          }
        }
    }

    final void i(int param0) {
        cr var4 = null;
        int var3 = Vertigo2.field_L ? 1 : 0;
        if (null != this.field_M) {
            var4 = (cr) ((Object) this.field_M.c((byte) -99));
            while (var4 != null) {
                var4.field_ib = true;
                var4.i(-2147483648);
                var4 = (cr) ((Object) this.field_M.b(false));
            }
        }
        if (param0 != -2147483648) {
            field_P = (int[]) null;
        }
    }

    final static er a(int param0, r param1, byte param2, int param3) {
        RuntimeException var4 = null;
        r var5 = null;
        Object stackIn_4_0 = null;
        er stackIn_6_0 = null;
        RuntimeException stackIn_9_0 = null;
        StringBuilder stackIn_9_1 = null;
        RuntimeException stackIn_10_0 = null;
        StringBuilder stackIn_10_1 = null;
        String stackIn_10_2 = null;
        int decompiledRegionSelector0 = 0;
        RuntimeException decompiledCaughtException = null;
        try {
          L0: {
            L1: {
              if (param2 <= -85) {
                break L1;
              } else {
                var5 = (r) null;
                cr.a(0, (r) null, (byte) 109, 59);
                break L1;
              }
            }
            if (hq.a(124, param3, param0, param1)) {
              stackIn_6_0 = ej.a((byte) 89);
              decompiledRegionSelector0 = 1;
              break L0;
            } else {
              stackIn_4_0 = null;
              decompiledRegionSelector0 = 0;
              break L0;
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L2: {
            var4 = decompiledCaughtException;
            stackIn_9_0 = (RuntimeException) (var4);

            stackIn_9_1 = new StringBuilder().append("cr.CA(").append(param0).append(',');

            if (param1 == null) {
              stackIn_10_0 = (RuntimeException) ((Object) stackIn_9_0);
              stackIn_10_1 = (StringBuilder) ((Object) stackIn_9_1);
              stackIn_10_2 = "null";
              break L2;
            } else {
              stackIn_10_0 = (RuntimeException) ((Object) stackIn_9_0);
              stackIn_10_1 = (StringBuilder) ((Object) stackIn_9_1);
              stackIn_10_2 = "{...}";
              break L2;
            }
          }
          throw wn.a((Throwable) ((Object) stackIn_10_0), stackIn_10_2 + ',' + param2 + ',' + param3 + ')');
        }
        if (decompiledRegionSelector0 == 0) {
          return (er) ((Object) stackIn_4_0);
        } else {
          return stackIn_6_0;
        }
    }

    protected cr() {
        this.field_H = -1;
        this.field_B = -1;
        this.field_sb = false;
        this.field_U = 256;
        this.field_Cb = -2147483648;
        this.field_bb = -2147483648;
        this.field_lb = true;
        this.field_F = -2147483648;
        this.field_nb = false;
        this.field_Gb = -2147483648;
        this.field_vb = 256;
        this.field_K = -1;
        this.field_D = -1;
        this.field_yb = -2147483648;
        this.field_Y = -2147483648;
    }

    cr(long param0, cr param1, int param2, int param3, int param4, int param5, String param6) {
        RuntimeException runtimeException = null;
        RuntimeException stackIn_6_0 = null;
        StringBuilder stackIn_6_1 = null;
        RuntimeException stackIn_7_0 = null;
        StringBuilder stackIn_7_1 = null;
        String stackIn_7_2 = null;
        StringBuilder stackIn_9_1 = null;
        StringBuilder stackIn_10_1 = null;
        String stackIn_10_2 = null;
        RuntimeException decompiledCaughtException = null;
        this.field_H = -1;
        this.field_B = -1;
        this.field_sb = false;
        this.field_U = 256;
        this.field_Cb = -2147483648;
        this.field_bb = -2147483648;
        this.field_lb = true;
        this.field_F = -2147483648;
        this.field_nb = false;
        this.field_Gb = -2147483648;
        this.field_vb = 256;
        this.field_K = -1;
        this.field_D = -1;
        this.field_yb = -2147483648;
        this.field_Y = -2147483648;
        try {
          L0: {
            L1: {
              this.field_wb = param2;
              this.field_k = param0;
              this.field_N = param3;
              this.field_Fb = param4;
              this.field_db = param5;
              this.a(param1, -44);
              if (param6 == null) {
                break L1;
              } else {
                this.field_S = param6;
                break L1;
              }
            }
            break L0;
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L2: {
            runtimeException = decompiledCaughtException;
            stackIn_6_0 = (RuntimeException) (runtimeException);

            stackIn_6_1 = new StringBuilder().append("cr.<init>(").append(param0).append(',');

            if (param1 == null) {
              stackIn_7_0 = (RuntimeException) ((Object) stackIn_6_0);
              stackIn_7_1 = (StringBuilder) ((Object) stackIn_6_1);
              stackIn_7_2 = "null";
              break L2;
            } else {
              stackIn_7_0 = (RuntimeException) ((Object) stackIn_6_0);
              stackIn_7_1 = (StringBuilder) ((Object) stackIn_6_1);
              stackIn_7_2 = "{...}";
              break L2;
            }
          }
          L3: {


            stackIn_9_1 = ((StringBuilder) (Object) stackIn_7_1).append(stackIn_7_2).append(',').append(param2).append(',').append(param3).append(',').append(param4).append(',').append(param5).append(',');

            if (param6 == null) {
              stackIn_7_0 = (RuntimeException) ((Object) stackIn_7_0);
              stackIn_10_1 = (StringBuilder) ((Object) stackIn_9_1);
              stackIn_10_2 = "null";
              break L3;
            } else {
              stackIn_7_0 = (RuntimeException) ((Object) stackIn_7_0);
              stackIn_10_1 = (StringBuilder) ((Object) stackIn_9_1);
              stackIn_10_2 = "{...}";
              break L3;
            }
          }
          throw wn.a((Throwable) ((Object) stackIn_7_0), stackIn_10_2 + ')');
        }
    }

    static {
        field_Z = "Not yet achieved";
        field_eb = new boolean[112];
    }
}
