/*
 * Decompiled by CFR-JS 0.4.0.
 */
import java.util.*;

final class mj extends gh {
    static double field_Xb;
    static String field_ac;
    private gh[] field_Ub;
    static kg field_Sb;
    private gh[] field_Tb;
    int field_bc;
    private gh field_Wb;
    static int field_cc;
    static de field_Vb;
    static gh field_Zb;
    static ja field_Yb;

    final void a(int param0, int param1, int param2, int param3, int param4, int param5, int param6) {
        this.field_zb = param3;
        this.field_Lb = param1;
        this.field_eb = param6;
        this.field_Y = param4;
        if (param0 != 10) {
            return;
        }
        this.a(param2, param5, 0);
    }

    final static void a(ak param0, int param1, int param2, int param3) {
        try {
            cl.field_b = param1;
            int var4_int = 50 % ((param3 - 53) / 62);
            ok.field_c = param2;
            gl.field_l = param0;
        } catch (RuntimeException runtimeException) {
            throw ci.a((Throwable) ((Object) runtimeException), "mj.C(" + (param0 != null ? "{...}" : "null") + ',' + param1 + ',' + param2 + ',' + param3 + ')');
        }
    }

    final static String b(long param0, int param1) {
        pg.field_x.setTime(new Date(param0));
        int var3 = pg.field_x.get(7);
        int var4 = pg.field_x.get(5);
        int var5 = pg.field_x.get(2);
        int var6 = pg.field_x.get(1);
        if (param1 != 0) {
            return (String) null;
        }
        int var7 = pg.field_x.get(11);
        int var8 = pg.field_x.get(12);
        int var9 = pg.field_x.get(13);
        return cf.field_t[-1 + var3] + ", " + var4 / 10 + var4 % 10 + "-" + ee.field_jb[var5] + "-" + var6 + " " + var7 / 10 + var7 % 10 + ":" + var8 / 10 + var8 % 10 + ":" + var9 / 10 + var9 % 10 + " GMT";
    }

    mj(long param0, gh param1, String[] param2, gh param3, gh[] param4, int param5) {
        super(param0, (gh) null);
        RuntimeException stackIn_11_0 = null;
        StringBuilder stackIn_11_1 = null;
        RuntimeException stackIn_12_0 = null;
        StringBuilder stackIn_12_1 = null;
        String stackIn_12_2 = null;
        StringBuilder stackIn_14_1 = null;
        StringBuilder stackIn_15_1 = null;
        String stackIn_15_2 = null;
        StringBuilder stackIn_17_1 = null;
        StringBuilder stackIn_18_1 = null;
        String stackIn_18_2 = null;
        StringBuilder stackIn_20_1 = null;
        StringBuilder stackIn_21_1 = null;
        String stackIn_21_2 = null;
        RuntimeException decompiledCaughtException = null;
        int var8_int = 0;
        RuntimeException var8 = null;
        gh var9 = null;
        int var11 = 0;
        try {
          L0: {
            this.field_Ub = new gh[param2.length];
            this.field_Wb = new gh(0L, param3);
            this.field_Tb = param4;
            var8_int = 0;
            L1: while (true) {
              if (param2.length <= var8_int) {
                this.a(this.field_Wb, 120);
                var11 = 0;
                var8_int = var11;
                L2: while (true) {
                  if (param4.length <= var11) {
                    this.field_bc = param5;
                    this.field_Ub[param5].field_G = true;
                    break L0;
                  } else {
                    this.field_Wb.a(param4[var11], 126);
                    var11++;
                    continue L2;
                  }
                }
              } else {
                var9 = new gh(0L, param1);
                var9.field_S = param2[var8_int];
                this.field_Ub[var8_int] = var9;
                this.a(var9, 124);
                var8_int++;
                continue L1;
              }
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L3: {
            var8 = decompiledCaughtException;
            stackIn_11_0 = (RuntimeException) (var8);

            stackIn_11_1 = new StringBuilder().append("mj.<init>(").append(param0).append(',');

            if (param1 == null) {
              stackIn_12_0 = (RuntimeException) ((Object) stackIn_11_0);
              stackIn_12_1 = (StringBuilder) ((Object) stackIn_11_1);
              stackIn_12_2 = "null";
              break L3;
            } else {
              stackIn_12_0 = (RuntimeException) ((Object) stackIn_11_0);
              stackIn_12_1 = (StringBuilder) ((Object) stackIn_11_1);
              stackIn_12_2 = "{...}";
              break L3;
            }
          }
          L4: {


            stackIn_14_1 = ((StringBuilder) (Object) stackIn_12_1).append(stackIn_12_2).append(',');

            if (param2 == null) {
              stackIn_12_0 = (RuntimeException) ((Object) stackIn_12_0);
              stackIn_15_1 = (StringBuilder) ((Object) stackIn_14_1);
              stackIn_15_2 = "null";
              break L4;
            } else {
              stackIn_12_0 = (RuntimeException) ((Object) stackIn_12_0);
              stackIn_15_1 = (StringBuilder) ((Object) stackIn_14_1);
              stackIn_15_2 = "{...}";
              break L4;
            }
          }
          L5: {


            stackIn_17_1 = ((StringBuilder) (Object) stackIn_15_1).append(stackIn_15_2).append(',');

            if (param3 == null) {
              stackIn_12_0 = (RuntimeException) ((Object) stackIn_12_0);
              stackIn_18_1 = (StringBuilder) ((Object) stackIn_17_1);
              stackIn_18_2 = "null";
              break L5;
            } else {
              stackIn_12_0 = (RuntimeException) ((Object) stackIn_12_0);
              stackIn_18_1 = (StringBuilder) ((Object) stackIn_17_1);
              stackIn_18_2 = "{...}";
              break L5;
            }
          }
          L6: {


            stackIn_20_1 = ((StringBuilder) (Object) stackIn_18_1).append(stackIn_18_2).append(',');

            if (param4 == null) {
              stackIn_12_0 = (RuntimeException) ((Object) stackIn_12_0);
              stackIn_21_1 = (StringBuilder) ((Object) stackIn_20_1);
              stackIn_21_2 = "null";
              break L6;
            } else {
              stackIn_12_0 = (RuntimeException) ((Object) stackIn_12_0);
              stackIn_21_1 = (StringBuilder) ((Object) stackIn_20_1);
              stackIn_21_2 = "{...}";
              break L6;
            }
          }
          throw ci.a((Throwable) ((Object) stackIn_12_0), stackIn_21_2 + ',' + param5 + ')');
        }
    }

    private final void a(int param0, int param1, int param2) {
        gh dupTemp$0 = null;
        int var4;
        int var5;
        int var6;
        int var7;
        int var8;
        var7 = SteelSentinels.field_G;
        var4 = 0;
        L0: while (true) {
          if (this.field_Ub.length <= var4) {
            this.field_Wb.a(param2 + param2, param1, this.field_Lb + -param1, 0, this.field_zb);
            var8 = 0;
            var4 = var8;
            L1: while (true) {
              if (this.field_Tb.length <= var8) {
                return;
              } else {
                this.field_Tb[var8].a(0, param0, -(2 * param0) + this.field_Wb.field_Lb, param0, -(param0 * 2) + this.field_Wb.field_zb);
                if (this.field_bc != var8) {
                  dupTemp$0 = this.field_Tb[var8];
                  dupTemp$0.field_Y = dupTemp$0.field_Y + 10000;
                  var8++;
                  continue L1;
                } else {
                  var8++;
                  continue L1;
                }
              }
            }
          } else {
            var5 = this.field_zb * var4 / this.field_Ub.length;
            var6 = (1 + var4) * this.field_zb / this.field_Ub.length;
            this.field_Ub[var4].field_Y = var5;
            this.field_Ub[var4].field_eb = 0;
            this.field_Ub[var4].field_zb = var6 + -var5;
            this.field_Ub[var4].field_Lb = param1;
            var4++;
            continue L0;
          }
        }
    }

    final static void a(byte param0, java.applet.Applet param1) {
        try {
            java.net.URL var2 = null;
            RuntimeException stackIn_8_0 = null;
            StringBuilder stackIn_8_1 = null;
            RuntimeException stackIn_9_0 = null;
            StringBuilder stackIn_9_1 = null;
            String stackIn_9_2 = null;
            int decompiledRegionSelector0 = 0;
            int decompiledRegionSelector1 = 0;
            Throwable decompiledCaughtException = null;
            Exception var2_ref = null;
            RuntimeException var2_ref2 = null;
            try {
              L0: {
                try {
                  L1: {
                    if (param0 == 49) {
                      var2 = new java.net.URL(param1.getCodeBase(), "tosupport.ws");
                      param1.getAppletContext().showDocument(ge.a(param0 + -49, var2, param1), "_top");
                      decompiledRegionSelector0 = 1;
                      break L1;
                    } else {
                      decompiledRegionSelector0 = 0;
                      break L1;
                    }
                  }
                } catch (java.lang.Exception decompiledCaughtParameter0) {
                  decompiledCaughtException = decompiledCaughtParameter0;
                  L2: {
                    var2_ref = (Exception) (Object) decompiledCaughtException;
                    var2_ref.printStackTrace();
                    decompiledRegionSelector0 = 1;
                    break L2;
                  }
                }
                if (decompiledRegionSelector0 == 0) {
                  decompiledRegionSelector1 = 0;
                  break L0;
                } else {
                  decompiledRegionSelector1 = 1;
                  break L0;
                }
              }
            } catch (java.lang.RuntimeException decompiledCaughtParameter1) {
              decompiledCaughtException = decompiledCaughtParameter1;
              L3: {
                var2_ref2 = (RuntimeException) (Object) decompiledCaughtException;
                stackIn_8_0 = (RuntimeException) (var2_ref2);

                stackIn_8_1 = new StringBuilder().append("mj.D(").append(param0).append(',');

                if (param1 == null) {
                  stackIn_9_0 = (RuntimeException) ((Object) stackIn_8_0);
                  stackIn_9_1 = (StringBuilder) ((Object) stackIn_8_1);
                  stackIn_9_2 = "null";
                  break L3;
                } else {
                  stackIn_9_0 = (RuntimeException) ((Object) stackIn_8_0);
                  stackIn_9_1 = (StringBuilder) ((Object) stackIn_8_1);
                  stackIn_9_2 = "{...}";
                  break L3;
                }
              }
              throw ci.a((Throwable) ((Object) stackIn_9_0), stackIn_9_2 + ')');
            }
            if (decompiledRegionSelector1 == 0) {
              return;
            } else {
              return;
            }
        } catch (RuntimeException | Error decompiledUncheckedException) {
            throw decompiledUncheckedException;
        } catch (Throwable decompiledCheckedException) {
            throw new RuntimeException(decompiledCheckedException);
        }
    }

    public static void d(byte param0) {
        if (param0 != -72) {
            return;
        }
        field_Vb = null;
        field_Sb = null;
        field_Yb = null;
        field_Zb = null;
        field_ac = null;
    }

    final static void b(String param0, byte param1) {
        try {
            int var2_int = 46 % ((6 - param1) / 37);
            d.field_U = param0;
            hc.a(12, 0);
        } catch (RuntimeException runtimeException) {
            throw ci.a((Throwable) ((Object) runtimeException), "mj.A(" + (param0 != null ? "{...}" : "null") + ',' + param1 + ')');
        }
    }

    final void j(int param0) {
        gh dupTemp$0 = null;
        gh dupTemp$1 = null;
        int var2;
        int var3;
        var3 = SteelSentinels.field_G;
        var2 = 0;
        if (param0 == 10000) {
          L0: while (true) {
            if (this.field_Ub.length <= var2) {
              return;
            } else {
              if (var2 != this.field_bc) {
                if (0 != this.field_Ub[var2].field_Eb) {
                  this.field_Ub[this.field_bc].field_G = false;
                  dupTemp$0 = this.field_Tb[this.field_bc];
                  dupTemp$0.field_Y = dupTemp$0.field_Y + 10000;
                  this.field_bc = var2;
                  this.field_Ub[var2].field_G = true;
                  dupTemp$1 = this.field_Tb[var2];
                  dupTemp$1.field_Y = dupTemp$1.field_Y - 10000;
                  var2++;
                  continue L0;
                } else {
                  var2++;
                  continue L0;
                }
              } else {
                var2++;
                continue L0;
              }
            }
          }
        } else {
          return;
        }
    }

    final static void a(int param0, int param1, int param2, int param3, int param4, int param5, int param6, byte param7) {
        int incrementValue$0 = 0;
        int incrementValue$1 = 0;
        String stackIn_11_0 = null;
        RuntimeException decompiledCaughtException = null;
        int[] var8 = null;
        RuntimeException var8_ref = null;
        int[] var9 = null;
        int var10 = 0;
        int var11 = 0;
        int[] var12_ref_int__ = null;
        int var12 = 0;
        int var13_int = 0;
        int[] var13 = null;
        int var14 = 0;
        int var15 = 0;
        int var16 = 0;
        int[] var17 = null;
        int[] var18 = null;
        int var19 = 0;
        var16 = SteelSentinels.field_G;
        try {
          L0: {
            L1: {
              if (-32768 > (param2 ^ -1)) {
                param2 = 32767;
                break L1;
              } else {
                break L1;
              }
            }
            L2: {
              if (1 + param3 <= fm.field_f[param0]) {
                if (param0 == 1) {
                  if (param2 < jh.field_a[param3]) {
                    jh.field_a[param3] = param2;
                    break L2;
                  } else {
                    break L2;
                  }
                } else {
                  if (-3 == (param0 ^ -1)) {
                    if (param2 < jh.field_a[10 + param3]) {
                      jh.field_a[param3 + 10] = param2;
                      break L2;
                    } else {
                      break L2;
                    }
                  } else {
                    break L2;
                  }
                }
              } else {
                L3: {
                  fm.field_f[param0] = 1 + param3;
                  if (param0 != 0) {
                    break L3;
                  } else {
                    if (fm.field_f[param0] == fj.field_d) {
                      L4: {
                        if (ni.b(122)) {
                          stackIn_11_0 = td.field_ac;
                          break L4;
                        } else {
                          stackIn_11_0 = ee.field_k;
                          break L4;
                        }
                      }
                      L5: {
                        km.field_a = stackIn_11_0;
                        if ((jc.field_f ^ -1) > -2) {
                          jc.field_f = 1;
                          break L5;
                        } else {
                          break L5;
                        }
                      }
                      ln.field_g = -1;
                      break L3;
                    } else {
                      break L3;
                    }
                  }
                }
                L6: {
                  if (-1 != (param0 ^ -1)) {
                    break L6;
                  } else {
                    if (10 != fm.field_f[param0]) {
                      break L6;
                    } else {
                      if (ni.b(param7 + 103)) {
                        km.field_a = ra.field_b;
                        break L6;
                      } else {
                        break L6;
                      }
                    }
                  }
                }
                L7: {
                  if (1 == param0) {
                    jh.field_a[param3] = param2;
                    break L7;
                  } else {
                    break L7;
                  }
                }
                if (-3 == (param0 ^ -1)) {
                  jh.field_a[param3 + 10] = param2;
                  break L2;
                } else {
                  break L2;
                }
              }
            }
            L8: {
              if (!ni.b(85)) {
                mm.field_g.a(71, (byte) -117);
                mm.field_g.a((byte) 114, 10 * param0 + param3);
                mm.field_g.d(param2, 54);
                mm.field_g.b(true, param5);
                mm.field_g.b(true, param1);
                mm.field_g.b(true, param6);
                mm.field_g.b(true, param4);
                break L8;
              } else {
                jb.field_P = jb.field_P | 1 << 10 * param0 + param3;
                break L8;
              }
            }
            var8 = d.field_T[10 * param0 + param3];
            var9 = bi.field_e[param3 + param0 * 10];
            var10 = 0;
            var11 = 0;
            var17 = var8;
            var12_ref_int__ = var17;
            var13_int = 0;
            L9: while (true) {
              if (var13_int >= var17.length) {
                var12_ref_int__ = var9;
                var19 = 0;
                var13_int = var19;
                L10: while (true) {
                  if (var12_ref_int__.length <= var19) {
                    ad.field_i = new int[var10];
                    vk.field_g = new int[var11];
                    var12 = 0;
                    var18 = var8;
                    var13 = var18;
                    var14 = 0;
                    L11: while (true) {
                      if (var14 >= var18.length) {
                        var12 = 0;
                        var13 = var9;
                        var14 = 0;
                        L12: while (true) {
                          if (var14 >= var13.length) {
                            L13: {
                              if (param7 == -65) {
                                break L13;
                              } else {
                                mj.b(70L, 70);
                                break L13;
                              }
                            }
                            L14: {
                              L15: {
                                if (var10 > 0) {
                                  break L15;
                                } else {
                                  if (var11 <= 0) {
                                    break L14;
                                  } else {
                                    break L15;
                                  }
                                }
                              }
                              vn.d(-106);
                              var12 = 0;
                              L16: while (true) {
                                if (va.field_k.length <= var12) {
                                  break L14;
                                } else {
                                  va.field_k[var12] = 0;
                                  var12++;
                                  continue L16;
                                }
                              }
                            }
                            break L0;
                          } else {
                            L17: {
                              var15 = var13[var14];
                              if (t.a(fe.field_D, var15, -20370)) {
                                break L17;
                              } else {
                                incrementValue$0 = var12;
                                var12++;
                                vk.field_g[incrementValue$0] = var15;
                                break L17;
                              }
                            }
                            var14++;
                            continue L12;
                          }
                        }
                      } else {
                        L18: {
                          var15 = var18[var14];
                          if (!t.a(rn.field_D, var15, -20370)) {
                            incrementValue$1 = var12;
                            var12++;
                            ad.field_i[incrementValue$1] = var15;
                            break L18;
                          } else {
                            break L18;
                          }
                        }
                        var14++;
                        continue L11;
                      }
                    }
                  } else {
                    L19: {
                      var14 = var12_ref_int__[var19];
                      if (!t.a(fe.field_D, var14, -20370)) {
                        var11++;
                        break L19;
                      } else {
                        break L19;
                      }
                    }
                    var19++;
                    continue L10;
                  }
                }
              } else {
                L20: {
                  var14 = var17[var13_int];
                  if (!t.a(rn.field_D, var14, -20370)) {
                    var10++;
                    break L20;
                  } else {
                    break L20;
                  }
                }
                var13_int++;
                continue L9;
              }
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          var8_ref = decompiledCaughtException;
          throw ci.a((Throwable) ((Object) var8_ref), "mj.G(" + param0 + ',' + param1 + ',' + param2 + ',' + param3 + ',' + param4 + ',' + param5 + ',' + param6 + ',' + param7 + ')');
        }
    }

    static {
        field_ac = "Private";
        field_Xb = Math.atan2(1.0, 0.0);
        field_Vb = new de("usename");
        field_Yb = new ja();
    }
}
