/*
 * Decompiled by CFR-JS 0.4.0.
 */
final class dk {
    private int field_e;
    private kd field_c;
    private kd[] field_b;
    private long field_g;
    static String field_i;
    static fh field_h;
    private int field_f;
    static int field_a;
    private kd field_d;

    final kd c(byte param0) {
        kd var3 = null;
        int var4 = SteelSentinels.field_G;
        if (((dk) this).field_d == null) {
            return null;
        }
        if (param0 <= 85) {
            return null;
        }
        kd var2 = ((dk) this).field_b[(int)((long)(((dk) this).field_e + -1) & ((dk) this).field_g)];
        while (((dk) this).field_d != var2) {
            if (!((((dk) this).field_g ^ -1L) != (((dk) this).field_d.field_w ^ -1L))) {
                var3 = ((dk) this).field_d;
                ((dk) this).field_d = ((dk) this).field_d.field_o;
                return var3;
            }
            ((dk) this).field_d = ((dk) this).field_d.field_o;
        }
        ((dk) this).field_d = null;
        return null;
    }

    public static void b(byte param0) {
        if (param0 > -75) {
            return;
        }
        field_i = null;
        field_h = null;
    }

    final kd a(int param0) {
        ((dk) this).field_f = 0;
        if (param0 <= 87) {
            ((dk) this).field_c = null;
        }
        return ((dk) this).a((byte) 95);
    }

    final kd a(byte param0) {
        kd var2_ref = null;
        int var2 = 0;
        kd var3 = null;
        int var4 = 0;
        L0: {
          var4 = SteelSentinels.field_G;
          if (((dk) this).field_f <= 0) {
            break L0;
          } else {
            if (((dk) this).field_b[-1 + ((dk) this).field_f] != ((dk) this).field_c) {
              var2_ref = ((dk) this).field_c;
              ((dk) this).field_c = var2_ref.field_o;
              return var2_ref;
            } else {
              break L0;
            }
          }
        }
        var2 = 52 % ((4 - param0) / 46);
        L1: while (true) {
          if (((dk) this).field_e > ((dk) this).field_f) {
            ((dk) this).field_f = ((dk) this).field_f + 1;
            var3 = ((dk) this).field_b[((dk) this).field_f].field_o;
            if (((dk) this).field_b[-1 + ((dk) this).field_f] != var3) {
              ((dk) this).field_c = var3.field_o;
              return var3;
            } else {
              continue L1;
            }
          } else {
            return null;
          }
        }
    }

    final static void b(int param0) {
        int var1 = 0;
        int var2 = 0;
        int var3 = 0;
        long var3_long = 0L;
        int var4 = 0;
        int var5 = 0;
        String var6 = null;
        String var6_ref = null;
        int var7 = 0;
        int var8 = 0;
        ak var9 = null;
        int var10 = 0;
        int stackIn_17_0 = 0;
        int stackIn_20_0 = 0;
        String stackIn_25_0 = null;
        String stackIn_31_0 = null;
        String stackIn_36_0 = null;
        int stackIn_36_1 = 0;
        String stackIn_37_0 = null;
        int stackIn_37_1 = 0;
        String stackIn_38_0 = null;
        int stackIn_38_1 = 0;
        int stackIn_38_2 = 0;
        ak stackIn_43_0 = null;
        int stackOut_16_0 = 0;
        int stackOut_15_0 = 0;
        int stackOut_19_0 = 0;
        int stackOut_18_0 = 0;
        String stackOut_24_0 = null;
        String stackOut_23_0 = null;
        String stackOut_30_0 = null;
        String stackOut_29_0 = null;
        String stackOut_35_0 = null;
        int stackOut_35_1 = 0;
        String stackOut_37_0 = null;
        int stackOut_37_1 = 0;
        int stackOut_37_2 = 0;
        String stackOut_36_0 = null;
        int stackOut_36_1 = 0;
        int stackOut_36_2 = 0;
        ak stackOut_42_0 = null;
        ak stackOut_41_0 = null;
        L0: {
          var10 = SteelSentinels.field_G;
          var1 = ug.field_Q;
          var2 = 0;
          if (-3 != in.field_a) {
            break L0;
          } else {
            var3_long = -rb.field_c + mm.a(117);
            var2 = (int)((10999L - var3_long) / 1000L);
            if (-1 > var2) {
              var2 = 0;
              break L0;
            } else {
              break L0;
            }
          }
        }
        var3 = 0;
        L1: while (true) {
          if (var3 >= r.field_j.length) {
            L2: {
              if (param0 == -6561) {
                break L2;
              } else {
                field_h = null;
                break L2;
              }
            }
            return;
          } else {
            L3: {
              var4 = bl.field_m[var3];
              if (var4 < 0) {
                var5 = se.field_e;
                break L3;
              } else {
                if (ra.field_f.field_e == var4) {
                  var5 = km.field_i;
                  break L3;
                } else {
                  var5 = kj.field_I;
                  break L3;
                }
              }
            }
            L4: {
              var6 = r.field_j[var3];
              if (2 != in.field_a) {
                break L4;
              } else {
                if (-2 == (var2 ^ -1)) {
                  L5: {
                    if (oc.field_d.length > nm.field_n.length) {
                      stackOut_16_0 = oc.field_d.length;
                      stackIn_17_0 = stackOut_16_0;
                      break L5;
                    } else {
                      stackOut_15_0 = nm.field_n.length;
                      stackIn_17_0 = stackOut_15_0;
                      break L5;
                    }
                  }
                  L6: {
                    var7 = stackIn_17_0;
                    if (a.field_g.length <= dj.field_j.length) {
                      stackOut_19_0 = dj.field_j.length;
                      stackIn_20_0 = stackOut_19_0;
                      break L6;
                    } else {
                      stackOut_18_0 = a.field_g.length;
                      stackIn_20_0 = stackOut_18_0;
                      break L6;
                    }
                  }
                  L7: {
                    var8 = stackIn_20_0;
                    if (-7 < (var3 ^ -1)) {
                      break L7;
                    } else {
                      if (var3 >= var7 + 6) {
                        break L7;
                      } else {
                        L8: {
                          if ((-var7 + (-6 + var3 + nm.field_n.length) ^ -1) <= -1) {
                            stackOut_24_0 = nm.field_n[-var7 + (var3 - 6 + nm.field_n.length)];
                            stackIn_25_0 = stackOut_24_0;
                            break L8;
                          } else {
                            stackOut_23_0 = "";
                            stackIn_25_0 = stackOut_23_0;
                            break L8;
                          }
                        }
                        var6_ref = stackIn_25_0;
                        break L7;
                      }
                    }
                  }
                  if (var3 < 7 + var7) {
                    break L4;
                  } else {
                    if (var7 + (7 - -var8) <= var3) {
                      break L4;
                    } else {
                      L9: {
                        if (dj.field_j.length > -7 + var3 + -var7) {
                          stackOut_30_0 = dj.field_j[-var7 + -7 + var3];
                          stackIn_31_0 = stackOut_30_0;
                          break L9;
                        } else {
                          stackOut_29_0 = "";
                          stackIn_31_0 = stackOut_29_0;
                          break L9;
                        }
                      }
                      var6_ref = stackIn_31_0;
                      break L4;
                    }
                  }
                } else {
                  break L4;
                }
              }
            }
            L10: {
              if (var4 == -2) {
                var6_ref = Integer.toString(var2);
                break L10;
              } else {
                break L10;
              }
            }
            L11: {
              stackOut_35_0 = (String) var6_ref;
              stackOut_35_1 = 52;
              stackIn_37_0 = stackOut_35_0;
              stackIn_37_1 = stackOut_35_1;
              stackIn_36_0 = stackOut_35_0;
              stackIn_36_1 = stackOut_35_1;
              if (0 > var4) {
                stackOut_37_0 = (String) (Object) stackIn_37_0;
                stackOut_37_1 = stackIn_37_1;
                stackOut_37_2 = 0;
                stackIn_38_0 = stackOut_37_0;
                stackIn_38_1 = stackOut_37_1;
                stackIn_38_2 = stackOut_37_2;
                break L11;
              } else {
                stackOut_36_0 = (String) (Object) stackIn_36_0;
                stackOut_36_1 = stackIn_36_1;
                stackOut_36_2 = 1;
                stackIn_38_0 = stackOut_36_0;
                stackIn_38_1 = stackOut_36_1;
                stackIn_38_2 = stackOut_36_2;
                break L11;
              }
            }
            L12: {
              var7 = wd.a(stackIn_38_0, (byte) stackIn_38_1, stackIn_38_2 != 0);
              var8 = mg.field_kc - (var7 >> -1667523775);
              if ((var4 ^ -1) <= -1) {
                L13: {
                  if (ra.field_f.field_e != var4) {
                    stackOut_42_0 = vd.field_f;
                    stackIn_43_0 = stackOut_42_0;
                    break L13;
                  } else {
                    stackOut_41_0 = kk.field_c;
                    stackIn_43_0 = stackOut_41_0;
                    break L13;
                  }
                }
                L14: {
                  var9 = stackIn_43_0;
                  var1 = var1 + ng.field_m;
                  if (var9 == null) {
                    break L14;
                  } else {
                    var9.a(var7 - -(ud.field_y << -1634260959), -15203, var8 + -ud.field_y, (rc.field_j << 1547059041) + rn.field_q, var1);
                    break L14;
                  }
                }
                var1 = var1 + rc.field_j;
                break L12;
              } else {
                break L12;
              }
            }
            if (var4 < 0) {
              pa.field_k.b(var6_ref, var8, var1 + ag.field_s, var5, -1);
              var1 = var1 + ih.field_r;
              var3++;
              continue L1;
            } else {
              wb.field_a.b(var6_ref, var8, var1 + ia.field_l, var5, -1);
              var1 = var1 + (rc.field_j + (ng.field_m + rn.field_q));
              var3++;
              continue L1;
            }
          }
        }
    }

    final static byte[] a(boolean param0, Object param1, int param2) {
        byte[] var3 = null;
        gb var4 = null;
        if (param2 != 0) {
            return null;
        }
        if (param1 == null) {
            return null;
        }
        if (!(!(param1 instanceof byte[]))) {
            var3 = (byte[]) param1;
            if (param0) {
                return ff.a((byte) 89, var3);
            }
            return var3;
        }
        if (param1 instanceof gb) {
            var4 = (gb) param1;
            return var4.a(param2 + -21109);
        }
        throw new IllegalArgumentException();
    }

    final void a(kd param0, int param1, long param2) {
        if (param0.field_v != null) {
            param0.e(480);
        }
        kd var5 = ((dk) this).field_b[(int)(param2 & (long)(((dk) this).field_e - param1))];
        param0.field_o = var5;
        param0.field_v = var5.field_v;
        param0.field_v.field_o = param0;
        param0.field_o.field_v = param0;
        param0.field_w = param2;
    }

    final kd a(long param0, byte param1) {
        kd var5_ref = null;
        int var6 = SteelSentinels.field_G;
        ((dk) this).field_g = param0;
        kd var4 = ((dk) this).field_b[(int)((long)(((dk) this).field_e - 1) & param0)];
        ((dk) this).field_d = var4.field_o;
        while (((dk) this).field_d != var4) {
            if ((param0 ^ -1L) == (((dk) this).field_d.field_w ^ -1L)) {
                var5_ref = ((dk) this).field_d;
                ((dk) this).field_d = ((dk) this).field_d.field_o;
                return var5_ref;
            }
            ((dk) this).field_d = ((dk) this).field_d.field_o;
        }
        int var5 = -63 / ((param1 - 11) / 47);
        ((dk) this).field_d = null;
        return null;
    }

    final static void a(int param0, int param1, int param2, int param3) {
        bf.field_Fb = param3;
        o.field_b = param1;
        if (param0 > 0) {
            field_h = null;
        }
        vg.field_c = param2;
    }

    dk(int param0) {
        int var2 = 0;
        kd var3 = null;
        ((dk) this).field_f = 0;
        ((dk) this).field_b = new kd[param0];
        ((dk) this).field_e = param0;
        for (var2 = 0; var2 < param0; var2++) {
            var3 = new kd();
            ((dk) this).field_b[var2] = new kd();
            var3.field_o = var3;
            var3.field_v = var3;
        }
    }

    static {
        $cfr$clinit();
    }

    private static void $cfr$clinit() {
        field_i = "modules:</col>";
    }
}
