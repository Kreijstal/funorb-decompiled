/*
 * Decompiled by CFR-JS 0.4.0.
 */
final class bf extends gd {
    static String field_Db;
    static boolean field_Gb;
    static gh field_Ab;
    static String field_Bb;
    static int field_Fb;
    static gk[] field_Cb;
    static String field_zb;
    static String field_Eb;

    bf(ka param0, lh param1) {
        super(param0, param1, 33, 20, 30);
    }

    public static void m(byte param0) {
        field_zb = null;
        field_Ab = null;
        field_Bb = null;
        field_Eb = null;
        field_Db = null;
        field_Cb = null;
        if (param0 > -110) {
            field_Ab = (gh) null;
        }
    }

    final static kf a(byte param0, int param1, int param2, int param3, int param4) {
        int var7 = SteelSentinels.field_G;
        kf var5 = (kf) ((Object) je.field_w.e(13058));
        int var6 = -56 % ((-3 - param0) / 44);
        while (var5 != null) {
            if (var5.field_C == param2) {
                return var5;
            }
            var5 = (kf) ((Object) je.field_w.a((byte) -28));
        }
        var5 = new kf();
        var5.field_u = param4;
        var5.field_w = param1;
        var5.field_C = param2;
        je.field_w.a(3, var5);
        ma.a(param3, var5, -2864);
        return var5;
    }

    final static void a(byte param0, boolean param1) {
        gh stackIn_9_0 = null;
        gh stackIn_10_0 = null;
        int stackIn_10_1 = 0;
        gh stackIn_13_0 = null;
        gh stackIn_14_0 = null;
        int stackIn_14_1 = 0;
        gh stackIn_23_0 = null;
        gh stackIn_24_0 = null;
        int stackIn_24_1 = 0;
        gh stackIn_27_0 = null;
        gh stackIn_28_0 = null;
        int stackIn_28_1 = 0;
        if (param0 <= -102) {
          L0: {
            fe.a(-640 + pb.field_c >> -258143103, 0, -102, 640 + pb.field_c >> -22908735, pb.field_j);
            if ((je.field_t ^ -1) < -1) {
              L1: {
                if (null == ta.field_e) {
                  break L1;
                } else {
                  ta.field_e.f(mg.field_Ub.field_Y, 0);
                  break L1;
                }
              }
              L2: {
                L3: {
                  stackIn_9_0 = mg.field_Ub;

                  if (!param1) {
                    break L3;
                  } else {
                    stackIn_9_0 = (gh) ((Object) stackIn_9_0);

                    if (im.field_b) {
                      break L3;
                    } else {
                      stackIn_10_0 = (gh) ((Object) stackIn_9_0);
                      stackIn_10_1 = 1;
                      break L2;
                    }
                  }
                }
                stackIn_10_0 = (gh) ((Object) stackIn_9_0);
                stackIn_10_1 = 0;
                break L2;
              }
              L4: {
                L5: {
                  ((gh) (Object) stackIn_10_0).a(stackIn_10_1 != 0, (byte) 126);
                  stackIn_13_0 = pc.field_q;

                  if (!param1) {
                    break L5;
                  } else {
                    stackIn_13_0 = (gh) ((Object) stackIn_13_0);

                    if (im.field_b) {
                      break L5;
                    } else {
                      stackIn_14_0 = (gh) ((Object) stackIn_13_0);
                      stackIn_14_1 = 1;
                      break L4;
                    }
                  }
                }
                stackIn_14_0 = (gh) ((Object) stackIn_13_0);
                stackIn_14_1 = 0;
                break L4;
              }
              ((gh) (Object) stackIn_14_0).a(stackIn_14_1 != 0, (byte) 127);
              break L0;
            } else {
              break L0;
            }
          }
          L6: {
            L7: {
              if (-1 > (bd.field_c ^ -1)) {
                break L7;
              } else {
                if ((fj.field_f ^ -1) < -1) {
                  break L7;
                } else {
                  break L6;
                }
              }
            }
            L8: {
              if (null == ta.field_e) {
                break L8;
              } else {
                ta.field_e.f(ql.field_ec.field_Y, 0);
                break L8;
              }
            }
            L9: {
              L10: {
                stackIn_23_0 = ql.field_ec;

                if (!param1) {
                  break L10;
                } else {
                  stackIn_23_0 = (gh) ((Object) stackIn_23_0);

                  if (vb.field_c) {
                    break L10;
                  } else {
                    stackIn_24_0 = (gh) ((Object) stackIn_23_0);
                    stackIn_24_1 = 1;
                    break L9;
                  }
                }
              }
              stackIn_24_0 = (gh) ((Object) stackIn_23_0);
              stackIn_24_1 = 0;
              break L9;
            }
            L11: {
              L12: {
                ((gh) (Object) stackIn_24_0).a(stackIn_24_1 != 0, (byte) 127);
                stackIn_27_0 = gl.field_y;

                if (!param1) {
                  break L12;
                } else {
                  stackIn_27_0 = (gh) ((Object) stackIn_27_0);

                  if (vb.field_c) {
                    break L12;
                  } else {
                    stackIn_28_0 = (gh) ((Object) stackIn_27_0);
                    stackIn_28_1 = 1;
                    break L11;
                  }
                }
              }
              stackIn_28_0 = (gh) ((Object) stackIn_27_0);
              stackIn_28_1 = 0;
              break L11;
            }
            ((gh) (Object) stackIn_28_0).a(stackIn_28_1 != 0, (byte) 126);
            break L6;
          }
          tk.f((byte) 98);
          return;
        } else {
          return;
        }
    }

    final static void a(int param0, int param1, int param2, cc param3, kg param4, int param5, int param6) {
        int var8 = 0;
        int var9 = 0;
        int var10 = 0;
        int var11 = 0;
        dd var12 = null;
        RuntimeException stackIn_5_0 = null;
        StringBuilder stackIn_5_1 = null;
        RuntimeException stackIn_6_0 = null;
        StringBuilder stackIn_6_1 = null;
        String stackIn_6_2 = null;
        StringBuilder stackIn_8_1 = null;
        StringBuilder stackIn_9_1 = null;
        String stackIn_9_2 = null;
        RuntimeException decompiledCaughtException = null;
        RuntimeException var7 = null;
        try {
          L0: {
            L1: {
              kk.a(param3, -1, -1, (int[]) null, param3.field_mc, param3.field_Vb, 1073741824, param3.field_ec, param4);
              an.field_i.a(true, (byte) -126);
              an.field_i.a((int[]) null, -2, 0);
              an.field_i.c(-121);
              var12 = an.field_i;
              var8 = param1;
              var9 = param6;
              var10 = param0;
              var11 = param5;
              var12.field_j.a(var11, var10, var9, (byte) -101, var8);
              if (param2 == 4) {
                break L1;
              } else {
                bf.a((byte) -65, -107, 25, 12, -73);
                break L1;
              }
            }
            break L0;
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L2: {
            var7 = decompiledCaughtException;
            stackIn_5_0 = (RuntimeException) (var7);

            stackIn_5_1 = new StringBuilder().append("bf.JB(").append(param0).append(',').append(param1).append(',').append(param2).append(',');

            if (param3 == null) {
              stackIn_6_0 = (RuntimeException) ((Object) stackIn_5_0);
              stackIn_6_1 = (StringBuilder) ((Object) stackIn_5_1);
              stackIn_6_2 = "null";
              break L2;
            } else {
              stackIn_6_0 = (RuntimeException) ((Object) stackIn_5_0);
              stackIn_6_1 = (StringBuilder) ((Object) stackIn_5_1);
              stackIn_6_2 = "{...}";
              break L2;
            }
          }
          L3: {


            stackIn_8_1 = ((StringBuilder) (Object) stackIn_6_1).append(stackIn_6_2).append(',');

            if (param4 == null) {
              stackIn_6_0 = (RuntimeException) ((Object) stackIn_6_0);
              stackIn_9_1 = (StringBuilder) ((Object) stackIn_8_1);
              stackIn_9_2 = "null";
              break L3;
            } else {
              stackIn_6_0 = (RuntimeException) ((Object) stackIn_6_0);
              stackIn_9_1 = (StringBuilder) ((Object) stackIn_8_1);
              stackIn_9_2 = "{...}";
              break L3;
            }
          }
          throw ci.a((Throwable) ((Object) stackIn_6_0), stackIn_9_2 + ',' + param5 + ',' + param6 + ')');
        }
    }

    final static void a(byte param0, boolean param1, String param2) {
        try {
            nm.field_p = param1;
            kc.field_A = true;
            ch.field_O = new k(ag.field_p, gf.field_d, param2, de.field_d, nm.field_p);
            ag.field_p.a(ch.field_O, -118);
            if (param0 > -63) {
                bf.m(57);
            }
        } catch (RuntimeException runtimeException) {
            throw ci.a((Throwable) ((Object) runtimeException), "bf.MB(" + param0 + ',' + param1 + ',' + (param2 != null ? "{...}" : "null") + ')');
        }
    }

    final static void m(int param0) {
        ea.field_l = new gh(0L, (gh) null);
        if (param0 != 854) {
            field_zb = (String) null;
        }
        if (!(!rm.field_p)) {
            ea.field_l.a(jb.field_C, 126);
        }
        ea.field_l.a(fl.field_c, 122);
        sn.field_f = new uf(kh.field_p, ea.field_l);
        mg.field_Ub = new gh(0L, (gh) null);
        mg.field_Ub.a(sn.field_f.field_d, param0 + -734);
        mg.field_Ub.a(sg.field_i, 120);
        vl.d(0);
    }

    final void c(lh param0, int param1) {
        try {
            super.c(param0, param1 ^ 0);
            if (param1 != -22908735) {
                field_Bb = (String) null;
            }
        } catch (RuntimeException runtimeException) {
            throw ci.a((Throwable) ((Object) runtimeException), "bf.NB(" + (param0 != null ? "{...}" : "null") + ',' + param1 + ')');
        }
    }

    final static String a(int param0, int param1, byte param2, CharSequence[] param3) {
        StringBuilder discarded$3 = null;
        StringBuilder discarded$4 = null;
        CharSequence var4 = null;
        int var4_int = 0;
        RuntimeException var4_ref = null;
        int var5 = 0;
        int var6_int = 0;
        StringBuilder var6 = null;
        CharSequence var7_ref_CharSequence = null;
        int var7 = 0;
        CharSequence var8 = null;
        int var9 = 0;
        CharSequence var10 = null;
        String stackIn_3_0 = null;
        String stackIn_9_0 = null;
        String stackIn_25_0 = null;
        RuntimeException stackIn_28_0 = null;
        StringBuilder stackIn_28_1 = null;
        RuntimeException stackIn_29_0 = null;
        StringBuilder stackIn_29_1 = null;
        String stackIn_29_2 = null;
        int decompiledRegionSelector0 = 0;
        RuntimeException decompiledCaughtException = null;
        var9 = SteelSentinels.field_G;
        try {
          L0: {
            if (0 != param0) {
              if ((param0 ^ -1) == -2) {
                var10 = param3[param1];
                var4 = var10;
                if (var4 == null) {
                  stackIn_9_0 = "null";
                  decompiledRegionSelector0 = 1;
                  break L0;
                } else {
                  return var10.toString();
                }
              } else {
                var4_int = param1 + param0;
                var5 = 0;
                var6_int = param1;
                L1: while (true) {
                  if (var6_int >= var4_int) {
                    L2: {
                      var6 = new StringBuilder(var5);
                      if (param2 == 50) {
                        break L2;
                      } else {
                        bf.m((byte) 37);
                        break L2;
                      }
                    }
                    var7 = param1;
                    L3: while (true) {
                      if (var7 >= var4_int) {
                        stackIn_25_0 = var6.toString();
                        decompiledRegionSelector0 = 2;
                        break L0;
                      } else {
                        L4: {
                          var8 = param3[var7];
                          if (var8 != null) {
                            discarded$3 = var6.append(var8);
                            break L4;
                          } else {
                            discarded$4 = var6.append("null");
                            break L4;
                          }
                        }
                        var7++;
                        continue L3;
                      }
                    }
                  } else {
                    L5: {
                      var7_ref_CharSequence = param3[var6_int];
                      if (var7_ref_CharSequence != null) {
                        var5 = var5 + var7_ref_CharSequence.length();
                        break L5;
                      } else {
                        var5 += 4;
                        break L5;
                      }
                    }
                    var6_int++;
                    continue L1;
                  }
                }
              }
            } else {
              stackIn_3_0 = "";
              decompiledRegionSelector0 = 0;
              break L0;
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L6: {
            var4_ref = decompiledCaughtException;
            stackIn_28_0 = (RuntimeException) (var4_ref);

            stackIn_28_1 = new StringBuilder().append("bf.KB(").append(param0).append(',').append(param1).append(',').append(param2).append(',');

            if (param3 == null) {
              stackIn_29_0 = (RuntimeException) ((Object) stackIn_28_0);
              stackIn_29_1 = (StringBuilder) ((Object) stackIn_28_1);
              stackIn_29_2 = "null";
              break L6;
            } else {
              stackIn_29_0 = (RuntimeException) ((Object) stackIn_28_0);
              stackIn_29_1 = (StringBuilder) ((Object) stackIn_28_1);
              stackIn_29_2 = "{...}";
              break L6;
            }
          }
          throw ci.a((Throwable) ((Object) stackIn_29_0), stackIn_29_2 + ')');
        }
        if (decompiledRegionSelector0 == 0) {
          return stackIn_3_0;
        } else {
          if (decompiledRegionSelector0 == 1) {
            return stackIn_9_0;
          } else {
            return stackIn_25_0;
          }
        }
    }

    static {
        field_Gb = false;
        field_Db = "Return to Campaign";
        field_Bb = "Elapsed time";
        field_zb = "TECH VALUE: <%0> (<%1>)";
        field_Eb = "Mission Failed";
    }
}
