/*
 * Decompiled by CFR-JS 0.4.0.
 */
final class on extends me {
    static int field_j;
    static int field_h;
    private String field_m;
    static int field_i;
    static int field_n;
    static String field_k;
    private String field_l;

    final static void b(int param0) {
        int decompiledRegionSelector0 = 0;
        RuntimeException decompiledCaughtException = null;
        RuntimeException var1 = null;
        qd[] var2 = null;
        int var3 = 0;
        qd var4 = null;
        int var5 = 0;
        int var6 = 0;
        qd[] var7 = null;
        qd[] var8 = null;
        var6 = SteelSentinels.field_G;
        try {
          L0: {
            lc.field_f = false;
            qf.field_b = false;
            qh.field_Wb = false;
            nb.field_I = false;
            bm.field_Q = false;
            if (param0 == 50) {
              vc.field_d = false;
              wl.field_lb = false;
              bf.field_Gb = false;
              var8 = new qd[]{};
              var7 = hl.field_v.a(var8, -21636, 4095);
              var2 = var7;
              var3 = 0;
              L1: while (true) {
                if (var3 >= var2.length) {
                  L2: {
                    if (-9 != (dc.field_g ^ -1)) {
                      break L2;
                    } else {
                      if (!nb.field_I) {
                        rc.a(false);
                        break L2;
                      } else {
                        break L2;
                      }
                    }
                  }
                  ef.field_b = true;
                  decompiledRegionSelector0 = 1;
                  break L0;
                } else {
                  L3: {
                    L4: {
                      var4 = var2[var3];
                      if (null == var4.field_d) {
                        break L4;
                      } else {
                        if (var4.field_d.field_N == 89) {
                          qh.field_Wb = true;
                          lc.field_f = true;
                          break L3;
                        } else {
                          break L4;
                        }
                      }
                    }
                    L5: {
                      if (null == var4.field_d) {
                        break L5;
                      } else {
                        if ((var4.field_d.field_N ^ -1) == -67) {
                          qh.field_Wb = true;
                          break L3;
                        } else {
                          break L5;
                        }
                      }
                    }
                    L6: {
                      if (null == var4.field_d) {
                        break L6;
                      } else {
                        if (76 == var4.field_d.field_N) {
                          qh.field_Wb = true;
                          break L3;
                        } else {
                          break L6;
                        }
                      }
                    }
                    if (null == var4.field_d) {
                      break L3;
                    } else {
                      if (83 == var4.field_d.field_N) {
                        break L3;
                      } else {
                        if ((var4.field_d.field_Q & 1008) == 0) {
                          break L3;
                        } else {
                          qh.field_Wb = true;
                          var5 = ja.a(var4.field_d.field_N, false);
                          if (var5 != 1) {
                            if (var5 != 0) {
                              if (7 != var5) {
                                if (var5 != 8) {
                                  L7: {
                                    if (11 != var5) {
                                      if (12 == var5) {
                                        break L7;
                                      } else {
                                        L8: {
                                          if (var5 == 13) {
                                            break L8;
                                          } else {
                                            if (14 != var5) {
                                              if (15 == var5) {
                                                break L8;
                                              } else {
                                                if (-17 != (var5 ^ -1)) {
                                                  if (var5 != 17) {
                                                    if (18 != var5) {
                                                      if (var5 == 19) {
                                                        break L8;
                                                      } else {
                                                        if ((var5 ^ -1) == -10) {
                                                          qf.field_b = true;
                                                          break L3;
                                                        } else {
                                                          break L3;
                                                        }
                                                      }
                                                    } else {
                                                      break L8;
                                                    }
                                                  } else {
                                                    break L8;
                                                  }
                                                } else {
                                                  break L8;
                                                }
                                              }
                                            } else {
                                              break L8;
                                            }
                                          }
                                        }
                                        nb.field_I = true;
                                        break L3;
                                      }
                                    } else {
                                      break L7;
                                    }
                                  }
                                  vc.field_d = true;
                                  bf.field_Gb = true;
                                  break L3;
                                } else {
                                  L9: {
                                    if (-48 != (var4.field_d.field_N ^ -1)) {
                                      break L9;
                                    } else {
                                      bm.field_Q = true;
                                      break L9;
                                    }
                                  }
                                  vc.field_d = true;
                                  break L3;
                                }
                              } else {
                                wl.field_lb = true;
                                vc.field_d = true;
                                bm.field_Q = true;
                                break L3;
                              }
                            } else {
                              vc.field_d = true;
                              wl.field_lb = true;
                              break L3;
                            }
                          } else {
                            wl.field_lb = true;
                            break L3;
                          }
                        }
                      }
                    }
                  }
                  var3++;
                  continue L1;
                }
              }
            } else {
              decompiledRegionSelector0 = 0;
              break L0;
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          var1 = decompiledCaughtException;
          throw ci.a((Throwable) ((Object) var1), "on.E(" + param0 + ')');
        }
        if (decompiledRegionSelector0 == 0) {
          return;
        } else {
          return;
        }
    }

    final void a(gi param0, boolean param1) {
        try {
            param0.a(13588, this.field_m);
            param0.a(this.field_l, (byte) -83);
            if (param1) {
                on.b(114);
            }
        } catch (RuntimeException runtimeException) {
            throw ci.a((Throwable) ((Object) runtimeException), "on.D(" + (param0 != null ? "{...}" : "null") + ',' + param1 + ')');
        }
    }

    final static pc a(em param0, int param1) {
        pc var2 = null;
        RuntimeException var2_ref = null;
        pc stackIn_2_0 = null;
        pc stackIn_4_0 = null;
        RuntimeException stackIn_7_0 = null;
        StringBuilder stackIn_7_1 = null;
        RuntimeException stackIn_8_0 = null;
        StringBuilder stackIn_8_1 = null;
        String stackIn_8_2 = null;
        int decompiledRegionSelector0 = 0;
        RuntimeException decompiledCaughtException = null;
        try {
          L0: {
            if (param1 == -48) {
              var2 = new pc(param0, param0);
              mm.field_b.a(3, var2);
              ll.field_e.b(param0);
              stackIn_4_0 = (pc) (var2);
              decompiledRegionSelector0 = 1;
              break L0;
            } else {
              stackIn_2_0 = (pc) null;
              decompiledRegionSelector0 = 0;
              break L0;
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L1: {
            var2_ref = decompiledCaughtException;
            stackIn_7_0 = (RuntimeException) (var2_ref);

            stackIn_7_1 = new StringBuilder().append("on.C(");

            if (param0 == null) {
              stackIn_8_0 = (RuntimeException) ((Object) stackIn_7_0);
              stackIn_8_1 = (StringBuilder) ((Object) stackIn_7_1);
              stackIn_8_2 = "null";
              break L1;
            } else {
              stackIn_8_0 = (RuntimeException) ((Object) stackIn_7_0);
              stackIn_8_1 = (StringBuilder) ((Object) stackIn_7_1);
              stackIn_8_2 = "{...}";
              break L1;
            }
          }
          throw ci.a((Throwable) ((Object) stackIn_8_0), stackIn_8_2 + ',' + param1 + ')');
        }
        if (decompiledRegionSelector0 == 0) {
          return stackIn_2_0;
        } else {
          return stackIn_4_0;
        }
    }

    final dj a(byte param0) {
        if (param0 < 47) {
            gi var3 = (gi) null;
            this.a((gi) null, true);
        }
        return nk.field_F;
    }

    public static void b(byte param0) {
        if (param0 != -116) {
            return;
        }
        field_k = null;
    }

    on(String param0, String param1) {
        try {
            this.field_m = param0;
            this.field_l = param1;
        } catch (RuntimeException runtimeException) {
            throw ci.a((Throwable) ((Object) runtimeException), "on.<init>(" + (param0 != null ? "{...}" : "null") + ',' + (param1 != null ? "{...}" : "null") + ')');
        }
    }

    static {
        field_h = 50;
    }
}
