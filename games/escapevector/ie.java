/*
 * Decompiled by CFR-JS 0.4.0.
 */
class ie {
    private static int field_g;
    int[] field_f;
    private long field_s;
    private static ge field_j;
    static boolean field_o;
    private int field_a;
    static int field_q;
    private boolean field_i;
    private wg field_u;
    private int field_r;
    private wg[] field_c;
    private int field_b;
    private long field_t;
    private int field_h;
    private int field_d;
    private long field_n;
    private int field_p;
    private boolean field_e;
    private wg[] field_l;
    private int field_k;
    private int field_m;

    void f() {
    }

    private final void a(int[] param0, int param1) {
        int var3;
        int var4;
        int var5;
        int var6;
        int var7_int;
        Object var7;
        int var8_int;
        wg[] var8;
        int var9;
        Object var10;
        wg var11;
        nh var12;
        int var13;
        wg var14;
        int var15_int;
        wg var15;
        L0: {
          var3 = param1;
          if (!field_o) {
            break L0;
          } else {
            var3 = var3 << 1;
            break L0;
          }
        }
        L1: {
          qg.a(param0, 0, var3);
          this.field_b = this.field_b - param1;
          if (this.field_u == null) {
            break L1;
          } else {
            if (this.field_b > 0) {
              break L1;
            } else {
              this.field_b = this.field_b + (field_q >> 4);
              ie.a(this.field_u);
              this.a(this.field_u, this.field_u.d());
              var4 = 0;
              var5 = 255;
              var6 = 7;
              L2: while (true) {
                L3: {
                  if (var5 == 0) {
                    break L3;
                  } else {
                    L4: {
                      if (var6 >= 0) {
                        var7_int = var6;
                        var8_int = 0;
                        break L4;
                      } else {
                        var7_int = var6 & 3;
                        var8_int = -(var6 >> 2);
                        break L4;
                      }
                    }
                    var9 = var5 >>> var7_int & 286331153;
                    L5: while (true) {
                      if (var9 != 0) {
                        L6: {
                          if ((var9 & 1) != 0) {
                            var5 = var5 & (1 << var7_int ^ -1);
                            var10 = null;
                            var11 = this.field_c[var7_int];
                            var14 = var11;
                            var14 = var11;
                            L7: while (true) {
                              if (var11 == null) {
                                break L6;
                              } else {
                                L8: {
                                  var12 = var11.field_g;
                                  if (var12 == null) {
                                    break L8;
                                  } else {
                                    if (var12.field_g <= var8_int) {
                                      break L8;
                                    } else {
                                      var5 = var5 | 1 << var7_int;
                                      var10 = var11;
                                      var11 = var11.field_h;
                                      continue L7;
                                    }
                                  }
                                }
                                L9: {
                                  var11.field_i = true;
                                  var13 = var11.a();
                                  var4 = var4 + var13;
                                  if (var12 == null) {
                                    break L9;
                                  } else {
                                    var12.field_g = var12.field_g + var13;
                                    break L9;
                                  }
                                }
                                if (var4 < this.field_a) {
                                  L10: {
                                    var14 = var11.c();
                                    if (var14 == null) {
                                      break L10;
                                    } else {
                                      var15_int = var11.field_j;
                                      L11: while (true) {
                                        if (var14 == null) {
                                          break L10;
                                        } else {
                                          this.a(var14, var15_int * var14.d() >> 8);
                                          var14 = var11.b();
                                          continue L11;
                                        }
                                      }
                                    }
                                  }
                                  L12: {
                                    var15 = var11.field_h;
                                    var11.field_h = null;
                                    if (var10 != null) {
                                      ((wg) (var10)).field_h = var15;
                                      break L12;
                                    } else {
                                      this.field_c[var7_int] = var15;
                                      break L12;
                                    }
                                  }
                                  L13: {
                                    if (var15 != null) {
                                      break L13;
                                    } else {
                                      this.field_l[var7_int] = (wg) (var10);
                                      break L13;
                                    }
                                  }
                                  var11 = var15;
                                  continue L7;
                                } else {
                                  break L3;
                                }
                              }
                            }
                          } else {
                            break L6;
                          }
                        }
                        var7_int += 4;
                        var8_int++;
                        var9 = var9 >>> 4;
                        continue L5;
                      } else {
                        var6--;
                        continue L2;
                      }
                    }
                  }
                }
                var6 = 0;
                L14: while (true) {
                  if (var6 >= 8) {
                    break L1;
                  } else {
                    var7 = this.field_c[var6];
                    var8 = this.field_c;
                    var9 = var6;
                    this.field_l[var6] = null;
                    var8[var9] = null;
                    L15: while (true) {
                      if (var7 == null) {
                        var6++;
                        continue L14;
                      } else {
                        var10 = ((wg) (var7)).field_h;
                        ((wg) (var7)).field_h = null;
                        var7 = var10;
                        continue L15;
                      }
                    }
                  }
                }
              }
            }
          }
        }
        L16: {
          if (this.field_b >= 0) {
            break L16;
          } else {
            this.field_b = 0;
            break L16;
          }
        }
        L17: {
          if (this.field_u == null) {
            break L17;
          } else {
            this.field_u.b(param0, 0, param1);
            break L17;
          }
        }
        this.field_s = td.b(128);
    }

    final static void a(int param0, boolean param1, int param2) {
        if (param0 < 8000 || param0 > 48000) {
            throw new IllegalArgumentException();
        }
        field_q = param0;
        field_o = param1 ? true : false;
        field_g = param2;
    }

    final synchronized void d() {
        try {
            int decompiledRegionSelector0 = 0;
            Throwable decompiledCaughtException = null;
            long var1 = 0L;
            Exception var3 = null;
            int var3_int = 0;
            int var4 = 0;
            if (!this.field_i) {
              var1 = td.b(128);
              try {
                L0: {
                  L1: {
                    if (var1 <= this.field_s + 6000L) {
                      break L1;
                    } else {
                      this.field_s = var1 - 6000L;
                      break L1;
                    }
                  }
                  L2: while (true) {
                    if (var1 <= this.field_s + 5000L) {
                      break L0;
                    } else {
                      this.a(256);
                      this.field_s = this.field_s + (long)(256000 / field_q);
                      var1 = td.b(128);
                      continue L2;
                    }
                  }
                }
              } catch (java.lang.Exception decompiledCaughtParameter0) {
                decompiledCaughtException = decompiledCaughtParameter0;
                L3: {
                  var3 = (Exception) (Object) decompiledCaughtException;
                  this.field_s = var1;
                  break L3;
                }
              }
              if (this.field_f != null) {
                try {
                  L4: {
                    L5: {
                      if (this.field_n == 0L) {
                        break L5;
                      } else {
                        if (var1 >= this.field_n) {
                          this.b(this.field_d);
                          this.field_n = 0L;
                          this.field_e = true;
                          break L5;
                        } else {
                          decompiledRegionSelector0 = 0;
                          break L4;
                        }
                      }
                    }
                    L6: {
                      var3_int = this.a();
                      if (this.field_k - var3_int <= this.field_r) {
                        break L6;
                      } else {
                        this.field_r = this.field_k - var3_int;
                        break L6;
                      }
                    }
                    L7: {
                      var4 = this.field_p + this.field_m;
                      if (var4 + 256 <= 16384) {
                        break L7;
                      } else {
                        var4 = 16128;
                        break L7;
                      }
                    }
                    L8: {
                      if (var4 + 256 <= this.field_d) {
                        break L8;
                      } else {
                        L9: {
                          this.field_d = this.field_d + 1024;
                          if (this.field_d <= 16384) {
                            break L9;
                          } else {
                            this.field_d = 16384;
                            break L9;
                          }
                        }
                        this.f();
                        this.b(this.field_d);
                        var3_int = 0;
                        this.field_e = true;
                        if (var4 + 256 <= this.field_d) {
                          break L8;
                        } else {
                          var4 = this.field_d - 256;
                          this.field_m = var4 - this.field_p;
                          break L8;
                        }
                      }
                    }
                    L10: while (true) {
                      if (var3_int >= var4) {
                        L11: {
                          if (var1 <= this.field_t) {
                            break L11;
                          } else {
                            L12: {
                              if (this.field_e) {
                                this.field_e = false;
                                break L12;
                              } else {
                                L13: {
                                  if (this.field_r != 0) {
                                    break L13;
                                  } else {
                                    if (this.field_h != 0) {
                                      break L13;
                                    } else {
                                      this.f();
                                      this.field_n = var1 + 2000L;
                                      decompiledRegionSelector0 = 1;
                                      break L4;
                                    }
                                  }
                                }
                                this.field_m = Math.min(this.field_h, this.field_r);
                                this.field_h = this.field_r;
                                break L12;
                              }
                            }
                            this.field_r = 0;
                            this.field_t = var1 + 2000L;
                            break L11;
                          }
                        }
                        this.field_k = var3_int;
                        decompiledRegionSelector0 = 2;
                        break L4;
                      } else {
                        this.a(this.field_f, 256);
                        this.h();
                        var3_int += 256;
                        continue L10;
                      }
                    }
                  }
                } catch (java.lang.Exception decompiledCaughtParameter1) {
                  decompiledCaughtException = decompiledCaughtParameter1;
                  L14: {
                    var3 = (Exception) (Object) decompiledCaughtException;
                    this.f();
                    this.field_n = var1 + 2000L;
                    decompiledRegionSelector0 = 2;
                    break L14;
                  }
                }
                if (decompiledRegionSelector0 == 0) {
                  return;
                } else {
                  if (decompiledRegionSelector0 == 1) {
                    return;
                  } else {
                    return;
                  }
                }
              } else {
                return;
              }
            } else {
              return;
            }
        } catch (RuntimeException | Error decompiledUncheckedException) {
            throw decompiledUncheckedException;
        } catch (Throwable decompiledCheckedException) {
            throw new RuntimeException(decompiledCheckedException);
        }
    }

    private final void a(wg param0, int param1) {
        int var3 = param1 >> 5;
        wg var4 = this.field_l[var3];
        if (var4 == null) {
            this.field_c[var3] = param0;
        } else {
            var4.field_h = param0;
        }
        this.field_l[var3] = param0;
        param0.field_j = param1;
    }

    public static void e() {
        field_j = null;
    }

    void g() throws Exception {
    }

    private final static void a(wg param0) {
        param0.field_i = false;
        if (param0.field_g != null) {
            param0.field_g.field_g = 0;
        }
        wg var1 = param0.c();
        while (var1 != null) {
            ie.a(var1);
            var1 = param0.b();
        }
    }

    int a() throws Exception {
        return this.field_d;
    }

    final synchronized void b(wg param0) {
        this.field_u = param0;
    }

    void b(int param0) throws Exception {
    }

    void a(java.awt.Component param0) throws Exception {
    }

    void h() throws Exception {
    }

    private final void a(int param0) {
        this.field_b = this.field_b - param0;
        if (this.field_b < 0) {
            this.field_b = 0;
        }
        if (this.field_u != null) {
            this.field_u.d(param0);
            return;
        }
    }

    final synchronized void b() {
        try {
            this.field_e = true;
            try {
                this.g();
            } catch (Exception exception) {
                this.f();
                this.field_n = td.b(128) + 2000L;
            }
        } catch (RuntimeException | Error decompiledUncheckedException) {
            throw decompiledUncheckedException;
        } catch (Throwable decompiledCheckedException) {
            throw new RuntimeException(decompiledCheckedException);
        }
    }

    final static ie a(lk param0, java.awt.Component param1, int param2, int param3) {
        try {
            vm var4 = null;
            Throwable var4_ref = null;
            vm var5 = null;
            vm stackIn_10_0 = null;
            int stackIn_10_1 = 0;
            vm stackIn_11_0 = null;
            int stackIn_11_1 = 0;
            int stackIn_11_2 = 0;
            vm stackIn_21_0 = null;
            Throwable decompiledCaughtException = null;
            if (field_q != 0) {
              L0: {
                if (param2 < 0) {
                  break L0;
                } else {
                  if (param2 < 2) {
                    L1: {
                      if (param3 >= 256) {
                        break L1;
                      } else {
                        param3 = 256;
                        break L1;
                      }
                    }
                    try {
                      L2: {
                        L3: {
                          var5 = new vm();
                          var4 = var5;
                          stackIn_10_0 = (vm) (var4);

                          stackIn_10_1 = 256;

                          if (!field_o) {
                            stackIn_11_0 = (vm) ((Object) stackIn_10_0);
                            stackIn_11_1 = stackIn_10_1;
                            stackIn_11_2 = 1;
                            break L3;
                          } else {
                            stackIn_11_0 = (vm) ((Object) stackIn_10_0);
                            stackIn_11_1 = stackIn_10_1;
                            stackIn_11_2 = 2;
                            break L3;
                          }
                        }
                        L4: {
                          ((ie) ((Object) stackIn_11_0)).field_f = new int[stackIn_11_1 * stackIn_11_2];
                          ((ie) ((Object) var4)).field_p = param3;
                          ((ie) ((Object) var4)).a(param1);
                          ((ie) ((Object) var4)).field_d = (param3 & -1024) + 1024;
                          if (((ie) ((Object) var4)).field_d <= 16384) {
                            break L4;
                          } else {
                            ((ie) ((Object) var4)).field_d = 16384;
                            break L4;
                          }
                        }
                        L5: {
                          ((ie) ((Object) var4)).b(((ie) ((Object) var4)).field_d);
                          if (field_g <= 0) {
                            break L5;
                          } else {
                            if (field_j != null) {
                              break L5;
                            } else {
                              field_j = new ge();
                              field_j.field_e = param0;
                              param0.a(true, field_g, (Runnable) ((Object) field_j));
                              break L5;
                            }
                          }
                        }
                        L6: {
                          if (field_j == null) {
                            break L6;
                          } else {
                            if (field_j.field_f[param2] == null) {
                              field_j.field_f[param2] = (ie) ((Object) var5);
                              break L6;
                            } else {
                              throw new IllegalArgumentException();
                            }
                          }
                        }
                        stackIn_21_0 = (vm) (var4);
                        break L2;
                      }
                    } catch (java.lang.Throwable decompiledCaughtParameter0) {
                      decompiledCaughtException = decompiledCaughtParameter0;
                      var4_ref = decompiledCaughtException;
                      return new ie();
                    }
                    return (ie) ((Object) stackIn_21_0);
                  } else {
                    break L0;
                  }
                }
              }
              throw new IllegalArgumentException();
            } else {
              throw new IllegalStateException();
            }
        } catch (RuntimeException | Error decompiledUncheckedException) {
            throw decompiledUncheckedException;
        } catch (Throwable decompiledCheckedException) {
            throw new RuntimeException(decompiledCheckedException);
        }
    }

    final synchronized void c() {
        int var1;
        int var2;
        L0: {
          if (field_j == null) {
            break L0;
          } else {
            var1 = 1;
            var2 = 0;
            L1: while (true) {
              if (var2 >= 2) {
                if (var1 == 0) {
                  break L0;
                } else {
                  field_j.field_c = true;
                  L2: while (true) {
                    if (!field_j.field_g) {
                      field_j = null;
                      break L0;
                    } else {
                      en.a((byte) -46, 50L);
                      continue L2;
                    }
                  }
                }
              } else {
                L3: {
                  if (field_j.field_f[var2] != this) {
                    break L3;
                  } else {
                    field_j.field_f[var2] = null;
                    break L3;
                  }
                }
                if (field_j.field_f[var2] != null) {
                  var1 = 0;
                  var2++;
                  continue L1;
                } else {
                  var2++;
                  continue L1;
                }
              }
            }
          }
        }
        this.f();
        this.field_f = null;
        this.field_i = true;
    }

    ie() {
        this.field_a = 32;
        this.field_i = false;
        this.field_s = td.b(128);
        this.field_b = 0;
        this.field_h = 0;
        this.field_t = 0L;
        this.field_c = new wg[8];
        this.field_r = 0;
        this.field_n = 0L;
        this.field_l = new wg[8];
        this.field_e = true;
        this.field_k = 0;
    }
}
