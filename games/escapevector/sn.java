/*
 * Decompiled by CFR-JS 0.4.0.
 */
final class sn {
    static mf field_c;
    private boolean field_e;
    static long field_d;
    static boolean[] field_b;
    private String field_a;

    final static void c(int param0) {
        int var1;
        int var2;
        int var3;
        L0: {
          var1 = l.a(-8462, od.field_q);
          var2 = vh.f(127);
          if (param0 == -1) {
            break L0;
          } else {
            field_b = (boolean[]) null;
            break L0;
          }
        }
        L1: {
          L2: {
            if (var1 != od.field_q) {
              break L2;
            } else {
              if (hn.field_l.field_h != var2) {
                break L2;
              } else {
                break L1;
              }
            }
          }
          var3 = di.field_d.field_j ? 1 : 0;
          ll.a(0, var1, var3 != 0, var2);
          break L1;
        }
    }

    public static void a(byte param0) {
        field_b = null;
        field_c = null;
        if (param0 > -66) {
            field_c = (mf) null;
        }
    }

    final static void a(int param0, int param1, boolean param2) {
        int stackIn_52_0 = 0;
        int stackIn_55_0 = 0;
        String[] stackIn_62_0 = null;
        int stackIn_62_1 = 0;
        String[] stackIn_63_0 = null;
        int stackIn_63_1 = 0;
        String stackIn_63_2 = null;
        String[] stackIn_67_0 = null;
        int stackIn_67_1 = 0;
        int stackIn_67_2 = 0;
        int stackIn_67_3 = 0;
        String stackIn_70_2 = null;
        int stackIn_90_0 = 0;
        String stackIn_90_1 = null;
        int stackIn_91_0 = 0;
        String stackIn_91_1 = null;
        int stackIn_91_2 = 0;
        int stackIn_112_0 = 0;
        int stackIn_113_0 = 0;
        int stackIn_113_1 = 0;
        Throwable caughtException = null;
        RuntimeException decompiledCaughtException = null;
        int var3_int = 0;
        RuntimeException var3 = null;
        int var4 = 0;
        String[] var4_ref_String__ = null;
        int var5 = 0;
        int var6 = 0;
        int var7 = 0;
        int var8 = 0;
        int var9 = 0;
        String[] var10 = null;
        String var11 = null;
        String var12 = null;
        var9 = EscapeVector.field_A;
        try {
          L0: {
            L1: {
              tb.field_D = true;
              f.field_j = param0;
              if (-1 == (f.field_j ^ -1)) {
                var3_int = un.a(pf.field_e, -118, nb.field_a, sb.field_h, ac.field_a);
                var4 = var3_int + 3;
                jj.field_a = new int[var4];
                s.field_I = new String[var4];
                var5 = 0;
                L2: while (true) {
                  if (var5 >= var4) {
                    wh.field_j = new int[2];
                    var5 = 0;
                    L3: while (true) {
                      if (var3_int <= var5) {
                        s.field_I[var4 + -3] = "";
                        s.field_I[var4 - 2] = lh.field_a;
                        jj.field_a[-2 + var4] = 0;
                        wh.field_j[0] = 4;
                        s.field_I[var4 - 1] = ah.field_k;
                        jj.field_a[-1 + var4] = 1;
                        wh.field_j[1] = 5;
                        break L1;
                      } else {
                        s.field_I[var5] = pf.field_e[var5];
                        var5++;
                        continue L3;
                      }
                    }
                  } else {
                    jj.field_a[var5] = -1;
                    var5++;
                    continue L2;
                  }
                }
              } else {
                if (-2 == (f.field_j ^ -1)) {
                  var3_int = un.a(pf.field_e, -126, nb.field_a, sb.field_h, ac.field_a);
                  var4 = 2 + var3_int;
                  s.field_I = new String[var4];
                  jj.field_a = new int[var4];
                  var5 = 0;
                  L4: while (true) {
                    if (var4 <= var5) {
                      wh.field_j = new int[1];
                      var5 = 0;
                      L5: while (true) {
                        if (var5 >= var3_int) {
                          s.field_I[-2 + var4] = "";
                          s.field_I[-1 + var4] = ah.field_k;
                          jj.field_a[var4 + -1] = 0;
                          wh.field_j[0] = 5;
                          break L1;
                        } else {
                          s.field_I[var5] = pf.field_e[var5];
                          var5++;
                          continue L5;
                        }
                      }
                    } else {
                      jj.field_a[var5] = -1;
                      var5++;
                      continue L4;
                    }
                  }
                } else {
                  if (-3 == (f.field_j ^ -1)) {
                    var3_int = un.a(pf.field_e, -101, nb.field_a, sb.field_h, r.a(fn.field_d, -63, new String[]{"<br><%0><br>"}));
                    var4 = -1;
                    var5 = 0;
                    L6: while (true) {
                      L7: {
                        if (var5 >= var3_int) {
                          break L7;
                        } else {
                          if ("<%0>".equals(pf.field_e[var5])) {
                            var4 = var5;
                            break L7;
                          } else {
                            var5++;
                            continue L6;
                          }
                        }
                      }
                      if (-1 == var4) {
                        throw new IllegalStateException();
                      } else {
                        tg.field_nb = new String[var4];
                        qg.a(pf.field_e, 0, tg.field_nb, 0, var4);
                        nn.field_i = new String[-1 + -var4 + var3_int];
                        qg.a(pf.field_e, 1 + var4, nn.field_i, 0, -var4 + (var3_int - 1));
                        var4 = -1;
                        var3_int = un.a(pf.field_e, param1 ^ 71, nb.field_a, sb.field_h, r.a(al.field_c, param1 ^ -63, new String[]{"<br><%0><br>"}));
                        var5 = 0;
                        L8: while (true) {
                          L9: {
                            if (var3_int <= var5) {
                              break L9;
                            } else {
                              if (!"<%0>".equals(pf.field_e[var5])) {
                                var5++;
                                continue L8;
                              } else {
                                var4 = var5;
                                break L9;
                              }
                            }
                          }
                          if ((var4 ^ -1) == 0) {
                            throw new IllegalStateException();
                          } else {
                            L10: {
                              qm.field_f = new String[var4];
                              qg.a(pf.field_e, 0, qm.field_f, 0, var4);
                              tb.field_G = new String[var3_int + -var4 - 1];
                              qg.a(pf.field_e, 1 + var4, tb.field_G, 0, -1 + var3_int - var4);
                              if (qm.field_f.length > tg.field_nb.length) {
                                stackIn_52_0 = qm.field_f.length;
                                break L10;
                              } else {
                                stackIn_52_0 = tg.field_nb.length;
                                break L10;
                              }
                            }
                            L11: {
                              var5 = stackIn_52_0;
                              if (nn.field_i.length >= tb.field_G.length) {
                                stackIn_55_0 = nn.field_i.length;
                                break L11;
                              } else {
                                stackIn_55_0 = tb.field_G.length;
                                break L11;
                              }
                            }
                            var6 = stackIn_55_0;
                            var7 = var5 + 7 - -var6;
                            jj.field_a = new int[var7];
                            s.field_I = new String[var7];
                            var8 = 0;
                            L12: while (true) {
                              if (var8 >= var7) {
                                wh.field_j = new int[2];
                                jj.field_a[1] = 0;
                                s.field_I[1] = jh.field_j;
                                s.field_I[0] = td.field_a;
                                s.field_I[2] = bb.field_I;
                                wh.field_j[0] = 5;
                                jj.field_a[3] = 1;
                                s.field_I[5] = "";
                                s.field_I[4] = p.field_e;
                                s.field_I[3] = bb.field_J;
                                wh.field_j[1] = 2;
                                var8 = 0;
                                L13: while (true) {
                                  if (var8 >= var5) {
                                    s.field_I[6 - -var5] = null;
                                    jj.field_a[var5 + 6] = -2;
                                    var8 = 0;
                                    L14: while (true) {
                                      if (var6 <= var8) {
                                        bd.field_k = td.b(128);
                                        break L1;
                                      } else {
                                        stackIn_67_0 = s.field_I;
                                        stackIn_67_1 = var8 + var5 + 7;
                                        stackIn_67_2 = var8 ^ -1;
                                        stackIn_67_3 = tb.field_G.length ^ -1;
                                        L15: {




                                          if (stackIn_67_2 > stackIn_67_3) {
                                            stackIn_67_0 = (String[]) ((Object) stackIn_67_0);

                                            stackIn_70_2 = tb.field_G[var8];
                                            break L15;
                                          } else {
                                            stackIn_67_0 = (String[]) ((Object) stackIn_67_0);

                                            stackIn_70_2 = "";
                                            break L15;
                                          }
                                        }
                                        stackIn_67_0[stackIn_67_1] = stackIn_70_2;
                                        var8++;
                                        continue L14;
                                      }
                                    }
                                  } else {
                                    L16: {
                                      stackIn_62_0 = s.field_I;

                                      stackIn_62_1 = 6 - -var8;

                                      if ((qm.field_f.length + var8 + -var5 ^ -1) > -1) {
                                        stackIn_63_0 = (String[]) ((Object) stackIn_62_0);
                                        stackIn_63_1 = stackIn_62_1;
                                        stackIn_63_2 = "";
                                        break L16;
                                      } else {
                                        stackIn_63_0 = (String[]) ((Object) stackIn_62_0);
                                        stackIn_63_1 = stackIn_62_1;
                                        stackIn_63_2 = qm.field_f[-var5 + (qm.field_f.length + var8)];
                                        break L16;
                                      }
                                    }
                                    stackIn_63_0[stackIn_63_1] = stackIn_63_2;
                                    var8++;
                                    continue L13;
                                  }
                                }
                              } else {
                                jj.field_a[var8] = -1;
                                var8++;
                                continue L12;
                              }
                            }
                          }
                        }
                      }
                    }
                  } else {
                    if ((f.field_j ^ -1) == -4) {
                      L17: {
                        if (!g.field_r.field_f) {
                          var3_int = un.a(pf.field_e, param1 ^ 114, nb.field_a, sb.field_h, tj.field_k);
                          break L17;
                        } else {
                          var3_int = un.a(pf.field_e, -122, nb.field_a, sb.field_h, aj.field_e);
                          break L17;
                        }
                      }
                      var4 = 2 + var3_int;
                      s.field_I = new String[var4];
                      jj.field_a = new int[var4];
                      var5 = 0;
                      L18: while (true) {
                        if (var4 <= var5) {
                          wh.field_j = new int[1];
                          var5 = 0;
                          L19: while (true) {
                            if (var3_int <= var5) {
                              s.field_I[-2 + var4] = "";
                              s.field_I[-1 + var4] = ah.field_k;
                              jj.field_a[var4 + -1] = 0;
                              wh.field_j[0] = 5;
                              break L1;
                            } else {
                              s.field_I[var5] = pf.field_e[var5];
                              var5++;
                              continue L19;
                            }
                          }
                        } else {
                          jj.field_a[var5] = -1;
                          var5++;
                          continue L18;
                        }
                      }
                    } else {
                      if (f.field_j == 4) {
                        var3_int = un.a(pf.field_e, -110, nb.field_a, sb.field_h, ol.field_K);
                        var4 = var3_int + 2;
                        s.field_I = new String[var4];
                        jj.field_a = new int[var4];
                        var5 = 0;
                        L20: while (true) {
                          if (var4 <= var5) {
                            wh.field_j = new int[1];
                            var5 = 0;
                            L21: while (true) {
                              if (var3_int <= var5) {
                                s.field_I[var4 - 2] = "";
                                s.field_I[-1 + var4] = ah.field_k;
                                jj.field_a[var4 + -1] = 0;
                                wh.field_j[0] = 5;
                                break L1;
                              } else {
                                s.field_I[var5] = pf.field_e[var5];
                                var5++;
                                continue L21;
                              }
                            }
                          } else {
                            jj.field_a[var5] = -1;
                            var5++;
                            continue L20;
                          }
                        }
                      } else {
                        if (5 != f.field_j) {
                          throw new IllegalArgumentException();
                        } else {
                          var3_int = un.a(pf.field_e, 122, nb.field_a, sb.field_h, eg.field_a);
                          var4 = 3 + var3_int;
                          s.field_I = new String[var4];
                          jj.field_a = new int[var4];
                          var5 = 0;
                          L22: while (true) {
                            if (var4 <= var5) {
                              wh.field_j = new int[2];
                              var5 = 0;
                              L23: while (true) {
                                if (var5 >= var3_int) {
                                  s.field_I[var4 + -3] = "";
                                  s.field_I[-2 + var4] = ve.field_f;
                                  jj.field_a[var4 - 2] = 0;
                                  wh.field_j[0] = 3;
                                  s.field_I[-1 + var4] = ah.field_k;
                                  jj.field_a[-1 + var4] = 1;
                                  wh.field_j[1] = 5;
                                  break L1;
                                } else {
                                  s.field_I[var5] = pf.field_e[var5];
                                  var5++;
                                  continue L23;
                                }
                              }
                            } else {
                              jj.field_a[var5] = -1;
                              var5++;
                              continue L22;
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
            cb.field_c.field_k = wh.field_j.length;
            var3_int = 0;
            var4 = 0;
            L24: while (true) {
              if (var4 >= s.field_I.length) {
                L25: {
                  if (2 != f.field_j) {
                    break L25;
                  } else {
                    var10 = tg.field_nb;
                    var4_ref_String__ = var10;
                    var5 = 0;
                    L26: while (true) {
                      if (var5 >= var10.length) {
                        var4_ref_String__ = nn.field_i;
                        var5 = 0;
                        L27: while (true) {
                          if (var5 >= var4_ref_String__.length) {
                            break L25;
                          } else {
                            L28: {
                              var12 = var4_ref_String__[var5];
                              var7 = ee.a(-104, var12, false);
                              if ((var7 ^ -1) < (var3_int ^ -1)) {
                                var3_int = var7;
                                break L28;
                              } else {
                                break L28;
                              }
                            }
                            var5++;
                            continue L27;
                          }
                        }
                      } else {
                        L29: {
                          var11 = var10[var5];
                          var7 = ee.a(param1 ^ -92, var11, false);
                          if (var3_int < var7) {
                            var3_int = var7;
                            break L29;
                          } else {
                            break L29;
                          }
                        }
                        var5++;
                        continue L26;
                      }
                    }
                  }
                }
                vi.field_a = -(var3_int >> -1566681567) + rn.field_e - -var3_int;
                ia.field_x = rn.field_e - (var3_int >> 1399245313);
                mh.field_f = cb.field_c.field_k * (hn.field_s + en.field_k << 803580577);
                var4 = param1;
                L30: while (true) {
                  if (s.field_I.length <= var4) {
                    hi.field_o = ng.field_nb + -(mh.field_f >> -1728702687);
                    cj.field_d = new int[s.field_I.length][];
                    var4 = 0;
                    var5 = hi.field_o;
                    L31: while (true) {
                      if (s.field_I.length <= var4) {
                        L32: {
                          if (2 != f.field_j) {
                            cb.field_c.a(param1 ^ 90, param2, hk.a((byte) 122, dh.field_d, bj.field_y), 0);
                            break L32;
                          } else {
                            cb.field_c.a(param1 + 91, param2, -1, -1);
                            break L32;
                          }
                        }
                        break L0;
                      } else {
                        L33: {
                          var6 = jj.field_a[var4];
                          if (-1 >= (var6 ^ -1)) {
                            var7 = ee.a(param1 ^ 61, s.field_I[var4], true);
                            var5 = var5 + hn.field_s;
                            var8 = rn.field_e + -(var7 >> 866689921);
                            cj.field_d[var4] = new int[4];
                            cj.field_d[var4][0] = var8 - fe.field_k;
                            cj.field_d[var4][1] = var5;
                            cj.field_d[var4][2] = (fe.field_k << -887181823) + var7;
                            cj.field_d[var4][3] = (en.field_k << -926682623) + ok.field_a;
                            var5 = var5 + (hn.field_s + (en.field_k << -425318559) + ok.field_a);
                            break L33;
                          } else {
                            var5 = var5 + je.field_b;
                            break L33;
                          }
                        }
                        var4++;
                        continue L31;
                      }
                    }
                  } else {
                    L34: {
                      stackIn_112_0 = mh.field_f;

                      if ((jj.field_a[var4] ^ -1) > -1) {
                        stackIn_113_0 = stackIn_112_0;
                        stackIn_113_1 = je.field_b;
                        break L34;
                      } else {
                        stackIn_113_0 = stackIn_112_0;
                        stackIn_113_1 = ok.field_a;
                        break L34;
                      }
                    }
                    mh.field_f = stackIn_113_0 + stackIn_113_1;
                    var4++;
                    continue L30;
                  }
                }
              } else {
                L35: {
                  stackIn_90_0 = 25;

                  stackIn_90_1 = s.field_I[var4];

                  if (0 > jj.field_a[var4]) {
                    stackIn_91_0 = stackIn_90_0;
                    stackIn_91_1 = (String) ((Object) stackIn_90_1);
                    stackIn_91_2 = 0;
                    break L35;
                  } else {
                    stackIn_91_0 = stackIn_90_0;
                    stackIn_91_1 = (String) ((Object) stackIn_90_1);
                    stackIn_91_2 = 1;
                    break L35;
                  }
                }
                L36: {
                  var5 = ee.a(stackIn_91_0, stackIn_91_1, stackIn_91_2 != 0);
                  if (var3_int < var5) {
                    var3_int = var5;
                    break L36;
                  } else {
                    break L36;
                  }
                }
                var4++;
                continue L24;
              }
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          var3 = decompiledCaughtException;
          throw t.a((Throwable) ((Object) var3), "sn.E(" + param0 + ',' + param1 + ',' + param2 + ')');
        }
    }

    final boolean b(int param0) {
        if (param0 <= 75) {
            return true;
        }
        return this.field_e;
    }

    sn(String param0) {
        this(param0, false);
    }

    sn(String param0, boolean param1) {
        RuntimeException runtimeException = null;
        Object stackIn_3_0 = null;
        Object stackIn_4_0 = null;
        int stackIn_4_1 = 0;
        RuntimeException stackIn_11_0 = null;
        StringBuilder stackIn_11_1 = null;
        RuntimeException stackIn_12_0 = null;
        StringBuilder stackIn_12_1 = null;
        String stackIn_12_2 = null;
        RuntimeException decompiledCaughtException = null;
        try {
          L0: {
            L1: {
              this.field_a = param0;
              stackIn_3_0 = this;

              if (!param1) {
                stackIn_4_0 = this;
                stackIn_4_1 = 0;
                break L1;
              } else {
                stackIn_4_0 = this;
                stackIn_4_1 = 1;
                break L1;
              }
            }
            L2: {
              ((sn) (this)).field_e = stackIn_4_1 != 0;
              if (this.field_a != null) {
                break L2;
              } else {
                this.field_a = "";
                break L2;
              }
            }
            L3: {
              if (this.field_a.length() != 0) {
                break L3;
              } else {
                this.field_e = false;
                break L3;
              }
            }
            break L0;
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L4: {
            runtimeException = decompiledCaughtException;
            stackIn_11_0 = (RuntimeException) (runtimeException);

            stackIn_11_1 = new StringBuilder().append("sn.<init>(");

            if (param0 == null) {
              stackIn_12_0 = (RuntimeException) ((Object) stackIn_11_0);
              stackIn_12_1 = (StringBuilder) ((Object) stackIn_11_1);
              stackIn_12_2 = "null";
              break L4;
            } else {
              stackIn_12_0 = (RuntimeException) ((Object) stackIn_11_0);
              stackIn_12_1 = (StringBuilder) ((Object) stackIn_11_1);
              stackIn_12_2 = "{...}";
              break L4;
            }
          }
          throw t.a((Throwable) ((Object) stackIn_12_0), stackIn_12_2 + ',' + param1 + ')');
        }
    }

    final String a(int param0) {
        if (param0 > -67) {
            sn.a(119, -87, true);
        }
        return this.field_a;
    }

    static {
        field_d = 20000000L;
        field_b = new boolean[64];
    }
}
