/*
 * Decompiled by CFR-JS 0.4.0.
 */
final class wu implements mfa {
    static jta field_a;

    final static boolean a(int param0, int param1, int param2) {
        return 0 != (2048 & param2);
    }

    public final dg a(la param0, kh param1, boolean param2) {
        RuntimeException var4 = null;
        int var4_int = 0;
        int var5 = 0;
        int var6 = 0;
        ria stackIn_32_0 = null;
        ioa stackIn_34_0 = null;
        am stackIn_36_0 = null;
        gaa stackIn_38_0 = null;
        lfa stackIn_40_0 = null;
        boa stackIn_42_0 = null;
        fu stackIn_44_0 = null;
        cv stackIn_46_0 = null;
        kva stackIn_48_0 = null;
        dc stackIn_50_0 = null;
        kga stackIn_52_0 = null;
        oo stackIn_54_0 = null;
        ki stackIn_56_0 = null;
        lma stackIn_58_0 = null;
        nq stackIn_60_0 = null;
        ce stackIn_62_0 = null;
        aka stackIn_64_0 = null;
        vsa stackIn_66_0 = null;
        pqa stackIn_68_0 = null;
        ao stackIn_70_0 = null;
        RuntimeException stackIn_72_0 = null;
        StringBuilder stackIn_72_1 = null;
        RuntimeException stackIn_73_0 = null;
        StringBuilder stackIn_73_1 = null;
        RuntimeException stackIn_74_0 = null;
        StringBuilder stackIn_74_1 = null;
        String stackIn_74_2 = null;
        RuntimeException stackIn_75_0 = null;
        StringBuilder stackIn_75_1 = null;
        RuntimeException stackIn_76_0 = null;
        StringBuilder stackIn_76_1 = null;
        RuntimeException stackIn_77_0 = null;
        StringBuilder stackIn_77_1 = null;
        String stackIn_77_2 = null;
        RuntimeException decompiledCaughtException = null;
        ria stackOut_31_0 = null;
        ioa stackOut_33_0 = null;
        boa stackOut_41_0 = null;
        fu stackOut_43_0 = null;
        kva stackOut_47_0 = null;
        kga stackOut_51_0 = null;
        oo stackOut_53_0 = null;
        lma stackOut_57_0 = null;
        nq stackOut_59_0 = null;
        vsa stackOut_65_0 = null;
        pqa stackOut_67_0 = null;
        ao stackOut_69_0 = null;
        aka stackOut_63_0 = null;
        ce stackOut_61_0 = null;
        ki stackOut_55_0 = null;
        dc stackOut_49_0 = null;
        cv stackOut_45_0 = null;
        lfa stackOut_39_0 = null;
        gaa stackOut_37_0 = null;
        am stackOut_35_0 = null;
        RuntimeException stackOut_71_0 = null;
        StringBuilder stackOut_71_1 = null;
        RuntimeException stackOut_73_0 = null;
        StringBuilder stackOut_73_1 = null;
        String stackOut_73_2 = null;
        RuntimeException stackOut_72_0 = null;
        StringBuilder stackOut_72_1 = null;
        String stackOut_72_2 = null;
        RuntimeException stackOut_74_0 = null;
        StringBuilder stackOut_74_1 = null;
        RuntimeException stackOut_76_0 = null;
        StringBuilder stackOut_76_1 = null;
        String stackOut_76_2 = null;
        RuntimeException stackOut_75_0 = null;
        StringBuilder stackOut_75_1 = null;
        String stackOut_75_2 = null;
        var6 = TombRacer.field_G ? 1 : 0;
        try {
          L0: {
            L1: {
              var4_int = param1.b((byte) 44, 6);
              if (!param2) {
                break L1;
              } else {
                field_a = null;
                break L1;
              }
            }
            var5 = var4_int;
            if (1 == var5) {
              stackOut_31_0 = new ria(var4_int, param0, param1);
              stackIn_32_0 = stackOut_31_0;
              return (dg) (Object) stackIn_32_0;
            } else {
              if (3 == var5) {
                stackOut_33_0 = new ioa(var4_int, param0, param1);
                stackIn_34_0 = stackOut_33_0;
                return (dg) (Object) stackIn_34_0;
              } else {
                if (var5 != 4) {
                  if (5 != var5) {
                    if (var5 != 6) {
                      if (var5 == 7) {
                        stackOut_41_0 = new boa(var4_int, param0, param1);
                        stackIn_42_0 = stackOut_41_0;
                        return (dg) (Object) stackIn_42_0;
                      } else {
                        if (var5 == 8) {
                          stackOut_43_0 = new fu(var4_int, param0, param1);
                          stackIn_44_0 = stackOut_43_0;
                          return (dg) (Object) stackIn_44_0;
                        } else {
                          if (9 != var5) {
                            if (10 == var5) {
                              stackOut_47_0 = new kva(var4_int, param0, param1);
                              stackIn_48_0 = stackOut_47_0;
                              return (dg) (Object) stackIn_48_0;
                            } else {
                              if (var5 != 11) {
                                if (var5 == 12) {
                                  stackOut_51_0 = new kga(var4_int, param0, param1);
                                  stackIn_52_0 = stackOut_51_0;
                                  return (dg) (Object) stackIn_52_0;
                                } else {
                                  if (var5 == 13) {
                                    stackOut_53_0 = new oo(var4_int, param0, param1);
                                    stackIn_54_0 = stackOut_53_0;
                                    return (dg) (Object) stackIn_54_0;
                                  } else {
                                    if (var5 != 14) {
                                      if (var5 == 15) {
                                        stackOut_57_0 = new lma(var4_int, param0, param1);
                                        stackIn_58_0 = stackOut_57_0;
                                        return (dg) (Object) stackIn_58_0;
                                      } else {
                                        if (16 == var5) {
                                          stackOut_59_0 = new nq(var4_int, param0, param1);
                                          stackIn_60_0 = stackOut_59_0;
                                          return (dg) (Object) stackIn_60_0;
                                        } else {
                                          if (17 != var5) {
                                            if (var5 != 18) {
                                              if (19 == var5) {
                                                stackOut_65_0 = new vsa(var4_int, param0, param1);
                                                stackIn_66_0 = stackOut_65_0;
                                                return (dg) (Object) stackIn_66_0;
                                              } else {
                                                if (var5 == 20) {
                                                  stackOut_67_0 = new pqa(var4_int, param0, param1);
                                                  stackIn_68_0 = stackOut_67_0;
                                                  return (dg) (Object) stackIn_68_0;
                                                } else {
                                                  stackOut_69_0 = new ao(var4_int, param0, param1);
                                                  stackIn_70_0 = stackOut_69_0;
                                                  break L0;
                                                }
                                              }
                                            } else {
                                              stackOut_63_0 = new aka(var4_int, param0, param1);
                                              stackIn_64_0 = stackOut_63_0;
                                              return (dg) (Object) stackIn_64_0;
                                            }
                                          } else {
                                            stackOut_61_0 = new ce(var4_int, param0, param1);
                                            stackIn_62_0 = stackOut_61_0;
                                            return (dg) (Object) stackIn_62_0;
                                          }
                                        }
                                      }
                                    } else {
                                      stackOut_55_0 = new ki(var4_int, param0, param1);
                                      stackIn_56_0 = stackOut_55_0;
                                      return (dg) (Object) stackIn_56_0;
                                    }
                                  }
                                }
                              } else {
                                stackOut_49_0 = new dc(var4_int, param0, param1);
                                stackIn_50_0 = stackOut_49_0;
                                return (dg) (Object) stackIn_50_0;
                              }
                            }
                          } else {
                            stackOut_45_0 = new cv(var4_int, param0, param1);
                            stackIn_46_0 = stackOut_45_0;
                            return (dg) (Object) stackIn_46_0;
                          }
                        }
                      }
                    } else {
                      stackOut_39_0 = new lfa(var4_int, param0, param1);
                      stackIn_40_0 = stackOut_39_0;
                      return (dg) (Object) stackIn_40_0;
                    }
                  } else {
                    stackOut_37_0 = new gaa(var4_int, param0, param1);
                    stackIn_38_0 = stackOut_37_0;
                    return (dg) (Object) stackIn_38_0;
                  }
                } else {
                  stackOut_35_0 = new am(var4_int, param0, param1);
                  stackIn_36_0 = stackOut_35_0;
                  return (dg) (Object) stackIn_36_0;
                }
              }
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L2: {
            var4 = decompiledCaughtException;
            stackOut_71_0 = (RuntimeException) var4;
            stackOut_71_1 = new StringBuilder().append("wu.A(");
            stackIn_73_0 = stackOut_71_0;
            stackIn_73_1 = stackOut_71_1;
            stackIn_72_0 = stackOut_71_0;
            stackIn_72_1 = stackOut_71_1;
            if (param0 == null) {
              stackOut_73_0 = (RuntimeException) (Object) stackIn_73_0;
              stackOut_73_1 = (StringBuilder) (Object) stackIn_73_1;
              stackOut_73_2 = "null";
              stackIn_74_0 = stackOut_73_0;
              stackIn_74_1 = stackOut_73_1;
              stackIn_74_2 = stackOut_73_2;
              break L2;
            } else {
              stackOut_72_0 = (RuntimeException) (Object) stackIn_72_0;
              stackOut_72_1 = (StringBuilder) (Object) stackIn_72_1;
              stackOut_72_2 = "{...}";
              stackIn_74_0 = stackOut_72_0;
              stackIn_74_1 = stackOut_72_1;
              stackIn_74_2 = stackOut_72_2;
              break L2;
            }
          }
          L3: {
            stackOut_74_0 = (RuntimeException) (Object) stackIn_74_0;
            stackOut_74_1 = ((StringBuilder) (Object) stackIn_74_1).append(stackIn_74_2).append(',');
            stackIn_76_0 = stackOut_74_0;
            stackIn_76_1 = stackOut_74_1;
            stackIn_75_0 = stackOut_74_0;
            stackIn_75_1 = stackOut_74_1;
            if (param1 == null) {
              stackOut_76_0 = (RuntimeException) (Object) stackIn_76_0;
              stackOut_76_1 = (StringBuilder) (Object) stackIn_76_1;
              stackOut_76_2 = "null";
              stackIn_77_0 = stackOut_76_0;
              stackIn_77_1 = stackOut_76_1;
              stackIn_77_2 = stackOut_76_2;
              break L3;
            } else {
              stackOut_75_0 = (RuntimeException) (Object) stackIn_75_0;
              stackOut_75_1 = (StringBuilder) (Object) stackIn_75_1;
              stackOut_75_2 = "{...}";
              stackIn_77_0 = stackOut_75_0;
              stackIn_77_1 = stackOut_75_1;
              stackIn_77_2 = stackOut_75_2;
              break L3;
            }
          }
          throw tba.a((Throwable) (Object) stackIn_77_0, stackIn_77_2 + ',' + param2 + ')');
        }
        return (dg) (Object) stackIn_70_0;
    }

    public final void a(int param0, kh param1, dg param2) {
        try {
            if (param0 != 15682) {
                field_a = null;
            }
            param1.a((byte) -128, param2.h((byte) 81), 6);
            param2.a((byte) 88, param1);
        } catch (RuntimeException runtimeException) {
            throw tba.a((Throwable) (Object) runtimeException, "wu.B(" + param0 + ',' + (param1 != null ? "{...}" : "null") + ',' + (param2 != null ? "{...}" : "null") + ')');
        }
    }

    public final dg a(int param0, int param1) {
        int var3 = 0;
        int var4 = 0;
        Object var5 = null;
        var4 = TombRacer.field_G ? 1 : 0;
        if (param1 >= 73) {
          var3 = param0;
          if (var3 != 1) {
            if (var3 != 3) {
              if (var3 != 4) {
                if (var3 != 5) {
                  if (var3 != 6) {
                    if (var3 == 7) {
                      return (dg) (Object) new boa(param0);
                    } else {
                      if (var3 == 8) {
                        return (dg) (Object) new fu(param0);
                      } else {
                        if (var3 != 9) {
                          if (10 == var3) {
                            return (dg) (Object) new kva(param0);
                          } else {
                            if (11 == var3) {
                              return (dg) (Object) new dc(param0);
                            } else {
                              if (var3 != 12) {
                                if (var3 == 13) {
                                  return (dg) (Object) new oo(param0);
                                } else {
                                  if (var3 == 14) {
                                    return (dg) (Object) new ki(param0);
                                  } else {
                                    if (var3 == 15) {
                                      return (dg) (Object) new lma(param0);
                                    } else {
                                      if (var3 == 16) {
                                        return (dg) (Object) new nq(param0);
                                      } else {
                                        if (var3 != 17) {
                                          if (var3 == 18) {
                                            return (dg) (Object) new aka(param0);
                                          } else {
                                            if (var3 == 19) {
                                              return (dg) (Object) new vsa(param0);
                                            } else {
                                              if (var3 != 20) {
                                                return (dg) (Object) new ao(param0);
                                              } else {
                                                return (dg) (Object) new pqa(param0);
                                              }
                                            }
                                          }
                                        } else {
                                          return (dg) (Object) new ce(param0);
                                        }
                                      }
                                    }
                                  }
                                }
                              } else {
                                return (dg) (Object) new kga(param0);
                              }
                            }
                          }
                        } else {
                          return (dg) (Object) new cv(param0);
                        }
                      }
                    }
                  } else {
                    return (dg) (Object) new lfa(param0);
                  }
                } else {
                  return (dg) (Object) new gaa(param0);
                }
              } else {
                return (dg) (Object) new am(param0);
              }
            } else {
              return (dg) (Object) new ioa(param0);
            }
          } else {
            return (dg) (Object) new ria(param0);
          }
        } else {
          var5 = null;
          dg discarded$1 = ((wu) this).a((la) null, (kh) null, false);
          var3 = param0;
          if (var3 != 1) {
            if (var3 != 3) {
              if (var3 != 4) {
                if (var3 != 5) {
                  if (var3 != 6) {
                    if (var3 == 7) {
                      return (dg) (Object) new boa(param0);
                    } else {
                      if (var3 == 8) {
                        return (dg) (Object) new fu(param0);
                      } else {
                        if (var3 != 9) {
                          if (10 == var3) {
                            return (dg) (Object) new kva(param0);
                          } else {
                            if (11 == var3) {
                              return (dg) (Object) new dc(param0);
                            } else {
                              if (var3 != 12) {
                                if (var3 == 13) {
                                  return (dg) (Object) new oo(param0);
                                } else {
                                  if (var3 == 14) {
                                    return (dg) (Object) new ki(param0);
                                  } else {
                                    if (var3 == 15) {
                                      return (dg) (Object) new lma(param0);
                                    } else {
                                      if (var3 == 16) {
                                        return (dg) (Object) new nq(param0);
                                      } else {
                                        if (var3 != 17) {
                                          if (var3 == 18) {
                                            return (dg) (Object) new aka(param0);
                                          } else {
                                            if (var3 == 19) {
                                              return (dg) (Object) new vsa(param0);
                                            } else {
                                              if (var3 != 20) {
                                                return (dg) (Object) new ao(param0);
                                              } else {
                                                return (dg) (Object) new pqa(param0);
                                              }
                                            }
                                          }
                                        } else {
                                          return (dg) (Object) new ce(param0);
                                        }
                                      }
                                    }
                                  }
                                }
                              } else {
                                return (dg) (Object) new kga(param0);
                              }
                            }
                          }
                        } else {
                          return (dg) (Object) new cv(param0);
                        }
                      }
                    }
                  } else {
                    return (dg) (Object) new lfa(param0);
                  }
                } else {
                  return (dg) (Object) new gaa(param0);
                }
              } else {
                return (dg) (Object) new am(param0);
              }
            } else {
              return (dg) (Object) new ioa(param0);
            }
          } else {
            return (dg) (Object) new ria(param0);
          }
        }
    }

    public static void a() {
        field_a = null;
    }

    public wu() {
        int var1 = 0;
        bb.field_g = new String[21];
        for (var1 = 0; var1 < 21; var1++) {
            bb.field_g[var1] = "(unused, placeholder)";
        }
        bb.field_g[8] = "Hazard";
        bb.field_g[9] = "Switch";
        bb.field_g[19] = "Bridge";
        bb.field_g[13] = "SnakePit";
        bb.field_g[18] = "Monster";
        bb.field_g[0] = "OLD directrender";
        bb.field_g[17] = "Platform";
        bb.field_g[15] = "Wall";
        bb.field_g[3] = "OLD switch";
        bb.field_g[6] = "Doorway";
        bb.field_g[14] = "Plant";
        bb.field_g[12] = "Item";
        bb.field_g[4] = "Furniture";
        bb.field_g[5] = "Light Source";
        bb.field_g[16] = "Trap Base";
        bb.field_g[1] = "TEMP model test";
        bb.field_g[7] = "Block";
        bb.field_g[10] = "Pit";
        bb.field_g[11] = "Crusher";
        bb.field_g[20] = "Gas Vent";
    }

    public final dg a(gr param0, gma param1, int param2) {
        RuntimeException var4 = null;
        int var4_int = 0;
        dg var5 = null;
        int var6 = 0;
        Object stackIn_31_0 = null;
        dg stackIn_33_0 = null;
        RuntimeException stackIn_35_0 = null;
        StringBuilder stackIn_35_1 = null;
        RuntimeException stackIn_36_0 = null;
        StringBuilder stackIn_36_1 = null;
        RuntimeException stackIn_37_0 = null;
        StringBuilder stackIn_37_1 = null;
        String stackIn_37_2 = null;
        RuntimeException stackIn_38_0 = null;
        StringBuilder stackIn_38_1 = null;
        RuntimeException stackIn_39_0 = null;
        StringBuilder stackIn_39_1 = null;
        RuntimeException stackIn_40_0 = null;
        StringBuilder stackIn_40_1 = null;
        String stackIn_40_2 = null;
        RuntimeException decompiledCaughtException = null;
        dg stackOut_32_0 = null;
        Object stackOut_30_0 = null;
        RuntimeException stackOut_34_0 = null;
        StringBuilder stackOut_34_1 = null;
        RuntimeException stackOut_36_0 = null;
        StringBuilder stackOut_36_1 = null;
        String stackOut_36_2 = null;
        RuntimeException stackOut_35_0 = null;
        StringBuilder stackOut_35_1 = null;
        String stackOut_35_2 = null;
        RuntimeException stackOut_37_0 = null;
        StringBuilder stackOut_37_1 = null;
        RuntimeException stackOut_39_0 = null;
        StringBuilder stackOut_39_1 = null;
        String stackOut_39_2 = null;
        RuntimeException stackOut_38_0 = null;
        StringBuilder stackOut_38_1 = null;
        String stackOut_38_2 = null;
        var6 = TombRacer.field_G ? 1 : 0;
        try {
          L0: {
            L1: {
              var4_int = 0;
              if (!(param1 instanceof db)) {
                if (!(param1 instanceof fsa)) {
                  break L1;
                } else {
                  if (param0 instanceof sea) {
                    var4_int = 13;
                    break L1;
                  } else {
                    if (param0 instanceof mo) {
                      var4_int = 10;
                      break L1;
                    } else {
                      if (param0 instanceof rea) {
                        var4_int = 11;
                        break L1;
                      } else {
                        if (!(param0 instanceof pd)) {
                          if (!(param0 instanceof mra)) {
                            L2: {
                              if (!(param0 instanceof eq)) {
                                break L2;
                              } else {
                                if (!((fsa) (Object) param1).d(true)) {
                                  break L2;
                                } else {
                                  var4_int = 16;
                                  break L1;
                                }
                              }
                            }
                            L3: {
                              if (param0 instanceof rh) {
                                break L3;
                              } else {
                                if (param0 instanceof dja) {
                                  break L3;
                                } else {
                                  L4: {
                                    if (!(param0 instanceof ut)) {
                                      break L4;
                                    } else {
                                      if (param0 instanceof kj) {
                                        break L4;
                                      } else {
                                        if (!(param0 instanceof wh)) {
                                          var4_int = 3;
                                          break L1;
                                        } else {
                                          break L4;
                                        }
                                      }
                                    }
                                  }
                                  L5: {
                                    if (param0 instanceof lpa) {
                                      break L5;
                                    } else {
                                      if (param0 instanceof im) {
                                        break L5;
                                      } else {
                                        var4_int = 0;
                                        break L1;
                                      }
                                    }
                                  }
                                  var4_int = 17;
                                  break L1;
                                }
                              }
                            }
                            var4_int = 9;
                            break L1;
                          } else {
                            var4_int = 7;
                            break L1;
                          }
                        } else {
                          var4_int = 12;
                          break L1;
                        }
                      }
                    }
                  }
                }
              } else {
                var4_int = 15;
                break L1;
              }
            }
            if (param2 < -87) {
              var5 = ((wu) this).a(var4_int, 118);
              var5.a(param1, -2159, param0);
              stackOut_32_0 = (dg) var5;
              stackIn_33_0 = stackOut_32_0;
              break L0;
            } else {
              stackOut_30_0 = null;
              stackIn_31_0 = stackOut_30_0;
              return (dg) (Object) stackIn_31_0;
            }
          }
        } catch (java.lang.RuntimeException decompiledCaughtParameter0) {
          decompiledCaughtException = decompiledCaughtParameter0;
          L6: {
            var4 = decompiledCaughtException;
            stackOut_34_0 = (RuntimeException) var4;
            stackOut_34_1 = new StringBuilder().append("wu.F(");
            stackIn_36_0 = stackOut_34_0;
            stackIn_36_1 = stackOut_34_1;
            stackIn_35_0 = stackOut_34_0;
            stackIn_35_1 = stackOut_34_1;
            if (param0 == null) {
              stackOut_36_0 = (RuntimeException) (Object) stackIn_36_0;
              stackOut_36_1 = (StringBuilder) (Object) stackIn_36_1;
              stackOut_36_2 = "null";
              stackIn_37_0 = stackOut_36_0;
              stackIn_37_1 = stackOut_36_1;
              stackIn_37_2 = stackOut_36_2;
              break L6;
            } else {
              stackOut_35_0 = (RuntimeException) (Object) stackIn_35_0;
              stackOut_35_1 = (StringBuilder) (Object) stackIn_35_1;
              stackOut_35_2 = "{...}";
              stackIn_37_0 = stackOut_35_0;
              stackIn_37_1 = stackOut_35_1;
              stackIn_37_2 = stackOut_35_2;
              break L6;
            }
          }
          L7: {
            stackOut_37_0 = (RuntimeException) (Object) stackIn_37_0;
            stackOut_37_1 = ((StringBuilder) (Object) stackIn_37_1).append(stackIn_37_2).append(',');
            stackIn_39_0 = stackOut_37_0;
            stackIn_39_1 = stackOut_37_1;
            stackIn_38_0 = stackOut_37_0;
            stackIn_38_1 = stackOut_37_1;
            if (param1 == null) {
              stackOut_39_0 = (RuntimeException) (Object) stackIn_39_0;
              stackOut_39_1 = (StringBuilder) (Object) stackIn_39_1;
              stackOut_39_2 = "null";
              stackIn_40_0 = stackOut_39_0;
              stackIn_40_1 = stackOut_39_1;
              stackIn_40_2 = stackOut_39_2;
              break L7;
            } else {
              stackOut_38_0 = (RuntimeException) (Object) stackIn_38_0;
              stackOut_38_1 = (StringBuilder) (Object) stackIn_38_1;
              stackOut_38_2 = "{...}";
              stackIn_40_0 = stackOut_38_0;
              stackIn_40_1 = stackOut_38_1;
              stackIn_40_2 = stackOut_38_2;
              break L7;
            }
          }
          throw tba.a((Throwable) (Object) stackIn_40_0, stackIn_40_2 + ',' + param2 + ')');
        }
        return stackIn_33_0;
    }

    static {
    }
}
